 ...
 * histogram that hyp_extract_file() produces for the shared fixture (e.g.
 ...
 * golden is stable. Labels come from HYPDefinition.label assigned during
...
static void label_histogram(HYPFileResult *r, char *out, size_t out_sz) {
...
TEST(grammar_label_goldens) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        HYPFileResult *r =
            hyp_extract_file(c->src, (int)strlen(c->src), c->lang, "lbl", c->path, 0, NULL, NULL);
        if (!r) {
...
static int non_module_defs(HYPFileResult *r) {
    int n = 0;
    for (int i = 0; i < r->defs.count; i++) {
...
TEST(grammar_code_extracts_defs) {
    ...
    for (int k = 0; MUST_EXTRACT_DEFS[k]; k++) {
        ...
        for (int i = 0; i < (int)HYP_GRAMMAR_CASES_COUNT; i++) {
            if (strcmp(HYP_GRAMMAR_CASES[i].name, name) == 0) {
                c = &HYP_GRAMMAR_CASES[i];
                break;
            }
        }
        if (!c) {
            fprintf(stderr, "  [CODE-DEFS] FAIL %-14s — no fixture in HYP_GRAMMAR_CASES\n", name);
            failures++;
            continue;
        }
        HYPFileResult *r =
            hyp_extract_file(c->src, (int)strlen(c->src), c->lang, "cd", c->path, 0, NULL, NULL);
        if (!r) {
