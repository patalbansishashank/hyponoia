...
TEST(usage_semantic_reference_candidate_trusts_marked_producer) {
    HYPUsage usage = {0};
    usage.kind = HYP_USAGE_VALUE;
    usage.may_be_call_reference = true;
    ASSERT_TRUE(hyp_pipeline_usage_semantic_reference_candidate(&usage));

    usage.may_be_call_reference = false;
    ASSERT_FALSE(hyp_pipeline_usage_semantic_reference_candidate(&usage));

    usage.kind = HYP_USAGE_CALL_REFERENCE;
    ASSERT_TRUE(hyp_pipeline_usage_semantic_reference_candidate(&usage));
...
static int setup_parallel_repo(void) {
    snprintf(g_par_tmpdir, sizeof(g_par_tmpdir), "/tmp/hyp_par_XXXXXX");
    if (!hyp_mkdtemp(g_par_tmpdir))
        return -1;
...
static void seed_test_file_nodes(hyp_gbuf_t *gbuf, const char *project,
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        char *file_qn = hyp_pipeline_fqn_compute(project, rel, "__file__");
        const char *slash = strrchr(rel, '/');
        const char *basename = slash ? slash + 1 : rel;
        if (file_qn) {
            hyp_gbuf_upsert_node(gbuf, "File", basename, file_qn, rel, 0, 0, "{}");
        }
        free(file_qn);
    }
}
...
static void harness_ctx_free_tables(hyp_pipeline_ctx_t *ctx) {
...
static hyp_gbuf_t *run_sequential(const char *project, const char *repo_path,
                                  hyp_file_info_t *files, int file_count) {
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, repo_path);
    ...
    hyp_init();
    hyp_pipeline_pass_definitions(&ctx, files, file_count);
    hyp_pipeline_pass_calls(&ctx, files, file_count);
    hyp_pipeline_pass_usages(&ctx, files, file_count);
    hyp_pipeline_pass_semantic(&ctx, files, file_count);
...
typedef void (*parallel_result_mutator_fn)(HYPFileResult **result_cache, int file_count, void *ud);
...
static hyp_gbuf_t *run_sequential_with_lsp_cross_and_mutator(
    const char *project, const char *repo_path, hyp_file_info_t *files, int file_count,
    parallel_result_mutator_fn mutator, void *mutator_ud, bool seed_structure) {
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, repo_path);
    hyp_registry_t *reg = hyp_registry_new();
    HYPFileResult **cache = (HYPFileResult **)calloc((size_t)file_count, sizeof(HYPFileResult *));
    if (!gbuf || !reg || !cache) {
        hyp_gbuf_free(gbuf);
        hyp_registry_free(reg);
        free(cache);
    ...
    hyp_init();
    hyp_pipeline_pass_definitions(&ctx, files, file_count);
    hyp_pipeline_pass_lsp_cross(&ctx, files, file_count, cache);
    if (mutator) {
    ...
    if (ctx.seq_cross_arena_live) {
        hyp_arena_destroy(&ctx.seq_cross_arena);
...
static hyp_gbuf_t *run_sequential_with_lsp_cross(const char *project, const char *repo_path,
                                                 hyp_file_info_t *files, int file_count) {
...
static hyp_gbuf_t *run_parallel_with_extract_opts_and_mutator(
    const char *project, const char *repo_path, hyp_file_info_t *files, int file_count,
    int worker_count, const hyp_parallel_extract_opts_t *extract_opts,
    parallel_result_mutator_fn mutator, void *mutator_ud, bool seed_structure) {
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, repo_path);
    hyp_registry_t *reg = hyp_registry_new();
    atomic_int cancelled;
    ...
    int64_t gbuf_next = hyp_gbuf_next_id(gbuf);
    atomic_init(&shared_ids, gbuf_next);

    HYPFileResult **result_cache = calloc((size_t)file_count, sizeof(HYPFileResult *));

    hyp_init();
    if (extract_opts) {
        hyp_parallel_extract_ex(&ctx, files, file_count, result_cache, &shared_ids, worker_count,
                                extract_opts);
    } else {
        hyp_parallel_extract(&ctx, files, file_count, result_cache, &shared_ids, worker_count);
    }
    hyp_gbuf_set_next_id(gbuf, atomic_load(&shared_ids));

    if (mutator) {
        mutator(result_cache, file_count, mutator_ud);
    }

    hyp_build_registry_from_cache(&ctx, files, file_count, result_cache);
    int64_t registry_next = hyp_gbuf_next_id(gbuf);
     ...
     * hyp_parallel_resolve where the fused resolve_worker invokes
     * hyp_pxc_run_one(_ts) per file BEFORE materializing CALLS edges. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    int def_count = 0;
    HYPLSPDef *all_defs =
        def_modules ? hyp_pxc_collect_all_defs(result_cache, files, file_count, ctx.project_name,
                                               def_modules, &def_count, NULL)
                    : NULL;
    HYPModuleDefIndex *module_def_index =
        all_defs ? hyp_pxc_build_module_def_index(all_defs, def_count) : NULL;

    hyp_parallel_resolve(&ctx, files, file_count, result_cache, &shared_ids, worker_count, all_defs,
                         def_count, def_modules, module_def_index,
                         NULL /* cross_registries — tests use per-file path */);
    hyp_gbuf_set_next_id(gbuf, atomic_load(&shared_ids));

    hyp_pxc_free_module_def_index(module_def_index);
    free(all_defs);
    if (def_modules) {
        for (int i = 0; i < file_count; i++) {
            free(def_modules[i]);
    ...
    for (int i = 0; i < file_count; i++)
        if (result_cache[i])
            hyp_free_result(result_cache[i]);
    free(result_cache);
...
static hyp_gbuf_t *run_parallel_with_extract_opts(const char *project, const char *repo_path,
                                                  hyp_file_info_t *files, int file_count,
                                                  int worker_count,
                                                  const hyp_parallel_extract_opts_t *extract_opts) {
...
static hyp_gbuf_t *run_parallel(const char *project, const char *repo_path, hyp_file_info_t *files,
...
static hyp_gbuf_t *g_seq_gbuf = NULL;
static hyp_gbuf_t *g_par_gbuf = NULL;
...
static int ensure_parity_setup(void) {
    ...
    int file_count = 0;
    if (hyp_discover(g_par_tmpdir, &opts, &files, &file_count) != 0)
        return -1;
    ...
    hyp_discover_free(files, file_count);
...
static void parity_teardown(void) {
    if (g_seq_gbuf) {
        hyp_gbuf_free(g_seq_gbuf);
        g_seq_gbuf = NULL;
    }
    if (g_par_gbuf) {
        hyp_gbuf_free(g_par_gbuf);
        g_par_gbuf = NULL;
    }
...
TEST(parallel_node_count) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = hyp_gbuf_node_count(g_seq_gbuf);
    int par = hyp_gbuf_node_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
static int assert_edge_type_parity(const char *type) {
    if (ensure_parity_setup() != 0)
        return -1;
    int seq = hyp_gbuf_edge_count_by_type(g_seq_gbuf, type);
    int par = hyp_gbuf_edge_count_by_type(g_par_gbuf, type);
    if (seq != par) {
...
TEST(parallel_semantic_fixture_expected_counts) {
    ...
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_seq_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_par_gbuf, "IMPLEMENTS"), 1);
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_seq_gbuf, "INHERITS"), 1);
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_par_gbuf, "INHERITS"), 1);
    /* Circle.area overrides Shape.area; Base.area overrides Circle.area. */
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_seq_gbuf, "OVERRIDE"), 2);
    ASSERT_EQ(hyp_gbuf_edge_count_by_type(g_par_gbuf, "OVERRIDE"), 2);
    PASS();
}
...
TEST(parallel_total_edges) {
    if (ensure_parity_setup() != 0)
        FAIL("setup failed");
    int seq = hyp_gbuf_edge_count(g_seq_gbuf);
    int par = hyp_gbuf_edge_count(g_par_gbuf);
    ASSERT_GT(seq, 0);
    ASSERT_EQ(seq, par);
    PASS();
}
...
TEST(parallel_empty_files) {
    hyp_gbuf_t *gbuf = hyp_gbuf_new("empty-proj", "/tmp");
    hyp_registry_t *reg = hyp_registry_new();
    atomic_int cancelled;
    atomic_init(&cancelled, 0);
    ...
    HYPFileResult **cache = NULL;
    int rc = hyp_parallel_extract(&ctx, NULL, 0, cache, &shared_ids, 2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(hyp_gbuf_node_count(gbuf), 0);
 ...
 * position exceed the fixed HYP_SZ_2K `props` stack buffer in
...
TEST(parallel_args_json_no_overflow) {
    char dir[256];
    snprintf(dir, sizeof(dir), "/tmp/hyp_argov_XXXXXX");
    ASSERT_TRUE(hyp_mkdtemp(dir) != NULL);
    ...
    hyp_file_info_t *files = NULL;
    int file_count = 0;
    ASSERT_EQ(hyp_discover(dir, &opts, &files, &file_count), 0);
    ASSERT_GT(file_count, 0);

    hyp_gbuf_t *gbuf = run_parallel("argov-test", dir, files, file_count, 4);
    ASSERT_TRUE(gbuf != NULL);
    ASSERT_GT(hyp_gbuf_edge_count(gbuf), 0);
...
TEST(gbuf_shared_ids_unique) {
    ...
    hyp_gbuf_t *ga = hyp_gbuf_new_shared_ids("proj", "/", &shared);
    hyp_gbuf_t *gb = hyp_gbuf_new_shared_ids("proj", "/", &shared);

    int64_t id1 = hyp_gbuf_upsert_node(ga, "Function", "foo", "proj.foo", "a.go", 1, 5, "{}");
    int64_t id2 = hyp_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "b.go", 1, 3, "{}");
    ASSERT_GT(id1, 0);
    ASSERT_GT(id2, 0);
    ASSERT_NEQ(id1, id2);
...
TEST(gbuf_merge_nodes) {
    _Atomic int64_t shared = 1;
    hyp_gbuf_t *dst = hyp_gbuf_new_shared_ids("proj", "/", &shared);
    hyp_gbuf_t *src = hyp_gbuf_new_shared_ids("proj", "/", &shared);

    hyp_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    hyp_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    hyp_gbuf_upsert_node(src, "Function", "c", "proj.c", "b.go", 1, 5, "{}");
    hyp_gbuf_upsert_node(src, "Function", "d", "proj.d", "b.go", 6, 10, "{}");

    ASSERT_EQ(hyp_gbuf_node_count(dst), 2);
    hyp_gbuf_merge(dst, src);
    ASSERT_EQ(hyp_gbuf_node_count(dst), 4);

    ASSERT_NOT_NULL(hyp_gbuf_find_by_qn(dst, "proj.c"));
    ASSERT_NOT_NULL(hyp_gbuf_find_by_qn(dst, "proj.d"));
    /* dst originals still there */
    ASSERT_NOT_NULL(hyp_gbuf_find_by_qn(dst, "proj.a"));
    ASSERT_NOT_NULL(hyp_gbuf_find_by_qn(dst, "proj.b"));
...
TEST(gbuf_merge_edges) {
    ...
    int64_t a = hyp_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int64_t b = hyp_gbuf_upsert_node(dst, "Function", "b", "proj.b", "a.go", 6, 10, "{}");
    /* Put an edge in src that references dst nodes (by ID) */
    hyp_gbuf_insert_edge(src, a, b, "CALLS", "{}");

    hyp_gbuf_merge(dst, src);
    ASSERT_GT(hyp_gbuf_edge_count(dst), 0);

    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
    hyp_gbuf_find_edges_by_source_type(dst, a, "CALLS", &edges, &count);
    ASSERT_EQ(count, 1);
...
TEST(gbuf_merge_empty_src) {
    ...
    hyp_gbuf_upsert_node(dst, "Function", "a", "proj.a", "a.go", 1, 5, "{}");
    int before = hyp_gbuf_node_count(dst);
    hyp_gbuf_merge(dst, src);
    ASSERT_EQ(hyp_gbuf_node_count(dst), before);
...
TEST(gbuf_merge_src_free_safe) {
    ...
    hyp_gbuf_upsert_node(src, "Function", "x", "proj.x", "x.go", 1, 5, "{}");
    hyp_gbuf_merge(dst, src);
    hyp_gbuf_free(src); /* must not crash */

    /* dst node still accessible */
    ASSERT_NOT_NULL(hyp_gbuf_find_by_qn(dst, "proj.x"));
    hyp_gbuf_free(dst);
    PASS();
}

TEST(gbuf_next_id_accessors) {
    hyp_gbuf_t *gb = hyp_gbuf_new("proj", "/");
    ASSERT_EQ(hyp_gbuf_next_id(gb), 1);

    hyp_gbuf_upsert_node(gb, "Function", "foo", "proj.foo", "f.go", 1, 5, "{}");
    ASSERT_GT(hyp_gbuf_next_id(gb), 1);

    hyp_gbuf_set_next_id(gb, 100);
    int64_t id = hyp_gbuf_upsert_node(gb, "Function", "bar", "proj.bar", "f.go", 6, 10, "{}");
    ASSERT_GTE(id, 100);
 ...
 * pass_parallel.c (parallel) on hyp_pipeline_find_lsp_resolution +
 * HYP_LSP_CONFIDENCE_FLOOR (lsp_resolve.h). Before the unification, the
...
static void count_lsp_call_edges(const hyp_gbuf_edge_t *edge, void *ud) {
...
static const hyp_gbuf_node_t *find_unique_callable_node_by_tail(const hyp_gbuf_t *gbuf,
                                                                const char *tail) {
    ...
    const hyp_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (hyp_gbuf_find_by_name(gbuf, method, &nodes, &count) != 0) {
        return NULL;
    }
    const hyp_gbuf_node_t *match = NULL;
    for (int i = 0; i < count; i++) {
        const hyp_gbuf_node_t *node = nodes[i];
...
static const hyp_gbuf_node_t *find_unique_node_by_name_label_qn_suffix(const hyp_gbuf_t *gbuf,
                                                                       const char *name,
                                                                       const char *label,
    ...
    const hyp_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (hyp_gbuf_find_by_name(gbuf, name, &nodes, &count) != 0) {
        return NULL;
    }
    const hyp_gbuf_node_t *match = NULL;
    for (int i = 0; i < count; i++) {
...
static int count_edges_between_nodes(const hyp_gbuf_t *gbuf, const hyp_gbuf_node_t *source,
                                     const hyp_gbuf_node_t *target, const char *edge_type) {
    if (!gbuf || !source || !target || !edge_type) {
        return -1;
    }
    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (hyp_gbuf_find_edges_by_source_type(gbuf, source->id, edge_type, &edges, &count) != 0) {
        return -1;
    }
    int matches = 0;
    for (int i = 0; i < count; i++) {
...
static bool has_edge_from_callable_to_node(const hyp_gbuf_t *gbuf, const char *source_tail,
                                           const hyp_gbuf_node_t *target, const char *edge_type) {
    const hyp_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
...
static const hyp_gbuf_edge_t *find_calls_edge_by_tails(const hyp_gbuf_t *gbuf,
                                                       const char *source_tail,
                                                       const char *target_tail) {
    const hyp_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
    const hyp_gbuf_node_t *target = find_unique_callable_node_by_tail(gbuf, target_tail);
    if (!source || !target) {
        return NULL;
    }
    ...
    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (hyp_gbuf_find_edges_by_source_type(gbuf, source->id, "CALLS", &edges, &count) != 0) {
        return NULL;
    }
    for (int i = 0; i < count; i++) {
        if (edges[i] && edges[i]->target_id == target->id) {
...
static bool has_edge_from_callable_to_qn(const hyp_gbuf_t *gbuf, const char *source_tail,
                                         const char *target_qn, const char *edge_type) {
    const hyp_gbuf_node_t *source = find_unique_callable_node_by_tail(gbuf, source_tail);
    const hyp_gbuf_node_t *target = hyp_gbuf_find_by_qn(gbuf, target_qn);
    if (!source || !target)
        return false;
    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
...
static bool callable_has_call_target_fragment(const hyp_gbuf_t *gbuf, const char *source_tail,
    ...
    for (int i = 0; i < count; i++) {
        const hyp_gbuf_node_t *target =
            edges[i] ? hyp_gbuf_find_by_id(gbuf, edges[i]->target_id) : NULL;
...
static int count_calls_edges_to_tail(const hyp_gbuf_t *gbuf, const char *target_tail) {
    const hyp_gbuf_node_t *target = find_unique_callable_node_by_tail(gbuf, target_tail);
    if (!target)
        return -1;
    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
    if (hyp_gbuf_find_edges_by_target_type(gbuf, target->id, "CALLS", &edges, &count) != 0)
        return -1;
    return count;
}
...
static void inject_higher_confidence_legacy_render(HYPFileResult **result_cache, int file_count,
                                                   void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        HYPFileResult *result = result_cache ? result_cache[file] : NULL;
        if (!result) {
            continue;
        }
        const HYPResolvedCall *exact = NULL;
        const char *beta_qn = NULL;
        for (int i = 0; i < result->resolved_calls.count; i++) {
            const HYPResolvedCall *candidate = &result->resolved_calls.items[i];
            if (candidate->kind == HYP_RESOLVED_INVOCATION && candidate->caller_qn &&
        ...
        for (int i = 0; i < result->defs.count; i++) {
            const HYPDefinition *definition = &result->defs.items[i];
        ...
        hyp_resolvedcall_push(&result->resolved_calls, &result->arena, legacy);
        state->injected = true;
        return;
    }
}
...
TEST(parallel_lsp_index_exact_site_beats_legacy_record_in_graph) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_lsp_site_XXXXXX");
    ...
    hyp_file_info_t files[1] = {0};
    files[0].path = path;
    files[0].rel_path = (char *)"app.py";
    files[0].language = HYP_LANG_PYTHON;
    lsp_legacy_injection_t injection = {0};
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_par_lsp_site", tmpdir, files, 1, 1, NULL, inject_higher_confidence_legacy_render,
        &injection, false);
    ASSERT_NOT_NULL(gbuf);
    ASSERT_TRUE(injection.injected);

    const hyp_gbuf_edge_t *exact = find_calls_edge_by_tails(gbuf, "Caller.run", "Alpha.render");
    const hyp_gbuf_edge_t *wrong = find_calls_edge_by_tails(gbuf, "Caller.run", "Beta.render");
    if (!exact || wrong) {
...
static void inject_distinct_exact_render_target(HYPFileResult **result_cache, int file_count,
                                                void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        HYPCall *carrier = NULL;
        const HYPResolvedCall *exact = NULL;
        const char *beta_qn = NULL;
        for (int i = 0; i < result->calls.count; i++) {
            HYPCall *candidate = &result->calls.items[i];
            if (candidate->callee_name && candidate->enclosing_func_qn &&
                strstr(candidate->enclosing_func_qn, "Caller.run") &&
                strcmp(hyp_pipeline_call_callee_leaf(candidate->callee_name), "render") == 0 &&
                hyp_pipeline_source_site_present(candidate->site_start_byte,
...
TEST(parallel_lsp_index_distinct_exact_targets_fail_closed_in_graph) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_lsp_ambiguous_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    hyp_file_info_t file_info = {0};
    file_info.path = path;
    file_info.rel_path = (char *)"app.py";
    file_info.language = HYP_LANG_PYTHON;
    lsp_exact_ambiguity_injection_t injection = {0};
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_par_lsp_ambiguous", tmpdir, &file_info, 1, 1, NULL,
    ...
    const hyp_gbuf_edge_t *alpha = find_calls_edge_by_tails(gbuf, "Caller.run", "Alpha.render");
    const hyp_gbuf_edge_t *beta = find_calls_edge_by_tails(gbuf, "Caller.run", "Beta.render");
    if (alpha || beta) {
...
static void inject_legacy_after_exact_render_ambiguity(HYPFileResult **result_cache, int file_count,
                                                       void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        probe->shared_matcher_failed_closed =
            hyp_pipeline_find_lsp_resolution(&result->resolved_calls, carrier, false) == NULL;
        return;
    }
}
...
TEST(parallel_lsp_index_exact_ambiguity_does_not_fall_through_to_legacy) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_lsp_ambiguous_legacy_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_par_lsp_ambiguous_legacy", tmpdir, &file_info, 1, 1, NULL,
    ...
    const hyp_gbuf_edge_t *legacy_edge =
...
static void inject_long_caller_exact_and_legacy(HYPFileResult **result_cache, int file_count,
                                                void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            HYPResolvedCall *candidate = &result->resolved_calls.items[i];
            if (candidate->kind == HYP_RESOLVED_INVOCATION && candidate->callee_qn &&
                strstr(candidate->callee_qn, "Alpha.render") &&
                candidate->site_end_byte > candidate->site_start_byte) {
                exact = candidate;
                break;
        ...
        const HYPResolvedCall *authoritative =
            hyp_pipeline_find_lsp_resolution(&result->resolved_calls, carrier, false);
...
TEST(parallel_lsp_long_exact_key_never_yields_legacy_target) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_lsp_long_key_XXXXXX");
    if (!hyp_mkdtemp(tmpdir))
        FAIL("mkdtemp failed");
    char path[512];
    snprintf(path, sizeof(path), "%s/app.py", tmpdir);
    ...
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_long_key", tmpdir, files, 1, 1, NULL, inject_long_caller_exact_and_legacy, &probe,
        false);
    ASSERT_NOT_NULL(gbuf);
    int alpha_edges = count_calls_edges_to_tail(gbuf, "Alpha.render");
    int beta_edges = count_calls_edges_to_tail(gbuf, "Beta.render");
...
static void inspect_synthetic_add_occurrences(HYPFileResult **result_cache, int file_count,
                                              void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        HYPFileResult *result = result_cache[file];
        if (!result) {
            continue;
        }
        for (int i = 0; i < result->calls.count; i++) {
            const HYPCall *call = &result->calls.items[i];
            if (!call->requires_lsp_resolution || !call->callee_name ||
                strcmp(call->callee_name, "__add__") != 0 ||
                !hyp_pipeline_source_site_present(call->site_start_byte, call->site_end_byte)) {
                continue;
            }
            probe->carrier_count++;
            const HYPResolvedCall *resolved =
                hyp_pipeline_find_lsp_resolution(&result->resolved_calls, call, false);
            if (resolved &&
                hyp_pipeline_source_site_eq(call->site_start_byte, call->site_end_byte,
...
TEST(parallel_lsp_exact_index_handles_repeated_synthetic_occurrences_without_linear_scan) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_lsp_perf_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    hyp_pp_lsp_linear_fallback_rows_reset();
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_par_lsp_perf", tmpdir, files, 1, 1, NULL, inspect_synthetic_add_occurrences, &probe,
        false);
    ASSERT_NOT_NULL(gbuf);
    ...
    uint64_t fallback_rows = hyp_pp_lsp_linear_fallback_rows();
    if (fallback_rows != 0) {
...
static void inject_call_reference_scale_rows(HYPFileResult **result_cache, int file_count,
                                             void *ud) {
    ...
    for (int file = 0; file < file_count; file++) {
        ...
        int inserted = 0;
        for (int i = 0; i < result->defs.count; i++) {
            const HYPDefinition *target = &result->defs.items[i];
            ...
            hyp_usages_push(&result->usages, &result->arena, usage);
            ...
            hyp_resolvedcall_push(&result->resolved_calls, &result->arena, resolved);
            inserted++;
        }
...
TEST(parallel_call_reference_lookup_rows_grow_linearly) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_ref_scale_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    hyp_pipeline_lsp_reference_lookup_test_reset();
    hyp_gbuf_t *gbuf = run_parallel_with_extract_opts_and_mutator(
        "hyp_par_ref_scale", tmpdir, &file_info, 1, 1, NULL, inject_call_reference_scale_rows,
        &probe, false);
    uint64_t rows_examined = hyp_pipeline_lsp_reference_lookup_test_rows_examined();

    const hyp_gbuf_node_t *caller =
        gbuf ? find_unique_callable_node_by_tail(gbuf, "app.scale_caller") : NULL;
    const hyp_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    int lookup_rc = caller ? hyp_gbuf_find_edges_by_source_type(gbuf, caller->id, "CALL_REFERENCE",
                                                                &edges, &edge_count)
                           : -1;
...
TEST(parallel_tsx_local_component_shadow_has_no_false_call) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_tsx_shadow_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    files[0].language = HYP_LANG_TSX;
    hyp_gbuf_t *sequential = run_sequential("hyp_tsx_shadow", tmpdir, files, 1);
    hyp_gbuf_t *parallel = run_parallel("hyp_tsx_shadow", tmpdir, files, 1, 1);
    ...
    hyp_gbuf_free(sequential);
    hyp_gbuf_free(parallel);
    unlink(path);
    rmdir(tmpdir);
...
static void inject_exact_noncallable_reference(HYPFileResult **result_cache, int file_count,
                                               void *ud) {
...
TEST(parallel_exact_semantic_noncallable_target_stays_usage) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_exact_noncallable_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    hyp_file_info_t file = {0};
    file.path = path;
    file.rel_path = (char *)"app.ts";
    file.language = HYP_LANG_TYPESCRIPT;
    exact_noncallable_reference_probe_t sequential_probe = {0};
    exact_noncallable_reference_probe_t parallel_probe = {0};
    hyp_gbuf_t *sequential = run_sequential_with_lsp_cross_and_mutator(
        "hyp_exact_noncallable", tmpdir, &file, 1, inject_exact_noncallable_reference,
        &sequential_probe, false);
    hyp_gbuf_t *parallel = run_parallel_with_extract_opts_and_mutator(
        "hyp_exact_noncallable", tmpdir, &file, 1, 1, NULL, inject_exact_noncallable_reference,
        &parallel_probe, false);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const hyp_gbuf_node_t *sequential_target = find_unique_node_by_name_label_qn_suffix(
        sequential, "semanticValue", "Variable", "app.semanticValue");
    const hyp_gbuf_node_t *parallel_target = find_unique_node_by_name_label_qn_suffix(
...
TEST(parallel_typescript_module_value_usage_respects_lexical_shadows) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_ts_usage_scope_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    hyp_file_info_t files[2] = {0};
    files[0].path = settings_path;
    files[0].rel_path = (char *)"settings.ts";
    files[0].language = HYP_LANG_TYPESCRIPT;
    files[1].path = shadows_path;
    files[1].rel_path = (char *)"shadows.ts";
    files[1].language = HYP_LANG_TYPESCRIPT;
    hyp_gbuf_t *sequential = run_sequential("hyp_ts_usage_scope", tmpdir, files, 2);
    hyp_gbuf_t *parallel = run_parallel("hyp_ts_usage_scope", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const hyp_gbuf_node_t *sequential_config = find_unique_node_by_name_label_qn_suffix(
        sequential, "config", "Variable", "settings.config");
    const hyp_gbuf_node_t *parallel_config =
...
static ts_exact_parity_observation_t run_ts_exact_parity_fixture(HYPLanguage main_language,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_ts_exact_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        return observation;
    }
    ...
    for (int route = 0; route < 2; route++) {
        hyp_gbuf_t *graph = graphs[route];
        const hyp_gbuf_node_t *choose =
            find_unique_node_by_name_label_qn_suffix(graph, "choose", "Function", "main.choose");
        const hyp_gbuf_node_t *selected = find_unique_node_by_name_label_qn_suffix(
            graph, "Config", "Interface", "a.types.Config");
        const hyp_gbuf_node_t *decoy = find_unique_node_by_name_label_qn_suffix(
            graph, "Config", "Interface", "b.types.Config");
        const hyp_gbuf_node_t *local_class =
            find_unique_node_by_name_label_qn_suffix(graph, "Config", "Class", "main.App.Config");
        const hyp_gbuf_node_t *normalise = find_unique_node_by_name_label_qn_suffix(
            graph, "normalise", "Function", "main.App.Utils.normalise");
        const hyp_gbuf_node_t *clamp = find_unique_node_by_name_label_qn_suffix(
            graph, "clamp", "Function", "main.App.Utils.clamp");
        const hyp_gbuf_node_t *constructor = find_unique_node_by_name_label_qn_suffix(
    ...
    hyp_gbuf_free(graphs[0]);
    hyp_gbuf_free(graphs[1]);
    th_rmtree(tmpdir);
    return observation;
}
...
TEST(parallel_typescript_import_namespace_exact_parity) {
    ts_exact_parity_observation_t observation =
        run_ts_exact_parity_fixture(HYP_LANG_TYPESCRIPT, "ts", "hyp_ts_exact_parity");
...
TEST(parallel_tsx_import_namespace_exact_parity) {
    ts_exact_parity_observation_t observation =
        run_ts_exact_parity_fixture(HYP_LANG_TSX, "tsx", "hyp_tsx_exact_parity");
...
static kotlin_implicit_probe_t probe_kotlin_implicit_site(const HYPFileResult *result,
    ...
    for (int j = 0; j < result->resolved_calls.count; j++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[j];
        ...
        if (resolved->kind != HYP_RESOLVED_INVOCATION || !resolved_caller || !resolved->callee_qn ||
        ...
        for (int i = 0; i < result->calls.count; i++) {
            ...
            if (hyp_pipeline_source_site_eq(call->site_start_byte, call->site_end_byte,
                                            resolved->site_start_byte, resolved->site_end_byte)) {
...
TEST(parallel_kotlin_external_protocol_does_not_use_project_class_method_tail) {
    ...
    const char *project = "hyp_kt_target_policy";

    HYPFileResult *raw = hyp_extract_file(main_source, (int)strlen(main_source), HYP_LANG_KOTLIN,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_kt_target_policy_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    files[0].language = HYP_LANG_KOTLIN;
    files[1].path = main_path;
    files[1].rel_path = (char *)"app/Main.kt";
    files[1].language = HYP_LANG_KOTLIN;
    hyp_gbuf_t *sequential = run_sequential_with_lsp_cross(project, tmpdir, files, 2);
    hyp_gbuf_t *parallel = run_parallel(project, tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const hyp_gbuf_node_t *sequential_iterator = find_unique_node_by_name_label_qn_suffix(
        sequential, "iterator", "Method", "Collisions.IntArray.iterator");
    const hyp_gbuf_node_t *parallel_iterator = find_unique_node_by_name_label_qn_suffix(
        parallel, "iterator", "Method", "Collisions.IntArray.iterator");
    const hyp_gbuf_node_t *sequential_component = find_unique_node_by_name_label_qn_suffix(
        sequential, "component1", "Method", "Collisions.Pair.component1");
    const hyp_gbuf_node_t *parallel_component = find_unique_node_by_name_label_qn_suffix(
 ...
 * HYPCall carrier so both graph paths can materialize CALLS without a textual
...
TEST(parallel_kotlin_nonbinary_operator_carriers_reach_graph) {
    ...
    static const char source[] = "package app\n"
                                 ...
                                 "fun isBag(value: Any): Boolean = value is Bag\n"
                                 "fun assign(slots: Slots) { slots[0] = 1 }\n";
    const char *project = "hyp_kt_nonbinary_operators";

    HYPFileResult *raw = hyp_extract_file(source, (int)strlen(source), HYP_LANG_KOTLIN, project,
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_kt_nonbinary_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
...
TEST(parallel_rust_known_macro_does_not_fallback_to_local_function) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_rust_macro_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    HYPFileResult *extracted = hyp_extract_file(source, (int)strlen(source), HYP_LANG_RUST,
                                                "hyp_rust_macro", "main.rs", 0, NULL, NULL);
    ...
    for (int i = 0; i < extracted->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &extracted->resolved_calls.items[i];
    ...
    for (int i = 0; i < extracted->calls.count; i++) {
        const HYPCall *call = &extracted->calls.items[i];
    ...
    hyp_free_result(extracted);

    hyp_file_info_t files[1] = {0};
    files[0].path = path;
    files[0].rel_path = (char *)"main.rs";
    files[0].language = HYP_LANG_RUST;
    hyp_gbuf_t *sequential = run_sequential("hyp_rust_macro", tmpdir, files, 1);
    hyp_gbuf_t *parallel = run_parallel("hyp_rust_macro", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);
...
TEST(parallel_rust_proc_macros_are_decorates_and_usage_only) {
    static const char source[] = "#[tokio::main]\nasync fn main() {}\n"
                                 "#[tracing::instrument]\nfn work() {}\n";
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_rust_proc_macro_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char path[512];
    ...
    files[0].rel_path = (char *)"main.rs";
    files[0].language = HYP_LANG_RUST;
    hyp_gbuf_t *sequential = run_sequential("hyp_rust_proc_macro", tmpdir, files, 1);
    hyp_gbuf_t *parallel = run_parallel("hyp_rust_proc_macro", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);
...
static int assert_c_family_preprocessed_collision_graph(HYPLanguage language, const char *extension,
                                                        const char *project, const char *source,
                                                        const char *target_tail) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_c_origin_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    char rel_path[64];
    ...
    hyp_gbuf_t *sequential = run_sequential(project, tmpdir, files, 1);
    hyp_gbuf_t *parallel = run_parallel(project, tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(sequential);
    ASSERT_NOT_NULL(parallel);

    const hyp_gbuf_edge_t *sequential_edge =
        find_calls_edge_by_tails(sequential, "main.occurrence_probe", target_tail);
    const hyp_gbuf_edge_t *parallel_edge =
...
TEST(parallel_c_preprocessed_coordinate_collision_preserves_hidden_target) {
    static const char source[] =
        ...
        "}\n";
    return assert_c_family_preprocessed_collision_graph(HYP_LANG_C, "c", "hyp_c_origin", source,
                                                        "main.bravo_target");
}

static int assert_cpp_like_preprocessed_collision_graph(HYPLanguage language, const char *extension,
                                                        const char *project) {
    static const char source[] =
...
TEST(parallel_cpp_preprocessed_coordinate_collision_preserves_hidden_target) {
    return assert_cpp_like_preprocessed_collision_graph(HYP_LANG_CPP, "cpp", "hyp_cpp_origin");
}

TEST(parallel_cuda_preprocessed_coordinate_collision_preserves_hidden_target) {
    return assert_cpp_like_preprocessed_collision_graph(HYP_LANG_CUDA, "cu", "hyp_cuda_origin");
}

TEST(parallel_java_kotlin_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_jvm_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    hyp_mkdir_p(jdir, 0755);
    FILE *jf = hyp_fopen(jpath, "w");
    if (!jf) {
    ...
    hyp_mkdir_p(kdir, 0755);
    FILE *kf = hyp_fopen(kpath, "w");
    if (!kf) {
        unlink(jpath);
        rmdir(tmpdir);
    ...
    files[0].language = HYP_LANG_JAVA;
    files[1].path = kpath;
    files[1].rel_path = (char *)"src/main/kotlin/com/example/KotlinService.kt";
    files[1].language = HYP_LANG_KOTLIN;

    hyp_gbuf_t *gbuf = run_parallel("com", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(gbuf);

    const hyp_gbuf_edge_t *java_to_kotlin =
        find_calls_edge_by_tails(gbuf, "JavaCaller.call", "KotlinService.ping");
    const hyp_gbuf_edge_t *kotlin_to_java =
 ...
 * hyp_pipeline_lsp_allow_tail_match must keep the fallbacks OFF there.
 *
...
TEST(parallel_lsp_tail_match_fallbacks_gated_to_jvm) {
    ...
    ASSERT_TRUE(hyp_pipeline_lsp_allow_tail_match(HYP_LANG_JAVA));
    ASSERT_TRUE(hyp_pipeline_lsp_allow_tail_match(HYP_LANG_KOTLIN));
    ASSERT_TRUE(!hyp_pipeline_lsp_allow_tail_match(HYP_LANG_PYTHON));
    ASSERT_TRUE(!hyp_pipeline_lsp_allow_tail_match(HYP_LANG_GO));
    ASSERT_TRUE(!hyp_pipeline_lsp_allow_tail_match(HYP_LANG_TYPESCRIPT));
    ASSERT_TRUE(!hyp_pipeline_lsp_allow_tail_match(HYP_LANG_CPP));
    ...
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&arr, &call, false) == NULL);
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&arr, &call, true) == &rc_item);
    ...
    HYPResolvedCall top_level_rows[2] = {0};
    ...
    top_level_rows[0].source_origin = HYP_SOURCE_ORIGIN_RAW;
    HYPResolvedCallArray top_level_arr = {0};
    top_level_arr.items = top_level_rows;
    top_level_arr.count = 1;
    top_level_arr.cap = 2;

    HYPCall top_level_call = {0};
    ...
    top_level_call.source_origin = HYP_SOURCE_ORIGIN_RAW;
    top_level_call.requires_lsp_resolution = true;
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, false) == NULL);
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, true) ==
                &top_level_rows[0]);

    top_level_call.enclosing_func_qn = "proj.app.Main.differentCaller";
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&top_level_arr, &top_level_call, true) == NULL);
    top_level_call.enclosing_func_qn = "proj.app.Main.operatorCaller";
    ...
    hyp_gbuf_t *tgbuf = hyp_gbuf_new("proj", "/tmp");
    ASSERT_NOT_NULL(tgbuf);
    int64_t nid = hyp_gbuf_upsert_node(tgbuf, "Method", "run", "proj.zeta.Helper.run",
                                       "zeta/helper.py", 1, 3, NULL);
    ASSERT_TRUE(nid != 0);
    ASSERT_TRUE(hyp_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", false) == NULL);
    const hyp_gbuf_node_t *jvm_hit =
        hyp_pipeline_lsp_target_node(tgbuf, "proj", "com.other.Helper.run", true);
    ...
    ASSERT_TRUE(hyp_pipeline_lsp_target_node_strict(tgbuf, "proj", "com.other.Helper.run", true) ==
                NULL);
    hyp_gbuf_free(tgbuf);
    PASS();
}
...
TEST(parallel_python_lsp_override_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_pylsp_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    hyp_gbuf_t *gbuf = run_parallel("hyp_par_pylsp", tmpdir, files, 1, 1);
    ASSERT_NOT_NULL(gbuf);

    lsp_edge_count_ctx_t c = {0};
    hyp_gbuf_foreach_edge(gbuf, count_lsp_call_edges, &c);
     ...
     * CALLS edge. This proves the unified hyp_pipeline_find_lsp_resolution
 ...
 * Before hyp_pipeline_lsp_target_node added the project-prefix fallback,
 * the LSP match succeeded (lsp_overrides counter incremented) but the
 * downstream hyp_gbuf_find_by_qn lookup missed silently, dropping the
...
TEST(parallel_python_lsp_override_cross_file_emits_lsp_strategy_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_pylsp_xf_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    files[1].language = HYP_LANG_PYTHON;

    hyp_gbuf_t *gbuf = run_parallel("hyp_par_pylsp_xf", tmpdir, files, 2, 2);
    ASSERT_NOT_NULL(gbuf);
     ...
     * hyp_pipeline_lsp_target_node this assertion would fail because the
...
TEST(parallel_cross_file_reread_preserves_unretained_edges) {
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_par_xf_reread_XXXXXX");
    if (!hyp_mkdtemp(tmpdir)) {
        FAIL("mkdtemp failed");
    }
    ...
    const hyp_parallel_extract_opts_t no_retain = {
        .retain_sources = false,
        .retain_sources_set = true,
    };
    const hyp_parallel_extract_opts_t over_cap = {
        .retain_sources = true,
        .retain_sources_set = true,
        .retain_per_file_max_bytes = 1, /* 1 byte → every file dropped by the size cap */
    };
    const hyp_parallel_extract_opts_t *scenarios[3] = {NULL, &no_retain, &over_cap};

    for (int s = 0; s < 3; s++) {
        hyp_gbuf_t *gbuf = run_parallel_with_extract_opts("com", tmpdir, files, 2, 2, scenarios[s]);
        ASSERT_NOT_NULL(gbuf);
...
/* ── Shared "::" normalization in hyp_pipeline_find_lsp_resolution (QA F3) ─
 *
 ...
 * against hyp_pipeline_find_lsp_resolution: (1) a qualified static call still
...
static HYPResolvedCall make_rc(const char *caller, const char *callee, float conf) {
...
static HYPCall make_call(const char *enclosing, const char *callee_name) {
    HYPCall c;
    memset(&c, 0, sizeof(c));
...
TEST(lsp_bare_segment_skips_preprocessor_spacing) {
    ...
    ASSERT_STR_EQ(hyp_lsp_bare_segment("bravo . render"), "render");
    ASSERT_STR_EQ(hyp_lsp_bare_segment("ptr -> run"), "run");
    PASS();
}
...
TEST(lsp_resolve_qualified_static_call_normalizes_colons) {
    ...
    HYPResolvedCallArray arr = {items, 1, 1};
    HYPCall call = make_call("proj.mod.caller", "Pkg::sub");
    const HYPResolvedCall *hit = hyp_pipeline_find_lsp_resolution(&arr, &call, false);
    ASSERT(hit != NULL);
    ...
    HYPCall bare = make_call("proj.mod.caller", "sub");
    const HYPResolvedCall *bare_hit = hyp_pipeline_find_lsp_resolution(&arr, &bare, false);
...
TEST(lsp_resolve_distinct_exact_caller_targets_fail_closed) {
    ...
    HYPResolvedCallArray arr = {items, 4, 4};
    HYPCall call = make_call("proj.mod.caller", "B::foo");
    const HYPResolvedCall *hit = hyp_pipeline_find_lsp_resolution(&arr, &call, false);
    ASSERT_NULL(hit);

    /* The cross-caller high-confidence entry only matches its own caller. */
    HYPCall other = make_call("proj.mod.other", "D::foo");
    const HYPResolvedCall *other_hit = hyp_pipeline_find_lsp_resolution(&arr, &other, false);
    ...
    HYPCall absent = make_call("proj.mod.absent", "foo");
    ASSERT(hyp_pipeline_find_lsp_resolution(&arr, &absent, false) == NULL);
    PASS();
}

TEST(lsp_resolve_duplicate_exact_caller_rows_same_target_are_not_ambiguous) {
    HYPCall call = make_call("proj.mod.caller", "receiver.render");
    call.site_start_byte = 75;
    call.site_end_byte = 91;
    HYPResolvedCall low = make_rc("proj.mod.caller", "proj.Alpha.render", 0.75f);
    low.site_start_byte = call.site_start_byte;
    low.site_end_byte = call.site_end_byte;
    HYPResolvedCall high = low;
    high.confidence = 0.95f;
    HYPResolvedCall items[] = {low, high};
    HYPResolvedCallArray arr = {items, 2, 2};

    ASSERT(hyp_pipeline_find_lsp_resolution(&arr, &call, false) == &items[1]);
    PASS();
}
...
TEST(lsp_resolve_distinct_exact_caller_site_targets_are_ambiguous) {
    ...
    HYPResolvedCall alpha = make_rc("proj.mod.caller", "proj.Alpha.render", 0.75f);
    alpha.site_start_byte = call.site_start_byte;
    alpha.site_end_byte = call.site_end_byte;
    HYPResolvedCall beta = make_rc("proj.mod.caller", "proj.Beta.render", 0.99f);
    beta.site_start_byte = call.site_start_byte;
    beta.site_end_byte = call.site_end_byte;
    HYPResolvedCall items[] = {alpha, beta};
    HYPResolvedCallArray arr = {items, 2, 2};

    ASSERT_NULL(hyp_pipeline_find_lsp_resolution(&arr, &call, false));
    PASS();
}
...
TEST(lsp_resolve_project_prefixed_duplicate_is_not_ambiguous) {
    ...
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = hyp_gbuf_upsert_node(gbuf, "Function", "handler",
                                             "proj.mod.Target.handler", "target.py", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    HYPCall call = make_call("proj.mod.Caller.run", "handler");
    call.site_start_byte = 110;
    call.site_end_byte = 119;
    HYPResolvedCall raw = make_rc("proj.mod.Caller.run", "mod.Target.handler", 0.75f);
    raw.site_start_byte = call.site_start_byte;
    raw.site_end_byte = call.site_end_byte;
    HYPResolvedCall prefixed =
    ...
    HYPResolvedCall items[] = {raw, prefixed};
    HYPResolvedCallArray arr = {items, 2, 2};

    const HYPResolvedCall *hit =
        hyp_pipeline_find_lsp_resolution_in_graph(&arr, &call, false, gbuf, project);
    hyp_gbuf_free(gbuf);
    ASSERT(hit == &items[1]);
    PASS();
}
...
TEST(lsp_resolve_project_prefix_spellings_stay_ambiguous_when_both_nodes_exist) {
    ...
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t raw_id = hyp_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                          "raw.py", 1, 1, "{}");
    int64_t prefixed_id = hyp_gbuf_upsert_node(gbuf, "Function", "handler",
...
TEST(lsp_resolve_jvm_package_alias_of_materialized_target_is_not_ambiguous) {
    ...
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id =
        hyp_gbuf_upsert_node(gbuf, "Method", "twice", "proj.Util.twice", "Util.java", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    HYPCall call = make_call("proj.Client.run", "twice");
    call.site_start_byte = 120;
    call.site_end_byte = 128;
    HYPResolvedCall local = make_rc("proj.Client.run", "proj.Util.twice", 0.92f);
    local.site_start_byte = call.site_start_byte;
    local.site_end_byte = call.site_end_byte;
    HYPResolvedCall package = make_rc("proj.Client.run", "demo.Util.twice", 0.92f);
    package.site_start_byte = call.site_start_byte;
    package.site_end_byte = call.site_end_byte;
    HYPResolvedCall items[] = {local, package};
    HYPResolvedCallArray arr = {items, 2, 2};

    const HYPResolvedCall *hit =
        hyp_pipeline_find_lsp_resolution_in_graph(&arr, &call, true, gbuf, project);
    hyp_gbuf_free(gbuf);
    ASSERT_NOT_NULL(hit);
    PASS();
}
...
TEST(lsp_resolve_jvm_two_tail_only_packages_stay_ambiguous) {
    ...
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = hyp_gbuf_upsert_node(gbuf, "Method", "run", "proj.generated.Service.run",
                                             "Service.java", 1, 1, "{}");
    ASSERT_GT(target_id, 0);

    HYPCall call = make_call("proj.Client.call", "run");
    call.site_start_byte = 130;
    call.site_end_byte = 138;
    HYPResolvedCall first = make_rc("proj.Client.call", "pkg1.Service.run", 0.80f);
    first.site_start_byte = call.site_start_byte;
    first.site_end_byte = call.site_end_byte;
    HYPResolvedCall second = make_rc("proj.Client.call", "pkg2.Service.run", 0.90f);
    second.site_start_byte = call.site_start_byte;
    second.site_end_byte = call.site_end_byte;
    HYPResolvedCall items[] = {first, second};
    HYPResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_exact_site_beats_higher_confidence_legacy_record) {
    ...
    HYPResolvedCall exact = make_rc("proj.mod.caller", "proj.Alpha.render", 0.80f);
     ...
     * carrier occurrence. */
    HYPResolvedCall legacy = make_rc("proj.mod.caller", "proj.Beta.render", 0.95f);
    HYPResolvedCall items[] = {exact, legacy};
    HYPResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_exact_site_beats_equal_confidence_legacy_record) {
    ...
    HYPResolvedCall legacy = make_rc("proj.mod.caller", "proj.Beta.render", 0.90f);
    HYPResolvedCall exact = make_rc("proj.mod.caller", "proj.Alpha.render", 0.90f);
    exact.site_start_byte = call.site_start_byte;
    exact.site_end_byte = call.site_end_byte;
    HYPResolvedCall items[] = {legacy, exact};
    HYPResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_malformed_span_is_not_legacy_compatible) {
    ...
    HYPResolvedCall malformed = make_rc("proj.mod.caller", "proj.Wrong.render", 0.99f);
    malformed.site_start_byte = 317;
    malformed.site_end_byte = 300;
    HYPResolvedCall legacy = make_rc("proj.mod.caller", "proj.Legacy.render", 0.75f);
    HYPResolvedCall items[] = {malformed, legacy};
    HYPResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_resolve_below_floor_exact_does_not_block_legacy) {
    ...
    HYPResolvedCall below_floor = make_rc("proj.mod.caller", "proj.Exact.render", 0.55f);
    below_floor.site_start_byte = call.site_start_byte;
    below_floor.site_end_byte = call.site_end_byte;
    HYPResolvedCall legacy = make_rc("proj.mod.caller", "proj.Legacy.render", 0.70f);
    HYPResolvedCall items[] = {below_floor, legacy};
    HYPResolvedCallArray arr = {items, 2, 2};
...
static HYPCall make_tail_call(uint32_t start, uint32_t end) {
    HYPCall call = make_call("proj.raw.Service.handle", "helper.run");
    call.site_start_byte = start;
    call.site_end_byte = end;
    return call;
}

static HYPResolvedCall make_tail_rc(const char *callee, float confidence, uint32_t start,
                                    uint32_t end) {
    HYPResolvedCall resolved = make_rc("com.example.generated.Service.handle", callee, confidence);
...
TEST(lsp_tail_exact_site_beats_legacy_regardless_of_order) {
    HYPCall call = make_tail_call(500, 512);
    HYPResolvedCall exact = make_tail_rc("com.example.Alpha.run", 0.80f, 500, 512);
    HYPResolvedCall legacy = make_tail_rc("com.example.Beta.run", 0.99f, 0, 0);
    HYPResolvedCall exact_first[] = {exact, legacy};
    HYPResolvedCall legacy_first[] = {legacy, exact};
    HYPResolvedCallArray first = {exact_first, 2, 2};
    HYPResolvedCallArray second = {legacy_first, 2, 2};

    ASSERT(hyp_pipeline_find_lsp_resolution(&first, &call, true) == &exact_first[0]);
    ASSERT(hyp_pipeline_find_lsp_resolution(&second, &call, true) == &legacy_first[1]);
    PASS();
}

TEST(lsp_tail_duplicate_exact_rows_same_target_are_not_ambiguous) {
    HYPCall call = make_tail_call(600, 612);
    HYPResolvedCall low = make_tail_rc("com.example.Helper.run", 0.75f, 600, 612);
    HYPResolvedCall high = make_tail_rc("com.example.Helper.run", 0.90f, 600, 612);
    HYPResolvedCall items[] = {low, high};
    HYPResolvedCallArray arr = {items, 2, 2};

    ASSERT(hyp_pipeline_find_lsp_resolution(&arr, &call, true) == &items[1]);
    PASS();
}

TEST(lsp_tail_distinct_exact_targets_are_ambiguous) {
    HYPCall call = make_tail_call(700, 712);
    HYPResolvedCall alpha = make_tail_rc("com.example.Alpha.run", 0.75f, 700, 712);
    HYPResolvedCall beta = make_tail_rc("com.example.Beta.run", 0.99f, 700, 712);
    HYPResolvedCall items[] = {alpha, beta};
    HYPResolvedCallArray arr = {items, 2, 2};

    ASSERT_NULL(hyp_pipeline_find_lsp_resolution(&arr, &call, true));
    PASS();
}

TEST(lsp_tail_duplicate_legacy_rows_same_target_are_not_ambiguous) {
    HYPCall call = make_tail_call(800, 812);
    HYPResolvedCall low = make_tail_rc("com.example.Helper.run", 0.75f, 0, 0);
    HYPResolvedCall high = make_tail_rc("com.example.Helper.run", 0.90f, 0, 0);
    HYPResolvedCall items[] = {low, high};
    HYPResolvedCallArray arr = {items, 2, 2};
...
TEST(lsp_tail_distinct_legacy_targets_are_ambiguous) {
    HYPCall call = make_tail_call(900, 912);
    HYPResolvedCall alpha = make_tail_rc("com.example.Alpha.run", 0.75f, 0, 0);
    HYPResolvedCall beta = make_tail_rc("com.example.Beta.run", 0.99f, 0, 0);
    HYPResolvedCall items[] = {alpha, beta};
    HYPResolvedCallArray arr = {items, 2, 2};
...
static HYPUsage make_reference_usage(const char *caller, const char *name, uint32_t start,
                                     uint32_t end) {
    HYPUsage usage = {0};
...
static HYPResolvedCall make_reference_rc(const char *caller, const char *callee, float confidence,
                                         uint32_t start, uint32_t end) {
    HYPResolvedCall resolved = make_rc(caller, callee, confidence);
    resolved.kind = HYP_RESOLVED_CALL_REFERENCE;
    resolved.site_start_byte = start;
    resolved.site_end_byte = end;
    return resolved;
...
TEST(lsp_reference_duplicate_exact_rows_same_target_are_not_ambiguous) {
    HYPUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1000, 1007);
    HYPResolvedCall low =
        make_reference_rc("proj.mod.Caller.run", "proj.Target.handler", 0.75f, 1000, 1007);
    HYPResolvedCall high =
        make_reference_rc("proj.mod.Caller.run", "proj.Target.handler", 0.90f, 1000, 1007);
    HYPResolvedCall items[] = {low, high};
    HYPResolvedCallArray arr = {items, 2, 2};

    ASSERT(hyp_pipeline_find_lsp_reference(&arr, &usage, false) == &items[1]);
    PASS();
}
...
TEST(lsp_reference_project_prefixed_duplicate_is_not_ambiguous) {
    ...
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t target_id = hyp_gbuf_upsert_node(gbuf, "Function", "handler", "proj.mod.Target.handler",
                                             "target.go", 1, 1, "{}");
    ASSERT_GT(target_id, 0);
    const hyp_gbuf_node_t *raw_target =
        hyp_pipeline_lsp_target_node(gbuf, project, "mod.Target.handler", false);
    const hyp_gbuf_node_t *prefixed_target =
        hyp_pipeline_lsp_target_node(gbuf, project, "proj.mod.Target.handler", false);
    ASSERT_NOT_NULL(raw_target);
    ASSERT_NOT_NULL(prefixed_target);
    ASSERT_EQ(raw_target->id, prefixed_target->id);

    HYPUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1050, 1057);
    HYPResolvedCall raw =
    ...
    const HYPResolvedCall *hit =
        hyp_pipeline_find_lsp_reference_in_graph(&arr, &usage, false, gbuf, project);
    hyp_gbuf_free(gbuf);
    ASSERT(hit == &items[1]);
    PASS();
}
...
TEST(lsp_reference_project_prefix_spellings_stay_ambiguous_when_both_nodes_exist) {
    ...
    int64_t raw_id = hyp_gbuf_upsert_node(gbuf, "Function", "handler", "mod.Target.handler",
                                          "raw.go", 1, 1, "{}");
    int64_t prefixed_id = hyp_gbuf_upsert_node(
        gbuf, "Function", "handler", "proj.mod.Target.handler", "prefixed.go", 1, 1, "{}");
    ASSERT_GT(raw_id, 0);
    ASSERT_GT(prefixed_id, 0);
    ASSERT(raw_id != prefixed_id);

    HYPUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1070, 1077);
    HYPResolvedCall raw =
...
TEST(lsp_reference_jvm_distinct_packages_do_not_collapse_via_tail_node) {
    ...
    hyp_gbuf_t *gbuf = hyp_gbuf_new(project, "/tmp");
    ASSERT_NOT_NULL(gbuf);
    int64_t materialized_id = hyp_gbuf_upsert_node(
        gbuf, "Method", "handler", "proj.generated.Service.handler", "Service.kt", 1, 1, "{}");
    ASSERT_GT(materialized_id, 0);

    HYPUsage usage = make_reference_usage("proj.consumer.Caller.run", "handler", 1080, 1087);
    HYPResolvedCall pkg1 =
        make_reference_rc("proj.consumer.Caller.run", "pkg1.Service.handler", 0.75f, 1080, 1087);
    HYPResolvedCall pkg2 =
        make_reference_rc("proj.consumer.Caller.run", "pkg2.Service.handler", 0.90f, 1080, 1087);
    HYPResolvedCall pkg1_first[] = {pkg1, pkg2};
    HYPResolvedCall pkg2_first[] = {pkg2, pkg1};
    HYPResolvedCallArray first = {pkg1_first, 2, 2};
    HYPResolvedCallArray second = {pkg2_first, 2, 2};

    const hyp_gbuf_node_t *pkg1_tail =
        hyp_pipeline_lsp_target_node(gbuf, project, pkg1.callee_qn, true);
    const hyp_gbuf_node_t *pkg2_tail =
        hyp_pipeline_lsp_target_node(gbuf, project, pkg2.callee_qn, true);
    bool same_tail_node = pkg1_tail && pkg2_tail && pkg1_tail->id == materialized_id &&
                          pkg2_tail->id == materialized_id;
    const HYPResolvedCall *first_hit =
        hyp_pipeline_find_lsp_reference_in_graph(&first, &usage, true, gbuf, project);
    const HYPResolvedCall *second_hit =
        hyp_pipeline_find_lsp_reference_in_graph(&second, &usage, true, gbuf, project);
    hyp_gbuf_free(gbuf);
...
TEST(lsp_reference_distinct_exact_targets_are_order_invariant) {
    HYPUsage usage = make_reference_usage("proj.mod.Caller.run", "handler", 1100, 1107);
    HYPResolvedCall alpha =
        make_reference_rc("proj.mod.Caller.run", "proj.Alpha.handler", 0.90f, 1100, 1107);
    HYPResolvedCall beta =
        make_reference_rc("proj.mod.Caller.run", "proj.Beta.handler", 0.90f, 1100, 1107);
    HYPResolvedCall alpha_first[] = {alpha, beta};
    HYPResolvedCall beta_first[] = {beta, alpha};
    HYPResolvedCallArray first = {alpha_first, 2, 2};
    HYPResolvedCallArray second = {beta_first, 2, 2};

    const HYPResolvedCall *first_hit = hyp_pipeline_find_lsp_reference(&first, &usage, false);
    const HYPResolvedCall *second_hit = hyp_pipeline_find_lsp_reference(&second, &usage, false);
...
TEST(lsp_reference_duplicate_tail_rows_same_target_are_not_ambiguous) {
    HYPUsage usage = make_reference_usage("proj.raw.Service.handle", "handler", 1200, 1207);
    HYPResolvedCall low =
    ...
    ASSERT(hyp_pipeline_find_lsp_reference(&arr, &usage, true) == &items[1]);
    PASS();
}

TEST(lsp_reference_distinct_tail_targets_are_ambiguous) {
    HYPUsage usage = make_reference_usage("proj.raw.Service.handle", "handler", 1300, 1307);
    HYPResolvedCall alpha =
    ...
    ASSERT_NULL(hyp_pipeline_find_lsp_reference(&arr, &usage, true));
    PASS();
}
...
TEST(lsp_target_node_supports_long_prefixed_qualified_name) {
    ...
    ASSERT_NOT_NULL(gbuf);
    int64_t id =
        hyp_gbuf_upsert_node(gbuf, "Function", "target", qualified, "target.c", 1, 1, "{}");
    ASSERT_GT(id, 0);
    const hyp_gbuf_node_t *found = hyp_pipeline_lsp_target_node(gbuf, project, callee, false);
    if (!found) {
