 ...
 * ──────────────────────
 * The hybrid LSP TYPE-RESOLUTION PASS (pass_lsp_cross.c / hyp_run_X_lsp_cross)
 * is wired for: Go, C, C++, CUDA, Python, JavaScript, TypeScript, TSX, PHP.
 * It is NOT wired (hyp_pxc_has_cross_lsp returns false) for:
 *   - Rust   — hyp_run_rust_lsp_cross EXISTS in rust_lsp.c but is never called
 *   - C#     — hyp_run_cs_lsp_cross EXISTS in cs_lsp.c but is never called
 *   - Java   — hyp_run_java_lsp_cross EXISTS in java_lsp.h but is never called
 *   - Kotlin — NO cross-file LSP function; uses per-file hyp_run_kotlin_lsp only
 *
...
static hyp_store_t *lrp_open_indexed(LRP_Proj *lp) {
     ...
     * last one. */
    free(lp->project);
    lp->project = hyp_project_name_from_path(lp->tmpdir);
    if (!lp->project)
        return NULL;
    ...
    if (!home)
        home = "/tmp";
    char cache_dir[512];
    snprintf(cache_dir, sizeof(cache_dir), "%s/.cache/hyponoia", home);
    hyp_mkdir(cache_dir);
    snprintf(lp->dbpath, sizeof(lp->dbpath), "%s/%s.db", cache_dir, lp->project);
    unlink(lp->dbpath);
    lp->srv = hyp_mcp_server_new(NULL);
    if (!lp->srv)
        return NULL;
    char args[700];
    snprintf(args, sizeof(args), "{\"repo_path\":\"%s\"}", lp->tmpdir);
    char *resp = hyp_mcp_handle_tool(lp->srv, "index_repository", args);
    if (resp)
        free(resp);
    return hyp_store_open_path(lp->dbpath);
}

static hyp_store_t *lrp_index(LRP_Proj *lp, const LRP_File *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/hyp_lrp_XXXXXX");
    if (!hyp_mkdtemp(lp->tmpdir))
        return NULL;
...
static void lrp_cleanup(LRP_Proj *lp, hyp_store_t *store) {
    if (store)
        hyp_store_close(store);
    if (lp->srv) {
        hyp_mcp_server_free(lp->srv);
        lp->srv = NULL;
    }
    free(lp->project);
    lp->project = NULL;
...
static int lrp_usage(LRP_Proj *lp, const LRP_File *files, int nfiles) {
    hyp_store_t *store = lrp_index(lp, files, nfiles);
    int n = store ? hyp_store_count_edges_by_type(store, lp->project, "USAGE") : -1;
...
static void lrp_diag(hyp_store_t *store, const char *project, const char *scenario) {
    if (!store) {
    ...
    for (int i = 0; LRP_ALL_EDGE_TYPES[i]; i++) {
        int c = hyp_store_count_edges_by_type(store, project, LRP_ALL_EDGE_TYPES[i]);
...
static int lrp_assert_calls(const LRP_File *files, int nfiles, int min_calls, const char *scenario,
    ...
    LRP_Proj lp;
    hyp_store_t *store = lrp_index(&lp, files, nfiles);
    int got = store ? hyp_store_count_edges_by_type(store, lp.project, "CALLS") : -1;
    if (got < min_calls) {
...
static int lrp_exact_edge_by_name(hyp_store_t *store, const char *project, const char *edge_type,
                                  const char *source_name, const char *target_name) {
    if (!store || !project || !edge_type || !source_name || !target_name) {
        return -1;
    }

    hyp_node_t *sources = NULL;
    hyp_node_t *targets = NULL;
    int source_count = 0;
    int target_count = 0;
    if (hyp_store_find_nodes_by_name(store, project, source_name, &sources, &source_count) !=
            HYP_STORE_OK ||
        hyp_store_find_nodes_by_name(store, project, target_name, &targets, &target_count) !=
            HYP_STORE_OK ||
        source_count != 1 || target_count != 1) {
        hyp_store_free_nodes(sources, source_count);
        hyp_store_free_nodes(targets, target_count);
        return -1;
    }
    ...
    int edge_count = 0;
    if (hyp_store_find_edges_by_source_type(store, source_id, edge_type, &edges, &edge_count) !=
        HYP_STORE_OK) {
...
static int lrp_exact_calls_by_name(hyp_store_t *store, const char *project, const char *source_name,
                                   const char *target_name) {
    return lrp_exact_edge_by_name(store, project, "CALLS", source_name, target_name);
}

static int lrp_exact_edge_by_qn_suffix(hyp_store_t *store, const char *project,
    ...
    int target_count = 0;
    if (!store ||
        hyp_store_find_nodes_by_qn_suffix(store, project, source_suffix, &sources, &source_count) !=
            HYP_STORE_OK ||
        hyp_store_find_nodes_by_qn_suffix(store, project, target_suffix, &targets, &target_count) !=
            HYP_STORE_OK ||
        source_count != 1 || target_count != 1) {
        hyp_store_free_nodes(sources, source_count);
        hyp_store_free_nodes(targets, target_count);
    ...
    int edge_count = 0;
    int matches = -1;
    if (hyp_store_find_edges_by_source_type(store, sources[0].id, edge_type, &edges, &edge_count) ==
        HYP_STORE_OK) {
        matches = 0;
        for (int i = 0; i < edge_count; i++) {
 ...
 * ══════════════════════════════════════════════════════════════════════
 * C has lsp_cross via hyp_run_c_lsp_cross.  Function-pointer dispatch,
...
TEST(lrp_c_s4_static_local) {
    ...
    LRP_Proj lp;
    hyp_store_t *store = lrp_index(&lp, f, 2);
    int calls = store ? hyp_store_count_edges_by_type(store, lp.project, "CALLS") : -1;
    lrp_cleanup(&lp, store);
...
TEST(lrp_cpp_s4_static_method) {
     ...
     * internal/hyp/lsp (hyp_run_c_lsp_cross, cpp path). */
...
TEST(lrp_cpp_s6_virtual_inherited) {
     ...
     * scope-qualified calls (internal/hyp/lsp, hyp_run_c_lsp_cross cpp path). */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * hyp_run_rust_lsp_cross EXISTS in rust_lsp.c but hyp_pxc_has_cross_lsp
 * returns false for HYP_LANG_RUST — the pass is NEVER called by the
 ...
 * outcome; they FAIL until hyp_pxc_has_cross_lsp is updated to include Rust).
 *
 * ROOT CAUSE: pass_lsp_cross.c hyp_pxc_has_cross_lsp() switch missing
 *   HYP_LANG_RUST case.
 ...
 * NOT resolve → calls=0.  hyp_registry_resolve (src/pipeline/registry.c:638)
 ...
 * lsp_cross is not wired (pass_lsp_cross.c:hyp_pxc_has_cross_lsp lacks
 * HYP_LANG_RUST).  Fix either the '::' split or wire Rust lsp_cross. */
...
TEST(lrp_rust_s2_method_dispatch) {
     ...
     * Root cause: hyp_pxc_has_cross_lsp returns false for HYP_LANG_RUST.
...
TEST(lrp_rust_s7_generic) {
     ...
     * `algo::max_of(...)` is not resolved → calls=0.  hyp_registry_resolve
     ...
     * (pass_lsp_cross.c:hyp_pxc_has_cross_lsp lacks HYP_LANG_RUST). */
 ...
 * HYPCall carrier before pass_calls can materialize the exact graph edge. */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * Python has hyp_run_py_lsp_cross.  Type inference is limited by duck
 ...
 * ══════════════════════════════════════════════════════════════════════
 * TypeScript has hyp_run_ts_lsp_cross.  Static types make S2–S8 more
...
TEST(lrp_ts_s3_constructor) {
     ...
     * Python constructor path in internal/hyp/lsp vs the TS new_expression path. */
...
TEST(lrp_ts_s6_inherited_method) {
    ...
    int calls = hyp_store_count_edges_by_type(store, lp.project, "CALLS");
    lrp_cleanup(&lp, store);
    ASSERT_EQ(calls, 0);
    PASS();
}
 ...
 * ══════════════════════════════════════════════════════════════════════
 * hyp_run_java_lsp_cross EXISTS (java_lsp.h) but hyp_pxc_has_cross_lsp
 * returns false for HYP_LANG_JAVA.
 * S1 (plain call via Util.method()): GREEN via name-based resolver.
 * S2–S8: RED until hyp_pxc_has_cross_lsp is updated.
 *
 * ROOT CAUSE: pass_lsp_cross.c hyp_pxc_has_cross_lsp() missing HYP_LANG_JAVA.
 * FIX LOCATION: src/pipeline/pass_lsp_cross.c line ~280.
 */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * Kotlin has hyp_run_kotlin_lsp (per-file) but NO hyp_run_kotlin_lsp_cross.
 ...
 * hyp_run_kotlin_lsp is never invoked with cross-file project-wide defs.
 * FIX LOCATION: internal/hyp/lsp/kotlin_lsp.c — add hyp_run_kotlin_lsp_cross,
 ...
 * Kotlin has no hyp_run_kotlin_lsp_cross and the generic name resolver does not
 * link this Kotlin cross-file call.  Fix = add Kotlin cross-file LSP
 * (internal/hyp/lsp/kotlin_lsp.c) and wire it in pass_lsp_cross.c. */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * hyp_run_cs_lsp_cross EXISTS (cs_lsp.c) and the HYPCrossLspRegistries
 * struct even has a `cs` field — but hyp_pxc_has_cross_lsp returns false
 * for HYP_LANG_CSHARP.  The cross-LSP pass is never invoked for C# files.
 *
 * ROOT CAUSE: pass_lsp_cross.c hyp_pxc_has_cross_lsp() missing HYP_LANG_CSHARP.
 * FIX LOCATION: src/pipeline/pass_lsp_cross.c line ~280.
 * NOTE: HYPCrossLspRegistries.cs is already defined and hyp_pxc_registry_for_lang
 * handles CSHARP — so the fix is a 1-line addition to hyp_pxc_has_cross_lsp().
 */
 ...
 * ══════════════════════════════════════════════════════════════════════
 * PHP has hyp_run_php_lsp_cross.  Key scenarios: $this->method(),
...
static int lrp_ts_imported_type_with_decoy(const char *main_filename) {
    ...
    LRP_Proj lp;
    hyp_store_t *store = lrp_index(&lp, files, 3);
...
SUITE(lsp_resolution_probe) {
    ...
    /* ── C# (lsp_cross NOT WIRED — but hyp_run_cs_lsp_cross EXISTS) ── */
