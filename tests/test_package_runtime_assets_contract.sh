...
# "Windows test jobs must install zip" was asserted against
# .github/workflows/_test.yml. There are no workflows any more (owner's
# instruction: no GitHub Actions at all), and Windows is a retired platform on
# top of that, so the assertion has neither a subject nor a platform. The
# packaging requirements above are the part that guards the product and they
# hold in every venue, because there is now only one.
