...
#include "../src/foundation/compat.h" /* hyp_setenv / hyp_unsetenv (Windows-portable) */
...
/* Worker staging files land under HYP_CACHE_DIR, which users may place at
...
TEST(platform_file_apis_survive_max_path_overflow) {
    char base[HYP_SZ_512];
    int written = snprintf(base, sizeof(base), "/tmp/hyp-longpath-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(hyp_mkdtemp(base));
    ...
    ASSERT_TRUE(hyp_mkdir_p(deep, 0700));

    char file_path[HYP_SZ_1K];
    written = snprintf(file_path, sizeof(file_path), "%s/store.db", deep);
    ASSERT_TRUE(written > 0 && written < (int)sizeof(file_path));
    FILE *file = hyp_fopen(file_path, "wb");
    ASSERT_NOT_NULL(file);
    ASSERT_GTE(fputs("deep\n", file), 0);
    ASSERT_EQ(fclose(file), 0);
    ASSERT_TRUE(hyp_file_exists(file_path));
    ASSERT_TRUE(hyp_is_dir(deep));
    ASSERT_EQ(hyp_unlink(file_path), 0);
    char cursor[HYP_SZ_1K];
    (void)snprintf(cursor, sizeof(cursor), "%s", deep);
    for (int index = 0; index < LONG_SEGMENTS; index++) {
        ASSERT_EQ(hyp_rmdir(cursor), 0);
...
TEST(platform_mkstemp_and_mkdtemp_survive_non_ascii_directory) {
    char base[HYP_SZ_256];
    int written = snprintf(base, sizeof(base), "/tmp/hyp-utf8-̩̬-XXXXXX");
    ASSERT_TRUE(written > 0 && written < (int)sizeof(base));
    ASSERT_NOT_NULL(hyp_mkdtemp(base));

    char file_template[HYP_SZ_512];
    ...
    int descriptor = hyp_mkstemp(file_template);
...
TEST(platform_counter_scaling_avoids_intermediate_overflow) {
    ...
    ASSERT(hyp_platform_scale_counter_ns(counter, frequency) == UINT64_C(2000000000000));
    ASSERT(hyp_platform_scale_counter_ns(UINT64_MAX, UINT64_MAX) == UINT64_C(1000000000));
    ASSERT(hyp_platform_scale_counter_ns(UINT64_MAX, 1) == UINT64_MAX);
    PASS();
}
...
TEST(platform_counter_scaling_preserves_monotonic_deadlines) {
    ...
    const uint64_t now_ns = hyp_platform_scale_counter_ns(counter, frequency);
    const uint64_t next_ns = hyp_platform_scale_counter_ns(counter + 1, frequency);
    const uint64_t deadline_ns = hyp_platform_scale_counter_ns(counter + 5 * frequency, frequency);
...
static void *platform_time_first_call(void *opaque) {
    ...
    worker->value = hyp_now_ns();
    return NULL;
}

TEST(platform_now_ns_concurrent_first_call) {
    hyp_thread_t threads[PLATFORM_TIME_THREADS];
    ...
    for (; created < PLATFORM_TIME_THREADS; created++) {
        ...
        if (hyp_thread_create(&threads[created], 0, platform_time_first_call, &workers[created]) !=
            0) {
            break;
        }
    }
...
TEST(platform_now_ns) {
    uint64_t t1 = hyp_now_ns();
    ASSERT_GT(t1, 0);
    /* Busy-wait a tiny bit */
    for (volatile int i = 0; i < 100000; i++) {}
    uint64_t t2 = hyp_now_ns();
    ASSERT_GT(t2, t1);
    PASS();
}

TEST(platform_now_ms) {
    uint64_t t1 = hyp_now_ms();
    ASSERT_GT(t1, 0);
    PASS();
}
...
TEST(platform_file_exists) {
    ...
    ASSERT_TRUE(hyp_file_exists("tests/test_platform.c"));
    ASSERT_FALSE(hyp_file_exists("nonexistent_file_xyz.txt"));
    PASS();
}

TEST(platform_is_dir) {
    ASSERT_TRUE(hyp_is_dir("tests"));
    ASSERT_FALSE(hyp_is_dir("tests/test_platform.c"));
    ASSERT_FALSE(hyp_is_dir("nonexistent_dir"));
    PASS();
}

TEST(platform_file_size) {
    int64_t sz = hyp_file_size("tests/test_platform.c");
    ASSERT_GT(sz, 0);
    ASSERT_EQ(hyp_file_size("nonexistent_file_xyz.txt"), -1);
    PASS();
}
...
TEST(platform_mmap) {
    ...
    size_t sz = 0;
    void *data = hyp_mmap_read("tests/test_platform.c", &sz);
...
TEST(platform_mmap_nonexistent) {
    size_t sz = 0;
    void *data = hyp_mmap_read("nonexistent_xyz.txt", &sz);
    ASSERT_NULL(data);
    PASS();
}
...
static void *platform_capture_path_buffers(void *opaque) {
    ...
    result->home = hyp_get_home_dir();
    result->cache = hyp_resolve_cache_dir();
...
TEST(platform_path_helpers_use_per_thread_storage) {
    ...
    ASSERT_EQ(hyp_setenv("HOME", "/tmp/hyp-platform-thread-home", 1), 0);
    ASSERT_EQ(hyp_setenv("HYP_CACHE_DIR", "/tmp/hyp-platform-thread-cache", 1), 0);
    ...
    bool started0 =
        hyp_thread_create(&threads[0], 0, platform_capture_path_buffers, &results[0]) == 0;
    bool started1 =
        hyp_thread_create(&threads[1], 0, platform_capture_path_buffers, &results[1]) == 0;
    ...
    if (saved_home_copy) {
        (void)hyp_setenv("HOME", saved_home_copy, 1);
    } else {
        (void)hyp_unsetenv("HOME");
    }
    if (saved_cache_copy) {
        (void)hyp_setenv("HYP_CACHE_DIR", saved_cache_copy, 1);
    } else {
...
TEST(platform_cache_dir_rejects_truncated_override) {
    ...
    value[length] = '\0';
    ASSERT_EQ(hyp_setenv("HYP_CACHE_DIR", value, 1), 0);

    const char *resolved = hyp_resolve_cache_dir();
...
#ifdef _WIN32
/* hyp_safe_getenv reads Windows' wide environment as UTF-8. Its matching
...
TEST(platform_setenv_preserves_utf8_in_wide_environment) {
    static const char utf8[] = "C:/hyp-cache-\xce\x94-\xe6\x97\xa5\xe6\x9c\xac";
    static const wchar_t wide[] = L"C:/hyp-cache-\u0394-\u65e5\u672c";
    ASSERT_EQ(hyp_setenv("HYP_CACHE_DIR", utf8, 1), 0);
    wchar_t observed_wide[128];
    ASSERT_EQ(GetEnvironmentVariableW(L"HYP_CACHE_DIR", observed_wide, 128), (DWORD)(wcslen(wide)));
    ASSERT_EQ(wcscmp(observed_wide, wide), 0);
    char observed_utf8[128];
    ASSERT_NOT_NULL(hyp_safe_getenv("HYP_CACHE_DIR", observed_utf8, sizeof(observed_utf8), NULL));
 ...
 * particular, an explicitly empty HYP_CACHE_DIR means "use the default"; it
...
TEST(platform_windows_empty_environment_is_read_and_unset_idempotently) {
    ASSERT_EQ(hyp_setenv("HYP_CACHE_DIR", "", 1), 0);
    char observed[9] = "sentinel";
    ASSERT_NOT_NULL(hyp_safe_getenv("HYP_CACHE_DIR", observed, sizeof(observed), "fallback"));
    ASSERT_STR_EQ(observed, "");
    ASSERT_EQ(hyp_unsetenv("HYP_CACHE_DIR"), 0);
    ASSERT_EQ(hyp_unsetenv("HYP_CACHE_DIR"), 0);
    PASS();
}
#endif
...
/*
 * HYP_WORKERS env override for hyp_default_worker_count.
 *
 * Containers running hyp on a host with more CPUs than the cgroup's
...
TEST(platform_default_workers_env_invalid) {
    ...
    int baseline = hyp_default_worker_count(true);
    ASSERT_GT(baseline, 0);

    hyp_setenv("HYP_WORKERS", "0", 1);
    ASSERT_EQ(hyp_default_worker_count(true), baseline);

    hyp_setenv("HYP_WORKERS", "-1", 1);
    ASSERT_EQ(hyp_default_worker_count(true), baseline);

    hyp_setenv("HYP_WORKERS", "9999", 1);
    ASSERT_EQ(hyp_default_worker_count(true), baseline);

    hyp_setenv("HYP_WORKERS", "not-a-number", 1);
    ASSERT_EQ(hyp_default_worker_count(true), baseline);
...
TEST(platform_default_workers_env_unset) {
    /* When HYP_WORKERS is unset the result matches today's behaviour
     * (info.total_cores for initial=true, perf_cores-1 for false). */
    hyp_unsetenv("HYP_WORKERS");
    hyp_system_info_t info = hyp_system_info();
    ASSERT_EQ(hyp_default_worker_count(true), info.total_cores);
    PASS();
}
...
static int cgroup_test_setup(char *root, size_t root_sz) {
    strncpy(root, "/tmp/hyp_cgroup_test_XXXXXX", root_sz);
...
TEST(cgroup_v2_cpu_quota) {
    ...
    ASSERT_EQ(hyp_detect_cgroup_cpus(root), 2);
...
TEST(cgroup_v2_cpu_unlimited) {
    ...
    ASSERT_EQ(hyp_detect_cgroup_cpus(root), -1);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem) {
    ...
    ASSERT_EQ(hyp_detect_cgroup_mem(root), (size_t)2147483648UL);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v2_mem_unlimited) {
    ...
    ASSERT_EQ(hyp_detect_cgroup_mem(root), (size_t)0);
    cgroup_test_teardown(root);
    PASS();
}
...
TEST(cgroup_v1_mem) {
    ...
    ASSERT_EQ(hyp_detect_cgroup_mem(root), (size_t)1073741824UL);
    cgroup_test_teardown(root);
    PASS();
}
