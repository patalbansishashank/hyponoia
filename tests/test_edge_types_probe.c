...
static hyp_store_t *et_index_files(EtProj *lp, const EtFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/hyp_et_XXXXXX");
...
static void et_cleanup(EtProj *lp, hyp_store_t *store) {
...
static int et_edge_present(const EtFile *files, int nfiles, const char *edge, int floor) {
    EtProj lp;
    hyp_store_t *store = et_index_files(&lp, files, nfiles);
...
static int et_routes_exact(const EtFile *files, int nfiles, const char **routes) {
    ...
    if (!store || hyp_store_find_nodes_by_label(store, lp.project, "Route", &nodes, &node_count) !=
                      HYP_STORE_OK) {
        ok = 0;
    } else {
...
static hyp_store_t *et_index_parallel(EtProj *lp, const EtFile *meaningful, int n_mean) {
...
static int et_calls_to_name_parallel(const EtFile *meaningful, int n_mean, const char *name) {
    EtProj lp;
    hyp_store_t *store = et_index_parallel(&lp, meaningful, n_mean);
    int hits = 0;
    if (store) {
        hyp_edge_t *edges = NULL;
        int n = 0;
        if (hyp_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &n) == HYP_STORE_OK) {
            for (int i = 0; i < n; i++) {
                hyp_node_t tgt;
 ...
 *  resolver (pass_calls.c → hyp_service_pattern_match → HYP_SVC_ROUTE_REG) fires.
 ...
 * fix, GREEN with it). Direction-agnostic count via hyp_store_count_edges_by_type. */
 ...
 * sequential resolver classifies the call as HYP_SVC_ROUTE_REG and emits
 ...
 * hyp_service_pattern_match as ROUTE_REG or the handler-arg (an identifier
 ...
 * REAL BUG: internal/hyp/extract_calls.c:extract_handler_arg only accepts an
 ...
 * routed through hyp_service_pattern_match for HTTP classification (the C# lsp
 ...
 * "Client.get" — it drops the namespace/path, so hyp_service_pattern_match never
 ...
 * resolve a `::`-qualified cross-file path (hyp_registry_resolve splits on '.',
 ...
 *     ("aws-sdk-go/service/sqs"), but hyp_fqn_compute (internal/hyp/helpers.c)
...
TEST(throws_java) {
    ...
    int throws = store ? hyp_store_count_edges_by_type(store, lp.project, "THROWS") : -1;
    if (throws < 1) {
 ...
 * internal/hyp/extract_semantic.c:resolve_exception_name does not recover the
...
TEST(raises_python) {
    ...
    hyp_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int raises = store ? hyp_store_count_edges_by_type(store, lp.project, "RAISES") : -1;
    if (raises < 1) {
 ...
 * internal/hyp/extract_semantic.c:resolve_exception_name fails to extract the
 ...
 *    Variable nodes ARE created (hyp_gbuf_upsert_node, "Variable" label) but are
 *    NEVER added to the resolver registry — both register_and_link_def()
 *    (src/pipeline/pass_parallel.c:781) and process_def()
 *    (src/pipeline/pass_definitions.c:262) only hyp_registry_add() the labels
 *    Function/Method/Class/Interface.  resolve_file_rw() resolves the written
 *    var via hyp_registry_resolve(var_name) → always empty → no target node →
...
TEST(writes_python) {
    ...
    hyp_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int writes = store ? hyp_store_count_edges_by_type(store, lp.project, "WRITES") : -1;
    if (writes < 1) {
 ...
 * (internal/hyp/extract_defs.c:2042-2047) but def.parent_class is NEVER derived
 ...
 *  Produced by pass_semantic.c:hyp_pipeline_implements_go().
 *  Parallel-path only (> 50 files).
 *
 *  REAL BUG (same root cause as defines_method_go): hyp_pipeline_implements_go
...
TEST(override_go_interface) {
    ...
    hyp_store_t *store =
        et_index_parallel(&lp, meaningful, (int)(sizeof(meaningful) / sizeof(meaningful[0])));
    int override_edges = store ? hyp_store_count_edges_by_type(store, lp.project, "OVERRIDE") : -1;
    int implements     = store ? hyp_store_count_edges_by_type(store, lp.project, "IMPLEMENTS") : -1;
