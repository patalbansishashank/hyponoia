...
for path in build/c build/linux-arm64 build/linux-amd64 build/win-cross; do
    hyp_require_safe_build_dir "$path"
done
...
for path in \
    ...
    build/nested/c \
    'build\..\outside'; do
    if hyp_require_safe_build_dir "$path" >/dev/null 2>&1; then
...
symlink_fixture="$(mktemp -d "${TMPDIR:-/tmp}/hyp-build-dir-symlink.XXXXXX")"
...
if make_real_dir_link "$symlink_fixture/outside" "$symlink_fixture/repo/build"; then
    if (
        ...
        hyp_require_safe_build_dir build/c "$symlink_fixture/repo"
    ); then
...
if make_real_dir_link "$symlink_fixture/final-target" "$symlink_fixture/safe-repo/build/c"; then
    if ! hyp_remove_build_dir "$symlink_fixture/safe-repo" build/c; then
...
hyp_remove_build_dir "$symlink_fixture/safe-repo" build/ordinary
...
if make -s -f "$ROOT/Makefile.hyp" clean-c BUILD_DIR=build/nested/c >/dev/null 2>&1; then
