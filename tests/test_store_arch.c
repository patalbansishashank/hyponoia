...
static hyp_store_t *setup_arch_test_store(void) {
    ...
    hyp_store_upsert_node(s, &pkg1);
    hyp_store_upsert_node(s, &pkg2);
    hyp_store_upsert_node(s, &pkg3);

    /* Functions with 4-segment QNs */
    hyp_node_t fn_main = {.project = "test",
                          .label = "Function",
                          .name = "main",
                          .qualified_name = "test.cmd.server.main",
                          .file_path = "cmd/server/main.go",
                          .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_main = hyp_store_upsert_node(s, &fn_main);

    hyp_node_t fn_handle = {.project = "test",
                            .label = "Function",
                            .name = "HandleRequest",
                            .qualified_name = "test.internal.handler.HandleRequest",
                            .file_path = "internal/handler/handler.go",
                            .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_handle = hyp_store_upsert_node(s, &fn_handle);

    hyp_node_t fn_process = {.project = "test",
                             .label = "Function",
                             .name = "ProcessOrder",
                             .qualified_name = "test.internal.service.ProcessOrder",
                             .file_path = "internal/service/service.go"};
    int64_t id_process = hyp_store_upsert_node(s, &fn_process);

    hyp_node_t fn_validate = {.project = "test",
                              .label = "Function",
                              .name = "ValidateOrder",
                              .qualified_name = "test.internal.service.ValidateOrder",
                              .file_path = "internal/service/service.go"};
    int64_t id_validate = hyp_store_upsert_node(s, &fn_validate);

    hyp_node_t fn_helper = {.project = "test",
                            .label = "Function",
                            .name = "formatDate",
                            .qualified_name = "test.internal.service.formatDate",
                            .file_path = "internal/service/service.go"};
    int64_t id_helper = hyp_store_upsert_node(s, &fn_helper);

    /* Test function (should be excluded) */
    hyp_node_t fn_test = {.project = "test",
                          .label = "Function",
                          ...
                          .properties_json = "{\"is_entry_point\":true}"};
    int64_t id_test = hyp_store_upsert_node(s, &fn_test);
...
TEST(arch_get_all) {
    hyp_store_t *s = setup_arch_test_store();
    hyp_architecture_info_t info;
    ASSERT_EQ(hyp_store_get_architecture(s, "test", NULL, NULL, 0, &info), HYP_STORE_OK);
    ...
    hyp_store_architecture_free(&info);
    hyp_store_close(s);
    PASS();
}
...
TEST(arch_entry_points_exclude_tests) {
    ...
    ASSERT_EQ(hyp_store_get_architecture(s, "test", NULL, aspects, 1, &info), HYP_STORE_OK);
...
TEST(arch_specific_aspects) {
    ...
    ASSERT_EQ(hyp_store_get_architecture(s, "test", NULL, aspects, 2, &info), HYP_STORE_OK);
...
TEST(arch_path_scoping) {
    hyp_store_t *s = hyp_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(hyp_store_upsert_project(s, "pscope", "/tmp/pscope"), HYP_STORE_OK);

    hyp_node_t f1 = {.project = "pscope",
                     .label = "File",
                     .name = "a.go",
                     .qualified_name = "pscope.apps.foo.a.go",
                     .file_path = "apps/foo/a.go"};
    hyp_node_t f2 = {.project = "pscope",
                     .label = "File",
                     .name = "b.go",
                     .qualified_name = "pscope.other.b.go",
                     .file_path = "other/b.go"};
    hyp_store_upsert_node(s, &f1);
    hyp_store_upsert_node(s, &f2);

    hyp_node_t fn_foo = {.project = "pscope",
                         .label = "Function",
                         .name = "Foo",
                         .qualified_name = "pscope.apps.foo.Foo",
                         .file_path = "apps/foo/a.go"};
    hyp_node_t fn_other = {.project = "pscope",
                           .label = "Function",
                           .name = "Bar",
                           .qualified_name = "pscope.other.Bar",
                           .file_path = "other/b.go"};
    hyp_store_upsert_node(s, &fn_foo);
    hyp_store_upsert_node(s, &fn_other);

    const char *aspects[] = {"languages", "packages"};
    hyp_architecture_info_t whole;
    memset(&whole, 0, sizeof(whole));
    ASSERT_EQ(hyp_store_get_architecture(s, "pscope", NULL, aspects, 2, &whole), HYP_STORE_OK);

    hyp_architecture_info_t scoped;
    memset(&scoped, 0, sizeof(scoped));
    ASSERT_EQ(hyp_store_get_architecture(s, "pscope", "apps/foo", aspects, 2, &scoped),
              HYP_STORE_OK);
    ...
    ASSERT_TRUE(hyp_store_count_nodes(s, "pscope") > hyp_store_count_nodes_scoped(s, "pscope", "apps/foo"));

    hyp_architecture_info_t scoped_slash;
    memset(&scoped_slash, 0, sizeof(scoped_slash));
    ASSERT_EQ(hyp_store_get_architecture(s, "pscope", "apps/foo/", aspects, 2, &scoped_slash),
              HYP_STORE_OK);
    int slash_go = 0;
    ...
    hyp_store_architecture_free(&scoped_slash);
    hyp_store_architecture_free(&whole);
    hyp_store_architecture_free(&scoped);
    hyp_store_close(s);
    PASS();
}
...
TEST(arch_empty_project) {
    hyp_store_t *s = hyp_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(hyp_store_upsert_project(s, "empty", "/tmp/empty"), HYP_STORE_OK);

    hyp_architecture_info_t info;
    const char *aspects[] = {"all"};
    ASSERT_EQ(hyp_store_get_architecture(s, "empty", NULL, aspects, 1, &info), HYP_STORE_OK);
...
static double timed_boundaries_ms(int n_nodes, int n_edges, int n_pkgs) {
    ...
    hyp_store_upsert_project(s, "perf", "/tmp/perf");
    ...
    int rc = hyp_store_get_architecture(s, "perf", NULL, aspects, 1, &info);
...
TEST(adr_store_and_retrieve) {
    hyp_store_t *s = hyp_store_open_memory();
    ASSERT_NOT_NULL(s);
    ASSERT_EQ(hyp_store_upsert_project(s, "test", "/tmp/test"), HYP_STORE_OK);

    const char *content = "## PURPOSE\nTest project for unit tests.\n\n## STACK\n- Go: speed";
    ASSERT_EQ(hyp_store_adr_store(s, "test", content), HYP_STORE_OK);

    hyp_adr_t adr;
    ASSERT_EQ(hyp_store_adr_get(s, "test", &adr), HYP_STORE_OK);
...
TEST(adr_upsert) {
    ...
    ASSERT_EQ(hyp_store_adr_store(s, "test", "v1"), HYP_STORE_OK);
    ASSERT_EQ(hyp_store_adr_store(s, "test", "v2"), HYP_STORE_OK);
...
TEST(adr_delete) {
    ...
    ASSERT_EQ(hyp_store_adr_store(s, "test", "## PURPOSE\nTest"), HYP_STORE_OK);
    ASSERT_EQ(hyp_store_adr_delete(s, "test"), HYP_STORE_OK);

    hyp_adr_t adr;
    ASSERT_TRUE(hyp_store_adr_get(s, "test", &adr) != HYP_STORE_OK);
...
TEST(adr_delete_not_found) {
    ...
    ASSERT_TRUE(hyp_store_adr_delete(s, "nonexistent") != HYP_STORE_OK);
...
TEST(adr_parse_sections_basic) {
    hyp_adr_sections_t sec = hyp_adr_parse_sections("## PURPOSE\nFoo\n\n## STACK\nBar");
    ...
    hyp_adr_sections_free(&sec);
    PASS();
}

TEST(adr_parse_sections_all_six) {
    hyp_adr_sections_t sec =
        hyp_adr_parse_sections("## PURPOSE\nA\n\n## STACK\nB\n\n## ARCHITECTURE\nC\n\n## "
                               "PATTERNS\nD\n\n## TRADEOFFS\nE\n\n## PHILOSOPHY\nF");
...
TEST(adr_parse_sections_empty) {
    hyp_adr_sections_t sec = hyp_adr_parse_sections("");
...
TEST(adr_parse_sections_preamble) {
    hyp_adr_sections_t sec = hyp_adr_parse_sections("preamble\n## PURPOSE\nFoo");
...
TEST(adr_render_all_sections) {
    hyp_adr_sections_t sec = {.count = 6};
...
TEST(adr_render_non_canonical) {
    hyp_adr_sections_t sec = {.count = 3};
...
TEST(adr_render_empty) {
    hyp_adr_sections_t sec = {.count = 0};
    char *rendered = hyp_adr_render(&sec);
...
TEST(adr_parse_render_roundtrip) {
    const char *original =
        ...
        "## TRADEOFFS\n- Speed over features\n\n## PHILOSOPHY\n- Keep it simple";
    hyp_adr_sections_t sec = hyp_adr_parse_sections(original);
    char *rendered = hyp_adr_render(&sec);
...
TEST(adr_update_sections) {
    ...
    hyp_adr_t updated;
    ASSERT_EQ(hyp_store_adr_update_sections(s, "test", keys, values, 1, &updated), HYP_STORE_OK);

    /* Verify all sections preserved */
    hyp_adr_sections_t sec = hyp_adr_parse_sections(updated.content);
...
TEST(adr_update_overflow) {
    ...
    /* Create huge content */
    char *huge = malloc(HYP_ADR_MAX_LENGTH + 2);
    memset(huge, 'x', HYP_ADR_MAX_LENGTH + 1);
    huge[HYP_ADR_MAX_LENGTH + 1] = '\0';

    const char *keys[] = {"STACK"};
    const char *values[] = {huge};
    hyp_adr_t out;
    ASSERT_TRUE(hyp_store_adr_update_sections(s, "test", keys, values, 1, &out) != HYP_STORE_OK);
...
TEST(adr_validate_all_sections) {
    const char *content = "## PURPOSE\nA\n\n## STACK\nB\n\n## ARCHITECTURE\nC\n\n## "
                          "PATTERNS\nD\n\n## TRADEOFFS\nE\n\n## PHILOSOPHY\nF";
    char errbuf[256];
    ASSERT_EQ(hyp_adr_validate_content(content, errbuf, sizeof(errbuf)), HYP_STORE_OK);
    PASS();
}
...
TEST(adr_validate_missing_sections) {
    const char *content = "## PURPOSE\nA\n\n## STACK\nB";
    char errbuf[512];
    ASSERT_TRUE(hyp_adr_validate_content(content, errbuf, sizeof(errbuf)) != HYP_STORE_OK);
...
TEST(adr_validate_empty) {
    char errbuf[256];
    ASSERT_TRUE(hyp_adr_validate_content("", errbuf, sizeof(errbuf)) != HYP_STORE_OK);
    PASS();
}
...
TEST(adr_validate_keys_valid) {
    ...
    char errbuf[256];
    ASSERT_EQ(hyp_adr_validate_section_keys(keys, 2, errbuf, sizeof(errbuf)), HYP_STORE_OK);
    PASS();
}
...
TEST(adr_validate_keys_invalid) {
    ...
    char errbuf[256];
    ASSERT_TRUE(hyp_adr_validate_section_keys(keys, 3, errbuf, sizeof(errbuf)) != HYP_STORE_OK);
...
TEST(adr_validate_keys_empty) {
    char errbuf[256];
    ASSERT_EQ(hyp_adr_validate_section_keys(NULL, 0, errbuf, sizeof(errbuf)), HYP_STORE_OK);
    PASS();
}
...
TEST(louvain_basic) {
    ...
    hyp_louvain_edge_t edges[] = {{1, 2}, {2, 3}, {1, 3}, {4, 5}};
    hyp_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(hyp_louvain(nodes, 5, edges, 4, &result, &count), HYP_STORE_OK);
    ASSERT_EQ(count, 5);
...
TEST(louvain_empty) {
    hyp_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(hyp_louvain(NULL, 0, NULL, 0, &result, &count), HYP_STORE_OK);
    ASSERT_EQ(count, 0);
    free(result);
    PASS();
}
...
TEST(louvain_single_node) {
    ...
    hyp_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(hyp_louvain(nodes, 1, NULL, 0, &result, &count), HYP_STORE_OK);
    ASSERT_EQ(count, 1);
...
TEST(louvain_converges) {
    ...
    for (int i = 1; i <= 10; i++) {
        for (int j = i + 1; j <= 10; j++) {
            edges[nedges++] = (hyp_louvain_edge_t){i, j};
        }
    }
    ...
    /* Bridge */
    edges[nedges++] = (hyp_louvain_edge_t){5, 15};

    hyp_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(hyp_louvain(nodes, 20, edges, nedges, &result, &count), HYP_STORE_OK);
    ASSERT_EQ(count, 20);
...
static int leiden_count_communities(const hyp_louvain_result_t *r, int n) {
...
static bool leiden_all_communities_connected(const hyp_louvain_result_t *r, int n,
                                             const hyp_louvain_edge_t *edges, int ne) {
...
TEST(leiden_multilevel_collapses_noise) {
    ...
    hyp_louvain_edge_t edges[CL * (SZ * (SZ - 1) / 2) + CL];
    int ne = 0;
    for (int c = 0; c < CL; c++) {
        int base = c * SZ + 1;
        for (int i = 0; i < SZ; i++) {
            for (int j = i + 1; j < SZ; j++) {
                edges[ne++] = (hyp_louvain_edge_t){base + i, base + j};
            }
        }
    }
    for (int c = 0; c + 1 < CL; c++) {
        edges[ne++] = (hyp_louvain_edge_t){c * SZ + 1, (c + 1) * SZ + 1};
    }

    hyp_louvain_result_t *result = NULL;
    int count = 0;
    ASSERT_EQ(hyp_louvain(nodes, N, edges, ne, &result, &count), HYP_STORE_OK);
    ASSERT_EQ(count, N);
...
TEST(leiden_resolution_controls_granularity) {
    ...
    hyp_louvain_edge_t edges[N - 1];
    int ne = 0;
    for (int i = 0; i + 1 < N; i++) {
        edges[ne++] = (hyp_louvain_edge_t){i + 1, i + 2};
    }

    hyp_louvain_result_t *lo = NULL;
    hyp_louvain_result_t *hi = NULL;
    int lc = 0;
    int hc = 0;
    ASSERT_EQ(hyp_leiden(nodes, N, edges, ne, 0.1, &lo, &lc), HYP_STORE_OK);
    ASSERT_EQ(hyp_leiden(nodes, N, edges, ne, 5.0, &hi, &hc), HYP_STORE_OK);
    ASSERT_EQ(lc, N);
    ASSERT_EQ(hc, N);
...
TEST(arch_clusters_basic) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        hyp_node_t node = {.project = "test",
                           ...
                           .qualified_name = qn,
                           .file_path = "f.go"};
        id[i] = hyp_store_upsert_node(s, &node);
    }
    for (int g = 0; g < 2; g++) {
        for (int a = 0; a < 4; a++) {
            for (int b = a + 1; b < 4; b++) {
    ...
    hyp_store_insert_edge(s, &bridge);
...
TEST(qn_to_package) {
    ...
    ASSERT_STR_EQ(hyp_qn_to_package("project.internal.store.search.Search"), "store");
    ASSERT_STR_EQ(hyp_qn_to_package("project.src.utils.helper.foo"), "utils");
    ASSERT_STR_EQ(hyp_qn_to_package("project.src.components.Button.render"), "components");
    ASSERT_STR_EQ(hyp_qn_to_package("project.cmd.server.main"), "server");
    /* 3 segments: falls back to segment[1] */
    ASSERT_STR_EQ(hyp_qn_to_package("project.main.foo"), "main");
    ASSERT_STR_EQ(hyp_qn_to_package("project.cmd"), "cmd");
    /* Edge cases */
    ASSERT_STR_EQ(hyp_qn_to_package("standalone"), "");
    ASSERT_STR_EQ(hyp_qn_to_package(""), "");
    PASS();
}

TEST(qn_to_top_package) {
    ASSERT_STR_EQ(hyp_qn_to_top_package("project.internal.store.search.Search"), "internal");
    ASSERT_STR_EQ(hyp_qn_to_top_package("project.src.components.Button"), "src");
    ASSERT_STR_EQ(hyp_qn_to_top_package("project.cmd"), "cmd");
    ASSERT_STR_EQ(hyp_qn_to_top_package("standalone"), "");
    PASS();
}

TEST(is_test_file_path) {
    ASSERT_TRUE(!hyp_is_test_file_path("internal/handler/handler.go"));
    ASSERT_TRUE(hyp_is_test_file_path("src/__tests__/handler.test.ts"));
    ASSERT_TRUE(hyp_is_test_file_path("src/test/java/com/example/Test.java"));
    ASSERT_TRUE(hyp_is_test_file_path("tests/test_handler.py"));
    ASSERT_TRUE(hyp_is_test_file_path("testdata/fixture.json"));
    ASSERT_TRUE(!hyp_is_test_file_path(""));
    PASS();
}
...
TEST(find_architecture_docs) {
    ...
    char **docs = NULL;
    int count = 0;
    ASSERT_EQ(hyp_store_find_architecture_docs(s, "test", &docs, &count), HYP_STORE_OK);
    ASSERT_EQ(count, 2);
...
TEST(search_case_insensitive_default) {
    ...
    ASSERT_EQ(hyp_store_search(s, &params, &out), HYP_STORE_OK);
