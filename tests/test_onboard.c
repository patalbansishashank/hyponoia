/*
 * test_onboard.c — §4 Track B: the probe (B1), the flow (B2), the TOML (B3)
 ...
 *      answer (the assumption §3's retraction forbids) fails the tests below,
...
typedef struct {
     ...
     * DIFFERENT axis than the same document alone — §3's contamination class,
...
static int ob_enc_encode_documents(void *self, const char *const *texts, int count, float *out) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (st->contaminate_batches && count > 1) {
             ...
             * forward pass, which is exactly the §3 failure. */
...
TEST(onboard_probe_gpu_failed_when_the_run_fails) {
    /* The §3 contamination class, injected. A probe that ASSUMED the answer —
