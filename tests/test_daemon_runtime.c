...
#define RUNTIME_TEST_BLOCKING_GIT_MARKER_ENV "HYP_TEST_RUNTIME_BLOCKING_GIT_PID_FILE"

_Static_assert(HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN == 8 &&
...
#endif
static char runtime_self_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE];
...
static hyp_daemon_build_identity_t runtime_test_identity(const char *version, const char *build) {
    hyp_daemon_build_identity_t identity = {
...
static const char *runtime_test_self_build(void) {
    if (runtime_self_build[0] == '\0') {
        (void)hyp_daemon_runtime_process_build_fingerprint(runtime_test_process_id(),
                                                           runtime_self_build);
    }
...
static bool runtime_test_is_fingerprint(const char *value) {
    if (!value || strlen(value) != HYP_DAEMON_BUILD_FINGERPRINT_SIZE - 1) {
        return false;
    }
    for (size_t i = 0; i < HYP_DAEMON_BUILD_FINGERPRINT_SIZE - 1; i++) {
        char ch = value[i];
...
static bool runtime_test_windows_copy_self(const char *destination) {
    ...
    wchar_t *destination_wide = hyp_path_to_wide(destination);
...
static bool runtime_test_windows_posix_replace(const char *source, const char *destination) {
    wchar_t *source_wide = hyp_utf8_to_wide(source);
    wchar_t *destination_wide = hyp_utf8_to_wide(destination);
...
static bool runtime_test_windows_spawn_image_holder(const char *image_path, const char *ready_event,
    ...
    int written = snprintf(command_line, sizeof(command_line),
                           "\"%s\" __hyp_runtime_image_holder \"%s\"", image_path, ready_event);
    wchar_t *application = hyp_path_to_wide(image_path);
    wchar_t *command =
        written > 0 && written < (int)sizeof(command_line) ? hyp_utf8_to_wide(command_line) : NULL;
...
static bool runtime_test_paths_refer_to_same_file(const char *left, const char *right) {
    ...
    if (!left || !right || !hyp_canonical_path(left, left_canonical, sizeof(left_canonical)) ||
        !hyp_canonical_path(right, right_canonical, sizeof(right_canonical))) {
        return false;
    }
#ifdef _WIN32
...
static bool runtime_test_process_image_matches(uint64_t process_id, const char *expected_image) {
    ...
    char *utf8 = queried ? hyp_wide_to_utf8(wide) : NULL;
    if (utf8) {
...
static bool runtime_test_wait_pid_marker(const char *path, uint32_t timeout_ms,
    ...
    uint64_t deadline = hyp_now_ms() + timeout_ms;
    do {
        FILE *marker = hyp_fopen(path, "rb");
    ...
    *exit_code_out = -1;
    ...
    int written =
        snprintf(command_line, sizeof(command_line),
                 "\"%s\" __hyp_runtime_hello_client \"%s\" %s %s %s", image_path, fixture->parent,
                 fixture->key, identity->semantic_version, identity->build_fingerprint);
    wchar_t *application = hyp_utf8_to_wide(image_path);
    wchar_t *command =
        written > 0 && written < (int)sizeof(command_line) ? hyp_utf8_to_wide(command_line) : NULL;
    STARTUPINFOW startup;
    ...
    if (child == 0) {
        ...
        execl(image_path, image_path, "__hyp_runtime_hello_client", fixture->parent, fixture->key,
...
static bool runtime_test_run_activation_image(const char *image_path,
    ...
    *exit_code_out = -1;
    ...
    char command_line[RUNTIME_TEST_PATH_CAP * 3];
    int written = snprintf(command_line, sizeof(command_line),
                           "\"%s\" __hyp_runtime_activation_client \"%s\" %s %s %s %u", image_path,
    ...
    if (child == 0) {
        (void)alarm(TF_RUNTIME_IMAGE_WATCHDOG_SECONDS);
        execl(image_path, image_path, "__hyp_runtime_activation_client", fixture->parent,
...
static bool runtime_test_run_mapped_hello_image(const char *image_path,
    ...
    *exit_code_out = -1;
    pid_t child = fork();
    if (child == 0) {
        execl(image_path, image_path, "__hyp_runtime_mapped_hello_client", mapped_image_path,
              fixture->parent, fixture->key, identity->semantic_version,
...
static hyp_daemon_runtime_application_session_t *runtime_application_session_open(
    void *opaque, hyp_daemon_client_id_t client_id, uint64_t authenticated_process_id) {
    runtime_application_context_t *context = opaque;
    runtime_application_session_t *session = calloc(1, sizeof(*session));
    if (!context || !session || client_id == HYP_DAEMON_CLIENT_ID_INVALID ||
    ...
    return (hyp_daemon_runtime_application_session_t *)session;
}

static hyp_daemon_runtime_application_status_t runtime_application_request(
    void *opaque, hyp_daemon_runtime_application_session_t *opaque_session,
    ...
    *response_length_out = 0;
    ...
    if (request_index == 0 &&
        ...
        return HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED;
    }
    if (request_length == 0) {
        return HYP_DAEMON_RUNTIME_APPLICATION_OK;
    }
...
static void *runtime_application_client_request_thread(void *opaque) {
    ...
    call->status = call->tagged
                       ? hyp_daemon_runtime_client_application_request_tagged(
                       ...
                       : hyp_daemon_runtime_client_application_request(
...
static void *runtime_real_application_detect_changes_thread(void *opaque) {
    ...
    call->status =
        hyp_daemon_application_client_tool(call->client, "detect_changes", call->arguments,
                                           &call->response, &call->response_length, timeout_ms);
    atomic_store_explicit(&call->completed, true, memory_order_release);
    return NULL;
}

static bool runtime_real_application_ingest_probe(hyp_daemon_runtime_client_t *client) {
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    hyp_daemon_runtime_application_status_t status =
        hyp_daemon_application_client_tool(client, "ingest_traces", "{\"traces\":[]}", &response,
                                           &response_length, RUNTIME_TEST_TIMEOUT_MS);
    bool usable = status == HYP_DAEMON_RUNTIME_APPLICATION_OK && response && response_length > 0 &&
...
static void runtime_test_restore_environment(const char *name, const char *saved_value,
                                             bool was_set) {
    if (was_set) {
        (void)hyp_setenv(name, saved_value, 1);
    } else {
        (void)hyp_unsetenv(name);
    }
}

static void *runtime_application_client_connect_thread(void *opaque) {
    runtime_application_connect_call_t *call = opaque;
    call->client = hyp_daemon_runtime_client_connect(call->endpoint, &call->identity,
...
static hyp_daemon_runtime_application_callbacks_t runtime_application_callbacks(
    runtime_application_context_t *context) {
...
static bool runtime_test_wait_atomic_bool(atomic_bool *value, uint32_t timeout_ms) {
    uint64_t deadline = hyp_now_ms() + (uint64_t)timeout_ms;
...
static bool runtime_test_raw_activation_exchange(
    const hyp_daemon_ipc_endpoint_t *endpoint, const uint8_t *request, uint32_t request_length,
    ...
    hyp_daemon_ipc_connection_t *connection =
        hyp_daemon_ipc_connect(endpoint, RUNTIME_TEST_TIMEOUT_MS);
    ...
    uint8_t *response = NULL;
    int received =
        sent ? hyp_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &response)
             : 0;
...
static bool runtime_test_activation_request_encode(
    ...
    return hyp_daemon_runtime_hello_request_encode(out + RUNTIME_TEST_ACTIVATION_IDENTITY_OFFSET,
                                                   identity);
}
...
static hyp_daemon_ipc_connection_t *runtime_test_raw_client_connect(
    const hyp_daemon_ipc_endpoint_t *endpoint, const hyp_daemon_build_identity_t *identity) {
    uint8_t hello[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    if (!hyp_daemon_runtime_hello_request_encode(hello, identity)) {
        return NULL;
    }
    hyp_daemon_ipc_connection_t *connection =
        hyp_daemon_ipc_connect(endpoint, RUNTIME_TEST_TIMEOUT_MS);
    if (!connection ||
        !hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                   HYP_DAEMON_RUNTIME_OP_HELLO, hello, (uint32_t)sizeof(hello))) {
        hyp_daemon_ipc_connection_close(connection);
        return NULL;
    }
    hyp_daemon_frame_t frame = {0};
    ...
    int received =
        hyp_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
    bool accepted = received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                    frame.flags == HYP_DAEMON_RUNTIME_OP_HELLO && frame.length > 0;
    free(payload);
    if (!accepted) {
        hyp_daemon_ipc_connection_close(connection);
        return NULL;
...
static bool runtime_test_raw_application_send_token(hyp_daemon_ipc_connection_t *connection,
    ...
    bool sent = hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_send(hyp_daemon_ipc_connection_t *connection,
...
static bool runtime_test_raw_application_cancel(hyp_daemon_ipc_connection_t *connection,
    ...
    runtime_test_put_u64(wire, request_token);
    return hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
...
static bool runtime_test_raw_application_receive_status_token(
    hyp_daemon_ipc_connection_t *connection, uint64_t expected_token,
    hyp_daemon_runtime_application_status_t expected_status) {
    hyp_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received =
        hyp_daemon_ipc_receive_frame(connection, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
    bool valid = received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                 frame.flags == HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST && frame.length >= 16 &&
...
static bool runtime_test_fixture_start_configured(
    runtime_test_fixture_t *fixture, const char *tag, const hyp_daemon_build_identity_t *identity,
    ...
    if (!hyp_daemon_rendezvous_key(fixture->key)) {
        return runtime_test_fixture_start_failed(tag, "rendezvous-key",
                                                 runtime_test_last_os_error());
    }
    fixture->endpoint = hyp_daemon_ipc_endpoint_new(fixture->key, fixture->parent);
    if (!fixture->endpoint) {
    ...
    if (!runtime_test_copy_path(fixture->runtime_dir,
                                hyp_daemon_ipc_endpoint_runtime_dir(fixture->endpoint))) {
    ...
    hyp_daemon_runtime_service_config_t config = {
    ...
    fixture->service = hyp_daemon_runtime_service_start(&config);
    if (!fixture->service) {
        return runtime_test_fixture_start_failed(tag, "service-start",
                                                 runtime_test_last_os_error());
    }
    hyp_daemon_runtime_service_state_t state = hyp_daemon_runtime_service_state(fixture->service);
    if (state != HYP_DAEMON_RUNTIME_SERVICE_RUNNING) {
...
TEST(daemon_runtime_convenience_service_owns_participant_guard) {
    ...
    int active = started ? hyp_daemon_ipc_legacy_generation_probe(fixture.endpoint) : -1;
    bool running_free_refused = started && !hyp_daemon_runtime_service_free(fixture.service);
    bool stopped =
        started && hyp_daemon_runtime_service_stop(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    bool freed = stopped && hyp_daemon_runtime_service_free(fixture.service);
    if (freed) {
        fixture.service = NULL;
    }
    int released = freed ? hyp_daemon_ipc_legacy_generation_probe(fixture.endpoint) : -1;
...
static bool runtime_test_fixture_start_application(runtime_test_fixture_t *fixture, const char *tag,
                                                   const hyp_daemon_build_identity_t *identity,
                                                   runtime_application_context_t *context) {
    hyp_daemon_runtime_application_callbacks_t application = runtime_application_callbacks(context);
...
static void runtime_test_fixture_finish(runtime_test_fixture_t *fixture) {
    if (fixture->service) {
        hyp_daemon_runtime_service_state_t state =
            hyp_daemon_runtime_service_state(fixture->service);
        bool stopped =
            state == HYP_DAEMON_RUNTIME_SERVICE_EXITED ||
            hyp_daemon_runtime_service_stop(fixture->service, RUNTIME_TEST_CLEANUP_TIMEOUT_MS);
        bool freed = false;
        for (size_t attempt = 0; stopped && !freed && attempt < RUNTIME_TEST_CLEANUP_FREE_ATTEMPTS;
             attempt++) {
            freed = hyp_daemon_runtime_service_free(fixture->service);
            if (!freed) {
                hyp_usleep(1000);
            }
        }
    ...
    (void)hyp_unlink(fixture->rotated_log_path);
    (void)hyp_unlink(fixture->log_path);
    (void)hyp_unlink(fixture->lock_log_path);
...
TEST(daemon_host_early_coordination_failure_is_durable) {
    ...
    int log_written =
        parent_created ? snprintf(log_path, sizeof(log_path), "%s/logs/hyp-daemon.log", cache) : -1;
    bool environment_ready = cache_written > 0 && cache_written < (int)sizeof(cache) &&
                             log_written > 0 && log_written < (int)sizeof(log_path) &&
                             hyp_mkdir_p(cache, 0700) && hyp_setenv("HYP_CACHE_DIR", cache, 1) == 0;
    hyp_daemon_ipc_endpoint_t *endpoint =
        environment_ready ? hyp_daemon_ipc_endpoint_new("0123456789abcdef", parent) : NULL;
    hyp_version_cohort_manager_t *active_manager =
        endpoint ? hyp_version_cohort_manager_new(endpoint) : NULL;
    hyp_version_cohort_lease_t *active_lease = NULL;
    hyp_version_cohort_daemon_claim_t *active_claim = NULL;
    hyp_daemon_conflict_t active_conflict;
    hyp_daemon_build_identity_t active_identity =
        runtime_test_identity("active-host", runtime_test_self_build());
    active_identity.cache_fingerprint = RUNTIME_BUILD_B;
    hyp_version_cohort_status_t active_status =
        active_manager ? hyp_version_cohort_acquire(active_manager, &active_identity, UINT64_MAX,
                                                    &active_lease, &active_conflict)
                       : HYP_VERSION_COHORT_IO;
    hyp_version_cohort_status_t claim_status =
        active_status == HYP_VERSION_COHORT_OK
            ? hyp_version_cohort_daemon_claim_acquire(active_manager, &active_claim)
            : HYP_VERSION_COHORT_IO;
    atomic_int stop_requested = ATOMIC_VAR_INIT(0);
    hyp_daemon_host_config_t config = {
        .endpoint = endpoint,
        .identity = active_identity,
        .executable_path = "/host-early-log-test",
        .stop_requested = &stop_requested,
    };
    int run_result = claim_status == HYP_VERSION_COHORT_OK ? hyp_daemon_host_run(&config) : 0;
    ...
    while (active_claim &&
           hyp_version_cohort_daemon_claim_release(&active_claim) != HYP_PRIVATE_FILE_LOCK_OK) {
        hyp_usleep(1000);
    }
    while (active_lease &&
           hyp_version_cohort_lease_release(&active_lease) != HYP_PRIVATE_FILE_LOCK_OK) {
        hyp_usleep(1000);
    }
    while (active_manager &&
           hyp_version_cohort_manager_free(&active_manager) != HYP_PRIVATE_FILE_LOCK_OK) {
        hyp_usleep(1000);
    }
    hyp_daemon_ipc_endpoint_free(endpoint);
    runtime_test_restore_environment("HYP_CACHE_DIR", saved_cache, had_cache);
    free(saved_cache);
    ...
    ASSERT_EQ(active_status, HYP_VERSION_COHORT_OK);
    ASSERT_EQ(claim_status, HYP_VERSION_COHORT_OK);
...
TEST(daemon_host_refuses_unopenable_runtime_config_database) {
    ...
    bool blocker_created = cache_written > 0 && cache_written < (int)sizeof(cache) &&
                           ...
                           hyp_mkdir_p(cache, 0700) && hyp_mkdir_p(config_blocker, 0700);
    bool environment_ready = blocker_created && hyp_setenv("HYP_CACHE_DIR", cache, 1) == 0;
    hyp_daemon_ipc_endpoint_t *endpoint =
        environment_ready ? hyp_daemon_ipc_endpoint_new("0123456789abcdef", parent) : NULL;
    bool endpoint_created = endpoint != NULL;
    bool prepared = endpoint && hyp_daemon_host_state_prepare_for_test(endpoint);
...
TEST(daemon_host_http_reconcile_rate_limits_and_retries_transient_failures) {
    ...
    hyp_daemon_host_http_reconcile_test_result_t result = {0};
    bool driven = hyp_daemon_host_http_reconcile_sequence_for_test(
...
TEST(daemon_host_http_reconcile_retains_busy_server_until_free_succeeds) {
    hyp_daemon_host_http_free_refusal_test_result_t result = {0};
    bool driven = hyp_daemon_host_http_reconcile_free_refusal_for_test(&result);
...
static int runtime_test_failed_host_child(const char *parent, const char *key) {
    ...
    int log_written = snprintf(log_path, sizeof(log_path), "%s/logs/hyp-daemon.log", cache);
    if (cache_written <= 0 || cache_written >= (int)sizeof(cache) || log_written <= 0 ||
        log_written >= (int)sizeof(log_path) || hyp_setenv("HYP_CACHE_DIR", cache, 1) != 0) {
        return 40;
    }

    hyp_daemon_ipc_endpoint_t *endpoint = hyp_daemon_ipc_endpoint_new(key, parent);
    atomic_int stop_requested = ATOMIC_VAR_INIT(0);
    hyp_daemon_build_identity_t mismatched =
    ...
    int run_result = endpoint ? hyp_daemon_host_run(&config) : 0;
...
TEST(daemon_host_failed_listener_reservation_starts_no_background_work) {
    char parent[RUNTIME_TEST_PATH_CAP];
    int written = snprintf(parent, sizeof(parent), "%s/hyp-host-order-XXXXXX", hyp_tmpdir());
    bool parent_created =
        written > 0 && written < (int)sizeof(parent) && hyp_mkdtemp(parent) != NULL;
    char key[HYP_DAEMON_KEY_SIZE] = {0};
    hyp_daemon_ipc_endpoint_t *endpoint = parent_created && hyp_daemon_rendezvous_key(key)
                                              ? hyp_daemon_ipc_endpoint_new(key, parent)
                                              : NULL;
    hyp_daemon_ipc_startup_lock_t *startup = NULL;
    bool setup = endpoint && hyp_daemon_ipc_startup_lock_try_acquire(endpoint, &startup) == 1 &&
                 hyp_daemon_ipc_startup_lock_prepare_handoff(startup);
    ...
    bool startup_released = hyp_daemon_ipc_startup_lock_release(&startup);
    hyp_daemon_ipc_endpoint_free(endpoint);
...
static _Noreturn void runtime_test_forced_host_shutdown_child(const char *parent) {
    ...
    (void)alarm(2);
    hyp_daemon_host_force_terminate_for_test("noncooperative_callback");
}
...
static _Noreturn void runtime_test_persistent_cleanup_failure_child(const char *parent) {
    ...
    hyp_daemon_host_cleanup_release_until_complete_for_test(
...
TEST(daemon_host_persistent_cleanup_release_failure_is_process_bounded) {
    ...
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written =
        snprintf(parent, sizeof(parent), "%s/hyp-host-cleanup-XXXXXX", hyp_tmpdir());
    bool parent_created =
        parent_written > 0 && parent_written < (int)sizeof(parent) && hyp_mkdtemp(parent) != NULL;
    int log_written = parent_created ? snprintf(log_path, sizeof(log_path),
                                                "%s/cache/logs/hyp-daemon.log", parent)
                                     : -1;
    ...
    uint64_t deadline = hyp_now_ms() + RUNTIME_TEST_TIMEOUT_MS;
    while (child > 0 && hyp_now_ms() < deadline) {
...
TEST(daemon_host_forced_shutdown_is_logged_flushed_and_process_bounded) {
    char parent[RUNTIME_TEST_PATH_CAP];
    char log_path[RUNTIME_TEST_PATH_CAP];
    int parent_written = snprintf(parent, sizeof(parent), "%s/hyp-host-force-XXXXXX", hyp_tmpdir());
    bool parent_created =
        parent_written > 0 && parent_written < (int)sizeof(parent) && hyp_mkdtemp(parent) != NULL;
    int log_written = parent_created ? snprintf(log_path, sizeof(log_path),
                                                "%s/cache/logs/hyp-daemon.log", parent)
...
TEST(daemon_runtime_exact_hello_issues_connection_bound_identity) {
    ...
    hyp_daemon_runtime_connect_result_t result = {0};
    ...
    if (started) {
        ...
        client = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    if (client) {
        uint64_t expected_pid = runtime_test_process_id();
        accepted = result.status == HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
                   result.hello_status == HYP_DAEMON_HELLO_COMPATIBLE &&
                   result.client_id != HYP_DAEMON_CLIENT_ID_INVALID &&
                   result.client_id == hyp_daemon_runtime_client_id(client);
        identity_anchored = result.authenticated_process_id == expected_pid &&
                            hyp_daemon_runtime_client_process_id(client) == expected_pid &&
                            hyp_daemon_runtime_service_client_process_id(
                                fixture.service, result.client_id) == expected_pid;
        closed = hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }

    if (client) {
        (void)hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_image_rejection_reaches_client_issue1383) {
    ...
    hyp_daemon_runtime_force_peer_image_unverified_for_testing(true);
    if (started) {
        client = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &result);
    }
    hyp_daemon_runtime_force_peer_image_unverified_for_testing(false);
...
TEST(daemon_runtime_unexpected_frame_payload_is_freed_once) {
    ...
    hyp_daemon_runtime_connect_result_t owner_result = {0};
    hyp_daemon_runtime_client_t *owner = NULL;
    hyp_daemon_ipc_connection_t *raw = NULL;
    ...
    if (started) {
        owner = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
    ...
    if (raw) {
        unexpected_sent = hyp_daemon_ipc_send_frame(
            raw, HYP_DAEMON_FRAME_RESPONSE, HYP_DAEMON_RUNTIME_OP_HEARTBEAT, unexpected_payload,
            (uint32_t)sizeof(unexpected_payload));
    }
    if (unexpected_sent) {
        bad_peer_released = hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                        RUNTIME_TEST_TIMEOUT_MS);
    }
    if (bad_peer_released) {
        hyp_daemon_frame_t frame = {0};
        uint8_t *payload = NULL;
        int received = hyp_daemon_ipc_receive_frame(raw, RUNTIME_TEST_TIMEOUT_MS, &frame, &payload);
        bad_peer_closed = received != 1;
        free(payload);
        owner_survived = hyp_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    hyp_daemon_ipc_connection_close(raw);
    raw = NULL;
    if (owner) {
        owner_closed = hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    if (started) {
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    if (owner) {
        (void)hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    hyp_daemon_ipc_connection_close(raw);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_activation_rejects_forged_and_malformed_without_stop) {
    hyp_daemon_build_identity_t identity =
        runtime_test_identity("2.4.0", runtime_test_self_build());
    hyp_daemon_build_identity_t forged = runtime_test_identity("9.9.9", RUNTIME_BUILD_B);
    runtime_test_fixture_t fixture;
    ...
    bool encoded = runtime_test_activation_request_encode(
        request, HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE, &identity);
    hyp_daemon_runtime_activation_result_t short_result = {0};
    hyp_daemon_runtime_activation_result_t action_result = {0};
    hyp_daemon_runtime_activation_result_t abi_result = {0};
    hyp_daemon_runtime_activation_result_t forged_result = {0};
    ...
    if (owner && encoded) {
        ...
        runtime_test_put_u32(request, (uint32_t)HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE);
        ...
        abi_rejected = runtime_test_raw_activation_exchange(fixture.endpoint, request,
                                                            ...
                                                            &abi_result) &&
                       !abi_result.accepted;
        forged_rejected = hyp_daemon_runtime_request_activation_shutdown(
                              fixture.endpoint, &forged, HYP_DAEMON_RUNTIME_ACTIVATION_UNINSTALL,
                              RUNTIME_TEST_TIMEOUT_MS, &forged_result) &&
                          !forged_result.accepted;
        unchanged = hyp_daemon_runtime_service_state(fixture.service) ==
                        HYP_DAEMON_RUNTIME_SERVICE_RUNNING &&
                    hyp_daemon_runtime_service_active_clients(fixture.service) == 1 &&
                    hyp_daemon_runtime_service_wait_for_connections(fixture.service, 1,
                                                                    RUNTIME_TEST_TIMEOUT_MS);
        heartbeat = hyp_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        closed = hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    if (started) {
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    ASSERT_TRUE(started);
    ASSERT_EQ(owner_result.status, HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_TRUE(encoded);
...
TEST(daemon_runtime_activation_ack_snapshots_then_interrupts_all_clients) {
    ...
    hyp_daemon_runtime_connect_result_t first_result = {0};
    hyp_daemon_runtime_connect_result_t second_result = {0};
    hyp_daemon_runtime_client_t *first = NULL;
    hyp_daemon_runtime_client_t *second = NULL;
    hyp_daemon_runtime_activation_result_t activation = {0};
    ...
    hyp_log_set_sink(runtime_test_activation_shutdown_sink);

    if (started) {
        first = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &first_result);
        second = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &second_result);
    }
    if (first && second) {
        requested = hyp_daemon_runtime_request_activation_shutdown(
            fixture.endpoint, &identity, HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE,
            RUNTIME_TEST_TIMEOUT_MS, &activation);
        first_interrupted = !hyp_daemon_runtime_client_heartbeat(first, RUNTIME_TEST_TIMEOUT_MS);
        second_interrupted = !hyp_daemon_runtime_client_heartbeat(second, RUNTIME_TEST_TIMEOUT_MS);
    }
    hyp_log_set_sink(NULL);
    if (first) {
        (void)hyp_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
    }
    if (second) {
        (void)hyp_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
    }
    if (started) {
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    ASSERT_TRUE(started);
    ASSERT_EQ(first_result.status, HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_EQ(second_result.status, HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED);
...
TEST(daemon_runtime_activation_accepts_authenticated_different_build) {
    ...
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/hyp-runtime-activation-XXXXXX", hyp_tmpdir());
    bool directory_created = directory_written > 0 && directory_written < (int)sizeof(directory) &&
                             hyp_mkdtemp(directory) != NULL;
...
#endif
    char foreign_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool fingerprinted =
        runnable && hyp_daemon_build_fingerprint_file(foreign_image, foreign_build);
    bool differs = fingerprinted && strcmp(foreign_build, active_identity.build_fingerprint) != 0;
    hyp_daemon_build_identity_t foreign_identity = runtime_test_identity("9.9.9", foreign_build);
    ...
    hyp_daemon_runtime_client_t *owner = NULL;
    if (started) {
        owner = hyp_daemon_runtime_client_connect(fixture.endpoint, &active_identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    int foreign_exit = -1;
    bool foreign_ran =
        ...
        runtime_test_run_activation_image(foreign_image, &fixture, &foreign_identity,
                                          HYP_DAEMON_RUNTIME_ACTIVATION_INSTALL, &foreign_exit);
    bool owner_interrupted = owner && foreign_ran &&
                             !hyp_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    if (owner) {
        (void)hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
    }
    bool exited =
        started && hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    runtime_test_fixture_finish(&fixture);
    if (image_written > 0 && image_written < (int)sizeof(foreign_image)) {
        (void)hyp_unlink(foreign_image);
    }
...
TEST(daemon_runtime_rendezvous_layout_is_frozen_and_detailed_abi_independent) {
    hyp_daemon_build_identity_t first = runtime_test_identity("2.4.0", runtime_test_self_build());
    hyp_daemon_build_identity_t different_detail = first;
    different_detail.protocol_abi = 0;
    different_detail.store_abi = 0;
    different_detail.feature_abi = 0;

    uint8_t first_wire[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    uint8_t second_wire[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    bool first_encoded = hyp_daemon_runtime_hello_request_encode(first_wire, &first);
    bool second_encoded = hyp_daemon_runtime_hello_request_encode(second_wire, &different_detail);

    ASSERT_TRUE(first_encoded);
    ASSERT_TRUE(second_encoded);
    ASSERT_EQ(HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE, RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
    ASSERT_EQ(HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE, RUNTIME_TEST_RENDEZVOUS_RESPONSE_SIZE);
    ASSERT_EQ(HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, HYP_DAEMON_VERSION_TEXT_SIZE);
    ASSERT_EQ(HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, HYP_DAEMON_BUILD_FINGERPRINT_SIZE);
    ASSERT_EQ(HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP, HYP_DAEMON_CONFLICT_MESSAGE_SIZE);
    ...
    ASSERT_TRUE(
        runtime_test_fixed_string_equals(first_wire + RUNTIME_TEST_RENDEZVOUS_VERSION_OFFSET,
                                         HYP_DAEMON_VERSION_TEXT_SIZE, first.semantic_version));
...
TEST(daemon_runtime_future_generation_gets_stable_explicit_conflict) {
    ...
    hyp_daemon_build_identity_t active = runtime_test_identity("2.4.0", active_build);
    hyp_daemon_build_identity_t future =
    ...
    hyp_daemon_runtime_connect_result_t owner_result = {0};
    hyp_daemon_runtime_client_t *owner = NULL;
    hyp_daemon_ipc_connection_t *future_connection = NULL;
    uint8_t future_wire[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    bool encoded = false;
    bool sent = false;
    bool explicit_conflict = false;
    bool logged = false;
    bool exited = false;
    hyp_daemon_frame_t response_frame = {0};
    uint8_t *response_payload = NULL;
    char log[RUNTIME_TEST_LOG_CAP] = {0};
    char expected_message[HYP_DAEMON_CONFLICT_MESSAGE_SIZE] = {0};
    ...
    if (started) {
        owner = hyp_daemon_runtime_client_connect(fixture.endpoint, &active,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    if (owner) {
        encoded = hyp_daemon_runtime_hello_request_encode(future_wire, &future);
        future_connection = hyp_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (future_connection && encoded) {
        ...
        sent = hyp_daemon_ipc_send_frame(future_connection, HYP_DAEMON_FRAME_REQUEST,
                                         HYP_DAEMON_RUNTIME_OP_HELLO, future_wire,
                                         RUNTIME_TEST_RENDEZVOUS_REQUEST_SIZE);
        int received = hyp_daemon_ipc_receive_frame(future_connection, RUNTIME_TEST_TIMEOUT_MS,
                                                    &response_frame, &response_payload);
        explicit_conflict =
            expected_message_valid && received == 1 && response_payload &&
            response_frame.type == HYP_DAEMON_FRAME_RESPONSE &&
            response_frame.flags == HYP_DAEMON_RUNTIME_OP_HELLO &&
            response_frame.length == RUNTIME_TEST_RENDEZVOUS_RESPONSE_SIZE &&
            runtime_test_get_u32(response_payload) == HYP_DAEMON_RUNTIME_CONNECT_CONFLICT &&
            runtime_test_get_u32(response_payload + 4) == HYP_DAEMON_HELLO_VERSION_CONFLICT &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_ACTIVE_VERSION_OFFSET,
                HYP_DAEMON_VERSION_TEXT_SIZE, active.semantic_version) &&
            runtime_test_fixed_string_equals(response_payload +
                                                 RUNTIME_TEST_RENDEZVOUS_ACTIVE_BUILD_OFFSET,
                                             HYP_DAEMON_BUILD_FINGERPRINT_SIZE, active_build) &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_REQUESTED_VERSION_OFFSET,
                HYP_DAEMON_VERSION_TEXT_SIZE, future.semantic_version) &&
            runtime_test_fixed_string_equals(
                response_payload + RUNTIME_TEST_RENDEZVOUS_REQUESTED_BUILD_OFFSET,
                HYP_DAEMON_BUILD_FINGERPRINT_SIZE, future.build_fingerprint) &&
            runtime_test_fixed_string_equals(response_payload +
                                                 RUNTIME_TEST_RENDEZVOUS_MESSAGE_OFFSET,
                                             HYP_DAEMON_CONFLICT_MESSAGE_SIZE, expected_message);
    }
    hyp_daemon_ipc_connection_close(future_connection);
...
TEST(daemon_runtime_matching_clients_share_one_service_endpoint) {
    ...
    hyp_daemon_ipc_endpoint_t *same_endpoint = NULL;
    hyp_daemon_runtime_client_t *first = NULL;
    hyp_daemon_runtime_client_t *second = NULL;
    hyp_daemon_runtime_connect_result_t first_result = {0};
    hyp_daemon_runtime_connect_result_t second_result = {0};
    ...
    char key[HYP_DAEMON_KEY_SIZE];
    if (started && hyp_daemon_rendezvous_key(key)) {
        same_endpoint = hyp_daemon_ipc_endpoint_new(key, fixture.parent);
    }
    if (same_endpoint) {
        same_address = strcmp(hyp_daemon_ipc_endpoint_address(fixture.endpoint),
                              hyp_daemon_ipc_endpoint_address(same_endpoint)) == 0;
        first = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &first_result);
        second = hyp_daemon_runtime_client_connect(same_endpoint, &identity,
                                                   RUNTIME_TEST_TIMEOUT_MS, &second_result);
    }
    if (first && second) {
        both_registered = first_result.client_id != second_result.client_id &&
                          hyp_daemon_runtime_service_active_clients(fixture.service) == 2;
        (void)hyp_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
        one_remains = hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                  RUNTIME_TEST_TIMEOUT_MS);
        (void)hyp_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    hyp_daemon_ipc_endpoint_free(same_endpoint);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_same_version_different_build_is_visible_and_logged) {
    const char *active_build = runtime_test_self_build();
    hyp_daemon_build_identity_t active = runtime_test_identity("2.4.0", active_build);
    hyp_daemon_build_identity_t rebuilt = runtime_test_identity("2.4.0", RUNTIME_BUILD_B);
    runtime_test_fixture_t fixture;
    bool started = runtime_test_fixture_start(&fixture, "build-conflict", &active);
    hyp_daemon_runtime_connect_result_t owner_result = {0};
    hyp_daemon_runtime_connect_result_t conflict_result = {0};
    hyp_daemon_runtime_client_t *owner = NULL;
    hyp_daemon_runtime_client_t *rejected = NULL;
    bool explicit_conflict = false;
    bool logged = false;
    ...
    if (owner) {
        rejected = hyp_daemon_runtime_client_connect(fixture.endpoint, &rebuilt,
                                                     RUNTIME_TEST_TIMEOUT_MS, &conflict_result);
        explicit_conflict = rejected == NULL &&
                            conflict_result.status == HYP_DAEMON_RUNTIME_CONNECT_CONFLICT &&
                            conflict_result.hello_status == HYP_DAEMON_HELLO_BUILD_CONFLICT &&
        ...
        owner_unchanged = hyp_daemon_runtime_service_active_clients(fixture.service) == 1;
        logged = runtime_test_read_log(fixture.log_path, log) &&
    ...
    if (rejected) {
        (void)hyp_daemon_runtime_client_close(rejected, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (owner) {
        (void)hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_conflict_log_failure_uses_operation_log_fallback) {
    ...
    bool obstruction_created = started && hyp_mkdir_p(fixture.log_path, 0700);
    hyp_daemon_runtime_connect_result_t owner_result = {0};
    hyp_daemon_runtime_connect_result_t conflict_result = {0};
    hyp_daemon_runtime_client_t *owner = NULL;
    hyp_daemon_runtime_client_t *rejected = NULL;
    HYPLogLevel prior_level = hyp_log_get_level();
    bool explicit_conflict = false;
    bool exited = false;
    ...
    if (obstruction_created) {
        ...
        hyp_log_set_sink(runtime_test_conflict_log_fallback_sink);
        owner = hyp_daemon_runtime_client_connect(fixture.endpoint, &active,
                                                  RUNTIME_TEST_TIMEOUT_MS, &owner_result);
    }
    if (owner) {
    ...
    if (obstruction_created) {
        (void)hyp_rmdir(fixture.log_path);
    }
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_disconnect_releases_only_connection_subscriptions) {
    ...
    hyp_daemon_runtime_connect_result_t first_result = {0};
    hyp_daemon_runtime_connect_result_t second_result = {0};
    hyp_daemon_subscription_id_t first_subscription = HYP_DAEMON_SUBSCRIPTION_ID_INVALID;
    hyp_daemon_subscription_id_t second_subscription = HYP_DAEMON_SUBSCRIPTION_ID_INVALID;
    ...
    if (first && second) {
        hyp_daemon_subscription_result_t first_status = hyp_daemon_runtime_client_job_subscribe(
            first, project, &first_subscription, RUNTIME_TEST_TIMEOUT_MS);
        hyp_daemon_subscription_result_t second_status = hyp_daemon_runtime_client_job_subscribe(
            second, project, &second_subscription, RUNTIME_TEST_TIMEOUT_MS);
        subscribed = first_status == HYP_DAEMON_SUBSCRIPTION_STARTED &&
                     second_status == HYP_DAEMON_SUBSCRIPTION_JOINED &&
                     first_subscription != HYP_DAEMON_SUBSCRIPTION_ID_INVALID &&
                     second_subscription != HYP_DAEMON_SUBSCRIPTION_ID_INVALID &&
                     first_subscription != second_subscription &&
                     hyp_daemon_runtime_service_job_subscribers(fixture.service, project) == 2;

        (void)hyp_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
        second_survived =
            hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                        RUNTIME_TEST_TIMEOUT_MS) &&
            hyp_daemon_runtime_service_job_subscribers(fixture.service, project) == 1 &&
            hyp_daemon_runtime_client_job_unsubscribe(second, second_subscription,
                                                      RUNTIME_TEST_TIMEOUT_MS) &&
            hyp_daemon_runtime_service_job_subscribers(fixture.service, project) == 0;
        reaped = hyp_daemon_runtime_service_job_reaped(fixture.service, project);
        (void)hyp_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_final_disconnect_automatically_exits_within_bound) {
    ...
    if (client) {
        (void)hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        hyp_daemon_runtime_service_state_t after_close =
            hyp_daemon_runtime_service_state(fixture.service);
        terminal_transition = after_close == HYP_DAEMON_RUNTIME_SERVICE_STOPPING ||
                              after_close == HYP_DAEMON_RUNTIME_SERVICE_EXITED;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_authenticated_idle_connection_outlives_lease_interval) {
    ...
    if (client) {
        ...
        (void)hyp_nanosleep(&beyond_lease, NULL);
        remained_connected = hyp_daemon_runtime_service_active_clients(fixture.service) == 1 &&
                             hyp_daemon_runtime_client_heartbeat(client, RUNTIME_TEST_TIMEOUT_MS);
        (void)hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_connection_cap_covers_slow_hello_and_stopping_is_terminal) {
    ...
    hyp_daemon_ipc_connection_t *slow_hello = NULL;
    hyp_daemon_runtime_client_t *accepted = NULL;
    hyp_daemon_runtime_client_t *overflow = NULL;
    hyp_daemon_runtime_client_t *resurrection = NULL;
    hyp_daemon_runtime_connect_result_t accepted_result = {0};
    hyp_daemon_runtime_connect_result_t overflow_result = {0};
    hyp_daemon_runtime_connect_result_t resurrection_result = {0};
    ...
    if (started) {
        slow_hello = hyp_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (slow_hello) {
        slow_slot_counted = hyp_daemon_runtime_service_wait_for_connections(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
        accepted = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                     RUNTIME_TEST_TIMEOUT_MS, &accepted_result);
    }
    if (accepted) {
        overflow = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                     RUNTIME_TEST_TIMEOUT_MS, &overflow_result);
        capacity_rejected = overflow == NULL &&
                            overflow_result.status == HYP_DAEMON_RUNTIME_CONNECT_REJECTED &&
                            strstr(overflow_result.message, "capacity") != NULL &&
                            hyp_daemon_runtime_service_active_connections(fixture.service) == 2 &&
                            hyp_daemon_runtime_service_active_clients(fixture.service) == 1;

        hyp_daemon_ipc_connection_close(slow_hello);
        slow_hello = NULL;
        slow_slot_released = hyp_daemon_runtime_service_wait_for_connections(
            fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
        (void)hyp_daemon_runtime_client_close(accepted, RUNTIME_TEST_TIMEOUT_MS);
        accepted = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);

        resurrection = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity, 100,
                                                         &resurrection_result);
        hyp_daemon_runtime_service_state_t terminal_state =
            hyp_daemon_runtime_service_state(fixture.service);
        no_resurrection = resurrection == NULL &&
                          (resurrection_result.status == HYP_DAEMON_RUNTIME_CONNECT_ERROR ||
                           resurrection_result.status == HYP_DAEMON_RUNTIME_CONNECT_REJECTED) &&
                          hyp_daemon_runtime_service_active_clients(fixture.service) == 0 &&
                          (terminal_state == HYP_DAEMON_RUNTIME_SERVICE_STOPPING ||
                           terminal_state == HYP_DAEMON_RUNTIME_SERVICE_EXITED);
    }

    if (resurrection) {
        (void)hyp_daemon_runtime_client_close(resurrection, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (overflow) {
        (void)hyp_daemon_runtime_client_close(overflow, RUNTIME_TEST_TIMEOUT_MS);
    }
    if (accepted) {
        (void)hyp_daemon_runtime_client_close(accepted, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_rejects_forged_identity_extension) {
    ...
    hyp_daemon_ipc_connection_t *raw = NULL;
    uint8_t forged[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE + sizeof(uint64_t) * 2] = {0};
    ...
    if (started) {
        encoded = hyp_daemon_runtime_hello_request_encode(forged, &identity);
        memcpy(forged + HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE, &forged_client_id,
               sizeof(forged_client_id));
        memcpy(forged + HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE + sizeof(forged_client_id),
               &forged_process_id, sizeof(forged_process_id));
        raw = hyp_daemon_ipc_connect(fixture.endpoint, RUNTIME_TEST_TIMEOUT_MS);
    }
    connected = raw != NULL;
    if (raw && encoded) {
         ...
         * attacker-controlled bytes are read (hyp_daemon_ipc_receive_frame_bounded).
        ...
        sent = hyp_daemon_ipc_send_frame(raw, HYP_DAEMON_FRAME_REQUEST, HYP_DAEMON_RUNTIME_OP_HELLO,
                                         forged, (uint32_t)sizeof(forged));
        int received = sent ? hyp_daemon_ipc_receive_frame(raw, RUNTIME_TEST_TIMEOUT_MS,
                                                           &response_frame, &response_payload)
                            : 0;
        ...
        rejected = received != 1 && hyp_daemon_runtime_service_wait_for_clients(
    ...
    hyp_daemon_runtime_connect_result_t valid_result = {0};
    hyp_daemon_runtime_client_t *valid = NULL;
    bool valid_after_rejection = false;
    bool exited = false;
    if (started) {
        valid = hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &valid_result);
    }
    if (valid) {
        valid_after_rejection =
            valid_result.status == HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED &&
            valid_result.authenticated_process_id == runtime_test_process_id() &&
            valid_result.client_id != forged_client_id;
        (void)hyp_daemon_runtime_client_close(valid, RUNTIME_TEST_TIMEOUT_MS);
        valid = NULL;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_application_response_roundtrip_is_byte_exact) {
    ...
    if (client) {
        status = hyp_daemon_runtime_client_application_request(
            client, request, (uint32_t)sizeof(request), &response, &response_length,
            RUNTIME_TEST_TIMEOUT_MS);
        exact = status == HYP_DAEMON_RUNTIME_APPLICATION_OK && response_length == sizeof(request) &&
...
TEST(daemon_runtime_final_disconnect_rejects_blocked_provisional_session) {
    ...
    if (owner) {
        ...
        connect_thread_create_rc = hyp_thread_create(
    ...
    if (provisional_started) {
        owner_closed = hyp_daemon_runtime_client_close(owner, RUNTIME_TEST_TIMEOUT_MS);
        owner = NULL;
        shutdown_won = hyp_daemon_runtime_service_state(fixture.service) ==
                       HYP_DAEMON_RUNTIME_SERVICE_STOPPING;
    }
    ...
    if (connect_thread_started) {
        connect_thread_join_rc = hyp_thread_join(&connect_thread);
    ...
    if (contender.client) {
        (void)hyp_daemon_runtime_client_close(contender.client, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_request_cancel_is_exact_and_session_remains_usable) {
    ...
    hyp_daemon_runtime_cancel_result_t wrong_cancel = HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    int cancels_after_wrong = -1;
    hyp_daemon_runtime_cancel_result_t exact_cancel = HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    bool cancel_delivered = false;
    hyp_daemon_runtime_cancel_result_t late_duplicate_cancel = HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (client) {
        token_reserved =
            hyp_daemon_runtime_client_application_token_reserve(client, &request_token);
    }
    if (token_reserved) {
        call.client = client;
        call.request_token = request_token;
        request_thread_create_rc = hyp_thread_create(
    ...
    if (callback_started) {
        wrong_cancel = hyp_daemon_runtime_client_application_cancel(client, request_token + 1U);
        cancels_after_wrong = atomic_load_explicit(&context.request_cancels, memory_order_acquire);
        exact_cancel = hyp_daemon_runtime_client_application_cancel(client, request_token);
        cancel_delivered =
            exact_cancel == HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED &&
    ...
    if (request_thread_started && !cancel_delivered && client) {
        close_begun = hyp_daemon_runtime_client_close_begin(client);
    }
    if (request_thread_started && (cancel_delivered || close_begun)) {
        request_thread_join_attempted = true;
        request_thread_join_rc = hyp_thread_join(&request_thread);
    ...
    if (client) {
        closed = close_begun
                     ? hyp_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                     : hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
    }
    if (started) {
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    bool cancelled_response = call.status == HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
    ...
    ASSERT_EQ(wrong_cancel, HYP_DAEMON_RUNTIME_CANCEL_STALE);
    ASSERT_EQ(cancels_after_wrong, 0);
    ASSERT_EQ(exact_cancel, HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED);
    ASSERT_TRUE(cancel_delivered);
    ASSERT_TRUE(request_thread_joined);
    ASSERT_TRUE(cancelled_response);
    ASSERT_EQ(late_duplicate_cancel, HYP_DAEMON_RUNTIME_CANCEL_STALE);
...
TEST(daemon_runtime_presend_request_cancel_is_sticky_and_nonterminal) {
    ...
    hyp_daemon_runtime_application_token_t request_token =
        HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    bool token_reserved = false;
    hyp_daemon_runtime_cancel_result_t cancel = HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    ...
    if (token_reserved) {
        cancel = hyp_daemon_runtime_client_application_cancel(client, request_token);
        cancelled_status = hyp_daemon_runtime_client_application_request_tagged(
    ...
    if (cancelled_status == HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED) {
        next_status = hyp_daemon_runtime_client_application_request(
            client, next_request, (uint32_t)sizeof(next_request), &next_response,
            &next_response_length, RUNTIME_TEST_TIMEOUT_MS);
        next_exact = next_status == HYP_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    ASSERT_TRUE(started);
    ASSERT_TRUE(token_reserved);
    ASSERT_EQ(cancel, HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED);
...
TEST(daemon_runtime_allows_only_one_unstarted_application_token) {
    ...
    hyp_daemon_runtime_connect_result_t result = {0};
    hyp_daemon_runtime_client_t *client = NULL;
    hyp_daemon_runtime_application_token_t first_token =
        HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    hyp_daemon_runtime_application_token_t rejected_token = UINT64_MAX;
    hyp_daemon_runtime_application_token_t second_token =
        HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    ...
    if (client) {
        first_reserved = hyp_daemon_runtime_client_application_token_reserve(client, &first_token);
        duplicate_reservation_rejected =
            !hyp_daemon_runtime_client_application_token_reserve(client, &rejected_token) &&
            rejected_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    if (first_reserved && duplicate_reservation_rejected) {
        ...
        hyp_daemon_runtime_application_status_t status =
            hyp_daemon_runtime_client_application_request_tagged(
                ...
                &response_length, RUNTIME_TEST_TIMEOUT_MS);
        first_exact = status == HYP_DAEMON_RUNTIME_APPLICATION_OK &&
    ...
    if (first_exact) {
        second_reserved =
            hyp_daemon_runtime_client_application_token_reserve(client, &second_token);
    }
    if (second_reserved) {
        uint8_t *response = NULL;
        uint32_t response_length = 0;
        hyp_daemon_runtime_application_status_t status =
            hyp_daemon_runtime_client_application_request_tagged(
                ...
                &response_length, RUNTIME_TEST_TIMEOUT_MS);
        second_exact = status == HYP_DAEMON_RUNTIME_APPLICATION_OK &&
...
TEST(daemon_runtime_consumes_busy_application_token_before_response) {
    ...
    if (first_started) {
        ...
        busy_received = busy_sent && runtime_test_raw_application_receive_status_token(
                                         raw, BUSY_TOKEN, HYP_DAEMON_RUNTIME_APPLICATION_BUSY);
    }
    if (busy_received) {
        ...
        cancellation_received =
            cancel_sent && runtime_test_raw_application_receive_status_token(
                               raw, FIRST_TOKEN, HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED);
    }
...
TEST(daemon_runtime_close_begin_retains_storage_and_rejects_late_exchange) {
    ...
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    hyp_daemon_runtime_application_status_t status = HYP_DAEMON_RUNTIME_APPLICATION_OK;
    bool close_begun = false;
    ...
    if (client) {
        close_begun = hyp_daemon_runtime_client_close_begin(client);
        duplicate_begin_rejected = close_begun && !hyp_daemon_runtime_client_close_begin(client);
    }
    if (close_begun) {
    ...
    if (client) {
        if (close_begun) {
            (void)hyp_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS);
        } else {
            (void)hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        }
    }
    ...
    ASSERT_EQ(status, HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR);
...
TEST(daemon_runtime_disconnect_cancels_blocked_application_before_exit) {
    ...
    if (client) {
        close_acknowledged =
            close_begun ? hyp_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                        : hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS);
        client = NULL;
    }
    if (started) {
    ...
    request_interrupted = (call.status == HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
                           call.status == HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED) &&
                          call.response == NULL && call.response_length == 0;
    free(call.response);
    runtime_test_fixture_finish(&fixture);
...
TEST(daemon_runtime_disconnect_cancels_blocked_non_index_child_and_preserves_other_session) {
    ...
    char *saved_cache = old_cache ? hyp_strdup(old_cache) : NULL;
    char *saved_path = old_path ? hyp_strdup(old_path) : NULL;
    char *saved_marker = old_marker ? hyp_strdup(old_marker) : NULL;
    bool snapshots_ok =
...
#else
    (void)snprintf(work, sizeof(work), "%s/hyp-runtime-non-index-XXXXXX", hyp_tmpdir());
    bool work_ready = snapshots_ok && hyp_mkdtemp(work) != NULL;
#endif
    ...
    bool paths_ready = work_ready && root_written > 0 && root_written < (int)sizeof(root) &&
                       ...
                       marker_written < (int)sizeof(marker) && hyp_mkdir_p(root, 0700) &&
                       hyp_mkdir_p(cache, 0700) && hyp_mkdir_p(bin, 0700) &&
                       runtime_test_copy_self_image(fake_git);

    char path_bin[RUNTIME_TEST_PATH_CAP] = {0};
#ifdef _WIN32
    bool path_bin_ready = paths_ready && hyp_canonical_path(bin, path_bin, sizeof(path_bin));
    if (path_bin_ready) {
    ...
    char *project = paths_ready ? hyp_project_name_from_path(root) : NULL;
    char db_path[RUNTIME_TEST_PATH_CAP] = {0};
    int db_written = project ? snprintf(db_path, sizeof(db_path), "%s/%s.db", cache, project) : -1;
    hyp_store_t *seed =
        db_written > 0 && db_written < (int)sizeof(db_path) ? hyp_store_open_path(db_path) : NULL;
    bool seeded = seed && hyp_store_upsert_project(seed, project, root) == HYP_STORE_OK;
    ...
    bool environment_ready = test_path && hyp_setenv("HYP_CACHE_DIR", cache, 1) == 0 &&
                             hyp_setenv("PATH", test_path, 1) == 0 &&
                             hyp_setenv(RUNTIME_TEST_BLOCKING_GIT_MARKER_ENV, marker, 1) == 0;

    hyp_daemon_application_t *application =
        environment_ready ? hyp_daemon_application_new(NULL) : NULL;
    hyp_daemon_runtime_application_callbacks_t callbacks =
    ...
    hyp_daemon_runtime_connect_result_t first_result = {0};
    hyp_daemon_runtime_connect_result_t second_result = {0};
    hyp_daemon_runtime_client_t *first =
        started ? hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                    RUNTIME_TEST_TIMEOUT_MS, &first_result)
                : NULL;
    hyp_daemon_runtime_client_t *second =
        first ? hyp_daemon_runtime_client_connect(fixture.endpoint, &identity,
                                                  RUNTIME_TEST_TIMEOUT_MS, &second_result)
              : NULL;
    bool contexts_set =
        first && second &&
        hyp_daemon_application_client_set_context(first, root, root, HYP_MCP_TOOL_PROFILE_ALL, NULL,
                                                  NULL, RUNTIME_TEST_TIMEOUT_MS) ==
            HYP_DAEMON_RUNTIME_APPLICATION_OK &&
        hyp_daemon_application_client_set_context(second, root, root, HYP_MCP_TOOL_PROFILE_ALL,
                                                  NULL, NULL, RUNTIME_TEST_TIMEOUT_MS) ==
            HYP_DAEMON_RUNTIME_APPLICATION_OK;
    bool second_usable_before =
        contexts_set && runtime_real_application_ingest_probe(second) &&
        hyp_daemon_runtime_client_heartbeat(second, RUNTIME_TEST_TIMEOUT_MS);
    ...
    int request_thread_create_rc =
        second_usable_before && arguments_written > 0 &&
            ...
            ? hyp_thread_create(&request_thread, 128U * 1024U,
    ...
    bool first_close_begun = child_identity_exact && hyp_daemon_runtime_client_close_begin(first);
    ...
    bool only_second_admitted =
        first_close_begun &&
        hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1, RUNTIME_TEST_TIMEOUT_MS);
    size_t clients_after_admission_wait =
        started ? hyp_daemon_runtime_service_active_clients(fixture.service) : 0;
    size_t connections_after_admission_wait =
        started ? hyp_daemon_runtime_service_active_connections(fixture.service) : 0;
    ...
    int request_thread_join_rc = request_completed ? hyp_thread_join(&request_thread) : -1;
    if (request_thread_join_rc == 0) {
        request_thread_started = false;
    }
    bool first_transport_interrupted =
        request_completed && call.status == HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR &&
        call.response == NULL && call.response_length == 0;
    bool first_close_finished = false;
    if (first && !request_thread_started) {
        first_close_finished =
            first_close_begun
                ? hyp_daemon_runtime_client_close_finish(first, RUNTIME_TEST_TIMEOUT_MS)
                : hyp_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        first = NULL;
    }
    ...
    if (second) {
        second_closed = hyp_daemon_runtime_client_close(second, RUNTIME_TEST_TIMEOUT_MS);
        second = NULL;
    }
    bool exited =
        started && hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    ...
    if (first) {
        if (!first_close_begun) {
            first_close_begun = hyp_daemon_runtime_client_close_begin(first);
        }
        if (request_thread_started) {
        ...
        if (first_close_begun) {
            (void)hyp_daemon_runtime_client_close_finish(first, RUNTIME_TEST_TIMEOUT_MS);
        } else {
            (void)hyp_daemon_runtime_client_close(first, RUNTIME_TEST_TIMEOUT_MS);
        }
        first = NULL;
    ...
    bool application_stopped =
        application && hyp_daemon_application_shutdown(application, RUNTIME_TEST_TIMEOUT_MS);
...
TEST(daemon_runtime_noncooperative_callback_does_not_detach_or_unbound_stop) {
     ...
     * scheduler starvation (the HYP_LOCAL_CI_CPUS=4 fidelity pass, CI runners)
    ...
    if (callback_started) {
        close_begun = hyp_daemon_runtime_client_close_begin(client);
        uint64_t stop_started_ms = hyp_now_ms();
        stop_returned = hyp_daemon_runtime_service_stop(fixture.service, STOP_BOUND_MS);
        stop_elapsed_ms = hyp_now_ms() - stop_started_ms;
        exited_before_release =
            hyp_daemon_runtime_service_state(fixture.service) == HYP_DAEMON_RUNTIME_SERVICE_EXITED;
    }
    ...
    if (started) {
        exited_after_release =
            hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
    free(call.response);
    runtime_test_fixture_finish(&fixture);

    ASSERT_TRUE(started);
    ASSERT_EQ(result.status, HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED);
    ASSERT_EQ(request_thread_create_rc, 0);
    ASSERT_TRUE(callback_started);
    ASSERT_TRUE(close_begun);
...
TEST(daemon_runtime_application_busy_cap_and_malformed_are_isolated) {
    ...
    hyp_daemon_runtime_application_status_t oversize_status = HYP_DAEMON_RUNTIME_APPLICATION_OK;
    uint8_t *valid_response = NULL;
    uint32_t valid_response_length = 0;
    hyp_daemon_runtime_application_status_t valid_status =
        HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    bool owner_survived = false;
    bool exited = false;
    ...
    if (callback_started) {
        ...
        busy_rejected = busy_sent && runtime_test_raw_application_receive_status(
                                         raw, HYP_DAEMON_RUNTIME_APPLICATION_BUSY);
    }
    if (busy_rejected) {
    ...
    if (owner && bad_peer_released) {
        oversize_status = hyp_daemon_runtime_client_application_request(
            owner, &sentinel, HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX + 1U, &oversize_response,
            &oversize_response_length, RUNTIME_TEST_TIMEOUT_MS);
        valid_status = hyp_daemon_runtime_client_application_request(
            owner, valid_request, (uint32_t)sizeof(valid_request), &valid_response,
            &valid_response_length, RUNTIME_TEST_TIMEOUT_MS);
        owner_survived = oversize_status == HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR &&
                         oversize_response == NULL && oversize_response_length == 0 &&
                         valid_status == HYP_DAEMON_RUNTIME_APPLICATION_OK &&
...
TEST(daemon_runtime_malformed_and_zero_cancel_close_only_offending_connections) {
    ...
    if (raw) {
        malformed_sent = hyp_daemon_ipc_send_frame(
            raw, HYP_DAEMON_FRAME_REQUEST, HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
            malformed_cancel, (uint32_t)sizeof(malformed_cancel));
    }
    if (malformed_sent) {
        malformed_released = hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                         RUNTIME_TEST_TIMEOUT_MS);
    }
    ...
    if (raw) {
        zero_sent = hyp_daemon_ipc_send_frame(
            raw, HYP_DAEMON_FRAME_REQUEST, HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
            zero_token_cancel, (uint32_t)sizeof(zero_token_cancel));
    }
    if (zero_sent) {
        zero_released = hyp_daemon_runtime_service_wait_for_clients(fixture.service, 1,
                                                                    RUNTIME_TEST_TIMEOUT_MS);
    }
    if (zero_released) {
        hyp_daemon_frame_t frame = {0};
    ...
    if (owner && zero_closed) {
        valid_status = hyp_daemon_runtime_client_application_request(
            owner, valid_request, (uint32_t)sizeof(valid_request), &valid_response,
            &valid_response_length, RUNTIME_TEST_TIMEOUT_MS);
        owner_survived = valid_status == HYP_DAEMON_RUNTIME_APPLICATION_OK &&
                         valid_response_length == sizeof(valid_request) && valid_response &&
                         memcmp(valid_response, valid_request, sizeof(valid_request)) == 0 &&
                         hyp_daemon_runtime_client_heartbeat(owner, RUNTIME_TEST_TIMEOUT_MS);
    }
...
TEST(daemon_runtime_kernel_process_fingerprint_is_stable_and_fail_closed) {
    char first[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char repeated[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char invalid[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool first_ok = hyp_daemon_runtime_process_build_fingerprint(runtime_test_process_id(), first);
    bool repeated_ok =
        hyp_daemon_runtime_process_build_fingerprint(runtime_test_process_id(), repeated);
    bool invalid_rejected = !hyp_daemon_runtime_process_build_fingerprint(UINT64_MAX, invalid);
...
TEST(daemon_runtime_process_fingerprint_supports_extended_length_image) {
    ...
    int root_written =
        snprintf(root, sizeof(root), "%s/hyp-runtime-long-image-XXXXXX", hyp_tmpdir());
    bool root_created =
        root_written > 0 && root_written < (int)sizeof(root) && hyp_mkdtemp(root) != NULL;
    ...
    bool directory_created = path_built && hyp_mkdir_p(directory, 0700);
    ...
    int event_written =
        snprintf(event_name, sizeof(event_name), "Local\\hyp-runtime-long-image-%lu-%llu",
    ...
    char observed[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool fingerprinted = queried_long_image && hyp_daemon_runtime_process_build_fingerprint(
    ...
    bool image_removed = !copied || hyp_unlink(image_path) == 0;
    ...
    if (root_created) {
        ...
        for (;;) {
            tree_removed = hyp_rmdir(cleanup) == 0 && tree_removed;
...
TEST(daemon_runtime_mac_fast_path_rejects_foreign_main_image_mapping_active) {
    ...
    char foreign_image[RUNTIME_TEST_PATH_CAP] = {0};
    char foreign_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/hyp-runtime-mapped-image-XXXXXX", hyp_tmpdir());
    bool directory_created = directory_written > 0 && directory_written < (int)sizeof(directory) &&
                             hyp_mkdtemp(directory) != NULL;
    ...
    bool foreign_fingerprinted =
        foreign_signed && hyp_daemon_build_fingerprint_file(foreign_image, foreign_fingerprint);
...
TEST(daemon_runtime_copied_image_fallback_accepts_identical_and_rejects_changed) {
    ...
    char identical_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool identical_bytes =
        identical_copied &&
        hyp_daemon_build_fingerprint_file(identical_path, identical_fingerprint) &&
    ...
    bool identical_exited =
        ...
        hyp_daemon_runtime_service_wait_exited(identical_fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    (void)hyp_unlink(identical_path);
    ...
    char changed_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool changed_bytes = changed_copied &&
                         hyp_daemon_build_fingerprint_file(changed_path, changed_fingerprint) &&
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    char event_name[128] = {0};
    int directory_written =
        snprintf(directory, sizeof(directory), "%s/hyp-runtime-image-XXXXXX", hyp_tmpdir());
    ...
    int replacement_written = setup ? snprintf(replacement_path, sizeof(replacement_path),
                                    ...
                                    : -1;
    int event_written =
        snprintf(event_name, sizeof(event_name), "Local\\hyp-runtime-image-%lu-%llu",
                 (unsigned long)GetCurrentProcessId(), (unsigned long long)GetTickCount64());
    ...
    FILE *replacement_file = setup ? hyp_fopen(replacement_path, "wb") : NULL;
    bool replacement_written_ok =
        replacement_file && fputs("hyp-windows-replacement-image", replacement_file) >= 0;
    if (replacement_file) {
        replacement_written_ok = fclose(replacement_file) == 0 && replacement_written_ok;
    }
    setup = setup && replacement_written_ok;

    char original[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char replacement[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    char observed[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    setup = setup && hyp_daemon_build_fingerprint_file(image_path, original) &&
            hyp_daemon_build_fingerprint_file(replacement_path, replacement) &&
    ...
    bool fingerprinted = ready && hyp_daemon_runtime_process_build_fingerprint(
                                      (uint64_t)process.dwProcessId, observed);
    ...
    (void)hyp_unlink(replacement_path);
    (void)hyp_unlink(image_path);
    (void)hyp_rmdir(directory);
...
TEST(daemon_runtime_process_fingerprint_never_hashes_replacement_path) {
    ...
    setup = setup && child > 0 && rename(replacement_path, image_path) == 0 &&
            hyp_daemon_build_fingerprint_file(image_path, replacement) &&
            strcmp(original, replacement) != 0;
    bool fingerprinted =
        setup && hyp_daemon_runtime_process_build_fingerprint((uint64_t)child, observed);
...
TEST(daemon_runtime_close_begin_releases_admission_with_inflight_request) {
    ...
    admission_released_at_begin = close_begun && hyp_daemon_runtime_service_wait_for_clients(
                                                     fixture.service, 0, RUNTIME_TEST_TIMEOUT_MS);
    if (request_thread_started) {
        request_thread_joined = hyp_thread_join(&request_thread) == 0;
        request_thread_started = false;
    }
    request_ended_without_payload =
        request_thread_joined &&
        (call.status == HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
         call.status == HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED) &&
        call.response == NULL && call.response_length == 0;
    if (client) {
        (void)(close_begun ? hyp_daemon_runtime_client_close_finish(client, RUNTIME_TEST_TIMEOUT_MS)
                           : hyp_daemon_runtime_client_close(client, RUNTIME_TEST_TIMEOUT_MS));
        client = NULL;
    }
    if (started) {
...
TEST(daemon_runtime_permanent_service_survives_last_disconnect_until_stop) {
    ...
    hyp_daemon_runtime_client_t *client = NULL;
    hyp_daemon_runtime_connect_result_t connect_result = {0};
    bool survived = false;
    ...
    if (client) {
        ...
        survived =
            !hyp_daemon_runtime_service_wait_exited(fixture.service, 300U) &&
            hyp_daemon_runtime_service_state(fixture.service) == HYP_DAEMON_RUNTIME_SERVICE_RUNNING;
        hyp_daemon_runtime_status_t status = {0};
        status_ok = hyp_daemon_runtime_request_status(fixture.endpoint, &identity,
                                                      RUNTIME_TEST_TIMEOUT_MS, &status) &&
                    status.permanent && !status.stopping && status.committed_clients == 0;
        hyp_daemon_runtime_stop_result_t stop_result = {0};
        stop_ok = hyp_daemon_runtime_request_stop(fixture.endpoint, &identity,
...
TEST(daemon_runtime_stop_refuses_while_committed_clients_exist) {
    ...
    if (client) {
        hyp_daemon_runtime_stop_result_t busy_result = {0};
        refused = hyp_daemon_runtime_request_stop(fixture.endpoint, &identity,
        ...
        accepted_after_close =
            hyp_daemon_runtime_service_wait_for_clients(fixture.service, 0,
                                                        RUNTIME_TEST_TIMEOUT_MS) &&
            hyp_daemon_runtime_request_stop(fixture.endpoint, &identity, RUNTIME_TEST_TIMEOUT_MS,
                                            &stop_result) &&
            stop_result.accepted && !stop_result.busy;
        exited = hyp_daemon_runtime_service_wait_exited(fixture.service, RUNTIME_TEST_TIMEOUT_MS);
    }
