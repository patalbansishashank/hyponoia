...
static void check_exact_ast_kind(const BehaviorCase *test_case, int *failures) {
    const TSLanguage *language = hyp_ts_language(test_case->language);
...
static HYPFileResult *extract_case_with_macros(const BehaviorCase *test_case,
                                               const HYPMacroTable *macro_table, int *failures) {
    HYPFileResult *result =
        hyp_extract_file_ex(test_case->source, (int)strlen(test_case->source), test_case->language,
...
static HYPFileResult *extract_case(const BehaviorCase *test_case, int *failures) {
    return extract_case_with_macros(test_case, NULL, failures);
}

static int call_count_exact(const HYPFileResult *result, const char *caller_qn,
                            const char *callee) {
    int count = 0;
...
static int usage_count_exact(const HYPFileResult *result, const char *caller_qn,
...
static int value_usage_count_exact(const HYPFileResult *result, const char *caller_qn,
                                   const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const HYPUsage *usage = &result->usages.items[i];
        if (usage->kind == HYP_USAGE_VALUE && usage->ref_name && usage->enclosing_func_qn &&
            strcmp(usage->ref_name, reference) == 0 &&
            strcmp(usage->enclosing_func_qn, caller_qn) == 0) {
            count++;
        }
...
static int callable_definition_count_exact(const HYPFileResult *result, const char *name,
...
static int call_count_in_routine(const HYPFileResult *result, const char *caller,
                                 const char *callee) {
    int count = 0;
    for (int i = 0; i < result->calls.count; i++) {
        const HYPCall *call = &result->calls.items[i];
...
static int usage_count_in_routine(const HYPFileResult *result, const char *caller,
                                  const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const HYPUsage *usage = &result->usages.items[i];
...
static int callable_reference_count_in_routine(const HYPFileResult *result, const char *caller,
                                               const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const HYPUsage *usage = &result->usages.items[i];
        if (usage->kind == HYP_USAGE_CALL_REFERENCE && usage->ref_name &&
            strcmp(usage->ref_name, reference) == 0 &&
            terminal_name_matches(usage->enclosing_func_qn, caller))
            count++;
    }
...
static int semantic_reference_candidate_count_in_routine(const HYPFileResult *result,
                                                         const char *caller,
                                                         const char *reference) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
...
static int resolved_call_count_in_routine(const HYPFileResult *result, const char *caller,
                                          const char *callee) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *call = &result->resolved_calls.items[i];
        if (call->kind == HYP_RESOLVED_INVOCATION &&
...
static int resolved_call_total_in_routine(const HYPFileResult *result, const char *caller) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        if (result->resolved_calls.items[i].kind == HYP_RESOLVED_INVOCATION &&
...
static int resolved_reference_count_in_routine(const HYPFileResult *result, const char *caller,
                                               const char *callee) {
    int count = 0;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *reference = &result->resolved_calls.items[i];
        if (reference->kind == HYP_RESOLVED_CALL_REFERENCE &&
...
static void check_caller_definition(const BehaviorCase *test_case, const HYPFileResult *result,
...
static const char *checked_module_qn(const BehaviorCase *test_case, const HYPFileResult *result,
                                     int *failures) {
...
static int begin_routine_case(const BehaviorCase *test_case, HYPFileResult **result_out,
                              int *failures) {
    check_exact_ast_kind(test_case, failures);
    HYPFileResult *result = extract_case(test_case, failures);
    *result_out = result;
    if (!result)
        return 0;
...
static int finish_case(HYPFileResult *result, int failures) {
    if (result)
...
#define DEFINE_CPP_CUDA_BEHAVIOR_TESTS(name, source_value, ast_value, runner, reason_value) \
    TEST(repro_call_node_behavior_cpp_##name) {                                             \
        static const BehaviorCase test_case = {"cpp/" #name, HYP_LANG_CPP, "behavior.cpp",  \
                                               source_value, ast_value,    reason_value};   \
        return runner(&test_case);                                                          \
    }                                                                                       \
    TEST(repro_call_node_behavior_cuda_##name) {                                            \
        static const BehaviorCase test_case = {"cuda/" #name, HYP_LANG_CUDA, "behavior.cu", \
                                               source_value,  ast_value,     reason_value}; \
        return runner(&test_case);                                                          \
    }
...
TEST(repro_call_node_behavior_objectscript_udl_macro) {
    ...
    HYPMacroTable *macro_table = calloc(1, sizeof(*macro_table));
    ASSERT_NOT_NULL(macro_table);
    hyp_macro_table_init_system(macro_table);
    ...
    hyp_macro_table_free(macro_table);
    return status;
}
