 ...
 * Python hybrid LSPs (internal/hyp/lsp/go_lsp.c, internal/hyp/lsp/py_lsp.c).
 *
 ...
 *   these can silently fail: hyp_pipeline_find_lsp_resolution joins each
 ...
 *   from the source (grep '"lsp_' internal/hyp/lsp/go_lsp.c), with its keying
 *   site:
 ...
 *   (grep '"lsp_' internal/hyp/lsp/py_lsp.c), with its keying site:
 ...
 * func_node is a bare identifier resolved via hyp_registry_lookup_symbol on the
 * package QN). */
 ...
 * resolves via hyp_registry_lookup_symbol on the module QN). */
 ...
 * (py_lsp.c:1620-1624: hyp_scope_lookup yields a NAMED type → emit constructor
 ...
 * MODULE and hyp_registry_lookup_symbol(module_qn, attr) hits → lsp_module_attr).
 ...
 * hyp_registry_lookup_symbol("builtins", fname) hits). EXPECTED RED in a
 ...
 * constructor (py_lsp.c:1640-1643: hyp_registry_lookup_type("builtins.str")
...
TEST(repro_lsp_go_strategy_cross_file) {
     ...
     * cross-file pass (hyp_go_fast_resolve_qualified_calls), which runs only when
