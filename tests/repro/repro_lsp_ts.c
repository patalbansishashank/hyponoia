 ...
 * JavaScript / JSX hybrid LSP (internal/hyp/lsp/ts_lsp.c).
 *
 ...
 *   the source (grep '"lsp_' internal/hyp/lsp/ts_lsp.c), with its keying site:
 ...
 *   production indexer does: ".ts" → HYP_LANG_TYPESCRIPT, ".tsx" → HYP_LANG_TSX.
 *   jsx_mode (required by resolve_jsx_element, ts_lsp.c:2620) is enabled ONLY for
 *   HYP_LANG_TSX (hyp.c:619, pass_lsp_cross.c:267), so the two JSX fixtures use
 ...
 * hyp_registry_lookup_symbol_by_args hits on the module QN). */
 ...
 * hyp_registry_lookup_symbol on the module QN. App() renders <Widget/> defined
 * in the same file. */
