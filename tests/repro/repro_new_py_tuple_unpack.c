...
static HYPFileResult *rx_py(const char *src) {
    return hyp_extract_file(src, (int)strlen(src), HYP_LANG_PYTHON, "proj", "mod.py",
                            0, NULL, NULL);
}

static int count_var_defs(HYPFileResult *r) {
    int n = 0;
...
static int has_var_def(HYPFileResult *r, const char *name) {
    for (int i = 0; i < r->defs.count; i++) {
