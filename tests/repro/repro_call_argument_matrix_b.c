/*
 * Exhaustive call-argument reproduction matrix, HYPLanguage 80..162.
 *
...
size_t repro_call_argument_matrix_b_copy_language_ids(HYPLanguage *language_ids, size_t capacity);
...
static int usage_count_scoped(const HYPFileResult *result, const char *target, const char *scope,
                              ScopeMode mode) {
    int count = 0;
...
static int usage_kind_count_scoped(const HYPFileResult *result, const char *target,
                                   const char *scope, ScopeMode mode, HYPUsageKind kind) {
    int count = 0;
    for (int i = 0; i < result->usages.count; i++) {
        const HYPUsage *usage = &result->usages.items[i];
...
static int local_shadow_usage_count_scoped(const HYPFileResult *result, const char *target,
...
static int call_count_scoped(const HYPFileResult *result, const char *target, const char *scope,
                             ScopeMode mode) {
    int count = 0;
...
static int usage_within_call_count_scoped(const HYPFileResult *result, const char *target,
                                          const char *scope, ScopeMode mode, const char *callee) {
    int count = 0;
    for (int u = 0; u < result->usages.count; u++) {
        const HYPUsage *usage = &result->usages.items[u];
        if (!terminal_name_matches(usage->ref_name, target) ||
            !scope_matches(usage->enclosing_func_qn, scope, mode) ||
        ...
        for (int c = 0; c < result->calls.count; c++) {
            const HYPCall *call = &result->calls.items[c];
...
static int callable_definition_count(const HYPFileResult *result, const char *target) {
    int count = 0;
...
static const char *checked_callable_qn(const CaseIdentity *identity, const char *variant,
                                       const HYPFileResult *result, const char *target,
                                       int *failures) {
...
static void check_ast_kind_present(const CaseIdentity *identity, const char *variant,
    ...
    const TSLanguage *language = hyp_ts_language(identity->language);
...
static HYPFileResult *extract_source(const CaseIdentity *identity, const char *variant,
                                     const char *source, int *failures) {
    HYPFileResult *result = hyp_extract_file(source, (int)strlen(source), identity->language,
...
static const char *checked_module_qn(const CaseIdentity *identity, const char *variant,
                                     const HYPFileResult *result, int *failures) {
...
static const RoutineArgumentCase RACKET_CASE = ROUTINE_ARGUMENT_CASE(
    "RACKET", HYP_LANG_RACKET, "sample.rkt", RACKET_INSIDE, RACKET_BARE, "list", "run", "accept",
...
static const RoutineArgumentCase ODIN_CASE = ROUTINE_ARGUMENT_CASE(
    "ODIN", HYP_LANG_ODIN, "sample.odin", ODIN_INSIDE, ODIN_BARE, "call_expression", "run",
    "accept", "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase RESCRIPT_CASE = ROUTINE_ARGUMENT_CASE(
    "RESCRIPT", HYP_LANG_RESCRIPT, "Sample.res", RESCRIPT_INSIDE, RESCRIPT_BARE, "call_expression",
    "run", "accept", "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase PURESCRIPT_CASE =
    ROUTINE_ARGUMENT_CASE("PURESCRIPT", HYP_LANG_PURESCRIPT, "Main.purs", PURESCRIPT_INSIDE,
...
static const RoutineArgumentCase NICKEL_CASE = ROUTINE_ARGUMENT_CASE(
    "NICKEL", HYP_LANG_NICKEL, "sample.ncl", NICKEL_INSIDE, NICKEL_BARE, "applicative", "run",
...
static const RoutineArgumentCase CRYSTAL_CASE = ROUTINE_ARGUMENT_CASE(
    "CRYSTAL", HYP_LANG_CRYSTAL, "sample.cr", CRYSTAL_INSIDE, CRYSTAL_BARE, "call", "run", "accept",
    "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase TEAL_CASE = ROUTINE_ARGUMENT_CASE(
    "TEAL", HYP_LANG_TEAL, "sample.tl", LUA_TYPED_INSIDE, LUA_TYPED_BARE, "function_call", "run",
    "accept", "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase HARE_CASE = ROUTINE_ARGUMENT_CASE(
    "HARE", HYP_LANG_HARE, "sample.ha", HARE_INSIDE, HARE_BARE, "call_expression", "run", "accept",
    "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase PONY_CASE = ROUTINE_ARGUMENT_CASE(
    "PONY", HYP_LANG_PONY, "sample.pony", PONY_INSIDE, PONY_BARE, "call_expression", "run",
    "accept", "watched", 1, 1, 0, "native method application");
static const RoutineArgumentCase LUAU_CASE = ROUTINE_ARGUMENT_CASE(
    "LUAU", HYP_LANG_LUAU, "sample.luau", LUA_TYPED_INSIDE, LUA_TYPED_BARE, "function_call", "run",
    "accept", "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase SWAY_CASE = ROUTINE_ARGUMENT_CASE(
    "SWAY", HYP_LANG_SWAY, "sample.sw", SWAY_INSIDE, SWAY_BARE, "call_expression", "run", "accept",
    "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase TEMPL_CASE = ROUTINE_ARGUMENT_CASE(
    "TEMPL", HYP_LANG_TEMPL, "sample.templ", TEMPL_INSIDE, TEMPL_BARE, "call_expression", "run",
    "accept", "watched", 1, 1, 0, "embedded Go routine application");
static const RoutineArgumentCase WGSL_CASE =
    ROUTINE_ARGUMENT_CASE("WGSL", HYP_LANG_WGSL, "sample.wgsl", WGSL_INSIDE, WGSL_BARE,
...
static const RoutineArgumentCase JSONNET_CASE = ROUTINE_ARGUMENT_CASE(
    "JSONNET", HYP_LANG_JSONNET, "sample.jsonnet", JSONNET_INSIDE, JSONNET_BARE, "functioncall",
...
static const RoutineArgumentCase STARLARK_CASE = ROUTINE_ARGUMENT_CASE(
    "STARLARK", HYP_LANG_STARLARK, "BUILD", STARLARK_INSIDE, STARLARK_BARE, "call", "run", "accept",
    "watched", 1, 1, 0, "native Starlark routine application");
static const RoutineArgumentCase BICEP_CASE = ROUTINE_ARGUMENT_CASE(
    "BICEP", HYP_LANG_BICEP, "sample.bicep", BICEP_INSIDE, BICEP_BARE, "call_expression", "run",
...
static const RoutineArgumentCase HLSL_CASE = ROUTINE_ARGUMENT_CASE(
    "HLSL", HYP_LANG_HLSL, "sample.hlsl", HLSL_FAMILY_INSIDE, HLSL_FAMILY_BARE, "call_expression",
...
static const RoutineArgumentCase VHDL_CASE = ROUTINE_ARGUMENT_CASE(
    "VHDL", HYP_LANG_VHDL, "sample.vhd", VHDL_INSIDE, VHDL_BARE, "parenthesis_group", "run",
...
static const RoutineArgumentCase SYSTEMVERILOG_CASE = ROUTINE_ARGUMENT_CASE(
    "SYSTEMVERILOG", HYP_LANG_SYSTEMVERILOG, "sample.sv", SYSTEMVERILOG_INSIDE, SYSTEMVERILOG_BARE,
...
static const RoutineArgumentCase ISPC_CASE = ROUTINE_ARGUMENT_CASE(
    "ISPC", HYP_LANG_ISPC, "sample.ispc", HLSL_FAMILY_INSIDE, HLSL_FAMILY_BARE, "call_expression",
...
static const RoutineArgumentCase CAIRO_CASE = ROUTINE_ARGUMENT_CASE(
    "CAIRO", HYP_LANG_CAIRO, "sample.cairo", CAIRO_INSIDE, CAIRO_BARE, "call_expression", "run",
...
static const RoutineArgumentCase MOVE_CASE = ROUTINE_ARGUMENT_CASE(
    "MOVE", HYP_LANG_MOVE, "sample.move", MOVE_INSIDE, MOVE_BARE, "call_expression", "run",
    "accept", "watched", 1, 1, 0, "native smart-contract routine application");
static const RoutineArgumentCase SQUIRREL_CASE = ROUTINE_ARGUMENT_CASE(
    "SQUIRREL", HYP_LANG_SQUIRREL, "sample.nut", SQUIRREL_INSIDE, SQUIRREL_BARE, "call_expression",
    "run", "accept", "watched", 1, 1, 0, "native routine application");
static const RoutineArgumentCase FUNC_CASE = ROUTINE_ARGUMENT_CASE(
    "FUNC", HYP_LANG_FUNC, "sample.fc", FUNC_INSIDE, FUNC_BARE, "function_application", "run",
    "accept", "watched", 1, 1, 0, "native FunC function application");
static const RoutineArgumentCase PUPPET_CASE = ROUTINE_ARGUMENT_CASE(
    "PUPPET", HYP_LANG_PUPPET, "sample.pp", PUPPET_INSIDE, PUPPET_BARE, "function_call", "run",
...
static const RoutineArgumentCase SLANG_CASE = ROUTINE_ARGUMENT_CASE(
    "SLANG", HYP_LANG_SLANG, "sample.slang", HLSL_FAMILY_INSIDE, HLSL_FAMILY_BARE,
...
static const RoutineArgumentCase LLVM_IR_CASE = ROUTINE_ARGUMENT_CASE(
    "LLVM_IR", HYP_LANG_LLVM_IR, "sample.ll", LLVM_IR_INSIDE, LLVM_IR_BARE, "call", "run", "accept",
...
static const RoutineArgumentCase TLAPLUS_CASE = ROUTINE_ARGUMENT_CASE(
    "TLAPLUS", HYP_LANG_TLAPLUS, "Sample.tla", TLAPLUS_INSIDE, TLAPLUS_BARE, "bound_op", "Guard",
...
static const RoutineArgumentCase APEX_CASE = ROUTINE_ARGUMENT_CASE(
    "APEX", HYP_LANG_APEX, "Sample.cls", APEX_INSIDE, APEX_BARE, "method_invocation", "run",
    "accept", "watched", 1, 1, 0, "native method application");
static const RoutineArgumentCase PINE_CASE = ROUTINE_ARGUMENT_CASE(
    "PINE", HYP_LANG_PINE, "sample.pine", PINE_INSIDE, PINE_BARE, "call", "run", "accept",
...
static const RoutineArgumentCase QML_CASE = ROUTINE_ARGUMENT_CASE(
    "QML", HYP_LANG_QML, "Sample.qml", QML_INSIDE, QML_BARE, "call_expression", "run", "accept",
...
static const RoutineArgumentCase CFSCRIPT_CASE = ROUTINE_ARGUMENT_CASE(
    "CFSCRIPT", HYP_LANG_CFSCRIPT, "Sample.cfc", CFSCRIPT_INSIDE, CFSCRIPT_BARE, "call_expression",
...
static const RoutineArgumentCase CFML_CASE = ROUTINE_ARGUMENT_CASE(
    "CFML", HYP_LANG_CFML, "sample.cfm", CFML_INSIDE, CFML_BARE, "call_expression", "run", "accept",
    "watched", 1, 1, 0, "native CFML tag-routine application");
static const RoutineArgumentCase MOJO_CASE =
    ROUTINE_ARGUMENT_CASE("MOJO", HYP_LANG_MOJO, "sample.mojo", MOJO_INSIDE, MOJO_BARE, "call",
...
static const RoutineArgumentCase OBJECTSCRIPT_UDL_CASE = ROUTINE_ARGUMENT_CASE(
    "OBJECTSCRIPT_UDL", HYP_LANG_OBJECTSCRIPT_UDL, "Sample.cls", OBJECTSCRIPT_UDL_INSIDE,
...
static const RoutineArgumentCase OBJECTSCRIPT_ROUTINE_CASE = ROUTINE_ARGUMENT_CASE(
    "OBJECTSCRIPT_ROUTINE", HYP_LANG_OBJECTSCRIPT_ROUTINE, "Sample.mac",
...
static const ModuleArgumentCase JUST_CASE = MODULE_ARGUMENT_CASE(
    "JUST", HYP_LANG_JUST, "justfile", JUST_INSIDE, JUST_BARE, "function_call", "uppercase",
    "watched", 1, 0, "Just expression call; recipe dependency is checked separately");
static const ModuleArgumentCase GOTEMPLATE_CASE =
    MODULE_ARGUMENT_CASE("GOTEMPLATE", HYP_LANG_GOTEMPLATE, "sample.tmpl", GOTEMPLATE_INSIDE,
...
static const ModuleArgumentCase LINKERSCRIPT_CASE =
    MODULE_ARGUMENT_CASE("LINKERSCRIPT", HYP_LANG_LINKERSCRIPT, "sample.ld", LINKERSCRIPT_INSIDE,
...
static const ModuleArgumentCase GN_CASE =
    MODULE_ARGUMENT_CASE("GN", HYP_LANG_GN, "BUILD.gn", GN_INSIDE, GN_BARE, "call_expression",
...
static int run_no_call_domain(const CaseIdentity *identity, const char *source,
    ...
    HYPFileResult *result = extract_source(identity, "domain-negative", source, &failures);
    if (result) {
...
static int run_module_domain_call(const CaseIdentity *identity, const char *source,
    ...
    HYPFileResult *result = extract_source(identity, "domain-call", source, &failures);
    if (result) {
...
static int run_tlaplus_bounded_quantification_control(void) {
    static const CaseIdentity identity = {
        "TLAPLUS", HYP_LANG_TLAPLUS, "Bounded.tla",
    ...
    HYPFileResult *result = extract_source(&identity, "bounded-quantification-control",
...
static int run_nasm_domain_control(void) {
    ...
    check_ast_kind_present(&identity, "instruction-control", NASM_SCOPE, "actual_instruction",
                           &failures);
    HYPFileResult *result = extract_source(&identity, "instruction-control", NASM_SCOPE, &failures);
    if (result) {
...
static int run_bitbake_domain_control(void) {
    static const CaseIdentity identity = {
        "BITBAKE", HYP_LANG_BITBAKE, "sample.bb",
    ...
    HYPFileResult *result = extract_source(&identity, "embedded-python", BITBAKE_DOMAIN, &failures);
    if (result) {
...
TEST(repro_call_argument_matrix_b_domain_prisma) {
    static const CaseIdentity identity = {
        "PRISMA", HYP_LANG_PRISMA, "schema.prisma",
...
TEST(repro_call_argument_matrix_b_domain_diff) {
    static const CaseIdentity identity = {
        "DIFF", HYP_LANG_DIFF, "sample.diff",
...
TEST(repro_call_argument_matrix_b_domain_bibtex) {
    static const CaseIdentity identity = {
        "BIBTEX", HYP_LANG_BIBTEX, "sample.bib",
...
TEST(repro_call_argument_matrix_b_domain_devicetree) {
    static const CaseIdentity identity = {
        "DEVICETREE", HYP_LANG_DEVICETREE, "sample.dts",
...
#define MATRIX_B_LANGUAGE_ROWS(X)                                                               \
    ...
    X(repro_call_argument_matrix_b_domain_nasm, HYP_LANG_NASM)                                  \
    X(repro_call_argument_matrix_b_domain_prisma, HYP_LANG_PRISMA)                              \
    X(repro_call_argument_matrix_b_domain_diff, HYP_LANG_DIFF)                                  \
    X(repro_call_argument_matrix_b_domain_bibtex, HYP_LANG_BIBTEX)                              \
    X(repro_call_argument_matrix_b_domain_devicetree, HYP_LANG_DEVICETREE)                      \
    X(repro_call_argument_matrix_b_domain_bitbake, HYP_LANG_BITBAKE)
