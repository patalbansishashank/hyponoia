...
//   https://github.com/patalbansishashank/hyponoia/issues/581
//
// OBSERVED BEHAVIOUR:
//   hyponoia in stdio MCP server mode grows from ~12 MB working
...
//
//   2. mimalloc page retention: hyp_mem_collect() is called after
...
//
//   (a) RSS MEASUREMENT: we use hyp_mem_rss() (src/foundation/mem.c) which
...
//
//   (d) LINUX-ONLY ALTERNATIVE: if hyp_mem_rss() returns 0 (e.g. MI_OVERRIDE=0
//       without the OS fallback compiled), the test falls back to reading
//       /proc/self/statm directly below.  On macOS and Windows hyp_mem_rss()
...
//   1. src/mcp/mcp.c, hyp_mcp_server_run event loop (or after each tool call
//      in hyp_mcp_handle_tool): periodically call
//        sqlite3_wal_checkpoint_v2(..., SQLITE_CHECKPOINT_TRUNCATE, ...)
//      and hyp_mem_collect() after query bursts (e.g. every N=50 calls or
//      after exceeding a RSS threshold via hyp_mem_over_budget()).
//   2. src/mcp/mcp.c, hyp_mcp_server_evict_idle: on idle eviction, call
//      hyp_mem_collect() so mimalloc returns pages to the OS, matching the
...
//
// GREEN after fix:
//   hyp_mem_collect() and/or TRUNCATE checkpoint called periodically by the
...
TEST(repro_issue581_query_rss_stable) {
    ...
    for (int i = 0; i < ITER_TOTAL; i++) {
        char *resp = hyp_mcp_handle_tool(lp.srv, "search_graph", args);
    ...
    //   (b) the fix — periodic SQLITE_CHECKPOINT_TRUNCATE + hyp_mem_collect() in
