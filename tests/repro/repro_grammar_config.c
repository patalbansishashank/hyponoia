 ...
 * comment.
 *
 * Languages covered (16) and the HYP_LANG_* enum each uses (all verified in
 * internal/hyp/hyp.h):
 *   JSON       -> HYP_LANG_JSON
 ...
 *   DOTENV     -> HYP_LANG_DOTENV
 *   KDL        -> HYP_LANG_KDL
 *   RON        -> HYP_LANG_RON
 *   PKL        -> HYP_LANG_PKL
 *   NICKEL     -> HYP_LANG_NICKEL
 *   JSONNET    -> HYP_LANG_JSONNET
 *   STARLARK   -> HYP_LANG_STARLARK
 *
 * BATTERY DIMENSIONS
 * ------------------
...
TEST(repro_grammar_config_json) {
    static const char src[] =
        ...
        "  \"name\": \"hyp\",\n"
        "  \"version\": \"0.8.1\",\n"
        "  \"description\": \"Hyponoia MCP server\",\n"
        "  \"config\": {\n"
        "    \"port\": 8080,\n"
        "    \"debug\": false,\n"
        "    \"tags\": [\"a\", \"b\"]\n"
    ...
    if (config_base_battery("JSON", src, HYP_LANG_JSON, "config.json") != 0)
        return 1;
    return config_robustness("JSON", bad, HYP_LANG_JSON, "config.json");
}
...
TEST(repro_grammar_config_json5) {
    static const char src[] =
        ...
        "{\n"
        "  name: 'hyp',       // unquoted keys + single-quoted values\n"
        "  version: '0.8.1',\n"
        "  features: [\n"
        "    'graph',\n"
        "    'lsp',\n"
    ...
    if (config_base_battery("JSON5", src, HYP_LANG_JSON5, "config.json5") != 0)
        return 1;
    return config_robustness("JSON5", bad, HYP_LANG_JSON5, "config.json5");
}
...
TEST(repro_grammar_config_yaml) {
    static const char src[] =
        ...
        "  - python\n"
        "  - typescript\n";
    static const char bad[] = "name: hyp\n  - broken: [";
    if (config_base_battery("YAML", src, HYP_LANG_YAML, "config.yaml") != 0)
        return 1;
    return config_robustness("YAML", bad, HYP_LANG_YAML, "config.yaml");
}
...
TEST(repro_grammar_config_toml) {
    static const char src[] =
        ...
        "enabled = true\n";
    static const char bad[] = "name = \"hyp\"\n[[language\n";
    if (config_struct_battery("TOML", src, HYP_LANG_TOML, "config.toml",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("TOML", bad, HYP_LANG_TOML, "config.toml");
}
...
TEST(repro_grammar_config_ini) {
    ...
    if (config_struct_battery("INI", src, HYP_LANG_INI, "config.ini",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("INI", bad, HYP_LANG_INI, "config.ini");
}
...
TEST(repro_grammar_config_hcl) {
    ...
    if (config_callable_battery("HCL", src, HYP_LANG_HCL, "main.tf",
                                NULL, "tomap") != 0)
        return 1;
    return config_robustness("HCL", bad, HYP_LANG_HCL, "main.tf");
}
...
TEST(repro_grammar_config_xml) {
    static const char src[] =
        ...
        "  </server>\n"
        "  <database>\n"
        "    <url>postgres://localhost/hyp</url>\n"
        "    <maxConns>10</maxConns>\n"
        "  </database>\n"
        "</config>\n";
    static const char bad[] = "<config>\n  <server>\n    <host>";
    if (config_struct_battery("XML", src, HYP_LANG_XML, "config.xml",
                              "Class", NULL) != 0)
        return 1;
    return config_robustness("XML", bad, HYP_LANG_XML, "config.xml");
}
...
TEST(repro_grammar_config_csv) {
    static const char src[] =
        ...
        "1,hyp-go,go,true\n"
        "2,hyp-py,python,true\n"
        "3,hyp-ts,typescript,false\n";
    static const char bad[] = "id,name\n1,\"unclosed";
    if (config_base_battery("CSV", src, HYP_LANG_CSV, "data.csv") != 0)
        return 1;
    return config_robustness("CSV", bad, HYP_LANG_CSV, "data.csv");
}
...
TEST(repro_grammar_config_properties) {
    static const char src[] =
        ...
        "server.port=8080\n"
        "db.url=jdbc:postgresql://localhost/hyp\n"
        "db.pool.size=10\n";
    static const char bad[] = "app.name=hyp\nbroken";
    if (config_struct_battery("PROPERTIES", src, HYP_LANG_PROPERTIES,
                              "app.properties", "Variable", NULL) != 0)
        return 1;
    return config_robustness("PROPERTIES", bad, HYP_LANG_PROPERTIES,
                             "app.properties");
}
...
TEST(repro_grammar_config_dotenv) {
    static const char src[] =
        "# Database\n"
        "DATABASE_URL=postgres://localhost:5432/hyp\n"
    ...
    if (config_base_battery("DOTENV", src, HYP_LANG_DOTENV, ".env") != 0)
        return 1;
    return config_robustness("DOTENV", bad, HYP_LANG_DOTENV, ".env");
}
...
TEST(repro_grammar_config_kdl) {
    ...
    if (config_base_battery("KDL", src, HYP_LANG_KDL, "config.kdl") != 0)
        return 1;
    return config_robustness("KDL", bad, HYP_LANG_KDL, "config.kdl");
}
...
TEST(repro_grammar_config_ron) {
    static const char src[] =
        ...
        ")\n";
    static const char bad[] = "Config(\n  name: \"hyp\",\n  broken: [";
    if (config_base_battery("RON", src, HYP_LANG_RON, "config.ron") != 0)
        return 1;
    return config_robustness("RON", bad, HYP_LANG_RON, "config.ron");
}
...
TEST(repro_grammar_config_pkl) {
    static const char src[] =
        ...
        "  port = 9000\n"
        "}\n";
    static const char bad[] = "module hyp.Config\nclass Server {\n  host:";
    if (config_struct_battery("PKL", src, HYP_LANG_PKL, "config.pkl",
                              "Class", "Function") != 0)
        return 1;
    return config_robustness("PKL", bad, HYP_LANG_PKL, "config.pkl");
}
...
TEST(repro_grammar_config_nickel) {
    ...
    if (config_callable_battery("Nickel", src, HYP_LANG_NICKEL, "config.ncl",
                                "Function", "addPort") != 0)
        return 1;
    if (config_robustness("Nickel", bad, HYP_LANG_NICKEL, "config.ncl") != 0)
        return 1;
...
TEST(repro_grammar_config_jsonnet) {
    ...
    if (config_callable_battery("Jsonnet", src, HYP_LANG_JSONNET, "config.jsonnet",
                                "Function", "makeServer") != 0)
        return 1;
    if (config_robustness("Jsonnet", bad, HYP_LANG_JSONNET, "config.jsonnet") != 0)
        return 1;
...
TEST(repro_grammar_config_starlark) {
    ...
    if (config_callable_battery("Starlark", src, HYP_LANG_STARLARK, "BUILD",
                                "Function", "go_binary") != 0)
        return 1;
    if (config_robustness("Starlark", bad, HYP_LANG_STARLARK, "BUILD") != 0)
        return 1;
