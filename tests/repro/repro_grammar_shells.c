 ...
 * read directly from internal/hyp/lang_specs.c (the func/class/field/call type
 * arrays per HYP_LANG_*). The dimensions applied per language are documented in
 * the per-TEST comment.
 *
 * Languages covered (19) and the HYP_LANG_* enum each uses (all verified present
 * in internal/hyp/hyp.h):
 *   BASH       -> HYP_LANG_BASH        (callable: func + call)
 *   ZSH        -> HYP_LANG_ZSH         (callable: func + call)
 *   FISH       -> HYP_LANG_FISH        (callable: func + call)
 *   POWERSHELL -> HYP_LANG_POWERSHELL  (callable: func + class + call)
 *   TCL        -> HYP_LANG_TCL         (callable: func + class + call)
 *   AWK        -> HYP_LANG_AWK         (callable: func + call)
 *   VIMSCRIPT  -> HYP_LANG_VIMSCRIPT   (callable: func + call)
 *   FENNEL     -> HYP_LANG_FENNEL      (callable: func + call, lisp)
 *   NIX        -> HYP_LANG_NIX         (callable: func + call)
 *   GDSCRIPT   -> HYP_LANG_GDSCRIPT    (callable: func + class + call)
 *   LUAU       -> HYP_LANG_LUAU        (callable: func + class + call)
 *   TEAL       -> HYP_LANG_TEAL        (callable: func + class + call)
 *   LLVM_IR    -> HYP_LANG_LLVM_IR     (callable: func + call)
 *   NASM       -> HYP_LANG_NASM        (callable: func(label) + call)
 *   JANET      -> HYP_LANG_JANET       (STRUCTURAL ONLY: spec has only module_types)
 *   SMALI      -> HYP_LANG_SMALI       (structural-with-defs: func/class/field, NO calls)
 *   DEVICETREE -> HYP_LANG_DEVICETREE  (structural: call_types but NO func anchor)
 *   KCONFIG    -> HYP_LANG_KCONFIG     (structural-with-defs: class_types, NO calls)
 *   HYPRLANG   -> HYP_LANG_HYPRLANG    (pure structural: only module_types)
 *
 * No language in this set was skipped; every HYP_LANG_* above is defined in hyp.h.
 *
 * SPEC-DRIVEN CLASSIFICATION (from internal/hyp/lang_specs.c)
 * ----------------------------------------------------------
 ...
 *   R. extract-on-malformed: hyp_extract_file on a truncated/broken fixture must
 ...
 * The enclosing-func walk hyp_find_enclosing_func() uses func_kinds_for_lang()
 * in internal/hyp/helpers.c. In that switch ONLY HYP_LANG_BASH has a dedicated
 ...
 */
static int sh_base_battery(const char *lang_tag, const char *src, HYPLanguage lang,
                           const char *file) {
 ...
 * Returns 0 on PASS.
 */
static int sh_struct_battery(const char *lang_tag, const char *src, HYPLanguage lang,
 ...
 */
static int sh_callable_battery(const char *lang_tag, const char *src, HYPLanguage lang,
                               const char *file, const char *expect_label,
 ...
 * Returns 0 on PASS.
 */
static int sh_robustness(const char *lang_tag, const char *bad_src, HYPLanguage lang,
                         const char *file) {
    const char *RED = tf_red();
    const char *RST = tf_reset();

    HYPFileResult *r =
        hyp_extract_file(bad_src, (int)strlen(bad_src), lang, "t", file, 0, NULL, NULL);
    if (!r) {
...
TEST(repro_grammar_shells_bash) {
    ...
    if (sh_callable_battery("BASH", src, HYP_LANG_BASH, "run.sh",
                            "Function", NULL, "compute_inner") != 0)
        return 1;
    if (sh_robustness("BASH", bad, HYP_LANG_BASH, "run.sh") != 0)
        return 1;
...
TEST(repro_grammar_shells_zsh) {
    ...
    if (sh_callable_battery("ZSH", src, HYP_LANG_ZSH, "run.zsh",
                            "Function", NULL, "inner_fn") != 0)
        return 1;
    if (sh_robustness("ZSH", bad, HYP_LANG_ZSH, "run.zsh") != 0)
        return 1;
...
TEST(repro_grammar_shells_fish) {
    ...
    if (sh_callable_battery("FISH", src, HYP_LANG_FISH, "run.fish",
                            "Function", NULL, "inner_fn") != 0)
        return 1;
    if (sh_robustness("FISH", bad, HYP_LANG_FISH, "run.fish") != 0)
        return 1;
...
TEST(repro_grammar_shells_powershell) {
    ...
    if (sh_callable_battery("PowerShell", src, HYP_LANG_POWERSHELL, "run.ps1",
                            "Function", NULL, "Get-Inner") != 0)
        return 1;
    if (sh_robustness("PowerShell", bad, HYP_LANG_POWERSHELL, "run.ps1") != 0)
        return 1;
...
TEST(repro_grammar_shells_tcl) {
    ...
    if (sh_callable_battery("TCL", src, HYP_LANG_TCL, "run.tcl",
                            "Function", NULL, "inner_proc") != 0)
        return 1;
    if (sh_robustness("TCL", bad, HYP_LANG_TCL, "run.tcl") != 0)
        return 1;
...
TEST(repro_grammar_shells_awk) {
    ...
    if (sh_callable_battery("AWK", src, HYP_LANG_AWK, "prog.awk",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("AWK", bad, HYP_LANG_AWK, "prog.awk") != 0)
        return 1;
...
TEST(repro_grammar_shells_vimscript) {
    ...
    if (sh_callable_battery("VimScript", src, HYP_LANG_VIMSCRIPT, "plugin.vim",
                            "Function", NULL, "Inner") != 0)
        return 1;
    if (sh_robustness("VimScript", bad, HYP_LANG_VIMSCRIPT, "plugin.vim") != 0)
        return 1;
...
TEST(repro_grammar_shells_fennel) {
    ...
    if (sh_callable_battery("Fennel", src, HYP_LANG_FENNEL, "init.fnl",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Fennel", bad, HYP_LANG_FENNEL, "init.fnl") != 0)
        return 1;
...
TEST(repro_grammar_shells_nix) {
    ...
    if (sh_callable_battery("Nix", src, HYP_LANG_NIX, "default.nix",
                            "Function", NULL, "addOne") != 0)
        return 1;
    if (sh_robustness("Nix", bad, HYP_LANG_NIX, "default.nix") != 0)
        return 1;
...
TEST(repro_grammar_shells_gdscript) {
    ...
    if (sh_callable_battery("GDScript", src, HYP_LANG_GDSCRIPT, "calc.gd",
                            "Function", NULL, "_inner") != 0)
        return 1;
    if (sh_robustness("GDScript", bad, HYP_LANG_GDSCRIPT, "calc.gd") != 0)
        return 1;
...
TEST(repro_grammar_shells_luau) {
    ...
    if (sh_callable_battery("Luau", src, HYP_LANG_LUAU, "mod.luau",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Luau", bad, HYP_LANG_LUAU, "mod.luau") != 0)
        return 1;
...
TEST(repro_grammar_shells_teal) {
    ...
    static const char src[] =
        ...
        "end\n";
    static const char bad[] = "local function outer(x: number): number\n    return inner(";
    if (sh_callable_battery("Teal", src, HYP_LANG_TEAL, "mod.tl",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("Teal", bad, HYP_LANG_TEAL, "mod.tl") != 0)
        return 1;
...
TEST(repro_grammar_shells_llvm_ir) {
    ...
    if (sh_callable_battery("LLVM-IR", src, HYP_LANG_LLVM_IR, "mod.ll",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("LLVM-IR", bad, HYP_LANG_LLVM_IR, "mod.ll") != 0)
        return 1;
...
TEST(repro_grammar_shells_nasm) {
    ...
    if (sh_callable_battery("NASM", src, HYP_LANG_NASM, "prog.asm",
                            "Function", NULL, "inner") != 0)
        return 1;
    if (sh_robustness("NASM", bad, HYP_LANG_NASM, "prog.asm") != 0)
        return 1;
    return sh_pipeline_battery("NASM", "prog.asm", src);
}

/* ── JANET (structural only) ──────────────────────────────────────────────────
 * Idiomatic Janet with a defn and a call. spec entry HYP_LANG_JANET maps ONLY
...
TEST(repro_grammar_shells_janet) {
    ...
    if (sh_base_battery("Janet", src, HYP_LANG_JANET, "init.janet") != 0)
        return 1;
    return sh_robustness("Janet", bad, HYP_LANG_JANET, "init.janet");
}
...
TEST(repro_grammar_shells_smali) {
    ...
    if (sh_struct_battery("Smali", src, HYP_LANG_SMALI, "Calculator.smali",
                          "Class", "Function", "Field") != 0)
        return 1;
    return sh_robustness("Smali", bad, HYP_LANG_SMALI, "Calculator.smali");
}
...
TEST(repro_grammar_shells_devicetree) {
    ...
    if (sh_base_battery("DeviceTree", src, HYP_LANG_DEVICETREE, "board.dts") != 0)
        return 1;
    return sh_robustness("DeviceTree", bad, HYP_LANG_DEVICETREE, "board.dts");
}
...
TEST(repro_grammar_shells_kconfig) {
    ...
    if (sh_struct_battery("Kconfig", src, HYP_LANG_KCONFIG, "Kconfig",
                          "Class", NULL, NULL) != 0)
        return 1;
    return sh_robustness("Kconfig", bad, HYP_LANG_KCONFIG, "Kconfig");
}

/* ── HYPRLANG (pure structural) ───────────────────────────────────────────────
 * Idiomatic Hyprland config with sections and key=value assignments. spec entry
 * HYP_LANG_HYPRLANG maps ONLY module_types=source_file; every other type array
...
TEST(repro_grammar_shells_hyprlang) {
    ...
    if (sh_base_battery("Hyprlang", src, HYP_LANG_HYPRLANG, "hyprland.conf") != 0)
        return 1;
    return sh_robustness("Hyprlang", bad, HYP_LANG_HYPRLANG, "hyprland.conf");
}
