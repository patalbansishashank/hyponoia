 ...
 *   src/cli/cli.c  hyp_cmd_install()  printed
 *     "Found %d existing index(es) that must be rebuilt:\n"
 *   then called hyp_remove_indexes(home) which unlinked every .db and NEVER
 ...
 *
 *     int hyp_install_handle_existing_indexes(const char *home,
 ...
 *        hyp_install_handle_existing_indexes(home, reset=false, dry_run=false)
 ...
 *        hyp_install_handle_existing_indexes(home, reset=true, dry_run=false)
 ...
 *      HYP_ASSUME_YES so the test is non-interactive.
 *
...
#define REPRO607_PROJECT "hyp-repro607-test"
...
static int repro607_make_index(const char *tmp_cache, char *db_path, size_t db_path_sz) {
    ...
    int upsert_rc =
        hyp_store_upsert_project(setup_store, REPRO607_PROJECT, "/home/user/my-project");
    hyp_store_close(setup_store);
    return (upsert_rc == HYP_STORE_OK) ? 1 : 0;
}
...
TEST(repro_issue607_reinstall_preserves_index) {
    /* Redirect HYP_CACHE_DIR to a fresh temp dir so the real user cache is
     * never touched and count_db_indexes()/hyp_list_indexes() see only the
     * DB we create here. */
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/hyp_repro607_XXXXXX");
     ...
     * call hyp_remove_indexes(). The fix must NOT delete here regardless.
     */
    int proceed =
        hyp_install_handle_existing_indexes(tmp_cache /* fake home */, false /* reset */,
                                            false /* dry_run */);
 ...
 * --reset-indexes flag. Auto-answers the delete prompt via HYP_ASSUME_YES so
 * the test stays non-interactive.
 * ─────────────────────────────────────────────────────────────────── */
TEST(repro_issue607_reset_indexes_deletes) {
    char tmp_cache[512];
    snprintf(tmp_cache, sizeof(tmp_cache), "/tmp/hyp_repro607r_XXXXXX");
    if (!hyp_mkdtemp(tmp_cache)) {
    ...
    hyp_set_auto_answer_for_test(1 /* AUTO_YES */);

    /* Opt-in destructive path: reset=true must delete the index. */
    int proceed =
        hyp_install_handle_existing_indexes(tmp_cache /* fake home */, true /* reset */,
                                            false /* dry_run */);
    int proceeded = (proceed == 1);
    ...
    hyp_set_auto_answer_for_test(0 /* prompt */);
