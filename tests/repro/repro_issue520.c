...
TEST(repro_issue520_detect_changes_includes_new_untracked_file) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_r520_XXXXXX");
    ...
    if (!srv) {
        th_rmtree(tmpdir);
        FAIL("hyp_mcp_server_new returned NULL");
    }
    ...
    char *dc_project = hyp_project_name_from_path(tmpdir);
    if (!dc_project) {
        hyp_mcp_server_free(srv);
        th_rmtree(tmpdir);
        FAIL("hyp_project_name_from_path failed");
    }
    char dc_args[640];
