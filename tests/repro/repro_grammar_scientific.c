 ...
 *
 * Languages covered (15) and the HYP_LANG_* enum each uses (all verified present
 * in internal/hyp/hyp.h -- none missing, none skipped):
 *   GLSL     -> HYP_LANG_GLSL      (shader; reuses C node types)
 *   HLSL     -> HYP_LANG_HLSL      (shader; C++-family node types)
 *   WGSL     -> HYP_LANG_WGSL      (shader; own grammar)
 *   ISPC     -> HYP_LANG_ISPC      (shader/SIMD; C-family node types)
 *   Slang    -> HYP_LANG_SLANG     (shader; C++-family node types)
 *   Cairo    -> HYP_LANG_CAIRO     (smart-contract; Rust-like)
 *   Sway     -> HYP_LANG_SWAY      (smart-contract; Rust-like)
 *   FunC     -> HYP_LANG_FUNC      (smart-contract; TON)
 *   Wolfram  -> HYP_LANG_WOLFRAM   (CAS; assignment-as-definition)
 *   MATLAB   -> HYP_LANG_MATLAB    (numeric)
 *   Magma    -> HYP_LANG_MAGMA     (CAS)
 *   FORM     -> HYP_LANG_FORM      (symbolic; procedure_definition / call_statement)
 *   TLA+     -> HYP_LANG_TLAPLUS   (formal spec; operator_definition)
 *   Agda     -> HYP_LANG_AGDA      (dependently-typed)
 *   Apex     -> HYP_LANG_APEX      (Salesforce; Java-like, methods only)
 *
 ...
 * via the same hyp_enclosing_func_qn -> Module fallback documented in
...
static int robustness_probe(const char *lang_tag, const char *bad_src,
    ...
    const char *RED = tf_red();
    const char *RST = tf_reset();
    HYPFileResult *r = inv_rx(bad_src, lang, file);
    if (!r) {
...
TEST(repro_grammar_scientific_glsl) {
    static const char src[] =
        ...
        "    gl_FragColor = vec4(v);\n"
        "}\n";
    if (single_file_battery("GLSL", src, HYP_LANG_GLSL, "shader.frag",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("GLSL", "void main() { float v = scale(",
                         HYP_LANG_GLSL, "shader.frag") != 0)
        return 1;
...
TEST(repro_grammar_scientific_hlsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("HLSL", src, HYP_LANG_HLSL, "shader.hlsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("HLSL", "float4 PSMain( { return scale(",
                         HYP_LANG_HLSL, "shader.hlsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wgsl) {
    static const char src[] =
        ...
        "}\n";
    if (single_file_battery("WGSL", src, HYP_LANG_WGSL, "shader.wgsl",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("WGSL", "fn fs_main() -> { let v = scale(",
                         HYP_LANG_WGSL, "shader.wgsl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_ispc) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("ISPC", src, HYP_LANG_ISPC, "kernel.ispc",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("ISPC", "export void run( { out[0] = scale(",
                         HYP_LANG_ISPC, "kernel.ispc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_slang) {
    static const char src[] =
        ...
        "    outBuf[tid.x] = v;\n"
        "}\n";
    if (single_file_battery("Slang", src, HYP_LANG_SLANG, "shader.slang",
                            "Function", NULL, "scale") != 0)
        return 1;
    if (robustness_probe("Slang", "void csMain( { float v = scale(",
                         HYP_LANG_SLANG, "shader.slang") != 0)
        return 1;
...
TEST(repro_grammar_scientific_cairo) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Cairo", src, HYP_LANG_CAIRO, "lib.cairo",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Cairo", "fn compute(x: felt252) -> { add(",
                         HYP_LANG_CAIRO, "lib.cairo") != 0)
        return 1;
...
TEST(repro_grammar_scientific_sway) {
    static const char src[] =
        ...
        "    add(x, 1)\n"
        "}\n";
    if (single_file_battery("Sway", src, HYP_LANG_SWAY, "main.sw",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Sway", "fn compute(x: u64) -> { add(",
                         HYP_LANG_SWAY, "main.sw") != 0)
        return 1;
...
TEST(repro_grammar_scientific_func) {
    static const char src[] =
        ...
        "    return add(x, 1);\n"
        "}\n";
    if (single_file_battery("FunC", src, HYP_LANG_FUNC, "contract.fc",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FunC", "int compute(int x) { return add(",
                         HYP_LANG_FUNC, "contract.fc") != 0)
        return 1;
...
TEST(repro_grammar_scientific_wolfram) {
    static const char src[] =
        ...
        "\n"
        "compute[x_] := add[x, 1]\n";
    if (single_file_battery("Wolfram", src, HYP_LANG_WOLFRAM, "calc.wl",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Wolfram", "compute[x_] := add[x,",
                         HYP_LANG_WOLFRAM, "calc.wl") != 0)
        return 1;
...
TEST(repro_grammar_scientific_matlab) {
    static const char src[] =
        ...
        "    s = a + b;\n"
        "end\n";
    if (single_file_battery("MATLAB", src, HYP_LANG_MATLAB, "calc.m",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("MATLAB", "function r = compute(x)\n  r = add(",
                         HYP_LANG_MATLAB, "calc.m") != 0)
        return 1;
...
TEST(repro_grammar_scientific_magma) {
    static const char src[] =
        ...
        "    return Add(x, 1);\n"
        "end function;\n";
    if (single_file_battery("Magma", src, HYP_LANG_MAGMA, "calc.magma",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("Magma", "function Compute(x)\n  return Add(",
                         HYP_LANG_MAGMA, "calc.magma") != 0)
        return 1;
...
TEST(repro_grammar_scientific_form) {
    static const char src[] =
        ...
        "    #call add(`y')\n"
        "#endprocedure\n";
    if (single_file_battery("FORM", src, HYP_LANG_FORM, "calc.frm",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("FORM", "#procedure compute(y)\n  #call add(",
                         HYP_LANG_FORM, "calc.frm") != 0)
        return 1;
...
TEST(repro_grammar_scientific_tlaplus) {
    static const char src[] =
        ...
        "Compute(x) == Add(x, 1)\n"
        "====\n";
    if (single_file_battery("TLA+", src, HYP_LANG_TLAPLUS, "Calc.tla",
                            "Function", NULL, "Add") != 0)
        return 1;
    if (robustness_probe("TLA+", "---- MODULE Calc ----\nCompute(x) == Add(",
                         HYP_LANG_TLAPLUS, "Calc.tla") != 0)
        return 1;
...
TEST(repro_grammar_scientific_agda) {
    static const char src[] =
        ...
        "compute : Nat -> Nat\n"
        "compute x = add x 1\n";
    if (single_file_battery("Agda", src, HYP_LANG_AGDA, "Calc.agda",
                            "Function", NULL, "add") != 0)
        return 1;
    if (robustness_probe("Agda", "module Calc where\ncompute x = add x",
                         HYP_LANG_AGDA, "Calc.agda") != 0)
        return 1;
...
TEST(repro_grammar_scientific_apex) {
    static const char src[] =
        ...
        "    }\n"
        "}\n";
    if (single_file_battery("Apex", src, HYP_LANG_APEX, "Calculator.cls",
                            "Method", "Class", "add") != 0)
        return 1;
    if (robustness_probe("Apex", "public class Calculator { Integer compute() { return add(",
                         HYP_LANG_APEX, "Calculator.cls") != 0)
        return 1;
