 ...
 *   helpers.c  hyp_find_enclosing_func() (helpers.c:700) walks a call node's
 ...
 *   generic strings, hyp_find_enclosing_func() never matches, returns the null
 *   node, and hyp_enclosing_func_qn() falls back to the MODULE qn. Every call
 ...
 *   language's spec->func_types (lang_specs.c) so hyp_find_enclosing_func uses
 ...
 *   hyp_store_find_node_by_id(store, e.source_id, &src) == HYP_STORE_OK AND
...
static int run_parity_case(const parity_case_t *c) {
    ...
    int         nedges = 0;
    int rc = hyp_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &nedges);
    if (rc != HYP_STORE_OK) {
        printf("  %sFAIL%s  [%s] hyp_store_find_edges_by_type rc=%d\n", RED, RST, c->name, rc);
        rh_cleanup(&lp, store);
        return 1;
    }
...
static const parity_case_t case_fortran = {
    HYP_LANG_FORTRAN, "Fortran", "a.f90",
 ...
 * HYP_LANG_VERILOG via EXT_TABLE.)
 */
static const parity_case_t case_verilog = {
    HYP_LANG_VERILOG, "Verilog", "a.sv",
    "module m;\n"
...
static const parity_case_t case_commonlisp = {
    HYP_LANG_COMMONLISP, "CommonLisp", "a.lisp",
    "(defun helper (x)\n"
    "  (* x 2))\n"
    "\n"
    "(defun run ()\n"
...
static const parity_case_t case_emacslisp = {
    HYP_LANG_EMACSLISP, "EmacsLisp", "a.el",
    "(defmacro run (n)\n"
 ...
 * CALLS edge ever forms. Verified to fail identically under HYP_LANG_SYSTEMVERILOG
 * (function_subroutine_call). This is a tree-sitter grammar defect, not a hyp
