 ...
 *   Graph quality audit over the real hyponoia repo showed only
 *   3.69% of CALLS edges are callable-sourced (207/5607). The dominant
 *   failure mode is hyp_enclosing_func_qn returning the module QN when
 *   hyp_find_enclosing_func cannot walk the TSNode ancestry back to a
 ...
 *   type matches the list, hyp_find_enclosing_func returns a null node and
 *   hyp_enclosing_func_qn falls back to the module QN. The LSP rescue path
 ...
 *                  Rust LSP is hybrid but hyp_pxc_has_cross_lsp returns false
 *                  for HYP_LANG_RUST (pass_lsp_cross.c:281). The enclosing-
 ...
 *
 * ASSERTION (per edge):
 *   For every hyp_edge_t e where e.type == "CALLS":
 *     hyp_store_find_node_by_id(store, e.source_id, &src) == HYP_STORE_OK
...
static int assert_calls_callable_sourced(const char *lang_tag,
    ...
    for (int i = 0; i < nedges; i++) {
        hyp_node_t src;
        if (hyp_store_find_node_by_id(store, edges[i].source_id, &src)
                != HYP_STORE_OK) {
 ...
 * hyp_enclosing_func_qn path does not correctly resolve the caller QN and
 ...
 * drop the class qualifier (issue #554 / helpers.c hyp_enclosing_func_qn).
 ...
 * hyp_pxc_has_cross_lsp returns false for HYP_LANG_RUST (pass_lsp_cross.c:281)
