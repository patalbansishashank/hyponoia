 ...
 *     def.qualified_name = hyp_fqn_compute(project, rel_path, name)
...
/* Extract a Rust source string and return the raw HYPFileResult.
 * Caller must hyp_free_result() the returned pointer. */
static HYPFileResult *rx(const char *src, const char *proj, const char *path) {
    return hyp_extract_file(src, (int)strlen(src), HYP_LANG_RUST, proj, path, 0, NULL, NULL);
}

/* Count how many defs in r have exactly this label AND name. */
static int count_defs_named(HYPFileResult *r, const char *label, const char *name) {
    int n = 0;
...
static HYPDefinition *nth_def_named(HYPFileResult *r, const char *label, const char *name, int nth) {
    int seen = 0;
    for (int i = 0; i < r->defs.count; i++) {
        HYPDefinition *d = &r->defs.items[i];
        if (label && (!d->label || strcmp(d->label, label) != 0))
...
TEST(repro_issue495_cfg_gated_twins_distinct) {
    ...
    HYPFileResult *r = rx(src, "t", "src.rs");
    ASSERT_NOT_NULL(r);
    ...
    HYPDefinition *d0 = nth_def_named(r, "Function", "try_extract_pdf_text", 0);
    HYPDefinition *d1 = nth_def_named(r, "Function", "try_extract_pdf_text", 1);
    ASSERT_NOT_NULL(d0);
    ASSERT_NOT_NULL(d1);
     ...
     * the qualified_name. */
    HYPDefinition *real_def = NULL;  /* #[cfg(feature = "rag-pdf")]     */
    HYPDefinition *stub_def = NULL;  /* #[cfg(not(feature = "rag-pdf"))] */
