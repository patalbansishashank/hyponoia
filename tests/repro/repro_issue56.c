 ...
 *   HYPResolvedCall is either absent or marked with low confidence and
 ...
 *   hyp_pipeline → graph store) persists a cross-crate CALLS edge for a
 ...
 *   the LSP layer (hyp_run_rust_lsp_with_manifest called with a parsed
 *   HYPCargoManifest) and confirms the resolver *can* route
 *   `engine::boot()` to `engine.boot` when given the manifest explicitly.
 *   BUT: the production pipeline's per-file extraction path
 *   (hyp_extract_file → hyp_run_rust_lsp) does NOT receive a pre-parsed
 *   workspace manifest — it only gets the individual file's content.
 *   Additionally, hyp_pxc_has_cross_lsp() returns false for HYP_LANG_RUST
 ...
 *   1. hyp_run_rust_lsp is called with NULL manifest (hyp.c:645), so no
 *      workspace metadata is available at extraction time.
 *   2. hyp_pxc_has_cross_lsp returns false for HYP_LANG_RUST
...
TEST(repro_issue56_cross_crate_calls) {
    ...
    int edge_count = 0;
    int rc = hyp_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &edge_count);
    ASSERT_EQ(rc, HYP_STORE_OK);

    int found_cross_crate = 0;
    for (int i = 0; i < edge_count && !found_cross_crate; i++) {
        hyp_node_t target_node;
        if (hyp_store_find_node_by_id(store, edges[i].target_id, &target_node) == HYP_STORE_OK) {
