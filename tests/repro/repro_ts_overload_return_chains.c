...
static int count_resolved_suffix(const HYPResolvedCallArray *calls, const char *callee_suffix,
                                 const char *strategy) {
    if (!calls)
        return 0;
    int count = 0;
    for (int i = 0; i < calls->count; i++) {
        const HYPResolvedCall *call = &calls->items[i];
        if (call->kind == HYP_RESOLVED_INVOCATION && call->confidence >= 0.5f && call->caller_qn &&
...
static int count_any_callee_suffix(const HYPResolvedCallArray *calls, const char *callee_suffix) {
    if (!calls)
        return 0;
    int count = 0;
    for (int i = 0; i < calls->count; i++) {
...
static void print_resolved_calls(const HYPResolvedCallArray *calls) {
    if (!calls)
        return;
    for (int i = 0; i < calls->count; i++) {
        const HYPResolvedCall *call = &calls->items[i];
...
static void init_overload_defs(HYPLSPDef defs[OVERLOAD_DEF_COUNT], HYPLanguage language,
...
static int assert_both_return_chains(const HYPResolvedCallArray *calls, const char *language_name,
...
static int assert_free_function_overload_returns(HYPLanguage language, bool jsx_mode,
    ...
    HYPLSPDef defs[OVERLOAD_DEF_COUNT];
    init_overload_defs(defs, language, &names);

    HYPArena arena;
    hyp_arena_init(&arena);
    HYPTypeRegistry *registry = hyp_ts_build_cross_registry(&arena, defs, OVERLOAD_DEF_COUNT);
    HYPResolvedCallArray calls = {0};
    if (registry) {
        hyp_run_ts_lsp_cross_with_registry(&arena, kFreeFunctionSource,
...
static int assert_namespace_overload_returns(HYPLanguage language, bool jsx_mode,
                                             const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.provider",
        "repro.consumer",
    ...
    HYPResolvedCallArray calls = {0};
    if (registry) {
        hyp_run_ts_lsp_cross_with_registry(&arena, kNamespaceSource, (int)strlen(kNamespaceSource),
...
static int assert_exact_symbol_import_overload_returns(HYPLanguage language, bool jsx_mode,
                                                       const char *language_name) {
    static const TSOverloadFixtureNames names = {
    ...
    HYPResolvedCallArray calls = {0};
    if (registry) {
        hyp_run_ts_lsp_cross_with_registry(
...
static int assert_contextual_callback_uses_selected_overload(HYPLanguage language, bool jsx_mode,
                                                             const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.callback",
        "repro.callback",
    ...
    HYPResolvedCallArray calls = {0};
    if (registry) {
        hyp_run_ts_lsp_cross_with_registry(&arena, kContextualCallbackOverloadSource,
...
static int assert_no_shadowed_return_chains(const HYPResolvedCallArray *calls,
...
static int assert_parameter_shadows_free_function(HYPLanguage language, bool jsx_mode,
                                                  const char *language_name) {
    static const TSOverloadFixtureNames names = {
        "repro.shadow.free",
        "repro.shadow.free",
    ...
    HYPResolvedCallArray calls = {0};
    if (registry) {
        hyp_run_ts_lsp_cross_with_registry(&arena, kShadowedFreeFunctionSource,
...
static int assert_parameter_shadows_namespace(HYPLanguage language, bool jsx_mode,
                                              const char *language_name) {
    static const TSOverloadFixtureNames names = {
...
TEST(repro_ts_free_function_overload_returns_typescript) {
    return assert_free_function_overload_returns(HYP_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_free_function_overload_returns_tsx) {
    return assert_free_function_overload_returns(HYP_LANG_TSX, true, "TSX");
}

TEST(repro_ts_namespace_overload_returns_typescript) {
    return assert_namespace_overload_returns(HYP_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_namespace_overload_returns_tsx) {
    return assert_namespace_overload_returns(HYP_LANG_TSX, true, "TSX");
}

TEST(repro_ts_exact_symbol_import_overload_returns_typescript) {
    return assert_exact_symbol_import_overload_returns(HYP_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_exact_symbol_import_overload_returns_tsx) {
    return assert_exact_symbol_import_overload_returns(HYP_LANG_TSX, true, "TSX");
}

TEST(repro_ts_contextual_callback_uses_selected_overload_typescript) {
    return assert_contextual_callback_uses_selected_overload(HYP_LANG_TYPESCRIPT, false,
                                                             "TypeScript");
}

TEST(repro_ts_contextual_callback_uses_selected_overload_tsx) {
    return assert_contextual_callback_uses_selected_overload(HYP_LANG_TSX, true, "TSX");
}

TEST(repro_ts_parameter_shadows_free_function_typescript) {
    return assert_parameter_shadows_free_function(HYP_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_parameter_shadows_free_function_tsx) {
    return assert_parameter_shadows_free_function(HYP_LANG_TSX, true, "TSX");
}

TEST(repro_ts_parameter_shadows_namespace_typescript) {
    return assert_parameter_shadows_namespace(HYP_LANG_TYPESCRIPT, false, "TypeScript");
}

TEST(repro_ts_parameter_shadows_namespace_tsx) {
    return assert_parameter_shadows_namespace(HYP_LANG_TSX, true, "TSX");
}
