...
int hyp_suite_enabled(const char *name) {
    const char *only = getenv("HYP_REPRO_ONLY");
    if (!only || !*only) {
        return 1;
    }
...
static char *save_selector(void) {
    const char *prior = getenv("HYP_REPRO_ONLY");
    return prior ? hyp_strdup(prior) : NULL;
}

static void restore_selector(char *saved) {
    if (saved) {
        hyp_setenv("HYP_REPRO_ONLY", saved, 1);
        free(saved);
    } else {
        hyp_unsetenv("HYP_REPRO_ONLY");
    }
}
...
TEST(repro_runner_filter_accepts_suite_substring) {
    ...
    hyp_setenv("HYP_REPRO_ONLY", "call_argument", 1);
    int enabled = hyp_suite_enabled("repro_call_argument_usages");
...
TEST(repro_runner_filter_accepts_comma_list) {
    char *saved = save_selector();
    hyp_setenv("HYP_REPRO_ONLY", "language_registry,call_argument", 1);
    int first = hyp_suite_enabled("repro_language_registry");
    int second = hyp_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
    if (!first || !second)
...
TEST(repro_runner_filter_rejects_nonmatch) {
    char *saved = save_selector();
    hyp_setenv("HYP_REPRO_ONLY", "language_registry", 1);
    int enabled = hyp_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
    if (enabled)
...
TEST(repro_runner_filter_unset_or_empty_enables_all) {
    char *saved = save_selector();
    hyp_unsetenv("HYP_REPRO_ONLY");
    int unset_enabled = hyp_suite_enabled("repro_call_argument_usages");
    hyp_setenv("HYP_REPRO_ONLY", "", 1);
    int empty_enabled = hyp_suite_enabled("repro_call_argument_usages");
    restore_selector(saved);
