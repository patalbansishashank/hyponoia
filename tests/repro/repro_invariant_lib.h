 ...
 *
 * Two harness tiers:
 *   - single-file extraction:  inv_rx() / the inv_extract_* checks (hyp_extract_file)
...
#include "repro_harness.h" /* RProj/RFile, rh_index*, hyp_store, <store/store.h> */
#include "hyp.h"
#include <string.h>

/* ── Single-file extraction ─────────────────────────────────────── */

static inline HYPFileResult *inv_rx(const char *src, HYPLanguage lang, const char *file) {
    return hyp_extract_file(src, (int)strlen(src), lang, "t", file, 0, NULL, NULL);
}
...
static inline int inv_extract_clean(const char *src, HYPLanguage lang, const char *file) {
...
static inline int inv_count_bad_labels(HYPFileResult *r) {
    int bad = 0;
...
static inline int inv_count_bad_fqns(HYPFileResult *r) {
    int bad = 0;
    for (int i = 0; i < r->defs.count; i++)
...
static inline int inv_count_bad_ranges(HYPFileResult *r) {
    int bad = 0;
    for (int i = 0; i < r->defs.count; i++) {
...
static inline int inv_count_label(HYPFileResult *r, const char *label) {
    int c = 0;
    for (int i = 0; i < r->defs.count; i++)
...
static inline int inv_has_call(HYPFileResult *r, const char *callee) {
...
static inline void inv_count_calls_by_source(hyp_store_t *store, const char *project,
    ...
    *callable_sourced = 0;
    ...
    for (int i = 0; i < n; i++) {
        hyp_node_t src;
        if (hyp_store_find_node_by_id(store, edges[i].source_id, &src) != HYP_STORE_OK)
            continue;
...
static inline int inv_count_dangling_edges(hyp_store_t *store, const char *project,
                                           const char *type) {
    hyp_edge_t *edges = NULL;
    int n = 0;
    if (hyp_store_find_edges_by_type(store, project, type, &edges, &n) != HYP_STORE_OK)
        return -1;
    int dangling = 0;
    for (int i = 0; i < n; i++) {
        hyp_node_t a, b;
        if (hyp_store_find_node_by_id(store, edges[i].source_id, &a) != HYP_STORE_OK)
            dangling++;
        else if (hyp_store_find_node_by_id(store, edges[i].target_id, &b) != HYP_STORE_OK)
            dangling++;
    }
    hyp_store_free_edges(edges, n);
    return dangling;
...
static inline int inv_edge_has_strategy(hyp_store_t *store, const char *project,
                                        const char *strategy) {
    hyp_edge_t *edges = NULL;
    int n = 0;
    if (hyp_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != HYP_STORE_OK)
...
static inline int inv_no_calls_edge_to_qn(hyp_store_t *store, const char *project,
...
static inline int inv_calls_target_qn_suffix(hyp_store_t *store, const char *project,
                                             const char *suffix) {
    hyp_edge_t *edges = NULL;
    int n = 0;
    if (hyp_store_find_edges_by_type(store, project, "CALLS", &edges, &n) != HYP_STORE_OK)
