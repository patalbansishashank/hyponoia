 ...
 * (internal/hyp/lsp/java_lsp.c) and C# (internal/hyp/lsp/cs_lsp.c) hybrid LSPs.
 *
 ...
 *   resolver can still be ABSENT here if hyp_pipeline_find_lsp_resolution
 ...
 *   grepped from source (grep '"lsp_' internal/hyp/lsp/java_lsp.c):
 ...
 *   from source (grep '"cs_' internal/hyp/lsp/cs_lsp.c):
...
static int assert_lsp_strategy(const char *filename, const char *src,
                               const char *strategy) {
    RProj lp;
    hyp_store_t *store = rh_index(&lp, filename, src);
    if (!store) {
...
static int count_edges_from_callable(hyp_store_t *store, const char *project, const char *edge_type,
    ...
    for (int i = 0; i < edge_count; i++) {
        hyp_node_t source = {0};
        if (hyp_store_find_node_by_id(store, edges[i].source_id, &source) == HYP_STORE_OK &&
...
static int assert_method_reference_semantics(const char *filename, const char *src,
    ...
    RProj project;
    hyp_store_t *store = rh_index(&project, filename, src);
    if (!store) {
