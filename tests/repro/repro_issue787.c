 ...
 *     - sequential: hyp_gbuf_upsert_node updated name/file_path/range in
 ...
 *   1. graph_buffer.c hyp_gbuf_upsert_node: a Module def colliding with a
 ...
 *      hyp_pipeline_node_is_dir_container) is treated as a miss, falling
 ...
 *   - sequential: hyp_gbuf_upsert_node clobbered the Folder node in place;
...
static int collect_usage_sources_n(const RFile *files, int nfiles, char **out, int cap) {
    ...
    int ncand = 0;
    int rc = hyp_store_find_nodes_by_name(store, lp.project, "IRepository",
                                         &candidates, &ncand);
    ...
    int nedges = 0;
    rc = hyp_store_find_edges_by_target_type(store, target_id, "USAGE", &edges, &nedges);
    if (rc != HYP_STORE_OK) {
        rh_cleanup(&lp, store);
        return -1;
    }
