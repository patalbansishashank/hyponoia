 ...
 *
 * Single-file extraction bugs do NOT need this — use hyp_extract_file directly
...
static inline hyp_store_t *rh_open_indexed(RProj *lp) {
    lp->project = hyp_project_name_from_path(lp->tmpdir);
    if (!lp->project)
        return NULL;
    if (!lp->cachedir[0]) {
        snprintf(lp->cachedir, sizeof(lp->cachedir), "/tmp/hyp_repro_cache_XXXXXX");
        if (!hyp_mkdtemp(lp->cachedir))
            return NULL;
    ...
    const char *prior_cache_dir = getenv("HYP_CACHE_DIR");
    char *saved_cache_dir = prior_cache_dir ? hyp_strdup(prior_cache_dir) : NULL;
    hyp_setenv("HYP_CACHE_DIR", lp->cachedir, 1);

    hyp_store_t *store = NULL;
    ...
    if (lp->srv) {
        ...
        store = hyp_store_open_path_query(lp->dbpath);
    }

    if (saved_cache_dir) {
        hyp_setenv("HYP_CACHE_DIR", saved_cache_dir, 1);
...
static inline void rh_cleanup(RProj *lp, hyp_store_t *store);
...
static inline hyp_store_t *rh_index_files(RProj *lp, const RFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/hyp_repro_XXXXXX");
    ...
    hyp_store_t *store = rh_open_indexed(lp);
    if (!store) {
        rh_cleanup(lp, NULL);
    }
    return store;
...
static inline hyp_store_t *rh_index(RProj *lp, const char *filename, const char *content) {
...
static inline int rh_count_edges(hyp_store_t *store, const char *project, const char *edge) {
    return store ? hyp_store_count_edges_by_type(store, project, edge) : -1;
}

/* Count nodes carrying `label`. Returns -1 on query error. */
static inline int rh_count_label(hyp_store_t *store, const char *project, const char *label) {
...
/* TIER B: returns true if hyp_extract_file CRASHES (signal) on `content`.
 * Runs in a forked child so the crash doesn't take down the repro runner. */
static inline bool rh_extract_crashes(const char *content, HYPLanguage lang, const char *relpath) {
#if defined(_WIN32)
    HYPFileResult *r =
        hyp_extract_file(content, (int)strlen(content), lang, "repro", relpath, 0, NULL, NULL);
    if (r)
        hyp_free_result(r);
    return false;
#else
