 ...
 *   https://github.com/patalbansishashank/hyponoia/issues/570
 *
 * TWO FILES WRONGLY WRITTEN (Codex SessionStart hook):
 *   ~/.codex/config.toml   -- always written by hyp_upsert_codex_hooks()
 ...
 *   target to hyp_upsert_codex_hooks():
 *
 *     snprintf(cp, sizeof(cp), "%s/.codex/config.toml", home);
 *     ...
 *     hyp_upsert_codex_hooks(cp);
 *
 ...
 *
 *   So hyp_build_install_plan_json() always lists config.toml as the Codex
 *   hook target, even when hooks.json is already in use.
 *
 * EXPECTED vs ACTUAL (oracle: hyp_build_install_plan_json plan JSON):
 ...
 *   hyp_detect_agents() sets agents.codex = dir_exists("~/.codex").
 ...
 *     - If it does, pass that path to hyp_upsert_codex_session_hooks_json()
 ...
 *
 * Oracle: hyp_build_install_plan_json(home, binary) -- dry-run plan, no writes.
 *
...
TEST(repro_issue570_no_dual_hook_write) {
    char home[256];
    snprintf(home, sizeof(home), "/tmp/hyp-repro570-XXXXXX");
    if (!hyp_mkdtemp(home))
    ...
    char *json = hyp_build_install_plan_json(home, "/usr/local/bin/hyponoia");
