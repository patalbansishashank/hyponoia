 ...
 *   handle_trace_call_path() calls hyp_store_find_nodes_by_name() to locate
 ...
 *   hyp_store_find_node_by_qn() also returns nothing, so the handler exits
 ...
 *   graph, so hyp_store_bfs() returns visited_count == 0 and the "callers"
 ...
 *   the resolved node id is passed to hyp_store_bfs(), NOT to edge creation.
 *
 ...
 *   FAILS because hyp_store_bfs() finds no hops from the resolved start node.
 *
 ...
 *   hyp_store_find_nodes_by_name() or hyp_store_bfs() in
...
TEST(repro_issue480_trace_path_nonempty_with_calls) {
     ...
     *   project        — lp.project (derived from tmpdir by hyp_project_name_from_path)
     ...
     * hyp_store_bfs() returning 0 hops serialises \"callers\":[] (no caller
