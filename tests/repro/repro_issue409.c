 ...
 *
 * Root cause (as filed):
 *   hyp_install_hook_gate_script wrote the legacy blocking shell gate
 ...
 *
 * Expected (correct) behaviour after hyp_upsert_claude_hooks +
 * hyp_install_hook_gate_script:
 *   1. The gate script written to
 *      <home>/.claude/hooks/hyp-code-discovery-gate
 ...
 *      "hyp-code-discovery-gate" (the shim), not an inline blocking script.
 *
 ...
 *   Then running both hyp_upsert_claude_hooks + hyp_install_hook_gate_script
 *   (the actual install/update code path) and asserting the CORRECT result.
 *
 *   This is the critical gap: existing tests call hyp_install_hook_gate_script
 ...
 *
 * After calling hyp_upsert_claude_hooks + hyp_install_hook_gate_script
 ...
 *   2. settings.json PreToolUse command references "hyp-code-discovery-gate"
 ...
 *   - settings.json does NOT contain "hyp-code-discovery-gate" (wrong command)
 *
 * Oracle used: hyp_upsert_claude_hooks(settings_path) +
 *              hyp_install_hook_gate_script(home, binary_path)
...
TEST(repro_issue409_install_wires_hook_augment_not_blocking_gate) {
     ...
     * overwritten (truncated) by hyp_install_hook_gate_script. */
    char script_path[512];
    snprintf(script_path, sizeof(script_path),
             "%s/hyp-code-discovery-gate", hooks_dir);
    rp409_write_file(script_path,
        "#!/bin/bash\n"
        "# Gate hook: nudges Claude toward hyponoia for code discovery.\n"
        "# First Grep/Glob/Read per session -> block. Subsequent -> allow.\n"
        "# PPID = Claude Code process PID, unique per session.\n"
        "GATE=/tmp/hyp-code-discovery-gate-$PPID\n"
        "if [ -f \"$GATE\" ]; then exit 0; fi\n"
        "touch \"$GATE\"\n"
        "echo 'BLOCKED: use hyponoia' >&2\n"
        "exit 2\n");
     ...
     * /tmp/hyp-code-discovery-gate-$PPID as a per-invocation state file.
    ...
    ASSERT(strstr(settings_data, "hyp-code-discovery-gate") != NULL);
