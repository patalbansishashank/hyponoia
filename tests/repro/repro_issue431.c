 ...
 *   hyp_install_vscode_mcp() itself takes a single config_path argument and
 ...
 *   Concretely: after hyp_build_install_plan_json() (the dry-run oracle for
 ...
 *   hyp_build_install_plan_json() runs the real install logic in dry-run mode.
 ...
 *   the plan) for each discovered profile path, using hyp_install_vscode_mcp.
 */
 ...
 * same on both platforms.  We use the portable hyp_app_config_dir() path on
 * non-Apple builds and the Library path on Apple builds so the detection in
 * hyp_detect_agents() actually fires, which is required for the plan to
 ...
 *
 * After hyp_build_install_plan_json(tmpdir, BIN) the plan JSON must contain:
...
TEST(repro_issue431_vscode_profile_inherits_mcp_json) {
    ...
    char tmpdir[512];
    snprintf(tmpdir, sizeof(tmpdir), "/tmp/hyp_repro431_XXXXXX");
    if (!hyp_mkdtemp(tmpdir))
        FAIL("hyp_mkdtemp failed");

    /* Create the VSCode User dir so hyp_detect_agents() marks vscode=true.
...
#else
    /* Linux: detection uses hyp_app_config_dir() which is XDG-derived.
     * hyp_detect_agents() resolves that internally; we emulate it with
    ...
    if (!agents.vscode) {
         ...
         * hyp_detect_agents() cannot see the fixture home on some platform — in
    ...
    char *plan_json =
        hyp_build_install_plan_json(tmpdir, "/usr/local/bin/hyponoia");
