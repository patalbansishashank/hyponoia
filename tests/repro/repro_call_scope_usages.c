...
static int usage_count(const HYPFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const HYPUsage *usage = &r->usages.items[i];
...
static int call_reference_count(const HYPFileResult *r, const char *caller, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const HYPUsage *usage = &r->usages.items[i];
...
static int call_count(const HYPFileResult *r, const char *caller, const char *target) {
    int count = 0;
...
static int exact_usage_count(const HYPFileResult *r, const char *caller_qn, const char *target) {
    int count = 0;
    for (int i = 0; i < r->usages.count; i++) {
        const HYPUsage *usage = &r->usages.items[i];
...
static int exact_call_count(const HYPFileResult *r, const char *caller_qn, const char *callee) {
    int count = 0;
    for (int i = 0; i < r->calls.count; i++) {
        const HYPCall *call = &r->calls.items[i];
...
static int exact_definition_count(const HYPFileResult *r, const char *name,
                                  const char *qualified_name) {
    int count = 0;
    for (int i = 0; i < r->defs.count; i++) {
        const HYPDefinition *def = &r->defs.items[i];
...
static int definition_count(const HYPFileResult *r, const char *name) {
    int count = 0;
    for (int i = 0; i < r->defs.count; i++) {
        const HYPDefinition *def = &r->defs.items[i];
...
static HYPFileResult *extract_case(const char *tag, const char *source, HYPLanguage language,
                                   const char *filename) {
    HYPFileResult *r =
...
TEST(repro_call_scope_same_spelling_callee_and_argument) {
    ...
    HYPFileResult *r = extract_case("same_spelling", source, HYP_LANG_JAVASCRIPT, "main.js");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_nested_calls_retain_each_argument) {
    ...
    HYPFileResult *r = extract_case("nested_calls", source, HYP_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_inline_callback_body_usage) {
    ...
    HYPFileResult *r = extract_case("inline_callback", source, HYP_LANG_JAVASCRIPT, "main.js");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_member_and_computed_components) {
    ...
    HYPFileResult *r = extract_case("member_computed", source, HYP_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_constructor_and_generic_arguments) {
    ...
    HYPFileResult *r = extract_case("constructor_generic", source, HYP_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_c_function_pointer_invocation_and_arguments) {
    ...
    HYPFileResult *r = extract_case("c_function_pointer", source, HYP_LANG_C, "main.c");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_cpp_function_pointer_invocation_and_arguments) {
    ...
    HYPFileResult *r = extract_case("cpp_function_pointer", source, HYP_LANG_CPP, "main.cpp");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_python_keyword_argument) {
    ...
    HYPFileResult *r = extract_case("python_keyword", source, HYP_LANG_PYTHON, "main.py");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_python_with_body_usage) {
    static const char source[] = "watched = 1\n"
                                 "def manager():\n    pass\n"
                                 "def run():\n    with manager():\n        watched\n";
    HYPFileResult *r = extract_case("python_with", source, HYP_LANG_PYTHON, "main.py");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_rust_macro_argument) {
    ...
    HYPFileResult *r = extract_case("rust_macro", source, HYP_LANG_RUST, "main.rs");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_elixir_binary_operator_operand) {
    static const char source[] = "defmodule Sample do\n"
                                 ...
                                 "end\n";
    HYPFileResult *r = extract_case("elixir_binary", source, HYP_LANG_ELIXIR, "sample.ex");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_fortran_keyword_argument) {
    static const char source[] = "module sample\n"
                                 ...
                                 "  end function run\n"
                                 "end module sample\n";
    HYPFileResult *r = extract_case("fortran_keyword", source, HYP_LANG_FORTRAN, "main.f90");
    ASSERT_NOT_NULL(r);
    int failures = 0;
 ...
 * subroutine invocation currently emits no HYPCall. Keep this independent from
...
TEST(repro_fortran_subroutine_call_is_extracted) {
    static const char source[] = "subroutine consume(value)\n"
                                 ...
                                 "end subroutine run\n";
    HYPFileResult *r =
        extract_case("fortran_subroutine_call", source, HYP_LANG_FORTRAN, "main.f90");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_kotlin_property_navigation) {
    ...
    HYPFileResult *r = extract_case("kotlin_navigation", source, HYP_LANG_KOTLIN, "Main.kt");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_php_first_class_callable) {
    static const char source[] = "<?php\n"
                                 ...
                                 "  foo(...$args);\n"
                                 "}\n";
    HYPFileResult *r = extract_case("php_first_class", source, HYP_LANG_PHP, "main.php");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_scope_stack_preserves_deepest_function) {
    ...
    HYPFileResult *r = extract_case("deep_scope", source, HYP_LANG_JAVASCRIPT, "deep.js");
    free(source);
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_usage_declarations_are_not_references) {
    static const char source[] = "function run(unusedParameter: number,\n"
                                 ...
                                 "}\n";
    HYPFileResult *r = extract_case("declaration_usage", source, HYP_LANG_TYPESCRIPT, "main.ts");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_objectscript_typed_receiver_exact_owner) {
    ...
    HYPFileResult *r = extract_case("objectscript_typed_receiver", source,
                                    HYP_LANG_OBJECTSCRIPT_UDL, "Behavior.cls");
    ASSERT_NOT_NULL(r);
    int failures = 0;
...
TEST(repro_call_scope_objectscript_routine_tag_exact_owner) {
    ...
    HYPFileResult *r = extract_case("objectscript_routine_scope", source,
                                    HYP_LANG_OBJECTSCRIPT_ROUTINE, "Behavior.mac");
    ASSERT_NOT_NULL(r);
    int failures = 0;
