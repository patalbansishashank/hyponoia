 ...
 *   append_args_json serializes each HYPCallArg as {"i":<index>,"e":"<expr>,...}
 *   into the CALLS edge properties_json column.  However,
 *   bfs_to_json_array() (mcp.c ~line 2283) only emits the node fields (name,
 *   qualified_name, hop, risk, is_test) from hyp_node_hop_t.  The edge that
 *   carried the arg expressions is NOT propagated by hyp_store_bfs() into the
 *   hyp_traverse_result_t (hyp_edge_info_t carries only from_name, to_name,
 ...
 *   hyp_store_bfs() in src/store/store.c must propagate edge properties_json
 *   into the hyp_traverse_result_t (extend hyp_edge_info_t or hyp_node_hop_t).
 ...
 * positional argument to callee().  The extractor captures this as a HYPCallArg
...
TEST(repro_issue514_data_flow_surfaces_arg_expr) {
    RProj lp;
    hyp_store_t *store = rh_index_files(&lp, k_files,
    ...
    char *resp = hyp_mcp_handle_tool(lp.srv, "trace_path", args);
     ...
     *
     * WHY RED on current code:
     *   bfs_to_json_array() (mcp.c ~line 2283) only emits hyp_node_hop_t fields
     *   (name, qualified_name, hop).  hyp_edge_info_t (store.h ~line 146) does
