 ...
 *   (internal/hyp/lsp/kotlin_lsp.c, php_lsp.c, rust_lsp.c).
 *
 ...
 *   HYP_LANG_RUST through the project-wide Rust registry, including exact
...
static int assert_rust_dispatch_files(const RFile *files, int nfiles, const char *caller_substr,
    ...
    int edge_count = 0;
    if (hyp_store_find_edges_by_type(store, lp.project, "CALLS", &edges, &edge_count) !=
        HYP_STORE_OK) {
 ...
 * hyp_registry_lookup_method finds a func whose receiver_type == recv type and
...
TEST(repro_lsp_rust_macro) {
    ...
    RProj lp;
    hyp_store_t *store = rh_index(&lp, "main.rs", kRustMacro);
    if (!store) {
