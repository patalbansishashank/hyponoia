 ...
 *   extract_decorators() in internal/hyp/extract_defs.c first scans
...
static HYPFileResult *rx(const char *src, HYPLanguage lang,
                         const char *proj, const char *path) {
    return hyp_extract_file(src, (int)strlen(src), lang, proj, path,
                            0, NULL, NULL);
}
...
static HYPDefinition *find_def(HYPFileResult *r, const char *label,
                               const char *name) {
 ...
 * hyp_extract_file carry their Java annotations in .decorators:
 *
 *   @Entity
 *   @RestController
 *   public class User {
...
TEST(repro_issue382_java_annotations_on_nodes) {
    HYPFileResult *r = rx(
        ...
        "    public String getUser(String id) { return id; }\n"
        "}\n",
        HYP_LANG_JAVA, "t", "User.java");

    ASSERT_NOT_NULL(r);
    ASSERT_FALSE(r->has_error);

    /* ── Class node: two class-level marker_annotations ── */
    HYPDefinition *cls = find_def(r, "Class", "User");
    ASSERT_NOT_NULL(cls);
    ...
    HYPDefinition *method = find_def(r, "Method", "getUser");
