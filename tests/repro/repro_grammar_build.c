 ...
 *
 * Languages covered (15) and the HYP_LANG_* enum each uses (all verified in
 * internal/hyp/hyp.h; none missing, none skipped):
 ...
 *   Just          -> HYP_LANG_JUST
 *   K8s           -> HYP_LANG_K8S
 *   Kustomize     -> HYP_LANG_KUSTOMIZE
 *   GoMod         -> HYP_LANG_GOMOD
 *   Requirements  -> HYP_LANG_REQUIREMENTS
 *   Gitignore     -> HYP_LANG_GITIGNORE
 *   Gitattributes -> HYP_LANG_GITATTRIBUTES
 *   SSHConfig     -> HYP_LANG_SSHCONFIG
 *   BitBake       -> HYP_LANG_BITBAKE
 *   Puppet        -> HYP_LANG_PUPPET
 *
 * Langs NOT in HYP_LANG_* (skipped, noted):
 *   none -- all 15 target languages are present in the enum.
 *
 * BATTERY DIMENSIONS
 * ------------------
 * SINGLE-FILE (hyp_extract_file, via inv_rx + inv_count_* helpers):
 ...
 *
 * FULL-PIPELINE (rh_index_files -> hyp_store_t*, via inv_count_* store helpers):
 ...
 *      Implemented inline at the end of each TEST via hyp_extract_file directly.
 *
 ...
 *     K8S            -- semantic extractor (hyp_extract_k8s); extracts kind -> "Resource".
 *     KUSTOMIZE      -- semantic extractor (hyp_extract_k8s); extracts kind -> "Resource".
 *
...
static int build_base_battery(const char *lang_tag, const char *src,
                              HYPLanguage lang, const char *file) {
    ...
    HYPFileResult *r = inv_rx(src, lang, file);
    if (!r) {
...
static int build_pipeline_battery(const char *lang_tag, const char *filename,
    ...
    RProj lp;
    hyp_store_t *store = rh_index_files(&lp, files, 1);
    if (!store) {
 ...
 *
 * A truncated version of the fixture is passed through hyp_extract_file.
...
static int build_robustness(const char *lang_tag, const char *bad_src,
    ...
    HYPFileResult *r = hyp_extract_file(bad_src, (int)strlen(bad_src),
...
TEST(repro_grammar_build_dockerfile) {
    static const char src[] =
        ...
        "ARG VERSION=0.8.1\n"
        "COPY . .\n"
        "RUN go build -o /hyp-server ./cmd/server\n"
        "\n"
        "FROM debian:bookworm-slim\n"
        "ENV PORT=8080\n"
        "ENV LOG_LEVEL=info\n"
        "COPY --from=builder /hyp-server /usr/local/bin/hyp-server\n"
        "EXPOSE 8080\n"
        "ENTRYPOINT [\"/usr/local/bin/hyp-server\"]\n";
    static const char bad[] = "FROM golang:1.22 AS\n";
    if (build_struct_battery("Dockerfile", src, HYP_LANG_DOCKERFILE,
                             "Dockerfile", "Variable", NULL) != 0)
        return 1;
    return build_robustness("Dockerfile", bad, HYP_LANG_DOCKERFILE, "Dockerfile");
}
...
TEST(repro_grammar_build_makefile) {
    ...
    if (build_callable_battery("Makefile", src, HYP_LANG_MAKEFILE, "Makefile",
                               "Function", "shell") != 0)
        return 1;
    return build_robustness("Makefile", bad, HYP_LANG_MAKEFILE, "Makefile");
}
 ...
 *
 * Dims asserted: 1-6 + R.
 * Dim 5 expected GREEN: "Function" def for the function_def "hyp_setup_target".
...
TEST(repro_grammar_build_cmake) {
    static const char src[] =
        ...
        "project(hyp VERSION 0.8.1 LANGUAGES C)\n"
        "\n"
        "set(CMAKE_C_STANDARD 11)\n"
        "\n"
        "function(hyp_setup_target target)\n"
        "    target_include_directories(${target} PRIVATE include)\n"
        "    target_compile_options(${target} PRIVATE -Wall -Wextra)\n"
        "endfunction()\n"
        "\n"
        "add_executable(hyp-server src/main.c src/server.c)\n"
        "hyp_setup_target(hyp-server)\n"
        "target_link_libraries(hyp-server PRIVATE sqlite3)\n";
    static const char bad[] = "cmake_minimum_required(VERSION 3.20\n";
    if (build_callable_battery("CMake", src, HYP_LANG_CMAKE, "CMakeLists.txt",
                               "Function", "add_executable") != 0)
        return 1;
    return build_robustness("CMake", bad, HYP_LANG_CMAKE, "CMakeLists.txt");
}
 ...
 *   "hyp_flags". RED would indicate function_expression->Function extraction or
...
TEST(repro_grammar_build_meson) {
     ...
     * `hyp_flags = func (target) ... endfunc` user-function syntax is not parsed
     ...
     * a hyp bug. Original assertions below are preserved (unreachable). */
    ...
    static const char src[] =
        ...
        "\n"
        "hyp_flags = func (target)\n"
        "    return ['-DVERSION=\"' + target + '\"']\n"
        "endfunc\n"
        "\n"
        "sqlite = dependency('sqlite3')\n"
        "executable('hyp-server',\n"
        "    sources: ['src/main.c', 'src/server.c'],\n"
        "    dependencies: [sqlite],\n"
        "    install: true)\n";
    static const char bad[] = "project('hyp', 'c',\n    version: '0.8.1'";
    if (build_callable_battery("Meson", src, HYP_LANG_MESON, "meson.build",
                               "Function", "executable") != 0)
        return 1;
    return build_robustness("Meson", bad, HYP_LANG_MESON, "meson.build");
}
...
TEST(repro_grammar_build_gn) {
    static const char src[] =
        ...
        "}\n";
    static const char bad[] = "executable(\"hyp-server\") {\n  sources = [";
    if (build_callable_battery("GN", src, HYP_LANG_GN, "BUILD.gn",
                               NULL, "executable") != 0)
        return 1;
    return build_robustness("GN", bad, HYP_LANG_GN, "BUILD.gn");
}
...
TEST(repro_grammar_build_just) {
    static const char src[] =
        ...
        "    just test\n";
    static const char bad[] = "build:\n    go build -o ";
    if (build_callable_battery("Just", src, HYP_LANG_JUST, "justfile",
                               "Function", "build") != 0)
        return 1;
    if (build_robustness("Just", bad, HYP_LANG_JUST, "justfile") != 0)
        return 1;
 ...
 * kind: Deployment). The K8s/Kustomize semantic extractor hyp_extract_k8s()
 * is called for HYP_LANG_K8S; it reads the kind field from the YAML tree and
 ...
 * Dim 5 expected GREEN: "Resource" def extracted by hyp_extract_k8s for the kind.
...
TEST(repro_grammar_build_k8s) {
    ...
    if (build_struct_battery("K8s", src, HYP_LANG_K8S, "deployment.yaml",
                             "Resource", NULL) != 0)
        return 1;
    return build_robustness("K8s", bad, HYP_LANG_K8S, "deployment.yaml");
}

/* ── Kustomize ────────────────────────────────────────────────────────────────
 * Idiomatic kustomization.yaml: the Kustomize overlay tool's root file
 * (kind: Kustomization). hyp_extract_k8s() is called for HYP_LANG_KUSTOMIZE
 * just as for HYP_LANG_K8S; it should mint a "Resource" def for the
 * "Kustomization" kind, which is the canonical Kustomize resource kind.
 *
 * Dims asserted: 1-5 + R ("Resource" for the Kustomization kind).
 * Dim 5 expected GREEN: "Resource" def for "Kustomization" from hyp_extract_k8s.
...
TEST(repro_grammar_build_kustomize) {
    ...
    if (build_struct_battery("Kustomize", src, HYP_LANG_KUSTOMIZE,
                             "kustomization.yaml", "Resource", NULL) != 0)
        return 1;
    return build_robustness("Kustomize", bad, HYP_LANG_KUSTOMIZE,
                            "kustomization.yaml");
}
...
TEST(repro_grammar_build_gomod) {
    static const char src[] =
        "module github.com/patalbansishashank/hyponoia\n"
        "\n"
        "go 1.22\n"
        "\n"
        "require (\n"
        ...
        ")\n";
    static const char bad[] = "module github.com/patalbansishashank/hyponoia\nrequire (";
    if (build_struct_battery("GoMod", src, HYP_LANG_GOMOD, "go.mod",
                             "Variable", NULL) != 0)
        return 1;
    return build_robustness("GoMod", bad, HYP_LANG_GOMOD, "go.mod");
}
...
TEST(repro_grammar_build_requirements) {
    static const char src[] =
        ...
        "\n"
        "# URL requirement\n"
        "hyp-client @ git+https://github.com/patalbansishashank/hyp-client.git@v0.8.1\n";
    static const char bad[] = "requests==2.31.0\nbroken>=";
    if (build_base_battery("Requirements", src, HYP_LANG_REQUIREMENTS,
                           "requirements.txt") != 0)
        return 1;
    return build_robustness("Requirements", bad, HYP_LANG_REQUIREMENTS,
                            "requirements.txt");
}
...
TEST(repro_grammar_build_gitignore) {
    static const char src[] =
        ...
        "coverage.out\n"
        "*.prof\n";
    static const char bad[] = "hyp-server\n[invalid";
    if (build_base_battery("Gitignore", src, HYP_LANG_GITIGNORE, ".gitignore") != 0)
        return 1;
    return build_robustness("Gitignore", bad, HYP_LANG_GITIGNORE, ".gitignore");
}
...
TEST(repro_grammar_build_gitattributes) {
    static const char src[] =
        ...
        "internal/hyp/vendored/** linguist-vendored\n";
    static const char bad[] = "* text=auto eol=lf\n*.go [broken";
    if (build_base_battery("Gitattributes", src, HYP_LANG_GITATTRIBUTES,
                           ".gitattributes") != 0)
        return 1;
    return build_robustness("Gitattributes", bad, HYP_LANG_GITATTRIBUTES,
                            ".gitattributes");
}
...
TEST(repro_grammar_build_sshconfig) {
    ...
    if (build_base_battery("SSHConfig", src, HYP_LANG_SSHCONFIG, "config") != 0)
        return 1;
    return build_robustness("SSHConfig", bad, HYP_LANG_SSHCONFIG, "config");
}
...
TEST(repro_grammar_build_bitbake) {
    ...
    return -1; /* skip — not counted as pass or fail */
    static const char src[] =
        "DESCRIPTION = \"HYP MCP server component\"\n"
        "HOMEPAGE    = \"https://github.com/patalbansishashank/hyponoia\"\n"
        "LICENSE     = \"MIT\"\n"
        "PV          = \"0.8.1\"\n"
        "\n"
        "SRC_URI = \"git://github.com/patalbansishashank/hyponoia.git;protocol=https\"\n"
        "\n"
        "do_fetch() {\n"
        "    git clone ${SRC_URI} ${S}\n"
        "}\n"
        ...
        "\n"
        "do_compile() {\n"
        "    go build -o ${B}/hyp-server ./cmd/server\n"
        "}\n"
        "\n"
        "do_install() {\n"
        "    install -d ${D}${bindir}\n"
        "    install -m 0755 ${B}/hyp-server ${D}${bindir}/\n"
        "}\n";
    static const char bad[] = "DESCRIPTION = \"HYP\"\ndo_fetch() {\n    git clone ";
    if (build_callable_battery("BitBake", src, HYP_LANG_BITBAKE,
                               "hyp-server_0.8.1.bb", "Function", "subprocess") != 0)
        return 1;
    if (build_robustness("BitBake", bad, HYP_LANG_BITBAKE,
                         "hyp-server_0.8.1.bb") != 0)
        return 1;
    return build_pipeline_battery("BitBake", "hyp-server_0.8.1.bb", src);
}
 ...
 *
 * Dims asserted: 1-8 (full battery).
 * Dim 5 expected GREEN: "Function" def for the function_declaration "hyp_validate"
 *   AND "Class" def for the class_definition "hyp". RED for either label
 ...
 * Dim 7 expected GREEN for calls inside function_declaration "hyp_validate"
...
TEST(repro_grammar_build_puppet) {
    ...
    static const char src[] =
        ...
        "}\n"
        "\n"
        "define hyp::port_config (\n"
        "  Integer $port,\n"
        ") {\n"
        "  file { '/etc/hyp/port.conf':\n"
        "    content => \"port=${port}\\n\",\n"
        "  }\n"
        "}\n"
        "\n"
        "function hyp_validate(String $version) >> Boolean {\n"
        "  $parts = split($version, /\\./ )\n"
        "  length($parts) == 3\n"
        "}\n";
    static const char bad[] = "class hyp (\n  String $version = '0.8.1',\n) {\n  include";
    if (build_callable_battery("Puppet", src, HYP_LANG_PUPPET, "hyp.pp",
                               "Function", "include") != 0)
        return 1;
    if (build_robustness("Puppet", bad, HYP_LANG_PUPPET, "hyp.pp") != 0)
        return 1;
    return build_pipeline_battery("Puppet", "hyp.pp", src);
}
