 ...
 *   is supposed to:
 *     1. Walk the repo filesystem for package.json manifests (hyp_pkgmap_scan_repo).
 *     2. Parse each sibling package.json, mapping its `"name"` field to its
 *        entry-point QN (parse_package_json → pkg_entries_push).
 *     3. On import resolution (hyp_pipeline_resolve_module), perform an exact
 ...
 *   maintainer confirmed: on macOS/Linux hyp_pkgmap_scan_repo may resolve
 *   workspace names at the manifest-parse level (hyp_pkgmap_try_parse), but
 ...
 *   in hyp_pipeline_resolve_module (step 3) silently falls through to
...
TEST(repro_issue408_workspace_crosspkg_import) {
    /*
     * Fixture layout mirrors the reporter's /tmp/hyp-issue408-repro tree
    ...
    RProj lp;
    hyp_store_t *store = rh_index_files(&lp, files, 5);
     ...
     * does not match any graph node, so hyp_pipeline_resolve_import_node
