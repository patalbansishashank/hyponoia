 ...
 * https://github.com/patalbansishashank/hyponoia/issues/403
 *
 ...
 *   hyp_should_skip_dir() (line 339) tests only the BARE directory name
 ...
 *   appears in either list.  Therefore hyp_discover() walks straight into the
 ...
 *     - The hypignore path (opts->ignore_file or .hypignore at root) is
 ...
 *   When hyp_discover() is called on a directory that contains an
 ...
 *   hyp_discover() returns files under Antigravity/ as normal discovered
 ...
 *   hyp_discover() is called on the temp dir.  The loop below asserts that
 ...
 * .hypignore file at the repo root (loaded unconditionally, unlike .gitignore
 ...
 *
 * hyp_discover() is called on <tmpdir> with no opts (NULL) so all default
...
TEST(repro_issue403_install_dir_excluded) {
    ...
    char tmpdir[256];
    snprintf(tmpdir, sizeof(tmpdir), "%s/hyp_repro403_XXXXXX", hyp_tmpdir());
    ...
    /* --- Run discovery (default opts: no .git, no .hypignore, no opts) --- */
     ...
     * hyp_should_skip_dir("Antigravity", ...) returns false and the walk
     ...
     * equivalent install-path exclusion applied) -- hyp_should_skip_dir
