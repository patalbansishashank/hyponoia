...
#
# This tier runs the COMPILED binary on the hyp-bench-validate repos and asserts
...
#   HYP_SCALE_INCLUDE_C=1 bash tests/scale_contract.sh c   # add the slow C tier
#   HYP_BENCH_DIR=/path bash tests/scale_contract.sh       # override repo root
...
BENCH="${HYP_BENCH_DIR:-$HOME/hyp-bench-validate}"
PY="${PYTHON:-python3.9}"
FAILURES=0
RAN=0
...
# project name == hyp_project_name_from_path(): strip leading '/', map every
...
assert_lang() {
    ...
    rm -f "$HOME/.cache/hyponoia/$proj.db"*
...
if [ "${#LANGS[@]}" -eq 0 ]; then
    ...
    [ "${HYP_SCALE_INCLUDE_C:-0}" = "1" ] && LANGS+=(c)
fi
