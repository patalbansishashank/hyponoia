...
static bool diagnostics_capture_paths(diagnostics_paths_t *paths) {
    return hyp_diag_test_copy_paths(paths->directory, sizeof(paths->directory), paths->snapshot,
...
static void diagnostics_cleanup_outputs(const diagnostics_paths_t *paths) {
    ...
    (void)hyp_unlink(paths->snapshot);
    (void)hyp_unlink(paths->trajectory);
    (void)hyp_rmdir(paths->directory);
}
...
TEST(diagnostics_rejects_predictable_tmp_symlinks) {
    ...
    ASSERT_LT(
        snprintf(snapshot_link, sizeof(snapshot_link), "/tmp/hyp-diagnostics-%d.json.tmp", pid),
        (int)sizeof(snapshot_link));
    ASSERT_LT(
        snprintf(trajectory_link, sizeof(trajectory_link), "/tmp/hyp-diagnostics-%d.ndjson", pid),
        (int)sizeof(trajectory_link));
    ASSERT_LT(snprintf(snapshot_target, sizeof(snapshot_target),
                       "/tmp/hyp-diagnostics-snapshot-sentinel-%d", pid),
              (int)sizeof(snapshot_target));
    ASSERT_LT(snprintf(trajectory_target, sizeof(trajectory_target),
                       "/tmp/hyp-diagnostics-trajectory-sentinel-%d", pid),
              (int)sizeof(trajectory_target));

    (void)hyp_unlink(snapshot_link);
    (void)hyp_unlink(trajectory_link);
    (void)hyp_unlink(snapshot_target);
    (void)hyp_unlink(trajectory_target);

    FILE *snapshot = hyp_fopen(snapshot_target, "wb");
    ASSERT_NOT_NULL(snapshot);
    ASSERT_GTE(fputs(snapshot_seed, snapshot), 0);
    ASSERT_EQ(fclose(snapshot), 0);
    FILE *trajectory = hyp_fopen(trajectory_target, "wb");
    ...
    ASSERT_EQ(hyp_setenv("HYP_DIAGNOSTICS", "1", 1), 0);
    ASSERT_TRUE(hyp_diag_start());
    ...
    hyp_diag_stop();
    ASSERT_EQ(hyp_unsetenv("HYP_DIAGNOSTICS"), 0);
...
TEST(diagnostics_stop_interrupts_periodic_wait) {
    ...
    int pid = (int)getpid();
    ASSERT_LT(snprintf(legacy_snapshot, sizeof(legacy_snapshot), "%s/hyp-diagnostics-%d.json",
                       hyp_tmpdir(), pid),
              (int)sizeof(legacy_snapshot));
    ASSERT_LT(snprintf(legacy_trajectory, sizeof(legacy_trajectory), "%s/hyp-diagnostics-%d.ndjson",
                       hyp_tmpdir(), pid),
              (int)sizeof(legacy_trajectory));
    (void)hyp_unlink(legacy_snapshot);
    (void)hyp_unlink(legacy_trajectory);
...
TEST(diagnostics_stalled_writer_cannot_retain_shutdown) {
    hyp_diag_test_hold_writer(true);
    ASSERT_EQ(hyp_setenv("HYP_DIAGNOSTICS", "1", 1), 0);
    if (!hyp_diag_start()) {
        hyp_diag_test_hold_writer(false);
        (void)hyp_unsetenv("HYP_DIAGNOSTICS");
        FAIL("failed to start held diagnostics writer");
    }
    diagnostics_paths_t outputs;
    bool captured = diagnostics_capture_paths(&outputs);
    uint64_t reach_deadline = hyp_now_ms() + 1000;
    while (!hyp_diag_test_writer_reached() && hyp_now_ms() < reach_deadline) {
        diagnostics_wait_ms(10);
    }
    bool reached = hyp_diag_test_writer_reached();
    if (!captured || !reached) {
        hyp_diag_test_hold_writer(false);
        hyp_diag_stop();
        (void)hyp_unsetenv("HYP_DIAGNOSTICS");
    ...
    hyp_diag_stop();
    uint64_t elapsed_ms = hyp_now_ms() - started_ms;
    bool abandoned = hyp_diag_test_abandoned();

    hyp_diag_test_hold_writer(false);
    bool reset = false;
    uint64_t reset_deadline = hyp_now_ms() + 1000;
    while (!reset && hyp_now_ms() < reset_deadline) {
        reset = hyp_diag_test_reset_abandoned();
        if (!reset) {
            diagnostics_wait_ms(10);
        }
    }
...
TEST(diagnostics_soak_discovers_daemon_emitted_paths) {
    ...
    bool isolates_daemon = strstr(script, "HYP_CACHE_DIR=\"$SOAK_CACHE_DIR_VALUE\"") != NULL;
    bool legacy_predictable_path = strstr(script, "/tmp/hyp-diagnostics-${SERVER_PID}") != NULL;
    free(script);
...
TEST(diagnostics_discovery_record_survives_suppressed_log_level) {
    ...
    hyp_log_set_level(HYP_LOG_NONE);
    hyp_log_set_sink_ex(diagnostics_discovery_sink, HYP_LOG_SINK_REPLACE);
    ASSERT_EQ(hyp_setenv("HYP_DIAGNOSTICS", "1", 1), 0);
    bool started = hyp_diag_start();
    diagnostics_paths_t outputs;
    ...
    ASSERT_TRUE(started);
    ASSERT_TRUE(captured);
    /* Exactly one discovery record, delivered despite HYP_LOG_LEVEL=none, and
...
TEST(diagnostics_placement_honors_tmpdir_with_spaces) {
    ...
    char base[256];
    ASSERT_LT(snprintf(base, sizeof(base), "/tmp/hyp diag spaces %d", (int)getpid()),
              (int)sizeof(base));
    (void)hyp_rmdir(base);
    ASSERT_EQ(mkdir(base, 0700), 0);
    ASSERT_EQ(hyp_setenv("TMPDIR", base, 1), 0);
    diagnostics_discovery_records = 0;
    diagnostics_discovery_line[0] = '\0';
    hyp_log_set_sink_ex(diagnostics_discovery_sink, HYP_LOG_SINK_REPLACE);
    ASSERT_EQ(hyp_setenv("HYP_DIAGNOSTICS", "1", 1), 0);
    ...
    hyp_log_set_sink(NULL);
    if (had_tmpdir) {
        (void)hyp_setenv("TMPDIR", saved_tmpdir, 1);
    } else {
        (void)hyp_unsetenv("TMPDIR");
    }
