...
static HYPFileResult *extract_java(const char *source) {
    return hyp_extract_file(source, (int)strlen(source), HYP_LANG_JAVA, "test", "Main.java", 0,
                            NULL, NULL);
}

static HYPFileResult *extract_java_at(const char *source, const char *rel_path) {
    return hyp_extract_file(source, (int)strlen(source), HYP_LANG_JAVA, "test", rel_path, 0, NULL,
                            NULL);
}
...
static int count_resolved(const HYPFileResult *r) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
...
TEST(jlsp_string_length) {
    const char *src =
        ...
        "  }\n"
        "}\n";
    HYPFileResult *r = extract_java(src);
    ASSERT_NOT_NULL(r);
...
TEST(jlsp_user_enum_methods) {
    ...
    for (int i = 0; i < r->defs.count; i++) {
        const HYPDefinition *definition = &r->defs.items[i];
...
TEST(jlsp_file_path) {
    ...
    HYPFileResult *r = extract_java_at(src, "src/main/java/com/example/Main.java");
    ASSERT_NOT_NULL(r);
    ASSERT_GTE(require_resolved(r, "run", "String.length"), 0);
    hyp_free_result(r);
    PASS();
