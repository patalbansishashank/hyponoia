...
FIXTURE="$(mktemp -d "${TMPDIR:-/tmp}/hyp-vendored-integrity.XXXXXX")"
...
mkdir -p "$grammar/internal/hyp/vendored/grammars/example"
printf '%s\n' 'int tree_sitter_example(void) { return 1; }' \
  > "$grammar/internal/hyp/vendored/grammars/example/parser.c"
