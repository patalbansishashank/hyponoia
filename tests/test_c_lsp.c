 ...
 *
 * AUTO-GENERATED from internal/hyp/lsp_c_test.go by scripts/port_clsp_tests.py
...
static int count_resolved(const HYPFileResult *r, const char *callerSub, const char *calleeSub) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const HYPResolvedCall *rc = &r->resolved_calls.items[i];
        if (rc->caller_qn && strstr(rc->caller_qn, callerSub) && rc->callee_qn &&
...
static HYPFileResult *extract_c(const char *src) {
    return hyp_extract_file(src, (int)strlen(src), HYP_LANG_C, "test", "main.c", 0, NULL, NULL);
}

static HYPFileResult *extract_cpp(const char *src) {
    return hyp_extract_file(src, (int)strlen(src), HYP_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
}

static HYPFileResult *extract_c_family(const char *src, HYPLanguage language) {
    const char *path = language == HYP_LANG_C      ? "main.c"
                       : language == HYP_LANG_CUDA ? "main.cu"
                                                   : "main.cpp";
    return hyp_extract_file(src, (int)strlen(src), language, "test", path, 0, NULL, NULL);
}

TEST(clsp_simple_var_decl) {
    HYPFileResult *r = extract_c("\n"
                                 "struct Foo {\n"
                                 "    int value;\n"
                                 "};\n"
                                 "\n"
...
TEST(clsp_calls_attributed_to_function_issue220) {
    HYPFileResult *r = extract_cpp("void helper() {}\n"
                                   "void doWork() {\n"
                                   "    helper();\n"
                                   "}\n");
    ASSERT_NOT_NULL(r);
...
TEST(clsp_nocrash_issue355_xxhash_header) {
    ...
    buf[rd] = '\0';
    HYPFileResult *r =
        hyp_extract_file(buf, (int)rd, HYP_LANG_C, "test", "xxhash.h", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    hyp_free_result(r);
    free(buf);
    PASS();
...
TEST(clsp_nocrash_issue312_default_template_auto_param) {
    HYPFileResult *r = extract_cpp("template<typename T, typename U = int>\n"
                                   "U f(auto, T t) {\n"
                                   "    return t;\n"
                                   "}\n"
                                   "\n"
...
TEST(clsp_nocrash_empty_source) {
    HYPFileResult *r = hyp_extract_file("", 0, HYP_LANG_CPP, "test", "main.cpp", 0, NULL, NULL);
    ASSERT_NOT_NULL(r);
    hyp_free_result(r);
    PASS();
}
...
TEST(clsp_nocrash_extremely_large_function) {
    ...
    HYPFileResult *r = extract_cpp(src);
    ASSERT_NOT_NULL(r);
 ...
 * (hyp_c_build_cross_registry), FINALIZES it (O(1) hash lookups), then shares it
 * READ-ONLY across the parallel resolve workers (ctx.registry_shared = true).
 * hyp_run_c_lsp_cross_with_registry must therefore NOT mutate it.
 *
...
TEST(clsp_tier2_shared_registry_readonly_c) {
    ...
    HYPTypeRegistry *reg = hyp_c_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    ...
    hyp_run_c_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod",
...
TEST(registry_short_name_indexes) {
    ...
    HYPTypeEmbedIter it;
    hyp_registry_types_by_embedded_bare(&reg, "Trait", &it);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), 1);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), 2);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), -1);

    /* Bare "Other": only type 3 (C). */
    hyp_registry_types_by_embedded_bare(&reg, "Other", &it);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), 3);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), -1);

    /* Bare with no implementers: empty. */
    hyp_registry_types_by_embedded_bare(&reg, "Nope", &it);
    ASSERT_EQ(hyp_type_embed_iter_next(&it), -1);

    /* Free-func index for "helper": func 0 then 1 (ascending), NOT the method (2). */
    HYPFreeFuncIter fit;
    hyp_registry_free_funcs_by_short_name(&reg, "helper", &fit);
    ASSERT_EQ(hyp_free_func_iter_next(&fit), 0);
    ASSERT_EQ(hyp_free_func_iter_next(&fit), 1);
    ASSERT_EQ(hyp_free_func_iter_next(&fit), -1);

    /* Free-func "other": only func 3. */
    hyp_registry_free_funcs_by_short_name(&reg, "other", &fit);
    ASSERT_EQ(hyp_free_func_iter_next(&fit), 3);
    ASSERT_EQ(hyp_free_func_iter_next(&fit), -1);
...
TEST(seal_py_shared_registry_readonly) {
    HYPArena arena;
    hyp_arena_init(&arena);
    HYPTypeRegistry *reg = hyp_py_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "def helper(x):\n    return x + 1\n\ndef caller():\n    return helper(1)\n";
    HYPResolvedCallArray out = {0};
    hyp_run_py_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out, NULL);
 ...
 * hyp_registry_add_func/_type (which the read_only seal no-ops). But
 ...
 * read_only registry (pipeline.c:901 → hyp_py_build_cross_registry), so this is a
...
TEST(seal_py_shared_registry_readonly_fields) {
    ...
    defs[0].lang = HYP_LANG_PYTHON;
    defs[1].qualified_name = "test.mod.Foo.m";
    defs[1].short_name = "m";
    defs[1].label = "Method";
    defs[1].receiver_type = "test.mod.Foo";
    defs[1].def_module_qn = "test.mod";
    defs[1].lang = HYP_LANG_PYTHON;

    HYPTypeRegistry *reg = hyp_py_build_cross_registry(&arena, defs, 2);
    ASSERT_NOT_NULL(reg);
    ...
    const HYPRegisteredType *rt = hyp_registry_lookup_type(reg, "test.mod.Foo");
    ASSERT_NOT_NULL(rt);
...
TEST(seal_cs_shared_registry_readonly) {
    HYPArena arena;
    hyp_arena_init(&arena);
    HYPTypeRegistry *reg = hyp_cs_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "namespace N { class Box { int Unwrap() { return 0; }\n"
                      "  int Caller() { return Unwrap(); } } }\n";
    HYPResolvedCallArray out = {0};
    hyp_run_cs_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, 0,
                                       NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    hyp_arena_destroy(&arena);
...
TEST(seal_ts_shared_registry_readonly) {
    HYPArena arena;
    hyp_arena_init(&arena);
    HYPTypeRegistry *reg = hyp_ts_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "class Box { unwrap(): number { return 0; } }\n"
                      "function caller(b: Box): number { return b.unwrap(); }\n";
    HYPResolvedCallArray out = {0};
    hyp_run_ts_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", false, false,
...
TEST(seal_go_shared_registry_readonly) {
    HYPArena arena;
    hyp_arena_init(&arena);
    HYPTypeRegistry *reg = hyp_go_build_cross_registry(&arena, NULL, 0);
    ASSERT_NOT_NULL(reg);
    int fb = reg->func_count, tb = reg->type_count;
    const char *src = "package main\nfunc helper(x int) int { return x + 1 }\n"
                      "func caller() int { return helper(1) }\n";
    HYPResolvedCallArray out = {0};
    hyp_run_go_lsp_cross_with_registry(&arena, src, (int)strlen(src), "test.mod", reg, NULL, NULL,
                                       0, NULL, &out);
    ASSERT_EQ(reg->func_count, fb);
    ASSERT_EQ(reg->type_count, tb);
    hyp_arena_destroy(&arena);
...
static int assert_cpp_family_repeated_method_occurrences(HYPLanguage language) {
    ...
    HYPFileResult *result = extract_c_family(source, language);
    ASSERT_NOT_NULL(result);
    const HYPCall *calls[2] = {NULL, NULL};
    const HYPResolvedCall *semantics[2] = {NULL, NULL};
    int carrier_count = 0;
    ...
    for (int i = 0; i < result->calls.count; i++) {
        ...
        if (!call->enclosing_func_qn || !strstr(call->enclosing_func_qn, "occurrence_probe") ||
            !call->callee_name || strcmp(hyp_lsp_bare_segment(call->callee_name), "render") != 0) {
            continue;
        }
        carrier_count++;
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        ...
        if (resolved->kind != HYP_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
            !resolved->caller_qn || !strstr(resolved->caller_qn, "occurrence_probe") ||
            !resolved->callee_qn ||
            strcmp(hyp_lsp_bare_segment(resolved->callee_qn), "render") != 0) {
            continue;
        }
        semantic_count++;
        for (int occurrence = 0; occurrence < 2; occurrence++) {
    ...
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&result->resolved_calls, calls[0], false) ==
                semantics[0]);
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&result->resolved_calls, calls[1], false) ==
                semantics[1]);
...
TEST(clsp_cpp_repeated_same_leaf_calls_join_exact_occurrences) {
    return assert_cpp_family_repeated_method_occurrences(HYP_LANG_CPP);
}

TEST(clsp_cuda_repeated_same_leaf_calls_join_exact_occurrences) {
    return assert_cpp_family_repeated_method_occurrences(HYP_LANG_CUDA);
}
...
TEST(clsp_c_reassigned_function_pointer_calls_join_exact_occurrences) {
    ...
    HYPFileResult *result = extract_c_family(source, HYP_LANG_C);
    ASSERT_NOT_NULL(result);
    const HYPCall *calls[2] = {NULL, NULL};
    const HYPResolvedCall *semantics[2] = {NULL, NULL};
    int carrier_count = 0;
...
TEST(clsp_c_nested_function_pointer_shadow_restores_outer_target) {
    ...
    HYPFileResult *result = extract_c_family(source, HYP_LANG_C);
    ASSERT_NOT_NULL(result);
    const HYPResolvedCall *inner_semantic = NULL;
    const HYPResolvedCall *outer_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != HYP_RESOLVED_INVOCATION || !resolved->reason ||
...
TEST(clsp_cpp_new_expression_joins_exact_occurrence) {
    ...
    HYPFileResult *result = extract_c_family(source, HYP_LANG_CPP);
    ASSERT_NOT_NULL(result);
    const HYPCall *carrier = NULL;
    const HYPResolvedCall *semantic = NULL;
    for (int i = 0; i < result->calls.count; i++) {
        const HYPCall *call = &result->calls.items[i];
    ...
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind == HYP_RESOLVED_INVOCATION && resolved->callee_qn &&
    ...
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&result->resolved_calls, carrier, false) ==
                semantic);
    hyp_free_result(result);
    PASS();
}
...
static int assert_cpp_family_preprocessed_collision_isolated(HYPLanguage language) {
    ...
    HYPFileResult *result = extract_c_family(source, language);
    ASSERT_NOT_NULL(result);
    const HYPCall *raw_alpha = NULL;
    const HYPCall *expanded_bravo = NULL;
    ...
    const HYPResolvedCall *raw_alpha_semantic = NULL;
    const HYPResolvedCall *expanded_bravo_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != HYP_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
    ...
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&result->resolved_calls, raw_alpha, false) ==
                raw_alpha_semantic);
    ASSERT_TRUE(hyp_pipeline_find_lsp_resolution(&result->resolved_calls, expanded_bravo, false) ==
...
TEST(clsp_cpp_preprocessed_coordinate_collision_isolated) {
    return assert_cpp_family_preprocessed_collision_isolated(HYP_LANG_CPP);
}

TEST(clsp_cuda_preprocessed_coordinate_collision_isolated) {
    return assert_cpp_family_preprocessed_collision_isolated(HYP_LANG_CUDA);
}
...
TEST(clsp_c_preprocessed_coordinate_collision_isolated) {
    ...
    HYPFileResult *result = extract_c_family(source, HYP_LANG_C);
    ASSERT_NOT_NULL(result);
    const HYPCall *colliding_calls[2] = {NULL, NULL};
    int colliding_carriers = 0;
    for (int i = 0; i < result->calls.count; i++) {
        const HYPCall *call = &result->calls.items[i];
        if (call->enclosing_func_qn && strstr(call->enclosing_func_qn, "occurrence_probe") &&
    ...
    const HYPResolvedCall *bravo_semantic = NULL;
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != HYP_RESOLVED_INVOCATION || resolved->confidence <= 0.0f ||
            resolved->site_start_byte != collision_start ||
    ...
    const HYPResolvedCall *first_join =
        hyp_pipeline_find_lsp_resolution(&result->resolved_calls, colliding_calls[0], false);
    const HYPResolvedCall *second_join =
        hyp_pipeline_find_lsp_resolution(&result->resolved_calls, colliding_calls[1], false);
...
TEST(clsp_preprocessed_destructor_rewrite_respects_origin_during_rewrite) {
    ...
    char *expanded = hyp_preprocess(source, written, "main.cpp", NULL, NULL, true);
    ...
    expanded = hyp_preprocess(source, written, "main.cpp", NULL, NULL, true);
    ASSERT_NOT_NULL(expanded);
    ...
    HYPFileResult *result = extract_cpp(source);
    ASSERT_NOT_NULL(result);
    const HYPCall *raw_carrier = NULL;
    const HYPCall *preprocessed_carrier = NULL;
    const HYPResolvedCall *raw_semantic = NULL;
    const HYPResolvedCall *preprocessed_semantic = NULL;
    for (int i = 0; i < result->calls.count; i++) {
        const HYPCall *call = &result->calls.items[i];
        ...
        if (call->source_origin == HYP_SOURCE_ORIGIN_RAW) {
            raw_carrier = call;
        } else if (call->source_origin == HYP_SOURCE_ORIGIN_PREPROCESSED) {
            preprocessed_carrier = call;
        }
    }
    for (int i = 0; i < result->resolved_calls.count; i++) {
        const HYPResolvedCall *resolved = &result->resolved_calls.items[i];
        if (resolved->kind != HYP_RESOLVED_INVOCATION || !resolved->strategy ||
            strcmp(resolved->strategy, "lsp_destructor") != 0 || !resolved->callee_qn ||
            resolved->site_start_byte != collision_start ||
            resolved->site_end_byte != collision_end) {
            continue;
        }
        if (resolved->source_origin == HYP_SOURCE_ORIGIN_RAW &&
            strstr(resolved->callee_qn, "~Alpha")) {
            raw_semantic = resolved;
        } else if (resolved->source_origin == HYP_SOURCE_ORIGIN_PREPROCESSED &&
    ...
    bool raw_name_ok = raw_carrier->callee_name &&
                       strcmp(hyp_lsp_bare_segment(raw_carrier->callee_name), "~Alpha") == 0;
    bool preprocessed_name_ok =
        preprocessed_carrier->callee_name &&
        strcmp(hyp_lsp_bare_segment(preprocessed_carrier->callee_name), "~Bravo") == 0;
    bool raw_join_ok = hyp_pipeline_find_lsp_resolution(&result->resolved_calls, raw_carrier,
                                                        false) == raw_semantic;
    bool preprocessed_join_ok =
        hyp_pipeline_find_lsp_resolution(&result->resolved_calls, preprocessed_carrier, false) ==
