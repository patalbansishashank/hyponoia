 ...
 * hyp_extract_file() populates the right fields.  Here we run the FULL
 * production pipeline (index_repository via MCP) and assert the resulting
 * graph DB contains the expected edges via hyp_store_count_edges_by_type.
 *
 ...
 *   Go implicit interface satisfaction: pass_semantic.c hyp_pipeline_implements_go()
...
static hyp_store_t *es_lang_open_indexed(ES_LangProj *lp) {
...
static hyp_store_t *es_lang_index_files(ES_LangProj *lp, const ES_LangFile *files, int nfiles) {
    memset(lp, 0, sizeof(*lp));
    snprintf(lp->tmpdir, sizeof(lp->tmpdir), "/tmp/hyp_es_XXXXXX");
...
static void es_lang_cleanup(ES_LangProj *lp, hyp_store_t *store) {
    if (store) {
...
static void es_dump_edge_histogram(hyp_store_t *store, const char *project) {
    if (!store) {
    ...
    for (int i = 0; ES_ALL_EDGE_TYPES[i]; i++) {
        int c = hyp_store_count_edges_by_type(store, project, ES_ALL_EDGE_TYPES[i]);
...
static int es_edge_present(const ES_LangFile *files, int nfiles, const char *edge, int floor) {
    ES_LangProj lp;
    hyp_store_t *store = es_lang_index_files(&lp, files, nfiles);
...
static int es_exact_edge_by_name(const ES_LangFile *files, int nfiles, const char *edge_type,
    ...
    int matches = -1;
    if (store &&
        hyp_store_find_nodes_by_name(store, lp.project, source_name, &sources, &source_count) ==
            HYP_STORE_OK &&
        hyp_store_find_nodes_by_name(store, lp.project, target_name, &targets, &target_count) ==
            HYP_STORE_OK &&
        source_count == 1 && target_count == 1) {
        hyp_edge_t *edges = NULL;
        int edge_count = 0;
        if (hyp_store_find_edges_by_source_type(store, sources[0].id, edge_type, &edges,
                                                &edge_count) == HYP_STORE_OK) {
            matches = 0;
...
TEST(es_inherits_crossfile_python_red) {
    ...
    ES_LangProj lp;
    hyp_store_t *store = es_lang_index_files(&lp, f, 2);
    int got = store ? hyp_store_count_edges_by_type(store, lp.project, "INHERITS") : -1;
    if (got >= 1) {
 ...
 * hyp_pipeline_implements_go() checks method-set coverage. */
TEST(es_implements_go_implicit) {
    ...
    /* GREEN: hyp_pipeline_implements_go() finds MyType satisfies Stringer
...
TEST(es_override_go_implicit) {
    ...
    /* GREEN: hyp_pipeline_implements_go() emits OVERRIDE for Entity.Name -> Namer.Name. */
...
TEST(es_decorates_annotation_java) {
    ...
    ES_LangProj lp;
    hyp_store_t *store = es_lang_index_files(&lp, f, 1);
    int got = store ? hyp_store_count_edges_by_type(store, lp.project, "DECORATES") : -1;
...
SUITE(edge_structural) {
    ...
    /* Expected GREEN: hyp_pipeline_implements_go() emits OVERRIDE. */
