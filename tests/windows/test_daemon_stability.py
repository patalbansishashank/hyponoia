...
each other and from any interactive HYP use on the host.

Exit code: 0 == all sections green, 1 == regression, 2 == setup error.

Usage:
    python test_daemon_stability.py <path-to-hyponoia[.exe]>
"""
import json
import os
import re
...
def section_hook_fail_open(binary, work):
    ...
    if first.returncode != 0:
        ...
        return False
    if "systemMessage" not in first_out or "no HYP daemon" not in first_err:
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="hyp_daemon_stab_")
    sections = [
        section_params,
