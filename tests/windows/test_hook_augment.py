...
`hyponoia hook-augment` is the non-blocking Claude Code PreToolUse
...
#619 added `hyp_is_walkable_abs_path` (accepts `X:/` drive-letter roots), so the
...
Usage:
    python test_hook_augment.py <path-to-hyponoia[.exe]>
"""
import json
import os
import re
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="hyp_win_hook_")
    try:
        ...
        print("\nREGRESSION (red): hook-augment produced no hookSpecificOutput on "
              ...
              "hyp_is_walkable_abs_path handling in hook_augment.c regressed?).")
        return 1
    finally:
