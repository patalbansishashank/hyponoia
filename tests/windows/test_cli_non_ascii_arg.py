...
The documented entrypoint `hyponoia cli index_repository '<json>'`
...
Usage:
    python test_cli_non_ascii_arg.py <path-to-hyponoia[.exe]>
"""
import json
import os
import shutil
...
def main():
    ...
    work = tempfile.mkdtemp(prefix="hyp_win_cliarg_")
    try:
        ...
        env = dict(os.environ)
        env["HYP_CACHE_DIR"] = os.path.join(work, "cache_ascii")
        ctrl = subprocess.run(
        ...
        # HYP_INDEX_SUPERVISOR=0 for determinism across the other guards; forcing it OFF
        ...
        env2.pop("HYP_INDEX_SUPERVISOR", None)
        ...
        # paths on CJK-codepage systems) before the hyp_canonical_path fix.
        cjk_repo = os.path.join(work, "\u96f7\u9054\u6e2c\u8a66")
        make_fixture(cjk_repo)
        env3 = dict(os.environ)
        env3["HYP_CACHE_DIR"] = os.path.join(work, "cache_cjk")
        env3.pop("HYP_INDEX_SUPERVISOR", None)
