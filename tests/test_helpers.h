 ...
 * leaf made by hyp_mkdtemp() has a private DACL. Keep these security-sensitive
...
#ifdef _WIN32
    const char *base = hyp_app_local_dir();
#else
    const char *base = hyp_tmpdir();
#endif
    if (!base || !base[0]) {
        out[0] = '\0';
        return false;
    }
    int written = snprintf(out, out_cap, "%s/hyp-runtime-%s-XXXXXX", base, tag);
    if (written <= 0 || (size_t)written >= out_cap) {
        out[0] = '\0';
        return false;
    }
    return hyp_mkdtemp(out) != NULL;
}
