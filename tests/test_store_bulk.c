 ...
 *
 * Verifies that hyp_store_begin_bulk / hyp_store_end_bulk never switch away
 ...
 * independent of any state held inside the hyp_store_t under test. */
...
static void make_temp_path(char *buf, size_t n) {
    snprintf(buf, n, "%s/cmm_bulk_test_%d.db", hyp_tmpdir(), (int)getpid());
}
...
TEST(bulk_crash_recovery) {
    ...
    hyp_store_t *s = hyp_store_open_path(db_path);
    ASSERT_NOT_NULL(s);
    int rc = hyp_store_upsert_project(s, "baseline", "/tmp/baseline");
    ASSERT_EQ(rc, HYP_STORE_OK);
    hyp_store_close(s);
    ...
    pid_t pid = fork();
    if (pid == 0) {
        hyp_store_t *cs = hyp_store_open_path(db_path);
        if (!cs)
            _exit(1);
        hyp_store_begin_bulk(cs);
        hyp_store_begin(cs); /* explicit open transaction */
        hyp_store_upsert_project(cs, "crashed", "/tmp/crashed");
    ...
    hyp_store_t *recovered = hyp_store_open_path(db_path);
    ASSERT_NOT_NULL(recovered); /* NULL would indicate corruption */

    /* Baseline commit must survive. */
    hyp_project_t p = {0};
    rc = hyp_store_get_project(recovered, "baseline", &p);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ...
    hyp_project_t p2 = {0};
    int rc2 = hyp_store_get_project(recovered, "crashed", &p2);
    ASSERT_NEQ(rc2, HYP_STORE_OK); /* row must be absent */

    hyp_store_close(recovered);
    cleanup_db(db_path);
    PASS();
}
#endif /* _WIN32 */
