#!/usr/bin/env python3
"""Real-binary smoke test for the mandatory per-account HYP daemon.
...
def probe_future_generation_rendezvous(
    ...
    header = struct.pack(
        ">4sBBHI", b"HYPD", 1, 1, 1, RENDEZVOUS_REQUEST_SIZE
    )
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as stream:
        ...
        check(magic == b"HYPD", "future probe received wrong frame magic")
...
def run_successful_activation(
    ...
    check(
        "Stopping active HYP sessions and operations for {}...".format(action)
        in stdout,
    ...
    records = [
        ...
        if record.get("event") == "hyp.activation"
...
def create_local_update_release(release_dir, candidate):
    ...
    with tarfile.open(archive, "w:gz") as stream:
        stream.add(str(candidate), arcname="hyponoia", recursive=False)
...
def assert_no_activation_artifacts(directory, label):
    artifacts = (
        sorted(path.name for path in directory.iterdir() if path.name.startswith(".hyp-"))
        if directory.exists()
        else []
    )
...
def main():
    if os.name == "nt" or sys.platform.startswith(("cygwin", "msys")):
        if os.environ.get("HYP_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
    ...
    binary = Path(sys.argv[1] if len(sys.argv) > 1 else root / "build/c/hyponoia")
    ...
    runtime_dir = runtime_parent / ("hyp-daemon-" + str(os.geteuid()))
    socket_path = runtime_dir / ("hyp-" + RENDEZVOUS_KEY + ".sock")
    startup_lock = runtime_dir / ("hyp-" + RENDEZVOUS_KEY + ".lock")
    lifetime_lock = runtime_dir / ("hyp-" + RENDEZVOUS_KEY + ".lifetime.lock")
    cohort_admission_lock = runtime_dir / "hyp-version-cohort-admission-v1.lock"
    cohort_lifetime_lock = runtime_dir / "hyp-version-cohort-lifetime-v1.lock"
    cohort_maintenance_lock = runtime_dir / "hyp-version-cohort-maintenance-v1.lock"
    cohort_daemon_lock = runtime_dir / "hyp-version-cohort-daemon-v1.lock"
    coordination_locks = (
        (startup_lock, False),
        (lifetime_lock, True),
    ...
    if any(status == "held" for _, status in coordination_before):
        if os.environ.get("HYP_DAEMON_SMOKE_REQUIRE_RUN") == "1":
            raise SmokeFailure(
                "another HYP process or activation is active or starting; "
                "required smoke needs a clean account-wide rendezvous"
            )
        print("SKIP: another HYP process or activation is active or starting")
        return 0
    for path, status in coordination_before:
        check(status == "free", "unsafe coordination lock {}: {}".format(path, status))

    clients = []
    with tempfile.TemporaryDirectory(prefix="hyp-daemon-smoke-") as raw_tmpdir:
        ...
        target_binary = target_dir / "hyponoia"
        ...
        daemon_log = cache / "logs/hyp-daemon.log"
        ...
        env.update(
            {
                "HOME": str(home),
                "HYP_CACHE_DIR": str(cache_alias),
                "HYP_LOG_LEVEL": "info",
                "HYP_LOG_FORMAT": "json",
                "HYP_TEST_HANG_ON": "hang_me",
                "HYP_TEST_WORKER_DESCENDANT_PID_FILE": str(descendant_pid_path),
            }
        )
        ...
        try:
            ...
            check(
                "Preparing one-shot local HYP command" in local_cli.stderr,
            ...
            initialize_params = {
                ...
                "capabilities": {},
                "clientInfo": {"name": "hyp-daemon-smoke", "version": "1"},
            }
            c1.send(
            ...
            mismatched_env["HYP_CACHE_DIR"] = str(mismatched_cache)
            ...
            check(
                ...
                "mismatched HYP_CACHE_DIR unexpectedly joined the account daemon",
            )
            check(
                "active account daemon uses a different cache directory"
                in mismatched_result.stderr
                and "HYP_CACHE_DIR" in mismatched_result.stderr,
            ...
            # even if the original HYP_CACHE_DIR symlink is retargeted later.
            ...
            conflict_binary = tmpdir / "hyponoia-conflict"
            ...
            if sys.platform == "darwin":
                ...
                with conflict_binary.open("ab") as stream:
                    stream.write(b"\x00hyp-daemon-smoke-conflicting-build")
            ...
            cli_first_env["HYP_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                cli_first_pid_path
            )
            cli_first_env["HYP_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
                cli_first_lock_owner_path
            )
            ...
            def competitor_waiting_or_exited():
                try:
                    ...
                    text = ""
                return (
                    "Waiting for another HYP mutation of smoke-cli-first" in text
            ...
            check(
                ...
                and "Waiting for another HYP mutation of smoke-cli-first"
            ...
            install_target = install_dir / "hyponoia"
            ...
            activation_local_env["HYP_TEST_WORKER_DESCENDANT_PID_FILE"] = str(
                install_pid_path
            )
            activation_local_env["HYP_TEST_WORKER_PROJECT_LOCK_PID_FILE"] = str(
            ...
            # the next daemon. HYP_DOWNLOAD_URL points at this file:// fixture,
            ...
            update_env = env.copy()
            update_env["HYP_DOWNLOAD_URL"] = create_local_update_release(
