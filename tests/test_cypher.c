...
TEST(cypher_lex_simple_match) {
    hyp_lex_result_t r = {0};
    int rc = hyp_lex("MATCH (n:Function)", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
...
TEST(cypher_lex_string_overflow) {
    ...
    hyp_lex_result_t r = {0};
    int rc = hyp_lex(query, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_GTE(r.count, 1);
    ASSERT_EQ(r.tokens[0].type, TOK_STRING);
    /* The string should be truncated to HYP_SZ_4K - 1 (4095) characters. */
...
TEST(cypher_lex_keywords_case_insensitive) {
    hyp_lex_result_t r = {0};
    int rc = hyp_lex("match WHERE Return limit", &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.tokens[0].type, TOK_MATCH);
...
TEST(cypher_lex_pipe_and_star) {
    hyp_lex_result_t r = {0};
    int rc = hyp_lex("[:TYPE1|TYPE2*1..3]", &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_simple_node) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(err);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(hyp_query_pattern(q).node_count, 1);
    ASSERT_EQ(hyp_query_pattern(q).rel_count, 0);
    ASSERT_STR_EQ(hyp_query_pattern(q).nodes[0].variable, "f");
    ASSERT_STR_EQ(hyp_query_pattern(q).nodes[0].label, "Function");
...
TEST(cypher_parse_relationship_outbound) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function)-[:CALLS]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(hyp_query_pattern(q).node_count, 2);
    ASSERT_EQ(hyp_query_pattern(q).rel_count, 1);
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].direction, "outbound");
    ASSERT_EQ(hyp_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(hyp_query_pattern(q).rels[0].max_hops, 1);
...
TEST(cypher_parse_relationship_inbound) {
    ...
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].direction, "inbound");
...
TEST(cypher_parse_relationship_any) {
    ...
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].direction, "any");
...
TEST(cypher_parse_variable_length) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function)-[:CALLS*1..3]->(g:Function)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(hyp_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(hyp_query_pattern(q).rels[0].max_hops, 3);
...
TEST(cypher_parse_variable_length_unbounded) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f)-[:CALLS*]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(hyp_query_pattern(q).rels[0].min_hops, 1);
    ASSERT_EQ(hyp_query_pattern(q).rels[0].max_hops, 0); /* 0 = unbounded */
...
TEST(cypher_parse_multiple_edge_types) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f)-[:CALLS|HTTP_CALLS]->(g)", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);

    ASSERT_EQ(hyp_query_pattern(q).rels[0].type_count, 2);
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].types[0], "CALLS");
    ASSERT_STR_EQ(hyp_query_pattern(q).rels[0].types[1], "HTTP_CALLS");
...
TEST(cypher_parse_where_clause) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) WHERE f.name = \"Foo\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
...
TEST(cypher_parse_where_regex) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
    ASSERT_STR_EQ(q->where->root->cond.op, "=~");
...
TEST(cypher_parse_where_and) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) WHERE f.name = \"A\" AND f.label = \"Function\"",
                              &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_return_simple) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) RETURN f.name, f.qualified_name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_return_count) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COUNT(g) AS cnt", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_return_order_limit) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f:Function) RETURN f.name ORDER BY f.name DESC LIMIT 5", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_order_by_over_cap_rejected_issue1334) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) RETURN f.name ORDER BY "
...
TEST(cypher_parse_return_distinct) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) RETURN DISTINCT f.label", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_inline_props) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function {name: \"Foo\"})", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(hyp_query_pattern(q).nodes[0].prop_count, 1);
    ASSERT_STR_EQ(hyp_query_pattern(q).nodes[0].props[0].key, "name");
    ASSERT_STR_EQ(hyp_query_pattern(q).nodes[0].props[0].value, "Foo");
...
TEST(cypher_parse_error) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("INVALID QUERY", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
    free(err);
    PASS();
 ...
 */
static hyp_store_t *setup_cypher_store(void) {
    ...
    int64_t id3 = hyp_store_upsert_node(s, &n3);
    int64_t id4 = hyp_store_upsert_node(s, &n4);
    int64_t id5 = hyp_store_upsert_node(s, &n5);

    hyp_edge_t e1 = {.project = "test", .source_id = id1, .target_id = id2, .type = "CALLS"};
    hyp_edge_t e2 = {.project = "test", .source_id = id2, .target_id = id3, .type = "CALLS"};
    hyp_edge_t e3 = {.project = "test", .source_id = id1, .target_id = id5, .type = "CALLS"};
    hyp_edge_t e4 = {.project = "test", .source_id = id4, .target_id = id1, .type = "DEFINES"};
...
TEST(cypher_deep_nesting_rejected_not_crash) {
    ...
    if (pid == 0) {
        ...
        q[n] = '\0';
        hyp_store_t *s = setup_cypher_store();
        hyp_cypher_result_t r = {0};
        ...
        (void)hyp_cypher_execute(s, q, "test", 0, &r);
        hyp_cypher_result_free(&r);
        hyp_store_close(s);
        free(q);
        _exit(0);
    }
...
TEST(cypher_exec_match_all_functions) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
 ...
 * bounds-checked in size_t by hyp_cypher_cross_join_alloc — exercised at its
...
TEST(cypher_exec_optional_empty_label_no_overflow) {
    hyp_store_t *s = setup_cypher_store(); /* 4 Function nodes */
    hyp_cypher_result_t r = {0};

    int rc = hyp_cypher_execute(
...
TEST(cypher_exec_optional_rel_saturated_no_overflow) {
    ...
    int64_t hub_id = hyp_store_upsert_node(s, &hub);
    for (int i = 0; i < 20; i++) {
        char nm[32];
        char qn[48];
    ...
    for (int i = 0; i < 300; i++) {
        ...
        hyp_node_t callee = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t cid = hyp_store_upsert_node(s, &callee);
        hyp_edge_t e = {.project = "test", .source_id = hub_id, .target_id = cid, .type = "CALLS"};
...
TEST(cypher_exec_optional_saturated_does_not_fabricate_no_match) {
    ...
    hyp_node_t a = {.project = "test", .label = "Function", .name = "A", .qualified_name = "test.A"};
    hyp_node_t b = {.project = "test", .label = "Function", .name = "B", .qualified_name = "test.B"};
    hyp_node_t c = {.project = "test", .label = "Function", .name = "C", .qualified_name = "test.C"};
    int64_t a_id = hyp_store_upsert_node(s, &a);
    (void)hyp_store_upsert_node(s, &b); /* B: no outgoing CALLS at all */
    int64_t c_id = hyp_store_upsert_node(s, &c);
    ASSERT_GT(a_id, 0);
    ASSERT_GT(c_id, 0);
    ...
    for (int i = 0; i < 40; i++) {
        ...
        hyp_node_t t = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t tid = hyp_store_upsert_node(s, &t);
        hyp_edge_t e = {.project = "test", .source_id = a_id, .target_id = tid, .type = "CALLS"};
        hyp_store_insert_edge(s, &e);
    }
    for (int i = 0; i < 5; i++) {
        char nm[32];
        ...
        hyp_node_t t = {.project = "test", .label = "Var", .name = nm, .qualified_name = qn};
        int64_t tid = hyp_store_upsert_node(s, &t);
        hyp_edge_t e = {.project = "test", .source_id = c_id, .target_id = tid, .type = "CALLS"};
        hyp_store_insert_edge(s, &e);
    }
 ...
 * #627 failure mode). hyp_cypher_cross_join_alloc now computes it in size_t and
...
TEST(cypher_exec_optional_rel_leaf_fallback_survives) {
     ...
     * CYPHER_RESULT_CEILING (100000) in hyp_cypher_execute before bind_cap is
...
TEST(cypher_exec_where_eq) {
    ...
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_varlength_path_semantics_issue797) {
    ...
    hyp_node_t leaf = {.project = "test",
                       ...
                       .start_line = 5,
                       .end_line = 6};
    int64_t id_loopy = hyp_store_upsert_node(s, &loopy);
    int64_t id_mid = hyp_store_upsert_node(s, &mid);
    int64_t id_leaf = hyp_store_upsert_node(s, &leaf);
    ASSERT_GT(id_loopy, 0);
    hyp_edge_t self_loop = {
        .project = "test", .source_id = id_loopy, .target_id = id_loopy, .type = "CALLS"};
    hyp_edge_t e1 = {
        .project = "test", .source_id = id_loopy, .target_id = id_mid, .type = "CALLS"};
    hyp_edge_t e2 = {.project = "test", .source_id = id_mid, .target_id = id_leaf, .type = "CALLS"};
    hyp_store_insert_edge(s, &self_loop);
    hyp_store_insert_edge(s, &e1);
    hyp_store_insert_edge(s, &e2);

    /* Bug 1: repeated variable must unify — only the self-loop matches. */
    hyp_cypher_result_t r1 = {0};
    ASSERT_EQ(hyp_cypher_execute(s, "MATCH (a)-[:CALLS]->(a) RETURN a.name", "test", 0, &r1), 0);
    ASSERT_EQ(r1.row_count, 1);
    hyp_cypher_result_free(&r1);

    /* Bug 2: *2..2 from loopy — only the REAL 2-chain (leaf); the self-loop
     * must not be reused to pad paths (relationship uniqueness). */
    hyp_cypher_result_t r2 = {0};
    ASSERT_EQ(hyp_cypher_execute(s,
                                 "MATCH (a {name: \"loopy\"})-[:CALLS*2..2]->(b) "
                                 "RETURN DISTINCT b.name",
                                 "test", 0, &r2),
              0);
    ASSERT_EQ(r2.row_count, 1); /* leaf only */
    hyp_cypher_result_free(&r2);

    /* Bug 2 amplifier: no directed path of length 5 exists at all. */
    hyp_cypher_result_t r3 = {0};
    ASSERT_EQ(hyp_cypher_execute(s,
...
TEST(cypher_exec_where_coalesce_issue874) {
    ...
    hyp_node_t c = {.project = "test",
                    ...
                    .start_line = 5,
                    .end_line = 6};
    ASSERT_GT(hyp_store_upsert_node(s, &a), 0);
    ASSERT_GT(hyp_store_upsert_node(s, &b), 0);
    ASSERT_GT(hyp_store_upsert_node(s, &c), 0);

    /* Default FAILS the predicate: only the node with depth 3 matches. */
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s,
...
TEST(cypher_exec_where_regex) {
    ...
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name =~ \".*Order.*\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_contains) {
    ...
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name CONTAINS \"Order\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_starts_with) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name STARTS WITH \"Handle\"", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_func_id) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"HandleOrder\" RETURN id(f)",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_issue874_where_unsupported_func_error) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f:Function) WHERE foo(f.name) = \"x\" RETURN f.name", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_limit) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 2);
...
TEST(cypher_exec_order_by) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
 ...
 * capped at the engine ceiling (hyp_cypher_max_depth(), default 10). On
 ...
 * to hyp_store_bfs — an unbounded traversal (a DoS on cyclic graphs). RED before
...
TEST(cypher_exec_no_results) {
    ...
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.name = \"NonExistent\"", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_distinct) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN DISTINCT f.label", "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_with_distinct_issue238) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) WITH DISTINCT f.label AS lbl RETURN lbl",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    hyp_cypher_result_free(&r);
    ...
    hyp_cypher_result_t r2 = {0};
    rc = hyp_cypher_execute(s, "MATCH (f:Function) WITH f.label AS lbl RETURN lbl", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_label_test_issue241) {
    ...
    hyp_cypher_result_t r = {0};
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE f:Function RETURN f.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 4);
    hyp_cypher_result_free(&r);
    ...
    hyp_cypher_result_t r2 = {0};
    rc = hyp_cypher_execute(s, "MATCH (f:Function) WHERE f:Class RETURN f.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    hyp_cypher_result_t r3 = {0};
    rc =
        hyp_cypher_execute(s, "MATCH (f:Function) WHERE NOT f:Class RETURN f.name", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_count_distinct_issue239) {
    ...
    hyp_cypher_result_t r = {0};
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.label)", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
    ...
    hyp_cypher_result_t r2 = {0};
    rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN count(f.label)", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ...
    hyp_cypher_result_t r3 = {0};
    rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN count(DISTINCT f.name)", "test", 0, &r3);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_unknown_func_return_errors) {
    ...
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN nosuchfunc(f.name)", "test", 0, &r);
    ASSERT_TRUE(rc != 0);
    ASSERT_NOT_NULL(r.error);
...
TEST(cypher_exec_label_alternation_issue242) {
    ...
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (n:Function|Module) RETURN n.name", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ...
    hyp_cypher_result_t r2 = {0};
    rc = hyp_cypher_execute(s, "MATCH (n:Function|Class) RETURN n.name", "test", 0, &r2);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r2.row_count, 4);
    hyp_cypher_result_free(&r2);
...
TEST(cypher_parse_where_starts_with) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f:Function) WHERE f.name STARTS WITH \"Send\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_contains) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f:Function) WHERE f.name CONTAINS \"Handler\" RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_where_numeric) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) WHERE f.start_line > 10 RETURN f", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
...
static hyp_store_t *setup_cypher_http_store(void) {
    hyp_store_t *s = hyp_store_open_memory();
    hyp_store_upsert_project(s, "test", "/tmp/test");
    ...
    hyp_store_insert_edge(s, &http);

    return s;
}

/* Helper: set up store with TWO HTTP_CALLS edges for filtering tests. */
static hyp_store_t *setup_cypher_multi_edge_store(void) {
    hyp_store_t *s = hyp_store_open_memory();
    hyp_store_upsert_project(s, "testproj", "/tmp/test");

    hyp_node_t n1 = {.project = "testproj",
                     .label = "Function",
                     .name = "SendOrder",
                     .qualified_name = "testproj.caller.SendOrder",
                     .file_path = "caller/client.go"};
    hyp_node_t n2 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleOrder",
                     .qualified_name = "testproj.handler.HandleOrder",
                     .file_path = "handler/routes.go"};
    hyp_node_t n3 = {.project = "testproj",
                     .label = "Function",
                     .name = "HandleHealth",
    ...
    hyp_edge_t e1 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id2,
                     .type = "HTTP_CALLS",
                     .properties_json =
                         "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
    hyp_edge_t e2 = {.project = "testproj",
                     .source_id = id1,
                     .target_id = id3,
                     .type = "HTTP_CALLS",
...
static const char *cypher_get_col(const hyp_cypher_result_t *r, int row, const char *col) {
...
static bool cypher_has_row_with(const hyp_cypher_result_t *r, const char *col, const char *val) {
    int ci = -1;
    for (int c = 0; c < r->col_count; c++) {
        if (strcmp(r->columns[c], col) == 0) {
            ci = c;
...
TEST(cypher_edge_prop_access) {
    hyp_store_t *s = setup_cypher_http_store();
    hyp_cypher_result_t r = {0};
...
static void *cypher_edge_props_concurrently(void *opaque) {
    ...
    hyp_store_t *store = setup_cypher_http_store();
    if (!store) {
        return NULL;
    }
    ...
    hyp_node_t source = {.project = "test",
                         .label = "Function",
                         .name = "HandleOrder",
                         .qualified_name = "test.main.HandleOrder",
                         .file_path = "main.go",
                         .start_line = 10,
                         .end_line = 30};
    int64_t source_id = hyp_store_upsert_node(store, &source);
    for (int i = 0; i < 256; i++) {
        char name[64];
        ...
        hyp_node_t target = {.project = "test",
                             .label = "Function",
                             .name = name,
                             .qualified_name = qualified_name,
                             .file_path = "concurrent.go"};
        int64_t target_id = hyp_store_upsert_node(store, &target);
        hyp_edge_t edge = {
            .project = "test",
            .source_id = source_id,
            .target_id = target_id,
            ...
            .properties_json =
                "{\"url_path\":\"/api/orders\",\"confidence\":0.85,\"method\":\"POST\"}"};
        if (source_id < 0 || target_id < 0 || hyp_store_insert_edge(store, &edge) < 0) {
    ...
    for (int i = 0; i < 128; i++) {
        ...
        int rc = hyp_cypher_execute(store,
                                    "MATCH (a:Function)-[r:HTTP_CALLS]->(b:Function) "
...
TEST(cypher_edge_prop_storage_is_per_thread) {
    ...
    bool started0 = hyp_thread_create(&threads[0], 0, cypher_edge_props_concurrently, &ctx[0]) == 0;
    bool started1 = hyp_thread_create(&threads[1], 0, cypher_edge_props_concurrently, &ctx[1]) == 0;
...
TEST(cypher_edge_type_prop) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) RETURN r.type", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_edge_filter_contains) {
    hyp_store_t *s = setup_cypher_multi_edge_store();
    hyp_cypher_result_t r = {0};
...
TEST(cypher_bare_edge_return_exposes_properties_json) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (a)-[r:HTTP_CALLS]->(b) WHERE r.method = 'POST' RETURN r",
                                "testproj", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_apply_limit) {
    ...
    /* LIMIT 5 → 5 rows */
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 5", "lim", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 5);
    hyp_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 10);
    hyp_cypher_result_free(&r);
    ...
    memset(&r, 0, sizeof(r));
    rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name LIMIT 30", "lim", 10, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_not_in) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f) WHERE NOT f.label IN [\"Module\"] RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_null) {
    ...
    hyp_store_upsert_node(s, &n2);
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_where_is_not_null) {
    ...
    hyp_store_upsert_node(s, &n2);
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) WHERE f.file_path IS NOT NULL RETURN f.name",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_return_star) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN * LIMIT 3", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 3);
...
TEST(cypher_parse_neq) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f:Function) WHERE f.name <> \"X\"", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_in) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f) WHERE f.label IN [\"Function\", \"Module\"]", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
    ASSERT_EQ(q->where->root->type, EXPR_CONDITION);
...
TEST(cypher_parse_is_null) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f) WHERE f.file_path IS NULL", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->where->root);
...
TEST(cypher_parse_expr_tree_and_or) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f) WHERE f.a = \"1\" AND f.b = \"2\" OR f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_expr_tree_nested) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f) WHERE (f.a = \"1\" OR f.b = \"2\") AND f.c = \"3\"", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_error_create) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("CREATE (n:Node {name: \"X\"})", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_delete) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("DELETE n", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_set) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("SET n.name = \"X\"", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_merge) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MERGE (n:Node)", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_error_call) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("CALL db.labels()", &q, &err);
    ASSERT_EQ(rc, -1);
    ASSERT_NOT_NULL(err);
...
TEST(cypher_exec_skip) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.name ORDER BY f.name ASC SKIP 2",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_exec_sum) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN SUM(f.start_line) AS total", "test",
                                0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_avg) {
    ...
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN AVG(f.start_line) AS avg_line",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_min) {
    ...
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) RETURN MIN(f.start_line) AS mn", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_max) {
    ...
    hyp_cypher_result_t r = {0};
    int rc =
        hyp_cypher_execute(s, "MATCH (f:Function) RETURN MAX(f.start_line) AS mx", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_count_star) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN COUNT(*) AS n", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_parse_skip) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f) RETURN f.name SKIP 5 LIMIT 10", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_NOT_NULL(q->ret);
...
TEST(cypher_parse_sum_avg) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f) RETURN SUM(f.x) AS s, AVG(f.y) AS a", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_collect) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f)-[:CALLS]->(g) RETURN f.name, COLLECT(g.name) AS names", &q,
                              &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->ret->count, 2);
...
TEST(cypher_parse_tolower) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f) RETURN toLower(f.name) AS n", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_with_where) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("MATCH (f)-[:CALLS]->(g) WITH f.name AS caller, COUNT(g) AS cnt "
...
TEST(cypher_parse_multi_match) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (a:Module) MATCH (b:Function) RETURN a.name, b.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(q->pattern_count, 2);
    ASSERT(!q->pattern_optional[0]);
    ASSERT(!q->pattern_optional[1]);
...
TEST(cypher_parse_union) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("MATCH (f) RETURN f.name UNION ALL MATCH (g) RETURN g.name", &q, &err);
    ASSERT_EQ(rc, 0);
    ASSERT(q->union_all);
...
TEST(cypher_parse_unwind) {
    ...
    char *err = NULL;
    int rc =
        hyp_cypher_parse("UNWIND [\"a\", \"b\", \"c\"] AS x MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_unwind_var) {
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse("UNWIND items AS item MATCH (f) RETURN f.name", &q, &err);
    ASSERT_EQ(rc, 0);
...
TEST(cypher_parse_unwind_oversized_literal_no_overflow) {
    ...
    hyp_query_t *q = NULL;
    char *err = NULL;
    int rc = hyp_cypher_parse(query, &q, &err);
...
TEST(cypher_issue240_labels_function) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (n:Module) RETURN labels(n) AS lbl", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    hyp_cypher_result_free(&r);
...
TEST(cypher_issue873_distinct_limit_dedupes_before_limit) {
    ...
    hyp_cypher_result_t r = {0};
    int rc =
        hyp_cypher_execute(s, "MATCH (n) RETURN DISTINCT n.label AS label LIMIT 2", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 2);
    hyp_cypher_result_free(&r);
...
TEST(cypher_issue252_tointeger) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN toInteger(f.start_line) AS ln",
                                "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    hyp_cypher_result_free(&r);
...
TEST(cypher_issue305_count_star_alias) {
    hyp_store_t *s = setup_cypher_store();
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (n) RETURN count(*) AS total", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_NULL(r.error);
    ASSERT_EQ(r.row_count, 1);
    hyp_cypher_result_free(&r);
 ...
 * (vals[HYP_SZ_32] / func_bufs[HYP_SZ_32][…] in execute_return_simple and its
...
TEST(cypher_exec_prop_array_with_internal_commas) {
    ...
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (m:Method) RETURN m.decorators, m.lines", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_exec_prop_string_with_escaped_quote) {
    ...
    hyp_cypher_result_t r = {0};
    int rc = hyp_cypher_execute(s, "MATCH (f:Function) RETURN f.signature", "test", 0, &r);
    ASSERT_EQ(rc, 0);
    ASSERT_EQ(r.row_count, 1);
...
TEST(cypher_wide_return_projection_bounded) {
    ...
    for (int i = 0; i < 48; i++) { /* 48 > HYP_SZ_32 (32) fixed columns */
    ...
    if (pid == 0) {
        hyp_store_t *s = setup_cypher_store();
        hyp_cypher_result_t r = {0};
        (void)hyp_cypher_execute(s, query, "test", 0, &r);
        hyp_cypher_result_free(&r);
        hyp_store_close(s);
...
TEST(cypher_exec_deadline_aborts_runaway_query_issue601) {
    ...
    hyp_cypher_test_set_deadline_ms(0); /* trip on the first hot-loop check */
    int rc = hyp_cypher_execute(
        s, "MATCH (a) OPTIONAL MATCH (a)-[:CALLS]->(b) RETURN a.qualified_name, count(b)", "test",
        0, &r);
    hyp_cypher_test_set_deadline_ms(-1); /* restore default before asserting (thread-local) */

    ASSERT_TRUE(rc != 0); /* HYP_NOT_FOUND (-1) — query aborted, not success */
    ASSERT_NOT_NULL(r.error);
