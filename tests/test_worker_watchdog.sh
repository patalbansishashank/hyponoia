...
# test-only injector (HYP_TEST_HANG_ON) busy-spins on one file, so the worker
...
# never ship — see src/main.c and TEST_SEAMS in Makefile.hyp). Against a
...
if ! LC_ALL=C grep -a -q -F 'HYP_TEST_WORKER_DESCENDANT_PID_FILE' "${BINARY}"; then
...
HYP_BINARY="${BINARY}" BUILD_FINGERPRINT="${BUILD_FINGERPRINT}" TMPDIR_PATH="${tmpdir}" \
  ARGS_JSON="{\"repo_path\":\"${tmpdir}/repo\"}" \
  HYP_TEST_HANG_ON=hang_me \
  HYP_TEST_WORKER_DESCENDANT_PID_FILE="${tmpdir}/descendant.pid" \
  "${tmpdir}/wrapper.sh" &
wrapper_pid=$!
