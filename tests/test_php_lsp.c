...
static HYPFileResult *extract_php(const char *source) {
    return hyp_extract_file(source, (int)strlen(source), HYP_LANG_PHP, "test", "main.php", 0, NULL,
                            NULL);
}
...
TEST(phplsp_local_method_via_new_assignment) {
    const char *src =
        ...
        "    }\n"
        "}\n";
    HYPFileResult *r = extract_php(src);
    ASSERT(r);
...
TEST(phplsp_unindexed_receiver_emits_block) {
    ...
    if (!rc) {
        printf("  expected php_method_typed_unindexed for Foo.value, got %d entries\n",
               r->resolved_calls.count);
        for (int i = 0; i < r->resolved_calls.count; i++) {
            const HYPResolvedCall *rcd = &r->resolved_calls.items[i];
...
TEST(phplsp_regression_prompt_value_not_routed_to_helper) {
    ...
    HYPFileResult *r = extract_php(src);
    ASSERT(r);
    const HYPResolvedCall *rc = find_resolved_with_strategy(r, "C.configure", "Prompt.value",
 ...
 * re-matched the loop filter while hyp_registry_add_func() grew the
...
TEST(phplsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    HYPFileResult *r = extract_php(source);
    ASSERT_NOT_NULL(r);
