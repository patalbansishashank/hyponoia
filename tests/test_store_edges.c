...
static hyp_store_t *setup_store_with_nodes(int n, int64_t *ids) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        ids[i] = hyp_store_upsert_node(s, &node);
    }
    return s;
}
...
TEST(store_edge_insert_find) {
    int64_t ids[2];
    hyp_store_t *s = setup_store_with_nodes(2, ids);

    /* Insert edge */
    hyp_edge_t e = {.project = "test", .source_id = ids[0], .target_id = ids[1], .type = "CALLS"};
    int64_t eid = hyp_store_insert_edge(s, &e);
    ASSERT_GT(eid, 0);

    /* Find by source */
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_source(s, ids[0], &edges, &count);
    ...
    /* Find by target */
    rc = hyp_store_find_edges_by_target(s, ids[1], &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 1);
    hyp_store_free_edges(edges, count);
...
TEST(store_imports_edge_local_name_coexist) {
    ...
    hyp_edge_t ea = {.project = "test",
                     .source_id = ids[0],
                     .target_id = ids[1],
                     .type = "IMPORTS",
                     .properties_json = "{\"local_name\":\"A\"}"};
    hyp_edge_t eb = {.project = "test",
                     .source_id = ids[0],
                     .target_id = ids[1],
                     .type = "IMPORTS",
                     .properties_json = "{\"local_name\":\"B\"}"};

    int64_t ida = hyp_store_insert_edge(s, &ea);
    int64_t idb = hyp_store_insert_edge(s, &eb);
    ASSERT_GT(ida, 0);
    ASSERT_GT(idb, 0);
    ASSERT_NEQ(ida, idb); /* distinct local_name -> distinct rows */

    /* Same (src,tgt,IMPORTS,local_name) again -> upsert onto the same row. */
    int64_t ida_again = hyp_store_insert_edge(s, &ea);
    ASSERT_EQ(ida_again, ida);

    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_type(s, "test", "IMPORTS", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 2);
    ...
    hyp_edge_t c1 = {.project = "test",
                     .source_id = ids[0],
                     .target_id = ids[1],
                     .type = "CALLS",
                     .properties_json = "{}"};
    hyp_edge_t c2 = {.project = "test",
                     .source_id = ids[0],
                     .target_id = ids[1],
                     .type = "CALLS",
                     .properties_json = "{\"weight\":5}"};
    int64_t idc1 = hyp_store_insert_edge(s, &c1);
    int64_t idc2 = hyp_store_insert_edge(s, &c2);
    ASSERT_GT(idc1, 0);
    ASSERT_EQ(idc1, idc2);
...
TEST(store_edge_find_by_source_type) {
    int64_t ids[3];
    hyp_store_t *s = setup_store_with_nodes(3, ids);
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_source_type(s, ids[0], "CALLS", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 1);
    ASSERT_STR_EQ(edges[0].type, "CALLS");
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_find_by_target_type) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_target_type(s, ids[2], "CALLS", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 1);
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_find_by_type) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_type(s, "test", "CALLS", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 2);
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_count_by_type) {
    ...
    int cnt = hyp_store_count_edges_by_type(s, "test", "CALLS");
    ASSERT_EQ(cnt, 2);

    cnt = hyp_store_count_edges_by_type(s, "test", "IMPORTS");
    ASSERT_EQ(cnt, 1);

    cnt = hyp_store_count_edges_by_type(s, "test", "NONEXISTENT");
    ASSERT_EQ(cnt, 0);
...
TEST(store_edge_delete_by_type) {
    ...
    hyp_store_delete_edges_by_type(s, "test", "CALLS");
    int ecnt = hyp_store_count_edges(s, "test");
    ASSERT_EQ(ecnt, 1);
...
TEST(store_edge_properties_json) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    hyp_store_find_edges_by_source(s, ids[0], &edges, &count);
    ASSERT_EQ(count, 1);
...
TEST(store_edge_batch_insert) {
    int64_t ids[10];
    hyp_store_t *s = setup_store_with_nodes(10, ids);
    ...
    int rc = hyp_store_insert_edge_batch(s, edges, 9);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ...
    rc = hyp_store_insert_edge_batch(s, edges, 9);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ecnt = hyp_store_count_edges(s, "test");
    ASSERT_EQ(ecnt, 9);
...
TEST(store_edge_batch_empty) {
    hyp_store_t *s = hyp_store_open_memory();
    int rc = hyp_store_insert_edge_batch(s, NULL, 0);
    ASSERT_EQ(rc, HYP_STORE_OK);
    hyp_store_close(s);
    PASS();
}
...
TEST(store_edge_cascade_on_node_delete) {
    ...
    /* A→B, A→C */
    hyp_edge_t e1 = {.project = "test", .source_id = ids[0], .target_id = ids[1], .type = "CALLS"};
    hyp_edge_t e2 = {.project = "test", .source_id = ids[0], .target_id = ids[2], .type = "CALLS"};
    hyp_store_insert_edge(s, &e1);
    hyp_store_insert_edge(s, &e2);

    /* Delete node A — edges should cascade */
    hyp_store_delete_nodes_by_project(s, "test");
    int ecnt = hyp_store_count_edges(s, "test");
    ASSERT_EQ(ecnt, 0);
...
TEST(store_edge_batch_insert_zero_count) {
    ...
    hyp_edge_t dummy = {.project = "test", .source_id = 1, .target_id = 2, .type = "CALLS"};
    int rc = hyp_store_insert_edge_batch(s, &dummy, 0);
    ASSERT_EQ(rc, HYP_STORE_OK);
...
TEST(store_edge_batch_insert_50) {
    ...
    int rc = hyp_store_insert_edge_batch(s, edges, 50);
    ASSERT_EQ(rc, HYP_STORE_OK);
...
TEST(store_edge_find_source_nonexistent) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_source(s, 999999, &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 0);
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_find_target_nonexistent) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_target(s, 999999, &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 0);
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_find_type_nonexistent) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_type(s, "test", "DOES_NOT_EXIST", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 0);
    hyp_store_free_edges(edges, count);
...
TEST(store_edge_count_empty_project) {
    hyp_store_t *s = hyp_store_open_memory();
    hyp_store_upsert_project(s, "empty", "/tmp/empty");

    int ecnt = hyp_store_count_edges(s, "empty");
    ASSERT_EQ(ecnt, 0);
...
TEST(store_edge_count_by_type_missing) {
    ...
    int cnt = hyp_store_count_edges_by_type(s, "test", "NEVER_EXISTS");
    ASSERT_EQ(cnt, 0);
...
TEST(store_edge_delete_by_type_preserves_others) {
    ...
    hyp_store_insert_edge(s, &e2);
    hyp_store_insert_edge(s, &e3);
    ASSERT_EQ(hyp_store_count_edges(s, "test"), 3);

    /* Delete only CALLS */
    hyp_store_delete_edges_by_type(s, "test", "CALLS");
    ASSERT_EQ(hyp_store_count_edges(s, "test"), 2);

    /* IMPORTS and HTTP_CALLS still exist */
    ASSERT_EQ(hyp_store_count_edges_by_type(s, "test", "IMPORTS"), 1);
    ASSERT_EQ(hyp_store_count_edges_by_type(s, "test", "HTTP_CALLS"), 1);
    ASSERT_EQ(hyp_store_count_edges_by_type(s, "test", "CALLS"), 0);
...
TEST(store_edge_delete_by_project_preserves_others) {
    hyp_store_t *s = hyp_store_open_memory();
    hyp_store_upsert_project(s, "alpha", "/tmp/alpha");
    hyp_store_upsert_project(s, "beta", "/tmp/beta");
    ...
    hyp_edge_t e1 = {.project = "alpha", .source_id = idA, .target_id = idB, .type = "CALLS"};
    hyp_edge_t e2 = {.project = "beta", .source_id = idC, .target_id = idD, .type = "CALLS"};
    hyp_store_insert_edge(s, &e1);
    hyp_store_insert_edge(s, &e2);

    ASSERT_EQ(hyp_store_count_edges(s, "alpha"), 1);
    ASSERT_EQ(hyp_store_count_edges(s, "beta"), 1);

    /* Delete alpha edges only */
    hyp_store_delete_edges_by_project(s, "alpha");
    ASSERT_EQ(hyp_store_count_edges(s, "alpha"), 0);
    ASSERT_EQ(hyp_store_count_edges(s, "beta"), 1);
...
TEST(store_edge_long_type_string) {
    ...
    hyp_edge_t e = {.project = "test", .source_id = ids[0], .target_id = ids[1], .type = long_type};
    int64_t eid = hyp_store_insert_edge(s, &e);
    ASSERT_GT(eid, 0);
...
TEST(store_edge_find_source_type_nonexistent) {
    ...
    hyp_edge_t *edges = NULL;
    int count = 0;
    int rc = hyp_store_find_edges_by_source_type(s, ids[0], "NOPE", &edges, &count);
    ASSERT_EQ(rc, HYP_STORE_OK);
    ASSERT_EQ(count, 0);
    hyp_store_free_edges(edges, count);
