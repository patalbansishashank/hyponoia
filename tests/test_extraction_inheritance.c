 ...
 * DESIGN INTENT
 * ─────────────
 * Every row asserts the CORRECT behaviour: that `HYPDefinition.base_classes`
...
static HYPDefinition *find_def_any(HYPFileResult *r, const char *name) {
    for (int i = 0; i < r->defs.count; i++) {
        HYPDefinition *d = &r->defs.items[i];
...
static int bases_contain(HYPDefinition *d, const char *want) {
    if (!d->base_classes)
        return 0;
...
static int bases_contain_substr(HYPDefinition *d, const char *substr) {
    if (!d->base_classes)
        return 0;
    for (const char **b = d->base_classes; *b; b++) {
        if (strstr(*b, substr))
...
static int bases_count(HYPDefinition *d) {
    if (!d->base_classes)
        return 0;
    int n = 0;
    for (const char **b = d->base_classes; *b; b++)
...
static HYPDefinition *find_def_flex(HYPFileResult *r, const char *name) {
    HYPDefinition *d;
...
static int run_inherit_case(const inherit_case_t *tc) {
    HYPFileResult *r =
        hyp_extract_file(tc->src, (int)strlen(tc->src), tc->lang, "t", tc->path, 0, NULL, NULL);
    if (!r) {
        printf("  FAIL  [%s] hyp_extract_file returned NULL\n", tc->class_name);
        return 1;
    }

    HYPDefinition *cls = find_def_flex(r, tc->class_name);
    if (!cls) {
 ...
 * extractor populates HYPFileResult.impl_traits (HYPImplTrait array)
 * with {trait_name, struct_name} pairs.
 * ═══════════════════════════════════════════════════════════════════ */

/* Rust helper: check impl_traits array. */
static int rust_has_impl(HYPFileResult *r, const char *trait_name, const char *struct_name) {
    for (int i = 0; i < r->impl_traits.count; i++) {
        HYPImplTrait *it = &r->impl_traits.items[i];
...
static int run_rust_impl(const rust_impl_case_t *tc) {
    HYPFileResult *r = hyp_extract_file(tc->src, (int)strlen(tc->src), HYP_LANG_RUST, "t", "lib.rs",
                                        0, NULL, NULL);
    if (!r) {
