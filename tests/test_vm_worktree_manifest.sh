...
FIXTURE="$(mktemp -d "${TMPDIR:-/tmp}/hyp-vm-manifest-test.XXXXXX")"
manifest="$(mktemp "${TMPDIR:-/tmp}/hyp-vm-manifest-output.XXXXXX")"
...
hyp_vm_write_untracked_manifest "$FIXTURE" "$manifest"
...
if ! grep -Fq 'HYP_CI_RUN_ID" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$' "$driver"; then
...
if grep -Fq 'C:\\hyp\\scripts\\ci' <<<"$preflight_block" ||
...
for entrypoint in "$driver" "$provisioner"; do
    ...
    if ! grep -Fq 'hyp_vm_require_safe_branch "$BRANCH"' "$entrypoint"; then
...
if ! grep -Fq 'hyp_vm_sync_windows_clock()' \
...
clock_helper=$(sed -n '/^hyp_vm_sync_windows_clock() {$/,/^}$/p' \
...
for entrypoint in "$driver" "$provisioner"; do
    if ! grep -Fq 'hyp_vm_sync_windows_clock "${SSH[@]}"' "$entrypoint"; then
        echo "FAIL: Windows VM entry point must repair suspended-guest clock drift: $entrypoint" >&2
        exit 1
    fi
done
if ! grep -Fq 'hyp_vm_run_powershell()' \
    "$ROOT/test-infrastructure/vm/ssh-common.sh" ||
...
if ! grep -Fq 'scripts/build.sh --with-ui CC=clang CXX=clang++ SANITIZE= BUILD_DIR=build/guards' \
    ...
    <<<"$guards_block" ||
    ! grep -Fq -- '-GuardsOnly -Binary build\\guards\\hyponoia.exe' <<<"$guards_block"; then
...
if ! grep -Fq 'cd /d ${VM_REPO_WIN}' <<<"$guards_block" ||
   grep -Fq 'cd /d C:\\hyp' <<<"$guards_block"; then
...
if ! grep -Fq 'HYP_VM_BRANCH' "$ROOT/test-infrastructure/vm/README.md"; then
