...
static HYPFileResult *extract_cs(const char *source) {
    return hyp_extract_file(source, (int)strlen(source), HYP_LANG_CSHARP, "test", "main.cs", 0,
                            NULL, NULL);
}
...
static int count_resolved_with_strategy(const HYPFileResult *r, const char *strategy) {
    int n = 0;
    for (int i = 0; i < r->resolved_calls.count; i++) {
        const HYPResolvedCall *rc = &r->resolved_calls.items[i];
...
TEST(cslsp_local_method_via_new) {
    const char *src =
        ...
        "    }\n"
        "}\n";
    HYPFileResult *r = extract_cs(src);
    ASSERT(r);
...
TEST(cslsp_ordinary_same_leaf_calls_join_by_exact_site) {
    ...
    HYPFileResult *r = extract_cs(source);
    ASSERT_NOT_NULL(r);
