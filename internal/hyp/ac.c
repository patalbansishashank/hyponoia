...
#define HYP_AC_MAX_BITMASK 64

#define HYP_AC_BYTE_RANGE 256
#define HYP_AC_NO_STATE (-1)
#define HYP_AC_ROOT_STATES 1
#define HYP_AC_ALLOC_ONE 1
#define HYP_AC_PATTERN_BIT(p) (1ULL << (p))
#define HYP_AC_CLEAR_LOW_BIT(b) ((b) & ((b) - 1ULL))
...
struct HYPAutomaton {
    ...
    uint8_t alpha_map[HYP_AC_BYTE_RANGE]; // byte → mapped index (identity if alpha_size==256)
...
static int ac_build_trie(HYPAutomaton *ac, const char **patterns, const int *lengths, int count) {
    int alpha_size = ac->alpha_size;
    int num_states = HYP_AC_ROOT_STATES; // state 0 = root
    ...
    for (int p = 0; p < count; p++) {
        ...
        for (int j = 0; j < lengths[p]; j++) {
            ...
            if (ac->go_table[idx] == HYP_AC_NO_STATE) {
                ac->go_table[idx] = num_states++;
            }
            state = ac->go_table[idx];
        }
        if (p < HYP_AC_MAX_BITMASK) {
            ac->output[state] |= HYP_AC_PATTERN_BIT(p);
        }
    ...
    for (int c = 0; c < alpha_size; c++) {
        if (ac->go_table[c] == HYP_AC_NO_STATE) {
            ac->go_table[c] = 0;
        }
    }
    return num_states;
...
static void ac_build_failure(HYPAutomaton *ac, int num_states) {
    int alpha_size = ac->alpha_size;
    ...
    while (!queue_empty(&q)) {
        ...
        for (int c = 0; c < alpha_size; c++) {
            ...
            if (s != HYP_AC_NO_STATE) {
                fail[s] = ac->go_table[(fail[r] * alpha_size) + c];
                ac->output[s] |= ac->output[fail[s]];
                if (ac->output_next[s] == HYP_AC_NO_STATE &&
                    ac->output_list[fail[s]] != HYP_AC_NO_STATE) {
...
static void ac_shrink_tables(HYPAutomaton *ac, int num_states, int max_states) {
...
// hyp_ac_build constructs an Aho-Corasick automaton from a set of patterns.
//
// Parameters:
...
//
// Returns a heap-allocated automaton. Caller must call hyp_ac_free().
HYPAutomaton *hyp_ac_build(const char **patterns, const int *lengths, int count,
    ...
    if (alpha_size <= 0) {
        alpha_size = HYP_AC_BYTE_RANGE;
    }

    int max_states = HYP_AC_ROOT_STATES;
    for (int i = 0; i < count; i++) {
        max_states += lengths[i];
    }

    HYPAutomaton *ac = (HYPAutomaton *)calloc(HYP_AC_ALLOC_ONE, sizeof(HYPAutomaton));
    ac->alpha_size = alpha_size;
    ac->num_patterns = count;

    if (alpha_map) {
        memcpy(ac->alpha_map, alpha_map, HYP_AC_BYTE_RANGE);
    } else {
        for (int i = 0; i < HYP_AC_BYTE_RANGE; i++) {
            ac->alpha_map[i] = (uint8_t)i;
        }
    }

    ac->go_table = (int *)malloc((size_t)max_states * alpha_size * sizeof(int));
    memset(ac->go_table, HYP_AC_NO_STATE, (size_t)max_states * alpha_size * sizeof(int));
    ...
    for (int i = 0; i < max_states; i++) {
        ac->output_list[i] = HYP_AC_NO_STATE;
        ac->output_next[i] = HYP_AC_NO_STATE;
    }
...
// hyp_ac_free releases all memory for an automaton.
...
// hyp_ac_scan_bitmask scans text through the automaton and returns a bitmask
...
// Each goroutine gets its own OS thread (via CGo), so HYP_TLS is safe.
static HYP_TLS char *tls_decomp_buf = NULL;
static HYP_TLS int tls_decomp_cap = 0;
...
// hyp_ac_scan_lz4_bitmask decompresses LZ4 data into a thread-local buffer
...
uint64_t hyp_ac_scan_lz4_bitmask(const HYPAutomaton *ac, const char *compressed, int compressed_len,
    ...
    int decompressed = hyp_lz4_decompress(compressed, compressed_len, buf, original_len);
    if (decompressed < 0) {
        return 0;
    }

    return hyp_ac_scan_bitmask(ac, buf, decompressed);
}

// ─── Batch LZ4 + AC scan ───────────────────────────────────────────────────

// HYPLz4Entry and HYPLz4Match defined in ac.h.

// hyp_ac_scan_lz4_batch decompresses and scans multiple files in one call.
...
int hyp_ac_scan_lz4_batch(const HYPAutomaton *ac, const HYPLz4Entry *entries, int num_entries,
    ...
    for (int i = 0; i < num_entries && total < max_matches; i++) {
        ...
        int decompressed = hyp_lz4_decompress(entries[i].data, entries[i].compressed_len, buf,
...
// HYPMatchResult defined in ac.h.

// hyp_ac_scan_batch scans multiple NUL-separated names through the automaton.
        ...
        for (int i = 0; i < text_len; i++) {
            ...
            while (s > 0 && total < max_matches) {
                ...
                while (bits && total < max_matches) {
                    ...
                    total++;
                    seen |= HYP_AC_PATTERN_BIT(pid);
                    bits = HYP_AC_CLEAR_LOW_BIT(bits);
                }

                // Follow output_next for patterns beyond bitmask range.
                int next_state = ac->output_next[s];
                if (next_state == HYP_AC_NO_STATE || next_state == s) {
                    break;
                }
                s = next_state;
            }
...
// hyp_ac_table_bytes returns the approximate memory used by the goto table.
