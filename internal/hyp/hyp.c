#include "hyp.h"
#include "arena.h" // HYPArena, hyp_arena_init/alloc/strdup/destroy
#include "helpers.h"
...
#include "foundation/compat_fs.h"  // hyp_fopen — crash-supervisor per-file marker write
#include "foundation/hash_table.h" // HYPHashTable — crash-supervisor quarantine set
...
/* Use compat.h's hyp_clock_gettime which accepts CLOCK_MONOTONIC (value
 * varies by platform: 1 on Linux/Windows, 6 on macOS). We pass the
 * platform value via the compat.h fallback. */
#if defined(CLOCK_MONOTONIC)
#define HYP_CLOCK_MONO CLOCK_MONOTONIC
#elif defined(__APPLE__)
#define HYP_CLOCK_MONO 6
#else
#define HYP_CLOCK_MONO 1
#endif

static uint64_t now_ns(void) {
    struct timespec ts;
    hyp_clock_gettime(HYP_CLOCK_MONO, &ts);
...
// hyp_reset_profile zeros the profiling counters.
...
#define GROW_ARRAY(arr, arena)                                                                   \
    do {                                                                                         \
        if ((arr)->count >= (arr)->cap) {                                                        \
            int new_cap = (arr)->cap == 0 ? HYP_SZ_32 : (arr)->cap * PAIR_LEN;                   \
            void *new_items = hyp_arena_alloc((arena), (size_t)new_cap * sizeof(*(arr)->items)); \
            if (!new_items)                                                                      \
                return;                                                                          \
...
static const char *hyp_string_read(void *payload, uint32_t byte, TSPoint point,
                                   uint32_t *bytes_read) {
    (void)point;
    HYPStringInput *self = (HYPStringInput *)payload;
    if (byte >= self->length) {
        *bytes_read = 0;
        return "";
    }
...
static bool hyp_timeout_cb(TSParseState *state) {
...
static HYP_TLS TSParser *tl_parser = NULL;
static HYP_TLS HYPLanguage tl_parser_lang = HYP_LANG_COUNT; // invalid sentinel

// Get or create a thread-local parser configured for the given language.
static TSParser *get_thread_parser(const TSLanguage *ts_lang, HYPLanguage lang) {
    if (!tl_parser) {
        tl_parser = ts_parser_new();
        if (!tl_parser) {
            return NULL;
        }
        tl_parser_lang = HYP_LANG_COUNT;
    }
 ...
 *
 * Guarded to the production build (HYP_BIND_TS_ALLOCATOR=1, which CFLAGS_PROD
 ...
 * (#581). */
static void *hyp_sqlite_malloc(int n) {
    void *block = mi_malloc((size_t)n);
    return block;
}
static void hyp_sqlite_free(void *p) {
    mi_free(p);
}
static void *hyp_sqlite_realloc(void *p, int n) {
    if (p) {}
    void *grown = mi_realloc(p, (size_t)n);
    return grown;
}
static int hyp_sqlite_size(void *p) {
    return (int)mi_usable_size(p);
}
static int hyp_sqlite_roundup(int n) {
...
static void *hyp_ts_malloc(size_t n) {
    void *block = mi_malloc(n);
    return block;
}
static void *hyp_ts_calloc(size_t count, size_t size) {
    void *block = mi_calloc(count, size);
    return block;
}
static void *hyp_ts_realloc(void *p, size_t n) {
    if (p) {}
    void *grown = mi_realloc(p, n);
    return grown;
}
static void hyp_ts_free(void *p) {
    mi_free(p);
}

static int hyp_sqlite_meminit(void *appdata) {
    (void)appdata;
    return SQLITE_OK;
}
static void hyp_sqlite_memshutdown(void *appdata) {
    (void)appdata;
}
#endif /* HYP_BIND_TS_ALLOCATOR */
...
void hyp_alloc_init(void) {
    ...
    /* tree-sitter runtime (was previously bound in hyp_init; consolidated here). */
    ts_set_allocator(hyp_ts_malloc, hyp_ts_calloc, hyp_ts_realloc, hyp_ts_free);
     ...
     * silently and the binding is ignored. hyp_alloc_init() runs as the very
     * first statement of main(), before hyp_mcp_server_new → hyp_store_open*. */
    static sqlite3_mem_methods hyp_sqlite_mem = {
        hyp_sqlite_malloc,      /* xMalloc */
        hyp_sqlite_free,        /* xFree */
        hyp_sqlite_realloc,     /* xRealloc */
        hyp_sqlite_size,        /* xSize */
        hyp_sqlite_roundup,     /* xRoundup */
        hyp_sqlite_meminit,     /* xInit */
        hyp_sqlite_memshutdown, /* xShutdown */
        NULL,                   /* pAppData */
    };
    int sqlite_rc = sqlite3_config(SQLITE_CONFIG_MALLOC, &hyp_sqlite_mem);
...
static int hyp_initialized = 0;

int hyp_init(void) {
    if (hyp_initialized) {
        return 0;
    }
    enum { HYP_INIT_DONE = 1 };
    hyp_initialized = HYP_INIT_DONE;
    /* Defense-in-depth allocator binds (idempotent). main() calls hyp_alloc_init
     * first; this covers non-main entry points (pipeline passes call hyp_init).
...
void hyp_reset_thread_parser(void) {
    ...
    // Must be called BEFORE hyp_slab_reset_thread() to avoid corrupting
...
// (Go: `s` in `func (s *Store) save()`, from HYPDefinition.receiver). Matching
...
static void hyp_index_mark(const char *rel_path, char event) {
    const char *mf = getenv("HYP_INDEX_MARKER_FILE");
    if (!mf || !mf[0] || !rel_path || !rel_path[0]) {
        return;
    }
    FILE *f = hyp_fopen(mf, "ab");
    if (f) {
        (void)fprintf(f, "%c %s\n", event, rel_path);
        (void)fclose(f);
    }
 ...
 * (hyp_extract_file): whether a pass re-extracts from disk on a cache miss
 ...
 * hyp_index_is_quarantined() so the crasher surfaces in the response skipped[].
...
static HYPHashTable *g_quarantine_set = NULL;
enum { HYP_QSET_UNINIT = 0, HYP_QSET_INITING = 1, HYP_QSET_INITED = 2 };
static atomic_int g_quarantine_state = HYP_QSET_UNINIT;

static void hyp_quarantine_load(void) {
    const char *qf = getenv("HYP_INDEX_QUARANTINE_FILE");
    if (!qf || !qf[0]) {
        return; /* normal path: empty set */
    }
    FILE *f = hyp_fopen(qf, "rb");
    if (!f) {
        return;
    }
    HYPHashTable *set = hyp_ht_create(16);
    if (!set) {
        (void)fclose(f);
        return;
    }
    ...
    while (fgets(line, sizeof(line), f)) {
         ...
         * The value stores the phase so hyp_index_quarantine_phase() can report
         * "crash" vs "hang"; membership (hyp_index_is_quarantined) is value != NULL. */
        char *pval = hyp_strdup(phase);
        if (!pval) {
            continue;
        }
        if (hyp_ht_has(set, line)) {
             ...
             * value it displaces. */
            free(hyp_ht_set(set, line, (void *)pval));
        } else {
            char *key = hyp_strdup(line);
            if (key) {
                hyp_ht_set(set, key, (void *)pval);
            } else {
...
bool hyp_index_is_quarantined(const char *rel_path) {
    ...
    if (state != HYP_QSET_INITED) {
        /* First caller wins the CAS and loads; racers spin until INITED.
         * Same once-init pattern as hyp_ui_log_init (http_server.c). */
        state = HYP_QSET_UNINIT;
        if (atomic_compare_exchange_strong(&g_quarantine_state, &state, HYP_QSET_INITING)) {
            hyp_quarantine_load();
            atomic_store(&g_quarantine_state, HYP_QSET_INITED);
        } else {
            while (atomic_load(&g_quarantine_state) != HYP_QSET_INITED) {
                hyp_usleep(1000); /* 1ms */
            }
        }
    }
    return g_quarantine_set && hyp_ht_has(g_quarantine_set, rel_path);
}

const char *hyp_index_quarantine_phase(const char *rel_path) {
    /* hyp_index_is_quarantined drives the lazy once-load and returns true only
    ...
    if (!hyp_index_is_quarantined(rel_path)) {
        return NULL;
    }
    return (const char *)hyp_ht_get(g_quarantine_set, rel_path);
}
...
static void hyp_test_fault_inject(const char *rel_path) {
    if (!rel_path || !rel_path[0]) {
        return;
    }
    const char *crash_on = getenv("HYP_TEST_CRASH_ON");
    if (crash_on && crash_on[0] && strstr(rel_path, crash_on)) {
        abort(); /* SIGABRT → WIFSIGNALED → classified as a crash */
    }
    const char *hang_on = getenv("HYP_TEST_HANG_ON");
 ...
 *
 * hyp_source_nesting_exceeds scans the raw bytes for the maximum bracket-nesting
...
#define HYP_PERL_MAX_PARSE_NESTING 128

static bool hyp_source_nesting_exceeds(const char *source, int source_len, int cap) {
    int depth = 0;
 ...
 * Bounded by HYP_MAX_ERROR_REGIONS so pathological input can't blow up the
 * output. The ranges mark where constructs were dropped; they are a detection
 * aid, never a completeness proof. */
#define HYP_MAX_ERROR_REGIONS 64
typedef struct {
    uint32_t starts[HYP_MAX_ERROR_REGIONS];
    uint32_t ends[HYP_MAX_ERROR_REGIONS];
    int count;
} hyp_error_regions_t;

static void hyp_error_regions_push(hyp_error_regions_t *acc, TSNode n) {
    if (acc->count >= HYP_MAX_ERROR_REGIONS) {
        return;
    }
...
static void hyp_collect_error_regions(TSNode n, hyp_error_regions_t *acc) {
    if (acc->count >= HYP_MAX_ERROR_REGIONS) {
        return;
    }
    uint32_t k = ts_node_child_count(n);
    for (uint32_t i = 0; i < k && acc->count < HYP_MAX_ERROR_REGIONS; i++) {
        TSNode c = ts_node_child(n, i);
        if (ts_node_is_missing(c) || strcmp(ts_node_type(c), "ERROR") == 0) {
            hyp_error_regions_push(acc, c); /* top-most region; do not descend */
        } else if (ts_node_has_error(c)) {
            hyp_collect_error_regions(c, acc);
        }
    }
}
...
static bool hyp_region_is_recovered(uint32_t rs, uint32_t re, const HYPDefArray *defs) {
    ...
    for (int i = 0; i < defs->count && n < MAX_COVER_DEFS; i++) {
        const HYPDefinition *d = &defs->items[i];
...
static bool hyp_line_contains(const char *src, int src_len, uint32_t line, const char *name) {
...
static bool hyp_identifier_char(char c) {
...
static bool hyp_span_contains_callable_def(const char *src, int src_len, uint32_t start_line,
    ...
    for (int pos = span_start; pos + (int)name_len <= span_end; pos++) {
        if (strncmp(src + pos, name, name_len) != 0 ||
            (pos > 0 && hyp_identifier_char(src[pos - 1])) ||
            (pos + (int)name_len < src_len && hyp_identifier_char(src[pos + name_len]))) {
            continue;
        }
        int before = pos;
...
static bool hyp_remap_preprocessed_def(HYPDefinition *def, const HYPPreprocessedSource *pp) {
...
static void hyp_subtract_recovered_regions(hyp_error_regions_t *regs, const HYPDefArray *defs) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        if (!hyp_region_is_recovered(regs->starts[i], regs->ends[i], defs)) {
...
static bool hyp_span_is_macro_invocation(const char *src, int src_len, uint32_t start_line,
                                         uint32_t end_line, const HYPDefArray *defs) {
    ...
    for (int di = 0; di < defs->count; di++) {
        const HYPDefinition *d = &defs->items[di];
        ...
        for (int pos = span_start; pos + nlen <= span_end; pos++) {
            if (strncmp(src + pos, d->name, (size_t)nlen) != 0 ||
                (pos > 0 && hyp_identifier_char(src[pos - 1])) ||
                (pos + nlen < src_len && hyp_identifier_char(src[pos + nlen]))) {
                continue;
            }
            int open = pos + nlen;
            while (open < span_end && isspace((unsigned char)src[open])) {
...
static bool hyp_region_inside_callable(uint32_t rs, uint32_t re, const HYPDefArray *defs) {
...
static void hyp_subtract_macro_invocation_regions(hyp_error_regions_t *regs,
                                                  const HYPDefArray *defs, const char *src,
                                                  int src_len) {
    int kept = 0;
    for (int i = 0; i < regs->count; i++) {
        bool benign =
            hyp_span_is_macro_invocation(src, src_len, regs->starts[i], regs->ends[i], defs) &&
            hyp_region_inside_callable(regs->starts[i], regs->ends[i], defs);
        if (!benign) {
            regs->starts[kept] = regs->starts[i];
            regs->ends[kept] = regs->ends[i];
            kept++;
...
static const char *hyp_error_ranges_str(HYPArena *a, const hyp_error_regions_t *regs) {
    if (regs->count <= 0) {
        return NULL;
    }
    enum { RANGE_MAX = 24 }; /* "4294967295-4294967295," */
    char *buf = (char *)hyp_arena_alloc(a, (size_t)regs->count * RANGE_MAX);
    if (!buf) {
        return NULL;
    }
    size_t off = 0;
...
HYPFileResult *hyp_extract_file(const char *source, int source_len, HYPLanguage language,
    ...
    HYPFileResult *r =
        hyp_extract_file_ex(source, source_len, language, project, rel_path, timeout_micros,
                            extra_defines, include_paths, NULL, NULL);
    return r;
}

HYPFileResult *hyp_extract_file_ex(const char *source, int source_len, HYPLanguage language,
                                   const char *project, const char *rel_path,
                                   int64_t timeout_micros, const char **extra_defines,
                                   const char **include_paths, const HYPMacroTable *macro_table,
                                   const HYPReturnTypeTable *return_type_table) {
    // Allocate result on heap (arena inside for all string data)
    enum { SINGLE = 1 };
    HYPFileResult *result = (HYPFileResult *)calloc(SINGLE, sizeof(HYPFileResult));
    if (!result) {
        return NULL;
    }

    hyp_arena_init(&result->arena);
    HYPArena *a = &result->arena;
    ...
    if (!spec) {
        ...
        result->error_msg = hyp_arena_strdup(a, "unsupported language");
        hyp_index_mark_done(rel_path);
        return result;
    }

    // Get tree-sitter language
    const TSLanguage *ts_lang = hyp_ts_language(language);
    if (!ts_lang) {
        result->has_error = true;
        result->error_msg = hyp_arena_strdup(a, "no tree-sitter grammar");
        hyp_index_mark_done(rel_path);
        return result;
    }
    ...
    // hyp_source_nesting_exceeds). Scoped to Perl: its ambiguous call grammar is
    // the only one that drives that recursion to the nesting depth.
    if (language == HYP_LANG_PERL &&
        hyp_source_nesting_exceeds(source, source_len, HYP_PERL_MAX_PARSE_NESTING)) {
        result->has_error = true;
        result->error_msg = hyp_arena_strdup(a, "perl source nesting too deep; skipped");
        return result;
    }
    ...
    if (!parser) {
        result->has_error = true;
        result->error_msg = hyp_arena_strdup(a, "parser alloc failed");
        hyp_index_mark_done(rel_path);
        return result;
    }
    ...
    HYPStringInput str_input = {source, (uint32_t)source_len};
    TSInput ts_input = {
        &str_input,
        hyp_string_read,
        TSInputEncodingUTF8,
    ...
    if (timeout_micros > 0) {
        ...
        opts.progress_callback = hyp_timeout_cb;
    }
    ...
    result->module_qn = hyp_fqn_module_source_lang(a, project, rel_path, language);
    result->is_test_file = hyp_is_test_file(rel_path, language);
    ...
    hyp_extract_definitions(&ctx);
    hyp_extract_imports(&ctx);
    hyp_extract_unified(&ctx);

    // Channel detection (Socket.IO / EventEmitter) — JS/TS only.
    hyp_extract_channels(&ctx);

    // K8s / Kustomize semantic pass (additional structured extraction for YAML-based infra files).
    if (ctx.language == HYP_LANG_KUSTOMIZE || ctx.language == HYP_LANG_K8S) {
        hyp_extract_k8s(&ctx);
    }
    ...
    {
        if (language == HYP_LANG_GO) {
            hyp_run_go_lsp(a, result, source, source_len, root);
        }
        if (language == HYP_LANG_C || language == HYP_LANG_CPP || language == HYP_LANG_CUDA) {
            hyp_run_c_lsp(a, result, source, source_len, root, language != HYP_LANG_C,
                          HYP_SOURCE_ORIGIN_RAW);
        }
        if (language == HYP_LANG_PHP) {
            hyp_run_php_lsp(a, result, source, source_len, root);
        }
        if (language == HYP_LANG_PERL) {
            hyp_run_perl_lsp(a, result, source, source_len, root);
        }
        if (language == HYP_LANG_PYTHON) {
            hyp_run_py_lsp(a, result, source, source_len, root);
        }
        if (language == HYP_LANG_JAVASCRIPT || language == HYP_LANG_TYPESCRIPT ||
            language == HYP_LANG_TSX) {
            bool js_mode = (language == HYP_LANG_JAVASCRIPT);
            // jsx_mode: TSX always; .jsx in the JS bucket also enables it.
            bool jsx_mode = (language == HYP_LANG_TSX);
            if (language == HYP_LANG_JAVASCRIPT && rel_path) {
            ...
            bool dts_mode = false;
            if (language == HYP_LANG_TYPESCRIPT && rel_path) {
                size_t rl = strlen(rel_path);
                if (rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0)
                    dts_mode = true;
            }
            hyp_run_ts_lsp(a, result, source, source_len, root, js_mode, jsx_mode, dts_mode);
        }
        if (language == HYP_LANG_CSHARP) {
            hyp_run_cs_lsp(a, result, source, source_len, root);
        }
    }
    if (language == HYP_LANG_JAVA) {
        hyp_run_java_lsp(a, result, source, source_len, root);
    }
    if (language == HYP_LANG_KOTLIN) {
        hyp_run_kotlin_lsp(a, result, source, source_len, root);
    }
    if (language == HYP_LANG_RUST) {
        hyp_run_rust_lsp(a, result, source, source_len, root);
    }
    ...
    if (language == HYP_LANG_C || language == HYP_LANG_CPP || language == HYP_LANG_CUDA) {
        ...
        HYPPreprocessedSource *preprocessed = hyp_preprocess_with_map(
            source, source_len, rel_path, extra_defines, include_paths, language != HYP_LANG_C);
        if (preprocessed && preprocessed->source) {
            ...
            if (pp_parser) {
                ...
                HYPStringInput pp_input = {expanded, (uint32_t)expanded_len};
                TSInput pp_ts_input = {
                    &pp_input,
                    hyp_string_read,
                    TSInputEncodingUTF8,
                ...
                if (pp_tree) {
                    ...
                    for (int i = calls_before; i < result->calls.count; i++) {
                        result->calls.items[i].source_origin = HYP_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    for (int i = usages_before; i < result->usages.count; i++) {
                        result->usages.items[i].source_origin = HYP_SOURCE_ORIGIN_PREPROCESSED;
                    }
                    ...
                    hyp_run_c_lsp(a, result, expanded, expanded_len, pp_root,
                                  language != HYP_LANG_C, HYP_SOURCE_ORIGIN_PREPROCESSED);
                    ...
                    if (ts_node_has_error(root)) {
                        hyp_error_regions_t raw_regs = {{0}, {0}, 0};
                        hyp_collect_error_regions(root, &raw_regs);
                        if (raw_regs.count > 0) {
                            int defs_before = result->defs.count;
                            hyp_extract_definitions(&pp_ctx);
                            int w = defs_before;
                            for (int i = defs_before; i < result->defs.count; i++) {
                                HYPDefinition *d = &result->defs.items[i];
                                bool adopt = false;
                                if (hyp_remap_preprocessed_def(d, preprocessed)) {
                                ...
                                if (adopt && (!d->name ||
                                              !hyp_line_contains(source, source_len, d->start_line,
                                                                 d->name) ||
                                              !hyp_span_contains_callable_def(
            ...
            hyp_preprocessed_source_free(preprocessed);
    ...
    // HYPFileResult share the same file). Range matching is used instead of
    ...
    for (int di = 0; di < def_count; di++) {
        HYPDefinition *d = &result->defs.items[di];
        int pc = 0;
        if (d->param_names) {
    ...
    for (int ci = 0; ci < orig_calls_count; ci++) {
        const HYPCall *c = &result->calls.items[ci];
        ...
        int best_span = -1;
        for (int di = 0; di < def_count; di++) {
            const HYPDefinition *d = &result->defs.items[di];
            if (!d->name || !d->label ||
        ...
        HYPDefinition *d = &result->defs.items[best];
    ...
    if (ts_node_has_error(root)) {
        hyp_error_regions_t regs = {{0}, {0}, 0};
        if (strcmp(ts_node_type(root), "ERROR") == 0) {
            hyp_error_regions_push(&regs, root); /* whole file unparseable */
        } else {
            hyp_collect_error_regions(root, &regs);
        }
        hyp_subtract_recovered_regions(&regs, &result->defs);
        /* #1071: don't flag a benign function-like-macro call (defined in-file)
         * that tree-sitter can't parse without the preprocessor. */
        hyp_subtract_macro_invocation_regions(&regs, &result->defs, source, source_len);
        if (regs.count > 0) {
            result->parse_incomplete = true;
            result->error_region_count = regs.count;
            result->error_ranges = hyp_error_ranges_str(a, &regs);
        }
    }
    ...
    // Retain tree for cross-file LSP reuse (caller frees via hyp_free_tree)
...
void hyp_free_result(HYPFileResult *result) {
    ...
    for (int i = 0; i < result->owned_result_count; i++) {
        hyp_free_result(result->owned_results[i]);
    }
