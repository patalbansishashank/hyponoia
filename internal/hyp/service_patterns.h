 ...
 */
#ifndef HYP_SERVICE_PATTERNS_H
#define HYP_SERVICE_PATTERNS_H

#include <stdbool.h>

/* Edge type returned by pattern match. */
typedef enum {
    HYP_SVC_NONE = 0,      /* Not a service pattern — use normal CALLS */
    HYP_SVC_HTTP = 1,      /* Synchronous HTTP client call */
    HYP_SVC_ASYNC = 2,     /* Async dispatch (message broker, task queue) */
    HYP_SVC_CONFIG = 3,    /* Config/env accessor */
    HYP_SVC_ROUTE_REG = 4, /* Route registration (router.GET, app.get, Route::post) */
    HYP_SVC_GRPC = 5,      /* gRPC client call (protobuf stub invocation) */
    HYP_SVC_GRAPHQL = 6,   /* GraphQL client query/mutation */
    HYP_SVC_TRPC = 7,      /* tRPC client procedure call */
} hyp_svc_kind_t;
 ...
 * Returns the pattern kind, or HYP_SVC_NONE if no match.
...
/* Per-worker TLS cache for hyp_service_pattern_match results. The
