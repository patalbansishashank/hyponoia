...
void hyp_macro_table_add(HYPMacroTable *t, HYPArena *arena, const char *name, int param_count,
    ...
    if (t->count >= HYP_MACRO_TABLE_CAP || !name)
        return;
    ...
    HYPMacroEntry *e = &t->entries[t->count++];
    e->name = hyp_arena_strdup(arena, name);
    e->param_count = param_count;
    e->expansion = expansion ? hyp_arena_strdup(arena, expansion) : NULL;
    e->resolved_callee = resolved_callee ? hyp_arena_strdup(arena, resolved_callee) : NULL;
    for (int p = 0; p < HYP_MACRO_MAX_PARAMS; p++) {
        e->param_names[p] =
            (param_names && p < param_count) ? hyp_arena_strdup(arena, param_names[p]) : NULL;
    }
}
...
void hyp_parse_inc_file(HYPMacroTable *t, HYPArena *arena, const char *content) {
    ...
    while (*line) {
        ...
        if (strncmp(p, "#define", 7) == 0 && (p[7] == ' ' || p[7] == '\t')) {
            ...
            int param_count = -1;
            char param_names_buf[HYP_MACRO_MAX_PARAMS][64];
            const char *param_name_ptrs[HYP_MACRO_MAX_PARAMS] = {NULL};
            ...
            if (*p == '(') {
                ...
                while (*p && *p != ')' && p < end) {
                    ...
                    while (plen > 0 && (pn_start[plen - 1] == ' ' || pn_start[plen - 1] == '\t'))
                        plen--;
                    if (plen > 0 && param_count < HYP_MACRO_MAX_PARAMS) {
            ...
            char *expansion = NULL;
            if (explen > 0) {
                expansion = hyp_arena_strndup(arena, p, explen);
            }

            hyp_macro_table_add(t, arena, name, param_count,
...
char *hyp_macro_expand(HYPArena *arena, const HYPMacroEntry *entry, const char **args,
    ...
    while (*p && out < (int)sizeof(buf) - 1) {
        if (*p == '%') {
            bool matched = false;
            for (int i = 0; i < entry->param_count && i < HYP_MACRO_MAX_PARAMS; i++) {
    ...
    buf[out] = '\0';
    return hyp_arena_strdup(arena, buf);
}
...
char *hyp_macro_extract_callee(HYPArena *arena, const char *expansion) {
    ...
    if (p) {
        ...
        if (clen <= 0 || mlen <= 0)
            return NULL;
        return hyp_arena_sprintf(arena, "%.*s.%.*s", clen, p, mlen, method_start);
    }
    ...
    if (p && p[2] != '$') {
        ...
        if (*tag_end == '^') {
            ...
            if (tlen > 0 && rlen > 0) {
                return hyp_arena_sprintf(arena, "%.*s^%.*s", tlen, p, rlen, rtn);
            }
        }
    }
