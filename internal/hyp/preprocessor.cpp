...
HYPPreprocessedSource *hyp_preprocess_with_map(const char *source, int source_len,
    ...
    try {
        ...
        HYPPreprocessedSource *pp = (HYPPreprocessedSource *)calloc(1, sizeof(*pp));
        if (!pp) {
            return NULL;
        }
        ...
        if (!pp->source || !pp->original_line_by_expanded_line || !pp->belongs_to_main_file) {
            hyp_preprocessed_source_free(pp);
            return NULL;
        }
...
char *hyp_preprocess(const char *source, int source_len, const char *filename,
    ...
    HYPPreprocessedSource *pp = hyp_preprocess_with_map(source, source_len, filename, extra_defines,
