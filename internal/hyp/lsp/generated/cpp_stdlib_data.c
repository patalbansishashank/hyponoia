...
static void reg_method(HYPTypeRegistry* reg, HYPArena* arena,
    const char* recv_qn, const char* method_name, const HYPType* ret_type) {
    ...
    char* qn = (char*)hyp_arena_alloc(arena, rlen + 1 + mlen + 1);
    if (!qn) return;
    ...
    rf.signature = hyp_type_func(arena, NULL, NULL,
        ret_type ? (const HYPType*[]){ret_type, NULL} : NULL);
    hyp_registry_add_func(reg, rf);
}

// Helper: register a type
static void reg_type(HYPTypeRegistry* reg, const char* qn, const char* short_name) {
...
static void reg_type_with_fields(HYPTypeRegistry* reg, HYPArena* arena,
    const char* qn, const char* short_name,
    const char** fnames, const HYPType** ftypes) {
    HYPRegisteredType rt;
    memset(&rt, 0, sizeof(rt));
    rt.qualified_name = qn;
    rt.short_name = short_name;
...
static void reg_func(HYPTypeRegistry* reg, HYPArena* arena,
    const char* qn, const char* short_name, const HYPType* ret_type) {
    HYPRegisteredFunc rf;
    memset(&rf, 0, sizeof(rf));
    rf.min_params = -1;
    rf.qualified_name = qn;
...
void hyp_cpp_stdlib_register(HYPTypeRegistry* reg, HYPArena* arena) {
    const HYPType* t_void = hyp_type_builtin(arena, "void");
    const HYPType* t_void_ptr = hyp_type_pointer(arena, t_void);
    const HYPType* t_bool = hyp_type_builtin(arena, "bool");
    const HYPType* t_int = hyp_type_builtin(arena, "int");
    const HYPType* t_size_t = hyp_type_builtin(arena, "size_t");
    const HYPType* t_long = hyp_type_builtin(arena, "long");
    const HYPType* t_char = hyp_type_builtin(arena, "char");
    const HYPType* t_char_ptr = hyp_type_pointer(arena, hyp_type_builtin(arena, "const char"));
    ...
    const HYPType* t_string = hyp_type_named(arena, string_qn);
    const HYPType* t_string_ref = hyp_type_reference(arena, t_string);
    ...
    reg_method(reg, arena, string_qn, "front", hyp_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "back", hyp_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "at", hyp_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "operator[]", hyp_type_reference(arena, t_char));
    reg_method(reg, arena, string_qn, "begin", hyp_type_named(arena, "std.string.iterator"));
    reg_method(reg, arena, string_qn, "end", hyp_type_named(arena, "std.string.iterator"));
    ...
    reg_method(reg, arena, sv_qn, "substr", hyp_type_named(arena, sv_qn));
    ...
    const HYPType* t_T = hyp_type_type_param(arena, "T");
    const HYPType* t_T_ref = hyp_type_reference(arena, t_T);
    const HYPType* t_T_ptr = hyp_type_pointer(arena, t_T);
    ...
    reg_method(reg, arena, vec_qn, "begin", hyp_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "end", hyp_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "erase", hyp_type_named(arena, "std.vector.iterator"));
    reg_method(reg, arena, vec_qn, "insert", hyp_type_named(arena, "std.vector.iterator"));
    ...
    const HYPType* t_V = hyp_type_type_param(arena, "V");
    const HYPType* t_V_ref = hyp_type_reference(arena, t_V);

    reg_method(reg, arena, map_qn, "operator[]", t_V_ref);
    reg_method(reg, arena, map_qn, "at", t_V_ref);
    reg_method(reg, arena, map_qn, "find", hyp_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "insert", hyp_type_named(arena, "std.pair"));
    reg_method(reg, arena, map_qn, "emplace", hyp_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, map_qn, "begin", hyp_type_named(arena, "std.map.iterator"));
    reg_method(reg, arena, map_qn, "end", hyp_type_named(arena, "std.map.iterator"));
    ...
    reg_method(reg, arena, umap_qn, "find", hyp_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "insert", hyp_type_named(arena, "std.pair"));
    reg_method(reg, arena, umap_qn, "emplace", hyp_type_named(arena, "std.pair"));
    ...
    reg_method(reg, arena, umap_qn, "begin", hyp_type_named(arena, "std.unordered_map.iterator"));
    reg_method(reg, arena, umap_qn, "end", hyp_type_named(arena, "std.unordered_map.iterator"));
    ...
    reg_method(reg, arena, set_qn, "insert", hyp_type_named(arena, "std.pair"));
    reg_method(reg, arena, set_qn, "find", hyp_type_named(arena, "std.set.iterator"));
    ...
    reg_method(reg, arena, set_qn, "begin", hyp_type_named(arena, "std.set.iterator"));
    reg_method(reg, arena, set_qn, "end", hyp_type_named(arena, "std.set.iterator"));

    const char* uset_qn = "std.unordered_set";
    reg_type(reg, uset_qn, "unordered_set");
    reg_method(reg, arena, uset_qn, "insert", hyp_type_named(arena, "std.pair"));
    reg_method(reg, arena, uset_qn, "find", hyp_type_named(arena, "std.unordered_set.iterator"));
    ...
    reg_method(reg, arena, wptr_qn, "lock", hyp_type_named(arena, sptr_qn));
    ...
    reg_func(reg, arena, "std.make_unique", "make_unique",
        hyp_type_named(arena, uptr_qn));
    reg_func(reg, arena, "std.make_shared", "make_shared",
        hyp_type_named(arena, sptr_qn));
    ...
    const char* pair_qn = "std.pair";
    {
        ...
        const HYPType** pair_field_types = (const HYPType**)hyp_arena_alloc(arena,
            3 * sizeof(const HYPType*));
        pair_field_types[0] = hyp_type_type_param(arena, "T1");
        pair_field_types[1] = hyp_type_type_param(arena, "T2");
    ...
    reg_func(reg, arena, "std.get", "get", hyp_type_unknown());
    ...
    reg_method(reg, arena, "std.function", "operator()", hyp_type_unknown());
    ...
    const HYPType* t_ostream_ref = hyp_type_reference(arena, hyp_type_named(arena, os_qn));
    ...
    const HYPType* t_istream_ref = hyp_type_reference(arena, hyp_type_named(arena, is_qn));
    ...
    reg_func(reg, arena, "std.cout", "cout", hyp_type_named(arena, os_qn));
    reg_func(reg, arena, "std.cin", "cin", hyp_type_named(arena, is_qn));
    reg_func(reg, arena, "std.cerr", "cerr", hyp_type_named(arena, os_qn));
    reg_func(reg, arena, "std.clog", "clog", hyp_type_named(arena, os_qn));
    ...
    const HYPType* t_path = hyp_type_named(arena, fspath_qn);
    ...
    reg_method(reg, arena, fspath_qn, "operator/=", hyp_type_reference(arena, t_path));
    ...
    reg_method(reg, arena, thread_qn, "get_id", hyp_type_named(arena, "std.thread.id"));
    ...
    reg_func(reg, arena, "std.chrono.system_clock.now", "now",
        hyp_type_named(arena, "std.chrono.time_point"));
    ...
    reg_func(reg, arena, "std.find", "find", hyp_type_unknown());
    reg_func(reg, arena, "std.find_if", "find_if", hyp_type_unknown());
    reg_func(reg, arena, "std.for_each", "for_each", hyp_type_unknown());
    reg_func(reg, arena, "std.transform", "transform", hyp_type_unknown());
    reg_func(reg, arena, "std.copy", "copy", hyp_type_unknown());
    reg_func(reg, arena, "std.move", "move", hyp_type_unknown());
    reg_func(reg, arena, "std.swap", "swap", t_void);
    reg_func(reg, arena, "std.min", "min", hyp_type_unknown());
    reg_func(reg, arena, "std.max", "max", hyp_type_unknown());
    reg_func(reg, arena, "std.accumulate", "accumulate", hyp_type_unknown());
    reg_func(reg, arena, "std.count", "count", t_size_t);
    reg_func(reg, arena, "std.count_if", "count_if", t_size_t);
    reg_func(reg, arena, "std.remove", "remove", hyp_type_unknown());
    reg_func(reg, arena, "std.remove_if", "remove_if", hyp_type_unknown());
    reg_func(reg, arena, "std.reverse", "reverse", t_void);
    reg_func(reg, arena, "std.unique", "unique", hyp_type_unknown());
    reg_func(reg, arena, "std.lower_bound", "lower_bound", hyp_type_unknown());
    reg_func(reg, arena, "std.upper_bound", "upper_bound", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.stof", "stof", hyp_type_builtin(arena, "float"));
    reg_func(reg, arena, "std.stod", "stod", hyp_type_builtin(arena, "double"));
    ...
    reg_method(reg, arena, "std.any", "type", hyp_type_unknown()); // type_info
    reg_method(reg, arena, "std.any", "reset", t_void);
    reg_func(reg, arena, "std.any_cast", "any_cast", t_T);
    reg_func(reg, arena, "std.make_any", "make_any", hyp_type_named(arena, "std.any"));
    ...
    reg_method(reg, arena, "std.smatch", "prefix", hyp_type_unknown());
    reg_method(reg, arena, "std.smatch", "suffix", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.partial_sum", "partial_sum", hyp_type_unknown());
    reg_func(reg, arena, "std.adjacent_difference", "adjacent_difference", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.distance", "distance", hyp_type_builtin(arena, "ptrdiff_t"));
    reg_func(reg, arena, "std.next", "next", hyp_type_unknown()); // iterator
    reg_func(reg, arena, "std.prev", "prev", hyp_type_unknown());
    reg_func(reg, arena, "std.begin", "begin", hyp_type_unknown());
    reg_func(reg, arena, "std.end", "end", hyp_type_unknown());
    reg_func(reg, arena, "std.rbegin", "rbegin", hyp_type_unknown());
    reg_func(reg, arena, "std.rend", "rend", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.min_element", "min_element", hyp_type_unknown());
    reg_func(reg, arena, "std.max_element", "max_element", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.partition", "partition", hyp_type_unknown());
    reg_func(reg, arena, "std.stable_partition", "stable_partition", hyp_type_unknown());
    ...
    reg_func(reg, arena, "std.merge", "merge", hyp_type_unknown());
    reg_func(reg, arena, "std.includes", "includes", t_bool);
    reg_func(reg, arena, "std.set_union", "set_union", hyp_type_unknown());
    reg_func(reg, arena, "std.set_intersection", "set_intersection", hyp_type_unknown());
    reg_func(reg, arena, "std.set_difference", "set_difference", hyp_type_unknown());
    reg_func(reg, arena, "std.generate", "generate", t_void);
    reg_func(reg, arena, "std.generate_n", "generate_n", hyp_type_unknown());
    ...
    reg_method(reg, arena, "std.bitset", "set", hyp_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "reset", hyp_type_named(arena, "std.bitset"));
    reg_method(reg, arena, "std.bitset", "flip", hyp_type_named(arena, "std.bitset"));
    ...
    reg_method(reg, arena, "std.bitset", "to_ulong", hyp_type_builtin(arena, "unsigned long"));
    ...
    reg_func(reg, arena, "std.filesystem.file_size", "file_size", hyp_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.remove_all", "remove_all", hyp_type_builtin(arena, "uintmax_t"));
    ...
    reg_func(reg, arena, "std.filesystem.current_path", "current_path",
        hyp_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "path", hyp_type_named(arena, fspath_qn));
    ...
    reg_method(reg, arena, direntry_qn, "file_size", hyp_type_builtin(arena, "uintmax_t"));

    // =========================================================================
    // std.chrono — additional
    // =========================================================================
    reg_type(reg, "std.chrono.time_point", "time_point");
    reg_method(reg, arena, "std.chrono.time_point", "time_since_epoch", hyp_type_unknown());
    reg_method(reg, arena, "std.chrono.system_clock", "now",
        hyp_type_named(arena, "std.chrono.time_point"));
    reg_method(reg, arena, "std.chrono.steady_clock", "now",
        hyp_type_named(arena, "std.chrono.time_point"));
    reg_func(reg, arena, "std.chrono.duration_cast", "duration_cast", hyp_type_unknown());
    ...
    reg_method(reg, arena, bfspath_qn, "parent_path", hyp_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "filename", hyp_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "extension", hyp_type_named(arena, bfspath_qn));
    reg_method(reg, arena, bfspath_qn, "stem", hyp_type_named(arena, bfspath_qn));
    ...
    reg_method(reg, arena, absv_qn, "data", hyp_type_pointer(arena, hyp_type_builtin(arena, "char")));
    ...
    reg_method(reg, arena, absv_qn, "substr", hyp_type_named(arena, absv_qn));

    const char* abstat_qn = "absl.Status";
    reg_type(reg, abstat_qn, "Status");
    reg_method(reg, arena, abstat_qn, "ok", t_bool);
    reg_method(reg, arena, abstat_qn, "code", hyp_type_unknown());
    reg_method(reg, arena, abstat_qn, "message", hyp_type_named(arena, absv_qn));
    reg_method(reg, arena, abstat_qn, "ToString", t_string);
    reg_func(reg, arena, "absl.OkStatus", "OkStatus", hyp_type_named(arena, abstat_qn));

    const char* absor_qn = "absl.StatusOr";
    reg_type(reg, absor_qn, "StatusOr");
    reg_method(reg, arena, absor_qn, "ok", t_bool);
    reg_method(reg, arena, absor_qn, "status", hyp_type_named(arena, abstat_qn));
    ...
    reg_method(reg, arena, abfhm_qn, "find", hyp_type_unknown());
    ...
    reg_method(reg, arena, abfhm_qn, "insert", hyp_type_unknown());
    ...
    reg_method(reg, arena, abfhs_qn, "insert", hyp_type_unknown());
    ...
    reg_method(reg, arena, abspan_qn, "subspan", hyp_type_named(arena, abspan_qn));
    ...
    reg_func(reg, arena, "absl.StrSplit", "StrSplit",
        hyp_type_named(arena, "std.vector"));
    ...
    reg_method(reg, arena, grpc_status_qn, "error_code", hyp_type_unknown());
    reg_method(reg, arena, grpc_status_qn, "error_message", t_string);
    reg_func(reg, arena, "grpc.Status.OK", "OK", hyp_type_named(arena, grpc_status_qn));

    const char* grpc_sb_qn = "grpc.ServerBuilder";
    reg_type(reg, grpc_sb_qn, "ServerBuilder");
    reg_method(reg, arena, grpc_sb_qn, "AddListeningPort", hyp_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "RegisterService", hyp_type_named(arena, grpc_sb_qn));
    reg_method(reg, arena, grpc_sb_qn, "BuildAndStart", hyp_type_unknown());

    const char* grpc_chan_qn = "grpc.Channel";
    reg_type(reg, grpc_chan_qn, "Channel");
    reg_func(reg, arena, "grpc.CreateChannel", "CreateChannel",
        hyp_type_named(arena, grpc_chan_qn));
    ...
    reg_func(reg, arena, "spdlog.get", "get",
        hyp_type_named(arena, spdlog_qn));
    ...
    reg_method(reg, arena, qobj_qn, "parent", hyp_type_pointer(arena, hyp_type_named(arena, qobj_qn)));
    reg_method(reg, arena, qobj_qn, "children", hyp_type_unknown());
    reg_method(reg, arena, qobj_qn, "objectName", hyp_type_named(arena, "QString"));
    ...
    reg_method(reg, arena, qstr_qn, "toUtf8", hyp_type_unknown());
    reg_method(reg, arena, qstr_qn, "toLatin1", hyp_type_unknown());
    ...
    reg_method(reg, arena, qstr_qn, "trimmed", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "simplified", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toLower", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toUpper", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "arg", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "split", hyp_type_unknown());
    reg_method(reg, arena, qstr_qn, "replace", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "mid", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "left", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "right", hyp_type_named(arena, qstr_qn));
    reg_method(reg, arena, qstr_qn, "toInt", t_int);
    reg_method(reg, arena, qstr_qn, "toDouble", hyp_type_builtin(arena, "double"));
    reg_func(reg, arena, "QString.number", "number", hyp_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromStdString", "fromStdString", hyp_type_named(arena, qstr_qn));
    reg_func(reg, arena, "QString.fromUtf8", "fromUtf8", hyp_type_named(arena, qstr_qn));
