...
#define REG_FUNC(qn, short, ret_type) do { \
    ...
    rf.short_name = (short); \
    rf.signature = hyp_type_func(arena, NULL, NULL, (const HYPType*[]){(ret_type), NULL}); \
...
void hyp_c_stdlib_register(HYPTypeRegistry* reg, HYPArena* arena) {
    ...
    const HYPType* t_double = hyp_type_builtin(arena, "double");
    const HYPType* t_void = hyp_type_builtin(arena, "void");
    const HYPType* t_char_ptr = hyp_type_pointer(arena, hyp_type_builtin(arena, "char"));
    const HYPType* t_void_ptr = hyp_type_pointer(arena, t_void);

    // FILE type
    REG_TYPE("FILE", "FILE");
    const HYPType* t_file_ptr = hyp_type_pointer(arena, hyp_type_named(arena, "FILE"));
    ...
    REG_FUNC("ftell", "ftell", hyp_type_builtin(arena, "long"));
    ...
    REG_FUNC("atol", "atol", hyp_type_builtin(arena, "long"));
    REG_FUNC("strtol", "strtol", hyp_type_builtin(arena, "long"));
    ...
    REG_FUNC("time", "time", hyp_type_builtin(arena, "time_t"));
    REG_FUNC("clock", "clock", hyp_type_builtin(arena, "clock_t"));
