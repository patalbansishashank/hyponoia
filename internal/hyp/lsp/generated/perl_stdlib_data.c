 ...
 *
 * Return types are left UNKNOWN (hyp_type_unknown) for v1: real signature
...
#define REG_FUNC(module_dot_, name_, ret_type_)                                                 \
    do {                                                                                        \
        memset(&rf, 0, sizeof(rf));                                                             \
        rf.min_params = -1;                                                                     \
        rf.qualified_name = hyp_arena_sprintf(arena, "%s.%s", (module_dot_), (name_));          \
        rf.short_name = (name_);                                                                \
        {                                                                                       \
            const HYPType **rets = (const HYPType **)hyp_arena_alloc(arena, 2 * sizeof(*rets)); \
            rets[0] = (ret_type_);                                                              \
...
void hyp_perl_stdlib_register(HYPTypeRegistry *reg, HYPArena *arena) {
    ...
    REG_BUILTIN("sprintf", hyp_type_builtin(arena, "string"));
    REG_BUILTIN("open", MIXED);
    REG_BUILTIN("close", MIXED);
    REG_BUILTIN("push", hyp_type_builtin(arena, "int"));
    REG_BUILTIN("pop", MIXED);
    REG_BUILTIN("shift", MIXED);
    REG_BUILTIN("unshift", hyp_type_builtin(arena, "int"));
    REG_BUILTIN("map", MIXED);
    REG_BUILTIN("grep", MIXED);
    REG_BUILTIN("sort", MIXED);
    REG_BUILTIN("join", hyp_type_builtin(arena, "string"));
    REG_BUILTIN("split", MIXED);
    REG_BUILTIN("length", hyp_type_builtin(arena, "int"));
    REG_BUILTIN("substr", hyp_type_builtin(arena, "string"));
    REG_BUILTIN("chomp", MIXED);
    REG_BUILTIN("chop", MIXED);
    REG_BUILTIN("die", MIXED);
    REG_BUILTIN("warn", MIXED);
    REG_BUILTIN("ref", hyp_type_builtin(arena, "string"));
    REG_BUILTIN("bless", MIXED);
    REG_BUILTIN("defined", hyp_type_builtin(arena, "bool"));
    REG_BUILTIN("exists", hyp_type_builtin(arena, "bool"));
    ...
    REG_FUNC("Scalar.Util", "reftype", hyp_type_builtin(arena, "string"));
    ...
    REG_FUNC("POSIX", "strftime", hyp_type_builtin(arena, "string"));
    REG_FUNC("POSIX", "INT_MAX", hyp_type_builtin(arena, "int"));

    /* ── Storable ───────────────────────────────────────────────────
     * Source: RESEARCH.md L367. Module QN "Storable". */
    REG_FUNC("Storable", "dclone", MIXED);
    REG_FUNC("Storable", "freeze", hyp_type_builtin(arena, "string"));
    ...
    REG_FUNC("Data.Dumper", "Dumper", hyp_type_builtin(arena, "string"));
}
