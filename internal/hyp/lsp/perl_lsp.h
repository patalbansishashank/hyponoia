#ifndef HYP_LSP_PERL_LSP_H
#define HYP_LSP_PERL_LSP_H
...
/* Evaluate a Perl expression's type. May return NULL / HYP_TYPE_UNKNOWN. */
 ...
 * Called from hyp_extract_file() via the language dispatch in hyp.c. */
 ...
 *
 * Mirrors hyp_run_php_lsp_cross / hyp_run_py_lsp_cross. Stub-declared here so
 * a later plan (Phase 23, cross-file) can implement it without touching the
 * wiring. Caller supplies the combined HYPLSPDef[] (file-local + cross-file)
 * and a resolved import map (use → target QN). */
void hyp_run_perl_lsp_cross(HYPArena *arena, const char *source, int source_len,
                            const char *module_qn, HYPLSPDef *defs, int def_count,
                            const char **import_names, const char **import_qns, int import_count,
                            TSTree *cached_tree, /* NULL = parse internally */
                            HYPResolvedCallArray *out);

#endif /* HYP_LSP_PERL_LSP_H */
