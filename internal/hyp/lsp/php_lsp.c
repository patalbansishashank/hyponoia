 ...
 * shape:
 *   1. Build a HYPTypeRegistry from file-local definitions + stdlib +
...
static const HYPType *php_substitute_template(HYPArena *arena, const HYPType *t,
                                              const char *const *param_names,
                                              const HYPType *const *args);
static const HYPType *eval_function_call_type(PHPLSPContext *ctx, TSNode call_node);
static const HYPType *eval_member_call_type(PHPLSPContext *ctx, TSNode call_node);
static const HYPType *eval_object_creation_type(PHPLSPContext *ctx, TSNode node);
...
static const HYPType *resolve_phpdoc_type(PHPLSPContext *ctx, const char *type_text);
...
static char *php_ns_to_dot(HYPArena *a, const char *ns) {
    if (!ns)
        return NULL;
    size_t n = strlen(ns);
    char *out = (char *)hyp_arena_alloc(a, n + 1);
    if (!out)
        return NULL;
...
void php_lsp_add_use(PHPLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->use_count >= ctx->use_cap) {
        ...
        const char **nl = (const char **)hyp_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nl));
        const char **nq = (const char **)hyp_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(*nq));
        int *nk = (int *)hyp_arena_alloc(ctx->arena, (size_t)new_cap * sizeof(int));
        if (!nl || !nq || !nk)
            return;
    ...
    ctx->use_target_qns[ctx->use_count] = hyp_arena_strdup(ctx->arena, target_qn);
...
const char *php_resolve_class_name(PHPLSPContext *ctx, const char *name) {
    ...
    char *first;
    if (*sep) {
        first = hyp_arena_strndup(ctx->arena, name, (size_t)(sep - name));
    } else {
        first = hyp_arena_strdup(ctx->arena, name);
    }

    const char *use_target = find_use(ctx, first, HYP_PHP_USE_CLASS);
    if (use_target) {
        if (*sep) {
            /* use App\\Models as M;  M\\User -> App.Models.User */
            return hyp_arena_sprintf(ctx->arena, "%s%s", use_target,
    ...
    if (ctx->module_qn && ctx->module_qn[0]) {
        return hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn,
...
static const HYPRegisteredType *lookup_type_with_project(PHPLSPContext *ctx, const char *qn) {
    if (!qn)
        return NULL;
    const HYPRegisteredType *t = hyp_registry_lookup_type(ctx->registry, qn);
    if (t)
        return t;

    if (ctx->module_qn && ctx->module_qn[0]) {
        const char *combo = hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, qn);
        t = hyp_registry_lookup_type(ctx->registry, combo);
        if (t)
            return t;
        ...
        while (last_dot) {
            char *prefix = hyp_arena_strndup(ctx->arena, m, (size_t)(last_dot - m));
            const char *try_qn = hyp_arena_sprintf(ctx->arena, "%s.%s", prefix, qn);
            t = hyp_registry_lookup_type(ctx->registry, try_qn);
            if (t)
                return t;
    ...
    const HYPRegisteredType *best = NULL;
    int best_score = -1;
    for (int i = 0; ctx->registry && i < ctx->registry->type_count; i++) {
        const HYPRegisteredType *cand = &ctx->registry->types[i];
...
const HYPRegisteredFunc *php_lookup_method(PHPLSPContext *ctx, const char *class_qn,
    ...
    const HYPRegisteredFunc *f = hyp_registry_lookup_method(ctx->registry, class_qn, method_name);
    if (f)
        return f;

    /* If the QN we have isn't the registry's canonical key, resolve the
     * type to its registered identity and retry. */
    const HYPRegisteredType *t = hyp_registry_lookup_type(ctx->registry, class_qn);
    if (!t)
        t = lookup_type_with_project(ctx, class_qn);
    if (!t)
        return NULL;
    if (strcmp(t->qualified_name, class_qn) != 0) {
        f = hyp_registry_lookup_method(ctx->registry, t->qualified_name, method_name);
        if (f)
            return f;
    }
    ...
    while (frontier_count > 0 && visited_count < 32) {
        ...
        f = hyp_registry_lookup_method(ctx->registry, parent, method_name);
        if (f)
            return f;

        const HYPRegisteredType *next = hyp_registry_lookup_type(ctx->registry, parent);
        if (!next)
        ...
        if (strcmp(next->qualified_name, parent) != 0) {
            f = hyp_registry_lookup_method(ctx->registry, next->qualified_name, method_name);
            if (f)
                return f;
        }
        if (next->method_qns && next->method_names) {
            for (int i = 0; next->method_names[i]; i++) {
                if (strcmp(next->method_names[i], method_name) == 0) {
                    const HYPRegisteredFunc *cand =
                        hyp_registry_lookup_func(ctx->registry, next->method_qns[i]);
                    if (cand)
                        return cand;
                }
            }
...
const HYPType *php_parse_type_node(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "primitive_type") == 0) {
        ...
        return hyp_type_builtin(ctx->arena, t ? t : "mixed");
    }
    ...
    if (strcmp(kind, "union_type") == 0 || strcmp(kind, "intersection_type") == 0 ||
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (ts_node_is_null(c) || !ts_node_is_named(c))
                continue;
            const HYPType *t = php_parse_type_node(ctx, c);
            if (t && t->kind != HYP_TYPE_UNKNOWN) {
                if (t->kind == HYP_TYPE_BUILTIN && strcmp(t->data.builtin.name, "null") == 0) {
                    continue;
                }
                return t;
            }
    ...
    if (php_is_builtin_type_name(t))
        return hyp_type_builtin(ctx->arena, t);
    const char *resolved = php_resolve_class_name(ctx, t);
    if (!resolved)
        return hyp_type_unknown();
    return hyp_type_named(ctx->arena, resolved);
...
static char *phpdoc_clean(HYPArena *a, const char *raw) {
    if (!raw)
        return NULL;
    size_t n = strlen(raw);
    char *out = (char *)hyp_arena_alloc(a, n + 1);
...
static int parse_phpdoc_template_args(PHPLSPContext *ctx, const char *p, const HYPType ***out_args,
    ...
    while (*p && depth > 0) {
        ...
        if (arg_len > 0) {
            char *arg_text = hyp_arena_strndup(ctx->arena, arg_start, arg_len);
            const HYPType *arg_type = resolve_phpdoc_type(ctx, arg_text);
            if (count >= cap) {
                int new_cap = cap * 2;
                const HYPType **new_buf = (const HYPType **)hyp_arena_alloc(
...
static void php_add_phpstan_alias(PHPLSPContext *ctx, const char *name, const HYPType *type) {
    if (!ctx || !name || !type)
        return;
    ...
    if (ctx->phpstan_alias_count >= ctx->phpstan_alias_cap) {
        ...
        const char **nn =
            (const char **)hyp_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const HYPType **nt =
            (const HYPType **)hyp_arena_alloc(ctx->arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < ctx->phpstan_alias_count; i++) {
    ...
    ctx->phpstan_alias_names[ctx->phpstan_alias_count] = hyp_arena_strdup(ctx->arena, name);
    ctx->phpstan_alias_types[ctx->phpstan_alias_count] = type;
    ctx->phpstan_alias_count++;
}

static const HYPType *php_lookup_phpstan_alias(PHPLSPContext *ctx, const char *name) {
    if (!ctx || !name)
        return NULL;
    for (int i = 0; i < ctx->phpstan_alias_count; i++) {
        if (strcmp(ctx->phpstan_alias_names[i], name) == 0) {
...
static const HYPType *resolve_phpdoc_type(PHPLSPContext *ctx, const char *type_text) {
    ...
    if (end == 0)
        return hyp_type_unknown();
    char *first = hyp_arena_strndup(ctx->arena, type_text, end);
    if (!first)
        return hyp_type_unknown();
    ...
    if (has_array_shape)
        return hyp_type_builtin(ctx->arena, "array");

    /* @phpstan-type alias takes precedence over name resolution. */
    {
        const HYPType *aliased = php_lookup_phpstan_alias(ctx, first);
        if (aliased)
            return aliased;
    }
    ...
    if (php_is_builtin_type_name(first)) {
        if (has_generic) {
             ...
             * later. */
            const HYPType **args = NULL;
            const char *after = NULL;
            int n = parse_phpdoc_template_args(ctx, type_text + end + 1, &args, &after);
            return hyp_type_template(ctx->arena, first, args, n);
        }
        return hyp_type_builtin(ctx->arena, first);
    }
    ...
    if (has_generic) {
        ...
        const char *after = NULL;
        int n = parse_phpdoc_template_args(ctx, type_text + end + 1, &args, &after);
        return hyp_type_template(ctx->arena, resolved, args, n);
    }
    return hyp_type_named(ctx->arena, resolved);
}
...
static void parse_phpdoc_for_params(PHPLSPContext *ctx, const char *docstring, TSNode params) {
    ...
    while ((p = strstr(p, "@param")) != NULL) {
        ...
        if (p == type_start)
            continue;
        char *type_text = hyp_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
        while (*p == ' ' || *p == '\t')
            p++;
        if (*p != '$')
            continue;
        ...
        if (p == name_start)
            continue;
        char *vname = hyp_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const HYPType *t = resolve_phpdoc_type(ctx, type_text);
        if (vname && t && t->kind != HYP_TYPE_UNKNOWN) {
            hyp_scope_bind(ctx->current_scope, vname, t);
        }
    }
}
...
const HYPType *php_eval_expr_type(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "variable_name") == 0) {
        ...
        if (t) {
            ...
            if (strcmp(name, "this") == 0) {
                ...
                const HYPType *bound = hyp_scope_lookup(ctx->current_scope, name);
                if (bound)
                    result = bound;
            }
        }
        ...
        result = hyp_type_named(ctx->arena, "Closure");
        ...
        if (!ts_node_is_null(lhs) && !ts_node_is_null(rhs)) {
            ...
            if (cls_text && member) {
                ...
                if (cls_qn) {
                    const HYPRegisteredType *t = hyp_registry_lookup_type(ctx->registry, cls_qn);
                    if (!t)
        ...
        if (!ts_node_is_null(obj)) {
            const HYPType *recv = php_eval_expr_type(ctx, obj);
            if (recv && recv->kind == HYP_TYPE_TEMPLATE) {
                const HYPType *const *args = recv->data.template_type.template_args;
                if (args) {
                    int n = 0;
                    while (args[n])
                        n++;
            ...
            } else if (recv && recv->kind == HYP_TYPE_NAMED) {
                /* offsetGet on a registered ArrayAccess-like class. */
                const HYPRegisteredFunc *og =
                    php_lookup_method(ctx, recv->data.named.qualified_name, "offsetGet");
                if (og && og->signature && og->signature->kind == HYP_TYPE_FUNC &&
        ...
        for (uint32_t i = 0; i < nc; i++) {
            ...
            if (strcmp(k, "match_block") == 0) {
                ...
                for (uint32_t j = 0; j < bnc; j++) {
                    ...
                    if (ts_node_is_null(val))
                        continue;
                    const HYPType *t = php_eval_expr_type(ctx, val);
                    if (t && t->kind != HYP_TYPE_UNKNOWN) {
                        result = t;
                        goto match_done;
                    }
    ...
    match_done:;
        ...
        if (!ts_node_is_null(then_n)) {
            const HYPType *t = php_eval_expr_type(ctx, then_n);
            if (t && t->kind != HYP_TYPE_UNKNOWN) {
                result = t;
            }
        }
        if ((!result || result->kind == HYP_TYPE_UNKNOWN) && !ts_node_is_null(else_n)) {
            const HYPType *t = php_eval_expr_type(ctx, else_n);
            if (t && t->kind != HYP_TYPE_UNKNOWN)
                result = t;
        }
        ...
        result = hyp_type_builtin(ctx->arena, "float");
               ...
               strcmp(kind, "false") == 0) {
        result = hyp_type_builtin(ctx->arena, "bool");
        ...
        if (!ts_node_is_null(obj) && !ts_node_is_null(field)) {
            ...
            if (recv && recv->kind == HYP_TYPE_NAMED) {
                ...
                const HYPRegisteredType *t =
                    hyp_registry_lookup_type(ctx->registry, recv->data.named.qualified_name);
                if (!t)
...
static const HYPType *eval_function_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    const char *target = find_use(ctx, name, HYP_PHP_USE_FUNCTION);
    const char *qn = NULL;
    if (target) {
        qn = target;
    } else if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        qn = hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
    } else {
        qn = hyp_arena_strdup(ctx->arena, name);
    }
    const HYPRegisteredFunc *f = hyp_registry_lookup_func(ctx->registry, qn);
    if (!f && ctx->module_qn) {
        f = hyp_registry_lookup_func(ctx->registry,
    ...
    if (!f && ctx->registry) {
        ...
        for (int i = 0; i < ctx->registry->func_count; i++) {
            const HYPRegisteredFunc *cand = &ctx->registry->funcs[i];
    ...
    const HYPType *sig = f->signature;
    if (sig->kind == HYP_TYPE_FUNC && sig->data.func.return_types &&
...
static const HYPType *eval_member_call_type(PHPLSPContext *ctx, TSNode call_node) {
    ...
    if (is_static) {
        ...
        const HYPType *recv_type =
            ts_node_is_null(recv_node) ? hyp_type_unknown() : php_eval_expr_type(ctx, recv_node);
        if (recv_type && recv_type->kind == HYP_TYPE_NAMED) {
            class_qn = recv_type->data.named.qualified_name;
        } else if (recv_type && recv_type->kind == HYP_TYPE_TEMPLATE) {
        ...
        if (recv_type && recv_type->kind == HYP_TYPE_TEMPLATE) {
            ...
            const HYPRegisteredFunc *f = php_lookup_method(ctx, class_qn, method_name);
            if (!f || !f->signature)
                return hyp_type_unknown();
            const HYPType *sig = f->signature;
            if (sig->kind == HYP_TYPE_FUNC && sig->data.func.return_types &&
...
static void emit_resolved_kind(PHPLSPContext *ctx, const char *callee_qn, const char *strategy,
                               float confidence, HYPResolvedKind kind) {
...
static void process_assignment(PHPLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(name, "this") == 0)
        return;
    const HYPType *rhs_type = php_eval_expr_type(ctx, rhs);
    if (rhs_type && rhs_type->kind != HYP_TYPE_UNKNOWN) {
        /* Don't override a more-specific (NAMED/TEMPLATE) binding with
         * a trivial null literal — preserves `@var T $x; $x = null;`. */
        if (rhs_type->kind == HYP_TYPE_BUILTIN && rhs_type->data.builtin.name &&
            strcmp(rhs_type->data.builtin.name, "null") == 0) {
            const HYPType *existing = hyp_scope_lookup(ctx->current_scope, name);
            if (existing &&
                (existing->kind == HYP_TYPE_NAMED || existing->kind == HYP_TYPE_TEMPLATE)) {
                return;
            }
        }
        hyp_scope_bind(ctx->current_scope, name, rhs_type);
    }
}
...
static void process_foreach(PHPLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(iterable)) {
        const HYPType *it_type = php_eval_expr_type(ctx, iterable);
        if (it_type && it_type->kind == HYP_TYPE_TEMPLATE) {
            const HYPType *const *args = it_type->data.template_type.template_args;
            if (args) {
                int n = 0;
                while (args[n])
                    n++;
        ...
        } else if (it_type && it_type->kind == HYP_TYPE_NAMED) {
            const HYPRegisteredFunc *cur =
                php_lookup_method(ctx, it_type->data.named.qualified_name, "current");
            if (cur && cur->signature && cur->signature->kind == HYP_TYPE_FUNC &&
                cur->signature->data.func.return_types &&
                cur->signature->data.func.return_types[0] &&
                cur->signature->data.func.return_types[0]->kind != HYP_TYPE_UNKNOWN) {
    ...
    for (int i = 0; i < loop_var_count; i++) {
        ...
        if (!t)
            continue;
        const char *name = (t[0] == '$') ? t + 1 : t;
        const HYPType *bind_type = (i == loop_var_count - 1) ? elem_type : hyp_type_unknown();
        hyp_scope_bind(ctx->current_scope, name, bind_type);
    }
}
...
static void process_catch(PHPLSPContext *ctx, TSNode node) {
    ...
    const HYPType *exc_type = hyp_type_unknown();
    ...
    hyp_scope_bind(ctx->current_scope, vname, exc_type);
}

/* ── call resolution ────────────────────────────────────────────── */

static void resolve_function_call(PHPLSPContext *ctx, TSNode call, HYPResolvedKind kind) {
    ...
    if (kind == HYP_RESOLVED_CALL_REFERENCE && strcmp(ts_node_type(fn), "variable_name") == 0) {
        const HYPType *receiver = php_eval_expr_type(ctx, fn);
        const char *class_qn = NULL;
        if (receiver && receiver->kind == HYP_TYPE_NAMED) {
            class_qn = receiver->data.named.qualified_name;
        } else if (receiver && receiver->kind == HYP_TYPE_TEMPLATE) {
            class_qn = receiver->data.template_type.template_name;
        }
        const HYPRegisteredFunc *invoke =
    ...
    const char *target = find_use(ctx, name, HYP_PHP_USE_FUNCTION);
    if (target) {
        const HYPRegisteredFunc *f = hyp_registry_lookup_func(ctx->registry, target);
        if (f) {
    ...
    if (ctx->current_namespace_qn && ctx->current_namespace_qn[0]) {
        const char *ns_qn = hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->current_namespace_qn, name);
        const HYPRegisteredFunc *f = hyp_registry_lookup_func(ctx->registry, ns_qn);
        if (f) {
            emit_resolved_kind(ctx, f->qualified_name, "php_function_namespaced", 0.95f, kind);
            return;
        }
    ...
    const HYPRegisteredFunc *best = NULL;
    int best_score = -1;
...
static void resolve_member_call(PHPLSPContext *ctx, TSNode call, HYPResolvedKind kind) {
    TSNode obj = ts_node_child_by_field_name(call, "object", 6);
    TSNode name_node = ts_node_child_by_field_name(call, "name", 4);
    if (ts_node_is_null(name_node))
        return;
    if (kind == HYP_RESOLVED_CALL_REFERENCE &&
    ...
    if (class_has_magic_call(ctx, class_qn, false)) {
        emit_resolved_reason_kind(ctx, hyp_arena_sprintf(ctx->arena, "%s.__call", class_qn),
    ...
    emit_resolved_kind(ctx, hyp_arena_sprintf(ctx->arena, "%s.%s", class_qn, method_name),
                       "php_method_typed_unindexed", 0.55f, kind);
}

static void resolve_static_call(PHPLSPContext *ctx, TSNode call, HYPResolvedKind kind) {
    ...
    const HYPRegisteredFunc *magic_static = php_lookup_method(ctx, class_qn, "__callStatic");
    if (magic_static && kind == HYP_RESOLVED_CALL_REFERENCE) {
...
typedef struct {
    ...
    const HYPType *type;  /* the narrowed type */
} php_narrowing_t;
...
static int parse_narrowing_one(PHPLSPContext *ctx, TSNode cond, php_narrowing_t *out) {
    ...
    if (strcmp(kind, "binary_expression") == 0) {
        ...
        if (opt && strcmp(opt, "instanceof") == 0) {
            ...
            if (!resolved)
                return false;
            out->var_name = hyp_arena_strdup(ctx->arena, vname);
            out->type = hyp_type_named(ctx->arena, resolved);
            return true;
        }
    ...
    if (strcmp(kind, "function_call_expression") == 0) {
        ...
        for (uint32_t i = 0; i < anc; i++) {
            ...
            const char *vname = (vt[0] == '$') ? vt + 1 : vt;
            out->var_name = hyp_arena_strdup(ctx->arena, vname);
            out->type = hyp_type_builtin(ctx->arena, builtin);
            return true;
        }
    }
    return false;
...
static void apply_assert_narrowing(PHPLSPContext *ctx, TSNode call) {
    ...
    for (uint32_t i = 0; i < anc; i++) {
        ...
        if (parse_narrowing(ctx, predicate, &nw) && nw.var_name && nw.type) {
            hyp_scope_bind(ctx->current_scope, nw.var_name, nw.type);
        }
        return;
    }
}
...
static void walk_with_narrowings(PHPLSPContext *ctx, TSNode body, const php_narrowing_t *nws,
    ...
    for (int i = 0; i < nw_count; i++) {
        if (nws[i].var_name && nws[i].type) {
            hyp_scope_bind(ctx->current_scope, nws[i].var_name, nws[i].type);
        }
    }
    php_resolve_calls_in_node(ctx, body);
...
static void process_if_statement(PHPLSPContext *ctx, TSNode node) {
    ...
    if (body_is_terminator && has_neg_nw && neg_nw.var_name && neg_nw.type) {
        hyp_scope_bind(ctx->current_scope, neg_nw.var_name, neg_nw.type);
    }
}
...
static void php_resolve_calls_in_node_inner(PHPLSPContext *ctx, TSNode node) {
    ...
    HYPResolvedKind resolved_kind =
        is_callable_reference ? HYP_RESOLVED_CALL_REFERENCE : HYP_RESOLVED_INVOCATION;
    if (strcmp(kind, "function_call_expression") == 0) {
            ...
            if (mn && strcmp(mn, "bindTo") == 0 &&
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    for (uint32_t i = 0; i < anc; i++) {
                        ...
                        const HYPType *t = php_eval_expr_type(ctx, v);
                ...
                if (!ts_node_is_null(args)) {
                    ...
                    if (!ts_node_is_null(closure_arg) && !ts_node_is_null(obj_arg)) {
                        ...
                        if (strcmp(ck, "anonymous_function") == 0 ||
                            ...
                            const HYPType *t = php_eval_expr_type(ctx, obj_arg);
                            if (t && t->kind == HYP_TYPE_NAMED) {
                                const char *saved = ctx->enclosing_class_qn;
                                ctx->enclosing_class_qn = t->data.named.qualified_name;
...
static void bind_typed_parameters(PHPLSPContext *ctx, TSNode params) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const HYPType *ptype = hyp_type_unknown();
        if (!ts_node_is_null(tnode))
        ...
        if (strcmp(pk, "variadic_parameter") == 0) {
            ptype = hyp_type_builtin(ctx->arena, "array");
        }
        hyp_scope_bind(ctx->current_scope, name, ptype);
    }
}
            ...
            if (fname) {
                if (is_method && ctx->enclosing_class_qn) {
                    ctx->enclosing_func_qn =
                        hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->enclosing_class_qn, fname);
                } else if (ctx->module_qn) {
                    ctx->enclosing_func_qn =
                        hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, fname);
                } else {
                    ctx->enclosing_func_qn = hyp_arena_strdup(ctx->arena, fname);
                }
            }
        }
    }
    ...
    if (strcmp(kind, "anonymous_function") == 0) {
        ...
        for (uint32_t i = 0; i < fnc; i++) {
            ...
            for (uint32_t j = 0; j < unc; j++) {
                ...
                const HYPType *captured = hyp_scope_lookup(saved_scope, vname);
                if (captured) {
                    hyp_scope_bind(ctx->current_scope, vname, captured);
                }
            }
        }
    }
...
static void process_class_decl(PHPLSPContext *ctx, TSNode node) {
    ...
    if (ctx->module_qn) {
        ...
        ctx->enclosing_class_qn = hyp_arena_strdup(ctx->arena, cname);
    }
...
static void collect_use_declaration(PHPLSPContext *ctx, TSNode use_decl) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(k, "function") == 0)
            kind = HYP_PHP_USE_FUNCTION;
        if (strcmp(k, "const") == 0)
            kind = HYP_PHP_USE_CONST;
    }
    for (uint32_t i = 0; i < nc; i++) {
        TSNode c = ts_node_child(use_decl, i);
        if (ts_node_is_null(c) || !ts_node_is_named(c))
...
/* ── parse return-type text from HYPDefinition.return_type ──────── */

static const HYPType *parse_return_type_text(HYPArena *arena, const char *text,
    ...
    char *first;
    if (cut) {
        first = hyp_arena_strndup(arena, text, (size_t)(cut - text));
    } else {
        first = hyp_arena_strdup(arena, text);
    }
    if (!first)
        return hyp_type_unknown();
    /* trim. */
    ...
    *end = '\0';
    if (!*first || strcmp(first, "null") == 0)
        return hyp_type_unknown();
    if (php_is_builtin_type_name(first))
        return hyp_type_builtin(arena, first);

    /* Resolve via use map. */
    if (first[0] == '\\') {
        return hyp_type_named(arena, php_ns_to_dot(arena, first + 1));
    }
    /* Find first segment for use lookup. */
    const char *bs = first;
    while (*bs && *bs != '\\')
        bs++;
    char *first_seg = (*bs) ? hyp_arena_strndup(arena, first, (size_t)(bs - first))
                            : hyp_arena_strdup(arena, first);
    for (int i = 0; i < use_count; i++) {
        if (strcmp(use_local_names[i], first_seg) == 0) {
            const char *target = use_target_qns[i];
            if (*bs) {
                return hyp_type_named(
                    arena, hyp_arena_sprintf(arena, "%s%s", target, php_ns_to_dot(arena, bs)));
            }
            return hyp_type_named(arena, target);
        }
    }
    /* Fallback: current namespace prefix. */
    if (current_namespace_qn && *current_namespace_qn) {
        return hyp_type_named(arena, hyp_arena_sprintf(arena, "%s.%s", current_namespace_qn,
                                                       php_ns_to_dot(arena, first)));
    }
    if (module_qn) {
        return hyp_type_named(
            arena, hyp_arena_sprintf(arena, "%s.%s", module_qn, php_ns_to_dot(arena, first)));
    }
    return hyp_type_named(arena, php_ns_to_dot(arena, first));
}
...
static const HYPType *php_signature_parse_type(HYPArena *arena, const char *text,
                                               void *parser_ctx) {
 ...
 * typed properties on the corresponding HYPRegisteredType.field_names /
 * field_types. Handles:
 ...
 * returns the property's HYPType when known. */

typedef struct {
    const char *class_qn;
    const char **field_names; /* arena-backed, NULL-terminated */
    const HYPType **field_types;
    int count;
    int cap;
} php_class_fields_t;
...
static php_class_fields_t *fields_for(php_class_field_table_t *tab, const char *class_qn,
    ...
    if (tab->count >= tab->cap) {
        ...
        php_class_fields_t *next =
            (php_class_fields_t *)hyp_arena_alloc(arena, (size_t)new_cap * sizeof(*next));
        if (!next)
            return NULL;
        for (int i = 0; i < tab->count; i++)
            next[i] = tab->items[i];
    ...
    memset(f, 0, sizeof(*f));
    f->class_qn = hyp_arena_strdup(arena, class_qn);
    return f;
}

static void add_field(php_class_fields_t *f, HYPArena *arena, const char *name,
                      const HYPType *type) {
    if (!f || !name || !type)
        return;
    ...
    if (f->count >= f->cap) {
        ...
        const char **nn =
            (const char **)hyp_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nn));
        const HYPType **nt =
            (const HYPType **)hyp_arena_alloc(arena, (size_t)(new_cap + 1) * sizeof(*nt));
        if (!nn || !nt)
            return;
        for (int i = 0; i < f->count; i++) {
    ...
    f->field_names[f->count] = hyp_arena_strdup(arena, name);
...
static const char *class_qn_for_node(PHPLSPContext *ctx, TSNode class_node) {
    ...
    if (!cname)
        return NULL;
    if (ctx->module_qn) {
        return hyp_arena_sprintf(ctx->arena, "%s.%s", ctx->module_qn, cname);
    }
    return hyp_arena_strdup(ctx->arena, cname);
}
...
static void extract_property_decl(PHPLSPContext *ctx, TSNode prop_decl, php_class_fields_t *out) {
    ...
    const HYPType *ptype = NULL;
    if (!ts_node_is_null(tnode))
        ptype = php_parse_type_node(ctx, tnode);
    if (!ptype || ptype->kind == HYP_TYPE_UNKNOWN) {
...
static void extract_ctor_assignments(PHPLSPContext *ctx, TSNode ctor_method,
    ...
    if (!ts_node_is_null(params)) {
        ...
        for (uint32_t i = 0; i < nc && pc < 32; i++) {
            ...
            if (!vt)
                continue;
            names[pc] = hyp_arena_strdup(ctx->arena, (vt[0] == '$') ? vt + 1 : vt);
            types[pc] = ptype;
            pc++;
        }
    }
...
static const char **parse_phpdoc_template_params(PHPLSPContext *ctx, const char *docstring) {
    ...
    while ((p = strstr(p, "@template")) != NULL) {
        ...
        if (p == name_start)
            continue;
        if (n < 16) {
            names[n++] = hyp_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        }
    }
    if (n == 0)
        return NULL;
    const char **out = (const char **)hyp_arena_alloc(ctx->arena, (size_t)(n + 1) * sizeof(*out));
    if (!out)
        return NULL;
    for (int i = 0; i < n; i++)
        out[i] = names[i];
 ...
 * (param_names, args) mapping, returning a new arena-allocated HYPType. */
static const HYPType *php_substitute_template(HYPArena *arena, const HYPType *t,
                                              const char *const *param_names,
                                              const HYPType *const *args) {
    ...
    if (t->kind == HYP_TYPE_TEMPLATE) {
        ...
        const HYPType **new_args =
            (const HYPType **)hyp_arena_alloc(arena, (size_t)(n + 1) * sizeof(*new_args));
        if (!new_args)
            return t;
        for (int i = 0; i < n; i++) {
...
static void apply_phpdoc_class_tags(PHPLSPContext *ctx, HYPTypeRegistry *reg, const char *class_qn,
        ...
        for (int pi = 0; prefixes[pi]; pi++) {
            ...
            while ((p = strstr(p, prefixes[pi])) != NULL) {
                ...
                if (p == name_start)
                    continue;
                char *alias_name =
                    hyp_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
                while (*p == ' ' || *p == '\t' || *p == '=')
                    p++;
                ...
                if (p == type_start)
                    continue;
                char *type_text =
                    hyp_arena_strndup(ctx->arena, type_start, (size_t)(p - type_start));
                ...
                if (!*type_text)
                    continue;
                const HYPType *aliased = resolve_phpdoc_type(ctx, type_text);
                if (aliased && aliased->kind != HYP_TYPE_UNKNOWN) {
     ...
     * inherited methods at least resolve (untyped). */
    {
        ...
        while ((p = strstr(p, "@extends")) != NULL) {
            ...
            if (p == parent_start)
                continue;
            char *parent_text =
                hyp_arena_strndup(ctx->arena, parent_start, (size_t)(p - parent_start));
            const HYPType *parent_type = resolve_phpdoc_type(ctx, parent_text);
            const char *parent_qn = NULL;
            if (parent_type && parent_type->kind == HYP_TYPE_NAMED) {
                parent_qn = parent_type->data.named.qualified_name;
            } else if (parent_type && parent_type->kind == HYP_TYPE_TEMPLATE) {
            ...
            for (int t = 0; t < reg->type_count; t++) {
                ...
                if (found)
                    break;
                const char **expanded = (const char **)hyp_arena_alloc(
    ...
    while ((p = strstr(p, "@property")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *fname = hyp_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        const HYPType *t = resolve_phpdoc_type(ctx, type_text);
        if (out_fields && fname && t && t->kind != HYP_TYPE_UNKNOWN) {
    ...
    while ((p = strstr(p, "@method")) != NULL) {
        ...
        if (p == name_start)
            continue;
        char *mname = hyp_arena_strndup(ctx->arena, name_start, (size_t)(p - name_start));
        ...
        rf.qualified_name = hyp_arena_sprintf(ctx->arena, "%s.%s", class_qn, mname);
        rf.short_name = mname;
        rf.receiver_type = class_qn;
        rf.min_params = -1;
        const HYPType *ret = resolve_phpdoc_type(ctx, type_text);
        const HYPType **rets = (const HYPType **)hyp_arena_alloc(ctx->arena, 2 * sizeof(*rets));
        if (rets) {
            rets[0] = ret;
            rets[1] = NULL;
        }
 ...
 *     HYPRegisteredFunc with receiver_type = class_qn
 ...
 */
static void flatten_trait_into_class(PHPLSPContext *ctx, HYPTypeRegistry *reg, const char *class_qn,
    ...
    const HYPRegisteredType *t = hyp_registry_lookup_type(reg, trait_qn);
    if (!t)
     ...
     * then re-matches the loop's filter while hyp_registry_add_func() keeps
     ...
     *
     * Snapshot the count before iterating: hyp_registry_add_func() below
    ...
    for (int i = 0; i < func_count_before; i++) {
        const HYPRegisteredFunc *src = &reg->funcs[i];
        ...
        HYPRegisteredFunc rf;
        memset(&rf, 0, sizeof(rf));
        rf.qualified_name = hyp_arena_sprintf(ctx->arena, "%s.%s", class_qn, new_short);
        rf.short_name = hyp_arena_strdup(ctx->arena, new_short);
        rf.receiver_type = class_qn;
        ...
        const HYPType *new_sig = src->signature;
        if (src->signature && src->signature->kind == HYP_TYPE_FUNC &&
            src->signature->data.func.return_types) {
            const HYPType *ret0 = src->signature->data.func.return_types[0];
            if (ret0 && ret0->kind == HYP_TYPE_NAMED && ret0->data.named.qualified_name &&
                strcmp(ret0->data.named.qualified_name, canonical_trait_qn) == 0) {
                const HYPType **rets =
                    (const HYPType **)hyp_arena_alloc(ctx->arena, 2 * sizeof(*rets));
                if (rets) {
                    rets[0] = hyp_type_named(ctx->arena, class_qn);
                    rets[1] = NULL;
                    new_sig = hyp_type_func_replace_returns(ctx->arena, src->signature, rets);
                }
            }
        }
...
static void process_trait_use(PHPLSPContext *ctx, HYPTypeRegistry *reg, const char *class_qn,
                              TSNode use_decl) {
        ...
        if (strcmp(k, "property_declaration") == 0) {
            ...
            if (!ts_node_is_null(case_name)) {
                ...
                if (cn) {
                     ...
                     * handles below. */
                    add_field(f, ctx->arena, cn, hyp_type_named(ctx->arena, cqn));
                }
            }
            ...
            if (!ts_node_is_null(rt_node) && mn) {
                ...
                const HYPType *parsed = php_parse_type_node(ctx, rt_node);
                ctx->enclosing_class_qn = saved_class;
                ctx->enclosing_parent_qn = saved_parent;
                if (parsed && parsed->kind != HYP_TYPE_UNKNOWN) {
                    for (int fi = 0; fi < reg->func_count; fi++) {
                        HYPRegisteredFunc *rf = &reg->funcs[fi];
                        ...
                        if (rets) {
                            ...
                            rets[1] = NULL;
                            rf->signature =
                                hyp_type_func_replace_returns(ctx->arena, rf->signature, rets);
                        }
                        break;
                    }
                }
                ...
                if (p) {
                    ...
                    if (p > type_start) {
                        ...
                        if (strcmp(type_text, "$this") == 0 || strcmp(type_text, "static") == 0 ||
                            ...
                            ret_t = hyp_type_named(ctx->arena, cqn);
                        }
                            ...
                            for (int t = 0; t < reg->type_count; t++) {
                                ...
                                for (int j = 0; reg->types[t].type_param_names[j]; j++) {
                                    if (strcmp(reg->types[t].type_param_names[j], type_text) == 0) {
                                        ret_t = hyp_type_named(ctx->arena, type_text);
                                        break;
                                    }
                                }
                                break;
                        ...
                        if (ret_t && ret_t->kind != HYP_TYPE_UNKNOWN) {
                            ...
                            for (int fi = 0; fi < reg->func_count; fi++) {
                                ...
                                if (rets) {
                                    rets[0] = ret_t;
                                    rets[1] = NULL;
                                    rf->signature = hyp_type_func_replace_returns(
...
static void php_lsp_collect_class_fields(PHPLSPContext *ctx, HYPTypeRegistry *reg, TSNode root,
                                         php_class_field_table_t *tab) {
    if (ts_node_is_null(root))
        return;
...
void hyp_run_php_lsp(HYPArena *arena, HYPFileResult *result, const char *source, int source_len,
    ...
    hyp_php_stdlib_register(&reg, arena);
    ...
    for (int i = 0; i < result->defs.count; i++) {
                    ...
                    if (emb) {
                        for (int j = 0; j < bc; j++) {
                            ...
                            if (base[0] != '\\' && !strchr(base, '.')) {
                                qualified = hyp_arena_sprintf(arena, "%s.%s", module_qn, base);
                            } else if (base[0] == '\\') {
        ...
        if (strcmp(d->label, "Function") == 0 || strcmp(d->label, "Method") == 0) {
            ...
            if (d->signature_param_types || d->signature_param_count > 0) {
                param_types = hyp_type_materialize_signature_params(
                    ...
                    php_signature_parse_type, &parse_ctx);
                       ...
                       d->param_types) {
                /* Legacy HYPDefinition callers have no counted carrier. Keep
            ...
            rf.signature = hyp_type_func(arena, NULL, param_types, rets);
            hyp_registry_add_func(&reg, rf);
        }
    }
     ...
     * for $this->prop and $obj->prop sees the field types. */
    {
        ...
        /* Collect top-level children once (O(n)); ts_node_child(root,i) is O(i) → O(n²). */
        uint32_t pkn = 0;
        TSNode *pkids = hyp_lsp_collect_children(ctx.arena, root, &pkn);
        ...
        hyp_php_refine_lsp_registry(&ctx, &reg, root);
    ...
    if (ctx.debug) {
        ...
        for (int i = 0; i < result->resolved_calls.count; i++) {
            const HYPResolvedCall *rc = &result->resolved_calls.items[i];
...
static const char **php_split_pipe(HYPArena *arena, const char *text) {
    if (!text || !text[0])
        return NULL;
    int count = 1;
...
/* Build a registry from caller-supplied HYPLSPDef[] — covers both the source
...
void hyp_run_php_lsp_cross(HYPArena *arena, const char *source, int source_len,
    ...
    hyp_php_register_lsp_defs(arena, &idx_arena, &reg, defs, def_count);
    ...
    for (int i = 0; i < import_count; i++) {
        if (import_names && import_qns && import_names[i] && import_qns[i]) {
            php_lsp_add_use(&ctx, import_names[i], import_qns[i], HYP_PHP_USE_CLASS);
        }
    }

    /* Class field collection — same flow as hyp_run_php_lsp. Walks the AST
...
void hyp_batch_php_lsp_cross(HYPArena *arena, HYPBatchPHPLSPFile *files, int file_count,
                             HYPResolvedCallArray *out) {
    if (!arena || !files || file_count <= 0 || !out)
        return;

    for (int f = 0; f < file_count; f++) {
        HYPBatchPHPLSPFile *file = &files[f];
        ...
        hyp_run_php_lsp_cross(&file_arena, file->source, file->source_len, file->module_qn,
