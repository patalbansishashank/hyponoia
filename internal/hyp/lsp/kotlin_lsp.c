 ...
 * tree-sitter Kotlin AST, populates a per-file HYPTypeRegistry, builds
 * lexical scopes, infers expression types, and emits HYPResolvedCall
...
#include "../helpers.h"
/* HYPHashTable — cross-registry field bucketing. Relative path: the LSP
 * amalgamation compiles with -Iinternal/hyp only, not -Isrc. */
#include "../../../src/foundation/hash_table.h"

/* Growable field list for one receiver type, bucketed per cross-registry call
 * (see hyp_kotlin_register_lsp_defs). qns carries each field's REAL def QN --
...
static void kt_register_class_members(KotlinLSPContext *ctx, const char *class_qn, TSNode body,
                                      bool is_object);
static const HYPType *kt_eval_call_expression_type(KotlinLSPContext *ctx, TSNode node);
static const HYPType *kt_eval_navigation_expression_type(KotlinLSPContext *ctx, TSNode node);
static const HYPType *kt_eval_navigation_expression_type_at(KotlinLSPContext *ctx, TSNode node,
                                                            TSNode site);
static const HYPType *kt_eval_user_type(KotlinLSPContext *ctx, TSNode node);
...
static const char *kt_type_qn_of(const HYPType *t);
static char *kt_join_dot(HYPArena *a, const char *prefix, const char *name);
...
static const HYPType *kt_try_smart_cast(KotlinLSPContext *ctx, TSNode call_or_nav);
...
static char *kt_join_dot(HYPArena *a, const char *prefix, const char *name) {
    ...
    if (!prefix || !*prefix) {
        return hyp_arena_strdup(a, name);
    }
    size_t pl = strlen(prefix);
    size_t nl = strlen(name);
    char *out = (char *)hyp_arena_alloc(a, pl + 1 + nl + 1);
    if (!out) {
        return NULL;
    }
...
static const HYPType *kt_unwrap_nullable(const HYPType *t) {
...
static void kt_emit_resolved_kind(KotlinLSPContext *ctx, const char *callee_qn,
                                  const char *strategy, float confidence, HYPResolvedKind kind) {
    if (ctx->debug) {
    ...
    HYPResolvedCallArray *arr = ctx->resolved_calls;
    if (arr->count >= arr->cap) {
        int new_cap = arr->cap == 0 ? 16 : arr->cap * 2;
        HYPResolvedCall *new_items = (HYPResolvedCall *)hyp_arena_alloc(
            ctx->arena, (size_t)new_cap * sizeof(HYPResolvedCall));
        if (!new_items) {
            return;
        }
        if (arr->items && arr->count > 0) {
            memcpy(new_items, arr->items, (size_t)arr->count * sizeof(HYPResolvedCall));
        }
        arr->items = new_items;
        arr->cap = new_cap;
    }
    HYPResolvedCall *rc = &arr->items[arr->count];
    memset(rc, 0, sizeof(HYPResolvedCall));
...
static void kt_emit_resolved_reference_at(KotlinLSPContext *ctx, const char *callee_qn,
    ...
    int first = ctx->resolved_calls ? ctx->resolved_calls->count : -1;
    kt_emit_resolved_kind(ctx, callee_qn, strategy, confidence, HYP_RESOLVED_CALL_REFERENCE);
    kt_stamp_resolved_site(ctx, first, site);
    if (!ctx->resolved_calls || !reason || first < 0)
        return;
    for (int i = first; i < ctx->resolved_calls->count; i++) {
        ctx->resolved_calls->items[i].reason = hyp_arena_strdup(ctx->arena, reason);
    }
}
...
static void kt_emit_resolved_invocation_reason_at(KotlinLSPContext *ctx, const char *callee_qn,
                                                  ...
                                                  float confidence, TSNode site) {
    int first = ctx->resolved_calls ? ctx->resolved_calls->count : -1;
    kt_emit_resolved_kind(ctx, callee_qn, strategy, confidence, HYP_RESOLVED_INVOCATION);
    kt_stamp_resolved_site(ctx, first, site);
    if (!ctx->resolved_calls || !reason || first < 0)
        return;
    for (int i = first; i < ctx->resolved_calls->count; i++) {
...
static void kt_emit_resolved_at(KotlinLSPContext *ctx, const char *callee_qn, const char *strategy,
                                float confidence, TSNode site) {
    kt_emit_resolved_kind_at(ctx, callee_qn, strategy, confidence, HYP_RESOLVED_INVOCATION, site);
}

static const HYPScope *kt_scope_declaring_name(const HYPScope *scope, const char *name) {
    if (!name)
        return NULL;
    for (const HYPScope *current = scope; current; current = current->parent) {
        for (const HYPScopeChunk *chunk = current->chunks; chunk; chunk = chunk->next) {
...
static const HYPKotlinDelegateBinding *kt_find_delegate_binding(const KotlinLSPContext *ctx,
                                                                const HYPScope *scope,
                                                                const char *owner_qn,
                                                                const char *name) {
    if (!ctx || !name)
        return NULL;
    for (int i = ctx->delegate_count - 1; i >= 0; i--) {
        const HYPKotlinDelegateBinding *binding = &ctx->delegate_bindings[i];
...
static void kt_record_delegate_binding(KotlinLSPContext *ctx, const HYPScope *scope,
    ...
    for (int i = 0; i < ctx->delegate_count; i++) {
        HYPKotlinDelegateBinding *existing = &ctx->delegate_bindings[i];
    ...
    if (ctx->delegate_count >= ctx->delegate_cap) {
        ...
        HYPKotlinDelegateBinding *grown = (HYPKotlinDelegateBinding *)hyp_arena_alloc(
    ...
    HYPKotlinDelegateBinding *binding = &ctx->delegate_bindings[ctx->delegate_count++];
    memset(binding, 0, sizeof(*binding));
    binding->scope = scope;
    binding->owner_qn = owner_qn ? hyp_arena_strdup(ctx->arena, owner_qn) : NULL;
    binding->name = hyp_arena_strdup(ctx->arena, name);
    binding->getter_qn = getter_qn ? hyp_arena_strdup(ctx->arena, getter_qn) : NULL;
    binding->setter_qn = setter_qn ? hyp_arena_strdup(ctx->arena, setter_qn) : NULL;
}

static void kt_clear_lexical_delegate_binding(KotlinLSPContext *ctx, const HYPScope *scope,
                                              const char *name) {
    if (!ctx || !scope || !name)
        return;
    for (int i = ctx->delegate_count - 1; i >= 0; i--) {
        HYPKotlinDelegateBinding *binding = &ctx->delegate_bindings[i];
...
static void kt_inject_delegate_call(KotlinLSPContext *ctx, const char *callee_qn, TSNode site) {
    ...
    for (int i = 0; i < ctx->synthetic_calls->count; i++) {
        const HYPCall *existing = &ctx->synthetic_calls->items[i];
    ...
    hyp_calls_push(ctx->synthetic_calls, ctx->arena, call);
}
...
static void kt_retarget_callable_carrier(KotlinLSPContext *ctx, TSNode site,
    ...
    for (int i = 0; i < ctx->synthetic_calls->count; i++) {
        HYPCall *call = &ctx->synthetic_calls->items[i];
        ...
        if (!overlaps && call->start_line != line)
            continue;
        call->callee_name = hyp_arena_strdup(ctx->arena, target_name);
        call->requires_lsp_resolution = true;
    }
}
...
static void kt_emit_delegate_target(KotlinLSPContext *ctx, const char *callee_qn, TSNode site) {
    ...
    for (int i = 0; ctx->resolved_calls && i < ctx->resolved_calls->count; i++) {
        const HYPResolvedCall *existing = &ctx->resolved_calls->items[i];
        if (existing->kind == HYP_RESOLVED_INVOCATION && existing->caller_qn &&
    ...
    for (int depth = 0; depth < 4; depth++) {
        ...
        if (strcmp(kind, "assignment") == 0) {
            ...
            if (ctx && rhs_start > lhs_end && rhs_start <= (uint32_t)ctx->source_len) {
                ...
                if (hyp_memmem(between, length, "+=", 2) || hyp_memmem(between, length, "-=", 2) ||
                    hyp_memmem(between, length, "*=", 2) || hyp_memmem(between, length, "/=", 2) ||
                    hyp_memmem(between, length, "%=", 2)) {
...
static void kt_emit_delegate_access(KotlinLSPContext *ctx, const char *name, const char *owner_qn,
    ...
    if (!ctx || !name)
        return;
    const HYPKotlinDelegateBinding *binding = NULL;
    if (owner_qn) {
        binding = kt_find_delegate_binding(ctx, NULL, owner_qn, name);
    } else {
        const HYPScope *scope = kt_scope_declaring_name(ctx->current_scope, name);
        if (scope) {
...
static const HYPType **kt_signature_param_types_from_ast(KotlinLSPContext *ctx, TSNode func_node) {
    ...
    if (nc == 0)
        return NULL;
    const HYPType **types =
        (const HYPType **)hyp_arena_alloc(ctx->arena, (size_t)(nc + 1) * sizeof(*types));
    if (!types)
        return NULL;
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        types[count++] = ts_node_is_null(type_node) ? hyp_type_unknown()
...
static const HYPType *kt_build_func_sig_with_return(KotlinLSPContext *ctx,
                                                    const HYPType **param_types,
                                                    const HYPType *return_type) {
    const HYPType **returns = NULL;
    if (return_type && !hyp_type_is_unknown(return_type)) {
        returns = (const HYPType **)hyp_arena_alloc(ctx->arena, 2 * sizeof(const HYPType *));
        if (!returns)
            return NULL;
        returns[0] = return_type;
        returns[1] = NULL;
    }
    return hyp_type_func(ctx->arena, NULL, param_types, returns);
}
...
void kotlin_lsp_init(KotlinLSPContext *ctx, HYPArena *arena, const char *source, int source_len,
    ...
    ctx->package_qn = package_qn ? hyp_arena_strdup(arena, package_qn) : "";
    ctx->module_qn = module_qn ? hyp_arena_strdup(arena, module_qn) : "";
    ctx->project_name = project_name ? hyp_arena_strdup(arena, project_name) : "";
    ctx->rel_path = rel_path ? hyp_arena_strdup(arena, rel_path) : "";
    ctx->resolved_calls = out;
    ctx->import_cap = KT_IMPORT_INITIAL_CAP;
    ctx->import_locals =
        (const char **)hyp_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_targets =
        (const char **)hyp_arena_alloc(arena, sizeof(const char *) * (size_t)ctx->import_cap);
    ctx->import_kinds = (HYPKotlinUseKind *)hyp_arena_alloc(arena, sizeof(HYPKotlinUseKind) *
                                                                       (size_t)ctx->import_cap);
    ctx->import_count = 0;
    ctx->debug = (getenv("HYP_LSP_DEBUG") != NULL);
    ...
    if (rel_path && *rel_path) {
        ...
        char *fn = (char *)hyp_arena_alloc(arena, name_len + 3);
        if (fn) {
...
void kotlin_lsp_add_import(KotlinLSPContext *ctx, const char *local_name, const char *target_qn,
    ...
    if (ctx->import_count >= ctx->import_cap) {
        ...
        const char **new_locals =
            (const char **)hyp_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        const char **new_targets =
            (const char **)hyp_arena_alloc(ctx->arena, sizeof(const char *) * (size_t)new_cap);
        HYPKotlinUseKind *new_kinds = (HYPKotlinUseKind *)hyp_arena_alloc(
            ctx->arena, sizeof(HYPKotlinUseKind) * (size_t)new_cap);
        ...
        memcpy(new_kinds, ctx->import_kinds, sizeof(HYPKotlinUseKind) * (size_t)ctx->import_count);
    ...
    ctx->import_locals[ctx->import_count] =
        local_name ? hyp_arena_strdup(ctx->arena, local_name) : NULL;
    ctx->import_targets[ctx->import_count] = hyp_arena_strdup(ctx->arena, target_qn);
...
const char *kotlin_resolve_class_name(KotlinLSPContext *ctx, const char *name) {
    ...
    if (ctx->registry && hyp_registry_lookup_type(ctx->registry, name)) {
        return hyp_arena_strdup(ctx->arena, name);
    }
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        ...
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == HYP_KT_USE_TYPE ||
             ctx->import_kinds[i] == HYP_KT_USE_UNKNOWN)) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        if (ctx->import_kinds[i] == HYP_KT_USE_WILDCARD && ctx->import_targets[i]) {
    ...
    int n = 0;
    const char *const *defs = hyp_kotlin_default_import_packages(&n);
    for (int i = 0; i < n; i++) {
    ...
    if (ctx->cross_type_short) {
        const char *qn =
            (const char *)hyp_ht_get((const HYPHashTable *)ctx->cross_type_short, name);
        if (qn && qn[0]) {
            return qn;
        }
    }
...
const char *kotlin_resolve_function_name(KotlinLSPContext *ctx, const char *name) {
    ...
    for (int i = 0; i < ctx->import_count; i++) {
        const char *local = ctx->import_locals[i];
        if (local && strcmp(local, name) == 0 &&
            (ctx->import_kinds[i] == HYP_KT_USE_FUNCTION ||
             ctx->import_kinds[i] == HYP_KT_USE_UNKNOWN)) {
            const char *target = ctx->import_targets[i];
            const HYPRegisteredFunc *direct =
                ctx->registry && target ? hyp_registry_lookup_func(ctx->registry, target) : NULL;
            ...
            if (ctx->registry && target) {
                ...
                const HYPRegisteredFunc *from_module =
                    member ? hyp_registry_lookup_func(ctx->registry, member) : NULL;
    ...
    if (ctx->package_qn && *ctx->package_qn) {
        char *cand = kt_join_dot(ctx->arena, ctx->package_qn, name);
        if (ctx->registry && hyp_registry_lookup_func(ctx->registry, cand)) {
            return cand;
        }
    }

    /* Bare-name lookup for files without a package declaration. */
    if (ctx->registry && hyp_registry_lookup_func(ctx->registry, name)) {
        return hyp_arena_strdup(ctx->arena, name);
    }
    ...
    /* Default imports */
    const char *via_default = kt_resolve_in_default_imports(ctx, name, HYP_KT_USE_FUNCTION);
    if (via_default) {
        return via_default;
    }
    ...
    if (ctx->registry && ctx->registry->funcs) {
        ...
        if (matches == 1 && only_qn) {
            return hyp_arena_strdup(ctx->arena, only_qn);
        }
    }
...
static const char *kt_resolve_in_default_imports(KotlinLSPContext *ctx, const char *name,
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (kind == HYP_KT_USE_FUNCTION || kind == HYP_KT_USE_UNKNOWN) {
            if (hyp_registry_lookup_func(ctx->registry, cand)) {
                return cand;
            }
        }
        if (kind == HYP_KT_USE_TYPE || kind == HYP_KT_USE_UNKNOWN) {
            if (hyp_registry_lookup_type(ctx->registry, cand)) {
                return cand;
            }
        }
    }
...
/* Synthesize a HYPRegisteredFunc on the fly for stdlib types where we
...
static const HYPRegisteredFunc *kt_synth_method(KotlinLSPContext *ctx, const char *class_qn,
                                                const char *method_name) {
    HYPRegisteredFunc *rf =
        (HYPRegisteredFunc *)hyp_arena_alloc(ctx->arena, sizeof(HYPRegisteredFunc));
    if (!rf) {
        return NULL;
    }
    memset(rf, 0, sizeof(HYPRegisteredFunc));
    rf->qualified_name = kt_join_dot(ctx->arena, class_qn, method_name);
    rf->receiver_type = hyp_arena_strdup(ctx->arena, class_qn);
    rf->short_name = hyp_arena_strdup(ctx->arena, method_name);
    rf->min_params = 0;
    return rf;
}
...
static const HYPType *kt_stdlib_method_return_type(KotlinLSPContext *ctx, const char *class_qn,
                                                   const char *method) {
    if (!class_qn || !method) {
        return hyp_type_unknown();
    }
    if (kt_method_returns_self(class_qn, method)) {
        return hyp_type_named(ctx->arena, class_qn);
    }
    if (strcmp(method, "toString") == 0 || strcmp(method, "toRegex") == 0 ||
        ...
        return hyp_type_named(ctx->arena, "kotlin.String");
    }
    if (strcmp(method, "size") == 0 || strcmp(method, "length") == 0 ||
        ...
        return hyp_type_named(ctx->arena, "kotlin.Int");
    }
    if (strcmp(method, "isEmpty") == 0 || strcmp(method, "isNotEmpty") == 0 ||
        ...
        return hyp_type_named(ctx->arena, "kotlin.Boolean");
    }
...
const HYPRegisteredFunc *kotlin_lookup_method(KotlinLSPContext *ctx, const char *class_qn,
    ...
    while (qhead < qtail) {
         ...
         * via hyp_registry_lookup_method_aliased). */
        const HYPRegisteredFunc *rf =
            hyp_registry_lookup_method_aliased(ctx->registry, cur_qn, method_name);
        if (rf) {
            return rf;
        }

        /* method_names fallback */
        const HYPRegisteredType *rt = hyp_registry_lookup_type(ctx->registry, cur_qn);
...
static const char *kt_materialized_constructor_qn(KotlinLSPContext *ctx, const char *class_qn) {
    ...
    const HYPRegisteredFunc *named =
        hyp_registry_lookup_method(ctx->registry, class_qn, class_name);
    if (named && named->qualified_name) {
        return named->qualified_name;
    }
    const HYPRegisteredFunc *init = hyp_registry_lookup_method(ctx->registry, class_qn, "<init>");
...
const HYPType *kotlin_lookup_property_type(KotlinLSPContext *ctx, const char *class_qn,
    ...
    const HYPRegisteredType *rt = hyp_registry_lookup_type(ctx->registry, class_qn);
    if (!rt) {
        return hyp_type_unknown();
    }
    ...
    if (rt->embedded_types) {
        for (int i = 0; rt->embedded_types[i]; i++) {
            const HYPType *t = kotlin_lookup_property_type(ctx, rt->embedded_types[i], prop_name);
...
static const char *kt_parse_package_header(KotlinLSPContext *ctx, TSNode header) {
    ...
    char *out = (char *)hyp_arena_alloc(ctx->arena, strlen(txt) + 1);
    if (!out) {
        return "";
    }
    int oi = 0;
...
static void kt_parse_import_directive(KotlinLSPContext *ctx, TSNode imp) {
    ...
    /* Compress whitespace */
    char *cleaned = (char *)hyp_arena_alloc(ctx->arena, strlen(qid_text) + 1);
    int oi = 0;
    ...
    if (full_text) {
        ...
        if (as_kw) {
            ...
            char *al = (char *)hyp_arena_alloc(ctx->arena, 128);
            if (al) {
                int ai = 0;
    ...
    if (wildcard) {
        kotlin_lsp_add_import(ctx, NULL, cleaned, HYP_KT_USE_WILDCARD);
        return;
    }

    /* Default: kind = UNKNOWN — we'll match it both ways at lookup time. */
    const char *local = alias ? alias : kt_short(cleaned);
    kotlin_lsp_add_import(ctx, local, cleaned, HYP_KT_USE_UNKNOWN);
}
...
static void kt_collect_top_level_decls(KotlinLSPContext *ctx, TSNode root) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(kind, "class_declaration") == 0) {
            ...
            if (!ts_node_is_null(rhs)) {
                const HYPType *underlying = kotlin_parse_type_node(ctx, rhs);
                if (underlying && underlying->kind == HYP_TYPE_NAMED &&
                    underlying->data.named.qualified_name) {
                    rt.alias_of = underlying->data.named.qualified_name;
                }
            }
            hyp_registry_add_type((HYPTypeRegistry *)ctx->registry, rt);
        }
    }
}
...
static void kt_process_class_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (!ts_node_is_null(pc)) {
        ...
        TSNode pcontainer = ts_node_is_null(params) ? pc : params;
        {
            ...
            for (uint32_t pi = 0; pi < pnc && field_count < 63; pi++) {
                ...
                const HYPType *ft = ts_node_is_null(type_node)
                                        ? hyp_type_unknown()
                                        : kotlin_parse_type_node(ctx, type_node);
    ...
    if (field_count > 0) {
        const char **fn = (const char **)hyp_arena_alloc(ctx->arena, sizeof(const char *) *
                                                                         (size_t)(field_count + 1));
        const HYPType **ft = (const HYPType **)hyp_arena_alloc(
            ctx->arena, sizeof(const HYPType *) * (size_t)(field_count + 1));
        if (fn && ft) {
...
static void kt_register_class_members(KotlinLSPContext *ctx, const char *class_qn, TSNode body,
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (strcmp(kind, "function_declaration") == 0) {
            ...
            rf.min_params = 0;
            const HYPType **param_types = kt_signature_param_types_from_ast(ctx, c);
            const HYPType *return_type = NULL;
             ...
             * site and propagate it as the lambda's `this`. */
            {
                ...
                if (!ts_node_is_null(params)) {
                    ...
                    for (uint32_t pi = 0; pi < pnc; pi++) {
                        ...
                        const char **dq =
                            (const char **)hyp_arena_alloc(ctx->arena, 2 * sizeof(const char *));
                        if (dq) {
                            char *tag = (char *)hyp_arena_alloc(
...
static void kt_process_function_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    const HYPType **param_types = kt_signature_param_types_from_ast(ctx, node);
    const HYPType *return_type = NULL;
    /* Capture return type for chain tracking. */
...
static void kt_process_property_decl(KotlinLSPContext *ctx, TSNode node) {
    ...
    hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, pname), pt);
}
...
const HYPType *kotlin_parse_type_node(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (resolved) {
        return hyp_type_named(ctx->arena, resolved);
    }
    return hyp_type_named(ctx->arena, txt);
}
...
static const HYPType *kt_eval_user_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    /* Strip whitespace */
    char *clean = (char *)hyp_arena_alloc(ctx->arena, strlen(txt) + 1);
    int oi = 0;
    for (int i = 0; txt[i]; i++) {
        if (txt[i] != ' ' && txt[i] != '\t' && txt[i] != '\n') {
            clean[oi++] = txt[i];
    ...
    if (!resolved) {
        return hyp_type_named(ctx->arena, clean);
    }
    return hyp_type_named(ctx->arena, resolved);
}
...
static const HYPType *kt_eval_literal_type(KotlinLSPContext *ctx, TSNode node) {
    const char *kind = ts_node_type(node);
    if (strcmp(kind, "string_literal") == 0 || strcmp(kind, "multiline_string_literal") == 0) {
        return hyp_type_named(ctx->arena, "kotlin.String");
    }
    if (strcmp(kind, "character_literal") == 0) {
        return hyp_type_named(ctx->arena, "kotlin.Char");
    }
    if (strcmp(kind, "number_literal") == 0) {
        ...
        if (tail == 'l') {
            return hyp_type_named(ctx->arena, "kotlin.Long");
        }
        if (tail == 'f') {
            return hyp_type_named(ctx->arena, "kotlin.Float");
        }
        if (strchr(txt, '.') || strchr(txt, 'e') || strchr(txt, 'E')) {
            return hyp_type_named(ctx->arena, "kotlin.Double");
        }
        return hyp_type_named(ctx->arena, "kotlin.Int");
    }
    ...
    if (strcmp(kind, "null_literal") == 0 ||
        ...
        return hyp_type_named(ctx->arena, "kotlin.Nothing");
    }
    return hyp_type_unknown();
}
...
static const char *kt_exact_callable_reference_target(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (count == 2) {
        ...
        if (hyp_scope_contains(ctx->current_scope, receiver_name)) {
            const HYPType *receiver_type = hyp_scope_lookup(ctx->current_scope, receiver_name);
            receiver_qn = kt_type_qn_of(receiver_type);
        } else {
            const char *candidate = kotlin_resolve_class_name(ctx, receiver_name);
            if (candidate && ctx->registry && hyp_registry_lookup_type(ctx->registry, candidate)) {
                receiver_qn = candidate;
            }
        }
        if (!receiver_qn)
            return NULL;
        const HYPRegisteredFunc *method = kotlin_lookup_method(ctx, receiver_qn, member);
        return method ? method->qualified_name : NULL;
    }

    if (hyp_scope_contains(ctx->current_scope, member)) {
        return hyp_scope_lookup_callable(ctx->current_scope, member);
    }

    const char *class_qn = kotlin_resolve_class_name(ctx, member);
    if (class_qn && ctx->registry && hyp_registry_lookup_type(ctx->registry, class_qn)) {
...
const HYPType *kotlin_eval_expr_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "identifier") == 0 || strcmp(kind, "simple_identifier") == 0) {
        ...
        if (strcmp(name, "true") == 0 || strcmp(name, "false") == 0) {
            result = hyp_type_named(ctx->arena, "kotlin.Boolean");
            goto out;
        }
        if (strcmp(name, "null") == 0) {
            result = hyp_type_named(ctx->arena, "kotlin.Nothing");
            goto out;
        }
        ...
        if (hyp_scope_contains(ctx->current_scope, name)) {
            const char *callable_qn = hyp_scope_lookup_callable(ctx->current_scope, name);
        ...
        if (cls_qn) {
            const HYPRegisteredType *rt = hyp_registry_lookup_type(ctx->registry, cls_qn);
            if (rt) {
                result = hyp_type_named(ctx->arena, cls_qn);
                goto out;
            }
        }
    ...
    if (strcmp(kind, "this_expression") == 0) {
        result = ctx->this_type ? ctx->this_type : hyp_type_unknown();
        goto out;
    }
    if (strcmp(kind, "super_expression") == 0) {
        result = ctx->super_type ? ctx->super_type : hyp_type_unknown();
        goto out;
    }
    if (strcmp(kind, "call_expression") == 0) {
    ...
    if (strcmp(kind, "binary_expression") == 0 || strcmp(kind, "additive_expression") == 0 ||
        ...
        const HYPType *lhs_t = kotlin_eval_expr_type(ctx, lhs);
        ...
        if (op_method && lhs_t && !hyp_type_is_unknown(lhs_t)) {
            const char *lhs_qn = kt_type_qn_of(lhs_t);
            if (lhs_qn) {
                const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, lhs_qn, op_method);
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                    if (rf->signature && rf->signature->kind == HYP_TYPE_FUNC &&
    ...
    if (strcmp(kind, "unary_expression") == 0 || strcmp(kind, "prefix_expression") == 0 ||
        ...
        if (nc > 0) {
            ...
            const HYPType *t = kotlin_eval_expr_type(ctx, operand);
            const char *op_method = kt_unary_operator_method(node, kind);
            if (op_method && t && !hyp_type_is_unknown(t)) {
                const char *qn = kt_type_qn_of(t);
                if (qn) {
                    const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, qn, op_method);
                    if (rf && rf->qualified_name) {
    ...
    if (strcmp(kind, "in_expression") == 0 || strcmp(kind, "check_expression") == 0) {
        ...
        if (is_membership && nc >= 2) {
            ...
            const HYPType *ct = kotlin_eval_expr_type(ctx, container);
            const char *cqn = kt_type_qn_of(ct);
            if (cqn) {
                const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, cqn, "contains");
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                }
    ...
    if (strcmp(kind, "index_expression") == 0 || strcmp(kind, "indexing_expression") == 0) {
        ...
        if (nc >= 1) {
            ...
            const HYPType *rt = kotlin_eval_expr_type(ctx, recv);
            const char *rqn = kt_type_qn_of(rt);
            if (rqn) {
                const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, rqn, "get");
                if (rf && rf->qualified_name) {
                    kt_emit_resolved_at(ctx, rf->qualified_name, "lsp_kt_operator", KT_CONF_METHOD,
                                        node);
                    if (rf->signature && rf->signature->kind == HYP_TYPE_FUNC &&
                ...
                if (nc >= 2) {
                    ...
                    if (lhs_text) {
                        ...
                        if (candidate && ctx->registry &&
                            hyp_registry_lookup_type(ctx->registry, candidate)) {
                            recv_qn = candidate;
                        }
                    }
                    if (!recv_qn) {
                        ...
                        const HYPType *t =
                            lhs_text ? hyp_scope_lookup(ctx->current_scope, lhs_text) : NULL;
                    ...
                    if (recv_qn) {
                        if (strcmp(member, "class") == 0) {
                            ...
                            result = hyp_type_named(ctx->arena, "kotlin.reflect.KClass");
                            goto out;
                        }
                        const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, recv_qn, member);
                        if (rf && rf->qualified_name) {
                            kt_emit_resolved_kind_at(ctx, rf->qualified_name, "lsp_kt_callable_ref",
                                                     KT_CONF_METHOD, HYP_RESOLVED_CALL_REFERENCE,
                                                     last);
                        } else if (!hyp_type_is_unknown(
                            ...
                            const char *prop_qn =
                                hyp_arena_sprintf(ctx->arena, "%s.%s", recv_qn, member);
                    ...
                    if (hyp_scope_contains(ctx->current_scope, member)) {
                        const HYPType *lexical = hyp_scope_lookup(ctx->current_scope, member);
                        const char *lexical_qn =
                            hyp_scope_lookup_callable(ctx->current_scope, member);
                        if (lexical_qn) {
                            kt_emit_resolved_kind_at(ctx, lexical_qn, "lsp_kt_callable_alias_ref",
                                                     KT_CONF_TOP_LEVEL, HYP_RESOLVED_CALL_REFERENCE,
                                                     last);
                        }
                    ...
                    const char *class_qn = kotlin_resolve_class_name(ctx, member);
                    const HYPRegisteredType *class_type =
                        class_qn && ctx->registry
                            ? hyp_registry_lookup_type(ctx->registry, class_qn)
                            : NULL;
                    if (class_type) {
                        ...
                        if (constructor_qn) {
                            kt_emit_resolved_kind_at(ctx, constructor_qn, "lsp_kt_constructor_ref",
                                                     KT_CONF_CONSTRUCTOR,
                                                     HYP_RESOLVED_CALL_REFERENCE, last);
                        }
                        result = hyp_type_unknown();
                        goto out;
                    }
    ...
    if (strcmp(kind, "object_literal") == 0) {
        ...
        result = hyp_type_named(ctx->arena, "kotlin.Any");
        goto out;
    }
    if (strcmp(kind, "collection_literal") == 0) {
        /* [a, b, c] — typically a List */
        result = hyp_type_named(ctx->arena, "kotlin.collections.List");
        goto out;
    }
    if (strcmp(kind, "range_expression") == 0) {
        result = hyp_type_named(ctx->arena, "kotlin.ranges.IntRange");
        goto out;
    }
...
static const HYPType *kt_eval_constructor_or_func_call(KotlinLSPContext *ctx, TSNode call_node,
    ...
    if (hyp_scope_contains(ctx->current_scope, callee_text)) {
        const char *target = hyp_scope_lookup_callable(ctx->current_scope, callee_text);
        const HYPType *bound = hyp_scope_lookup(ctx->current_scope, callee_text);
        if (target) {
            ...
            const HYPRegisteredFunc *rf =
                ctx->registry ? hyp_registry_lookup_func(ctx->registry, target) : NULL;
        ...
        if (bound && bound->kind == HYP_TYPE_CALLABLE) {
            return bound->data.callable.return_type ? bound->data.callable.return_type
                                                    : hyp_type_unknown();
        }
        if (bound && bound->kind == HYP_TYPE_FUNC && bound->data.func.return_types &&
    ...
    if (ctx->this_type && !hyp_type_is_unknown(ctx->this_type)) {
        const char *this_qn = kt_type_qn_of(ctx->this_type);
        if (this_qn) {
            const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, this_qn, callee_text);
            if (rf && rf->qualified_name) {
    ...
    if (cls_qn && ctx->registry) {
        ...
        if (rt) {
            ...
            return hyp_type_named(ctx->arena, cls_qn);
        }
    }
    /* 3. Top-level fun */
    ...
    if (fn_qn) {
        ...
        if (ctx->registry) {
            const HYPRegisteredFunc *rf = hyp_registry_lookup_func(ctx->registry, fn_qn);
            if (rf && rf->signature && rf->signature->kind == HYP_TYPE_FUNC) {
        ...
        for (int i = 0; stdlib_returns[i].qn; i++) {
            if (strcmp(stdlib_returns[i].qn, fn_qn) == 0) {
                return hyp_type_named(ctx->arena, stdlib_returns[i].ret);
            }
        }
    }
    (void)call_node;
...
static const HYPType *kt_eval_call_expression_type(KotlinLSPContext *ctx, TSNode node) {
    ...
    if (strcmp(kind, "navigation_expression") == 0) {
        ...
        const HYPType *t = kt_eval_navigation_expression_type_at(ctx, callee, node);
        return t;
    }
...
static const HYPType *kt_eval_navigation_expression_type_at(KotlinLSPContext *ctx, TSNode node,
    ...
    const HYPType *recv_type = kotlin_eval_expr_type(ctx, receiver_node);
    ...
    if (recv_qn && ctx->registry) {
        ...
        const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, recv_qn, member_text);
        if (rf && rf->qualified_name) {
            ...
            if (is_member) {
                ...
                const HYPRegisteredType *recv_rt =
            ...
            const HYPType *guess = kt_stdlib_method_return_type(ctx, recv_qn, member_text);
            if (!hyp_type_is_unknown(guess)) {
                return guess;
            }
            return hyp_type_unknown();
        }
        ...
        const HYPType *pt = kotlin_lookup_property_type(ctx, recv_qn, member_text);
        if (!hyp_type_is_unknown(pt)) {
            ...
            if (ts_node_eq(site, node)) {
                ...
                if (ctx->cross_field_map) {
                    const KtCrossFieldList *fl = (const KtCrossFieldList *)hyp_ht_get(
                        (const HYPHashTable *)ctx->cross_field_map, recv_qn);
                ...
                kt_emit_resolved_kind_at(ctx, prop_qn, "lsp_kt_property_ref", KT_CONF_METHOD,
                                         HYP_RESOLVED_CALL_REFERENCE, selector);
            }
            return pt;
        }
        ...
        rf = hyp_registry_lookup_method(ctx->registry, recv_qn, member_text);
        if (rf && rf->qualified_name) {
    ...
    if (ctx->it_type && kt_node_is(receiver_node, "identifier")) {
        ...
        if (recv_text && strcmp(recv_text, "it") == 0) {
            ...
            if (it_qn) {
                const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, it_qn, member_text);
                if (rf && rf->qualified_name) {
...
static void kt_register_property_delegate(KotlinLSPContext *ctx, TSNode prop,
    ...
    const HYPType *delegate_type = kotlin_eval_expr_type(ctx, inner);
    const char *delegate_qn = kt_type_qn_of(delegate_type);
    if (!delegate_qn)
        return;
    const HYPRegisteredFunc *getter = kotlin_lookup_method(ctx, delegate_qn, "getValue");
    const HYPRegisteredFunc *setter = NULL;
    ...
    const HYPScope *scope = owner ? NULL : ctx->current_scope;
...
static void kt_bind_property_to_scope(KotlinLSPContext *ctx, TSNode prop) {
        ...
        if (init_t && !hyp_type_is_unknown(init_t)) {
            ...
            for (uint32_t i = 0; i < mnc; i++) {
                ...
                if (iqn) {
                    ...
                    const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, iqn, comp);
                    if (rf && rf->qualified_name) {
                ...
                if (!ts_node_is_null(vid)) {
                    ...
                    if (vname) {
                        hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, vname),
                                       hyp_type_unknown());
                    }
                }
            }
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        if (ts_node_is_null(type_node)) {
            const HYPType *candidate = kotlin_eval_expr_type(ctx, child);
            if (!hyp_type_is_unknown(candidate)) {
                t = candidate;
                break;
            }
        }
    ...
    if (callable_qn) {
        hyp_scope_bind_callable(ctx->current_scope, hyp_arena_strdup(ctx->arena, name), t,
                                callable_qn);
    } else {
        hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, name), t);
    }
}
...
static void kt_bind_assignment_to_scope(KotlinLSPContext *ctx, TSNode assignment) {
    ...
    if (callable_qn) {
        hyp_scope_update_callable(ctx->current_scope, name, callable_qn);
    } else {
...
static void kt_apply_smart_cast(KotlinLSPContext *ctx, TSNode condition_expr, bool then_branch) {
    ...
    if (!is_check && kt_node_is(condition_expr, "check_expression")) {
        ...
        if (ctext &&
            (hyp_memmem(ctext, strlen(ctext), " is ", 4) != NULL || strstr(ctext, "!is") != NULL)) {
            is_check = true;
        }
    }
    if (!is_check) {
    ...
    const HYPType *t = kotlin_parse_type_node(ctx, rhs);
    if (!hyp_type_is_unknown(t)) {
        hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, name), t);
    }
}
...
static void kt_process_when_expression(KotlinLSPContext *ctx, TSNode node) {
    ...
    const HYPType *subject_type = NULL;
        ...
        for (uint32_t e = 0; e < en; e++) {
            ...
            if (kt_node_is(ec, "_when_condition") || kt_node_is(ec, "when_condition") ||
                ...
                if (!ts_node_is_null(tt) && subject_name) {
                    const HYPType *narrow = kotlin_parse_type_node(ctx, tt);
                    if (!hyp_type_is_unknown(narrow)) {
                        hyp_scope_bind(ctx->current_scope,
                                       hyp_arena_strdup(ctx->arena, subject_name), narrow);
                    }
                }
            }
        }
...
static void kt_process_for_statement(KotlinLSPContext *ctx, TSNode node) {
    ...
    const HYPType *iter_t = kotlin_eval_expr_type(ctx, iter);
    if (iter_t && !hyp_type_is_unknown(iter_t)) {
        const char *iqn = kt_type_qn_of(iter_t);
        if (iqn) {
            const char *protocol[] = {"iterator", "hasNext", "next", NULL};
            for (int i = 0; protocol[i]; i++) {
                const HYPRegisteredFunc *rf = kotlin_lookup_method(ctx, iqn, protocol[i]);
                if (rf && rf->qualified_name) {
    ...
    if (!ts_node_is_null(loop_var)) {
        ...
        if (!ts_node_is_null(id)) {
            char *name = kt_node_text(ctx, id);
            if (name) {
                hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, name),
                               hyp_type_unknown());
            }
        }
    }
...
static void kt_process_lambda(KotlinLSPContext *ctx, TSNode lambda, const HYPType *receiver_type) {
    ...
    ctx->current_scope = hyp_scope_push(ctx->arena, ctx->current_scope);
    const HYPType *prev_it = ctx->it_type;
    ...
    const HYPType *prev_this = ctx->this_type;
    if (receiver_type && !hyp_type_is_unknown(receiver_type)) {
        ...
        for (uint32_t i = 0; i < pnc; i++) {
            ...
            if (!ts_node_is_null(id)) {
                ...
                if (name) {
                    ...
                    const HYPType *t =
                        ts_node_is_null(tn) ? hyp_type_unknown() : kotlin_parse_type_node(ctx, tn);
                    hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, name), t);
                }
            }
        }
...
static void kt_process_statement(KotlinLSPContext *ctx, TSNode stmt) {
    ...
    if (strcmp(kind, "function_declaration") == 0) {
        ...
        {
            HYPRegisteredFunc lrf = {0};
            lrf.qualified_name = new_qn;
            lrf.short_name = fname;
            lrf.min_params = 0;
            hyp_registry_add_func((HYPTypeRegistry *)ctx->registry, lrf);
            ...
            hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, fname),
                           hyp_type_callable(ctx->arena, NULL, 0, hyp_type_unknown()));
            ...
            if (ctx->package_qn && *ctx->package_qn) {
                HYPRegisteredFunc alias = lrf;
                alias.qualified_name = kt_join_dot(ctx->arena, ctx->package_qn, fname);
                hyp_registry_add_func((HYPTypeRegistry *)ctx->registry, alias);
            } else {
                /* No package — register at root. */
                HYPRegisteredFunc alias = lrf;
                alias.qualified_name = hyp_arena_strdup(ctx->arena, fname);
                hyp_registry_add_func((HYPTypeRegistry *)ctx->registry, alias);
            }
        }
    ...
    if (strcmp(kind, "callable_reference") == 0) {
        ...
        if (ctx->resolved_calls) {
            ...
            for (int i = 0; i < ctx->resolved_calls->count; i++) {
                const HYPResolvedCall *resolved = &ctx->resolved_calls->items[i];
                if (resolved->kind == HYP_RESOLVED_CALL_REFERENCE &&
...
static void kt_bind_function_params(KotlinLSPContext *ctx, TSNode func_node) {
    ...
    for (uint32_t i = 0; i < nc; i++) {
        ...
        const HYPType *t = ts_node_is_null(type_node) ? hyp_type_unknown()
                                                      : kotlin_parse_type_node(ctx, type_node);
        hyp_scope_bind(ctx->current_scope, hyp_arena_strdup(ctx->arena, pname), t);
    }

    /* For extension fun and method: bind `this` */
    if (ctx->this_type) {
        hyp_scope_bind(ctx->current_scope, "this", ctx->this_type);
    }
}
...
static void kt_process_function_body(KotlinLSPContext *ctx, TSNode func_node, const char *func_qn,
    ...
    const char *prev_class = ctx->enclosing_class_qn;
    const HYPType *prev_this = ctx->this_type;
    const HYPType *prev_super = ctx->super_type;
    const char *prev_super_qn = ctx->enclosing_super_qn;

    ctx->enclosing_func_qn = func_qn;
    const HYPRegisteredType *receiver_rt = NULL;
    if (receiver_class_qn) {
        ctx->this_type = hyp_type_named(ctx->arena, receiver_class_qn);
        receiver_rt = hyp_registry_lookup_type(ctx->registry, receiver_class_qn);
        if (receiver_rt && receiver_rt->embedded_types && receiver_rt->embedded_types[0]) {
            ...
            ctx->super_type = hyp_type_named(ctx->arena, receiver_rt->embedded_types[0]);
        }
    }
    ...
    if (receiver_rt && receiver_rt->field_names && receiver_rt->field_types) {
        for (int i = 0; receiver_rt->field_names[i]; i++) {
            ...
            hyp_scope_bind(ctx->current_scope,
                           hyp_arena_strdup(ctx->arena, receiver_rt->field_names[i]),
                           receiver_rt->field_types[i]);
            const HYPKotlinDelegateBinding *member_delegate =
...
/* Debug AST dumper — only active when HYP_LSP_KOTLIN_AST=1 in env. */
...
/* ── public entry point used by hyp.c dispatcher ──────────────────── */
...
static char *kt_repair_bodyless_interface_methods(HYPArena *arena, const char *src, int src_len,
    ...
    char *out = (char *)hyp_arena_alloc(arena, (size_t)new_len + 1);
    if (!out) {
        return NULL;
    }
    int oi = 0;
...
void hyp_run_kotlin_lsp(HYPArena *arena, HYPFileResult *result, const char *source, int source_len,
    ...
    bool debug = (getenv("HYP_LSP_DEBUG") != NULL);
    ...
    /* Curated stdlib */
    hyp_kotlin_stdlib_register(&registry, arena);
    ...
    if (first_dot) {
        ...
        char *pn = (char *)hyp_arena_alloc(arena, pl + 1);
        if (pn) {
...
static const HYPType *kt_signature_parse_type(HYPArena *arena, const char *text, void *parser_ctx) {
    const KotlinSignatureParseContext *ctx = (const KotlinSignatureParseContext *)parser_ctx;
    const HYPLSPDef *def = ctx ? ctx->def : NULL;
    if (!text || !text[0])
        return hyp_type_unknown();

    char *clean = hyp_arena_strdup(arena, text);
    if (!clean)
        return hyp_type_unknown();
    ...
    return hyp_type_named(arena, qualified);
}

static const HYPType *kt_cross_return_type(HYPArena *arena, const HYPLSPDef *d) {
    if (!arena || !d || !d->return_types || !d->return_types[0]) {
        return NULL;
    }
    const char *text = d->return_types;
    const char *bar = strchr(text, '|');
    const char *first = bar ? hyp_arena_strndup(arena, text, (size_t)(bar - text)) : text;
    if (!first || !first[0]) {
        return NULL;
    }
    if (!strchr(first, '.')) {
    ...
    return hyp_type_named(arena, first);
}

static const HYPType *kt_cross_func_sig_with_return(HYPArena *arena, const HYPLSPDef *d) {
    const HYPType *ret = kt_cross_return_type(arena, d);
    ...
    if (ret && !hyp_type_is_unknown(ret)) {
        rets = (const HYPType **)hyp_arena_alloc(arena, 2 * sizeof(*rets));
        if (!rets)
            return NULL;
        rets[0] = ret;
        rets[1] = NULL;
    }
    return hyp_type_func(arena, NULL, param_types, rets);
}

static void kt_register_cross_def(HYPTypeRegistry *reg, HYPArena *arena, const HYPLSPDef *d,
                                  const HYPHashTable *field_map) {
    ...
    if (strcmp(d->label, "Class") == 0 || strcmp(d->label, "Interface") == 0 ||
        ...
        if (field_map) {
            const KtCrossFieldList *fl =
                (const KtCrossFieldList *)hyp_ht_get((HYPHashTable *)field_map,
                                                     d->qualified_name);
            if (fl && fl->count > 0) {
        ...
        if (d->embedded_types && d->embedded_types[0]) {
            ...
            const char **emb =
                (const char **)hyp_arena_alloc(arena, (size_t)(n + 1) * sizeof(*emb));
            int idx = 0;
            ...
            while (*start) {
                ...
                if (end > start) {
                    emb[idx++] = hyp_arena_strndup(arena, start, (size_t)(end - start));
                }
                if (!*end) {
                    break;
                }
...
void hyp_kotlin_register_lsp_defs(HYPArena *arena, HYPTypeRegistry *reg, const HYPLSPDef *defs,
                                  int def_count, HYPHashTable **out_field_map) {
     ...
     * quadratic pattern. */
    HYPHashTable *field_map = hyp_ht_create(64);
    if (field_map) {
        for (int i = 0; i < def_count; i++) {
            ...
            KtCrossFieldList *fl = (KtCrossFieldList *)hyp_ht_get(field_map, d->receiver_type);
            if (!fl) {
                fl = (KtCrossFieldList *)hyp_arena_alloc(arena, sizeof(*fl));
                if (!fl) {
                    continue;
                }
                memset(fl, 0, sizeof(*fl));
                hyp_ht_set(field_map, d->receiver_type, fl);
            }
            if (fl->count + 1 >= fl->cap) {
                int new_cap = fl->cap ? fl->cap * 2 : 4;
                const char **nn =
                    (const char **)hyp_arena_alloc(arena, (size_t)new_cap * sizeof(*nn));
                const HYPType **nt =
                    (const HYPType **)hyp_arena_alloc(arena, (size_t)new_cap * sizeof(*nt));
                const char **nq =
                    (const char **)hyp_arena_alloc(arena, (size_t)new_cap * sizeof(*nq));
                if (!nn || !nt || !nq) {
                    continue;
                }
                if (fl->count > 0) {
            ...
            const HYPType *ft = kt_cross_return_type(arena, d);
            ...
            fl->types[fl->count] = ft ? ft : hyp_type_named(arena, "kotlin.Any");
...
void hyp_run_kotlin_lsp_cross(HYPArena *arena, const char *source, int source_len,
    ...
    hyp_kotlin_stdlib_register(&reg, arena);
    ...
    HYPHashTable *cross_field_map = NULL;
    hyp_kotlin_register_lsp_defs(arena, &reg, defs, def_count, &cross_field_map);
    ...
    HYPHashTable *type_short = hyp_ht_create(64);
    if (type_short) {
        for (int i = 0; i < def_count; i++) {
            const HYPLSPDef *d = &defs[i];
            if (!d->qualified_name || !d->short_name || !d->label) {
            ...
            const char *prev = (const char *)hyp_ht_get(type_short, d->short_name);
            if (!prev) {
                hyp_ht_set(type_short, d->short_name, (void *)d->qualified_name);
            } else if (strcmp(prev, d->qualified_name) != 0) {
                hyp_ht_set(type_short, d->short_name, (void *)"");
            }
        }
    }
    ...
    for (int i = 0; i < import_count; i++) {
        ...
        kotlin_lsp_add_import(&ctx, import_names[i], import_qns[i], HYP_KT_USE_UNKNOWN);
    }

    kotlin_lsp_process_file(&ctx, root);
    hyp_arena_destroy(&idx_arena);
    hyp_ht_free(type_short);
    hyp_ht_free(cross_field_map);
