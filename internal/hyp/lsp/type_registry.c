...
static void build_qn_index(HYPTypeRegistry *reg, HYPArena *idx_arena, bool for_funcs) {
    ...
    int *buckets = (int *)hyp_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    HYPRegistryHashEntry *entries = (HYPRegistryHashEntry *)hyp_arena_alloc(
        idx_arena, (size_t)count * sizeof(HYPRegistryHashEntry));
    if (!buckets || !entries)
        return;
...
static void build_method_index(HYPTypeRegistry *reg, HYPArena *idx_arena) {
    if (reg->func_count == 0)
        return;
    ...
    int *buckets = (int *)hyp_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    HYPRegistryHashEntry *entries = (HYPRegistryHashEntry *)hyp_arena_alloc(
        reg->arena, (size_t)reg->func_count * sizeof(HYPRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
    ...
    for (int i = 0; i < reg->func_count; i++) {
        const HYPRegisteredFunc *f = &reg->funcs[i];
...
static void build_type_embed_index(HYPTypeRegistry *reg, HYPArena *idx_arena) {
    int ecount = 0;
    ...
    int *buckets = (int *)hyp_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    HYPRegistryHashEntry *entries = (HYPRegistryHashEntry *)hyp_arena_alloc(
        idx_arena, (size_t)ecount * sizeof(HYPRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
static void build_ffunc_short_index(HYPTypeRegistry *reg, HYPArena *idx_arena) {
    int fcount = 0;
    for (int i = 0; i < reg->func_count; i++) {
        const HYPRegisteredFunc *f = &reg->funcs[i];
        if (!f->receiver_type && f->short_name)
    ...
    int *buckets = (int *)hyp_arena_alloc(idx_arena, (size_t)bucket_count * sizeof(int));
    HYPRegistryHashEntry *entries = (HYPRegistryHashEntry *)hyp_arena_alloc(
        idx_arena, (size_t)fcount * sizeof(HYPRegistryHashEntry));
    if (!buckets || !entries)
        return;
    for (int i = 0; i < bucket_count; i++)
        buckets[i] = -1;
...
int hyp_type_embed_iter_next(HYPTypeEmbedIter *it) {
    const HYPTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const HYPRegistryHashEntry *e = &reg->type_embed_entries[it->chain_idx];
...
int hyp_free_func_iter_next(HYPFreeFuncIter *it) {
    const HYPTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const HYPRegistryHashEntry *e = &reg->ffunc_short_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash)
...
int hyp_method_iter_next(HYPMethodIter *it) {
    ...
    const HYPTypeRegistry *reg = it->reg;
    while (it->chain_idx >= 0) {
        const HYPRegistryHashEntry *e = &reg->method_entries[it->chain_idx];
        int p = e->payload_index;
        uint64_t h = e->hash;
        it->chain_idx = e->next_index;
        if (h != it->hash) {
        ...
        const HYPRegisteredFunc *f = &reg->funcs[p];
...
void hyp_registry_finalize(HYPTypeRegistry *reg) {
    if (!reg)
        return;
    hyp_registry_finalize_into(reg, reg->arena);
}

void hyp_registry_init(HYPTypeRegistry *reg, HYPArena *arena) {
    memset(reg, 0, sizeof(HYPTypeRegistry));
    reg->arena = arena;
}
...
void hyp_registry_add_func(HYPTypeRegistry *reg, HYPRegisteredFunc func) {
    ...
    if (reg->func_count >= reg->func_cap) {
        ...
        HYPRegisteredFunc *new_items = (HYPRegisteredFunc *)hyp_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(HYPRegisteredFunc));
        if (!new_items)
            return;
        if (reg->funcs && reg->func_count > 0) {
            memcpy(new_items, reg->funcs, (size_t)reg->func_count * sizeof(HYPRegisteredFunc));
        }
        reg->funcs = new_items;
...
void hyp_registry_add_type(HYPTypeRegistry *reg, HYPRegisteredType type) {
    ...
    if (reg->type_count >= reg->type_cap) {
        ...
        HYPRegisteredType *new_items = (HYPRegisteredType *)hyp_arena_alloc(
            reg->arena, (size_t)new_cap * sizeof(HYPRegisteredType));
        if (!new_items)
            return;
        if (reg->types && reg->type_count > 0) {
            memcpy(new_items, reg->types, (size_t)reg->type_count * sizeof(HYPRegisteredType));
        }
        reg->types = new_items;
...
static const HYPRegisteredFunc *lookup_method_self(const HYPTypeRegistry *reg,
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            if (reg->method_entries[idx].hash != h)
                continue;
            const HYPRegisteredFunc *f = &reg->funcs[reg->method_entries[idx].payload_index];
...
const HYPRegisteredFunc *hyp_registry_lookup_method(const HYPTypeRegistry *reg,
    ...
    if (!reg || !receiver_qn || !method_name)
        return NULL;
    const HYPRegisteredFunc *r = lookup_method_self(reg, receiver_qn, method_name);
    if (!r && reg->fallback) {
        return hyp_registry_lookup_method(reg->fallback, receiver_qn, method_name);
    }
    return r;
}

static const HYPRegisteredType *lookup_type_self(const HYPTypeRegistry *reg,
...
const HYPRegisteredType *hyp_registry_lookup_type(const HYPTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const HYPRegisteredType *r = lookup_type_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return hyp_registry_lookup_type(reg->fallback, qualified_name);
    }
    return r;
}

static const HYPRegisteredFunc *lookup_func_self(const HYPTypeRegistry *reg,
                                                 const char *qualified_name) {
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        uint64_t h = fnv1a(qualified_name);
...
const HYPRegisteredFunc *hyp_registry_lookup_func(const HYPTypeRegistry *reg,
    ...
    if (!reg || !qualified_name)
        return NULL;
    const HYPRegisteredFunc *r = lookup_func_self(reg, qualified_name);
    if (!r && reg->fallback) {
        return hyp_registry_lookup_func(reg->fallback, qualified_name);
    }
    return r;
}
...
const HYPRegisteredType *hyp_registry_resolve_alias(const HYPTypeRegistry *reg,
    ...
    if (!reg || !type_qn)
        return NULL;
    const HYPRegisteredType *rt = hyp_registry_lookup_type(reg, type_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        const HYPRegisteredType *next = hyp_registry_lookup_type(reg, rt->alias_of);
        if (!next)
            return rt;
        rt = next;
    }
...
const HYPRegisteredFunc *hyp_registry_lookup_method_aliased(const HYPTypeRegistry *reg,
    ...
    // Direct lookup first
    const HYPRegisteredFunc *f = hyp_registry_lookup_method(reg, receiver_qn, method_name);
    if (f)
        return f;

    // Follow alias chain
    const HYPRegisteredType *rt = hyp_registry_lookup_type(reg, receiver_qn);
    for (int i = 0; i < 16 && rt && rt->alias_of; i++) {
        f = hyp_registry_lookup_method(reg, rt->alias_of, method_name);
        if (f)
            return f;
        rt = hyp_registry_lookup_type(reg, rt->alias_of);
    }
    return NULL;
}
...
const HYPRegisteredFunc *hyp_registry_lookup_symbol(const HYPTypeRegistry *reg,
    ...
    return hyp_registry_lookup_func(reg, buf);
}

// Count parameters in a FUNC signature.
static int count_func_params(const HYPRegisteredFunc *f) {
...
const HYPRegisteredFunc *hyp_registry_lookup_method_by_args(const HYPTypeRegistry *reg,
    ...
    const HYPRegisteredFunc *first_match = NULL;
    const HYPRegisteredFunc *range_match = NULL;
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        for (int idx = reg->method_buckets[slot]; idx >= 0;
            ...
            const HYPRegisteredFunc *f = &reg->funcs[pi];
            if (!f->receiver_type || !f->short_name || strcmp(f->receiver_type, receiver_qn) != 0 ||
                strcmp(f->short_name, method_name) != 0)
                continue;
        ...
        if (reg->fallback) {
            return hyp_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                      arg_count);
        }
        return NULL;
    }
    ...
    const HYPRegisteredFunc *res = range_match ? range_match : first_match;
    if (!res && reg->fallback) {
        return hyp_registry_lookup_method_by_args(reg->fallback, receiver_qn, method_name,
                                                  arg_count);
    }
...
static const char *type_to_qn_simple(const HYPType *t) {
    if (!t)
        return NULL;
...
static int score_overload_match(const HYPRegisteredFunc *f, const HYPType **arg_types,
                                int arg_count) {
    int pc = count_func_params(f);
    int min_pc = (f->min_params >= 0) ? f->min_params : pc;
    if (arg_count < min_pc || arg_count > pc)
    ...
    for (int i = 0; i < arg_count; i++) {
        const HYPType *expected = f->signature->data.func.param_types[i];
        const HYPType *actual = arg_types[i];
        if (!expected || !actual || hyp_type_is_unknown(actual))
            continue; // neutral
...
const HYPRegisteredFunc *hyp_registry_lookup_method_by_types(const HYPTypeRegistry *reg,
    ...
    if (!arg_types)
        return hyp_registry_lookup_method_by_args(reg, receiver_qn, method_name, arg_count);
    ...
    if (reg->method_buckets && reg->method_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return hyp_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                       arg_types, arg_count);
        }
        return NULL;
    }
    ...
    const HYPRegisteredFunc *res = best ? best : first_match;
    if (!res && reg->fallback) {
        return hyp_registry_lookup_method_by_types(reg->fallback, receiver_qn, method_name,
                                                   arg_types, arg_count);
    }
...
const HYPRegisteredFunc *hyp_registry_lookup_symbol_by_types(const HYPTypeRegistry *reg,
    ...
    if (!reg || !package_qn || !name)
        return NULL;
    if (!arg_types)
        return hyp_registry_lookup_symbol_by_args(reg, package_qn, name, arg_count);
    ...
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return hyp_registry_lookup_symbol_by_types(reg->fallback, package_qn, name, arg_types,
                                                       arg_count);
        }
        return NULL;
    }
...
const HYPRegisteredFunc *hyp_registry_lookup_symbol_by_args(const HYPTypeRegistry *reg,
    ...
    // hyp_registry_lookup_symbol_by_types). O(overloads) not O(func_count).
    if (reg->func_qn_buckets && reg->func_qn_bucket_count > 0) {
        int exact_pi = -1, range_pi = -1, first_pi = -1;
        uint64_t h = fnv1a(buf);
        int slot = (int)(h & (uint64_t)(reg->func_qn_bucket_count - 1));
        ...
        if (sel >= 0)
            return &reg->funcs[sel];
        if (reg->fallback) {
            return hyp_registry_lookup_symbol_by_args(reg->fallback, package_qn, name, arg_count);
        }
        return NULL;
    }
...
const HYPRegisteredFunc *hyp_registry_lookup_callable(const HYPTypeRegistry *reg, HYPArena *arena,
    ...
    // Synthesise a HYPRegisteredFunc whose QN is "<type_qn>.__call".
    ...
    char *qn = (char *)hyp_arena_alloc(arena, total);
    if (!qn)
        return NULL;
    memcpy(qn, type_qn, qn_len);
    memcpy(qn + qn_len, suffix, sizeof(suffix));

    HYPRegisteredFunc *f = (HYPRegisteredFunc *)hyp_arena_alloc(arena, sizeof(HYPRegisteredFunc));
    if (!f)
        return NULL;
    memset(f, 0, sizeof(*f));
    f->qualified_name = qn;
...
const HYPType *hyp_registry_lookup_index_signature(const HYPTypeRegistry *reg, const char *type_qn,
    ...
    if (rt->index_key_type->kind == HYP_TYPE_BUILTIN && key_type->kind == HYP_TYPE_BUILTIN) {
