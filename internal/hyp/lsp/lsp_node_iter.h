 ...
 * run for ~133 minutes). hyp_lsp_collect_children() enumerates the children in
 * a single O(n) cursor pass into an arena array; callers then index the array.
 */
#ifndef HYP_LSP_NODE_ITER_H
#define HYP_LSP_NODE_ITER_H
...
static inline TSNode* hyp_lsp_collect_children(HYPArena* arena, TSNode node, uint32_t* out_n) {
    uint32_t nc = ts_node_child_count(node);
    *out_n = 0;
    if (nc == 0) return NULL;
