 ...
 *
 * Architecture mirrors go_lsp / py_lsp: build a `HYPTypeRegistry` from the
 ...
 *
 * Public entry points:
 *   - `hyp_run_rust_lsp` — single-file resolution invoked from
 *     `hyp_extract_file()`.
 *   - `hyp_run_rust_lsp_cross` — cross-file resolution given a list of
 *     `HYPLSPDef`s gathered by the pipeline.
 *   - `hyp_batch_rust_lsp_cross` — batch wrapper that processes several
 *     files in one CGo call (per-file arenas + result copy).
 */
#ifndef HYP_LSP_RUST_LSP_H
#define HYP_LSP_RUST_LSP_H
...
#include "../hyp.h"
#include "go_lsp.h" // for HYPLSPDef (pipeline def), used by the Tier-2 builder
 ...
 * scoring >= HYP_LSP_CONFIDENCE_FLOOR (0.6). Numbers here mirror the Go
 * LSP so the call-edge mix from a polyglot project stays comparable. */
#define HYP_RUST_CONF_DIRECT 0.95f      /* path::to::func or alias hit  */
#define HYP_RUST_CONF_METHOD 0.95f      /* inherent method dispatch     */
#define HYP_RUST_CONF_TRAIT_SOLE 0.92f  /* trait method, single impl    */
#define HYP_RUST_CONF_TRAIT_AMB 0.85f   /* trait method, many impls     */
#define HYP_RUST_CONF_UFCS 0.93f        /* T::method() / Self::new()    */
#define HYP_RUST_CONF_PROMOTED 0.90f    /* through Deref / blanket impl */
#define HYP_RUST_CONF_MACRO_KNOWN 0.85f /* known std macro mapped to fn */
#define HYP_RUST_CONF_OPERATOR 0.88f    /* a+b → T::add (operator trait) */

/* Rust-flavoured LSP context: one per file, lifetime tied to a single
 * `hyp_extract_file()` invocation (or the cross-file caller's arena). */
typedef struct {
    HYPArena *arena;
    const char *source;
    int source_len;
     ...
     * to every emitted HYPResolvedCall as `caller_qn`. */
    ...
    const HYPType *pending_closure_param_type;
    ...
    const struct HYPCargoManifest *cargo_manifest;
     ...
     * into a CALLS edge when a *syntactic* HYPCall with the same
    ...
    /* HYP_LSP_DEBUG=1 in env enables verbose stderr trace. */
    bool debug;
 ...
 * `hyp_type_unknown()` for anything we cannot type. */
 ...
 * reference_type, primitive_type, …) into a HYPType. */
...
/* Populate the same per-file registry used by hyp_run_rust_lsp, including
 ...
 * `HYPLSPDef` in `go_lsp.h` for compatibility with the pipeline plumbing
...
/* Same as `hyp_run_rust_lsp_cross`, plus an optional parsed Cargo manifest
