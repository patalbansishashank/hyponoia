 ...
 * (reg->read_only, set at finalize; hyp_registry_add_* hard-return then) and
 ...
 *
 * Per-language wiring is a few lines: add a HYPNegMemo to the language ctx,
 * key each cascade entry with hyp_negmemo_key (site tag + query strings),
 ...
 */
#ifndef HYP_LSP_NEG_MEMO_H
#define HYP_LSP_NEG_MEMO_H
...
enum {
    HYP_NEGMEMO_INIT_CAP = 1024,
    HYP_NEGMEMO_GROW = 2,
    HYP_NEGMEMO_LOAD_NUM = 7, /* grow at 70% load */
    HYP_NEGMEMO_LOAD_DEN = 10,
};
...
static inline uint64_t hyp_negmemo_key(uint8_t site, const char *a, const char *b) {
...
static inline bool hyp_negmemo_contains(const HYPNegMemo *m, uint64_t key) {
...
static inline void hyp_negmemo_insert_raw(uint64_t *slots, int cap, uint64_t key) {
...
static inline void hyp_negmemo_insert(HYPNegMemo *m, HYPArena *arena, uint64_t key) {
    if (!arena) {
        return; /* no arena — memo silently disabled */
    }
    if (!m->slots) {
        m->slots = hyp_arena_alloc(arena, sizeof(uint64_t) * HYP_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(uint64_t) * HYP_NEGMEMO_INIT_CAP);
        m->cap = HYP_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * HYP_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * HYP_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * HYP_NEGMEMO_GROW;
        uint64_t *ns = hyp_arena_alloc(arena, sizeof(uint64_t) * (size_t)new_cap);
        if (!ns) {
        ...
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i]) {
                hyp_negmemo_insert_raw(ns, new_cap, m->slots[i]);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    }
    if (!hyp_negmemo_contains(m, key)) {
        hyp_negmemo_insert_raw(m->slots, m->cap, key);
        m->count++;
    }
}

/* ── HYPIdxMemo: exact-match string-key → int index map ─────────────────────
...
static inline int32_t hyp_idxmemo_get(const HYPIdxMemo *m, const char *key) {
    if (!m->slots || !key || m->count == 0) {
        return -1;
    }
    uint64_t h = hyp_negmemo_key(0, key, NULL);
    uint64_t mask = (uint64_t)(m->cap - 1);
...
static inline void hyp_idxmemo_put_raw(struct hyp_idxmemo_slot *slots, int cap, uint64_t h,
...
static inline void hyp_idxmemo_put_if_absent(HYPIdxMemo *m, HYPArena *arena, const char *key,
                                             int32_t val) {
    if (!arena || !key) {
        return;
    }
    if (!m->slots) {
        m->slots = hyp_arena_alloc(arena,
                                   sizeof(struct hyp_idxmemo_slot) * HYP_NEGMEMO_INIT_CAP);
        if (!m->slots) {
            return;
        }
        memset(m->slots, 0, sizeof(struct hyp_idxmemo_slot) * HYP_NEGMEMO_INIT_CAP);
        m->cap = HYP_NEGMEMO_INIT_CAP;
        m->count = 0;
    }
    if ((int64_t)(m->count + 1) * HYP_NEGMEMO_LOAD_DEN >=
        (int64_t)m->cap * HYP_NEGMEMO_LOAD_NUM) {
        int new_cap = m->cap * HYP_NEGMEMO_GROW;
        struct hyp_idxmemo_slot *ns =
            hyp_arena_alloc(arena, sizeof(struct hyp_idxmemo_slot) * (size_t)new_cap);
        if (!ns) {
            return;
        }
        memset(ns, 0, sizeof(struct hyp_idxmemo_slot) * (size_t)new_cap);
        for (int i = 0; i < m->cap; i++) {
            if (m->slots[i].h) {
                hyp_idxmemo_put_raw(ns, new_cap, m->slots[i].h, m->slots[i].key, m->slots[i].val);
            }
        }
        m->slots = ns;
        m->cap = new_cap;
    ...
    uint64_t h = hyp_negmemo_key(0, key, NULL);
    if (hyp_idxmemo_get(m, key) < 0) {
        hyp_idxmemo_put_raw(m->slots, m->cap, h, key, val);
        m->count++;
    }
}

#endif /* HYP_LSP_NEG_MEMO_H */
