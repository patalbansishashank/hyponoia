#ifndef HYP_LSP_PY_LSP_H
#define HYP_LSP_PY_LSP_H
...
typedef struct {
    ...
    const HYPType *result; // full-fidelity result, never NULL in a live entry
} HYPPyTypeCacheEntry;
...
typedef struct {
    ...
    // Built from HYPFileResult.imports.
    ...
    // non-NULL, py_emit_dunder_call injects a matching synthetic HYPCall so the
    ...
    HYPPyTypeCacheEntry *type_cache; // open addressing, linear probe, arena-allocated
    ...
    struct {
        ...
        const HYPType *field_type;
    } *field_overlay;
    int field_overlay_count;
    int field_overlay_cap;

    // Debug mode (HYP_LSP_DEBUG env, shared across all language LSPs).
    bool debug;
} PyLSPContext;
...
void py_lsp_init(PyLSPContext *ctx, HYPArena *arena, const char *source, int source_len,
                 const HYPTypeRegistry *registry, const char *module_qn, HYPResolvedCallArray *out);

// Add an import mapping (call once per HYPImport entry).
...
// Mirrors hyp_run_go_lsp / hyp_run_c_lsp.
