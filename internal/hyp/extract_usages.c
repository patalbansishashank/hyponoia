...
// Forward declaration
static void walk_usages(HYPExtractCtx *ctx, TSNode root, const HYPLangSpec *spec);
...
static bool is_inside_call(TSNode node, const HYPLangSpec *spec) {
    TSNode cur = ts_node_parent(node);
    while (!ts_node_is_null(cur)) {
        if (hyp_kind_in_set(cur, spec->call_node_types)) {
            return true;
        }
...
static bool is_inside_import(TSNode node, const HYPLangSpec *spec) {
    ...
    TSNode cur = ts_node_parent(node);
    while (!ts_node_is_null(cur)) {
        if ((has_imports && hyp_kind_in_set(cur, spec->import_node_types)) ||
            (has_from_imports && hyp_kind_in_set(cur, spec->import_from_types))) {
            return true;
        }
        cur = ts_node_parent(cur);
    }
...
static bool is_reference_node(TSNode node, HYPLanguage lang) {
    ...
    if (lang == HYP_LANG_PYTHON && strcmp(kind, "attribute") == 0 &&
    ...
    if ((lang == HYP_LANG_PUPPET || lang == HYP_LANG_VIMSCRIPT) &&
        strcmp(kind, "identifier") == 0) {
        TSNode parent = ts_node_parent(node);
        if (!ts_node_is_null(parent) &&
            ((lang == HYP_LANG_PUPPET && strcmp(ts_node_type(parent), "variable") == 0) ||
             (lang == HYP_LANG_VIMSCRIPT && strcmp(ts_node_type(parent), "argument") == 0))) {
            return false;
        }
    }
    ...
    case HYP_LANG_CFML:
        return strcmp(kind, "property_identifier") == 0;
    case HYP_LANG_OBJECTSCRIPT_UDL:
    case HYP_LANG_OBJECTSCRIPT_ROUTINE:
...
static bool is_forward_sibling_callee(HYPLanguage language, TSNode node) {
    if (language == HYP_LANG_DART && strcmp(ts_node_type(node), "identifier") == 0) {
...
typedef enum {
    HYP_OCCURRENCE_STANDARD = 0,
    HYP_OCCURRENCE_LISP_DEF,
    HYP_OCCURRENCE_COMMONLISP_DEFUN,
    HYP_OCCURRENCE_FENNEL_FN,
    HYP_OCCURRENCE_ELIXIR_DEF,
    HYP_OCCURRENCE_JULIA_FUNCTION,
    HYP_OCCURRENCE_WOLFRAM_SET,
    HYP_OCCURRENCE_TYPST_LET,
    HYP_OCCURRENCE_AGDA_FUNCTION,
    HYP_OCCURRENCE_TLAPLUS_OPERATOR,
    HYP_OCCURRENCE_COBOL_MOVE,
    HYP_OCCURRENCE_HCL_ATTRIBUTE,
    HYP_OCCURRENCE_ELM_VALUE,
    HYP_OCCURRENCE_RESCRIPT_LET,
    HYP_OCCURRENCE_PURESCRIPT_LHS,
    HYP_OCCURRENCE_NICKEL_LET,
    HYP_OCCURRENCE_ERLANG_CLAUSE,
    HYP_OCCURRENCE_NIX_FUNCTION,
    HYP_OCCURRENCE_MATLAB_ARGUMENTS,
    HYP_OCCURRENCE_LEAN_BINDER,
    HYP_OCCURRENCE_PASCAL_PROC,
    HYP_OCCURRENCE_TEAL_FUNCTION,
    HYP_OCCURRENCE_VHDL_INTERFACE,
    HYP_OCCURRENCE_PINE_FUNCTION,
    HYP_OCCURRENCE_LLVM_FUNCTION,
} HYPOccurrencePolicy;

typedef struct {
    const char *const *whole_binding_nodes;
    const char *const *write_nodes;
    HYPOccurrencePolicy policy;
 ...
 * the positional HYPLangSpec initializer used by every language. */
static const HYPOccurrenceSpec occurrence_specs[HYP_LANG_COUNT] = {
    [HYP_LANG_SQL] = {sql_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_CLOJURE] = {NULL, NULL, HYP_OCCURRENCE_LISP_DEF, false},
    [HYP_LANG_SCHEME] = {NULL, NULL, HYP_OCCURRENCE_LISP_DEF, false},
    [HYP_LANG_RACKET] = {NULL, NULL, HYP_OCCURRENCE_LISP_DEF, false},
    [HYP_LANG_COMMONLISP] = {NULL, NULL, HYP_OCCURRENCE_COMMONLISP_DEFUN, false},
    [HYP_LANG_FENNEL] = {NULL, NULL, HYP_OCCURRENCE_FENNEL_FN, false},
    [HYP_LANG_ELIXIR] = {NULL, NULL, HYP_OCCURRENCE_ELIXIR_DEF, false},
    [HYP_LANG_JULIA] = {NULL, NULL, HYP_OCCURRENCE_JULIA_FUNCTION, false},
    [HYP_LANG_WOLFRAM] = {NULL, NULL, HYP_OCCURRENCE_WOLFRAM_SET, false},
    [HYP_LANG_TYPST] = {NULL, NULL, HYP_OCCURRENCE_TYPST_LET, false},
    [HYP_LANG_AGDA] = {NULL, NULL, HYP_OCCURRENCE_AGDA_FUNCTION, false},
    [HYP_LANG_TLAPLUS] = {NULL, NULL, HYP_OCCURRENCE_TLAPLUS_OPERATOR, false},
    [HYP_LANG_COBOL] = {NULL, NULL, HYP_OCCURRENCE_COBOL_MOVE, false},
    [HYP_LANG_HCL] = {NULL, NULL, HYP_OCCURRENCE_HCL_ATTRIBUTE, false},
    [HYP_LANG_ELM] = {NULL, NULL, HYP_OCCURRENCE_ELM_VALUE, false},
    [HYP_LANG_RESCRIPT] = {rescript_binding_nodes, NULL, HYP_OCCURRENCE_RESCRIPT_LET, false},
    [HYP_LANG_PURESCRIPT] = {purescript_binding_nodes, NULL, HYP_OCCURRENCE_PURESCRIPT_LHS, false},
    [HYP_LANG_NICKEL] = {nickel_binding_nodes, NULL, HYP_OCCURRENCE_NICKEL_LET, false},
    [HYP_LANG_JSONNET] = {jsonnet_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_HASKELL] = {haskell_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_ERLANG] = {NULL, NULL, HYP_OCCURRENCE_ERLANG_CLAUSE, false},
    [HYP_LANG_FSHARP] = {fsharp_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_NIX] = {NULL, NULL, HYP_OCCURRENCE_NIX_FUNCTION, false},
    [HYP_LANG_MATLAB] = {NULL, NULL, HYP_OCCURRENCE_MATLAB_ARGUMENTS, false},
    [HYP_LANG_LEAN] = {NULL, NULL, HYP_OCCURRENCE_LEAN_BINDER, false},
    [HYP_LANG_PASCAL] = {NULL, NULL, HYP_OCCURRENCE_PASCAL_PROC, false},
    [HYP_LANG_VERILOG] = {systemverilog_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_AWK] = {awk_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_CRYSTAL] = {crystal_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_TEAL] = {teal_binding_nodes, NULL, HYP_OCCURRENCE_TEAL_FUNCTION, false},
    [HYP_LANG_VHDL] = {NULL, NULL, HYP_OCCURRENCE_VHDL_INTERFACE, false},
    [HYP_LANG_SYSTEMVERILOG] = {systemverilog_binding_nodes, NULL, HYP_OCCURRENCE_STANDARD, false},
    [HYP_LANG_PINE] = {NULL, NULL, HYP_OCCURRENCE_PINE_FUNCTION, false},
    [HYP_LANG_PUPPET] = {NULL, NULL, HYP_OCCURRENCE_STANDARD, true},
    [HYP_LANG_LLVM_IR] = {llvm_binding_nodes, NULL, HYP_OCCURRENCE_LLVM_FUNCTION, false},
    [HYP_LANG_MESON] = {NULL, meson_write_nodes, HYP_OCCURRENCE_STANDARD, true},
    [HYP_LANG_GN] = {NULL, gn_write_nodes, HYP_OCCURRENCE_STANDARD, true},
    [HYP_LANG_LINKERSCRIPT] = {NULL, linkerscript_write_nodes, HYP_OCCURRENCE_STANDARD, true},
    [HYP_LANG_OBJECTSCRIPT_UDL] = {objectscript_binding_nodes, objectscript_write_nodes,
                                   HYP_OCCURRENCE_STANDARD, true},
    [HYP_LANG_OBJECTSCRIPT_ROUTINE] = {objectscript_binding_nodes, objectscript_write_nodes,
                                       HYP_OCCURRENCE_STANDARD, true},
};

static bool text_equals(HYPExtractCtx *ctx, TSNode node, const char *expected) {
    char *text = hyp_node_text(ctx->arena, node, ctx->source);
...
static bool is_lisp_def_binding(HYPExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
        ...
        char *head = hyp_node_text(ctx->arena, head_node, ctx->source);
        ...
        if (ctx->language == HYP_LANG_CLOJURE && ts_node_named_child_count(form) > 2 &&
...
static bool is_fennel_fn_binding(HYPExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool is_elixir_def_binding(HYPExtractCtx *ctx, TSNode node) {
    for (TSNode form = ts_node_parent(node); !ts_node_is_null(form); form = ts_node_parent(form)) {
...
static bool fish_argument_marker(HYPExtractCtx *ctx, TSNode node) {
    char *text = hyp_node_text(ctx->arena, node, ctx->source);
    return text && (strcmp(text, "-a") == 0 || strcmp(text, "--argument") == 0 ||
                    strcmp(text, "--argument-names") == 0);
}

static bool is_fish_function_parameter(HYPExtractCtx *ctx, TSNode node, WalkState *state) {
    ...
    for (TSNode previous = ts_node_prev_named_sibling(node); !ts_node_is_null(previous);
        ...
        char *text = hyp_node_text(ctx->arena, previous, ctx->source);
        if (!text || text[0] != '-') {
            continue;
        }
...
static bool cfml_argument_tag_name_binding(HYPExtractCtx *ctx, TSNode node) {
    ...
    TSNode attribute_name = hyp_find_child_by_kind(attribute, "cf_attribute_name");
    char *name_text = hyp_node_text(ctx->arena, attribute_name, ctx->source);
    ...
    char *tag_text = hyp_node_text(ctx->arena, tag, ctx->source);
...
static bool is_exact_language_binding(HYPExtractCtx *ctx, TSNode node, WalkState *state) {
    const char *kind = ts_node_type(node);
    switch (ctx->language) {
    case HYP_LANG_OCAML: {
...
static bool is_policy_binding(HYPExtractCtx *ctx, TSNode node,
                              const HYPOccurrenceSpec *occurrence) {
    switch (occurrence->policy) {
    case HYP_OCCURRENCE_LISP_DEF:
        return is_lisp_def_binding(ctx, node);
    case HYP_OCCURRENCE_COMMONLISP_DEFUN:
        return is_commonlisp_defun_binding(node);
    case HYP_OCCURRENCE_FENNEL_FN:
        return is_fennel_fn_binding(ctx, node);
    case HYP_OCCURRENCE_ELIXIR_DEF:
        return is_elixir_def_binding(ctx, node);
    case HYP_OCCURRENCE_JULIA_FUNCTION:
        return is_first_named_part_of(node, "function_definition");
    case HYP_OCCURRENCE_WOLFRAM_SET:
        return is_wolfram_lhs(node);
    case HYP_OCCURRENCE_TYPST_LET:
        return is_first_named_part_of(node, "let");
    case HYP_OCCURRENCE_AGDA_FUNCTION:
        for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
             parent = ts_node_parent(parent)) {
    ...
    case HYP_OCCURRENCE_LLVM_FUNCTION:
        ...
        return false;
    case HYP_OCCURRENCE_STANDARD:
    default:
        return false;
    }
}

static bool elixir_binary_operator_binds(HYPExtractCtx *ctx, TSNode node) {
    if (ctx->language != HYP_LANG_ELIXIR || strcmp(ts_node_type(node), "binary_operator") != 0) {
        return false;
    }
...
static bool is_binding_occurrence(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec,
                                  WalkState *state) {
    const HYPOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    ...
    while (!ts_node_is_null(current)) {
        ...
        bool variable_container =
            spec->variable_node_types && hyp_kind_in_set(parent, spec->variable_node_types);
        if (ctx->language == HYP_LANG_ELIXIR && strcmp(kind, "binary_operator") == 0) {
        ...
        bool declared_container =
            ...
            (spec->function_node_types && hyp_kind_in_set(parent, spec->function_node_types)) ||
            (spec->class_node_types && hyp_kind_in_set(parent, spec->class_node_types)) ||
            (spec->field_node_types && hyp_kind_in_set(parent, spec->field_node_types)) ||
            variable_container;
...
static bool is_write_occurrence(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec,
                                WalkState *state) {
    const HYPOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    TSNode current = node;
    TSTreeCursor *cursor = reset_occurrence_cursor(state, node);
    while (!ts_node_is_null(current)) {
        ...
        bool assignment =
            (spec->assignment_node_types && hyp_kind_in_set(parent, spec->assignment_node_types)) ||
            kind_in_exact_set(ts_node_type(parent), occurrence->write_nodes);
        if (ctx->language == HYP_LANG_ELIXIR &&
        ...
        if (assignment) {
            ...
            if (ctx->language == HYP_LANG_LINKERSCRIPT &&
...
static bool language_may_stamp_exact_callable_value_candidate(HYPLanguage language) {
    switch (language) {
    case HYP_LANG_JAVASCRIPT:
    case HYP_LANG_TYPESCRIPT:
    case HYP_LANG_TSX:
...
static TSNode call_reference_candidate_site(HYPExtractCtx *ctx, TSNode node, const char *name,
                                            WalkState *state) {
    ...
    bool ts_family = ctx->language == HYP_LANG_JAVASCRIPT || ctx->language == HYP_LANG_TYPESCRIPT ||
    ...
    if (ctx->language == HYP_LANG_PYTHON && strcmp(kind, "identifier") == 0 &&
    ...
    if (ctx->language == HYP_LANG_GO && strcmp(kind, "field_identifier") == 0 &&
    ...
    if (ctx->language == HYP_LANG_RUST && strcmp(kind, "scoped_identifier") == 0) {
     ...
     * as before. */
    if (ctx->language == HYP_LANG_KOTLIN && !ts_node_is_null(parent) &&
    ...
    if (ctx->language == HYP_LANG_PYTHON && !ts_node_is_null(parent) &&
        strcmp(ts_node_type(parent), "assignment") == 0) {
...
static char *reference_name(HYPExtractCtx *ctx, TSNode node) {
    char *name = hyp_node_text(ctx->arena, node, ctx->source);
    if (!name || ctx->language != HYP_LANG_MAKEFILE ||
    ...
    return parenthesized ? hyp_arena_strndup(ctx->arena, name + 2, length - 3) : name;
}

static bool lexical_identifier_case_insensitive(HYPLanguage language) {
    switch (language) {
    case HYP_LANG_POWERSHELL:
...
static const char *lexical_binding_key(HYPExtractCtx *ctx, WalkState *state, const char *name) {
    if (!ctx || !name) {
        return name;
    }
    ...
    if (ctx->language == HYP_LANG_TCL && name[0] == '$') {
        name++;
    }
    ...
    char *folded = (char *)hyp_arena_alloc(ctx->arena, n + 1U);
    if (!folded) {
        if (state) {
...
static void stamp_usage_site(HYPExtractCtx *ctx, HYPUsage *usage, TSNode node, const char *name,
                             WalkState *state) {
...
static const HYPLexicalScope *usage_lexical_scope(const WalkState *state, uint32_t id) {
...
static uint32_t usage_lexical_scope_id_for_node(HYPExtractCtx *ctx, const WalkState *state,
                                                TSNode node) {
    uint32_t scope_id = active_lexical_scope_id(state);
    if (!ctx || ctx->language != HYP_LANG_PYTHON || !python_default_value_reference(node)) {
        return scope_id;
    }
    uint32_t function_id = lexical_ancestor_of_kind(state, scope_id, true, false);
    const HYPLexicalScope *function_scope = usage_lexical_scope(state, function_id);
...
static uint32_t lexical_ancestor_of_kind(const WalkState *state, uint32_t start_id,
    ...
    while (id != 0 && remaining-- > 0) {
        const HYPLexicalScope *scope = usage_lexical_scope(state, id);
        if (!scope) {
            return 0;
        }
        bool function_scope = scope->kind == HYP_LEXICAL_SCOPE_FUNCTION ||
                              scope->kind == HYP_LEXICAL_SCOPE_COMPREHENSION;
        bool block_scope = scope->kind == HYP_LEXICAL_SCOPE_BLOCK;
...
static uint32_t python_nearest_namespace(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == HYP_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == HYP_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == HYP_LEXICAL_SCOPE_CLASS || scope->kind == HYP_LEXICAL_SCOPE_MODULE) {
            return id;
        }
        id = scope->parent_id;
    }
...
static bool ensure_lexical_binding_capacity(WalkState *state) {
    ...
    HYPLexicalBinding *grown =
        (HYPLexicalBinding *)hyp_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static TSNode declared_function_name_owner(TSNode node, const HYPLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->function_node_types && hyp_kind_in_set(parent, spec->function_node_types)) {
...
static TSNode declared_class_name_owner(TSNode node, const HYPLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        if (spec->class_node_types && hyp_kind_in_set(parent, spec->class_node_types)) {
            return field_contains_node(parent, "name", node) ? parent : (TSNode){0};
        }
    }
    return (TSNode){0};
...
static bool ensure_python_directive_capacity(WalkState *state) {
    ...
    HYPPythonDirective *grown =
        (HYPPythonDirective *)hyp_arena_alloc(state->arena, (size_t)new_capacity * sizeof(*grown));
    if (!grown) {
        state->lexical_binding_tracking_failed = true;
        return false;
    }
...
static void record_python_directive(WalkState *state, uint32_t function_scope_id, const char *name,
    ...
    HYPPythonDirective *directive = &state->python_directives[state->python_directive_count++];
    directive->function_scope_id = function_scope_id;
    directive->name = name;
    directive->kind = (uint8_t)kind;
}

static HYPPythonDirectiveKind python_directive_for(const WalkState *state,
                                                   uint32_t function_scope_id, const char *name) {
    for (int i = state ? state->python_directive_count - 1 : -1; i >= 0; i--) {
        const HYPPythonDirective *directive = &state->python_directives[i];
        if (directive->function_scope_id == function_scope_id && directive->name &&
            strcmp(directive->name, name) == 0) {
            return (HYPPythonDirectiveKind)directive->kind;
        }
    }
    return 0;
}
...
static uint32_t python_enclosing_function_namespace(const WalkState *state,
    ...
    const HYPLexicalScope *inner = usage_lexical_scope(state, inner_function_id);
...
static bool binding_is_parameter(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec) {
    const HYPOccurrenceSpec *occurrence = &occurrence_specs[ctx->language];
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
        const char *kind = ts_node_type(parent);
...
static bool import_kind_matches(TSNode node, const HYPLangSpec *spec) {
    return (spec->import_node_types && hyp_kind_in_set(node, spec->import_node_types)) ||
           (spec->import_from_types && hyp_kind_in_set(node, spec->import_from_types));
}

static TSNode nearest_import_ancestor(TSNode node, const HYPLangSpec *spec) {
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
         parent = ts_node_parent(parent)) {
 ...
 * name. */
static bool is_import_binding_occurrence(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec) {
...
static uint32_t lexical_import_scope(const WalkState *state, uint32_t start_id) {
    ...
    while (id != 0 && remaining-- > 0) {
        ...
        if (scope->kind == HYP_LEXICAL_SCOPE_MODULE || scope->kind == HYP_LEXICAL_SCOPE_FUNCTION ||
            scope->kind == HYP_LEXICAL_SCOPE_COMPREHENSION ||
            scope->kind == HYP_LEXICAL_SCOPE_BLOCK) {
            return id;
        }
        id = scope->parent_id;
    }
...
static void record_lexical_binding(HYPExtractCtx *ctx, WalkState *state, TSNode node,
                                   const HYPLangSpec *spec, const char *raw_name,
                                   bool import_binding) {
    if (!ctx || !state || !raw_name || !raw_name[0] || state->lexical_binding_tracking_failed) {
        return;
    }
    bool parameter = binding_is_parameter(ctx, node, spec);
    if (ctx->language == HYP_LANG_VIMSCRIPT && parameter && !strchr(raw_name, ':')) {
        raw_name = hyp_arena_sprintf(ctx->arena, "a:%s", raw_name);
        if (!raw_name) {
            state->lexical_binding_tracking_failed = true;
            return;
        }
    ...
    if (ctx->language == HYP_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"))) {
        uint32_t function_id = lexical_ancestor_of_kind(state, current_id, true, false);
        HYPPythonDirectiveKind directive = lexical_ancestor_kind(node, "global_statement")
                                               ? HYP_PYTHON_DIRECTIVE_GLOBAL
    ...
    if (!ts_node_is_null(function_declaration)) {
        ...
        if (ctx->language == HYP_LANG_PYTHON) {
            scope_id = python_nearest_namespace(state, parent_id);
            const HYPLexicalScope *owner = usage_lexical_scope(state, scope_id);
            if (!owner || (owner->kind != HYP_LEXICAL_SCOPE_FUNCTION &&
                           owner->kind != HYP_LEXICAL_SCOPE_COMPREHENSION)) {
                return;
            }
        } else {
        ...
        whole_scope = true;
    } else if (ctx->language == HYP_LANG_PYTHON && !ts_node_is_null(class_declaration)) {
        uint32_t class_id = python_nearest_namespace(state, current_id);
        const HYPLexicalScope *class_scope = usage_lexical_scope(state, class_id);
        ...
        whole_scope = true;
    } else if (ctx->language == HYP_LANG_PYTHON) {
        uint32_t namespace_id = python_nearest_namespace(state, current_id);
        const HYPLexicalScope *namespace_scope = usage_lexical_scope(state, namespace_id);
        uint32_t function_id =
            namespace_scope && (namespace_scope->kind == HYP_LEXICAL_SCOPE_FUNCTION ||
                                namespace_scope->kind == HYP_LEXICAL_SCOPE_COMPREHENSION)
                ? namespace_id
                : lexical_ancestor_of_kind(state, current_id, true, false);
        HYPPythonDirectiveKind directive = python_directive_for(state, function_id, name);
        if (directive == HYP_PYTHON_DIRECTIVE_GLOBAL) {
            scope_id = state->root_lexical_scope_id;
        } else if (directive == HYP_PYTHON_DIRECTIVE_NONLOCAL) {
        ...
        whole_scope = directive == 0 && (owner->kind == HYP_LEXICAL_SCOPE_FUNCTION ||
                                         owner->kind == HYP_LEXICAL_SCOPE_COMPREHENSION);
    } else if (ctx->language == HYP_LANG_POWERSHELL) {
        scope_id = lexical_ancestor_of_kind(state, current_id, true, false);
        whole_scope = true;
    } else if (ctx->language == HYP_LANG_JAVASCRIPT || ctx->language == HYP_LANG_TYPESCRIPT ||
               ctx->language == HYP_LANG_TSX) {
    ...
    } else if (ctx->language == HYP_LANG_RUST && import_binding) {
    ...
    const HYPLexicalScope *scope = usage_lexical_scope(state, scope_id);
    if (!scope || !ensure_lexical_binding_capacity(state)) {
        return;
    }
    HYPLexicalBinding *binding = &state->lexical_bindings[state->lexical_binding_count++];
...
static int compare_lexical_bindings(const void *left_ptr, const void *right_ptr) {
    const HYPLexicalBinding *left = (const HYPLexicalBinding *)left_ptr;
    const HYPLexicalBinding *right = (const HYPLexicalBinding *)right_ptr;
...
static int compare_binding_key(const HYPLexicalBinding *binding, uint32_t scope_id,
                               const char *name) {
...
static bool lexical_binding_active_at(const WalkState *state, uint32_t scope_id, const char *name,
    ...
    while (index < state->lexical_binding_count) {
        const HYPLexicalBinding *binding = &state->lexical_bindings[index++];
...
static const HYPLexicalScope *lexical_binding_scope_for_usage(const WalkState *state,
                                                              const HYPUsage *usage,
                                                              const char *name) {
...
void hyp_finalize_lexical_usages(HYPExtractCtx *ctx, WalkState *state) {
    ...
    for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
        HYPUsage *usage = &ctx->result->usages.items[i];
        ...
        const HYPLexicalScope *binding_scope = lexical_binding_scope_for_usage(state, usage, name);
        if (!binding_scope) {
            continue;
        }
        usage->semantic_reference_blocked = true;
        usage->semantic_reference_local_shadow = binding_scope->kind != HYP_LEXICAL_SCOPE_MODULE;
    }
    ...
    if (state->lexical_binding_tracking_failed) {
        for (int i = state->usage_start_index; i < ctx->result->usages.count; i++) {
            HYPUsage *usage = &ctx->result->usages.items[i];
            usage->semantic_reference_blocked = true;
            const HYPLexicalScope *scope = usage_lexical_scope(state, usage->lexical_scope_id);
            usage->semantic_reference_local_shadow =
                scope && scope->kind != HYP_LEXICAL_SCOPE_MODULE;
        }
    }
}

static char *perl_direct_coderef_name(HYPExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != HYP_LANG_PERL ||
    ...
    return end > start ? hyp_arena_strndup(ctx->arena, start, (size_t)(end - start)) : NULL;
}

static bool inside_direct_perl_coderef(HYPExtractCtx *ctx, TSNode node) {
    if (!ctx || ctx->language != HYP_LANG_PERL) {
        return false;
    }
    for (TSNode parent = ts_node_parent(node); !ts_node_is_null(parent);
...
static bool emit_direct_perl_coderef_usage(HYPExtractCtx *ctx, TSNode node,
    ...
    hyp_usages_push(&ctx->result->usages, ctx->arena, usage);
    return true;
}

// Try to emit a usage for a reference node. Returns early if the node should be skipped.
static void try_emit_usage(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec) {
    if (emit_direct_perl_coderef_usage(ctx, node, hyp_enclosing_func_qn_cached(ctx, node), 0)) {
        return;
    }
    ...
    if (name && name[0] && !hyp_is_keyword(name, ctx->language)) {
        HYPUsage usage = {0};
        usage.ref_name = name;
        usage.enclosing_func_qn = hyp_enclosing_func_qn_cached(ctx, node);
...
void handle_usages(HYPExtractCtx *ctx, TSNode node, const HYPLangSpec *spec, WalkState *state) {
    ...
    if (exact_callee) {
        if (reference_node && state->invocation_kind == HYP_INVOCATION_CALLABLE_REFERENCE) {
            char *name = reference_name(ctx, node);
            if (name && name[0] && !hyp_is_keyword(name, ctx->language)) {
                HYPUsage usage = {0};
                usage.ref_name = name;
    ...
    if (state->inside_import) {
        if (is_import_binding_occurrence(ctx, node, spec)) {
            ...
            if (binding_name && binding_name[0] && !hyp_is_keyword(binding_name, ctx->language)) {
    ...
    bool python_scope_directive =
        ctx->language == HYP_LANG_PYTHON && (lexical_ancestor_kind(node, "global_statement") ||
                                             lexical_ancestor_kind(node, "nonlocal_statement"));
