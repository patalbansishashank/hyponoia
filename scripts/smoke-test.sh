...
shopt -s nullglob
SMOKE_UI_PACK_CANDIDATES=("$(dirname "$BINARY")"/hyp-ui-*.pack)
shopt -u nullglob
...
smoke_mktemp_file() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp "$SMOKE_TEMP_ROOT/hyp-smoke.XXXXXX"
  else
    mktemp
  fi
}
...
smoke_mktemp_dir() {
  if [ -n "${SMOKE_TEMP_ROOT:-}" ]; then
    mktemp -d "$SMOKE_TEMP_ROOT/hyp-smoke.XXXXXX"
  else
    mktemp -d
  fi
}
...
#                     hyp_mem_init is decoration and freed pages stay committed
...
SC=$(cli search_code --project "$PROJECT" --pattern hyp_ --mode compact --limit 5)
...
SCW=$(cli search_code --project "$PROJECT" --pattern "hyp_init|hyp_nope" --regex false --limit 5)
...
SCA=$(cli search_code --project "$PROJECT" --pattern hyp_ --file-pattern "*R&D*.c" --limit 5)
case "$SCA" in
...
if command -v pgrep &>/dev/null && [ "$(uname)" != "MINGW64_NT" ] 2>/dev/null; then
  ...
  sleep 1
  RESIDUAL=$(pgrep -f "hyponoia.*cli" 2>/dev/null | wc -l | tr -d ' \n' || echo "0")
  RESIDUAL="${RESIDUAL:-0}"
  if [ "$RESIDUAL" -gt 0 ]; then
    echo "WARNING: $RESIDUAL residual hyponoia process(es) found"
  else
...
mcp_dump_diagnostics() {
  ...
  local cache="${HYP_CACHE_DIR:-$HOME/.cache/hyponoia}"
  for log in hyp-daemon.log daemon-conflicts.ndjson; do
...
DRYRUN_CACHE="$DRYRUN_HOME/.cache/hyponoia"
...
if [[ "$BINARY" == *.exe ]] &&
   [ -n "${HYP_TEST_WINDOWS_USER_PATH_RUN_ID:-}" ]; then
  if INVALID_PATH_OUT=$(
    HYP_TEST_WINDOWS_USER_PATH_RUN_ID=invalid \
...
copy_smoke_binary "$INSTALL_DIR/hyponoia"
chmod 755 "$INSTALL_DIR/hyponoia"

# Verify installed binary works
INSTALLED_VER=$("$INSTALL_DIR/hyponoia" --version 2>&1)
...
copy_smoke_binary "$REPLACE_DIR/smoke-hyponoia"

# 3. Simulate hyp_replace_binary: unlink old, copy new
rm -f "$INSTALL_DIR/hyponoia"
cp "$REPLACE_DIR/smoke-hyponoia" "$INSTALL_DIR/hyponoia"
chmod 755 "$INSTALL_DIR/hyponoia"

# 4. Verify replaced binary works
REPLACED_VER=$("$INSTALL_DIR/hyponoia" --version 2>&1)
...
# 5. Test replacement of read-only binary (edge case — hyp_replace_binary
#    handles this via unlink-before-write, which works even on read-only files)
chmod 444 "$INSTALL_DIR/hyponoia"
rm -f "$INSTALL_DIR/hyponoia"
cp "$REPLACE_DIR/smoke-hyponoia" "$INSTALL_DIR/hyponoia"
chmod 755 "$INSTALL_DIR/hyponoia"
READONLY_VER=$("$INSTALL_DIR/hyponoia" --version 2>&1)
...
DEVIN_SKILL="$DEVIN_DIR/skills/hyponoia/SKILL.md"
CODEBUDDY_MCP="$FAKE_HOME/.codebuddy/.mcp.json"
CODEBUDDY_INSTRUCTIONS="$FAKE_HOME/.codebuddy/CODEBUDDY.md"
CODEBUDDY_SKILL="$FAKE_HOME/.codebuddy/skills/hyponoia/SKILL.md"
CODEBUDDY_AGENT="$FAKE_HOME/.codebuddy/agents/hyponoia.md"
...
BOB_RULE="$FAKE_HOME/.bob/rules/hyponoia.md"
BOB_SKILL="$FAKE_HOME/.bob/skills/hyponoia/SKILL.md"
BOB_AGENT="$FAKE_HOME/.bob/agents/hyponoia.md"
POCHI_MCP="$FAKE_HOME/.pochi/config.jsonc"
POCHI_INSTRUCTIONS="$FAKE_HOME/.pochi/README.pochi.md"
POCHI_SKILL="$FAKE_HOME/.pochi/skills/hyponoia/SKILL.md"
POCHI_AGENT="$FAKE_HOME/.pochi/agents/hyponoia.md"
ROVO_MCP="$FAKE_HOME/.rovodev/mcp.json"
ROVO_INSTRUCTIONS="$FAKE_HOME/.rovodev/AGENTS.md"
ROVO_SKILL="$FAKE_HOME/.rovodev/skills/hyponoia/SKILL.md"
ROVO_AGENT="$FAKE_HOME/.rovodev/subagents/hyponoia.md"
...
if [[ "$BINARY" == *.exe ]]; then
  SELF_PATH="$FAKE_HOME/.local/bin/hyponoia.exe"
else
  cp "$BINARY" "$FAKE_HOME/.local/bin/hyponoia"
  SELF_PATH="$FAKE_HOME/.local/bin/hyponoia"
fi
create_agent_stub() {
  local name="$1"
  if [[ "$BINARY" == *.exe ]]; then
...
# Run install — override platform config dirs so hyp_app_config_dir() and
# hyp_app_local_dir() resolve to FAKE_HOME paths on all platforms.
PHASE8_INSTALL_RC=0
...
if [[ "$BINARY" == *.exe ]]; then
  ...
  PHASE8_CANONICAL="$FAKE_HOME/.local/bin/hyponoia.exe"
  ...
  if [ -e "$FAKE_HOME/.local/bin/hyponoia.payload.exe" ]; then
    ...
    if [ "$access" = "direct" ]; then
      if ! grep -Fq 'source read/grep fallback' "$file" 2>/dev/null; then
        ...
        exit 1
         ...
         grep -qE '(mcpServers|mcp__hyponoia__|mcp_hyponoia_|@hyponoia/|hyponoia/)' "$file" 2>/dev/null; then
...
assert_tier_profile_set_removed() {
  ...
  local suffix="$3"
  local slug file
  for slug in hyponoia-scout hyponoia hyponoia-auditor; do
    file="$directory/$slug$suffix"
    if [ -e "$file" ]; then
...
CMD=$(json_get "$FAKE_HOME/.claude.json" "d.get('mcpServers',{}).get('hyponoia',{}).get('command','')")
...
sys.exit(0 if 'hyponoia' in d.get('mcpServers', {}) else 1)
" 2>/dev/null; then
...
CLAUDE_AGENT="$FAKE_HOME/.claude/agents/hyponoia.md"
if ! grep -q '^mcpServers: \[hyponoia\]$' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__hyponoia__search_graph' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__hyponoia__check_index_coverage' "$CLAUDE_AGENT" 2>/dev/null ||
   ! grep -q '^permissionMode: plan$' "$CLAUDE_AGENT" 2>/dev/null ||
   grep -qE 'mcp__hyponoia__(index_repository|delete_project|manage_adr|ingest_traces)' "$CLAUDE_AGENT" 2>/dev/null; then
...
case "$(uname -s)" in
  MINGW*|MSYS*|CYGWIN*)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/hyp-code-discovery-gate.cmd"
    if [ ! -f "$GATE_SCRIPT" ]; then
      echo "FAIL 8e: .cmd shim missing on Windows"
      exit 1
    fi
    if [ -f "$FAKE_HOME/.claude/hooks/hyp-code-discovery-gate" ]; then
      echo "FAIL 8e: legacy extensionless shim still installed on Windows"
      exit 1
    fi
    ;;
  *)
    GATE_SCRIPT="$FAKE_HOME/.claude/hooks/hyp-code-discovery-gate"
...
# 8f-8h: Codex TOML
if ! grep -q '\[mcp_servers.hyponoia\]' "$FAKE_HOME/.codex/config.toml"; then
...
if [ ! -f "$FAKE_HOME/.codex/AGENTS.md" ] || ! grep -q 'hyponoia' "$FAKE_HOME/.codex/AGENTS.md"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/settings.json" "d['mcpServers']['hyponoia']['command']")
...
GEMINI_AGENT="$FAKE_HOME/.gemini/agents/hyponoia.md"
if ! grep -q '^name: hyponoia$' "$GEMINI_AGENT" 2>/dev/null ||
   ...
   ! grep -q 'mcp_hyponoia_search_graph' "$GEMINI_AGENT" 2>/dev/null ||
   ! grep -q 'mcp_hyponoia_check_index_coverage' "$GEMINI_AGENT" 2>/dev/null ||
   grep -qE 'mcp_hyponoia_(index_repository|delete_project|manage_adr|ingest_traces)' "$GEMINI_AGENT" 2>/dev/null; then
...
if [ -f "$ZED_CFG" ]; then
  CMD=$(json_get "$ZED_CFG" "d['context_servers']['hyponoia']['command']")
  if ! path_match "$CMD" "$SELF_PATH"; then
...
if ! grep -q 'Hyponoia' "$ZED_INSTR" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.config/opencode/opencode.json" "d['mcp']['hyponoia']['command'][0]")
if [ -n "$CMD" ]; then
  if ! path_match "$CMD" "$SELF_PATH"; then
...
CMD=$(json_get "$FAKE_HOME/.gemini/config/mcp_config.json" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
if [ -f "$FAKE_HOME/CONVENTIONS.md" ]; then
  if ! grep -q 'hyponoia' "$FAKE_HOME/CONVENTIONS.md"; then
...
CMD=$(json_get "$KILO_CFG" "d['mcp']['hyponoia']['command'][0]")
if ! path_match "$CMD" "$SELF_PATH"; then
...
KILO_RULE="$FAKE_HOME/.config/kilo/rules/hyponoia.md"
if [ ! -f "$KILO_RULE" ]; then
...
KILO_AGENT="$FAKE_HOME/.config/kilo/agents/hyponoia.md"
if ! grep -q '^mode: subagent$' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"*": deny' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"hyponoia_search_graph": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"hyponoia_get_code_snippet": allow' "$KILO_AGENT" 2>/dev/null ||
   ! grep -Fq '"hyponoia_check_index_coverage": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -Fq '"hyponoia_*": allow' "$KILO_AGENT" 2>/dev/null ||
   grep -qE 'hyponoia_(index_repository|delete_project|manage_adr|ingest_traces)' "$KILO_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$VSCODE_CFG" "d['servers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8v: VS Code command='$CMD'"
  exit 1
fi
echo "OK 8v: VS Code MCP"
CMD=$(json_get "$VSCODE_PROFILE_CFG" "d['servers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
...
# 8w: OpenClaw MCP
CMD=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8w: OpenClaw command='$CMD'"
  exit 1
fi
ENABLED=$(json_get "$FAKE_HOME/.openclaw/openclaw.json" "d['mcp']['servers']['hyponoia'].get('enabled')")
...
CMD=$(json_get "$FAKE_HOME/.kiro/settings/mcp.json" "d['mcpServers']['hyponoia']['command']")
KIRO_AGENT="$FAKE_HOME/.kiro/agents/hyponoia.json"
KIRO_AGENT_CMD=$(json_get "$KIRO_AGENT" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! path_match "$KIRO_AGENT_CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$FAKE_HOME/.kiro/steering/hyponoia.md" 2>/dev/null ||
   ! cat "$KIRO_AGENT" 2>/dev/null | python3 -c "
import json, sys
d = json.load(sys.stdin)
tools = d.get('tools', [])
server = d.get('mcpServers', {}).get('hyponoia', {})
ok = (d.get('name') == 'hyponoia' and tools[:3] == ['read', 'grep', 'glob'] and
      '@hyponoia/search_graph' in tools and
      '@hyponoia/check_index_coverage' in tools and
      all('@hyponoia/' + name not in tools for name in
          ('index_repository', 'delete_project', 'manage_adr', 'ingest_traces')) and
      '@hyponoia' not in tools and d.get('includeMcpJson') is False and
      set(d.get('mcpServers', {})) == {'hyponoia'} and
...
if ! grep -q '^name: hyponoia$' "$FAKE_HOME/.hermes/skills/hyponoia/SKILL.md" 2>/dev/null ||
   ! grep -q 'delegate_task' "$FAKE_HOME/.hermes/skills/hyponoia/SKILL.md" 2>/dev/null ||
   ! grep -q '`context`' "$FAKE_HOME/.hermes/skills/hyponoia/SKILL.md" 2>/dev/null; then
...
# 8y: OpenHands MCP
CMD=$(json_get "$FAKE_HOME/.openhands/mcp.json" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH"; then
  echo "FAIL 8y: OpenHands command='$CMD'"
  exit 1
fi
echo "OK 8y: OpenHands MCP"
if ! grep -q '^name: hyponoia$' "$FAKE_HOME/.agents/skills/hyponoia/SKILL.md" 2>/dev/null ||
   ! grep -q 'trace_path' "$FAKE_HOME/.agents/skills/hyponoia/SKILL.md" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.qwen/settings.json" "d['mcpServers']['hyponoia']['command']")
...
COPILOT_SKILL="$FAKE_HOME/.copilot/skills/hyponoia/SKILL.md"
COPILOT_AGENT="$FAKE_HOME/.copilot/agents/hyponoia.agent.md"
if ! grep -q 'search_graph' "$COPILOT_SKILL" 2>/dev/null ||
   ! grep -q '^tools:' "$COPILOT_AGENT" 2>/dev/null ||
   ! grep -q 'hyponoia/trace_path' "$COPILOT_AGENT" 2>/dev/null ||
   grep -qE '^  - (edit|shell|bash|hyponoia/(index_repository|delete_project|manage_adr|ingest_traces))$' "$COPILOT_AGENT" 2>/dev/null; then
...
COPILOT_HOOKS="$FAKE_HOME/.copilot/hooks/hyponoia.json"
...
CMD=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['hyponoia']['command']")
FACTORY_TYPE=$(json_get "$FAKE_HOME/.factory/mcp.json" "d['mcpServers']['hyponoia']['type']")
...
FACTORY_AGENT="$FAKE_HOME/.factory/droids/hyponoia.md"
if ! grep -q '^tools: \["Read", "LS", "Grep", "Glob",' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__hyponoia__search_graph' "$FACTORY_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__hyponoia__check_index_coverage' "$FACTORY_AGENT" 2>/dev/null ||
   grep -q '^mcpServers:' "$FACTORY_AGENT" 2>/dev/null ||
   grep -qE 'mcp__hyponoia__(index_repository|delete_project|manage_adr|ingest_traces)' "$FACTORY_AGENT" 2>/dev/null; then
...
CMD=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['hyponoia']['command']")
CRUSH_TYPE=$(json_get "$FAKE_HOME/.config/crush/crush.json" "d['mcp']['hyponoia']['type']")
CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('hyponoia.md') for p in d['options']['context_paths']))")
if ! path_match "$CMD" "$SELF_PATH" || [ "$CRUSH_TYPE" != "stdio" ] ||
   [ "$CRUSH_CONTEXT" != "True" ] ||
   ! grep -q 'does not inherit MCP access' "$FAKE_HOME/.config/crush/hyponoia.md" 2>/dev/null; then
...
GOOSE_CMD=$(sed -n '/^  hyponoia:/,/^  [^ ]/{s/^ *cmd: *//p;}' \
  "$GOOSE_CFG" 2>/dev/null | head -1)
if ! grep -q '^extensions:' "$GOOSE_CFG" 2>/dev/null ||
   ! grep -q '^  hyponoia:' "$GOOSE_CFG" 2>/dev/null ||
...
if ! grep -q '^\[\[mcp_servers\]\]' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
   ! grep -q '^name = "hyponoia"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null ||
...
VIBE_AGENT="$FAKE_HOME/.vibe/agents/hyponoia.toml"
VIBE_PROMPT="$FAKE_HOME/.vibe/prompts/hyponoia.md"
if ! grep -q '^agent_type = "subagent"$' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq 'enabled_tools = ["read_file", "grep_search", "hyponoia_search_graph"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"hyponoia_get_code_snippet"' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -Fq '"hyponoia_check_index_coverage"' "$VIBE_AGENT" 2>/dev/null ||
   grep -Fq '"hyponoia_*"' "$VIBE_AGENT" 2>/dev/null ||
   grep -qE 'hyponoia_(index_repository|delete_project|manage_adr|ingest_traces)' "$VIBE_AGENT" 2>/dev/null ||
   ! grep -q '^system_prompt_id = "hyponoia"$' "$VIBE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$FAKE_HOME/.codeium/windsurf/mcp_config.json" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
AUGMENT_RULE="$FAKE_HOME/.augment/rules/hyponoia.md"
AUGMENT_AGENT="$FAKE_HOME/.augment/agents/hyponoia.md"
if [[ "$BINARY" == *.exe ]]; then
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/hyponoia-session.ps1"
else
  AUGMENT_SCRIPT="$FAKE_HOME/.augment/hooks/hyponoia-session.sh"
fi
CMD=$(json_get "$AUGMENT_SETTINGS" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$AUGMENT_RULE" 2>/dev/null ||
   ! grep -q '^name: hyponoia$' "$AUGMENT_AGENT" 2>/dev/null ||
...
owned = [e for e in entries if any('hyponoia-session' in str(h.get('command', '')) for h in e.get('hooks', []))]
...
SKILL_FILE="$FAKE_HOME/.claude/skills/hyponoia/SKILL.md"
if [ ! -s "$SKILL_FILE" ]; then
  echo "FAIL 8ai: skill hyponoia missing or empty"
  exit 1
fi
...
QODER_SKILL="$FAKE_HOME/.qoder/skills/hyponoia/SKILL.md"
QODER_AGENT="$FAKE_HOME/.qoder/agents/hyponoia.md"
CMD=$(json_get "$QODER_SETTINGS" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ! grep -q 'search_graph' "$QODER_SKILL" 2>/dev/null ||
   ! grep -q '^tools: Read,Grep,Glob,mcp__hyponoia__search_graph' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q 'mcp__hyponoia__check_index_coverage' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^mcpServers:$' "$QODER_AGENT" 2>/dev/null ||
   ! grep -q '^  - hyponoia$' "$QODER_AGENT" 2>/dev/null ||
   grep -qE 'mcp__hyponoia__(index_repository|delete_project|manage_adr|ingest_traces)' "$QODER_AGENT" 2>/dev/null ||
...
KIMI_SKILL="$CUSTOM_KIMI_HOME/skills/hyponoia/SKILL.md"
KIMI_CONFIG="$CUSTOM_KIMI_HOME/config.toml"
CMD=$(json_get "$KIMI_MCP" "d['mcpServers']['hyponoia']['command']")
...
PI_SKILL="$FAKE_HOME/.pi/agent/skills/hyponoia/SKILL.md"
...
WARP_SKILL="$FAKE_HOME/.agents/skills/hyponoia/SKILL.md"
...
JUNIE_SKILL="$FAKE_HOME/.junie/skills/hyponoia/SKILL.md"
JUNIE_AGENT="$FAKE_HOME/.junie/agents/hyponoia.md"
CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['hyponoia']['command']")
JUNIE_SCOUT_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['hyponoia-scout']['command']")
JUNIE_ANALYSIS_CMD=$(json_get "$JUNIE_MCP" "d['mcpServers']['hyponoia-analysis']['command']")
JUNIE_SCOUT_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['hyponoia-scout']['args']")
JUNIE_ANALYSIS_ARGS=$(json_get "$JUNIE_MCP" "d['mcpServers']['hyponoia-analysis']['args']")
if ! path_match "$CMD" "$SELF_PATH" ||
   ...
   ! grep -q 'mcpServers: \["hyponoia-analysis"\]' "$JUNIE_AGENT" 2>/dev/null ||
...
CMD=$(json_get "$ROO_CFG" "d['mcpServers']['hyponoia']['command']")
...
CMD=$(json_get "$GITLAB_MCP" "d['mcpServers']['hyponoia']['command']")
...
CMD=$(json_get "$DEVIN_CONFIG" "d['mcpServers']['hyponoia']['command']")
if ! path_match "$CMD" "$SELF_PATH" ||
...
" 2>/dev/null ||
   ...
   ! grep -q 'hyp-code-discovery-gate' "$FAKE_HOME/.claude/settings.json" 2>/dev/null; then
...
CMD=$(json_get "$CODEBUDDY_MCP" "d['mcpServers']['hyponoia']['command']")
...
if ! path_match "$CMD" "$SELF_PATH" || [ "$CODEBUDDY_KEEP" != "codebuddy" ] ||
   ...
   ! grep -q '^tools: Read,Grep,Glob,mcp__hyponoia__search_graph,' "$CODEBUDDY_AGENT" 2>/dev/null ||
...
BOB_IDE_CMD=$(json_get "$BOB_IDE_MCP" "d['mcpServers']['hyponoia']['command']")
BOB_SHELL_CMD=$(json_get "$BOB_SHELL_MCP" "d['mcpServers']['hyponoia']['command']")
...
POCHI_CMD=$(sed '/^[[:space:]]*\/\//d' "$POCHI_MCP" 2>/dev/null | python3 -c "import json,sys; print(json.load(sys.stdin)['mcp']['hyponoia']['command'])" 2>/dev/null || echo "")
...
ROVO_CMD=$(json_get "$ROVO_MCP" "d['mcpServers']['hyponoia']['command']")
...
AMAZON_Q_CMD=$(json_get "$AMAZON_Q_MCP" "d['mcpServers']['hyponoia']['command']")
...
for VIBE_SLUG in hyponoia-scout hyponoia hyponoia-auditor; do
...
import json, sys
d = json.load(sys.stdin)
if 'hyponoia' in d.get('mcpServers', {}):
    sys.exit(1)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'hyponoia' in d.get('mcpServers', {}) else 0)
" 2>/dev/null; then
...
d = json.load(sys.stdin)
hooks = d.get('hooks', {})
found = any('hyp-code-discovery-gate' in str(h) or
            'hyp-session-reminder' in str(h) or
            'hyp-subagent-reminder' in str(h)
            for entries in hooks.values() for h in entries)
sys.exit(1 if found else 0)
" 2>/dev/null; then
  for HOOK_SCRIPT in hyp-code-discovery-gate hyp-session-reminder hyp-subagent-reminder; do
...
if grep -q '\[mcp_servers.hyponoia\]' "$FAKE_HOME/.codex/config.toml" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
has_mcp = 'hyponoia' in d.get('mcpServers', {})
has_existing = d.get('existingKey', False)
hooks = d.get('hooks', {}).get('BeforeTool', [])
has_hook = any('hyponoia' in str(h) for h in hooks)
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'hyponoia' in d.get('servers', {}) else 0)
" 2>/dev/null; then
...
import json, sys
d = json.load(sys.stdin)
sys.exit(1 if 'hyponoia' in d.get('$ROOT', {}) else 0)
" 2>/dev/null; then
...
names = {'hyponoia', 'hyponoia-scout', 'hyponoia-analysis'}
...
import json, sys
d = json.load(sys.stdin)
print('present' if 'hyponoia' in d.get('mcp', {}) else 'absent')
...
if grep -q 'hook-augment' "$QODER_SETTINGS" 2>/dev/null ||
   ...
   grep -q 'hook-augment' "$FAKE_HOME/.copilot/hooks/hyponoia.json" 2>/dev/null ||
   grep -q 'hook-augment' "$FAKE_HOME/.factory/hooks.json" 2>/dev/null ||
   grep -q 'hyponoia-session' "$AUGMENT_SETTINGS" 2>/dev/null ||
...
if CRUSH_CONTEXT=$(json_get "$FAKE_HOME/.config/crush/crush.json" "str(any(str(p).endswith('hyponoia.md') for p in d.get('options', {}).get('context_paths', [])))") &&
...
if grep -q '^  hyponoia:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  pre_llm_call:' "$FAKE_HOME/.hermes/config.yaml" 2>/dev/null ||
   grep -q '^  hyponoia:' "$GOOSE_CFG" 2>/dev/null ||
   grep -q '^name = "hyponoia"' "$FAKE_HOME/.vibe/config.toml" 2>/dev/null; then
...
for CONTEXT_FILE in \
  "$ZED_INSTR" \
  "$FAKE_HOME/.kiro/steering/hyponoia.md" \
  "$FAKE_HOME/.factory/AGENTS.md" \
  "$AUGMENT_RULE" \
  "$FAKE_HOME/.config/crush/hyponoia.md" \
  ...
  "$PI_INSTRUCTIONS"; do
  if grep -q 'hyponoia:start' "$CONTEXT_FILE" 2>/dev/null; then
...
if [ -d "$FAKE_HOME/.claude/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.hermes/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.agents/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.qoder/skills/hyponoia" ] ||
   [ -d "$CUSTOM_KIMI_HOME/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.pi/agent/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.junie/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.rovodev/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.copilot/skills/hyponoia" ] ||
   [ -d "$FAKE_HOME/.vibe/skills/hyponoia" ] ||
   [ -e "$DEVIN_SKILL" ] ||
   [ -e "$CODEBUDDY_SKILL" ] ||
   [ -e "$BOB_SKILL" ] ||
   [ -e "$POCHI_SKILL" ] ||
...
# Note: hyp_find_cli searches hardcoded paths (/usr/local/bin, /opt/homebrew/bin)
...
import json, sys
d = json.load(sys.stdin)
print(list(d.get('mcpServers',{}).keys()).count('hyponoia'))
" 2>/dev/null || echo "0")
if [ "$COUNT" != "1" ]; then
...
CLEAN_UNINSTALLER="$CLEAN_HOME/hyp-uninstall-driver"
if [[ "$BINARY" == *.exe ]]; then
...
copy_smoke_binary "$CORRUPT_HOME/.local/bin/hyponoia"
...
DBL_UNINSTALLER="$DBL_HOME/.local/bin/hyponoia"
DBL_RETRY_UNINSTALLER="$DBL_HOME/hyp-uninstall-retry"
if [[ "$BINARY" == *.exe ]]; then
...
SECURITY_BIN="$SECURITY_DIR/hyponoia"
...
if [ -n "${SMOKE_DOWNLOAD_URL:-}" ]; then
  ...
  # HYP_DOWNLOAD_URL test override, so point it directly at the same fixture.
  ...
  # through install.sh or `install`, so nothing populates ~/.hyp/assets. The
  ...
  BINARY_ASSET="$(dirname "$BINARY")/hyp-integrations.json"
  if [[ "$BINARY" == *.exe ]]; then
    copy_smoke_binary "$UPDATE_HOME/.local/bin/hyponoia.exe"
    mkdir -p "$UPDATE_HOME/retired-install"
    copy_smoke_binary "$UPDATE_HOME/retired-install/hyponoia.exe"
    [ -f "$BINARY_ASSET" ] && cp "$BINARY_ASSET" "$UPDATE_HOME/.local/bin/hyp-integrations.json"
  else
    copy_smoke_binary "$UPDATE_HOME/.local/bin/hyponoia"
    chmod 755 "$UPDATE_HOME/.local/bin/hyponoia"
    mkdir -p "$UPDATE_HOME/retired-install"
    copy_smoke_binary "$UPDATE_HOME/retired-install/hyponoia"
    chmod 755 "$UPDATE_HOME/retired-install/hyponoia"
    [ -f "$BINARY_ASSET" ] && cp "$BINARY_ASSET" "$UPDATE_HOME/.local/bin/hyp-integrations.json"
    if [ "$(uname -s)" = "Darwin" ]; then
      codesign --sign - --force "$UPDATE_HOME/.local/bin/hyponoia" 2>/dev/null || true
      codesign --sign - --force "$UPDATE_HOME/retired-install/hyponoia" \
        2>/dev/null || true
    fi
  fi
  ...
  # uninstall phases.
  if [[ "$BINARY" == *.exe ]]; then
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/hyponoia.exe"
  else
    UPDATE_DRIVER="$UPDATE_HOME/.local/bin/hyponoia"
  fi

  # Pre-install agent config with positive prior-install identity. POSIX runs
  # update from that exact retired HYP image, so refresh requires only string
  ...
  HOME="$UPDATE_HOME" HYP_DOWNLOAD_URL="$UPDATE_DOWNLOAD_URL" \
  ...
  if [[ "$BINARY" == *.exe ]]; then
    UPD_BIN="$UPDATE_HOME/.local/bin/hyponoia.exe"
  else
    UPD_BIN="$UPDATE_HOME/.local/bin/hyponoia"
  fi
  if [ ! -f "$UPD_BIN" ]; then
  ...
  if [ -f "$UPDATE_HOME/.local/bin/hyponoia" ] || [ -f "$UPDATE_HOME/.local/bin/hyponoia.exe" ]; then
...
import json, sys
d = json.load(sys.stdin)
if 'hyponoia' in d.get('mcpServers', {}): sys.exit(1)
sys.exit(0)
" 2>/dev/null; then
  ...
  copy_smoke_binary "$UPDATE_DIR/install/hyponoia"
  chmod 755 "$UPDATE_DIR/install/hyponoia"
  copy_smoke_binary "$UPDATE_DIR/smoke-downloaded"
  rm -f "$UPDATE_DIR/install/hyponoia"
  cp "$UPDATE_DIR/smoke-downloaded" "$UPDATE_DIR/install/hyponoia"
  chmod 755 "$UPDATE_DIR/install/hyponoia"
  if [ "$(uname -s)" = "Darwin" ]; then
    codesign --sign - --force "$UPDATE_DIR/install/hyponoia" 2>/dev/null || true
  fi
  if ! "$UPDATE_DIR/install/hyponoia" --version > /dev/null 2>&1; then
...
DL_ARCHIVE="hyponoia-${DL_OS}-${DL_ARCH}.${DL_EXT}"
DL_ARCHIVE_UI="hyponoia-ui-${DL_OS}-${DL_ARCH}.${DL_EXT}"
...
if [ "$DL_OS" != "windows" ] && [ -f "$REPO_ROOT/install.sh" ]; then
  ...
  HOME="$INSTALL_TEST_HOME" HYP_DOWNLOAD_URL="$SMOKE_DOWNLOAD_URL" \
    "$REPO_ROOT/install.sh" "$DL_VARIANT_ARG" --dir="$INSTALL_TEST_DIR" 2>&1 || true

  # 13b: binary placed
  if [ ! -f "$INSTALL_TEST_DIR/hyponoia" ]; then
  ...
  shopt -s nullglob
  INSTALLED_UI_PACKS=("$INSTALL_TEST_DIR"/hyp-ui-*.pack)
  shopt -u nullglob
  if [ "$DL_VARIANT_ARG" = "--ui" ]; then
    if [ "${#INSTALLED_UI_PACKS[@]}" -ne 1 ] ||
       ! [[ "$(basename "${INSTALLED_UI_PACKS[0]}")" =~ ^hyp-ui-[0-9a-f]{64}\.pack$ ]]; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    codesign --sign - --force "$INSTALL_TEST_DIR/hyponoia" 2>/dev/null || true
  fi
  if ! "$INSTALL_TEST_DIR/hyponoia" --version > /dev/null 2>&1; then
  ...
  if [ "$DL_OS" = "darwin" ]; then
    if codesign -v "$INSTALL_TEST_DIR/hyponoia" 2>/dev/null; then
  ...
  if [ -f "$INSTALL_TEST_HOME/.claude.json" ] && grep -q 'hyponoia' "$INSTALL_TEST_HOME/.claude.json" 2>/dev/null; then
  ...
  if ! HOME="$WIN_HOME" TEMP="$WIN_HOME" TMP="$WIN_HOME" \
    HYP_DOWNLOAD_URL="$WIN_URL" HYP_ARCH="$DL_ARCH" MSYS2_ARG_CONV_EXCL='*' \
  ...
  # 13g: binary placed
  PS1_BIN="$PS1_TEST_DIR/hyponoia.exe"
  if [ ! -f "$PS1_BIN" ] && [ -f "$PS1_TEST_DIR/hyponoia" ]; then
    PS1_BIN="$PS1_TEST_DIR/hyponoia"
  fi
  if [ -f "$PS1_BIN" ]; then
  ...
  shopt -s nullglob
  PS1_UI_PACKS=("$PS1_TEST_DIR"/hyp-ui-*.pack)
  shopt -u nullglob
  if [ "$DL_VARIANT_ARG" = "--ui" ]; then
    if [ "${#PS1_UI_PACKS[@]}" -ne 1 ] ||
       ! [[ "$(basename "${PS1_UI_PACKS[0]}")" =~ ^hyp-ui-[0-9a-f]{64}\.pack$ ]]; then
