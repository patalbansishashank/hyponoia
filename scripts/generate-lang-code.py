...
GRAMMAR_DIR = os.path.join(PROJECT_DIR, "internal", "hyp")
...
def generate_enum(langs):
    """Print enum entries for hyp.h."""
    print("\n=== Enum Entries (paste into hyp.h before HYP_LANG_KUSTOMIZE) ===")
    for lang in langs:
        print(f"    HYP_LANG_{lang['enum']},")
...
def generate_specs(langs):
    ...
    for lang in langs:
        ...
        print(f"    // HYP_LANG_{lang['enum']}")
        print(
            f"    [HYP_LANG_{lang['enum']}] = {{HYP_LANG_{lang['enum']}, "
...
def generate_language_c(langs):
    ...
    for ext, enum, display in sorted(ext_entries, key=lambda x: x[0].lower()):
        print(f'    /* {display} */')
        print(f'    {{"{ext}", HYP_LANG_{enum}}},')
        print()

    print("\n=== FILENAME_TABLE Entries ===")
    for lang in langs:
        for fn in lang["filenames"]:
            print(f'    {{"{fn}", HYP_LANG_{lang["enum"]}}},')

    print("\n=== LANG_NAMES Entries ===")
    for lang in langs:
        print(
            f'    [HYP_LANG_{lang["enum"]}] = "{lang["display"]}",'
        )
