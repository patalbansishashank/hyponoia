...
Legs (default: quick,query-leak — the release-gating sequence):
  ...
  query-leak   HYP_SOAK_MODE=query-leak, --skip-crash-test, results in
...
Examples:
  scripts/soak-legs.sh build/c/hyponoia 10          # both legs, 10m each
  scripts/soak-legs.sh --legs quick build/c/hyponoia 15   # ASan-style
EOF
}
...
# keys its native-Windows handling — cygpath'd HYP_CACHE_DIR, coproc stdio —
...
run_leg() {
    ...
    log="$(mktemp "${TMPDIR:-/tmp}/hyp-soak-leg-XXXXXX")"
    ...
    quick)
        ...
        ;;
    query-leak)
        HYP_SOAK_MODE=query-leak RESULTS_DIR=soak-results-query-leak \
