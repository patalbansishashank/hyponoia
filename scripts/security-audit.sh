...
FUNC_LIST="system popen hyp_popen execl fork"
...
while IFS= read -r file; do
    ...
    for func in $FUNC_LIST; do
        ...
        # 'popen(' must NOT match 'hyp_popen(' — use [^a-z] negative class
        case "$func" in
            hyp_popen) pattern="hyp_popen(" ;;
...
while IFS= read -r file; do
    ...
    DANGER_LINES=$(grep -n 'system(\|popen(\|hyp_popen(\|connect(\|execl(' "$file" 2>/dev/null \
