...
Environment:
  HYP_TEST_SEQUENTIAL=1   Single-process runner instead of the parallel harness.
  HYP_RUN_HANG_TEST=1     Opt-in C++ index-hang regression (#410, needs prod).
  HYP_NO_CCACHE=1         Disable the content-verified compiler cache.
  HYP_TEST_SHARD/_LEG     Set by CI's sharded legs; leave unset locally.
...
if [ "$TSAN" -eq 1 ]; then
    ...
    make -j"$NPROC" -f Makefile.hyp "$BUILD_DIR/test-runner-tsan" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    make -f Makefile.hyp test-tsan ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
    exit "$?"
fi
...
if [ -n "$SUITES" ]; then
    ...
    make -j"$NPROC" -f Makefile.hyp "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
...
# HYP_TEST_SEQUENTIAL=1 restores the single-process runner.
make -j"$NPROC" -f Makefile.hyp "$BUILD_DIR/test-runner" ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
if [ "${HYP_TEST_SEQUENTIAL:-0}" = "1" ]; then
    make -f Makefile.hyp test ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
else
    make -f Makefile.hyp test-par ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
fi
...
# Opt-in via HYP_RUN_HANG_TEST=1 (it needs the prod binary, which the ASan unit
# run above does not build). Skipped by default so the fast unit run stays fast.
if [ "${HYP_RUN_HANG_TEST:-0}" = "1" ]; then
...
make -j"$NPROC" -f Makefile.hyp hyp TEST_SEAMS=1 ${MAKE_ARGS[@]+"${MAKE_ARGS[@]}"}
WATCHDOG_BINARY="$ROOT/$BUILD_DIR/hyponoia"
HYP_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_parent_watchdog.sh"
...
HYP_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_watchdog.sh"
...
HYP_TEST_BINARY="$WATCHDOG_BINARY" bash "$ROOT/tests/test_worker_error_response.sh"
...
# HYP_TEST_HOOK_CLIENT_BUILD seam and asserts the stdout systemMessage. It is
...
#   make -f Makefile.hyp hyp TEST_SEAMS=1 && bash tests/test_hook_conflict_notice.sh
...
# pack (no npm/network dependency) and prove it waits for the HYP HTTP root,
...
make -j"$NPROC" -f Makefile.hyp hyp-with-ui TEST_SEAMS=1 \
