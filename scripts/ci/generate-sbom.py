...
per-grammar license list is internal/hyp/vendored/grammars/MANIFEST.md.
"""
...
version = sys.argv[1]
n_grammars = len(
    [d for d in glob.glob("internal/hyp/vendored/grammars/*") if os.path.isdir(d)]
)
sbom = {
    "spdxVersion": "SPDX-2.3",
    "dataLicense": "CC0-1.0",
    "SPDXID": "SPDXRef-DOCUMENT",
    "name": f"hyponoia-{version}",
    "documentNamespace": f"https://github.com/patalbansishashank/hyponoia/releases/{version}",
    "creationInfo": {
        "created": datetime.datetime.now(datetime.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "creators": ["Tool: hyponoia-release-pipeline"],
    },
    "packages": [
