<#
.SYNOPSIS
Sweep hyp test residue from a Windows test host and assert a runner-like free
...
Sweeps only hyp-prefixed directories under the profile and TEMP. The checkout
(C:\hyp) is not under either, and is never touched.
...
# non-representable characters, deep .hyp-retired/generations nesting - whose
...
foreach ($root in $roots) {
    ...
    Get-ChildItem -LiteralPath $root -Directory -Filter 'hyp-*' -ErrorAction SilentlyContinue |
        ForEach-Object {
