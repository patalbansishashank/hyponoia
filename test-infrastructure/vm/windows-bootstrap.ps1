...
#   mkdir /tmp/hypiso
#   cp windows-bootstrap.ps1 /tmp/hypiso/
#   cp ~/.claude/hyp-vm/id_ed25519.pub /tmp/hypiso/host-key.pub
#   printf '@echo off\r\npowershell -NoProfile -ExecutionPolicy Bypass -File "%%~dp0windows-bootstrap.ps1" -SshPublicKeyPath "%%~dp0host-key.pub"\r\n' > /tmp/hypiso/RUN-ME.bat
#   hdiutil makehybrid -iso -joliet -default-volume-name HYPSETUP -o ~/hyp-setup.iso /tmp/hypiso
...
New-NetFirewallRule -Name hyp-sshd -DisplayName "OpenSSH (hyp test VM)" `
...
Write-Host "On the host, write ~/.claude/hyp-vm/config with:"
Write-Host "  HYP_VM_HOST=$ip"
Write-Host ("  HYP_VM_USER=" + $env:USERNAME)
Write-Host "  HYP_VM_HOST_KEY_SHA256=<copy the SHA256:... value above>"
