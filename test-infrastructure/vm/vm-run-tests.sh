...
Environment:
  HYP_VM_RUNNER      sanitizer-variant runner path (ubsan/trap-ubsan builds);
                     switches to direct-runner mode for those iteration tools.
  HYP_VM_SOAK_BINARY product binary for --soak (default build/c/hyponoia.exe)
  HYP_VM_TEST_LOG    VM-side log path (default /tmp/win-test.log)
...
RUNNER="${HYP_VM_RUNNER:-}"

# Per-run identity. The VM holds ONE checkout (/c/hyp), so two concurrent runs
...
CALLER_RUN_ID="${HYP_CI_RUN_ID:-}"
RUN_ID="${HYP_CI_RUN_ID:-$$-$(date +%s)}"
export HYP_CI_RUN_ID="$RUN_ID"
LOG="${HYP_VM_TEST_LOG:-/tmp/win-test-${RUN_ID}.log}"

# A caller that sets HYP_CI_RUN_ID is declaring concurrency, so give that run
...
if [ "$1" = "--soak" ]; then
    ...
    binary="${HYP_VM_SOAK_BINARY:-build/c/hyponoia.exe}"
    [ -x "$binary" ] || { echo "ERROR: binary '$binary' missing — build first" >&2; exit 2; }
    artifact="$binary"
elif [ -n "$RUNNER" ]; then
    # Explicit HYP_VM_RUNNER = the sanitizer-variant iteration mode (ubsan /
...
# profile carry no Authenticated-Users ACE, but C:\hyp (like CI's workspace
...
# Only an explicit HYP_VM_RUNNER (sanitizer-variant builds) bypasses test.sh.
if [ -n "$RUNNER" ]; then
    if [ "$1" = "--par" ]; then
...
if [ -n "$CALLER_RUN_ID" ] && [ "${HYP_CI_KEEP:-0}" != "1" ]; then
    if [ "${rc:-1}" -eq 0 ]; then
