...
metadata from
`https://api.github.com/repos/patalbansishashank/hyponoia/releases/latest`.
...
Explicit install, package-manager, and `hyponoia update` flows are
...
[GitHub Security Advisory](https://github.com/patalbansishashank/hyponoia/security/advisories)
...
- **Shell injection prevention** — `hyp_validate_shell_arg()` rejects metacharacters before all `popen()`/`system()` calls
...
gh attestation verify <downloaded-file> \
  ...
  --signer-workflow patalbansishashank/hyponoia/.github/workflows/_build.yml
