...
The binary is output to `build/c/hyponoia`.
...
internal/hyp/       Language registry, AST extraction, and vendored grammars
...
1. **Tree-sitter extraction** (`internal/hyp/`): Grammar loading, AST node type configuration in `lang_specs.c`, function/call/import extraction in `extract_*.c`
2. **Pipeline passes** (`src/pipeline/`): Call resolution, usage tracking, HTTP route linking

**Workflow for language fixes:**

1. Check the language spec in `internal/hyp/lang_specs.c`
2. Use regression tests to verify extraction: `tests/test_extraction.c`
3. Check parity tests: `internal/hyp/regression_test.go` (legacy, being migrated)
...
1. **Detection helpers** in `src/pipeline/pass_infrascan.c` — functions like `hyp_is_dockerfile()`, `hyp_is_k8s_manifest()`, `hyp_is_kustomize_file()` identify files by name and/or content heuristics (e.g., presence of `apiVersion:`).
2. **Custom extractors** in `internal/hyp/extract_k8s.c` — tree-sitter-based parsers that walk the YAML AST (using the tree-sitter YAML grammar) and populate `HYPFileResult` with imports and definitions.
...
- Add a detection helper (`hyp_is_<lang>_file()`) in `pass_infrascan.c` or a new `pass_<lang>.c`.
- Add the `HYP_LANG_<LANG>` enum value in `internal/hyp/hyp.h` and a row in the language table in `lang_specs.c`.
- Write a custom extractor that returns `HYPFileResult*` — do not add a tree-sitter grammar.
...
Check [issues labeled `good first issue`](https://github.com/patalbansishashank/hyponoia/labels/good%20first%20issue) for beginner-friendly tasks with clear scope and guidance.
