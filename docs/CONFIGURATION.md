...
This page documents the configuration files that `hyponoia` reads or writes today.

## At a Glance

| Purpose | Path | Format | Notes |
|---|---|---|---|
| Global custom extension mapping | `$XDG_CONFIG_HOME/hyponoia/config.json` | JSON | Falls back to `~/.config/hyponoia/config.json` when `XDG_CONFIG_HOME` is unset. |
| Per-project custom extension mapping | `{repo_root}/.hyponoia.json` | JSON | Overrides conflicting global `extra_extensions` entries. |
| CLI-managed runtime settings | `${HYP_CACHE_DIR:-~/.cache/hyponoia}/_config.db` | SQLite | Written by `hyponoia config set/reset`. |
| UI settings | `${HYP_CACHE_DIR:-~/.cache/hyponoia}/config.json` | JSON | Stores `ui_enabled` and `ui_port`. |
| Daemon operation log | `${HYP_CACHE_DIR:-~/.cache/hyponoia}/logs/hyp-daemon.log` | Structured log | Durable daemon lifecycle, watcher/indexing, UI, resource, and error events. |
| Admission conflict log | `${HYP_CACHE_DIR:-~/.cache/hyponoia}/logs/daemon-conflicts.ndjson` | NDJSON | Exact-build, ABI, and canonical-cache conflicts. |
| Activation log | `${HYP_CACHE_DIR:-~/.cache/hyponoia}/logs/activation-events.ndjson` | NDJSON | Install/update/uninstall activation progress and outcomes. |

HYP resolves `HYP_CACHE_DIR` to a canonical per-account path before using any of these locations. The log directory and files are private to the account.
...
```text
$XDG_CONFIG_HOME/hyponoia/config.json
```

Fallback when `XDG_CONFIG_HOME` is unset:

```text
~/.config/hyponoia/config.json
```
...
```text
${HYP_CACHE_DIR:-~/.cache/hyponoia}/_config.db
```

Inspect or change values with the CLI:

```bash
hyponoia config list
hyponoia config get auto_index
hyponoia config set auto_index true
hyponoia config set auto_index_limit 50000
hyponoia config reset auto_index
```
...
```text
${HYP_CACHE_DIR:-~/.cache/hyponoia}/config.json
```
...
- `HYP_CACHE_DIR` changes both the UI config location and the runtime settings database location.
- HYP resolves `HYP_CACHE_DIR` to one canonical per-account cache root. A process configured with a different root fails while any HYP session or command is active; close them before switching roots.
...
| `HYP_WORKERS` | auto-detected | Override the indexing worker count. |
...
Independently of `HYP_ALLOWED_ROOT`, some directories are refused as an indexing
