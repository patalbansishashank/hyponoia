# `.hypignore` — Excluding Files from Indexing

`.hypignore` is a project-specific ignore file that controls which files the
indexer sees. It uses gitignore-style syntax and is read from the **root of
the indexed directory** (`<repo>/.hypignore`). Nested `.hypignore` files in
...
matched by `.hypignore` never enters the graph. Changes to `.hypignore` take
...
that rejects a path wins, except `.hypignore` negations (layer 4) can un-skip
...
1. **Built-in skip list** — `.git`, `node_modules`, `dist`, `target`,
  ...
  more, e.g. `docs`, `examples`, `testdata`). A `.hypignore` negation
...
4. **`.hypignore`** — a positive match skips the path; a negated match can
...
after them; none of these are overridable from `.hypignore`. Symlinks are
always skipped.

## Negation (`!`) — current behavior

- **Within `.hypignore`**: standard gitignore semantics. Patterns are
...
- **Parent pruning** (same caveat as git): when a directory is excluded, the
  ...
  contents.
- **Across layers**: a `.hypignore` negation overrides the **git global
  ...
  SQL should be indexed — add `!*.sql` to `.hypignore`. Negation still cannot
