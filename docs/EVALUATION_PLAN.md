...
Measure how well `hyponoia`'s structured knowledge-graph queries answer real developer
...
lowercased enum suffix (`HYP_LANG_GO` → `go`, `HYP_LANG_CSHARP` → `csharp`,
`HYP_LANG_COMMONLISP` → `commonlisp`). The complete repo assignment for all 159 is the master table
in §8.

### The 9 LSP-hybrid languages (deep-dive cohort)

These have dedicated hybrid LSP modules under `internal/hyp/lsp/` and therefore type-aware
...
**One language at a time, in the main channel, never concurrently.** The HYP/graph side of the
...
for each language L in order (skip if manifest says done):
  ...
  8. DELETE L's index from ~/.cache/hyponoia so the next language starts clean,
...
for lang in $ALL_LANGS; do                      # ALL_LANGS = full 159-name list (§2 / §8)
  ...
  t0=$(now_ms)
  scripts/benchmark-index.sh ~/.local/bin/hyponoia "$lang" /tmp/bench/"$lang" /tmp/eval-results
...
`MATCH ()-[r]->() RETURN type(r), count(*)`) / `get_graph_schema` / `hyp_store_count_edges_by_type`,
...
file (`internal/hyp/lang_specs.c`) and the tree-sitter node type that would fix it. Plus a
