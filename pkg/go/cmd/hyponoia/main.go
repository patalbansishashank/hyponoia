...
// Install:
//
//	go install github.com/patalbansishashank/hyponoia/pkg/go/cmd/hyponoia@latest
package main
...
const (
	...
	runtimeSetLockName               = ".hyponoia-runtime.lock"
	...
	runtimeBackupPrefix              = ".hyp-runtime-backup-"
...
func main() {
	...
	if mutation == "update" {
		fmt.Fprintln(
			os.Stderr,
			"This Go wrapper is maintained by Go. Update it with \"go install github.com/patalbansishashank/hyponoia/pkg/go/cmd/hyponoia@latest\".",
		)
		os.Exit(2)
	}
	executable, err := ensureBinary()
	if err != nil {
		fmt.Fprintf(os.Stderr, "hyponoia: %v\n", err)
		os.Exit(1)
	}
...
func printPortableMutationGuidance(args []string) {
	...
	packageCommand := "Remove-Item (Get-Command hyponoia).Source"
	fmt.Fprintf(
		os.Stderr,
...
func defaultManagedInstallDir() string {
	if runtime.GOOS == "windows" {
		...
		return filepath.Join(base, "Programs", "hyponoia")
	}
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".local", "bin")
	}
	return filepath.Join(os.TempDir(), "hyponoia-managed")
}
...
func mutationSnapshotEnvironment() []string {
	blocked := []string{"HYP_ASSETS_DIR", "HYP_UI_ASSETS_DIR"}
...
func cacheDir() string {
	if d := os.Getenv("HYP_CACHE_DIR"); d != "" {
		return d
	}
	switch runtime.GOOS {
	case "windows":
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".cache", "hyponoia")
	}
...
func download(dest string) error {
	...
	fmt.Fprintf(os.Stderr, "hyponoia: downloading v%s for %s/%s...\n", version, platform, arch)

	tmp, err := os.MkdirTemp("", "hyp-install-*")
	if err != nil {
		return err
	}
	defer os.RemoveAll(tmp)

	archivePath := filepath.Join(tmp, "hyp."+ext)
...
func uiPackMatchesDigest(directory, name string) bool {
	...
	expected := strings.TrimSuffix(strings.TrimPrefix(name, "hyp-ui-"), ".pack")
...
func runtimeSetNames(
	...
	for _, entry := range entries {
		name := entry.Name()
		if strings.HasPrefix(name, "hyp-ui-") && strings.HasSuffix(name, ".pack") {
...
func runtimeBackupTargetName(name, binaryName string) bool {
	return name == binaryName || name == integrationsFileName ||
		(strings.HasPrefix(name, "hyp-ui-") && strings.HasSuffix(name, ".pack"))
}
...
func execBinaryWithRuntimeLockAndRunner(
	...
	defer func() {
		...
		if cleanupErr := runtimeMutationSnapshotCleanup(snapshotDirectory); cleanupErr != nil {
			fmt.Fprintf(
				os.Stderr,
				"hyponoia: warning: private mutation snapshot cleanup failed: %v\n",
				cleanupErr,
			)
		}
	}()
