"""Downloads hyponoia on first run, then runs its native entry point."""
...
_WINDOWS_BINARY_NAME = "hyponoia.exe"
_INTEGRATIONS_NAME = "hyp-integrations.json"
_UI_PACK_RE = re.compile(r"hyp-ui-[0-9a-f]{64}\.pack\Z")
_UNIX_ARCHIVE_NAMES = (
    "hyponoia",
    _INTEGRATIONS_NAME,
    "LICENSE",
...
_RUNTIME_LOCK_NAME = ".hyponoia-runtime.lock"
...
_RUNTIME_BACKUP_PREFIX = ".hyp-runtime-backup-"
_RUNTIME_BACKUP_RE = re.compile(r"\.hyp-runtime-backup-[0-9a-f]{32}\Z")
...
def _validate_url_scheme(url: str) -> None:
    ...
    if (
        ...
        sys.exit(
            f"hyponoia: refusing to fetch invalid, credentialed, or "
            f"non-https URL: {url}"
        )
...
def _download_https(url: str, dest: str, max_bytes: int = 0) -> None:
    ...
    for redirect_count in range(_MAX_REDIRECTS + 1):
        ...
        request = urllib.request.Request(
            current_url, headers={"User-Agent": "hyponoia-installer"}
        )
        try:
...
def _validate_archive_names(names, archive_names, ui: bool, casefold: bool = False):
    ...
    for name in names:
        ...
        if key in seen:
            sys.exit(
                f"hyponoia: refusing unsafe archive entry "
        ...
        if name not in required_set and not is_ui_pack:
            sys.exit(
                f"hyponoia: archive must contain only the exact "
                f"root files: {', '.join(required)}"
            )
        if is_ui_pack:
            if ui_pack is not None:
                sys.exit(
                    "hyponoia: archive must contain exactly one "
    ...
    if found != required_set or len(seen) != expected_count or (ui and ui_pack is None):
        sys.exit(
            f"hyponoia: archive must contain exactly one of each "
...
def _safe_extract_tar(
    ...
    for member in members:
        if not member.isfile():
            sys.exit(
                f"hyponoia: refusing unsafe tar entry "
    ...
    for name in runtime_names:
        ...
        if source is None:
            sys.exit(f"hyponoia: could not read archive member: {name}")
...
def _safe_extract_zip(
    ...
    for info in zf.infolist():
        ...
        if (
            ...
            sys.exit(
                f"hyponoia: refusing unsafe zip entry "
...
def _verify_checksum(archive_path: str, archive_name: str, version: str) -> None:
    ...
    try:
        ...
        with open(tmp_path, encoding="utf-8") as f:
            for line in f:
                ...
                if len(digest) != 64 or any(
                    ...
                    sys.exit(
                        f"hyponoia: invalid SHA256 checksum for "
                        f"{archive_name}"
                    )
                if expected is not None and expected != digest:
                    sys.exit(
                        f"hyponoia: conflicting SHA256 checksums for "
                        f"{archive_name}"
                    )
                expected = digest
        if expected is None:
            sys.exit(
                f"hyponoia: no checksum for {archive_name} in checksums.txt"
            )
        h = hashlib.sha256()
        ...
        if expected != actual:
            sys.exit(
                f"hyponoia: CHECKSUM MISMATCH for {archive_name}\n"
                f"  expected: {expected}\n"
                f"  actual:   {actual}"
            )
        print("hyponoia: checksum verified.", file=sys.stderr)
    except SystemExit:
        raise
    except Exception as exc:
        sys.exit(f"hyponoia: checksum verification failed: {exc}")
    finally:
...
def _arch() -> str:
    ...
    if m in ("x86_64", "amd64"):
        return "amd64"
    sys.exit(f"hyponoia: unsupported architecture: {m}")
...
def _ui_pack_path_matches_name(path: Path, name: str) -> bool:
    try:
        expected = name[len("hyp-ui-") : -len(".pack")]
...
def _runtime_set_names(directory: Path, binary_name: str, variant: str):
    ...
    try:
        pack_like = [
            ...
            if path.name.startswith("hyp-ui-") and path.name.endswith(".pack")
        ]
    except OSError:
        return None
    valid_packs = [
...
def _runtime_backup_target_name(name: str, binary_name: str) -> bool:
    return (
        ...
        or (name.startswith("hyp-ui-") and name.endswith(".pack"))
    )
...
def _copy_runtime_backup_file(
    ...
    descriptor, staged_name = tempfile.mkstemp(
        dir=str(target.parent), prefix=".hyp-runtime-recovery-"
    )
    ...
    try:
        ...
        try:
            ...
            pack_like = sorted(
                ...
                if path.name.startswith("hyp-ui-")
...
def _download(version: str) -> Path:
    ...
    print(
        f"hyponoia: downloading v{version} for {os_name}/{arch}...",
        file=sys.stderr,
    )

    with tempfile.TemporaryDirectory() as tmp:
        tmp_archive = os.path.join(tmp, f"hyp.{ext}")
        try:
            _download_https(url, tmp_archive)
        except (OSError, RuntimeError, urllib.error.URLError) as e:
            sys.exit(
                f"hyponoia: download failed ({e})\n"
                f"URL: {url}\n"
        ...
        bin_name = (
            _WINDOWS_BINARY_NAME if os_name == "windows" else "hyponoia"
        )
        ...
        for name in runtime_names:
            ...
            if not _regular_file(extracted_path):
                sys.exit(
                    f"hyponoia: required runtime member not found after "
                    f"extraction: {name}"
                )
            if name == bin_name:
        ...
        if _runtime_set_names(Path(tmp), bin_name, selected_variant) is None:
            sys.exit(
                "hyponoia: extracted runtime set failed content "
                "verification"
            )
        try:
            _verify_candidate(extracted_paths[bin_name])
        except (OSError, RuntimeError) as exc:
            sys.exit(f"hyponoia: {exc}")
    ...
    if mutation in ("install", "uninstall"):
        ...
        if sys.platform == "win32" and result != 0 and mutation == "uninstall":
            print(
                ...
                '"python -m pip uninstall hyponoia" for package maintenance. '
...
def _default_managed_install_dir() -> Path:
    ...
    return local_app_data / "Programs" / "hyponoia"
...
def _run_cache_sensitive_mutation(version: str, executable: Path, args) -> int:
    ...
    try:
        ...
        thread = threading.Thread(
            name="hyp-cache-lock-heartbeat", target=heartbeat, daemon=True
        )
        thread.start()
