 ...
 *   Matches tree-sitter SubtreeHeapData (HYP_SZ_64 bytes). Backed by
 ...
 *
 * Usage:
 *   hyp_slab_install();         // once, before any parsing
 *   ... parse files ...
 *   hyp_slab_destroy_thread();  // on thread exit — frees owned memory
 */
#ifndef HYP_SLAB_ALLOC_H
#define HYP_SLAB_ALLOC_H
 ...
 * Only safe after hyp_destroy_thread_parser() has been called. */
void hyp_slab_reset_thread(void);
...
void hyp_slab_destroy_thread(void);
...
void hyp_slab_reclaim(void);

/* Test/diagnostic API: direct access to the slab allocator.
 * Use these to unit test slab (≤64B) and heap (>64B) paths. */
void *hyp_slab_test_malloc(size_t size);
void hyp_slab_test_free(void *ptr);
void *hyp_slab_test_realloc(void *ptr, size_t size);
void *hyp_slab_test_calloc(size_t count, size_t size);

#endif /* HYP_SLAB_ALLOC_H */
