 ...
 *
 * Restructured from internal/hyp/arena.c with additions:
 *   - hyp_arena_init_sized() for custom block sizes
 *   - hyp_arena_calloc() for zero-initialized allocations
 *   - hyp_arena_reset() for reuse without full destroy
 *   - hyp_arena_total() for allocation tracking
 *
...
void hyp_arena_init(HYPArena *a) {
    hyp_arena_init_sized(a, HYP_ARENA_DEFAULT_BLOCK_SIZE);
}

void hyp_arena_init_sized(HYPArena *a, size_t block_size) {
    memset(a, 0, sizeof(*a));
    if (block_size < HYP_SZ_64) {
        block_size = HYP_SZ_64; /* minimum sanity */
    }
...
static int arena_grow(HYPArena *a, size_t min_size) {
    if (a->nblocks >= HYP_ARENA_MAX_BLOCKS) {
        return 0;
    }
...
char *hyp_arena_strdup(HYPArena *a, const char *s) {
    ...
    size_t len = strlen(s);
    char *dst = (char *)hyp_arena_alloc(a, len + SKIP_ONE);
    if (dst) {
...
char *hyp_arena_sprintf(HYPArena *a, const char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    ...
    char *dst = (char *)hyp_arena_alloc(a, (size_t)needed + SKIP_ONE);
    if (!dst) {
        return NULL;
    }
...
void hyp_arena_reset(HYPArena *a) {
     ...
     * block_size grew during previous allocations (e.g., HYP_SZ_128 → HYP_SZ_256). */
    if (a->nblocks == SKIP_ONE) {
