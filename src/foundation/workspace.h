#ifndef HYP_FOUNDATION_WORKSPACE_H
#define HYP_FOUNDATION_WORKSPACE_H
 ...
 * of HYP_WS_ALLOW here means "this path is not obviously too broad", never
...
typedef enum {
    ...
    HYP_WS_DENY_TOO_SHALLOW,
    /* A filesystem root, a drive root, a UNC share root, or a path that is or
     * contains the hyp cache directory. Never overridable. */
    HYP_WS_DENY_ABSOLUTE,
 ...
 * human override for a HYP_WS_DENY_SENSITIVE path; absolute denials and shallow
 ...
 * HYP_ALLOWED_ROOT / session policy, treated as an additional grant, or NULL.
...
#endif /* HYP_FOUNDATION_WORKSPACE_H */

/*
 * ── Per-project request manifest (.hyppathwhitelist) ─────────────────────────
 *
...
#define HYP_WS_MANIFEST_NAME ".hyppathwhitelist"

enum { HYP_WS_MANIFEST_MAX_ENTRIES = 64 };

typedef struct {
    /* Requested roots, verbatim from the file (already control-char screened). */
    char entries[HYP_WS_MANIFEST_MAX_ENTRIES][1024];
    int count;
...
/* Read <project_root>/.hyppathwhitelist. Returns false only on a malformed file
