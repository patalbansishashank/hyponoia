#ifndef HYP_WIN_UTF8_H
#define HYP_WIN_UTF8_H
...
static inline wchar_t *hyp_utf8_to_wide(const char *utf8) {
    if (!utf8) {
        return NULL;
    }
...
static inline int hyp_win_path_separator(wchar_t value) {
    return value == L'\\' || value == L'/';
}

static inline int hyp_win_path_has_namespace_prefix(const wchar_t *path, size_t length) {
    return length >= 4U && hyp_win_path_separator(path[0]) && hyp_win_path_separator(path[1]) &&
           (path[2] == L'?' || path[2] == L'.') && hyp_win_path_separator(path[3]);
}

static inline void hyp_win_path_normalize_wide_separators(wchar_t *path) {
    if (!path) {
        return;
    }
 ...
 * registry paths). */
static inline wchar_t *hyp_wide_path_to_extended(const wchar_t *wide_path) {
    if (!wide_path) {
        return NULL;
    }
    ...
    if (hyp_win_path_has_namespace_prefix(wide, length)) {
        hyp_win_path_normalize_wide_separators(wide);
        return wide;
    }
    if (length < 240U) {
        return wide;
    ...
    hyp_win_path_normalize_wide_separators(full);
...
static inline wchar_t *hyp_path_to_wide(const char *utf8_path) {
    wchar_t *wide = hyp_utf8_to_wide(utf8_path);
    wchar_t *extended = hyp_wide_path_to_extended(wide);
    free(wide);
    return extended;
}
...
static inline void hyp_module_path_unprefix(wchar_t *wide) {
    if (!wide) {
        return;
    }
...
static inline char *hyp_wide_to_utf8(const wchar_t *wide) {
    if (!wide) {
        return NULL;
    }
...
static inline char *hyp_module_path_utf8(void) {
    DWORD capacity = 512U;
    while (capacity <= 32768U) {
        ...
        if (length > 0U && length < capacity) {
            hyp_module_path_unprefix(wide);
            char *utf8 = hyp_wide_to_utf8(wide);
            free(wide);
            return utf8;
        }
