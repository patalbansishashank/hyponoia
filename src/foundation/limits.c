...
long hyp_max_file_bytes(void) {
    ...
    const char *raw = getenv("HYP_MAX_FILE_BYTES");
    if (raw && raw[0]) {
        errno = 0;
        char *end = NULL;
 ...
 * Read fresh each call (see hyp_max_file_bytes rationale — cheap, test-friendly,
...
int hyp_cypher_max_depth(void) {
    ...
    return env_positive_int("HYP_CYPHER_MAX_DEPTH", 10);
}
...
int hyp_mcp_max_depth(void) {
    ...
    return env_positive_int("HYP_MCP_MAX_DEPTH", 15);
}
