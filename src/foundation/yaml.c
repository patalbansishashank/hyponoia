...
static hyp_yaml_node_t *node_new(yaml_type_t type) {
    hyp_yaml_node_t *n = calloc(HYP_ALLOC_ONE, sizeof(*n));
    if (n) {
        n->type = type;
    }
    return n;
...
static bool node_add_child(hyp_yaml_node_t *parent, hyp_yaml_node_t *child) {
    if (!parent || !child) {
        return false;
    }
    if (parent->child_count >= parent->child_cap) {
        ...
        hyp_yaml_node_t **new_arr = realloc(parent->children, (size_t)new_cap * sizeof(*new_arr));
        if (!new_arr) {
            return false;
        }
...
void hyp_yaml_free(hyp_yaml_node_t *root) {
    ...
    hyp_yaml_node_t *stack[YAML_FREE_STACK];
    int top = 0;
    stack[top++] = root;
    while (top > 0) {
        hyp_yaml_node_t *node = stack[--top];
...
typedef struct {
    hyp_yaml_node_t *node;
    int indent;
} stack_entry_t;

/* Parse a list item "- value" and add to the correct parent. */
static void parse_list_item(const char *content, int content_len, hyp_yaml_node_t *parent) {
    ...
    hyp_yaml_node_t *list_parent = parent;
    if (parent->type == YAML_MAP && parent->child_count > 0) {
        hyp_yaml_node_t *last = parent->children[parent->child_count - SKIP_ONE];
        if (last->type == YAML_LIST) {
            list_parent = last;
        }
    }

    hyp_yaml_node_t *item = node_new(YAML_SCALAR);
    if (item) {
...
static void parse_key_line(const char *content, int content_len, int indent,
                           hyp_yaml_node_t *parent, stack_entry_t *stack, int *stack_depth,
    ...
    if (after_len > 0) {
        /* "key: value" — scalar */
        hyp_yaml_node_t *child = node_new(YAML_SCALAR);
        if (child) {
            child->key = key;
        ...
        hyp_yaml_node_t *child = node_new(is_list ? YAML_LIST : YAML_MAP);
        if (child) {
            child->key = key;
            if (!node_add_child(parent, child)) {
                hyp_yaml_free(child);
            } else if (*stack_depth < HYP_SZ_32) {
...
hyp_yaml_node_t *hyp_yaml_parse(const char *text, int len) {
    ...
    hyp_yaml_node_t *root = node_new(YAML_MAP);
    if (!root) {
        return NULL;
    }

    /* Stack for tracking parent context */
    stack_entry_t stack[HYP_SZ_32];
    int stack_depth = 0;
    ...
    while (p < end) {
        ...
        hyp_yaml_node_t *parent = stack[stack_depth - SKIP_ONE].node;
...
static const hyp_yaml_node_t *find_child(const hyp_yaml_node_t *node, const char *key) {
    if (!node || !key) {
        return NULL;
    }
...
static const hyp_yaml_node_t *navigate(const hyp_yaml_node_t *root, const char *path) {
    if (!root || !path) {
        return NULL;
    }

    const hyp_yaml_node_t *cur = root;
    char buf[HYP_SZ_256];
    const char *p = path;
...
const char *hyp_yaml_get_str(const hyp_yaml_node_t *root, const char *path) {
    const hyp_yaml_node_t *node = navigate(root, path);
...
double hyp_yaml_get_float(const hyp_yaml_node_t *root, const char *path, double default_val) {
    const char *str = hyp_yaml_get_str(root, path);
    if (!str) {
        return default_val;
    }
    char *endptr;
