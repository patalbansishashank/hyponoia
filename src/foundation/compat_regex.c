...
_Static_assert(sizeof(regex_t) <= HYP_SZ_256,
               "hyp_regex_t opaque buffer too small for TRE regex_t");

static int translate_flags_tre(int flags) {
    int tre_flags = 0;
    if (flags & HYP_REG_EXTENDED)
        tre_flags |= REG_EXTENDED;
    if (flags & HYP_REG_ICASE)
        tre_flags |= REG_ICASE;
    if (flags & HYP_REG_NOSUB)
        tre_flags |= REG_NOSUB;
    if (flags & HYP_REG_NEWLINE)
...
int hyp_regexec(const hyp_regex_t *r, const char *str, int nmatch, hyp_regmatch_t *matches,
    ...
    if (nmatch <= 0 || !matches) {
        ...
        return rc == 0 ? HYP_REG_OK : HYP_REG_NOMATCH;
    }
    regmatch_t pmatch[HYP_SZ_32];
    int n = nmatch > HYP_SZ_32 ? HYP_SZ_32 : nmatch;
    int rc = tre_regexec(re, str, (size_t)n, pmatch, eflags);
    if (rc != 0)
        return HYP_REG_NOMATCH;
    for (int i = 0; i < n; i++) {
 ...
 * If this fires, increase hyp_regex_t.opaque size. */
_Static_assert(sizeof(regex_t) <= HYP_SZ_256, "hyp_regex_t opaque buffer too small for regex_t");
