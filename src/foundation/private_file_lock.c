...
struct hyp_private_lock_directory {
    int fd;
    char *path;
    dev_t device;
    ino_t inode;
...
struct hyp_private_file_lock {
    int fd;
    pid_t owner_pid;
    hyp_private_file_lock_mode_t mode;
    struct hyp_private_file_lock *next_tracked;
    bool unlocked;
...
struct hyp_private_fork_condition {
...
static hyp_private_file_lock_t *private_tracked_locks;
...
static void private_atfork_child(void) {
    hyp_private_file_lock_t *lock = private_tracked_locks;
    while (lock) {
        if (lock->fd >= 0) {
...
hyp_private_fork_condition_t *hyp_private_fork_condition_new(void) {
    hyp_private_fork_condition_t *condition = calloc(1, sizeof(*condition));
...
hyp_private_fork_wait_status_t hyp_private_fork_condition_wait_until_while_guarded(
    ...
    if (!condition) {
        return HYP_PRIVATE_FORK_WAIT_ERROR;
    }
    int status;
    ...
    if (status == 0) {
        return HYP_PRIVATE_FORK_WAIT_SIGNALED;
    }
    return status == ETIMEDOUT ? HYP_PRIVATE_FORK_WAIT_TIMEOUT : HYP_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool private_directory_revalidate(const hyp_private_lock_directory_t *directory) {
    ...
    return directory && directory->owner_pid == getpid() && directory->fd >= 0 &&
           ...
           hyp_macos_extended_acl_fd_is_empty(directory->fd);
}
...
static bool private_file_revalidate(const hyp_private_lock_directory_t *directory,
    ...
    return private_directory_revalidate(directory) && fstat(fd, &by_handle) == 0 &&
           ...
           by_path.st_ino == by_handle.st_ino && hyp_macos_extended_acl_fd_is_empty(fd) &&
           (!expected ||
            (expected->st_dev == by_handle.st_dev && expected->st_ino == by_handle.st_ino));
}

static hyp_private_file_lock_status_t private_open_failure_status(
    const hyp_private_lock_directory_t *directory, const char *base_name) {
    struct stat status;
    if (fstatat(directory->fd, base_name, &status, AT_SYMLINK_NOFOLLOW) == 0) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    return errno == ENOENT ? HYP_PRIVATE_FILE_LOCK_IO : HYP_PRIVATE_FILE_LOCK_UNSAFE;
}
...
static int private_release_unlock(hyp_private_file_lock_t *lock) {
...
static int private_release_close(hyp_private_file_lock_t *lock, bool *attempted_out) {
    *attempted_out = false;
...
hyp_private_file_lock_status_t hyp_private_lock_directory_adopt_posix(
    ...
    if (fstat(directory_fd, &status) != 0 || lstat(stable_path, &by_path) != 0 ||
        ...
        status.st_ino != by_path.st_ino || !hyp_macos_extended_acl_fd_is_empty(directory_fd)) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    hyp_private_lock_directory_t *directory = calloc(1, sizeof(*directory));
    if (directory) {
...
hyp_private_file_lock_status_t hyp_private_file_lock_try_acquire(
    ...
    if (!directory || !lock_out || !private_base_name_valid(base_name) ||
        (mode != HYP_PRIVATE_FILE_LOCK_SH && mode != HYP_PRIVATE_FILE_LOCK_EX)) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    ...
    if (fd < 0) {
        hyp_private_file_lock_status_t status = private_open_failure_status(directory, base_name);
    ...
    hyp_private_file_lock_t *lock = calloc(1, sizeof(*lock));
    if (!lock) {
        (void)close(fd);
        hyp_private_file_lock_fork_guard_leave();
        return HYP_PRIVATE_FILE_LOCK_IO;
    ...
    int operation = mode == HYP_PRIVATE_FILE_LOCK_SH ? LOCK_SH : LOCK_EX;
    if (private_flock_set(fd, operation | LOCK_NB) != 0) {
        int lock_error = errno;
        (void)close(fd);
        free(lock);
        hyp_private_file_lock_fork_guard_leave();
        return lock_error == EWOULDBLOCK || lock_error == EAGAIN ? HYP_PRIVATE_FILE_LOCK_BUSY
    ...
    if (forced_cleanup || !private_file_revalidate(directory, base_name, fd, &initial)) {
        hyp_private_file_lock_status_t failure_status =
            forced_cleanup ? HYP_PRIVATE_FILE_LOCK_IO : HYP_PRIVATE_FILE_LOCK_UNSAFE;
        lock->test_fail_unlock_once = fail_cleanup_unlock;
        lock->test_fail_close_once = fail_cleanup_close;
        *lock_out = lock;
        hyp_private_file_lock_fork_guard_leave();
        hyp_private_file_lock_status_t cleanup_status = hyp_private_file_lock_release(lock_out);
        return cleanup_status == HYP_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : HYP_PRIVATE_FILE_LOCK_IO;
    }
    *lock_out = lock;
    hyp_private_file_lock_fork_guard_leave();
...
static bool private_lock_is_tracked(const hyp_private_file_lock_t *lock) {
    for (const hyp_private_file_lock_t *cursor = private_tracked_locks; cursor;
...
static bool private_payload_fd_valid(const hyp_private_file_lock_t *lock, struct stat *status_out) {
    struct stat status;
    bool valid = lock && lock->fd >= 0 && !lock->unlocked && lock->owner_pid == getpid() &&
                 ...
                 hyp_macos_extended_acl_fd_is_empty(lock->fd);
    if (valid && status_out) {
        *status_out = status;
    }
    return valid;
...
hyp_private_file_lock_status_t hyp_private_file_lock_payload_write(hyp_private_file_lock_t *lock,
    ...
    if (!lock || !buffer || length == 0 || length > PRIVATE_FILE_LOCK_PAYLOAD_CAP ||
        lock->mode != HYP_PRIVATE_FILE_LOCK_EX) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!hyp_private_file_lock_fork_guard_enter()) {
        return HYP_PRIVATE_FILE_LOCK_IO;
    ...
    return valid ? HYP_PRIVATE_FILE_LOCK_OK : HYP_PRIVATE_FILE_LOCK_IO;
}
...
hyp_private_file_lock_status_t hyp_private_file_lock_release(hyp_private_file_lock_t **lock_io) {
    ...
    hyp_private_file_lock_t *lock = *lock_io;
    ...
    hyp_private_file_lock_t **cursor = &private_tracked_locks;
    ...
    *lock_io = NULL;
    free(lock);
    return close_status == 0 ? HYP_PRIVATE_FILE_LOCK_OK : HYP_PRIVATE_FILE_LOCK_IO;
}
...
bool hyp_private_file_lock_fail_next_release_step_for_test(
    ...
    if (step == HYP_PRIVATE_FILE_LOCK_RELEASE_UNLOCK) {
        lock->test_fail_unlock_once = true;
        return true;
    }
    if (step == HYP_PRIVATE_FILE_LOCK_RELEASE_CLOSE) {
...
static hyp_private_file_lock_status_t private_win_path_from_utf8(const char *path,
                                                                 wchar_t **wide_out) {
    *wide_out = NULL;
...
static bool private_win_directory_handle_valid(hyp_private_lock_directory_t *directory,
...
static bool private_win_directory_path_matches(hyp_private_lock_directory_t *directory) {
    HANDLE probe =
...
static bool private_win_directory_revalidate(hyp_private_lock_directory_t *directory) {
    BY_HANDLE_FILE_INFORMATION information;
...
static wchar_t *private_win_file_path(const hyp_private_lock_directory_t *directory,
...
static bool private_win_file_handle_valid(hyp_private_lock_directory_t *directory, HANDLE handle,
                                          BY_HANDLE_FILE_INFORMATION *information_out) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_file_revalidate(hyp_private_lock_directory_t *directory,
...
static hyp_private_file_lock_status_t private_win_open_failure_status(const wchar_t *path) {
    DWORD attributes = GetFileAttributesW(path);
    return attributes == INVALID_FILE_ATTRIBUTES ? HYP_PRIVATE_FILE_LOCK_IO
                                                 : HYP_PRIVATE_FILE_LOCK_UNSAFE;
}
...
hyp_private_file_lock_status_t hyp_private_lock_directory_adopt_windows(
    ...
    hyp_private_file_lock_status_t path_status =
        private_win_path_from_utf8(stable_path, &wide_path);
    if (path_status != HYP_PRIVATE_FILE_LOCK_OK) {
        return path_status;
    }
    private_win_security_t security;
    ...
    hyp_private_lock_directory_t candidate;
...
hyp_private_file_lock_status_t hyp_private_file_lock_try_acquire(
    ...
    if (handle == INVALID_HANDLE_VALUE) {
        hyp_private_file_lock_status_t status = private_win_open_failure_status(path);
        hyp_private_file_lock_fork_guard_leave();
        free(path);
        return status;
    }
    ...
    if (!native_locked) {
        ...
        *lock_out = lock;
        hyp_private_file_lock_fork_guard_leave();
        free(path);
        hyp_private_file_lock_status_t failure_status = lock_error == ERROR_LOCK_VIOLATION
                                                            ? HYP_PRIVATE_FILE_LOCK_BUSY
                                                            : HYP_PRIVATE_FILE_LOCK_IO;
        hyp_private_file_lock_status_t cleanup_status = hyp_private_file_lock_release(lock_out);
        return cleanup_status == HYP_PRIVATE_FILE_LOCK_OK ? failure_status
                                                          : HYP_PRIVATE_FILE_LOCK_IO;
...
static bool private_win_payload_handle_valid(const hyp_private_file_lock_t *lock) {
    BY_HANDLE_FILE_INFORMATION information;
...
static bool private_win_release_unlock(hyp_private_file_lock_t *lock) {
    lock->test_unlock_attempts++;
    if (lock->test_fail_unlock_once) {
        lock->test_fail_unlock_once = false;
...
static bool private_win_release_close(hyp_private_file_lock_t *lock) {
    lock->test_close_attempts++;
    if (lock->test_fail_close_once) {
        lock->test_fail_close_once = false;
        SetLastError(ERROR_GEN_FAILURE);
...
hyp_private_fork_wait_status_t hyp_private_fork_condition_wait_until_while_guarded(
    ...
    return GetLastError() == ERROR_TIMEOUT ? HYP_PRIVATE_FORK_WAIT_TIMEOUT
                                           : HYP_PRIVATE_FORK_WAIT_ERROR;
}
