...
static hyp_mutex_t g_profile_mutex;
static profile_site_t g_sites[PROFILE_SITES];
static profile_pointer_t g_pointers[PROFILE_POINTERS];
static hyp_mem_profile_totals_t g_totals;
static atomic_int g_profile_enabled = -1;
static size_t g_profile_min = PROFILE_DEFAULT_MIN;

size_t hyp_mem_profile_threshold(void) {
    (void)hyp_mem_profile_enabled();
    return g_profile_min;
}

bool hyp_mem_profile_enabled(void) {
    int state = atomic_load_explicit(&g_profile_enabled, memory_order_acquire);
    if (state >= 0) {
        return state == 1;
    }
    char buf[HYP_SZ_32];
    bool on = hyp_safe_getenv("HYP_MEM_PROFILE", buf, sizeof(buf), NULL) != NULL && buf[0] == '1';
    if (on) {
        char min_buf[HYP_SZ_32];
        if (hyp_safe_getenv("HYP_MEM_PROFILE_MIN", min_buf, sizeof(min_buf), NULL) != NULL) {
            long parsed = strtol(min_buf, NULL, 10);
            if (parsed > 0) {
                g_profile_min = (size_t)parsed;
            }
        }
        hyp_mutex_init(&g_profile_mutex);
    }
...
void hyp_mem_profile_alloc(void *block, size_t size) {
    hyp_mem_profile_alloc_at(block, size, NULL);
}

void hyp_mem_profile_alloc_at(void *block, size_t size, void *caller) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!hyp_mem_profile_enabled() || size < g_profile_min) {
    ...
    if (count <= 0) {
        hyp_mutex_lock(&g_profile_mutex);
        g_totals.capture_failed++;
        hyp_mutex_unlock(&g_profile_mutex);
        g_profile_reentrant = false;
        return;
    }
    hyp_mutex_lock(&g_profile_mutex);
...
void hyp_mem_profile_free(void *block) {
    if (!block || g_profile_reentrant) {
        return;
    }
    g_profile_reentrant = true;
    if (!hyp_mem_profile_enabled()) {
        g_profile_reentrant = false;
        return;
    }
    hyp_mutex_lock(&g_profile_mutex);
...
void hyp_mem_profile_totals(hyp_mem_profile_totals_t *out) {
    if (!out) {
        return;
    }
...
bool hyp_mem_profile_dump(const char *path, const char *label) {
    if (!path || !hyp_mem_profile_enabled()) {
        return false;
    }
    FILE *out = hyp_fopen(path, "ab");
    if (!out) {
        return false;
    }
    hyp_mutex_lock(&g_profile_mutex);
