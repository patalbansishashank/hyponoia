...
static _Atomic HYPLogLevel g_log_level = HYP_LOG_INFO;
static _Atomic HYPLogFormat g_log_format = HYP_LOG_FORMAT_TEXT;
...
static _Atomic hyp_log_sink_fn g_log_sink = (hyp_log_sink_fn)NULL;
static _Atomic HYPLogSinkMode g_log_sink_mode = HYP_LOG_SINK_REPLACE;

/* HYP_LOG_LEVEL support — distilled from #414 (closes #413, thanks @santanusinha). */
void hyp_log_init_from_env(void) {
    ...
    const char *raw = getenv("HYP_LOG_LEVEL");
    if (raw && raw[0] != '\0') {
        ...
        if (raw[i] == '\0') { /* fully consumed — candidate textual match */
            for (size_t lvl = 0; lvl < sizeof(names) / sizeof(names[0]); lvl++) {
                if (strcmp(lower, names[lvl]) == 0) {
                    hyp_log_set_level((HYPLogLevel)lvl);
                    goto parse_format;
                }
            }
        }

        /* Numeric form: 0=debug .. 4=none, matching HYPLogLevel. */
        char *end = NULL;
        long n = strtol(raw, &end, HYP_DECIMAL_BASE);
        if (end != raw && *end == '\0' && n >= HYP_LOG_DEBUG && n <= HYP_LOG_NONE) {
            hyp_log_set_level((HYPLogLevel)n);
        }
    }
...
parse_format:;
    const char *fmt = getenv("HYP_LOG_FORMAT");
    if (fmt && fmt[0] != '\0') {
        char lower_fmt[8];
        size_t i = 0;
...
void hyp_log_set_sink(hyp_log_sink_fn fn) {
    hyp_log_set_sink_ex(fn, HYP_LOG_SINK_REPLACE);
}
...
static const char *level_str(HYPLogLevel level) {
    switch (level) {
    case HYP_LOG_DEBUG:
        return "debug";
    case HYP_LOG_INFO:
...
static void emit_line(const char *line) {
     ...
     * let a concurrent hyp_log_set_sink turn a checked pointer into a NULL
     * call. */
    hyp_log_sink_fn sink = atomic_load_explicit(&g_log_sink, memory_order_relaxed);
    if (sink) {
        sink(line);
        if (atomic_load_explicit(&g_log_sink_mode, memory_order_relaxed) == HYP_LOG_SINK_REPLACE) {
            return;
        }
    }
...
void hyp_log(HYPLogLevel level, const char *msg, ...) {
    ...
    char line_buf[HYP_SZ_4K];
    size_t pos = 0;
    va_list args;
    va_start(args, msg);

    if (atomic_load_explicit(&g_log_format, memory_order_relaxed) == HYP_LOG_FORMAT_JSON) {
...
void hyp_log_control_record(const char *msg, ...) {
     ...
     * HYP_LOG_LEVEL suppression. Always JSON, independent of HYP_LOG_FORMAT,
...
void hyp_log_int(HYPLogLevel level, const char *msg, const char *key, int64_t value) {
    char value_buf[HYP_SZ_32];
    snprintf(value_buf, sizeof(value_buf), "%" PRId64, value);
    hyp_log(level, msg, key ? key : "?", value_buf, NULL);
}
...
void hyp_log_mcp_request(const char *method, const char *tool_name, bool is_error,
                         int64_t duration_us) {
    char duration_ms[HYP_SZ_32];
    snprintf(duration_ms, sizeof(duration_ms), "%" PRId64, duration_us / 1000);
    if (tool_name && tool_name[0] != '\0') {
        hyp_log(is_error ? HYP_LOG_WARN : HYP_LOG_INFO, "mcp.request", "protocol", "jsonrpc",
...
void hyp_log_http_request(const char *component, const char *method, const char *path, int status,
    ...
    char safe_path[HYP_SZ_1K];
    char status_buf[HYP_SZ_16];
    char duration_buf[HYP_SZ_32];
    char request_buf[HYP_SZ_32];
    char response_buf[HYP_SZ_32];
    ...
    HYPLogLevel level = HYP_LOG_INFO;
    if (status >= 500) {
        level = HYP_LOG_ERROR;
    } else if (status >= 400) {
        level = HYP_LOG_WARN;
    ...
    hyp_log(level, "http.request", "component", component ? component : "", "method",
