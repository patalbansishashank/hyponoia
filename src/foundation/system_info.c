...
enum { DEFAULT_CORES = 1, MIN_WORKERS = 1, HYP_WORKERS_MAX = 256 };
...
static hyp_system_info_t detect_system_macos(void) {
    hyp_system_info_t info;
...
static hyp_system_info_t detect_system_bsd(void) {
    hyp_system_info_t info;
    memset(&info, 0, sizeof(info));
...
int hyp_detect_cgroup_cpus(const char *cgroup_root) {
    ...
    long quota = strtol(buf, NULL, HYP_DECIMAL_BASE);
    if (quota <= 0) {
        return -1;
    }
    ...
    long period = strtol(buf, NULL, HYP_DECIMAL_BASE);
    if (period <= 0) {
        return -1;
    }
...
size_t hyp_detect_cgroup_mem(const char *cgroup_root) {
    char path[HYP_PATH_MAX];
    char buf[HYP_SZ_64];
    ...
    if (read_small_file(path, buf, sizeof(buf)) > 0) {
        ...
        char *end = NULL;
        unsigned long long n = strtoull(buf, &end, HYP_DECIMAL_BASE);
        if (end == buf || n == 0) {
            return 0;
        }
        return (size_t)n;
...
static hyp_system_info_t detect_system_linux(void) {
    hyp_system_info_t info;
    memset(&info, 0, sizeof(info));
    ...
    int cg_cpus = hyp_detect_cgroup_cpus("/sys/fs/cgroup");
    ...
    size_t cg_ram = hyp_detect_cgroup_mem("/sys/fs/cgroup");
...
#ifdef _WIN32
static hyp_system_info_t detect_system_windows(void) {
    hyp_system_info_t info;
    memset(&info, 0, sizeof(info));
...
static hyp_system_info_t cached_info;

hyp_system_info_t hyp_system_info(void) {
    if (!info_cached) {
#ifdef _WIN32
...
int hyp_default_worker_count(bool initial) {
    /* HYP_WORKERS env override (clamped to [1, HYP_WORKERS_MAX]).
     ...
     * Same precedence shape as other HYP_* env overrides:
     * explicit override > implicit detection. */
    char buf[HYP_SZ_32];
    if (hyp_safe_getenv("HYP_WORKERS", buf, sizeof(buf), NULL) != NULL) {
        long n = strtol(buf, NULL, HYP_DECIMAL_BASE);
        if (n >= MIN_WORKERS && n <= HYP_WORKERS_MAX) {
            return (int)n;
        }
        hyp_log_warn("workers.env.invalid", "value", buf, "fallback", "sysconf");
    }
