...
hyp_query_stats_t g_query_stats = {0};
static atomic_bool g_diag_stop = false;
static atomic_bool g_diag_done = false;
static hyp_thread_t g_diag_thread;
static bool g_diag_started = false;
static bool g_diag_abandoned = false;
static time_t g_start_time = 0;
static hyp_private_lock_directory_t *g_diag_directory = NULL;
#ifndef _WIN32
static int g_diag_directory_fd = -1;
#endif
static char g_diag_directory_path[HYP_PATH_MAX] = "";
static char g_diag_path[HYP_PATH_MAX] = "";
static char g_diag_ndjson_path[HYP_PATH_MAX] = "";
 ...
 * committed bytes cannot tell apart. Off unless HYP_MEM_STATS=1. */
...
#ifdef _WIN32
    /* _wopen mirrors hyp_mkstemp's Windows contract — the ANSI CRT interprets
...
static void diag_write_allocator_stats(void) {
    char flag[HYP_SZ_16];
    if (hyp_safe_getenv("HYP_MEM_STATS", flag, sizeof(flag), NULL) == NULL || flag[0] != '1') {
        return;
    }
    ...
    int written =
        snprintf(path, sizeof(path), "%s/hyp-allocator-stats-%d.txt", hyp_tmpdir(), (int)getpid());
...
#endif
    hyp_private_lock_directory_close(g_diag_directory);
    g_diag_directory = NULL;
    if (remove_directory && g_diag_directory_path[0] != '\0') {
        (void)hyp_rmdir(g_diag_directory_path);
    }
}

/* Diagnostics placement honors the documented contract ($TMPDIR, /tmp
 * fallback) without touching hyp_tmpdir(), whose many other callers keep
...
static const char *diag_tmp_base(char *buffer, size_t size) {
    ...
    return hyp_tmpdir();
#else
    hyp_safe_getenv("TMPDIR", buffer, size, NULL);
    if (buffer[0] != '\0') {
        return buffer;
    }
    return "/tmp";
...
static bool diag_directory_prepare(void) {
    char tmp_base[HYP_PATH_MAX];
    char directory_template[HYP_PATH_MAX];
    int written =
        snprintf(directory_template, sizeof(directory_template), "%s/hyp-diagnostics-%d-XXXXXX",
                 diag_tmp_base(tmp_base, sizeof(tmp_base)), (int)getpid());
    if (written <= 0 || (size_t)written >= sizeof(directory_template) ||
        !hyp_mkdtemp(directory_template)) {
        return false;
    }
    written =
    ...
    if (written <= 0 || (size_t)written >= sizeof(g_diag_directory_path)) {
        (void)hyp_rmdir(directory_template);
        return false;
    }

#ifdef _WIN32
    wchar_t *wide_directory = hyp_path_to_wide(g_diag_directory_path);
    HANDLE handle =
        wide_directory
    ...
    free(wide_directory);
    hyp_private_file_lock_status_t status = HYP_PRIVATE_FILE_LOCK_IO;
    if (handle != INVALID_HANDLE_VALUE) {
        status = hyp_private_lock_directory_adopt_windows(handle, g_diag_directory_path,
                                                          &g_diag_directory);
    ...
    hyp_private_file_lock_status_t status = HYP_PRIVATE_FILE_LOCK_IO;
    if (same_directory) {
        status = hyp_private_lock_directory_adopt_posix(validation_fd, g_diag_directory_path,
                                                        &g_diag_directory);
        if (status == HYP_PRIVATE_FILE_LOCK_OK) {
            validation_fd = -1;
    ...
    if (status != HYP_PRIVATE_FILE_LOCK_OK || !diag_set_output_paths()) {
...
static bool diag_posix_file_valid(int descriptor) {
    ...
    return descriptor >= 0 && fstat(descriptor, &state) == 0 && S_ISREG(state.st_mode) &&
           ...
           hyp_macos_extended_acl_fd_is_empty(descriptor);
}
...
static void diag_wait_for_interval(void) {
    uint64_t deadline = hyp_now_ms() + DIAG_INTERVAL_MS;
...
bool hyp_diag_start(void) {
    ...
    hyp_safe_getenv("HYP_DIAGNOSTICS", env_buf, sizeof(env_buf), NULL);
    ...
    if (hyp_thread_create(&g_diag_thread, 0, diag_thread_fn, NULL) != 0) {
        diag_directory_close(true);
        return false;
    }

    g_diag_started = true;
    char interval[HYP_SZ_32];
    (void)snprintf(interval, sizeof(interval), "%d", DIAG_INTERVAL_MS / 1000);
    /* Discovery must survive HYP_LOG_LEVEL suppression and paths containing
     * spaces: a control record is the only announced copy of these paths. */
    hyp_log_control("diagnostics.start", "snapshot", g_diag_path, "trajectory", g_diag_ndjson_path,
                    "interval_s", interval);
    return true;
}
...
void hyp_diag_stop(void) {
    ...
    uint64_t deadline = hyp_now_ms() + DIAG_STOP_TIMEOUT_MS;
    while (!atomic_load_explicit(&g_diag_done, memory_order_acquire) && hyp_now_ms() < deadline) {
        diag_sleep_ms(DIAG_WAIT_SLICE_MS);
    }
    ...
    if (completed) {
        ...
        (void)hyp_thread_detach(&g_diag_thread);
