...
struct hyp_lock_registry {
    hyp_private_lock_directory_t *directory;
    hyp_private_fork_condition_t *condition;
    hyp_mutex_t mutex;
    ...
    uint64_t owner_pid;
    bool closing;
    hyp_lock_registry_stage_hook_fn stage_hook;
    void *stage_context;
    hyp_lock_registry_release_handle_t test_release_fault_handle;
    hyp_private_file_lock_release_step_t test_release_fault_step;
    bool test_release_fault_armed;
    hyp_lock_registry_abort_failure_t test_abort_failure;
    bool test_abort_failure_armed;
    atomic_uint_fast64_t test_condition_wait_calls;
    atomic_size_t test_condition_waiters_now;
    struct hyp_lock_registry *next_live;
    struct hyp_lock_registry *next_retired;
};

struct hyp_lock_lease {
    hyp_lock_registry_t *registry;
    lock_registry_entry_t *entry;
    lock_registry_waiter_t waiter;
    hyp_private_file_lock_mode_t mode;
    hyp_private_file_lock_t *turn;
    hyp_private_file_lock_t *rw;
    uint64_t owner_pid;
    bool active;
    bool cleanup_only;
...
/* Protected by hyp_private_file_lock_fork_guard_enter(). Besides serializing
 ...
 * freed mutex. */
static hyp_lock_registry_t *lock_registry_live;
...
static hyp_lock_registry_t *lock_registry_retired;
...
static bool lock_registry_is_live_unlocked(const hyp_lock_registry_t *registry) {
    for (const hyp_lock_registry_t *cursor = lock_registry_live; cursor;
...
static bool lock_registry_lock(hyp_lock_registry_t *registry) {
    if (!registry || !hyp_private_file_lock_fork_guard_enter()) {
        return false;
    }
    if (!lock_registry_is_live_unlocked(registry)) {
        hyp_private_file_lock_fork_guard_leave();
        return false;
    }
    hyp_mutex_lock(&registry->mutex);
    if (registry->closing || registry->owner_pid != lock_registry_current_pid()) {
        hyp_mutex_unlock(&registry->mutex);
        hyp_private_file_lock_fork_guard_leave();
        return false;
    }
    return true;
...
static void lock_registry_unlock(hyp_lock_registry_t *registry) {
    hyp_mutex_unlock(&registry->mutex);
    hyp_private_file_lock_fork_guard_leave();
}

/* Requires the global fork guard and registry mutex, in that order. */
static void lock_registry_broadcast_locked(hyp_lock_registry_t *registry) {
    hyp_private_fork_condition_broadcast_while_guarded(registry->condition);
}
...
static hyp_private_fork_wait_status_t lock_registry_wait_locked(hyp_lock_registry_t *registry,
    ...
    hyp_mutex_unlock(&registry->mutex);
    hyp_private_fork_wait_status_t status =
        hyp_private_fork_condition_wait_until_while_guarded(registry->condition, deadline_ms);
    hyp_mutex_lock(&registry->mutex);
...
static bool lock_registry_park_native(hyp_lock_registry_t *registry, uint64_t deadline_ms,
                                      const hyp_lock_cancel_token_t *cancel_token) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    hyp_private_fork_wait_status_t status = HYP_PRIVATE_FORK_WAIT_SIGNALED;
    ...
    return status != HYP_PRIVATE_FORK_WAIT_ERROR;
}
...
static bool lock_registry_should_stop(uint64_t deadline_ms,
    ...
    return deadline_ms != UINT64_MAX && hyp_now_ms() >= deadline_ms;
}

static void lock_registry_notify(hyp_lock_registry_t *registry, hyp_private_file_lock_mode_t mode,
                                 hyp_lock_registry_stage_t stage) {
    if (!lock_registry_lock(registry)) {
        return;
    }
    hyp_lock_registry_stage_hook_fn hook = registry->stage_hook;
    if (!hook) {
        lock_registry_unlock(registry);
        return;
    }
...
static lock_registry_entry_t *lock_registry_find_entry(hyp_lock_registry_t *registry,
...
static void lock_registry_waiter_push(hyp_lock_registry_t *registry, lock_registry_entry_t *entry,
                                      lock_registry_waiter_t *waiter) {
    waiter->next = NULL;
    waiter->queued = true;
...
static bool lock_registry_waiter_remove(hyp_lock_registry_t *registry, lock_registry_entry_t *entry,
                                        lock_registry_waiter_t *waiter) {
    if (!waiter->queued) {
        return false;
    }
...
static void lock_registry_apply_release_fault(hyp_lock_registry_t *registry,
    ...
    hyp_private_file_lock_release_step_t step = registry->test_release_fault_step;
    if (inject) {
        registry->test_release_fault_armed = false;
    }
    lock_registry_unlock(registry);
    if (inject) {
        (void)hyp_private_file_lock_fail_next_release_step_for_test(lock, step);
    }
}

/* Rollback and writer release are deliberately rw-before-turn. */
static hyp_private_file_lock_status_t lock_registry_release_native(
    hyp_lock_registry_t *registry, hyp_private_file_lock_t **rw_io,
    hyp_private_file_lock_t **turn_io) {
    hyp_private_file_lock_status_t result = HYP_PRIVATE_FILE_LOCK_OK;
    lock_registry_apply_release_fault(registry, HYP_LOCK_REGISTRY_RELEASE_RW, *rw_io);
    if (*rw_io && hyp_private_file_lock_release(rw_io) != HYP_PRIVATE_FILE_LOCK_OK) {
        result = HYP_PRIVATE_FILE_LOCK_IO;
        if (*rw_io && !hyp_private_file_lock_unlock_complete(*rw_io)) {
            return result;
        }
    }
    lock_registry_apply_release_fault(registry, HYP_LOCK_REGISTRY_RELEASE_TURN, *turn_io);
    if (*turn_io && hyp_private_file_lock_release(turn_io) != HYP_PRIVATE_FILE_LOCK_OK) {
        result = HYP_PRIVATE_FILE_LOCK_IO;
    }
    return result;
}
...
bool hyp_lock_registry_resource_names(const char *resource_key,
    ...
    hyp_sha256_hex(resource_key, length, digest);
    int turn_written = snprintf(turn_out, HYP_LOCK_REGISTRY_NAME_CAP, "hyp-%s.turn", digest);
    int rw_written = snprintf(rw_out, HYP_LOCK_REGISTRY_NAME_CAP, "hyp-%s.rw", digest);
    return turn_written > 0 && turn_written < (int)HYP_LOCK_REGISTRY_NAME_CAP && rw_written > 0 &&
           rw_written < (int)HYP_LOCK_REGISTRY_NAME_CAP;
}

hyp_lock_registry_t *hyp_lock_registry_new(hyp_private_lock_directory_t *directory) {
    if (!directory) {
        return NULL;
    }
    hyp_lock_registry_t *registry = calloc(1, sizeof(*registry));
    if (!registry) {
        return NULL;
    }
    registry->directory = directory;
    registry->owner_pid = lock_registry_current_pid();
    hyp_mutex_init(&registry->mutex);
    registry->condition = hyp_private_fork_condition_new();
    if (!registry->condition) {
        hyp_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
    if (!hyp_private_file_lock_fork_guard_enter()) {
        hyp_private_fork_condition_free(registry->condition);
        hyp_mutex_destroy(&registry->mutex);
        free(registry);
        return NULL;
    }
...
hyp_private_file_lock_status_t hyp_lock_registry_request_cancel(hyp_lock_registry_t *registry,
...
static void lock_registry_retain_waiter_cleanup(hyp_lock_lease_t *lease,
                                                hyp_private_file_lock_t **rw_io,
                                                hyp_private_file_lock_t **turn_io,
                                                hyp_lock_lease_t **lease_out) {
    lease->rw = *rw_io;
    lease->turn = *turn_io;
...
static bool lock_registry_abort_lock(hyp_lock_registry_t *registry) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
    bool fail_lock = registry->test_abort_failure_armed &&
                     registry->test_abort_failure == HYP_LOCK_REGISTRY_ABORT_FAIL_LOCK;
    if (fail_lock) {
...
static hyp_private_file_lock_status_t lock_registry_abort_attempt(
    hyp_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    hyp_private_file_lock_t **rw_io, hyp_private_file_lock_t **turn_io,
    hyp_private_file_lock_status_t requested_status, hyp_lock_lease_t *lease,
    hyp_lock_lease_t **lease_out) {
    ...
    bool fail_remove = registry->test_abort_failure_armed &&
                       registry->test_abort_failure == HYP_LOCK_REGISTRY_ABORT_FAIL_REMOVE;
    if (fail_remove) {
        registry->test_abort_failure_armed = false;
    }
    ...
    if (!has_native) {
        free(lease);
        return owns_attempt ? requested_status : HYP_PRIVATE_FILE_LOCK_IO;
    }
    hyp_lock_lease_t *cleanup = lease;
    if (hyp_lock_lease_release(&cleanup) != HYP_PRIVATE_FILE_LOCK_OK) {
        *lease_out = cleanup;
        return HYP_PRIVATE_FILE_LOCK_IO;
    }
    return owns_attempt ? requested_status : HYP_PRIVATE_FILE_LOCK_IO;
...
static hyp_private_file_lock_status_t lock_registry_attempt_native(
    hyp_lock_registry_t *registry, lock_registry_entry_t *entry, lock_registry_waiter_t *waiter,
    uint64_t deadline_ms, const hyp_lock_cancel_token_t *cancel_token, bool try_once,
    hyp_lock_lease_t *lease, hyp_lock_lease_t **lease_out) {
    hyp_private_file_lock_t *turn = NULL;
    hyp_private_file_lock_t *rw = NULL;
    ...
    while (!rw) {
        ...
        if (!turn) {
            hyp_private_file_lock_mode_t turn_mode = waiter->mode == HYP_PRIVATE_FILE_LOCK_SH
                                                         ? HYP_PRIVATE_FILE_LOCK_EX
                                                         : HYP_PRIVATE_FILE_LOCK_SH;
            status = hyp_private_file_lock_try_acquire(registry->directory, entry->turn_name,
                                                       turn_mode, &turn);
            if (status == HYP_PRIVATE_FILE_LOCK_BUSY) {
                lock_registry_notify(registry, waiter->mode, HYP_LOCK_REGISTRY_STAGE_TURN_BUSY);
                if (try_once) {
                    break;
                }
            ...
            lock_registry_notify(registry, waiter->mode, HYP_LOCK_REGISTRY_STAGE_TURN_HELD);
        }
        ...
        status = hyp_private_file_lock_try_acquire(registry->directory, entry->rw_name,
                                                   waiter->mode, &rw);
        if (status == HYP_PRIVATE_FILE_LOCK_BUSY) {
            lock_registry_notify(registry, waiter->mode, HYP_LOCK_REGISTRY_STAGE_RW_BUSY);
            if (try_once) {
                break;
            }
            if (!lock_registry_park_native(registry, deadline_ms, cancel_token)) {
        ...
        if (waiter->mode == HYP_PRIVATE_FILE_LOCK_SH) {
            status = hyp_private_file_lock_release(&turn);
            if (status != HYP_PRIVATE_FILE_LOCK_OK) {
                break;
            }
        }
        lock_registry_notify(registry, waiter->mode, HYP_LOCK_REGISTRY_STAGE_NATIVE_READY);
    }

    if (!rw || status != HYP_PRIVATE_FILE_LOCK_OK) {
    ...
    for (;;) {
        if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               HYP_PRIVATE_FILE_LOCK_BUSY, lease, lease_out);
        }
        if (!lock_registry_lock(registry)) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               HYP_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
        bool owns_attempt = entry->attempt_waiter == waiter && entry->waiter_head == waiter;
        bool local_state_ready = waiter->mode == HYP_PRIVATE_FILE_LOCK_SH
                                     ? !entry->writer_active
        ...
        hyp_private_fork_wait_status_t wait_status = HYP_PRIVATE_FORK_WAIT_SIGNALED;
        if (removed) {
            entry->attempt_waiter = NULL;
            if (waiter->mode == HYP_PRIVATE_FILE_LOCK_SH) {
        ...
        if (wait_status == HYP_PRIVATE_FORK_WAIT_ERROR) {
            return lock_registry_abort_attempt(registry, entry, waiter, &rw, &turn,
                                               HYP_PRIVATE_FILE_LOCK_IO, lease, lease_out);
        }
    }
...
static hyp_private_file_lock_status_t lock_registry_acquire_internal(
    hyp_lock_registry_t *registry, const char *resource_key, hyp_private_file_lock_mode_t mode,
    uint64_t deadline_ms, const hyp_lock_cancel_token_t *cancel_token, bool try_once,
    hyp_lock_lease_t **lease_out) {
    if (lease_out) {
        *lease_out = NULL;
    ...
    if (mode != HYP_PRIVATE_FILE_LOCK_SH && mode != HYP_PRIVATE_FILE_LOCK_EX) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    char turn_name[HYP_LOCK_REGISTRY_NAME_CAP];
    char rw_name[HYP_LOCK_REGISTRY_NAME_CAP];
    if (!hyp_lock_registry_resource_names(resource_key, turn_name, rw_name)) {
        return HYP_PRIVATE_FILE_LOCK_UNSAFE;
    }
    if (!try_once && lock_registry_should_stop(deadline_ms, cancel_token)) {
        return HYP_PRIVATE_FILE_LOCK_BUSY;
    }

    lock_registry_entry_t *candidate = calloc(1, sizeof(*candidate));
    hyp_lock_lease_t *lease = calloc(1, sizeof(*lease));
    if (!candidate || !lease) {
        free(candidate);
        free(lease);
        return HYP_PRIVATE_FILE_LOCK_IO;
...
hyp_private_file_lock_status_t hyp_lock_registry_acquire(
    hyp_lock_registry_t *registry, const char *resource_key, hyp_private_file_lock_mode_t mode,
    uint64_t deadline_ms, const hyp_lock_cancel_token_t *cancel_token,
    hyp_lock_lease_t **lease_out) {
    return lock_registry_acquire_internal(registry, resource_key, mode, deadline_ms, cancel_token,
                                          false, lease_out);
}

hyp_private_file_lock_status_t hyp_lock_registry_try_acquire(hyp_lock_registry_t *registry,
...
static hyp_private_file_lock_status_t lock_registry_cleanup_lease_release(
    hyp_lock_lease_t **lease_io) {
    hyp_lock_lease_t *lease = *lease_io;
    hyp_lock_registry_t *registry = lease->registry;
...
hyp_private_file_lock_status_t hyp_lock_lease_release(hyp_lock_lease_t **lease_io) {
    ...
    } else if (lease->mode == HYP_PRIVATE_FILE_LOCK_SH) {
        native_state_valid = lease->rw && !lease->turn;
    } else if (lease->mode == HYP_PRIVATE_FILE_LOCK_EX) {
    ...
    if (valid && lease->mode == HYP_PRIVATE_FILE_LOCK_SH) {
        valid = entry->active_readers > 0;
    } else if (valid && lease->mode == HYP_PRIVATE_FILE_LOCK_EX) {
    ...
    if (!lease->critical_released) {
        ...
        if (lease->rw && !hyp_private_file_lock_unlock_complete(lease->rw)) {
            return status;
        }
    ...
    if (lease->mode == HYP_PRIVATE_FILE_LOCK_SH) {
        entry->active_readers--;
    } else {
    ...
    hyp_private_file_lock_status_t result =
        lease->release_error ? HYP_PRIVATE_FILE_LOCK_IO : HYP_PRIVATE_FILE_LOCK_OK;
    *lease_io = NULL;
    free(lease);
    return result;
}

hyp_private_file_lock_status_t hyp_lock_registry_free(hyp_lock_registry_t **registry_io) {
    if (!registry_io || !*registry_io || !hyp_private_file_lock_fork_guard_enter()) {
        return HYP_PRIVATE_FILE_LOCK_IO;
    }
    hyp_lock_registry_t *registry = *registry_io;
    hyp_lock_registry_t **live_cursor = &lock_registry_live;
...
bool hyp_lock_registry_is_retired_for_test(const hyp_lock_registry_t *registry) {
    ...
    bool retired = false;
    for (const hyp_lock_registry_t *cursor = lock_registry_retired; cursor;
...
bool hyp_lock_lease_fail_next_release_step_for_test(hyp_lock_lease_t *lease,
    ...
    hyp_private_file_lock_t *lock = handle == HYP_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == HYP_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return hyp_private_file_lock_fail_next_release_step_for_test(lock, step);
}
...
bool hyp_lock_lease_has_release_handle_for_test(const hyp_lock_lease_t *lease,
    ...
    if (handle == HYP_LOCK_REGISTRY_RELEASE_RW) {
        return lease->rw != NULL;
    }
    if (handle == HYP_LOCK_REGISTRY_RELEASE_TURN) {
...
bool hyp_lock_lease_fail_close_after_consuming_for_test(hyp_lock_lease_t *lease,
    ...
    hyp_private_file_lock_t *lock = handle == HYP_LOCK_REGISTRY_RELEASE_RW     ? lease->rw
                                    : handle == HYP_LOCK_REGISTRY_RELEASE_TURN ? lease->turn
                                                                               : NULL;
    return hyp_private_file_lock_fail_close_after_consuming_for_test(lock);
}
#endif
...
bool hyp_lock_registry_fail_next_native_release_step_for_test(
    ...
    hyp_private_file_lock_release_step_t step) {
    if ((handle != HYP_LOCK_REGISTRY_RELEASE_RW && handle != HYP_LOCK_REGISTRY_RELEASE_TURN) ||
        (step != HYP_PRIVATE_FILE_LOCK_RELEASE_UNLOCK &&
         step != HYP_PRIVATE_FILE_LOCK_RELEASE_CLOSE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
...
bool hyp_lock_registry_fail_next_abort_bookkeeping_for_test(
    ...
    if ((failure != HYP_LOCK_REGISTRY_ABORT_FAIL_LOCK &&
         failure != HYP_LOCK_REGISTRY_ABORT_FAIL_REMOVE) ||
        !lock_registry_lock(registry)) {
        return false;
    }
    bool idle = registry->waiter_count == 0 && registry->active_lease_count == 0 &&
...
bool hyp_lock_registry_set_stage_hook_for_test(hyp_lock_registry_t *registry,
                                               hyp_lock_registry_stage_hook_fn hook,
                                               void *context) {
    if (!lock_registry_lock(registry)) {
        return false;
    }
