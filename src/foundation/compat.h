...
#include <stddef.h>
#include <stdio.h>
/* stdlib.h declares getenv (hyp_tmpdir) and, on Windows, _putenv_s (hyp_setenv/
 * hyp_unsetenv). The x86-64 mingw toolchain pulled it in transitively, but the
...
#endif
#include <windows.h>
#define hyp_usleep(us) Sleep((DWORD)((us) / 1000))
#else
#include <unistd.h>
#define hyp_usleep(us) usleep((useconds_t)(us))
#endif

/* ── strdup / strndup ─────────────────────────────────────────── */
#ifdef _WIN32
#define hyp_strdup _strdup
/* Implemented in compat.c */
char *hyp_strndup(const char *s, size_t n);
#else
#define hyp_strdup strdup
#define hyp_strndup strndup
#endif
...
#else
#define hyp_getline getline
#endif

/* ── fileno ───────────────────────────────────────────────────── */
#ifdef _WIN32
#define hyp_fileno _fileno
#else
#define hyp_fileno fileno
#endif
...
#else
#define hyp_strcasestr strcasestr
#endif

/* ── mkdir portability ───────────────────────────────────────── */
#ifdef _WIN32
#include <direct.h>
#define hyp_mkdir(path) _mkdir(path)
#else
#include <sys/stat.h>
#define hyp_mkdir(path) mkdir(path, 0755)
#endif
...
static inline int hyp_nanosleep(const struct timespec *req, struct timespec *rem) {
    (void)rem;
    Sleep((DWORD)(req->tv_sec * 1000 + req->tv_nsec / 1000000));
    return 0;
}
#else
#define hyp_clock_gettime clock_gettime
#define hyp_nanosleep nanosleep
#endif

/* ── gmtime_r (Windows lacks it) ─────────────────────────────── */
#ifdef _WIN32
static inline struct tm *hyp_gmtime_r(const time_t *timep, struct tm *result) {
    return gmtime_s(result, timep) == 0 ? result : NULL;
}
#else
#define hyp_gmtime_r gmtime_r
#endif

/* ── mkdtemp (Windows lacks it) ──────────────────────────────── */
#ifdef _WIN32
/* Translates /tmp/ to %TEMP%\ and copies result back to tmpl.
 * Callers MUST use char buf[HYP_SZ_256] or larger. */
char *hyp_mkdtemp(char *tmpl);
#else
#define hyp_mkdtemp mkdtemp
#endif
...
#ifdef _WIN32
static inline int hyp_setenv(const char *name, const char *value, int overwrite) {
    (void)overwrite;
    if (!name || !value) {
        return EINVAL;
    }
...
static inline int hyp_unsetenv(const char *name) {
    if (!name) {
        return EINVAL;
    }
    int name_chars = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, NULL, 0);
...
#else
#define hyp_setenv setenv
#define hyp_unsetenv unsetenv
#endif
...
#include <io.h>
#include <fcntl.h>
#define hyp_pipe(fds) _pipe(fds, 4096, _O_BINARY)
#else
#define hyp_pipe(fds) pipe(fds)
#endif

/* ── Temp directory helper ───────────────────────────────────── */
static inline const char *hyp_tmpdir(void) {
#ifdef _WIN32
...
#ifdef _WIN32
#define HYP_HAS_SIGACTION 0
#else
#define HYP_HAS_SIGACTION 1
#endif
