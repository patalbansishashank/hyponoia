 ...
 */
#ifndef HYP_SUBPROCESS_H
#define HYP_SUBPROCESS_H

#include <stdbool.h>
#include <stddef.h> /* size_t (hyp_build_win_cmdline) */

/* How a supervised child ended. */
typedef enum {
    HYP_PROC_CLEAN = 0,    /* exited with code 0 */
    HYP_PROC_EXIT_NONZERO, /* exited with a nonzero code (a graceful failure) */
    HYP_PROC_CRASH,        /* died from a fault: POSIX SIGSEGV/BUS/ILL/FPE/ABRT/SYS,
                            * or a Windows NTSTATUS exception exit code (>= 0xC0000000) */
    HYP_PROC_HANG,         /* made no progress within the quiet-timeout; we killed it */
    HYP_PROC_KILLED,       /* terminated by a non-fault signal we did not initiate */
    HYP_PROC_SPAWN_FAILED  /* fork/exec/CreateProcess failed — no child ever ran */
} hyp_proc_outcome_t;

typedef struct {
    hyp_proc_outcome_t outcome;
...
typedef void (*hyp_proc_log_cb)(const char *line, void *ud);
...
typedef struct {
    ...
    hyp_proc_log_cb on_log_line;     /* optional per-line callback */
    ...
    int cancel_grace_ms;             /* graceful tree-termination window; <= 0 uses the finite
                                      * HYP_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS */
    bool delete_log_on_exit;         /* unlink log_file after reaping */
} hyp_proc_opts_t;

#define HYP_SUBPROCESS_DEFAULT_CANCEL_GRACE_MS 1000
#define HYP_SUBPROCESS_MAX_CANCEL_GRACE_MS 1000
#define HYP_SUBPROCESS_FORCE_SETTLE_MS 1000
...
typedef struct hyp_subprocess hyp_subprocess_t;

typedef enum {
    HYP_PROC_POLL_ERROR = -1, /* invalid handle/arguments */
    HYP_PROC_POLL_RUNNING = 0,
    HYP_PROC_POLL_TERMINAL = 1
} hyp_proc_poll_t;
 ...
 * but the terminal outcome remains HYP_PROC_HANG. */
 ...
 * (out->outcome == HYP_PROC_SPAWN_FAILED). */
 ...
 * is unit-tested on Linux/macOS CI, and so both spawn sites (hyp_subprocess_run and
 ...
 * command-language payload. Unlike hyp_build_win_cmdline(), payload is not a C
