 ...
 *
 * Usage:
 *   HYP_DYN_ARRAY(int) nums = {0};    // zero-init
 *   hyp_da_push(&nums, 42);
 *   hyp_da_push(&nums, 99);
 ...
 *   - Uses realloc — NOT arena-compatible (use HYPDefArray etc. for arena)
 *   - Header-only: no .c file needed
 */
#ifndef HYP_DYN_ARRAY_H
#define HYP_DYN_ARRAY_H

#include <stdlib.h>
#include <string.h>

/* Declare a dynamic array type for a given element type. */
#define HYP_DYN_ARRAY(T) \
    struct {             \
        T *items;        \
        int count;       \
        int cap;         \
...
#define hyp_da_pop(da) ((da)->items[--(da)->count])

/* Get last element without removing. Undefined if empty. */
#define hyp_da_last(da) ((da)->items[(da)->count - 1])

/* Clear without freeing (reset count to 0). */
#define hyp_da_clear(da) ((da)->count = 0)

/* Free all memory. */
#define hyp_da_free(da)     \
    do {                    \
        free((da)->items);  \
        (da)->items = NULL; \
        (da)->count = 0;    \
...
#define hyp_da_insert(da, idx, item)                                           \
    do {                                                                       \
        hyp_da_push(da, item); /* ensure space */                              \
        if ((idx) < (da)->count - 1) {                                         \
