 ...
 * The HYP_MEM_BUDGET_MB env var, when set to a positive integer, overrides
 ...
 * HYP_MEM_BUDGET_MB wins, while a larger/default budget is capped. */
void hyp_mem_init_with_cap(double ram_fraction, size_t hard_cap_bytes);

/* Result of hyp_mem_resolve_budget: the resolved budget plus the metadata
 * hyp_mem_init logs — so the parse/clamp logic lives in exactly ONE place and
 * the caller never re-parses the env string. */
typedef struct {
    size_t budget;      /* resolved budget in bytes */
    const char *source; /* log token: "ram_fraction" | "HYP_MEM_BUDGET_MB" */
...
/* Pure budget resolver shared by hyp_mem_init (exposed for testing).
 * Returns ram_fraction * total_ram, unless `budget_mb` is a STRICTLY valid
 * positive integer string (the HYP_MEM_BUDGET_MB override) — then it returns
...
/* TEST HOOK: overwrite the budget directly, bypassing hyp_mem_init's
 ...
 * guard: a later hyp_mem_init still initializes normally. Callers must
 * save hyp_mem_budget() first and restore it before their assertions.
 ...
 */
enum { HYP_MEM_MAP_BUCKETS = 8 };

typedef struct {
    /* Is plain malloc actually served by the configured allocator on this
     * build? If false, every allocator tuning in hyp_mem_init (immediate
    ...
    size_t bucket_bytes[HYP_MEM_MAP_BUCKETS];
    size_t bucket_blocks[HYP_MEM_MAP_BUCKETS];
} hyp_mem_map_t;
 ...
 */
enum { HYP_MEM_OWNERSHIP_CLASSES = 6 };

typedef struct {
    size_t probe_bytes[HYP_MEM_OWNERSHIP_CLASSES]; /* size probed per class */
    bool owned[HYP_MEM_OWNERSHIP_CLASSES];         /* predicate said "ours" */
    bool allocated[HYP_MEM_OWNERSHIP_CLASSES];     /* the probe itself worked */
    int owned_count;
    int probed_count;
    bool all_owned; /* every successfully probed class came back owned */
} hyp_mem_ownership_audit_t;
 ...
 *   - Off unless HYP_MEM_PHASES=1, so the hot path pays one atomic load.
 */
 ...
 */
enum { HYP_MEM_TRACKED_REGIONS = 6 };
...
typedef struct {
    ...
    void *large_base[HYP_MEM_TRACKED_REGIONS];
    size_t large_size[HYP_MEM_TRACKED_REGIONS];
 ...
 * read straight out of the daemon log. No-op unless HYP_MEM_CENSUS=1. */
void hyp_mem_census_log(const char *tag);

/* Census logging gate: HYP_MEM_CENSUS=1. Off by default — HeapWalk locks each
