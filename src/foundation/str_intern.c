...
HYPInternPool *hyp_intern_create(void) {
    HYPInternPool *p = (HYPInternPool *)calloc(HYP_ALLOC_ONE, sizeof(*p));
    if (!p) {
        return NULL;
    }
    hyp_arena_init(&p->arena);
    p->capacity = HYP_SZ_256;
    p->mask = p->capacity - SKIP_ONE;
    p->buckets = (InternEntry *)calloc(p->capacity, sizeof(InternEntry));
    if (!p->buckets) {
        hyp_arena_destroy(&p->arena);
        free(p);
        return NULL;
    }
    return p;
...
void hyp_intern_free(HYPInternPool *pool) {
    ...
    hyp_arena_destroy(&pool->arena);
    free(pool->buckets);
    free(pool);
}

static void intern_resize(HYPInternPool *p) {
...
const char *hyp_intern_n(HYPInternPool *pool, const char *s, size_t len) {
    ...
    /* Resize at 70% load */
    if (pool->count * HYP_DECIMAL_BASE >= pool->capacity * INTERN_LOAD_NUM) {
        intern_resize(pool);
    ...
    char *copy = hyp_arena_strndup(&pool->arena, s, len);
    if (!copy) {
        return NULL;
    }
...
const char *hyp_intern(HYPInternPool *pool, const char *s) {
    ...
    return hyp_intern_n(pool, s, strlen(s));
}
