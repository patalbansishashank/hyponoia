...
static uint64_t hyp_platform_scale_fraction(uint64_t remainder, uint64_t multiplier,
                                            uint64_t divisor) {
    uint64_t quotient = 0;
    uint64_t reduced = 0;
    uint64_t mask = 1;
...
uint64_t hyp_platform_scale_counter_ns(uint64_t counter, uint64_t frequency) {
    if (frequency == 0) {
        return UINT64_MAX;
    }
    uint64_t whole_seconds = counter / frequency;
    uint64_t remainder = counter % frequency;
    if (whole_seconds > UINT64_MAX / HYP_NSEC_PER_SEC) {
        return UINT64_MAX;
    }
    uint64_t whole_ns = whole_seconds * HYP_NSEC_PER_SEC;
    uint64_t fraction_ns = hyp_platform_scale_fraction(remainder, HYP_NSEC_PER_SEC, frequency);
...
static void hyp_canonicalize_drive(char *path) {
...
void *hyp_mmap_read(const char *path, size_t *out_size) {
    if (!path || !out_size) {
        return NULL;
    }
    *out_size = 0;
...
void hyp_munmap(void *addr, size_t size) {
    (void)size;
    if (addr) {
...
uint64_t hyp_now_ns(void) {
    LARGE_INTEGER freq, count;
    QueryPerformanceFrequency(&freq);
    QueryPerformanceCounter(&count);
    return hyp_platform_scale_counter_ns((uint64_t)count.QuadPart, (uint64_t)freq.QuadPart);
}

#define HYP_USEC_PER_SEC 1000000ULL

uint64_t hyp_now_ms(void) {
    return hyp_now_ns() / HYP_USEC_PER_SEC;
}
...
bool hyp_file_exists(const char *path) {
    wchar_t *wpath = hyp_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
bool hyp_is_dir(const char *path) {
    wchar_t *wpath = hyp_path_to_wide(path);
    if (!wpath) {
        return false;
    }
...
int64_t hyp_file_size(const char *path) {
    wchar_t *wpath = hyp_path_to_wide(path);
    if (!wpath) {
...
char *hyp_normalize_path_sep(char *path) {
    if (path) {
        for (char *p = path; *p; p++) {
            if (*p == '\\') {
                *p = '/';
...
static void hyp_timebase_initialize(void) {
...
uint64_t hyp_now_ns(void) {
    (void)pthread_once(&timebase_once, hyp_timebase_initialize);
...
#ifdef _WIN32
#include <stdlib.h>
#define HYP_ENVIRON _environ
#elif defined(__APPLE__)
#include <crt_externs.h>
#define HYP_ENVIRON (*_NSGetEnviron())
#else
extern char **environ;
#define HYP_ENVIRON environ
#endif
...
const char *hyp_safe_getenv(const char *name, char *buf, size_t buf_sz, const char *fallback) {
     ...
     * HYP_CACHE_DIR) arrives here either mojibake'd or with unrepresentable
     ...
     * UTF-8-path convention the rest of the codebase (hyp_fopen, _wmkdir,
     * SQLite VFS) already assumes. */
    {
        wchar_t wname[HYP_SZ_256];
        int wn = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, name, -1, wname, HYP_SZ_256);
        if (wn > 0) {
            ...
            char *utf8 = hyp_wide_to_utf8(wval);
            free(wval);
            if (!utf8) {
                return NULL;
            }
...
const char *hyp_get_home_dir(void) {
    static HYP_TLS char buf[HYP_SZ_1K];
    char tmp[HYP_SZ_256] = "";

    hyp_safe_getenv("HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
    ...
    hyp_safe_getenv("USERPROFILE", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        hyp_normalize_path_sep(buf);
        return buf;
...
const char *hyp_app_config_dir(void) {
    static HYP_TLS char buf[HYP_SZ_1K];
    char tmp[HYP_SZ_256] = "";
#ifdef _WIN32
    hyp_safe_getenv("APPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        hyp_normalize_path_sep(buf);
        return buf;
    ...
    hyp_safe_getenv("XDG_CONFIG_HOME", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        return buf;
    }
...
const char *hyp_app_local_dir(void) {
#ifdef _WIN32
    static HYP_TLS char buf[HYP_SZ_1K];
    char tmp[HYP_SZ_256] = "";
    hyp_safe_getenv("LOCALAPPDATA", tmp, sizeof(tmp), NULL);
    if (tmp[0]) {
        snprintf(buf, sizeof(buf), "%s", tmp);
        hyp_normalize_path_sep(buf);
        return buf;
    ...
    return NULL;
#else
    return hyp_app_config_dir();
#endif
}

/* ── Cache directory ────────────────────────── */

const char *hyp_resolve_cache_dir(void) {
    static HYP_TLS char buf[HYP_SZ_4K];
    static const char missing[] = "\x1f"
                                  "HYP_CACHE_DIR_MISSING"
                                  "\x1f";
    const char *configured = hyp_safe_getenv("HYP_CACHE_DIR", buf, sizeof(buf), missing);
    if (!configured) {
    ...
    int written = snprintf(buf, sizeof(buf), "%s/.cache/hyponoia", home);
