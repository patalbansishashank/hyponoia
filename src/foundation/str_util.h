 ...
 */
#ifndef HYP_STR_UTIL_H
#define HYP_STR_UTIL_H
 ...
 * Everything hyp_validate_shell_arg rejects, plus the cmd.exe expansion
 ...
 *
 * Use this — not hyp_validate_shell_arg — for every path that crosses into a
 ...
 *
 * Usage: HYP_SNPRINTF_APPEND(buf, sizeof(buf), off, "fmt %s", arg);
 * Requires: <stdio.h> included by caller. */
#define HYP_SNPRINTF_APPEND(buf, sz, off, ...)                                       \
    do {                                                                             \
        if ((off) >= 0 && (off) < (int)(sz)) {                                       \
            int _hyp_r = snprintf((buf) + (off), (sz) - (size_t)(off), __VA_ARGS__); \
            if (_hyp_r > 0)                                                          \
                (off) += _hyp_r;                                                     \
            if ((off) >= (int)(sz))                                                  \
                (off) = (int)(sz) - 1;                                               \
