...
#include "foundation/constants.h" /* HYP_SZ_4K — containment scratch buffers */
#include "foundation/platform.h"
#include "foundation/sha256.h"
#ifdef _WIN32
#include "foundation/win_utf8.h" /* hyp_utf8_to_wide + <windows.h> */
#include <limits.h>              /* INT_MAX — CompareStringOrdinal length cast */
#include <wchar.h>
#endif
...
int hyp_workspace_path_depth(const char *canonical_path) {
...
hyp_ws_verdict_t hyp_workspace_classify_root(const char *canonical_path, const char *home_dir,
    ...
    if (!canonical_path || !canonical_path[0] || ws_volume_prefix_len(canonical_path) == 0) {
        ...
        return HYP_WS_DENY_ABSOLUTE;
    }

    bool windows_style = ws_is_windows_style(canonical_path);
    int depth = hyp_workspace_path_depth(canonical_path);
    ...
    if (depth < min_depth) {
        return HYP_WS_DENY_TOO_SHALLOW;
    }
...
const char *hyp_workspace_verdict_reason(hyp_ws_verdict_t verdict) {
    switch (verdict) {
    case HYP_WS_ALLOW:
        return "allowed";
    case HYP_WS_DENY_TOO_SHALLOW:
        return "path is too broad to index as one root; name a project directory below it";
    case HYP_WS_DENY_ABSOLUTE:
        return "path is a volume root or holds the hyponoia cache; it cannot be indexed";
    case HYP_WS_DENY_SENSITIVE:
...
bool hyp_workspace_verdict_is_overridable(hyp_ws_verdict_t verdict) {
    return verdict == HYP_WS_DENY_SENSITIVE;
}
...
bool hyp_workspace_grant_path(const char *cache_dir, char *out, size_t out_sz) {
...
static int ws_grant_walk(const char *cache_dir,
    ...
    if (!hyp_workspace_grant_path(cache_dir, store, sizeof(store))) {
        return 0;
    }
    FILE *f = hyp_fopen(store, "r");
    if (!f) {
        return 0;
    }
...
static bool ws_match_visit(const char *root, bool sensitive, void *ctx) {
    ws_match_t *m = ctx;
    if (hyp_path_within_root(root, m->candidate)) {
        m->contained = true;
...
bool hyp_workspace_grant_list(const char *cache_dir, char *out, size_t out_sz) {
    if (!out || out_sz == 0) {
        return false;
    }
    out[0] = '\0';
...
bool hyp_workspace_grant_add(const char *cache_dir, const char *home_dir,
    ...
    hyp_ws_verdict_t verdict = hyp_workspace_classify_root(canonical_path, home_dir, cache_dir);
    if (verdict != HYP_WS_ALLOW) {
        if (!hyp_workspace_verdict_is_overridable(verdict)) {
            if (err) {
                snprintf(err, err_sz, "%s", hyp_workspace_verdict_reason(verdict));
            }
            return false;
        }
        if (!approve_sensitive) {
            ...
            if (err) {
                snprintf(err, err_sz, "%s; re-run with --approve-sensitive if that is intended",
                         hyp_workspace_verdict_reason(verdict));
            }
            return false;
        }
    }
    ...
    bool sensitive = verdict == HYP_WS_DENY_SENSITIVE;
...
bool hyp_workspace_root_allowed(const char *canonical_path, const char *home_dir,
     ...
     * HYP_ALLOWED_ROOT deployments keep working unchanged. */
    bool configured = configured_root && configured_root[0];
    bool configured_contains = configured && hyp_path_within_root(configured_root, canonical_path);
     ...
     * walking a project's approved extra roots. hyp_workspace_manifest_allows is
     ...
     * lifted at all. */
    if (verdict == HYP_WS_DENY_SENSITIVE && match.exact_sensitive) {
        return true;
    }
    if (err) {
        if (hyp_workspace_verdict_is_overridable(verdict)) {
            snprintf(err, err_sz,
                     "%s: %s. To index it anyway, run: hyponoia allow-root "
                     "--approve-sensitive %s",
                     canonical_path, hyp_workspace_verdict_reason(verdict), canonical_path);
        } else {
            snprintf(err, err_sz, "%s: %s", canonical_path, hyp_workspace_verdict_reason(verdict));
        }
    }
    return false;
}
...
const char *hyp_workspace_home_dir(void) {
...
const char *hyp_workspace_cache_dir(void) {
    return hyp_resolve_cache_dir();
}

/* ── Per-project request manifest ─────────────────────────────────────────── */

bool hyp_workspace_manifest_read(const char *project_root, hyp_ws_manifest_t *out) {
    if (!out) {
        return false;
    }
    ...
    int n = snprintf(path, sizeof(path), "%s/%s", project_root, HYP_WS_MANIFEST_NAME);
    ...
    (void)fclose(f);
    out->present = true;
    hyp_sha256_hex(raw ? raw : "", raw_len, out->digest);
    ...
    for (size_t i = 0; i <= raw_len && ok; i++) {
        ...
        if (len > 0 && raw[line_start] != '#') {
            if (len >= sizeof(out->entries[0]) || out->count >= HYP_WS_MANIFEST_MAX_ENTRIES) {
                ok = false;
                break;
            }
...
bool hyp_workspace_manifest_is_approved(const char *cache_dir, const char *project_root,
                                        const hyp_ws_manifest_t *manifest) {
...
bool hyp_workspace_manifest_approve(const char *cache_dir, const char *home_dir,
                                    const char *project_root, char *err, size_t err_sz) {
    if (err && err_sz) {
        err[0] = '\0';
    }
    hyp_ws_manifest_t m;
    if (!hyp_workspace_manifest_read(project_root, &m)) {
        if (err) {
            snprintf(err, err_sz,
                     "%s is malformed (control characters, an over-long entry, or "
                     "more than %d entries)",
                     HYP_WS_MANIFEST_NAME, HYP_WS_MANIFEST_MAX_ENTRIES);
        }
        return false;
    }
    if (!m.present) {
        if (err) {
            snprintf(err, err_sz, "no %s in %s", HYP_WS_MANIFEST_NAME,
    ...
    for (int i = 0; i < m.count; i++) {
        hyp_ws_verdict_t v = hyp_workspace_classify_root(m.entries[i], home_dir, cache_dir);
        if (v != HYP_WS_ALLOW) {
            if (err) {
                snprintf(err, err_sz, "requested path %s: %s", m.entries[i],
                         hyp_workspace_verdict_reason(v));
            }
            return false;
        }
    }
    ...
    if (hyp_workspace_manifest_is_approved(cache_dir, project_root, &m)) {
        return true;
    }
    FILE *f = hyp_fopen(store, "a");
    if (!f) {
...
bool hyp_workspace_manifest_allows(const char *cache_dir, const char *home_dir,
                                   const char *project_root, const char *candidate) {
    if (!candidate || !candidate[0]) {
        return false;
    }
    hyp_ws_manifest_t m;
    if (!hyp_workspace_manifest_read(project_root, &m) || !m.present) {
        return false;
    }
    if (!hyp_workspace_manifest_is_approved(cache_dir, project_root, &m)) {
        return false;
    ...
    for (int i = 0; i < m.count; i++) {
        ...
        if (hyp_workspace_classify_root(m.entries[i], home_dir, cache_dir) != HYP_WS_ALLOW) {
            continue;
        }
        if (hyp_path_within_root(m.entries[i], candidate)) {
            return true;
        }
    }
    return false;
...
/* ── Path containment ────────────────────────────────────────────
 *
 * Moved here from src/mcp/mcp.c. workspace.h has always been where
 * hyp_path_within_root is DECLARED, and workspace.c calls it three times
 * (ws_match_visit, hyp_workspace_root_allowed, hyp_workspace_manifest_allows),
 * so keeping the body in the MCP layer made src/foundation depend on src/mcp —
 * a backwards edge that no foundation-only link could satisfy. The code below
