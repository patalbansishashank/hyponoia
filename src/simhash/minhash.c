...
static void weighted_minhash_update(hyp_minhash_t *out, const char *trigram, int len, int w) {
    for (int k = 0; k < HYP_MINHASH_K; k++) {
...
static int hash_trigrams(const char **tokens, int token_count, hyp_minhash_t *out) {
    for (int k = 0; k < HYP_MINHASH_K; k++) {
...
bool hyp_minhash_compute(TSNode func_body, const char *source, int language, hyp_minhash_t *out) {
    (void)source;
    (void)language;
    ...
    if (token_count < HYP_MINHASH_MIN_NODES) {
        return false;
    }
...
double hyp_minhash_jaccard(const hyp_minhash_t *a, const hyp_minhash_t *b) {
    if (!a || !b) {
        return 0.0;
    }
    int matching = 0;
    ...
    return (double)matching / (double)HYP_MINHASH_K;
}

/* ── Hex encoding/decoding ───────────────────────────────────────── */

void hyp_minhash_to_hex(const hyp_minhash_t *fp, char *buf, int bufsize) {
    if (!fp || !buf || bufsize < HYP_MINHASH_HEX_BUF) {
        if (buf && bufsize > 0) {
            buf[0] = '\0';
        }
        return;
...
bool hyp_minhash_from_hex(const char *hex, hyp_minhash_t *out) {
    if (!hex || !out) {
        return false;
    }
    size_t len = strlen(hex);
    if ((int)len != HYP_MINHASH_HEX_LEN) {
        return false;
    }
    for (int i = 0; i < HYP_MINHASH_K; i++) {
...
static uint32_t band_hash(const hyp_minhash_t *fp, int band) {
    int base = band * HYP_LSH_ROWS;
    /* Combine r=2 values into a single hash */
    uint32_t combined[HYP_LSH_ROWS];
    for (int r = 0; r < HYP_LSH_ROWS; r++) {
...
hyp_lsh_index_t *hyp_lsh_new(void) {
    hyp_lsh_index_t *idx = calloc(SKIP_ONE, sizeof(hyp_lsh_index_t));
    return idx;
}

void hyp_lsh_insert(hyp_lsh_index_t *idx, const hyp_lsh_entry_t *entry) {
    ...
    if (idx->entry_count >= idx->entry_cap) {
        ...
        hyp_lsh_entry_t *new_entries =
            realloc(idx->entries, (size_t)new_cap * sizeof(hyp_lsh_entry_t));
        if (!new_entries) {
            return;
        }
...
static bool result_push(hyp_lsh_index_t *idx, const hyp_lsh_entry_t *candidate) {
    if (idx->result_count >= idx->result_cap) {
        int new_cap =
            idx->result_cap < RESULT_INIT_CAP ? RESULT_INIT_CAP : idx->result_cap * GROW_FACTOR;
        const hyp_lsh_entry_t **new_buf =
            realloc(idx->result_buf, (size_t)new_cap * sizeof(const hyp_lsh_entry_t *));
        if (!new_buf) {
            return false;
        }
...
void hyp_lsh_query(const hyp_lsh_index_t *idx, const hyp_minhash_t *fp,
                   const hyp_lsh_entry_t ***out, int *count) {
    *out = NULL;
    *count = 0;
    ...
    hyp_lsh_index_t *mut_idx = (hyp_lsh_index_t *)idx;
    ...
    for (int b = 0; b < HYP_LSH_BANDS; b++) {
        ...
        for (int i = 0; i < bucket->count; i++) {
            const hyp_lsh_entry_t *candidate = &idx->entries[bucket->items[i]];
...
int hyp_lsh_query_into(const hyp_lsh_index_t *idx, const hyp_minhash_t *fp,
                       const hyp_lsh_entry_t **out_buf, int out_cap) {
...
void hyp_lsh_free(hyp_lsh_index_t *idx) {
    if (!idx) {
        return;
    }
    /* Free bucket arrays */
