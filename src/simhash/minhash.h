 ...
 *
 * Workflow:
 *   1. hyp_minhash_compute()  — during AST extraction (per function)
 *   2. hyp_minhash_jaccard()  — pairwise similarity (K=64 agreement)
 *   3. hyp_lsh_*              — locality-sensitive hashing for O(n)
 *                               candidate generation
 */
#ifndef HYP_MINHASH_H
#define HYP_MINHASH_H
 ...
 * Returns false if the body has fewer than HYP_MINHASH_MIN_NODES
...
typedef struct hyp_lsh_index hyp_lsh_index_t;

/* Entry stored in the LSH index. */
typedef struct {
    int64_t node_id;
    const hyp_minhash_t *fingerprint;
