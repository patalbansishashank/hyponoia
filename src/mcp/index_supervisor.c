...
#include "foundation/compat_fs.h" /* hyp_mkdir_p, hyp_fopen */
#include "foundation/constants.h"
#include "foundation/log.h"
#include "foundation/platform.h" /* hyp_safe_getenv, path normalization */
#include "foundation/profile.h"  /* hyp_profile_active (keep worker log under HYP_PROFILE) */
#include "ui/http_server.h"      /* hyp_http_server_resolve_binary_path */
...
_Static_assert(HYP_INDEX_WORKER_BUILD_FINGERPRINT_SIZE == HYP_DAEMON_BUILD_FINGERPRINT_SIZE,
               "worker and daemon build fingerprint sizes must match");

/* ── Worker-role state ────────────────────────────────────────────── */

static bool g_worker_active = false;
static char g_worker_response_out[HYP_SZ_4K] = {0};
static size_t g_worker_memory_budget_bytes = 0;
static bool g_build_fingerprint_capture_attempted = false;
static char g_build_fingerprint[HYP_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};

void hyp_index_set_worker_role(bool is_worker, const char *response_out) {
...
static void worker_set_local_env(const char *name, const char *value) {
    if (value && value[0]) {
        (void)hyp_setenv(name, value, 1);
    } else {
        (void)hyp_unsetenv(name);
    }
}

void hyp_index_set_worker_role_options(bool is_worker, const char *response_out, bool single_thread,
                                       const char *marker_file, const char *quarantine_file,
                                       size_t memory_budget_bytes) {
    hyp_index_set_worker_role(is_worker, response_out);
    g_worker_memory_budget_bytes = is_worker ? memory_budget_bytes : 0;
    if (!is_worker) {
        return;
    }
    worker_set_local_env("HYP_INDEX_SINGLE_THREAD", single_thread ? "1" : NULL);
    worker_set_local_env("HYP_INDEX_MARKER_FILE", marker_file);
    worker_set_local_env("HYP_INDEX_QUARANTINE_FILE", quarantine_file);
}

bool hyp_index_worker_active(void) {
    return g_worker_active;
}

const char *hyp_index_worker_response_out(void) {
    return g_worker_response_out[0] ? g_worker_response_out : NULL;
}

size_t hyp_index_worker_memory_budget_bytes(void) {
    return g_worker_memory_budget_bytes;
}

static bool worker_fingerprint_valid(const char *fingerprint);

bool hyp_index_supervisor_capture_build_fingerprint(void) {
    if (g_build_fingerprint_capture_attempted) {
        return g_build_fingerprint[0] != '\0';
    }
    g_build_fingerprint_capture_attempted = true;
#if defined(HYP_CLI_ENABLE_TEST_API)
    ...
    char test_fingerprint[HYP_INDEX_WORKER_BUILD_FINGERPRINT_SIZE];
    const char *test_value = hyp_safe_getenv("HYP_TEST_BUILD_FINGERPRINT", test_fingerprint,
...
#endif
    char captured[HYP_INDEX_WORKER_BUILD_FINGERPRINT_SIZE] = {0};
    if (!hyp_daemon_runtime_process_build_fingerprint((uint64_t)worker_getpid(), captured)) {
        return false;
    }
...
const char *hyp_index_supervisor_build_fingerprint(void) {
    return g_build_fingerprint[0] ? g_build_fingerprint : NULL;
}

static bool worker_fingerprint_valid(const char *fingerprint) {
    if (!fingerprint || strlen(fingerprint) != HYP_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH) {
        return false;
    }
    for (size_t i = 0; i < HYP_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH; i++) {
...
hyp_index_worker_argv_status_t hyp_index_worker_parse_process_argv(
    int argc, char *const argv[], hyp_index_worker_invocation_t *invocation_out) {
    if (invocation_out) {
    ...
    if (!invocation_out || argc < 9 || !argv || !argv[0] || !argv[0][0] || !argv[1] ||
        ...
        return HYP_INDEX_WORKER_ARGV_INVALID;
    }

    hyp_index_worker_invocation_t parsed = {
        .expected_build_fingerprint = argv[4],
        .args_json = argv[6],
        .response_out = argv[8],
    };
    int next = 9;
    if (next < argc && argv[next] && strcmp(argv[next], HYP_INDEX_WORKER_MEMORY_BUDGET_ARG) == 0) {
        if (next + 1 >= argc ||
    ...
    if (next < argc && argv[next] && strcmp(argv[next], HYP_INDEX_WORKER_SINGLE_THREAD_ARG) == 0) {
        parsed.single_thread = true;
        next++;
    }
    if (next < argc && argv[next] && strcmp(argv[next], HYP_INDEX_WORKER_MARKER_ARG) == 0) {
    ...
    if (next < argc && argv[next] && strcmp(argv[next], HYP_INDEX_WORKER_QUARANTINE_ARG) == 0) {
        if (next + 1 >= argc || !argv[next + 1] || !argv[next + 1][0]) {
            return HYP_INDEX_WORKER_ARGV_INVALID;
        }
    ...
    if (!g_build_fingerprint[0]) {
        return HYP_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE;
    }
    if (strcmp(parsed.expected_build_fingerprint, g_build_fingerprint) != 0) {
        return HYP_INDEX_WORKER_ARGV_BUILD_MISMATCH;
    }
    *invocation_out = parsed;
    return HYP_INDEX_WORKER_ARGV_VALID;
}

const char *hyp_index_worker_argv_status_message(hyp_index_worker_argv_status_t status) {
    switch (status) {
    case HYP_INDEX_WORKER_ARGV_VALID:
        return "worker invocation accepted";
    case HYP_INDEX_WORKER_ARGV_INVALID:
        return "invalid internal worker arguments";
    case HYP_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE:
        return "worker executable fingerprint could not be verified";
    case HYP_INDEX_WORKER_ARGV_BUILD_MISMATCH:
        return "worker executable build conflicts with its supervisor; close all HYP sessions "
               "and retry";
    case HYP_INDEX_WORKER_ARGV_NOT_WORKER:
    default:
...
int hyp_index_supervisor_spawn_count(void) {
...
int hyp_index_supervisor_spawn_st_count(void) {
...
bool hyp_index_supervisor_should_wrap(void) {
    if (!g_host_marked) {
    ...
    /* Supervision is a safety boundary for the physical HYP host. An ambient
     * variable must never turn a long-lived MCP/CLI parent into the indexer. */
    return true;
}

static bool supervisor_disable_requested(void) {
    char supervisor_setting[HYP_SZ_32] = {0};
    return hyp_safe_getenv("HYP_INDEX_SUPERVISOR", supervisor_setting, sizeof(supervisor_setting),
                           NULL) &&
 ...
 * HYP_INDEX_WORKER_TIMEOUT_S override (seconds → ms) tightens it for tests. */
static int worker_quiet_timeout_ms(void) {
    enum { DEFAULT_QUIET_TIMEOUT_MS = 900000 }; /* 15 min with no progress */
    char timeout_seconds[HYP_SZ_32] = {0};
    if (hyp_safe_getenv("HYP_INDEX_WORKER_TIMEOUT_S", timeout_seconds, sizeof(timeout_seconds),
                        NULL) &&
        timeout_seconds[0]) {
...
static char *slurp_worker_response(const char *path, worker_response_read_status_t *status_out) {
    *status_out = WORKER_RESPONSE_READ_ERROR;
    ...
    if ((uint64_t)n > (uint64_t)HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX) {
...
enum {
    INDEX_WORKER_PATH_CAP = HYP_SZ_4K,
...
struct hyp_index_worker_handle {
    hyp_subprocess_t *process;
    char response_path[INDEX_WORKER_PATH_CAP];
    char log_path[INDEX_WORKER_PATH_CAP];
    hyp_proc_log_cb log_callback;
    void *log_context;
    long relay_tail_pos;
    bool process_terminal;
    hyp_proc_result_t process_result;
 ...
 * left undelivered, matching hyp_proc_log_cb's completed-line contract. */
static bool worker_relay_log(hyp_index_worker_handle_t *handle) {
    if (!handle || !handle->log_callback) {
        return true;
    }

#ifdef _WIN32
    FILE *log = hyp_fopen(handle->log_path, "rb");
#else
...
/* hyp_mkstemp's Windows compatibility implementation uses an internal buffer.
...
static void worker_result_init(hyp_index_worker_result_t *result) {
...
static bool worker_cache_dir(char out[INDEX_WORKER_PATH_CAP]) {
    ...
    int written = snprintf(out, INDEX_WORKER_PATH_CAP, "%s/.cache/hyponoia", home);
    if (written <= 0 || written >= INDEX_WORKER_PATH_CAP) {
        return false;
    }
    hyp_normalize_path_sep(out);
...
static bool worker_result_succeeded(const hyp_index_worker_result_t *result) {
    return result && result->outcome == HYP_PROC_CLEAN && !result->cancellation_requested &&
           result->tree_quiesced && !result->supervision_failed;
}

static void worker_terminal_log(hyp_index_worker_handle_t *handle) {
    char signal_text[16];
    char exit_text[16];
    ...
    hyp_log_info("index.supervisor.reap", "outcome", hyp_proc_outcome_str(handle->result.outcome),
                 "exit_code", exit_text, "signal", signal_text);
    if (handle->result.response_rejected) {
        hyp_log_error("index.supervisor.response_rejected", "reason", "payload_too_large", "log",
                      handle->log_path);
    } else if (handle->result.supervision_failed || !handle->result.tree_quiesced) {
        hyp_log_error("index.supervisor.containment_failed", "outcome",
                      hyp_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.cancellation_requested) {
        hyp_log_warn("index.supervisor.worker_cancelled", "outcome",
                     hyp_proc_outcome_str(handle->result.outcome), "log", handle->log_path);
    } else if (handle->result.outcome == HYP_PROC_CLEAN && !hyp_profile_active) {
        (void)hyp_unlink(handle->log_path);
    } else if (handle->result.outcome == HYP_PROC_CLEAN) {
        hyp_log_info("index.supervisor.profile_log", "log", handle->log_path);
    } else {
        hyp_log_warn("index.supervisor.worker_failed", "outcome",
                     hyp_proc_outcome_str(handle->result.outcome), "exit_code", exit_text, "log",
                     handle->log_path);
    }
}

int hyp_index_worker_start_with_log(const char *args_json, size_t memory_budget_bytes,
                                    bool single_thread, const char *marker_file,
                                    const char *quarantine_file, hyp_proc_log_cb log_callback,
                                    void *log_context, hyp_index_worker_handle_t **handle_out) {
    if (handle_out) {
        *handle_out = NULL;
    }
    ...
    if (g_host_marked && !g_worker_active && supervisor_disable_requested()) {
        ...
        hyp_log_error("index.supervisor.disable_refused", "action", "fail_closed");
        return -1;
    }
    const char *build_fingerprint = hyp_index_supervisor_build_fingerprint();
    if (!build_fingerprint) {
        hyp_log_error("index.supervisor.build_fingerprint_unavailable", "action", "refuse_spawn");
        return -1;
    }

    char self[INDEX_WORKER_PATH_CAP] = {0};
    if (!hyp_http_server_resolve_binary_path(NULL, self, sizeof(self)) || !self[0]) {
        hyp_log_error("index.supervisor.no_self_path", "action", "fail_closed");
        return -1;
    }
    hyp_index_worker_handle_t *handle = calloc(1, sizeof(*handle));
    if (!handle) {
        return -1;
    }
    ...
    if (!worker_unique_file(handle->response_path, sizeof(handle->response_path), "response") ||
        ...
        (void)hyp_unlink(handle->response_path);
        (void)hyp_unlink(handle->log_path);
        free(handle);
        return -1;
    }
    ...
    if (hyp_subprocess_spawn(&options, &handle->process) != 0) {
        (void)hyp_unlink(handle->response_path);
        (void)hyp_unlink(handle->log_path);
        free(handle);
        hyp_log_error("index.supervisor.spawn_failed", "action", "fail_closed");
        return -1;
    }
    *handle_out = handle;
    return 0;
...
int hyp_index_worker_start(const char *args_json, size_t memory_budget_bytes, bool single_thread,
                           const char *marker_file, const char *quarantine_file,
                           hyp_index_worker_handle_t **handle_out) {
    return hyp_index_worker_start_with_log(args_json, memory_budget_bytes, single_thread,
                                           marker_file, quarantine_file, NULL, NULL, handle_out);
}

hyp_index_worker_poll_t hyp_index_worker_poll(hyp_index_worker_handle_t *handle,
    ...
    if (!handle->process_terminal) {
        hyp_proc_result_t process_result;
        hyp_proc_poll_t state = hyp_subprocess_poll(handle->process, &process_result);
    ...
    const hyp_proc_result_t *process_result = &handle->process_result;
    ...
    if (worker_result_succeeded(&handle->result)) {
        ...
        if (response_status == WORKER_RESPONSE_READ_TOO_LARGE) {
            ...
            handle->result.outcome = HYP_PROC_EXIT_NONZERO;
...
bool hyp_index_worker_request_cancel(hyp_index_worker_handle_t *handle) {
    return handle && !atomic_load_explicit(&handle->terminal, memory_order_acquire) &&
           hyp_subprocess_request_cancel(handle->process);
}

const char *hyp_index_worker_response_path(const hyp_index_worker_handle_t *handle) {
    return handle ? handle->response_path : NULL;
}

const char *hyp_index_worker_log_path(const hyp_index_worker_handle_t *handle) {
    return handle ? handle->log_path : NULL;
}

void hyp_index_worker_destroy(hyp_index_worker_handle_t *handle) {
    if (!handle) {
        return;
    }
    if (!atomic_load_explicit(&handle->terminal, memory_order_acquire)) {
        hyp_log_error("index.supervisor.destroy_running", "action", "ignored");
        return;
    }
    hyp_subprocess_destroy(handle->process);
    free(handle->result.response);
    handle->result.response = NULL;
    free(handle);
}

int hyp_index_spawn_worker_with_log_cancel(const char *args_json, bool single_thread,
                                           const char *marker_file, const char *quarantine_file,
                                           hyp_proc_log_cb log_callback, void *log_context,
    ...
    hyp_index_worker_handle_t *handle = NULL;
    if (hyp_index_worker_start_with_log(args_json, 0, single_thread, marker_file, quarantine_file,
                                        log_callback, log_context, &handle) != 0) {
        return -1;
    }
    const hyp_index_worker_result_t *cached = NULL;
    ...
    for (;;) {
        if (!cancellation_forwarded && cancel_requested &&
            ...
            cancellation_forwarded = hyp_index_worker_request_cancel(handle);
        }
        hyp_index_worker_poll_t state = hyp_index_worker_poll(handle, &cached);
        if (state == HYP_INDEX_WORKER_POLL_TERMINAL) {
            break;
        }
        if (state == HYP_INDEX_WORKER_POLL_ERROR) {
            (void)hyp_index_worker_request_cancel(handle);
        }
        const struct timespec pause = {0, INDEX_WORKER_SYNC_POLL_NS};
        (void)hyp_nanosleep(&pause, NULL);
    }
    *result = *cached;
    result->response = cached->response ? hyp_strdup(cached->response) : NULL;
    hyp_index_worker_destroy(handle);
    return 0;
}

int hyp_index_spawn_worker_with_log(const char *args_json, bool single_thread,
                                    const char *marker_file, const char *quarantine_file,
                                    hyp_proc_log_cb log_callback, void *log_context,
                                    hyp_index_worker_result_t *result) {
    return hyp_index_spawn_worker_with_log_cancel(args_json, single_thread, marker_file,
                                                  quarantine_file, log_callback, log_context, NULL,
                                                  result);
}

int hyp_index_spawn_worker(const char *args_json, bool single_thread, const char *marker_file,
                           const char *quarantine_file, hyp_index_worker_result_t *result) {
    return hyp_index_spawn_worker_with_log(args_json, single_thread, marker_file, quarantine_file,
                                           NULL, NULL, result);
}

void hyp_index_worker_result_free(hyp_index_worker_result_t *result) {
    if (result) {
