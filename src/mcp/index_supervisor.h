 ...
 */
#ifndef HYP_INDEX_SUPERVISOR_H
#define HYP_INDEX_SUPERVISOR_H

#include <stdbool.h>
#include <stddef.h>
#include <stdatomic.h>

#include "foundation/subprocess.h" /* hyp_proc_outcome_t */
...
#define HYP_INDEX_WORKER_SINGLE_THREAD_ARG "--index-worker-single-thread"
#define HYP_INDEX_WORKER_MARKER_ARG "--index-worker-marker"
#define HYP_INDEX_WORKER_QUARANTINE_ARG "--index-worker-quarantine"
#define HYP_INDEX_WORKER_MEMORY_BUDGET_ARG "--index-worker-memory-budget-bytes"
#define HYP_INDEX_WORKER_BUILD_ARG "--index-worker-build"
#define HYP_INDEX_WORKER_BUILD_FINGERPRINT_LENGTH 64U
#define HYP_INDEX_WORKER_BUILD_FINGERPRINT_SIZE 65U
...
typedef struct {
    ...
    size_t memory_budget_bytes;
} hyp_index_worker_invocation_t;

typedef enum {
    HYP_INDEX_WORKER_ARGV_NOT_WORKER = 0,
    HYP_INDEX_WORKER_ARGV_VALID,
    HYP_INDEX_WORKER_ARGV_INVALID,
    HYP_INDEX_WORKER_ARGV_BUILD_UNAVAILABLE,
    HYP_INDEX_WORKER_ARGV_BUILD_MISMATCH,
} hyp_index_worker_argv_status_t;
 ...
 * hyponoia binary calls this (first thing in main(), before any
 * subcommand dispatch, so MCP server + CLI + HTTP paths are all covered).
 * EMBEDDERS of hyp_mcp_handle_tool (test binaries, future library users) never
 ...
 * this process called hyp_index_supervisor_mark_host() (i.e. it IS the real
 ...
 * entry to hyp_index_worker_start (including calls made by the synchronous
...
typedef struct {
    hyp_proc_outcome_t outcome; /* how the worker ended */
...
} hyp_index_worker_result_t;

/* Daemon-owned, nonblocking supervisor for one contained worker process tree. */
typedef struct hyp_index_worker_handle hyp_index_worker_handle_t;

typedef enum {
    HYP_INDEX_WORKER_POLL_ERROR = -1,
    HYP_INDEX_WORKER_POLL_RUNNING = 0,
    HYP_INDEX_WORKER_POLL_TERMINAL = 1,
} hyp_index_worker_poll_t;
 ...
 * and applied after exec by hyp_index_set_worker_role_options(). */
 ...
 *   - single_thread   → HYP_INDEX_SINGLE_THREAD=1
 *   - marker_file     → HYP_INDEX_MARKER_FILE
 *   - quarantine_file → HYP_INDEX_QUARANTINE_FILE
