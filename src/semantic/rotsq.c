...
static float g_rsq_diag[HYP_RSQ_DIM];
enum {
...
static void rsq_fwht(float *v) {
    for (int len = 1; len < HYP_RSQ_DIM; len <<= 1) {
        for (int i = 0; i < HYP_RSQ_DIM; i += len << 1) {
...
void hyp_rsq_encode(const float *v, hyp_rsq_code_t *out) {
    ...
    float rot[HYP_RSQ_DIM];
    for (int d = 0; d < HYP_RSQ_IN_DIM; d++) {
        rot[d] = v[d] * g_rsq_diag[d];
    }
    for (int d = HYP_RSQ_IN_DIM; d < HYP_RSQ_DIM; d++) {
        rot[d] = 0.0F;
    }
    rsq_fwht(rot);
    ...
    /* Per-vector scalar quantization over [lo, hi] at HYP_RSQ_BITS. */
    float range = hi - lo;
    float step = range > 0.0F ? range / (float)HYP_RSQ_LEVELS : 1.0F;
    out->offset = lo;
    out->scale = step;
...
float hyp_rsq_ip(const hyp_rsq_code_t *a, const hyp_rsq_code_t *b) {
    ...
    int64_t dot = 0;
    for (int i = 0; i < HYP_RSQ_CODE_BYTES; i++) {
