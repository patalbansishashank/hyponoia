 ...
 */
#ifndef HYP_CONFIG_JSON_LIKE_H
#define HYP_CONFIG_JSON_LIKE_H
 ...
 * members produce HYP_JSON_LIKE_OBJECT_MISMATCH. STRING fields may require an
...
typedef enum {
    ...
    HYP_JSON_LIKE_VALUE_EMPTY_ARRAY,
    HYP_JSON_LIKE_VALUE_SINGLE_STRING_ARRAY,
} hyp_json_like_value_shape_t;

enum {
    HYP_JSON_LIKE_FIELD_REQUIRED = 1U << 0U,
    HYP_JSON_LIKE_FIELD_CAPTURE_STRING = 1U << 1U,
};

typedef struct {
    const char *key;
    hyp_json_like_value_shape_t shape;
    const char *expected_string;
    unsigned flags;
} hyp_json_like_object_field_t;

enum {
    HYP_JSON_LIKE_OBJECT_MATCH = 0,
    HYP_JSON_LIKE_OBJECT_MISSING = 1,
    HYP_JSON_LIKE_OBJECT_MISMATCH = 2,
};

/* captured_string_out receives malloc-owned decoded content only on MATCH.
 * Returns one of HYP_JSON_LIKE_OBJECT_* or -1 for malformed input. */
...
/* Top-level convenience wrapper for hyp_json_like_add_unique_string_at_path. */
int hyp_json_like_add_unique_string(const char *file_path, const char *array_key,
                                    const char *string_value);

/* Top-level convenience wrapper for hyp_json_like_remove_string_at_path. */
...
typedef void (*hyp_json_like_precommit_test_hook_t)(const char *file_path, void *context);
