 ...
 */
#ifndef HYP_ACTIVATION_TRANSACTION_H
#define HYP_ACTIVATION_TRANSACTION_H

#include <stdbool.h>
#include <stddef.h>

typedef struct hyp_activation_transaction hyp_activation_transaction_t;

typedef enum {
    HYP_ACTIVATION_TRANSACTION_OK = 0,
    ...
    HYP_ACTIVATION_TRANSACTION_DEFERRED = 1,
    HYP_ACTIVATION_TRANSACTION_INVALID_ARGUMENT = -1,
    HYP_ACTIVATION_TRANSACTION_NO_MEMORY = -2,
    HYP_ACTIVATION_TRANSACTION_IO = -3,
    HYP_ACTIVATION_TRANSACTION_INVALID_STATE = -4,
    /* The post-commit validator rejected the candidate and rollback succeeded. */
    HYP_ACTIVATION_TRANSACTION_VALIDATION_FAILED = -5,
    /* The target changed, but restoring the retained backup also failed. */
    HYP_ACTIVATION_TRANSACTION_ROLLBACK_FAILED = -6,
} hyp_activation_transaction_status_t;

typedef bool (*hyp_activation_transaction_validator_fn)(const char *target_path, void *context);
 ...
 * leave this unset. */
typedef void (*hyp_activation_transaction_before_absent_publish_for_test_fn)(
...
const char *hyp_activation_transaction_target_path(const hyp_activation_transaction_t *transaction);
const char *hyp_activation_transaction_staged_path(const hyp_activation_transaction_t *transaction);
const char *hyp_activation_transaction_backup_path(const hyp_activation_transaction_t *transaction);
...
#endif /* HYP_ACTIVATION_TRANSACTION_H */
