...
#include "foundation/str_util.h" // hyp_json_escape
...
static void route_edge_visitor(const hyp_gbuf_edge_t *edge, void *userdata) {
    ...
    if (strcmp(edge->type, "HTTP_CALLS") == 0) {
        ...
        snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method ? method : "ANY",
                 hyp_route_canon_path(url, cpath, sizeof(cpath)));
    } else {
    ...
    hyp_gbuf_upsert_node(ctx->gb, "Route", url, route_qn, "", 0, 0, route_props);
    ctx->created++;
...
static int match_one_infra_route(hyp_gbuf_t *gb, const hyp_gbuf_node_t *infra,
                                 const char *infra_path, const char *svc_name,
                                 const hyp_gbuf_node_t **all_routes, int route_count) {
    for (int j = 0; j < route_count; j++) {
        const hyp_gbuf_node_t *handler_route = all_routes[j];
        ...
        if (path_match || root_svc_match) {
            const hyp_gbuf_edge_t **fn_handles = NULL;
            int fn_hcount = 0;
            hyp_gbuf_find_edges_by_target_type(gb, handler_route->id, "HANDLES", &fn_handles,
                                               &fn_hcount);
            for (int fh = 0; fh < fn_hcount; fh++) {
                hyp_gbuf_insert_edge(gb, fn_handles[fh]->source_id, infra->id, "HANDLES",
                                     "{\"source\":\"infra_match\"}");
            }
            return SKIP_ONE;
        }
...
/* Phase 2: Match infra Route URLs to handler Route nodes by URL path + service name. */
static void match_infra_routes(hyp_gbuf_t *gb) {
    const hyp_gbuf_node_t **all_routes = NULL;
    int route_count = 0;
    if (hyp_gbuf_find_by_label(gb, "Route", &all_routes, &route_count) != 0 || route_count == 0) {
        return;
    }
    ...
    for (int i = 0; i < route_count; i++) {
        const hyp_gbuf_node_t *infra = all_routes[i];
    ...
    if (matched > 0) {
        ...
        hyp_log_info("pass.route_match", "infra_matched", buf);
    }
}
...
static int ensure_one_decorator_route(hyp_gbuf_t *gb, const hyp_gbuf_node_t *func) {
    if (!func->properties_json) {
        return 0;
    }

    char path[HYP_SZ_256];
    ...
    snprintf(route_qn, sizeof(route_qn), "__route__%s__%s", method,
             hyp_route_canon_path(path, cpath, sizeof(cpath)));
    const hyp_gbuf_node_t *existing = hyp_gbuf_find_by_qn(gb, route_qn);

    char rprops[HYP_SZ_256];
    snprintf(rprops, sizeof(rprops), "{\"method\":\"%s\",\"source\":\"decorator\"}", method);
    int64_t route_id = hyp_gbuf_upsert_node(gb, "Route", path, route_qn,
                                            func->file_path ? func->file_path : "", 0, 0, rprops);

    if (existing) {
        const hyp_gbuf_edge_t **existing_handles = NULL;
        int eh_count = 0;
        hyp_gbuf_find_edges_by_target_type(gb, route_id, "HANDLES", &existing_handles, &eh_count);
    ...
    hyp_gbuf_insert_edge(gb, func->id, route_id, "HANDLES", hprops);
    return SKIP_ONE;
}

/* Phase 2a: Ensure all functions with route_path properties have Route+HANDLES edges. */
static void ensure_decorator_routes(hyp_gbuf_t *gb) {
    ...
    for (int li = 0; li < RN_STRIP_PASSES; li++) {
        ...
        int count = 0;
        if (hyp_gbuf_find_by_label(gb, labels[li], &nodes, &count) != 0) {
            continue;
        }
    ...
    if (created > 0) {
        ...
        hyp_log_info("pass.ensure_decorator_routes", "created", buf);
    }
}
...
static int bridge_funcs_to_prefix(hyp_gbuf_t *gb, const hyp_gbuf_node_t *prefix_route,
                                  const char *registrar_path, int dir_len,
                                  const char *prefix_segs) {
    const hyp_gbuf_node_t **funcs = NULL;
    int func_count = 0;
    hyp_gbuf_find_by_label(gb, "Function", &funcs, &func_count);

    int connected = 0;
    for (int fi = 0; fi < func_count; fi++) {
        const hyp_gbuf_node_t *func = funcs[fi];
        if (!func->file_path) {
            continue;
        }
        ...
        hyp_gbuf_insert_edge(gb, func->id, prefix_route->id, "HANDLES",
...
/* Phase 2b: Connect prefix Routes to decorator handler Functions. */
static void connect_prefix_to_decorators(hyp_gbuf_t *gb) {
    const hyp_gbuf_node_t **routes = NULL;
    int route_count = 0;
    if (hyp_gbuf_find_by_label(gb, "Route", &routes, &route_count) != 0) {
        return;
    }
    ...
    for (int ri = 0; ri < route_count; ri++) {
        const hyp_gbuf_node_t *prefix_route = routes[ri];
        ...
        const hyp_gbuf_edge_t **calls_in = NULL;
        int calls_count = 0;
        hyp_gbuf_find_edges_by_target_type(gb, prefix_route->id, "CALLS", &calls_in, &calls_count);
        if (calls_count == 0) {
            continue;
        }

        const hyp_gbuf_node_t *registrar = hyp_gbuf_find_by_id(gb, calls_in[0]->source_id);
    ...
    if (connected > 0) {
        ...
        hyp_log_info("pass.prefix_bridge", "connected", buf);
    }
}
...
static int has_direct_call(const hyp_gbuf_t *gb, int64_t source, int64_t target) {
    const hyp_gbuf_edge_t **edges = NULL;
    int count = 0;
    hyp_gbuf_find_edges_by_source_type(gb, source, "CALLS", &edges, &count);
    for (int i = 0; i < count; i++) {
...
static void extract_param_names(const hyp_gbuf_node_t *node, char *buf, int bufsize) {
    buf[0] = '\0';
...
static bool http_call_edge_has_valid_route(const hyp_gbuf_edge_t *edge) {
    char url_buf[HYP_SZ_512];
    const char *url = json_extract(edge->properties_json, "url_path", url_buf, sizeof(url_buf));
    char callee_buf[HYP_SZ_256];
    const char *callee =
        json_extract(edge->properties_json, "callee", callee_buf, sizeof(callee_buf));
    return hyp_service_pattern_is_http_route_literal(url, callee);
}
...
static int try_create_data_flow(hyp_gbuf_t *gb, int64_t caller_id, int64_t handler_id,
                                const hyp_gbuf_node_t *route, const char *edge_type,
    ...
    const hyp_gbuf_node_t *handler_node = hyp_gbuf_find_by_id(gb, handler_id);
    char handler_params[HYP_SZ_512];
    ...
    char esc_rname[HYP_SZ_256];
    char esc_rqn[HYP_SZ_512];
    hyp_json_escape(esc_rname, sizeof(esc_rname), route->name ? route->name : "");
    hyp_json_escape(esc_rqn, sizeof(esc_rqn), route->qualified_name ? route->qualified_name : "");
    ...
    hyp_gbuf_insert_edge(gb, caller_id, handler_id, "DATA_FLOWS", props);
    return SKIP_ONE;
}

/* Collect extra handler IDs reachable via INFRA_MAPS chain from a route. */
static int collect_infra_handlers(hyp_gbuf_t *gb, int64_t route_id, int64_t *out, int max_out) {
    const hyp_gbuf_edge_t **infra_edges = NULL;
    int infra_count = 0;
    hyp_gbuf_find_edges_by_source_type(gb, route_id, "INFRA_MAPS", &infra_edges, &infra_count);

    int n = 0;
    for (int ie = 0; ie < infra_count; ie++) {
        const hyp_gbuf_edge_t **ep_handles = NULL;
        int ep_hcount = 0;
        hyp_gbuf_find_edges_by_target_type(gb, infra_edges[ie]->target_id, "HANDLES", &ep_handles,
                                           &ep_hcount);
...
static int collect_caller_edges(hyp_gbuf_t *gb, int64_t route_id, caller_edge_ref_t *out,
                                int max_out) {
    int n = 0;
    const hyp_gbuf_edge_t **http_edges = NULL;
    int http_count = 0;
    hyp_gbuf_find_edges_by_target_type(gb, route_id, "HTTP_CALLS", &http_edges, &http_count);
    ...
    const hyp_gbuf_edge_t **async_edges = NULL;
    int async_count = 0;
    hyp_gbuf_find_edges_by_target_type(gb, route_id, "ASYNC_CALLS", &async_edges, &async_count);
...
static void create_route_data_flows(hyp_gbuf_t *gb, const hyp_gbuf_node_t *route,
                                    const caller_edge_ref_t *callers, int n_callers, int *flows,
                                    int *skipped) {
    const hyp_gbuf_edge_t **handles_edges = NULL;
    int handles_count = 0;
    hyp_gbuf_find_edges_by_target_type(gb, route->id, "HANDLES", &handles_edges, &handles_count);

    int64_t extra_handlers[HYP_SZ_32];
    int n_extra = collect_infra_handlers(gb, route->id, extra_handlers, HYP_SZ_32);
...
static const hyp_gbuf_node_t *find_enclosing_service(const hyp_gbuf_node_t *fn,
                                                     const hyp_gbuf_node_t **services,
                                                     int svc_count) {
...
static void create_grpc_routes(hyp_gbuf_t *gb) {
    const hyp_gbuf_node_t **classes = NULL;
    int class_count = 0;
    if (hyp_gbuf_find_by_label(gb, "Class", &classes, &class_count) != 0 || class_count == 0) {
        return;
    }

    const hyp_gbuf_node_t *services[HYP_SZ_64];
    int svc_count = 0;
    for (int i = 0; i < class_count && svc_count < HYP_SZ_64; i++) {
    ...
    const hyp_gbuf_node_t **funcs = NULL;
    int func_count = 0;
    if (hyp_gbuf_find_by_label(gb, "Function", &funcs, &func_count) != 0 || func_count == 0) {
        return;
    }

    int grpc_routes = 0;
    for (int f = 0; f < func_count; f++) {
        const hyp_gbuf_node_t *fn = funcs[f];
        if (!fn->file_path || !strstr(fn->file_path, ".proto") || !fn->name) {
            continue;
        }
        const hyp_gbuf_node_t *svc = find_enclosing_service(fn, services, svc_count);
        if (!svc) {
            continue;
        }
        char route_qn[HYP_ROUTE_QN_SIZE];
        ...
        char props[HYP_SZ_128];
        snprintf(props, sizeof(props), "{\"source\":\"proto\",\"service\":\"%s\"}", svc->name);

        int64_t route_id = hyp_gbuf_upsert_node(gb, "Route", fn->name, route_qn, fn->file_path,
                                                fn->start_line, fn->end_line, props);
        hyp_gbuf_insert_edge(gb, fn->id, route_id, "HANDLES", "{\"via\":\"proto_rpc\"}");
        grpc_routes++;
    }
    if (grpc_routes > 0) {
        char buf[HYP_SZ_16];
        snprintf(buf, sizeof(buf), "%d", grpc_routes);
        hyp_log_info("pass.route_nodes.grpc", "routes", buf);
    }
}

/* Phase 3: Create DATA_FLOWS edges by linking callers through Route to handlers. */
static void create_data_flows(hyp_gbuf_t *gb) {
    const hyp_gbuf_node_t **routes = NULL;
    int route_count = 0;
    if (hyp_gbuf_find_by_label(gb, "Route", &routes, &route_count) != 0 || route_count == 0) {
        return;
    }
    ...
    for (int ri = 0; ri < route_count; ri++) {
        caller_edge_ref_t caller_edges[HYP_SZ_64];
        int n_callers = collect_caller_edges(gb, routes[ri]->id, caller_edges, HYP_SZ_64);
    ...
    if (flows > 0 || skipped > 0) {
        ...
        hyp_log_info("pass.data_flows", "created", buf1, "skipped_has_call", buf2);
    }
}
...
static void sveltekit_file_visitor(const hyp_gbuf_node_t *node, void *userdata) {
    ...
    const hyp_gbuf_edge_t **edges = NULL;
    int edge_count = 0;
    if (hyp_gbuf_find_edges_by_source_type(ctx->gb, node->id, "DEFINES", &edges, &edge_count) !=
            0 ||
        edge_count == 0) {
        return;
    }
    ...
    for (int i = 0; i < edge_count; i++) {
        const hyp_gbuf_node_t *child = hyp_gbuf_find_by_id(ctx->gb, edges[i]->target_id);
        ...
        int64_t route_id =
            hyp_gbuf_upsert_node(ctx->gb, "Route", route_path, route_qn, "", 0, 0, route_props);
        if (route_id == 0) {
            continue;
        }
        ctx->routes_created++;

        char hprops[HYP_SZ_256];
        ...
        hyp_gbuf_insert_edge(ctx->gb, child->id, route_id, "HANDLES", hprops);
        ctx->handles_created++;
    }
}
...
static void create_sveltekit_routes(hyp_gbuf_t *gb) {
    if (!gb) {
        return;
    }
    sveltekit_ctx_t ctx = {.gb = gb, .routes_created = 0, .handles_created = 0, .files_seen = 0};
    hyp_gbuf_foreach_node(gb, sveltekit_file_visitor, &ctx);
    if (ctx.files_seen > 0) {
        char b1[HYP_SZ_16];
        char b2[HYP_SZ_16];
        char b3[HYP_SZ_16];
        ...
        hyp_log_info("pass.sveltekit_routes", "files", b1, "routes", b2, "handles", b3);
    }
}
...
void hyp_pipeline_create_route_nodes(hyp_gbuf_t *gb) {
    ...
    hyp_gbuf_foreach_edge(gb, route_edge_visitor, &ctx);

    if (ctx.created > 0) {
        char buf[HYP_SZ_16];
        snprintf(buf, sizeof(buf), "%d", ctx.created);
        hyp_log_info("pass.route_nodes", "created", buf);
    }
