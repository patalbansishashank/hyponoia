...
typedef HYP_DYN_ARRAY(char *) qn_array_t;
 ...
 * contention. */
static HYP_TLS HYPHashTable *_reach_cache = NULL;
...
void hyp_registry_reach_cache_begin(int estimated_capacity) {
    if (_reach_cache) {
        ...
        hyp_ht_foreach(_reach_cache, reach_cache_free_key, NULL);
        hyp_ht_clear(_reach_cache);
        return;
    }
    if (estimated_capacity < 16)
        estimated_capacity = 16;
    _reach_cache = hyp_ht_create((uint32_t)estimated_capacity);
}
...
static HYP_TLS HYPHashTable *_import_map_cache = NULL;

void hyp_registry_import_map_cache_begin(const char **keys, const char **vals, int count) {
    if (_import_map_cache) {
        hyp_ht_free(_import_map_cache);
        _import_map_cache = NULL;
    }
    if (!keys || !vals || count <= 0)
        return;
    _import_map_cache = hyp_ht_create((uint32_t)count * 2u + 8u);
    if (!_import_map_cache)
        return;
    for (int i = 0; i < count; i++) {
        if (keys[i] && vals[i]) {
            hyp_ht_set(_import_map_cache, keys[i], (void *)(uintptr_t)vals[i]);
        }
    }
}
...
/* ── Per-file full-result cache for hyp_registry_resolve ──────────
 *
 * THE big one: 98.7% of resolve_calls CPU on kubernetes lives inside
 * hyp_registry_resolve's strategy chain (suffix_match iterating
 ...
 * to the same QN every time. Cache the full hyp_resolution_t result
...
static HYP_TLS HYPHashTable *_resolve_cache = NULL;
...
void hyp_registry_resolve_cache_begin(int estimated_capacity) {
    if (_resolve_cache) {
        hyp_ht_foreach(_resolve_cache, resolve_cache_free_entry, NULL);
        hyp_ht_free(_resolve_cache);
        _resolve_cache = NULL;
    }
    if (estimated_capacity < 32)
        estimated_capacity = 32;
    _resolve_cache = hyp_ht_create((uint32_t)estimated_capacity);
}
...
static bool is_import_reachable(const char *candidate_qn, const char **import_vals,
                                int import_count) {
    if (_reach_cache) {
        void *cached = hyp_ht_get(_reach_cache, candidate_qn);
    ...
    if (_reach_cache) {
        ...
        if (kdup) {
            hyp_ht_set(_reach_cache, kdup, reachable ? REACH_CACHE_TRUE : REACH_CACHE_FALSE);
        }
    }
    return reachable;
}
...
static hyp_resolution_t empty_result(void) {
    hyp_resolution_t r = {0};
    return r;
}
 ...
 * hyp_resolution_t.strategy of a non-empty match;
...
bool hyp_perl_suppress_generic_match(bool is_perl, bool is_method, const char *callee_name,
    ...
    if (!(is_method || hyp_perl_is_builtin(callee_name))) {
        return false;
    }
...
bool hyp_tsjs_suppress_weak_method_match(bool is_tsjs, bool is_method, const char *strategy) {
     ...
     * "fuzzy" is listed as defensive insurance only — hyp_registry_fuzzy_resolve
...
hyp_registry_t *hyp_registry_new(void) {
    hyp_registry_t *r = calloc(HYP_ALLOC_ONE, sizeof(hyp_registry_t));
    if (!r) {
        return NULL;
    }
    r->exact = hyp_ht_create(HYP_SZ_1K);
    r->by_name = hyp_ht_create(HYP_SZ_512);
    return r;
}
...
void hyp_registry_free(hyp_registry_t *r) {
    ...
    hyp_ht_foreach(r->by_name, free_qn_array, NULL);
    hyp_ht_free(r->by_name);
    hyp_ht_foreach(r->exact, free_label, NULL);
    hyp_ht_free(r->exact);
...
void hyp_registry_add(hyp_registry_t *r, const char *name, const char *qualified_name,
    ...
    /* Check for duplicate */
    if (hyp_ht_get(r->exact, qualified_name)) {
        return;
    }
    ...
    hyp_ht_set(r->exact, strdup(qualified_name), (void *)interned);
    const char *owned_qn = hyp_ht_get_key(r->exact, qualified_name);

    /* Index by simple name.
     * No array dedup needed: exact-map check above guarantees uniqueness. */
    const char *simple = simple_name(qualified_name);
    qn_array_t *arr = hyp_ht_get(r->by_name, simple);
    if (!arr) {
        arr = calloc(HYP_ALLOC_ONE, sizeof(qn_array_t));
        hyp_ht_set(r->by_name, strdup(simple), arr);
    }
    hyp_da_push(arr, (char *)owned_qn);
}
...
bool hyp_registry_exists(const hyp_registry_t *r, const char *qn) {
    ...
    return hyp_ht_get(r->exact, qn) != NULL;
}
...
int hyp_registry_find_by_name(const hyp_registry_t *r, const char *name, const char ***out,
    ...
    qn_array_t *arr = hyp_ht_get(r->by_name, name);
    if (arr && arr->count > 0) {
...
int hyp_registry_size(const hyp_registry_t *r) {
    return r ? (int)hyp_ht_count(r->exact) : 0;
}
...
static hyp_resolution_t resolve_import_map(const hyp_registry_t *r, const char *prefix,
    ...
    if (_import_map_cache) {
        resolved = (const char *)hyp_ht_get(_import_map_cache, prefix);
    } else {
    ...
    if (!suffix || !suffix[0]) {
        const char *direct = hyp_ht_get_key(r->exact, resolved);
        if (direct) {
            return (hyp_resolution_t){direct, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
        }
    }
    char candidate[HYP_SZ_512];
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, suffix);
    } else {
        snprintf(candidate, sizeof(candidate), "%s.%s", resolved, prefix);
    }
    /* Use hyp_ht_get_key to get the persistent heap-owned key string */
    const char *stored_key = hyp_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (hyp_resolution_t){stored_key, "import_map", CONF_IMPORT_MAP, REG_RESOLVED};
    }
     ...
     * candidates instead of hyp_ht_foreach over the WHOLE exact table — that
    ...
    if (suffix && suffix[0]) {
        char resolved_dot[HYP_SZ_512];
        char dot_suffix[HYP_SZ_256];
        snprintf(resolved_dot, sizeof(resolved_dot), "%s.", resolved);
        snprintf(dot_suffix, sizeof(dot_suffix), ".%s", suffix);
        qn_array_t *arr = hyp_ht_get(r->by_name, simple_name(suffix));
        if (arr) {
            ...
            for (int i = 0; i < arr->count; i++) {
                ...
                if (klen >= rd_len + ds_len && strncmp(qn, resolved_dot, rd_len) == 0 &&
                    ...
                    return (hyp_resolution_t){qn, "import_map_suffix", CONF_IMPORT_MAP_SUFFIX,
                                              REG_RESOLVED};
                }
            }
        }
...
static hyp_resolution_t resolve_same_module(const hyp_registry_t *r, const char *callee_name,
                                            const char *suffix, const char *module_qn) {
    char candidate[HYP_SZ_512];
    snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, callee_name);
    const char *stored_key = hyp_ht_get_key(r->exact, candidate);
    if (stored_key) {
        return (hyp_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
    }
    if (suffix && suffix[0]) {
        snprintf(candidate, sizeof(candidate), "%s.%s", module_qn, suffix);
        stored_key = hyp_ht_get_key(r->exact, candidate);
        if (stored_key) {
            return (hyp_resolution_t){stored_key, "same_module", CONF_SAME_MODULE, REG_RESOLVED};
        }
    }
...
static hyp_resolution_t resolve_multi_with_imports(const qn_array_t *arr, const char *module_qn,
                                                   const char **import_vals, int import_count) {
    const char *filtered[HYP_SZ_256];
    int fcount = 0;
    for (int i = 0; i < arr->count && fcount < HYP_SZ_256; i++) {
    ...
    if (fcount == SKIP_ONE) {
        ...
        return (hyp_resolution_t){filtered[0], "suffix_match", conf, arr->count};
    }
    if (fcount > SKIP_ONE) {
        ...
        if (best) {
            ...
            return (hyp_resolution_t){best, "suffix_match", conf, fcount};
        }
    }
    ...
    if (best) {
        ...
        return (hyp_resolution_t){best, "suffix_match", conf, arr->count};
    }
    return empty_result();
}
...
static hyp_resolution_t resolve_name_lookup(const hyp_registry_t *r, const char *callee_name,
                                            const char *module_qn, const char **import_vals,
                                            int import_count) {
    const char *lookup = simple_name(callee_name);
    qn_array_t *arr = hyp_ht_get(r->by_name, lookup);
    if (!arr || arr->count == 0) {
        return empty_result();
    }
    ...
    if (arr->count > 1) {
        ...
        if (q) {
            return (hyp_resolution_t){q, "qualified_suffix", CONF_QUALIFIED_SUFFIX, REG_RESOLVED};
        }
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (hyp_resolution_t){arr->items[0], "unique_name", conf, REG_RESOLVED};
    }
...
hyp_resolution_t hyp_registry_resolve(const hyp_registry_t *r, const char *callee_name,
    ...
    if (_resolve_cache) {
        resolve_cache_entry_t *cached =
            (resolve_cache_entry_t *)hyp_ht_get(_resolve_cache, callee_name);
        if (cached) {
            return cached->res;
        }
    }
    ...
    if (_resolve_cache) {
        ...
        if (e) {
            ...
            if (kdup) {
                hyp_ht_set(_resolve_cache, kdup, e);
            } else {
                free(e);
            }
        }
...
hyp_fuzzy_result_t hyp_registry_fuzzy_resolve(const hyp_registry_t *r, const char *callee_name,
    ...
    hyp_fuzzy_result_t no_match = {{0}, false};
    if (!r || !callee_name) {
        return no_match;
    }
    ...
    if (arr->count == SKIP_ONE) {
        ...
        return (hyp_fuzzy_result_t){{arr->items[0], "fuzzy", conf, REG_RESOLVED}, true};
    }
    ...
    if (have_imports) {
        fcount = filter_import_reachable((const char **)arr->items, arr->count, import_map_vals,
                                         import_map_count, filtered, HYP_SZ_256);
        fptr = filtered;
    }
...
int hyp_registry_find_ending_with(const hyp_registry_t *r, const char *suffix, const char ***out) {
    ...
    hyp_ht_foreach(r->exact, few_scan, &ctx);
