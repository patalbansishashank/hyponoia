...
#include "foundation/str_util.h" /* hyp_validate_shell_arg — git shell-out hardening */
...
static _Thread_local char g_export_error[HYP_SZ_512];

static const char *itoa_buf(int v) {
    static _Thread_local char bufs[ART_RING][HYP_SZ_32];
...
const char *hyp_artifact_export_last_error(void) {
...
static int artifact_export_fail(const char *stage, const char *path, const char *err, int err_no) {
    ...
    if (path && err_no != 0) {
        hyp_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no), "path", path);
    } else if (path) {
        hyp_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "path", path);
    } else if (err_no != 0) {
        hyp_log_error("artifact.export", "stage", safe_stage, "err", safe_err, "errno",
                      itoa_buf(err_no));
    } else {
        hyp_log_error("artifact.export", "stage", safe_stage, "err", safe_err);
    }
...
/* Build path: <repo>/.hyponoia/<name> into caller-owned buf. */
static bool artifact_path(char *buf, size_t bufsz, const char *repo_path, const char *name) {
    int n = snprintf(buf, bufsz, "%s/%s/%s", repo_path, HYP_ARTIFACT_DIR, name);
 ...
 * git shell-out): hyp_validate_shell_arg rejects quote / backslash / substitution
...
bool hyp_artifact_repo_path_is_shell_safe(const char *repo_path) {
    return hyp_validate_shell_path_arg(repo_path);
}

/* Get current git HEAD hash. buf must be >= HYP_SZ_64. Returns false on error. */
static bool git_head_hash(const char *repo_path, char *buf, size_t bufsz) {
    char cmd[HYP_SZ_1K];
    if (!hyp_artifact_repo_path_is_shell_safe(repo_path)) {
        buf[0] = '\0';
        return false;
    }
    int n =
...
static int read_metadata_version(const char *repo_path) {
    char meta_path[HYP_SZ_4K];
    artifact_path(meta_path, sizeof(meta_path), repo_path, HYP_ARTIFACT_META);
    ...
    int version = ver ? yyjson_get_int(ver) : HYP_NOT_FOUND;
    yyjson_doc_free(doc);
    return version;
}
...
static int write_metadata(const char *repo_path, const char *project_name, int nodes, int edges,
    ...
    yyjson_mut_obj_add_int(doc, root, "schema_version", HYP_ARTIFACT_SCHEMA_VERSION);
    ...
    char meta_path[HYP_SZ_4K];
    if (!artifact_path(meta_path, sizeof(meta_path), repo_path, HYP_ARTIFACT_META)) {
        free(json);
...
static void ensure_gitattributes(const char *repo_path) {
    ...
    if (fd < 0) {
        if (errno != EEXIST) {
            hyp_log_warn("artifact.gitattributes.open path=%s err=%s", ga_path, strerror(errno));
        }
        ...
        if (fp) {
            ...
            (void)fputs("# Auto-generated by hyponoia\n"
                        "# Prevent merge conflicts on compressed artifact\n" HYP_ARTIFACT_FILENAME
                        " binary merge=ours\n",
                        fp);
            (void)fclose(fp);
        } else {
 ...
 * hyp_mkstemp would — sqlite has to be the one that creates it. Containing it in
 * a directory only this user can enter buys the same protection: hyp_mkdtemp
 ...
 * "<tmp>/hyp_artifact_tmp.db" was vulnerable on both counts — another local user
 * could pre-plant a symlink there to redirect the copy, and two concurrent
 * exports collided on the one name. */
typedef struct {
    char dir[HYP_SZ_512]; /* hyp_mkdtemp copies its result back into this buffer */
    char db[HYP_SZ_4K];
...
static bool artifact_snapshot_tmp_open(artifact_snapshot_tmp_t *tmp) {
    tmp->dir[0] = '\0';
    tmp->db[0] = '\0';
    int written = snprintf(tmp->dir, sizeof(tmp->dir), "%s/hyp-artifact-XXXXXX", hyp_tmpdir());
    if (written <= 0 || (size_t)written >= sizeof(tmp->dir) || !hyp_mkdtemp(tmp->dir)) {
        tmp->dir[0] = '\0';
        return false;
    }
    written = snprintf(tmp->db, sizeof(tmp->db), "%s/snapshot.db", tmp->dir);
    if (written <= 0 || (size_t)written >= sizeof(tmp->db)) {
        (void)hyp_rmdir(tmp->dir);
        tmp->dir[0] = '\0';
        return false;
    }
    return true;
...
static char *prepare_snapshot_db(const char *db_path, size_t *out_size, bool strip_indexes) {
    ...
    if (!artifact_snapshot_tmp_open(&tmp)) {
        artifact_export_fail("prepare_snapshot_dir", hyp_tmpdir(), "private_tmpdir_failed", errno);
        return NULL;
    }
...
int hyp_artifact_export(const char *db_path, const char *repo_path, const char *project_name,
                        int quality) {
    clear_export_error();
    ...
    if (!db_data || db_size == 0) {
        free(db_data);
        if (hyp_artifact_export_last_error()) {
            return HYP_NOT_FOUND;
        }
    ...
    /* Compress with zstd */
    size_t bound = hyp_zstd_compress_bound(db_size);
    ...
    int64_t clen = hyp_zstd_compress(db_data, db_size, compressed, bound, compression_level);
    free(db_data);
    ...
    char zst_path[HYP_SZ_4K];
    if (!artifact_path(zst_path, sizeof(zst_path), repo_path, HYP_ARTIFACT_FILENAME)) {
        free(compressed);
    ...
    int nodes = 0;
    int edges = 0;
    hyp_store_t *count_store = hyp_store_open_path_query(db_path);
    if (count_store) {
        nodes = hyp_store_count_nodes(count_store, project_name);
        edges = hyp_store_count_edges(count_store, project_name);
        hyp_store_close(count_store);
    }
    ...
    hyp_log_info("artifact.export", "quality", quality == HYP_ARTIFACT_BEST ? "best" : "fast",
...
int hyp_artifact_import(const char *repo_path, const char *cache_db_path) {
    ...
    if (version < 0 || version > HYP_ARTIFACT_SCHEMA_VERSION) {
        hyp_log_info("artifact.import", "skip", "schema_version_mismatch", "artifact_ver",
                     itoa_buf(version), "current_ver", itoa_buf(HYP_ARTIFACT_SCHEMA_VERSION));
        return HYP_NOT_FOUND;
    }
    ...
    if (original_size == 0) {
        hyp_log_error("artifact.import", "err", "missing_original_size");
        return HYP_NOT_FOUND;
    }

    /* Read compressed artifact */
    char zst_path[HYP_SZ_4K];
    artifact_path(zst_path, sizeof(zst_path), repo_path, HYP_ARTIFACT_FILENAME);

    size_t clen = 0;
    char *compressed = read_file_alloc(zst_path, &clen);
    if (!compressed) {
        hyp_log_error("artifact.import", "err", "read_artifact");
        return HYP_NOT_FOUND;
    }
    ...
    size_t frame_size = hyp_zstd_frame_content_size(compressed, clen);
    if (frame_size == 0 || frame_size > ART_MAX_DECOMPRESSED_BYTES || frame_size != original_size) {
        free(compressed);
        hyp_log_error("artifact.import", "err", "bad_decompressed_size");
        return HYP_NOT_FOUND;
    }
    ...
    int64_t dlen = hyp_zstd_decompress(compressed, clen, decompressed, frame_size);
    free(compressed);

    if (dlen <= 0 || (size_t)dlen != frame_size) {
        free(decompressed);
        hyp_log_error("artifact.import", "err", "zstd_decompress");
        return HYP_NOT_FOUND;
    }

    /* Write to temp file, then rename for atomicity */
    char tmp_path[HYP_SZ_4K];
    ...
    if (last_slash) {
        *last_slash = '\0';
        hyp_mkdir_p(cache_dir, ART_DIR_PERMS);
    }
    ...
    if (wrc != 0) {
        if (ioerr.err_no != 0) {
            hyp_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "errno",
                          itoa_buf(ioerr.err_no), "path", tmp_path);
        } else {
            hyp_log_error("artifact.import", "err", "write_temp_db", "detail", ioerr.err, "path",
                          tmp_path);
        }
        return HYP_NOT_FOUND;
    }

    /* Open with hyp_store_open_path to auto-create missing indexes + FTS5 */
    hyp_store_t *store = hyp_store_open_path(tmp_path);
    if (!store) {
        hyp_log_error("artifact.import", "err", "open_imported_db");
        hyp_unlink(tmp_path);
        return HYP_NOT_FOUND;
    }
    ...
    if (!hyp_store_check_integrity_deep(store)) {
        hyp_log_error("artifact.import", "err", "integrity_check_failed");
    ...
    hyp_remove_db_sidecars(cache_db_path);
    if (rename(tmp_path, cache_db_path) != 0) {
        hyp_log_error("artifact.import", "err", "rename_to_cache");
        hyp_unlink(tmp_path);
        return HYP_NOT_FOUND;
    }
    ...
    hyp_log_info("artifact.import", "db", cache_db_path, "size_mb",
...
bool hyp_artifact_exists(const char *repo_path) {
    if (!repo_path) {
        return false;
    }
    ...
    int version = read_metadata_version(repo_path);
    return version >= 0 && version <= HYP_ARTIFACT_SCHEMA_VERSION;
}

/* ── Commit hash extraction ──────────────────────────────────────── */

char *hyp_artifact_commit(const char *repo_path) {
    if (!repo_path) {
        return NULL;
    }
