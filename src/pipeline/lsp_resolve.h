 ...
 *
 * Both pipeline paths (sequential hyp_pipeline_pass_calls and parallel
 * hyp_parallel_extract → resolve_file_calls) need to look up an
 ...
 */
#ifndef HYP_PIPELINE_LSP_RESOLVE_H
#define HYP_PIPELINE_LSP_RESOLVE_H
 ...
 * (Go, C/C++, Python, PHP). */
#define HYP_LSP_CONFIDENCE_FLOOR 0.6f
...
void hyp_pipeline_lsp_reference_lookup_test_note_row(void);
#else
static inline void hyp_pipeline_lsp_reference_lookup_test_note_row(void) {}
#endif
 ...
 * for them. */
static inline const char *hyp_lsp_bare_segment(const char *name) {
    if (!name) {
        return name;
    }
    const char *seg = name;
...
static inline const char *hyp_pipeline_qn_class_method_tail(const char *qn) {
    if (!qn) {
        return NULL;
    }
...
static inline const char *hyp_pipeline_call_callee_leaf(const char *callee_name) {
    return hyp_lsp_bare_segment(callee_name);
}
...
static inline bool hyp_pipeline_lsp_allow_tail_match(HYPLanguage lang) {
...
static inline bool hyp_pipeline_call_reference_usage_fallback_allowed(HYPLanguage lang,
                                                                      const char *strategy) {
    if (lang != HYP_LANG_JAVA && lang != HYP_LANG_KOTLIN) {
        return true;
    }
...
static inline bool hyp_pipeline_reference_candidate_fallback_allowed(
    HYPLanguage lang, const HYPUsage *usage, const char *strategy, const char *const *import_names,
    int import_count) {
    if (!usage) {
        return false;
    }
    bool reference_candidate =
        ...
        (lang == HYP_LANG_KOTLIN && usage->kind == HYP_USAGE_VALUE && usage->may_be_call_reference);
    if (!reference_candidate ||
        hyp_pipeline_call_reference_usage_fallback_allowed(lang, strategy)) {
        return true;
    }
    ...
    if (lang == HYP_LANG_KOTLIN && usage->ref_name && import_names) {
...
static inline bool hyp_pipeline_usage_semantic_reference_candidate(const HYPUsage *usage) {
    if (!usage) {
        return false;
    }
    return usage->kind == HYP_USAGE_CALL_REFERENCE ||
           (usage->kind == HYP_USAGE_VALUE && usage->may_be_call_reference);
}
...
static inline bool hyp_pipeline_usage_allows_semantic_reference(const HYPUsage *usage,
                                                                const HYPResolvedCall *resolved) {
    if (!usage || !resolved) {
        return false;
    }
    if (usage->kind != HYP_USAGE_VALUE || !usage->semantic_reference_blocked ||
...
static inline bool hyp_pipeline_node_is_callable_target(const hyp_gbuf_node_t *node) {
    if (!node || !node->label) {
        return false;
    }
...
static inline int hyp_pipeline_qn_class_method_tail_eq(const char *qn, const char *tail) {
    const char *qt = hyp_pipeline_qn_class_method_tail(qn);
    return qt && tail && strcmp(qt, tail) == 0;
}

static inline const hyp_gbuf_node_t *hyp_pipeline_lsp_target_node_policy(
    const hyp_gbuf_t *gbuf, const char *project_name, const char *callee_qn, bool allow_tail_match,
 ...
 * class and method. */
static inline bool hyp_pipeline_invocation_targets_equal(const char *left_qn, const char *right_qn,
                                                         const hyp_gbuf_t *gbuf,
    ...
    const hyp_gbuf_node_t *left_exact =
        hyp_pipeline_lsp_target_node_policy(gbuf, project_name, left_qn, false, false);
    const hyp_gbuf_node_t *right_exact =
        hyp_pipeline_lsp_target_node_policy(gbuf, project_name, right_qn, false, false);
    ...
    const char *left_tail = hyp_pipeline_qn_class_method_tail(left_qn);
    const char *right_tail = hyp_pipeline_qn_class_method_tail(right_qn);
    ...
    const hyp_gbuf_node_t *left =
        left_exact ? left_exact
                   : hyp_pipeline_lsp_target_node_policy(gbuf, project_name, left_qn, true, true);
    const hyp_gbuf_node_t *right =
        right_exact ? right_exact
                    : hyp_pipeline_lsp_target_node_policy(gbuf, project_name, right_qn, true, true);
...
static inline bool hyp_pipeline_source_site_present(uint32_t start, uint32_t end) {
    return end > start;
}

static inline bool hyp_pipeline_source_site_legacy(uint32_t start, uint32_t end) {
    return start == 0 && end == 0;
}

static inline bool hyp_pipeline_source_site_eq(uint32_t lhs_start, uint32_t lhs_end,
                                               uint32_t rhs_start, uint32_t rhs_end) {
    return hyp_pipeline_source_site_present(lhs_start, lhs_end) &&
           hyp_pipeline_source_site_present(rhs_start, rhs_end) && lhs_start == rhs_start &&
           lhs_end == rhs_end;
}

static inline bool hyp_pipeline_source_occurrence_eq(uint32_t lhs_start, uint32_t lhs_end,
                                                     HYPSourceOrigin lhs_origin, uint32_t rhs_start,
                                                     uint32_t rhs_end, HYPSourceOrigin rhs_origin) {
    return lhs_origin == rhs_origin &&
           hyp_pipeline_source_site_eq(lhs_start, lhs_end, rhs_start, rhs_end);
}
...
static inline int hyp_pipeline_invocation_site_rank(const HYPResolvedCall *resolved,
                                                    const HYPCall *call) {
    if (!resolved || !call) {
        return 0;
    }
    ...
    bool call_is_legacy =
        hyp_pipeline_source_site_legacy(call->site_start_byte, call->site_end_byte);
    if (!call_has_site && !call_is_legacy) {
        return 0;
    }
    if (hyp_pipeline_source_site_present(resolved->site_start_byte, resolved->site_end_byte)) {
        return hyp_pipeline_source_occurrence_eq(call->site_start_byte, call->site_end_byte,
                                                 call->source_origin, resolved->site_start_byte,
                                                 resolved->site_end_byte, resolved->source_origin)
                   ? 2
                   : 0;
    }
    if (!hyp_pipeline_source_site_legacy(resolved->site_start_byte, resolved->site_end_byte)) {
        return 0;
    }
    return call->requires_lsp_resolution || call->source_origin != resolved->source_origin ? 0 : 1;
}

static inline bool hyp_pipeline_invocation_reason_join_strategy(const char *strategy) {
    return strategy &&
...
static inline bool hyp_pipeline_invocation_leaf_matches(const HYPResolvedCall *resolved,
                                                        const HYPCall *call, int site_rank) {
    if (!resolved || !resolved->callee_qn || !call || !call->callee_name) {
        return false;
    }
    const char *resolved_leaf = hyp_lsp_bare_segment(resolved->callee_qn);
    const char *call_leaf = hyp_lsp_bare_segment(call->callee_name);
    ...
    return hyp_pipeline_invocation_reason_join_strategy(resolved->strategy) && resolved->reason &&
           call_leaf && strcmp(hyp_lsp_bare_segment(resolved->reason), call_leaf) == 0;
}
 ...
 *      (JVM callers only, see hyp_pipeline_lsp_allow_tail_match), a
 ...
 * bare last segment by hyp_lsp_bare_segment before matching.
 *
 * The pointer returned aliases into `arr` and stays valid as long as the
 * underlying HYPFileResult is alive. */
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_resolution_in_graph(
    const HYPResolvedCallArray *arr, const HYPCall *call, bool allow_tail_match,
    ...
    const HYPResolvedCall *best_exact_caller = NULL;
    int best_exact_caller_rank = 0;
    bool exact_caller_ambiguous = false;
    for (int i = 0; i < arr->count; i++) {
        const HYPResolvedCall *rc = &arr->items[i];
        if (rc->kind != HYP_RESOLVED_INVOCATION) {
            continue;
        }
        if (!rc->caller_qn || !rc->callee_qn) {
            continue;
        }
        if (rc->confidence < HYP_LSP_CONFIDENCE_FLOOR) {
            continue;
        }
        if (strcmp(rc->caller_qn, call->enclosing_func_qn) != 0) {
            continue;
        }
        int site_rank = hyp_pipeline_invocation_site_rank(rc, call);
        if (site_rank == 0 || !hyp_pipeline_invocation_leaf_matches(rc, call, site_rank)) {
            continue;
        }
        ...
        if (!hyp_pipeline_invocation_targets_equal(best_exact_caller->callee_qn, rc->callee_qn,
    ...
    const char *call_tail = hyp_pipeline_qn_class_method_tail(call->enclosing_func_qn);

    const HYPResolvedCall *best_tail_exact = NULL;
    const HYPResolvedCall *best_tail_legacy = NULL;
    ...
    for (int i = 0; i < arr->count; i++) {
        ...
        bool caller_matches =
            call_tail && hyp_pipeline_qn_class_method_tail_eq(rc->caller_qn, call_tail);
        if (!caller_matches && site_rank == 2) {
            const char *resolved_caller = hyp_lsp_bare_segment(rc->caller_qn);
            const char *source_caller = hyp_lsp_bare_segment(call->enclosing_func_qn);
            caller_matches =
                resolved_caller && source_caller && strcmp(resolved_caller, source_caller) == 0;
        }
        if (!caller_matches || !hyp_pipeline_invocation_leaf_matches(rc, call, site_rank)) {
            continue;
        }

        const HYPResolvedCall **best = site_rank == 2 ? &best_tail_exact : &best_tail_legacy;
        bool *ambiguous = site_rank == 2 ? &tail_exact_ambiguous : &tail_legacy_ambiguous;
        if (!*best) {
            *best = rc;
            continue;
        }
        if (!hyp_pipeline_invocation_targets_equal((*best)->callee_qn, rc->callee_qn, gbuf,
...
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_resolution(
    const HYPResolvedCallArray *arr, const HYPCall *call, bool allow_tail_match) {
    return hyp_pipeline_find_lsp_resolution_in_graph(arr, call, allow_tail_match, NULL, NULL);
}

static inline bool hyp_pipeline_reference_reason_matches(const HYPResolvedCall *resolved,
    ...
    return reason_join_strategy &&
           strcmp(hyp_lsp_bare_segment(resolved->reason), reference_leaf) == 0;
}

static inline const hyp_gbuf_node_t *hyp_pipeline_lsp_target_node(const hyp_gbuf_t *gbuf,
                                                                  const char *project_name,
                                                                  const char *callee_qn,
                                                                  bool allow_tail_match) {
    return hyp_pipeline_lsp_target_node_policy(gbuf, project_name, callee_qn, allow_tail_match,
                                               true);
}
...
static inline const hyp_gbuf_node_t *hyp_pipeline_lsp_target_node_strict(const hyp_gbuf_t *gbuf,
                                                                         const char *project_name,
                                                                         const char *callee_qn,
                                                                         bool allow_tail_match) {
    (void)allow_tail_match;
    return hyp_pipeline_lsp_target_node_policy(gbuf, project_name, callee_qn, false, false);
}
...
static inline bool hyp_pipeline_kotlin_external_target(HYPLanguage lang, const char *callee_qn) {
    if (lang != HYP_LANG_KOTLIN || !callee_qn) {
        return false;
    }
...
static inline bool hyp_pipeline_reference_targets_equal(const char *left_qn, const char *right_qn,
                                                        const hyp_gbuf_t *gbuf,
                                                        const char *project_name,
                                                        bool allow_tail_match) {
    if (!left_qn || !right_qn) {
    ...
    (void)allow_tail_match;
    const hyp_gbuf_node_t *left = hyp_pipeline_lsp_target_node(gbuf, project_name, left_qn, false);
    const hyp_gbuf_node_t *right =
        hyp_pipeline_lsp_target_node(gbuf, project_name, right_qn, false);
    return left && right && left->id == right->id;
}

typedef struct {
    const HYPResolvedCall *row;
    int original_index;
} hyp_pipeline_lsp_reference_index_entry_t;

typedef struct {
    hyp_pipeline_lsp_reference_index_entry_t *entries;
    int count;
} hyp_pipeline_lsp_reference_index_t;
...
static inline int hyp_pipeline_lsp_reference_index_entry_cmp(const void *left_ptr,
                                                             const void *right_ptr) {
    const hyp_pipeline_lsp_reference_index_entry_t *left =
        (const hyp_pipeline_lsp_reference_index_entry_t *)left_ptr;
    const hyp_pipeline_lsp_reference_index_entry_t *right =
        (const hyp_pipeline_lsp_reference_index_entry_t *)right_ptr;
...
static inline bool hyp_pipeline_lsp_reference_index_build(
    const HYPResolvedCallArray *arr, hyp_pipeline_lsp_reference_index_t *index) {
    if (!index) {
        return false;
    }
    index->entries = NULL;
    ...
    for (int i = 0; i < arr->count; i++) {
        if (arr->items[i].kind == HYP_RESOLVED_CALL_REFERENCE) {
            reference_count++;
        }
    }
    ...
    if ((size_t)reference_count > SIZE_MAX / sizeof(hyp_pipeline_lsp_reference_index_entry_t)) {
        return false;
    }

    hyp_pipeline_lsp_reference_index_entry_t *entries =
        (hyp_pipeline_lsp_reference_index_entry_t *)malloc((size_t)reference_count *
                                                           sizeof(*entries));
    if (!entries) {
        return false;
    }
    ...
    qsort(entries, (size_t)reference_count, sizeof(*entries),
          hyp_pipeline_lsp_reference_index_entry_cmp);
    index->entries = entries;
    index->count = reference_count;
    return true;
}

static inline void hyp_pipeline_lsp_reference_index_free(
    hyp_pipeline_lsp_reference_index_t *index) {
    if (!index) {
        return;
    }
    free(index->entries);
...
static inline int hyp_pipeline_lsp_reference_site_cmp_usage(const HYPResolvedCall *row,
...
static inline const HYPResolvedCall *hyp_pipeline_lsp_reference_view_row(
    const HYPResolvedCallArray *arr, const hyp_pipeline_lsp_reference_index_entry_t *entries,
...
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_reference_view_in_graph(
    const HYPResolvedCallArray *arr, const hyp_pipeline_lsp_reference_index_entry_t *entries,
    int first, int row_count, const HYPUsage *usage, bool allow_tail_match, const hyp_gbuf_t *gbuf,
    const char *project_name) {
    ...
    const char *ref_leaf = hyp_lsp_bare_segment(usage->ref_name);
    if (!ref_leaf || !ref_leaf[0]) {
        return NULL;
    }

    const HYPResolvedCall *best_exact = NULL;
    bool exact_ambiguous = false;
    for (int i = 0; i < row_count; i++) {
        hyp_pipeline_lsp_reference_lookup_test_note_row();
        const HYPResolvedCall *rc = hyp_pipeline_lsp_reference_view_row(arr, entries, first, i);
        if (rc->kind != HYP_RESOLVED_CALL_REFERENCE || !rc->caller_qn || !rc->callee_qn ||
            rc->confidence < HYP_LSP_CONFIDENCE_FLOOR) {
            continue;
        }
        if (strcmp(rc->caller_qn, usage->enclosing_func_qn) != 0 ||
            !hyp_pipeline_source_occurrence_eq(usage->site_start_byte, usage->site_end_byte,
                                               usage->source_origin, rc->site_start_byte,
                                               rc->site_end_byte, rc->source_origin)) {
            continue;
        }
        bool name_matches = strcmp(hyp_lsp_bare_segment(rc->callee_qn), ref_leaf) == 0;
        bool semantic_reason_matches = hyp_pipeline_reference_reason_matches(rc, ref_leaf);
        ...
        if (!hyp_pipeline_reference_targets_equal(best_exact->callee_qn, rc->callee_qn, gbuf,
                                                  project_name, allow_tail_match)) {
    ...
    const char *caller_tail = hyp_pipeline_qn_class_method_tail(usage->enclosing_func_qn);
    if (!caller_tail) {
        return NULL;
    }

    const HYPResolvedCall *best_tail = NULL;
    ...
    for (int i = 0; i < row_count; i++) {
        ...
        if (!hyp_pipeline_qn_class_method_tail_eq(rc->caller_qn, caller_tail) ||
            !hyp_pipeline_source_occurrence_eq(usage->site_start_byte, usage->site_end_byte,
                                               usage->source_origin, rc->site_start_byte,
                                               rc->site_end_byte, rc->source_origin)) {
            continue;
        ...
        if (!hyp_pipeline_reference_targets_equal(best_tail->callee_qn, rc->callee_qn, gbuf,
                                                  project_name, allow_tail_match)) {
            tail_ambiguous = true;
            continue;
        }
...
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_reference_in_graph(
    const HYPResolvedCallArray *arr, const HYPUsage *usage, bool allow_tail_match,
    const hyp_gbuf_t *gbuf, const char *project_name) {
    int count = arr ? arr->count : 0;
    return hyp_pipeline_find_lsp_reference_view_in_graph(arr, NULL, 0, count, usage,
...
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_reference_indexed_in_graph(
    const HYPResolvedCallArray *arr, const hyp_pipeline_lsp_reference_index_t *index,
    const HYPUsage *usage, bool allow_tail_match, const hyp_gbuf_t *gbuf,
    const char *project_name) {
    if (!index) {
        return hyp_pipeline_find_lsp_reference_in_graph(arr, usage, allow_tail_match, gbuf,
                                                        project_name);
    }
    ...
    while (low < high) {
        ...
        if (hyp_pipeline_lsp_reference_site_cmp_usage(index->entries[middle].row, usage) < 0) {
            low = middle + 1;
        } else {
            high = middle;
        }
    ...
    return hyp_pipeline_find_lsp_reference_view_in_graph(
...
static inline const HYPResolvedCall *hyp_pipeline_find_lsp_reference(
    const HYPResolvedCallArray *arr, const HYPUsage *usage, bool allow_tail_match) {
    return hyp_pipeline_find_lsp_reference_in_graph(arr, usage, allow_tail_match, NULL, NULL);
}
 ...
 *      hyp_pipeline_lsp_allow_tail_match), use the exact node-name index
...
static inline const hyp_gbuf_node_t *hyp_pipeline_lsp_target_node_policy(
    ...
    const hyp_gbuf_node_t *direct = hyp_gbuf_find_by_qn(gbuf, callee_qn);
    if (direct) {
        return direct;
    }
        ...
        if (!(strncmp(callee_qn, project_name, proj_len) == 0 && callee_qn[proj_len] == '.')) {
            ...
            if (proj_len <= SIZE_MAX - callee_len - 2U) {
                ...
                if (prefixed_qn) {
                    ...
                    const hyp_gbuf_node_t *prefixed = hyp_gbuf_find_by_qn(gbuf, prefixed_qn);
                    free(prefixed_qn);
                    if (prefixed) {
                        return prefixed;
                    }
    ...
    const char *callee_tail = hyp_pipeline_qn_class_method_tail(callee_qn);
    if (!callee_tail) {
        return NULL;
    }
    const hyp_gbuf_node_t **hits = NULL;
    int hit_count = 0;
    if (hyp_gbuf_find_by_name(gbuf, short_name, &hits, &hit_count) != 0 || hit_count == 0) {
        return NULL;
    }

    const hyp_gbuf_node_t *match = NULL;
    const hyp_gbuf_node_t *unique_callable = NULL;
    bool tail_ambiguous = false;
    bool callable_ambiguous = false;
    for (int i = 0; i < hit_count; i++) {
        const hyp_gbuf_node_t *cand = hits[i];
        ...
        if (!hyp_pipeline_qn_class_method_tail_eq(cand->qualified_name, callee_tail)) {
            continue;
        }
...
#endif /* HYP_PIPELINE_LSP_RESOLVE_H */
