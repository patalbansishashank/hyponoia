...
#define HYP_ARTIFACT_SCHEMA_VERSION 2

#define HYP_ARTIFACT_FILENAME "graph.db.zst"
#define HYP_ARTIFACT_META "artifact.json"
#define HYP_ARTIFACT_DIR ".hyponoia"

/* Export quality levels */
enum {
    HYP_ARTIFACT_FAST = 0, /* zstd -3, no index stripping (watcher path) */
    HYP_ARTIFACT_BEST = 1, /* zstd -9 + drop indexes + VACUUM INTO (explicit index) */
};

/* Export DB to .hyponoia/graph.db.zst artifact.
 * quality: HYP_ARTIFACT_FAST or HYP_ARTIFACT_BEST.
 * Creates .hyponoia/ dir, .gitattributes, and artifact.json.
 ...
 * command (as artifact.c does via hyp_popen). Rejects quote / backslash / shell
 * substitution metacharacters (hyp_validate_shell_arg); on Windows also rejects the
