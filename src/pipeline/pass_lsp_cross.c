 ...
 * (HYPDefinition / HYPImport / IMPORTS-edge gbuf state) into the input
 * shape each language LSP's hyp_run_X_lsp_cross expects, then merges
 * the resulting HYPResolvedCall entries back into per-file results.
 *
 * The pass is a no-op for any file whose HYPFileResult is missing or
 ...
 * extraction-side hyp_lang_module_is_dir() in internal/hyp/helpers.c so the
 * cross-file LSP caller_qn agrees with the def-node QN (the lsp_resolve join
 * keys on exact equality). */
static bool pxc_module_is_dir(HYPLanguage lang) {
...
static char *pxc_read_file(const char *path, int *out_len) {
    ...
    if (size <= 0 || size > (long)PXC_MAX_FILE_BYTES_FACTOR * (long)HYP_SZ_1K * (long)HYP_SZ_1K) {
        (void)fclose(f);
        return NULL;
    }
    /* +pad: tree-sitter lexer lookahead reads past EOF; keep it in-bounds */
    enum { HYP_TS_LOOKAHEAD_PAD = 16 };
    char *buf = (char *)malloc((size_t)size + HYP_TS_LOOKAHEAD_PAD);
    if (!buf) {
        (void)fclose(f);
        return NULL;
    }
...
/* Map a HYPDefinition.label to a HYPLSPDef.label. Per-language LSP registrars
...
static const char *pxc_map_label(const char *label) {
    if (!label)
        return NULL;
    if (hyp_label_is_type_like(label) || strcmp(label, "Protocol") == 0 ||
...
static const char *pxc_join_pipe(HYPArena *arena, const char *const *items) {
    if (!items || !items[0])
        return NULL;
    int count = 0;
    size_t total = 0;
    ...
    char *buf = (char *)hyp_arena_alloc(arena, bufsz);
    if (!buf)
        return NULL;
    char *p = buf;
...
static bool pxc_is_jvm_lang(HYPLanguage lang);
...
static const char *pxc_jvm_type_qn(HYPArena *arena, const char *namespace_name,
    ...
    return hyp_arena_sprintf(arena, "%s.%s", namespace_name, short_name);
}

static const char *pxc_jvm_def_qn(HYPArena *arena, const HYPDefinition *src,
    ...
    if (strcmp(label, "Method") == 0 || strcmp(label, "Function") == 0 ||
        ...
        if (src->parent_class && src->parent_class[0]) {
            return hyp_arena_sprintf(arena, "%s.%s.%s", namespace_name,
                                     pxc_last_component(src->parent_class), src->name);
        }
        return hyp_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
    }
    return hyp_arena_sprintf(arena, "%s.%s", namespace_name, src->name);
}

static const char *pxc_infer_jvm_namespace(HYPArena *arena, const char *rel_path,
                                           HYPLanguage lang) {
    if (!arena || !rel_path || !pxc_is_jvm_lang(lang)) {
        return NULL;
    }
    const char *root = NULL;
    const char *lang_root = lang == HYP_LANG_KOTLIN ? "kotlin/" : "java/";
    if (strncmp(rel_path, "src/main/", 9) == 0 &&
        ...
        const char *needle = lang == HYP_LANG_KOTLIN ? "/kotlin/" : "/java/";
...
/* Convert one HYPDefinition into a HYPLSPDef. Returns 0 on success, -1
...
static int pxc_build_lsp_def(HYPArena *arena, const HYPDefinition *src, const char *module_qn,
                             const char *namespace_name, HYPLanguage lang, HYPLSPDef *dst) {
...
static int pxc_build_rust_impl_relation(HYPArena *arena, const HYPImplTrait *impl,
                                        const char *project_name, const char *rel_path,
                                        const char *module_qn, HYPLSPDef *dst) {
    ...
    dst->lang = HYP_LANG_RUST;
    dst->is_rust_impl_relation = true;
    return 0;
}

/* Collect a project-wide HYPLSPDef[] from all cached results. Returns a
...
HYPLSPDef *hyp_pxc_collect_all_defs(HYPFileResult **cache, const hyp_file_info_t *files,
    ...
    for (int i = 0; i < file_count; i++) {
        if (cache[i]) {
            ...
            if (files[i].language == HYP_LANG_RUST) {
    ...
    HYPLSPDef *defs = (HYPLSPDef *)calloc((size_t)total, sizeof(HYPLSPDef));
    if (!defs) {
        *out_count = 0;
        if (out_def_starts) {
            memset(out_def_starts, 0, (size_t)(file_count + 1) * sizeof(int));
    ...
    for (int fi = 0; fi < file_count; fi++) {
        ...
        if (!cache[fi])
            continue;
        if (!def_modules[fi]) {
            def_modules[fi] = hyp_pipeline_fqn_module_dir(project_name, files[fi].rel_path,
        ...
        if (files[fi].language == HYP_LANG_RUST) {
...
static const char *pxc_unique_import_path(const HYPFileResult *result, const char *local_name,
                                          bool *out_ambiguous) {
    if (out_ambiguous)
        *out_ambiguous = false;
...
static char *pxc_kotlin_import_from_metadata(const HYPFileResult *result, const char *local_name) {
...
static char *pxc_import_value_qn(HYPLanguage lang, const HYPFileResult *result,
                                 const char *local_name, const hyp_gbuf_node_t *target) {
...
static char *pxc_python_import_from_metadata(const hyp_gbuf_t *gbuf, const char *project_name,
    ...
    const hyp_gbuf_node_t *exact =
        hyp_pipeline_lsp_target_node(gbuf, project_name, module_path, false);
    ...
    if (last_dot && last_dot > module_path) {
        ...
        module[module_len] = '\0';
        const hyp_gbuf_node_t *module_node =
            hyp_pipeline_lsp_target_node(gbuf, project_name, module, false);
        free(module);
...
int hyp_pxc_build_import_map(const hyp_gbuf_t *gbuf, const char *project_name, const char *rel_path,
                             HYPLanguage lang, const HYPFileResult *result, const char ***out_keys,
    ...
    *out_count = 0;
    ...
    int edge_count = 0;
    char *file_qn = hyp_pipeline_fqn_compute(project_name, rel_path, "__file__");
    const hyp_gbuf_node_t *file_node = file_qn ? hyp_gbuf_find_by_qn(gbuf, file_qn) : NULL;
    free(file_qn);
    if (file_node && hyp_gbuf_find_edges_by_source_type(gbuf, file_node->id, "IMPORTS", &edges,
                                                        &edge_count) != 0) {
        edges = NULL;
        edge_count = 0;
    }
    ...
    int metadata_count =
        (lang == HYP_LANG_PYTHON || lang == HYP_LANG_KOTLIN) && result ? result->imports.count : 0;
    ...
    for (int i = 0; i < edge_count; i++) {
        const hyp_gbuf_edge_t *e = edges[i];
        const hyp_gbuf_node_t *target = hyp_gbuf_find_by_id(gbuf, e->target_id);
...
void hyp_pxc_free_import_map(const char **keys, const char **vals, int count) {
    if (keys) {
        for (int i = 0; i < count; i++)
            free((void *)keys[i]);
        free((void *)keys);
...
void hyp_pxc_ts_modes(HYPLanguage lang, const char *rel_path, bool *out_js, bool *out_jsx,
                      bool *out_dts) {
    *out_js = (lang == HYP_LANG_JAVASCRIPT);
    *out_jsx = (lang == HYP_LANG_TSX);
    *out_dts = false;
    if (!rel_path)
        return;
    size_t rl = strlen(rel_path);
    if (lang == HYP_LANG_JAVASCRIPT && rl >= 4 && strcmp(rel_path + rl - 4, ".jsx") == 0) {
        *out_jsx = true;
    }
    if (lang == HYP_LANG_TYPESCRIPT && rl >= 5 && strcmp(rel_path + rl - 5, ".d.ts") == 0) {
        *out_dts = true;
    }
}

/* Returns true when this language has a cross-file LSP wired up. */
bool hyp_pxc_has_cross_lsp(HYPLanguage lang) {
    switch (lang) {
    case HYP_LANG_GO:
    case HYP_LANG_C:
    case HYP_LANG_CPP:
    ...
    case HYP_LANG_TSX:
    case HYP_LANG_PHP:
    case HYP_LANG_CSHARP: /* tier-2 prebuilt registry path (pass_parallel.c) */
    case HYP_LANG_JAVA:   /* fallback hyp_pxc_run_one path */
    case HYP_LANG_KOTLIN: /* fallback hyp_pxc_run_one path */
    case HYP_LANG_RUST:   /* fallback hyp_pxc_run_one path (manifest-aware) */
        return true;
    default:
        return false;
    }
...
static void pxc_append_results(HYPArena *dst_arena, HYPResolvedCallArray *dst_calls,
                               const HYPResolvedCallArray *src_out) {
    if (!dst_calls || !src_out)
        return;

    HYPArena keys;
    hyp_arena_init(&keys);
    HYPHashTable *seen = hyp_ht_create((uint32_t)(dst_calls->count + src_out->count + 1));

    for (int i = 0; i < dst_calls->count; i++) {
        const HYPResolvedCall *rc = &dst_calls->items[i];
        if (rc->caller_qn && rc->callee_qn) {
            char *k = hyp_arena_sprintf(&keys, "%u\x1f%s\x1f%s\x1f%u:%u\x1f%u", (unsigned)rc->kind,
                                        rc->caller_qn, rc->callee_qn, rc->site_start_byte,
                                        rc->site_end_byte, (unsigned)rc->source_origin);
            if (k) {
                void *prior = hyp_ht_get(seen, k);
                if (!prior ||
                    rc->confidence > dst_calls->items[(int)(uintptr_t)prior - 1].confidence) {
                    const char *stored = hyp_ht_get_key(seen, k);
                    hyp_ht_set(seen, stored ? stored : k, (void *)(uintptr_t)(i + 1));
                }
            }
        }
    }
    ...
    for (int j = 0; j < src_out->count; j++) {
        const HYPResolvedCall *src = &src_out->items[j];
        if (!src->caller_qn || !src->callee_qn)
            continue;
        char *k = hyp_arena_sprintf(&keys, "%u\x1f%s\x1f%s\x1f%u:%u\x1f%u", (unsigned)src->kind,
                                    src->caller_qn, src->callee_qn, src->site_start_byte,
                                    src->site_end_byte, (unsigned)src->source_origin);
        void *prior = k ? hyp_ht_get(seen, k) : NULL;
        if (prior) {
            HYPResolvedCall *dst = &dst_calls->items[(int)(uintptr_t)prior - 1];
            if (src->confidence > dst->confidence) {
                dst->caller_qn = hyp_arena_strdup(dst_arena, src->caller_qn);
                dst->callee_qn = hyp_arena_strdup(dst_arena, src->callee_qn);
                dst->strategy = src->strategy ? hyp_arena_strdup(dst_arena, src->strategy) : NULL;
                dst->confidence = src->confidence;
                dst->reason = src->reason ? hyp_arena_strdup(dst_arena, src->reason) : NULL;
                dst->kind = src->kind;
        ...
        dst.site_end_byte = src->site_end_byte;
        dst.source_origin = src->source_origin;
        hyp_resolvedcall_push(dst_calls, dst_arena, dst);
        if (k) {
            hyp_ht_set(seen, k, (void *)(uintptr_t)dst_calls->count);
        }
    }

    hyp_ht_free(seen);
    hyp_arena_destroy(&keys);
}

/* Merge exact synthetic call carriers produced by a cross-LSP resolver. The
 * source may live in the per-file result arena or in hyp_pxc_run_one's scratch
...
static void pxc_append_synthetic_calls(HYPArena *dst_arena, HYPCallArray *dst_calls,
                                       const HYPCallArray *src_calls) {
    if (!dst_arena || !dst_calls || !src_calls || src_calls->count <= 0)
        return;

    HYPArena keys;
    hyp_arena_init(&keys);
    HYPHashTable *seen = hyp_ht_create((uint32_t)(dst_calls->count + src_calls->count + 1));

    for (int i = 0; i < dst_calls->count; i++) {
        const HYPCall *call = &dst_calls->items[i];
        if (!call->callee_name || !call->enclosing_func_qn ||
            call->site_end_byte <= call->site_start_byte) {
            continue;
        }
        char *key = hyp_arena_sprintf(
        ...
        if (key && !hyp_ht_get(seen, key))
            hyp_ht_set(seen, key, (void *)(uintptr_t)(i + 1));
    }

    for (int i = 0; i < src_calls->count; i++) {
        const HYPCall *src = &src_calls->items[i];
        ...
        HYPCall dst = *src;
        dst.callee_name = hyp_arena_strdup(dst_arena, src->callee_name);
        dst.enclosing_func_qn = hyp_arena_strdup(dst_arena, src->enclosing_func_qn);
        dst.first_string_arg =
            src->first_string_arg ? hyp_arena_strdup(dst_arena, src->first_string_arg) : NULL;
        dst.second_arg_name =
            src->second_arg_name ? hyp_arena_strdup(dst_arena, src->second_arg_name) : NULL;
        for (int ai = 0; ai < HYP_MAX_CALL_ARGS; ai++) {
            dst.args[ai].expr =
                src->args[ai].expr ? hyp_arena_strdup(dst_arena, src->args[ai].expr) : NULL;
            dst.args[ai].value =
                src->args[ai].value ? hyp_arena_strdup(dst_arena, src->args[ai].value) : NULL;
            dst.args[ai].keyword =
                src->args[ai].keyword ? hyp_arena_strdup(dst_arena, src->args[ai].keyword) : NULL;
        }
        hyp_calls_push(dst_calls, dst_arena, dst);
        if (key)
            hyp_ht_set(seen, key, (void *)(uintptr_t)dst_calls->count);
    }
 ...
 *
 * hyp_pxc_run_one's signature is shared with the parallel pass
 ...
 * sequential driver (hyp_pipeline_pass_lsp_cross, below) sets up once per
...
static _Thread_local const HYPCargoManifest *g_pxc_rust_manifest = NULL;

void hyp_pxc_set_rust_manifest(const HYPCargoManifest *m) {
    g_pxc_rust_manifest = m;
}

const struct HYPCargoManifest *hyp_pxc_get_rust_manifest(void) {
    return g_pxc_rust_manifest;
}

/* Convert a HYPLSPDef array (the pipeline's lingua franca, go_lsp.h:73)
 * into a HYPRustLSPDef array (rust_lsp.h) inside `arena`. The two structs
 * have similar fields but different layouts; HYPLSPDef adds
 * language/namespace metadata, so a memcpy is unsafe — copy field-by-field. */
static HYPRustLSPDef *pxc_lspdefs_to_rust(HYPArena *arena, const HYPLSPDef *defs, int def_count) {
    if (!defs || def_count <= 0)
        return NULL;
    HYPRustLSPDef *out =
        (HYPRustLSPDef *)hyp_arena_alloc(arena, (size_t)def_count * sizeof(HYPRustLSPDef));
    if (!out)
        return NULL;
...
void hyp_pxc_run_one(HYPLanguage lang, HYPFileResult *r, const char *source, int source_len,
                     const char *module_qn, HYPLSPDef *defs, int def_count, const char **imp_names,
                     const char **imp_qns, int imp_count) {
    TSTree *tree = r->cached_tree; /* may be NULL — LSP re-parses then */

    HYPArena scratch;
    hyp_arena_init(&scratch);
    ...
    switch (lang) {
    case HYP_LANG_GO:
        hyp_run_go_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                             imp_qns, imp_count, tree, &out);
        break;
    case HYP_LANG_C:
    case HYP_LANG_CPP:
    case HYP_LANG_CUDA: {
        bool cpp_mode = (lang != HYP_LANG_C);
        ...
        hyp_run_c_lsp_cross(&scratch, source, source_len, module_qn, cpp_mode, defs, def_count,
                            NULL, NULL, 0, tree, &out);
        break;
    }
    case HYP_LANG_PYTHON:
        hyp_run_py_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                             imp_qns, imp_count, tree, &out, &synthetic_calls);
        break;
    case HYP_LANG_PHP:
        hyp_run_php_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                              imp_qns, imp_count, tree, &out);
        break;
    case HYP_LANG_JAVA:
        hyp_run_java_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count, imp_names,
                               imp_qns, imp_count, tree, &out);
        break;
    case HYP_LANG_KOTLIN:
        hyp_run_kotlin_lsp_cross(&scratch, source, source_len, module_qn, defs, def_count,
                                 imp_names, imp_qns, imp_count, tree, &out);
        break;
    case HYP_LANG_RUST: {
        /* The Rust resolver wants HYPRustLSPDef (rust_lsp.h), not the
         * pipeline's HYPLSPDef — the structs share their first 9 fields
        ...
        HYPRustLSPDef *rdefs = pxc_lspdefs_to_rust(&scratch, defs, def_count);
        hyp_run_rust_lsp_cross_with_manifest(&scratch, source, source_len, module_qn, rdefs,
    ...
    hyp_arena_destroy(&scratch);
}

/* Variant of hyp_pxc_run_one for TS/JS/JSX/TSX with explicit dialect
 * flags. Same scratch-arena lifecycle as hyp_pxc_run_one. */
void hyp_pxc_run_one_ts(HYPFileResult *r, const char *source, int source_len, const char *module_qn,
                        HYPLSPDef *defs, int def_count, const char **imp_names,
    ...
    hyp_run_ts_lsp_cross(&scratch, source, source_len, module_qn, js_mode, jsx_mode, dts_mode, defs,
...
void hyp_pxc_dispatch_file(HYPLanguage lang, HYPFileResult *result, const char *source,
                           int source_len, const char *rel, const char *def_module,
                           const HYPCrossLspRegistries *cross_registries,
                           const HYPModuleDefIndex *module_def_index, HYPLSPDef *all_defs,
                           int all_def_count, const char **imp_keys, const char **imp_vals,
                           int imp_count, HYPTypeRegistry *(*rust_shared_get)(void *),
                           void *rust_shared_ctx) {
    if (!result) {
        return;
    }
    bool used_prebuilt = false;
    HYPTypeRegistry *prebuilt =
        cross_registries ? hyp_pxc_registry_for_lang(cross_registries, lang) : NULL;
    if (prebuilt) {
        switch (lang) {
        case HYP_LANG_GO:
            /* Tier 3 (metadata-driven): pure lookup over the Tier-1
             * lsp_unresolved entries — no parse, no AST walk. */
            hyp_go_fast_resolve_qualified_calls(result, prebuilt, imp_keys, imp_vals, imp_count);
            used_prebuilt = true;
            break;
        case HYP_LANG_PYTHON: {
            HYPArena scratch;
            ...
            HYPCallArray synthetic_calls = {0};
            hyp_run_py_lsp_cross_with_registry(&scratch, source, source_len, def_module, prebuilt,
        ...
        case HYP_LANG_CPP:
        case HYP_LANG_CUDA:
            hyp_run_c_lsp_cross_with_registry(
                &result->arena, source, source_len, def_module, (lang != HYP_LANG_C), prebuilt,
                imp_keys, imp_vals, imp_count, result->cached_tree, &result->resolved_calls);
            used_prebuilt = true;
            break;
        case HYP_LANG_CSHARP:
            hyp_run_cs_lsp_cross_with_registry(&result->arena, source, source_len, def_module,
        ...
        case HYP_LANG_TSX: {
            ...
            bool jsx;
            bool dts;
            hyp_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
            HYPLSPDef *ts_defs = all_defs;
            int ts_def_count = all_def_count;
            HYPLSPDef *ts_filtered = NULL;
            if (module_def_index) {
                int fc = 0;
                bool filter_succeeded = false;
                ts_filtered = hyp_pxc_filter_defs_for_file(
            ...
            hyp_run_ts_lsp_cross_with_registry(&result->arena, source, source_len, def_module, js,
    ...
    HYPLSPDef *filtered = NULL;
    HYPLSPDef *file_defs = all_defs;
    int file_def_count = all_def_count;
    if (module_def_index && lang != HYP_LANG_RUST) {
        int filtered_count = 0;
        bool filter_succeeded = false;
        filtered = hyp_pxc_filter_defs_for_file(module_def_index, all_defs, lang,
    ...
    if (lang == HYP_LANG_RUST) {
        HYPTypeRegistry *shared = rust_shared_get ? rust_shared_get(rust_shared_ctx) : NULL;
        if (shared) {
            HYPArena scratch;
            hyp_arena_init(&scratch);
            HYPResolvedCallArray out = {0};
            HYPCallArray synthetic_calls = {0};
            hyp_run_rust_lsp_cross_with_registry(
                &scratch, source, source_len, def_module, shared, imp_keys, imp_vals, imp_count,
                result->cached_tree, hyp_pxc_get_rust_manifest(), &out, &synthetic_calls);
            pxc_append_results(&result->arena, &result->resolved_calls, &out);
            pxc_append_synthetic_calls(&result->arena, &result->calls, &synthetic_calls);
            hyp_arena_destroy(&scratch);
        } else {
            hyp_pxc_run_one(lang, result, source, source_len, def_module, file_defs, file_def_count,
                            imp_keys, imp_vals, imp_count);
        }
    } else if (lang == HYP_LANG_JAVASCRIPT || lang == HYP_LANG_TYPESCRIPT || lang == HYP_LANG_TSX) {
        bool js;
        bool jsx;
        bool dts;
        hyp_pxc_ts_modes(lang, rel, &js, &jsx, &dts);
        hyp_pxc_run_one_ts(result, source, source_len, def_module, file_defs, file_def_count,
...
static bool pxc_build_rust_manifest(const hyp_pipeline_ctx_t *ctx, HYPArena *marena,
                                    HYPCargoManifest *out_m) {
    ...
    hyp_cargo_parse(marena, toml, toml_len, out_m);
...
int hyp_pipeline_pass_lsp_cross(hyp_pipeline_ctx_t *ctx, const hyp_file_info_t *files,
    ...
    hyp_log_info("pass.start", "pass", "lsp_cross", "files", itoa_buf(file_count));
    ...
    for (int i = 0; i < file_count; i++) {
        if (cache[i] && files[i].language == HYP_LANG_RUST) {
            have_rust = true;
            break;
        }
    }
    HYPArena cargo_arena;
    HYPCargoManifest cargo_manifest;
    bool have_manifest = false;
    if (have_rust) {
        hyp_arena_init(&cargo_arena);
        have_manifest = pxc_build_rust_manifest(ctx, &cargo_arena, &cargo_manifest);
        hyp_pxc_set_rust_manifest(have_manifest ? &cargo_manifest : NULL);
    }

    /* Per-file module QN cache so we don't recompute it once per def + once
     * per call. hyp_pipeline_fqn_module mallocs; freed at end. */
    char **def_modules = (char **)calloc((size_t)file_count, sizeof(char *));
    if (!def_modules) {
        hyp_log_error("pass.err", "pass", "lsp_cross", "phase", "alloc");
        return 0;
    }

    int def_count = 0;
    int *def_starts = (int *)calloc((size_t)file_count + 1, sizeof(int));
    HYPLSPDef *all_defs = hyp_pxc_collect_all_defs(cache, files, file_count, ctx->project_name,
    ...
    if (all_defs) {
        HYPArena *xa = &ctx->seq_cross_arena;
        if (!ctx->seq_cross_arena_live) {
            hyp_arena_init(xa);
            ctx->seq_cross_arena_live = true;
        }
        cross_registries.go = hyp_go_build_cross_registry(xa, all_defs, def_count);
        cross_registries.python = hyp_py_build_cross_registry(xa, all_defs, def_count);
        cross_registries.c = hyp_c_build_cross_registry(xa, all_defs, def_count);
        cross_registries.cs = hyp_cs_build_cross_registry(xa, all_defs, def_count);
        cross_registries.ts = hyp_ts_build_cross_registry(xa, all_defs, def_count);
    }
    ...
    for (int i = 0; i < file_count; i++) {
        ...
        if (!def_modules[i]) {
            def_modules[i] = hyp_pipeline_fqn_module_dir(ctx->project_name, files[i].rel_path,
        ...
        int imp_count = 0;
        hyp_pxc_build_import_map(ctx->gbuf, ctx->project_name, files[i].rel_path, lang, cache[i],
                                 &imp_keys, &imp_vals, &imp_count);
         ...
         * failure mode). */
        hyp_index_mark_start(files[i].rel_path);
        hyp_pxc_dispatch_file(lang, cache[i], source, source_len, files[i].rel_path, def_modules[i],
                              &cross_registries, module_def_index, all_defs, def_count, imp_keys,
                              imp_vals, imp_count, NULL, NULL);
        hyp_index_mark_done(files[i].rel_path);
        per_lang_calls++;
        processed++;
    ...
    if (have_rust) {
        hyp_pxc_set_rust_manifest(NULL);
        hyp_arena_destroy(&cargo_arena);
    }
    (void)have_manifest;

    hyp_log_info("pass.done", "pass", "lsp_cross", "files_processed", itoa_buf(processed),
...
struct HYPModuleDefIndex {
    HYPHashTable *ht;           /* module_qn → pxc_module_entry_t* */
    HYPHashTable *namespace_ht; /* declared package/namespace → pxc_module_entry_t* */
    int def_count;              /* total entries in the all_defs[] array */
};

/* hyp_ht_foreach callback: free each pxc_module_entry_t. */
...
static pxc_module_entry_t *pxc_module_entry_get_or_create(HYPHashTable *ht, const char *key) {
    if (!ht || !key || !key[0]) {
        return NULL;
    }
    pxc_module_entry_t *e = (pxc_module_entry_t *)hyp_ht_get(ht, key);
    if (e) {
        return e;
    }
...
static bool pxc_is_jvm_lang(HYPLanguage lang);
static bool pxc_def_lang_matches(HYPLanguage caller_lang, HYPLanguage def_lang);

static int pxc_mark_entry_defs(bool *selected, const pxc_module_entry_t *e,
                               const HYPLSPDef *all_defs, HYPLanguage caller_lang) {
    if (!selected || !e) {
        return 0;
    }
    int added = 0;
    for (int j = 0; j < e->count; j++) {
        int idx = e->indices[j];
        const HYPLSPDef *def = &all_defs[idx];
...
static bool pxc_is_jvm_lang(HYPLanguage lang) {
    return lang == HYP_LANG_JAVA || lang == HYP_LANG_KOTLIN;
}
...
static void pxc_mark_module_defs(const HYPModuleDefIndex *idx, bool *selected,
                                 const HYPLSPDef *all_defs, HYPLanguage caller_lang,
                                 const char *module_qn, int *total) {
    if (!idx || !idx->ht || !module_qn || !module_qn[0]) {
        return;
    }
    pxc_module_entry_t *e = (pxc_module_entry_t *)hyp_ht_get(idx->ht, module_qn);
...
static void pxc_mark_import_defs(const HYPModuleDefIndex *idx, bool *selected,
                                 const HYPLSPDef *all_defs, HYPLanguage caller_lang,
    ...
    bool matched = false;
    for (;;) {
        pxc_module_entry_t *entry = (pxc_module_entry_t *)hyp_ht_get(idx->ht, candidate);
        if (entry) {
    ...
    if (!matched && pxc_is_jvm_lang(caller_lang) && idx->namespace_ht) {
        ...
        for (;;) {
            pxc_module_entry_t *entry = (pxc_module_entry_t *)hyp_ht_get(
...
HYPModuleDefIndex *hyp_pxc_build_module_def_index(HYPLSPDef *all_defs, int def_count) {
    if (!all_defs || def_count <= 0) {
        return NULL;
    }

    HYPHashTable *ht = hyp_ht_create(64);
    HYPHashTable *namespace_ht = hyp_ht_create(64);
    if (!ht || !namespace_ht) {
        hyp_ht_free(ht);
        hyp_ht_free(namespace_ht);
        return NULL;
    }
    ...
    HYPModuleDefIndex *idx = (HYPModuleDefIndex *)calloc(1, sizeof(*idx));
    if (!idx) {
        hyp_ht_foreach(ht, pxc_module_entry_free_cb, NULL);
        hyp_ht_free(ht);
        hyp_ht_foreach(namespace_ht, pxc_module_entry_free_cb, NULL);
        hyp_ht_free(namespace_ht);
        return NULL;
    }
    idx->ht = ht;
...
void hyp_pxc_free_module_def_index(HYPModuleDefIndex *idx) {
    if (!idx) {
        return;
    }
    if (idx->ht) {
        hyp_ht_foreach(idx->ht, pxc_module_entry_free_cb, NULL);
        hyp_ht_free(idx->ht);
    }
    if (idx->namespace_ht) {
        hyp_ht_foreach(idx->namespace_ht, pxc_module_entry_free_cb, NULL);
        hyp_ht_free(idx->namespace_ht);
    }
    free(idx);
}

HYPLSPDef *hyp_pxc_filter_defs_for_file(const HYPModuleDefIndex *idx, HYPLSPDef *all_defs,
                                        HYPLanguage caller_lang, const char *caller_namespace,
    ...
    if (pxc_is_jvm_lang(caller_lang) && idx->namespace_ht) {
        ...
        pxc_module_entry_t *e = (pxc_module_entry_t *)hyp_ht_get(idx->namespace_ht, namespace_key);
    ...
    HYPLSPDef *out = (HYPLSPDef *)malloc((size_t)total * sizeof(HYPLSPDef));
    if (!out) {
        free(selected);
        return NULL;
    }
