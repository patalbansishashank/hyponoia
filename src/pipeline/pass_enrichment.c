...
int hyp_split_camel_case(const char *s, char **out, int max_out) {
    ...
    for (size_t i = SKIP_ONE; i < len; i++) {
        if (s[i] >= 'A' && s[i] <= 'Z' && s[i - SKIP_ONE] >= 'a' && s[i - SKIP_ONE] <= 'z') {
            if (count < max_out) {
                out[count] = hyp_strndup(s + start, i - start);
                count++;
            }
            start = i;
        }
    ...
    /* Emit remaining */
    if (count < max_out) {
        out[count] = hyp_strndup(s + start, len - start);
        count++;
    }
    return count;
}
...
int hyp_tokenize_decorator(const char *dec, char **out, int max_out) {
    ...
    while (part && count < max_out) {
        ...
        int camel_count = hyp_split_camel_case(part, camel_parts, ENRICH_MAX_CAMEL);
...
static int extract_decorator_words(const char *json, char ***out_words) {
    ...
    char *all_words[HYP_SZ_256];
    int total = 0;
    HYPHashTable *seen = hyp_ht_create(HYP_SZ_32);

    for (int i = 0; decorators[i]; i++) {
        char *tokens[HYP_SZ_32];
        int tc = hyp_tokenize_decorator(decorators[i], tokens, HYP_SZ_32);
        for (int j = 0; j < tc; j++) {
            if (!hyp_ht_get(seen, tokens[j]) && total < HYP_SZ_256) {
                hyp_ht_set(seen, tokens[j], intptr_to_ptr(SKIP_ONE));
...
static int collect_decorated_nodes(hyp_gbuf_t *gbuf, tagged_node_t **out_nodes,
                                   HYPHashTable *word_counts) {
    ...
    for (int l = 0; l < nlabels; l++) {
        ...
        int fc = 0;
        hyp_gbuf_find_by_label(gbuf, labels[l], &found, &fc);
        if (!found || fc <= 0) {
            continue;
        }
        ...
        for (int i = 0; i < fc; i++) {
            ...
            if (node_count >= node_cap) {
                node_cap = node_cap ? node_cap * PAIR_LEN : HYP_SZ_64;
            ...
            tn->word_count = wc;
            for (int w = 0; w < wc; w++) {
                intptr_t cnt = (intptr_t)hyp_ht_get(word_counts, words[w]);
                hyp_ht_set(word_counts, words[w], intptr_to_ptr(cnt + SKIP_ONE));
            }
        }
    }
...
static int apply_decorator_tags(hyp_gbuf_t *gbuf, tagged_node_t *nodes, int node_count,
                                HYPHashTable *candidates) {
    int tagged = 0;
    for (int n = 0; n < node_count; n++) {
        char *tag_words[HYP_SZ_256];
        int tag_count = 0;
        for (int w = 0; w < nodes[n].word_count; w++) {
            if (hyp_ht_get(candidates, nodes[n].words[w]) && tag_count < HYP_SZ_256) {
        ...
        const hyp_gbuf_node_t *gn = hyp_gbuf_find_by_qn(gbuf, nodes[n].qualified_name);
        if (!gn) {
            continue;
        }
        ...
        if (new_props) {
            hyp_gbuf_upsert_node(gbuf, gn->label, gn->name, gn->qualified_name, gn->file_path,
...
int hyp_pipeline_pass_decorator_tags(hyp_gbuf_t *gbuf, const char *project) {
    ...
    HYPHashTable *word_counts = hyp_ht_create(HYP_SZ_128);
    tagged_node_t *nodes = NULL;
    int node_count = collect_decorated_nodes(gbuf, &nodes, word_counts);
    if (node_count == 0) {
        hyp_ht_free(word_counts);
        free(nodes);
        return 0;
    }

    /* Phase 2: Determine candidates (words on 2+ nodes) */
    HYPHashTable *candidates = hyp_ht_create(HYP_SZ_64);
    int candidate_count = 0;
    for (int n = 0; n < node_count; n++) {
        for (int w = 0; w < nodes[n].word_count; w++) {
            const char *word = nodes[n].words[w];
            intptr_t cnt = (intptr_t)hyp_ht_get(word_counts, word);
            if (cnt >= PAIR_LEN && !hyp_ht_get(candidates, word)) {
                hyp_ht_set(candidates, word, intptr_to_ptr(SKIP_ONE));
                candidate_count++;
            }
        }
    }
    ...
    hyp_ht_free(word_counts);
    hyp_ht_free(candidates);

    hyp_log_info("pass.decorator_tags", "candidates", candidate_count > 0 ? "yes" : "0", "tagged",
                 tagged > 0 ? "yes" : "0");
    return tagged;
}
