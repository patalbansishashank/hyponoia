 ...
 *     values in the HYPLSPDef contract — receiver_type NULL means "not a
 ...
 * carry into the HYPLSPDef set. Their (name, qn, label) triple must still
...
static char *surface_file_to_json(const HYPFileResult *result, const HYPLSPDef *defs,
                                  int def_count) {
    ...
    if (result) {
        for (int i = 0; i < result->defs.count; i++) {
            const HYPDefinition *d = &result->defs.items[i];
...
int hyp_lsp_surface_build_rows(const char *project, HYPFileResult **cache,
                               const hyp_file_info_t *files, int file_count,
                               const HYPLSPDef *all_defs, const int *def_starts,
    ...
    *out_count = 0;
    ...
    hyp_lsp_surface_row_t *rows = calloc((size_t)file_count, sizeof(*rows));
    if (!rows) {
        return -1;
    }
    int n = 0;
    for (int i = 0; i < file_count; i++) {
        ...
        hyp_sha256_hex(json, strlen(json), sha);
...
static const char *arena_str_or_null(HYPArena *arena, yyjson_val *v) {
    if (!v || yyjson_is_null(v)) {
        return NULL;
    }
    const char *s = yyjson_get_str(v);
    return s ? hyp_arena_strdup(arena, s) : NULL;
}

static const char **arena_str_array(HYPArena *arena, yyjson_val *arr, bool null_terminated,
                                    int *out_count) {
    *out_count = 0;
    ...
    const char **items =
        hyp_arena_alloc(arena, (size_t)(count + (null_terminated ? 1 : 0)) * sizeof(char *));
    if (!items) {
        return NULL;
    }
    for (int i = 0; i < count; i++) {
        const char *s = yyjson_get_str(yyjson_arr_get(arr, (size_t)i));
        items[i] = s ? hyp_arena_strdup(arena, s) : "?";
    }
    if (null_terminated) {
        items[count] = NULL;
    }
...
int hyp_lsp_surface_defs_from_json(HYPArena *arena, const char *defs_json, HYPLSPDef **out_defs) {
    *out_defs = NULL;
    if (!defs_json) {
        return -1;
    }
    ...
    HYPLSPDef *defs = hyp_arena_alloc(arena, (size_t)count * sizeof(HYPLSPDef));
    if (!defs) {
        yyjson_doc_free(doc);
        return -1;
    }
    memset(defs, 0, (size_t)count * sizeof(HYPLSPDef));
    for (int i = 0; i < count; i++) {
        ...
        d->lang = (HYPLanguage)yyjson_get_int(yyjson_obj_get(o, "lg"));
