 ...
 * it: the HYPLSPDefs pass_lsp_cross registers into the cross registries,
 ...
 */
#ifndef HYP_PIPELINE_LSP_SURFACE_H
#define HYP_PIPELINE_LSP_SURFACE_H
 ...
 * is the (file_count + 1)-entry prefix array from hyp_pxc_collect_all_defs:
 * all_defs[def_starts[i] .. def_starts[i+1]) are file i's defs. Rows carry
 * heap strings; release with hyp_store_free_lsp_surfaces. Files with an
...
/* Decode one row's defs_json back into the HYPLSPDef array pass_lsp_cross
...
#endif /* HYP_PIPELINE_LSP_SURFACE_H */
