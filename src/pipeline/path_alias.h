#ifndef HYP_PATH_ALIAS_H
#define HYP_PATH_ALIAS_H
 ...
 *
 * Lookup is two-step:
 *   1. hyp_path_alias_find_for_file picks the nearest ancestor scope for
 *      a given source file (longest matching dir_prefix wins).
 *   2. hyp_path_alias_resolve runs the chosen scope's alias map against
...
typedef struct {
    ...
    hyp_path_alias_map_t *map;
} hyp_path_alias_scope_t;
 ...
 * linear scan. */
typedef struct {
    hyp_path_alias_scope_t *scopes;
    int count;
} hyp_path_alias_collection_t;
 ...
 * hyp_path_alias_collection_free. */
...
/* Free a collection produced by hyp_load_path_aliases. NULL-safe. */
