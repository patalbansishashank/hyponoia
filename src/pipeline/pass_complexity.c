...
enum { HYP_TLD_MAX_DEPTH = 256 }; /* recursion-depth cap (cycle/stack guard) */
...
static int json_get_int(const char *json, const char *key, int dflt) {
    ...
    return (int)strtol(p, NULL, HYP_DECIMAL_BASE);
}
...
static void append_complexity_props(hyp_gbuf_node_t *node, int tld, bool recursive) {
    ...
    bool empty = (olen == 2); /* "{}" */
    char *neu = malloc(olen + HYP_SZ_64);
    if (!neu) {
        return;
    }
    memcpy(neu, old, olen - 1); /* copy without trailing '}' */
    int w =
        snprintf(neu + (olen - 1), HYP_SZ_64, "%s\"transitive_loop_depth\":%d,\"recursive\":%s}",
...
static int tld_dfs(const hyp_gbuf_t *gb, int64_t id, const int *loop_depth, int *tld, char *state,
    ...
    int ne = 0;
    hyp_gbuf_find_edges_by_source_type(gb, id, "CALLS", &edges, &ne);
    for (int i = 0; i < ne; i++) {
...
static void seed_loop_depths(const hyp_gbuf_t *gb, const char *label, int *loop_depth,
                             bool *recursive, hyp_gbuf_node_t **nptr, int64_t maxid) {
    const hyp_gbuf_node_t **nodes = NULL;
    int count = 0;
    if (hyp_gbuf_find_by_label(gb, label, &nodes, &count) != 0) {
        return;
    }
...
void hyp_pipeline_pass_complexity(hyp_pipeline_ctx_t *ctx) {
    ...
    int64_t maxid = hyp_gbuf_next_id(gb) - 1;
    if (maxid < 1) {
        return;
    }
    ...
    hyp_gbuf_node_t **nptr = calloc(sz, sizeof(hyp_gbuf_node_t *));
    ...
    hyp_log_info("pass.complexity", "functions", itoa_cx(updated));
