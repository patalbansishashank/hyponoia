...
void hyp_parse_range(const char *s, int *out_start, int *out_count) {
    ...
    *out_count = SKIP_ONE;
    ...
    if (!comma) {
        *out_start = (int)strtol(s, NULL, HYP_DECIMAL_BASE);
        return;
    }
    ...
    memcpy(buf, s, len);
    buf[len] = '\0';
    *out_start = (int)strtol(buf, NULL, HYP_DECIMAL_BASE);

    /* Parse count */
    *out_count = (int)strtol(comma + SKIP_ONE, NULL, HYP_DECIMAL_BASE);
}

#define HUNK_LINE_BUF 1536

/* Parse a single name-status line into a hyp_changed_file_t.
 * Returns true if a valid entry was produced. */
static bool parse_one_name_status(const char *line, size_t line_len, hyp_changed_file_t *out_f) {
    ...
    *tab1 = '\0';
    ...
    return hyp_is_trackable_file(out_f->path);
}
...
static bool parse_hunk_line(const char *line, size_t line_len, const char *current_file,
    ...
    int start;
    int cnt;
    hyp_parse_range(range_str, &start, &cnt);

    if (start <= 0 || !hyp_is_trackable_file(current_file)) {
        return false;
    }
...
int hyp_parse_hunks(const char *output, hyp_changed_hunk_t *out, int max_out) {
    ...
    int count = 0;
    char current_file[HYP_SZ_512] = {0};
    const char *line = output;
