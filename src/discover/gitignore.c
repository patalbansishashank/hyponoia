 ...
 * number of groups. Patterns come from a committed .gitignore / .hypignore and
...
static void gi_add_pattern(hyp_gitignore_t *gi, const char *line, int len) {
...
hyp_gitignore_t *hyp_gitignore_parse(const char *content) {
    ...
    hyp_gitignore_t *gi = calloc(HYP_ALLOC_ONE, sizeof(hyp_gitignore_t));
    if (!gi) {
        return NULL;
    }
...
hyp_gitignore_t *hyp_gitignore_load(const char *path) {
    ...
    hyp_gitignore_t *gi = hyp_gitignore_parse(buf);
    free(buf);
    return gi;
}
...
bool hyp_gitignore_matches(const hyp_gitignore_t *gi, const char *rel_path, bool is_dir) {
    return hyp_gitignore_match_result(gi, rel_path, is_dir) > 0;
}
...
char *(*hyp_gitignore_merge_dup_hook_for_test)(const char *) = NULL;
...
bool hyp_gitignore_merge(hyp_gitignore_t *dst, const hyp_gitignore_t *src) {
    ...
    for (int i = 0; i < src->count; i++) {
        char *pat = hyp_gitignore_merge_dup_hook_for_test
                        ? hyp_gitignore_merge_dup_hook_for_test(src->patterns[i].pattern)
