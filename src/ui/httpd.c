...
#include <winsock2.h>
#include <ws2tcpip.h>
typedef SOCKET hyp_sock_t;
#define HYP_SOCK_BAD INVALID_SOCKET
#define hyp_sock_close closesocket
#else
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
...
#include <time.h>
#include <unistd.h>
typedef int hyp_sock_t;
#define HYP_SOCK_BAD (-1)
#define hyp_sock_close close
#endif

/* Suppress SIGPIPE on send: Linux has the per-call flag, macOS has the
 * per-socket option (set at accept), Windows has no signals. */
#ifdef MSG_NOSIGNAL
#define HYP_SEND_FLAGS MSG_NOSIGNAL
#else
#define HYP_SEND_FLAGS 0
#endif

/* A non-reading peer may occupy the sequential event loop only briefly. */
#define HYP_HTTP_SEND_DEADLINE_MS 1000
#define HYP_HTTP_SEND_SLICE_BYTES (64U * 1024U)
#ifdef _WIN32
...
#define HYP_HTTP_WAIT_SLICE_MS 50
#endif
...
struct hyp_httpd {
    ...
    hyp_mutex_t active_mutex;
    struct hyp_http_conn *active;
    bool interrupted;
    int send_buffer_for_test;
    int send_deadline_for_test_ms;
};

struct hyp_http_conn {
    hyp_sock_t fd;
    hyp_httpd_t *owner;
    int recv_deadline_ms;
...
static int wait_ready(hyp_sock_t fd, bool writing, int timeout_ms) {
#ifdef _WIN32
    fd_set ready;
    fd_set errors;
    FD_ZERO(&ready);
...
static int wait_readable(hyp_sock_t fd, int timeout_ms) {
...
static bool socket_set_nonblocking(hyp_sock_t fd) {
#ifdef _WIN32
    u_long enabled = 1;
...
static void socket_shutdown(hyp_sock_t fd) {
#ifdef _WIN32
...
static bool connection_interrupted(hyp_http_conn_t *connection) {
    hyp_httpd_t *owner = connection->owner;
    if (!owner)
        return false;
    hyp_mutex_lock(&owner->active_mutex);
    bool interrupted = owner->interrupted;
    hyp_mutex_unlock(&owner->active_mutex);
    return interrupted;
}
...
static int wait_connection_ready(hyp_http_conn_t *connection, bool writing, int64_t deadline) {
    for (;;) {
...
#ifdef _WIN32
        if (timeout_ms > HYP_HTTP_WAIT_SLICE_MS)
            timeout_ms = HYP_HTTP_WAIT_SLICE_MS;
#endif
...
static int send_all(hyp_http_conn_t *connection, const void *data, size_t len, int64_t deadline) {
    const char *p = data;
    size_t off = 0;
    while (off < len) {
        ...
        if (available > HYP_HTTP_SEND_SLICE_BYTES)
            available = HYP_HTTP_SEND_SLICE_BYTES;
#ifdef _WIN32
        int n = send(connection->fd, p + off, (int)available, HYP_SEND_FLAGS);
#else
        ssize_t n = send(connection->fd, p + off, available, HYP_SEND_FLAGS);
#endif
...
hyp_httpd_t *hyp_httpd_listen(int port) {
    ...
    hyp_sock_t fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd == HYP_SOCK_BAD)
        return NULL;
    ...
    hyp_httpd_t *d = calloc(1, sizeof(*d));
    if (!d) {
        hyp_sock_close(fd);
        return NULL;
    }
    ...
    d->recv_deadline_ms = HYP_HTTP_RECV_DEADLINE_MS;
    hyp_mutex_init(&d->active_mutex);
    return d;
}
...
void hyp_httpd_set_recv_deadline_ms(hyp_httpd_t *d, int ms) {
    if (d && ms > 0) {
        hyp_mutex_lock(&d->active_mutex);
        d->recv_deadline_ms = ms;
        hyp_mutex_unlock(&d->active_mutex);
    }
}
...
bool hyp_httpd_close(hyp_httpd_t *d) {
    ...
    hyp_mutex_unlock(&d->active_mutex);
    hyp_sock_close(d->fd);
    hyp_mutex_destroy(&d->active_mutex);
    free(d);
    return true;
}
...
hyp_httpd_activity_t hyp_httpd_activity_for_test(hyp_httpd_t *d) {
    if (!d)
        return HYP_HTTPD_ACTIVITY_IDLE;
    hyp_mutex_lock(&d->active_mutex);
    hyp_httpd_activity_t activity = HYP_HTTPD_ACTIVITY_IDLE;
    if (d->active) {
        activity = atomic_load_explicit(&d->active->response_started, memory_order_acquire)
                       ? HYP_HTTPD_ACTIVITY_RESPONDING
                       : HYP_HTTPD_ACTIVITY_READING_REQUEST;
    }
    hyp_mutex_unlock(&d->active_mutex);
    return activity;
}
...
hyp_http_conn_t *hyp_httpd_accept(hyp_httpd_t *d, int timeout_ms) {
    ...
    hyp_sock_t cfd = accept(d->fd, NULL, NULL);
    if (cfd == HYP_SOCK_BAD)
        return NULL;
    ...
    hyp_http_conn_t *c = calloc(1, sizeof(*c));
    if (!c) {
        hyp_sock_close(cfd);
        return NULL;
    }
...
int hyp_http_parse_head(const char *data, size_t len, hyp_http_req_t *req, size_t *body_offset,
    ...
    *content_length = 0;
    ...
    size_t scan = len < HYP_HTTP_MAX_HEAD ? len : HYP_HTTP_MAX_HEAD;
    ...
    if (head_end == 0)
        return len >= HYP_HTTP_MAX_HEAD ? 431 : HYP_HTTP_NEED_MORE;
    ...
    while (p < head_stop) {
        ...
        if (header_name_is(p, nlen, "origin")) {
            ...
            for (const char *v = val; *v; v++) {
                ...
                if (cl > (uint64_t)HYP_HTTP_MAX_BODY)
                    return 413;
            }
...
int hyp_httpd_read_request(hyp_http_conn_t *c, hyp_http_req_t *req) {
    ...
    char *head = malloc(HYP_HTTP_MAX_HEAD);
    if (!head)
        return -1;
    ...
    for (;;) {
...
#ifdef _WIN32
        int n = recv(c->fd, head + have, (int)(HYP_HTTP_MAX_HEAD - have), 0);
#else
        ssize_t n = recv(c->fd, head + have, HYP_HTTP_MAX_HEAD - have, 0);
#endif
        if (n < 0 && socket_would_block())
            continue;
        if (n <= 0) {
        ...
        rc = hyp_http_parse_head(head, have, req, &body_off, &clen);
        if (rc == 0)
            break;
        if (rc != HYP_HTTP_NEED_MORE) {
            free(head);
            return rc;
        }
        if (have >= HYP_HTTP_MAX_HEAD) {
            free(head);
            return 431;
        }
    }
...
void hyp_http_replyf(hyp_http_conn_t *c, int status, const char *extra_headers, const char *fmt,
    ...
    hyp_http_reply_buf(c, status, extra_headers, body, (size_t)need);
    if (body != stack_buf)
        free(body);
}
