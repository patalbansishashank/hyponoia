...
/* hyp_store_batch_count_degrees builds a bound "?,?,..." IN clause into a fixed
...
/* Ceiling for a caller-requested node budget. HYP_UI_MAX_RENDER_NODES lowers
...
static int render_node_limit(void) {
    const char *raw = getenv("HYP_UI_MAX_RENDER_NODES");
    if (!raw || !raw[0]) {
...
static octree_node_t *octree_new(float ox, float oy, float oz, float half) {
    octree_node_t *n = calloc(HYP_ALLOC_ONE, sizeof(*n));
    if (!n)
        return NULL;
    n->ox = ox;
    n->oy = oy;
...
static void free_edge_array(hyp_edge_t *edges, int count) {
    if (!edges)
        return;
...
hyp_layout_result_t *hyp_layout_compute(hyp_store_t *store, const char *project,
    ...
    hyp_search_output_t search_out;
    memset(&search_out, 0, sizeof(search_out));
    if (hyp_store_search(store, &params, &search_out) != HYP_STORE_OK)
        return calloc(HYP_ALLOC_ONE, sizeof(hyp_layout_result_t));

    int n = search_out.count, total_count = search_out.total;
    if (n == 0) {
        hyp_store_search_free(&search_out);
        hyp_layout_result_t *r = calloc(HYP_ALLOC_ONE, sizeof(*r));
        if (r)
    ...
    int mapped = 0;
    int edge_cap = HYP_SZ_256;
    hyp_edge_t *all_edges = malloc((size_t)edge_cap * sizeof(hyp_edge_t));
    int *es = malloc((size_t)edge_cap * sizeof(int));
    int *ed = malloc((size_t)edge_cap * sizeof(int));
    hyp_schema_info_t schema;
    memset(&schema, 0, sizeof(schema));
    if (deg && all_edges && es && ed &&
        hyp_store_get_schema(store, project, &schema) == HYP_STORE_OK) {
        for (int t = 0; t < schema.edge_type_count; t++) {
            hyp_edge_t *te = NULL;
            int tc = 0;
            if (hyp_store_find_edges_by_type(store, project, schema.edge_types[t].type, &te, &tc) ==
                HYP_STORE_OK) {
                for (int e = 0; e < tc; e++) {
                    ...
                    if (si >= 0 && di >= 0) {
                        if (mapped >= edge_cap) {
                            ...
                            hyp_edge_t *te2 = realloc(all_edges, (size_t)nc * sizeof(hyp_edge_t));
                        ...
                        memset(&te[e], 0, sizeof(hyp_edge_t));
                        es[mapped] = si;
                        ed[mapped] = di;
                        deg[si]++;
                        deg[di]++;
    ...
    hyp_layout_result_t *result = calloc(HYP_ALLOC_ONE, sizeof(*result));
    if (!result || !bodies) {
        free(bodies);
        free(deg);
        free(es);
        free(ed);
        free(cdepth);
        hyp_layout_free(result);
        free_edge_array(all_edges, mapped);
        hyp_store_search_free(&search_out);
        return NULL;
    }
    result->nodes = calloc((size_t)n, sizeof(hyp_layout_node_t));
    result->node_count = n;
    ...
    if (dead_degrees_valid) {
        ...
        for (int off = 0; off < n; off += DEAD_DEGREE_CHUNK) {
            ...
            if (hyp_store_batch_count_degrees(store, node_ids + off, cnt, "CALLS", in_calls + off,
                                              deg_dummy + off) != HYP_STORE_OK ||
                hyp_store_batch_count_degrees(store, node_ids + off, cnt, "USAGE", in_usage + off,
                                              deg_dummy + off) != HYP_STORE_OK ||
                hyp_store_batch_count_degrees(store, node_ids + off, cnt, "CALL_REFERENCE",
    ...
    for (int i = 0; i < n; i++) {
        ...
        bool testish = nf.is_test || (sn->file_path && hyp_is_test_file_path(sn->file_path));
    ...
    /* 8. Output edges */
    if (mapped > 0) {
        result->edges = calloc((size_t)mapped, sizeof(hyp_layout_edge_t));
...
void hyp_layout_free(hyp_layout_result_t *r) {
    if (!r)
        return;
...
char *hyp_layout_to_json(const hyp_layout_result_t *r) {
    if (!r)
        return NULL;
    ...
    if (!json) {
        ...
        hyp_log_error("layout.json.fail", "code", code, "msg",
