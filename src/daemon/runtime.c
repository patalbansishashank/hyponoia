...
void hyp_daemon_runtime_force_peer_image_unverified_for_testing(bool force) {
...
enum {
    ...
    RENDEZVOUS_REQUEST_BUILD_OFFSET =
        RENDEZVOUS_REQUEST_VERSION_OFFSET + HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    ...
    RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET + HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET =
        RENDEZVOUS_RESPONSE_ACTIVE_BUILD_OFFSET + HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
    RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_VERSION_OFFSET + HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
    RENDEZVOUS_RESPONSE_MESSAGE_OFFSET =
        RENDEZVOUS_RESPONSE_REQUESTED_BUILD_OFFSET + HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
...
_Static_assert(RENDEZVOUS_REQUEST_BUILD_OFFSET + HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP ==
                   HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE,
               "rendezvous request layout changed");
_Static_assert(RENDEZVOUS_RESPONSE_MESSAGE_OFFSET + HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP ==
                   HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE,
               "rendezvous response layout changed");
_Static_assert(ACTIVATION_REQUEST_IDENTITY_OFFSET + HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE ==
                       HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE &&
                   ACTIVATION_RESPONSE_CONNECTIONS_OFFSET + 8 ==
                       HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE,
               "activation shutdown layout changed");
_Static_assert(HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP == HYP_DAEMON_VERSION_TEXT_SIZE,
               "service version capacity requires a new rendezvous strategy");
_Static_assert(HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP == HYP_DAEMON_BUILD_FINGERPRINT_SIZE,
               "service fingerprint capacity requires a new rendezvous strategy");
_Static_assert(HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP == HYP_DAEMON_CONFLICT_MESSAGE_SIZE,
               "service message capacity requires a new rendezvous strategy");
_Static_assert(HYP_DAEMON_RUNTIME_CONNECT_ERROR == 0 && HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED == 1 &&
                   HYP_DAEMON_RUNTIME_CONNECT_CONFLICT == 2 &&
                   HYP_DAEMON_RUNTIME_CONNECT_REJECTED == 3,
               "rendezvous connect status values changed");
_Static_assert(HYP_DAEMON_HELLO_INVALID == 0 && HYP_DAEMON_HELLO_COMPATIBLE == 1 &&
                   HYP_DAEMON_HELLO_VERSION_CONFLICT == 2 && HYP_DAEMON_HELLO_BUILD_CONFLICT == 3 &&
                   HYP_DAEMON_HELLO_PROTOCOL_ABI_CONFLICT == 4 &&
                   HYP_DAEMON_HELLO_STORE_ABI_CONFLICT == 5 &&
                   HYP_DAEMON_HELLO_FEATURE_ABI_CONFLICT == 6,
               "rendezvous hello status values changed");
_Static_assert(HYP_DAEMON_RENDEZVOUS_FRAME_VERSION == 1 && HYP_DAEMON_RENDEZVOUS_ABI == 1 &&
                   HYP_DAEMON_FRAME_REQUEST == 1 && HYP_DAEMON_FRAME_RESPONSE == 2 &&
                   HYP_DAEMON_RUNTIME_OP_HELLO == 1 &&
                   HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN == 8 &&
                   HYP_DAEMON_RUNTIME_ACTIVATION_INSTALL == 1 &&
                   HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE == 2 &&
                   HYP_DAEMON_RUNTIME_ACTIVATION_UNINSTALL == 3,
               "rendezvous framing values changed");

typedef struct hyp_daemon_runtime_worker hyp_daemon_runtime_worker_t;
...
struct hyp_daemon_runtime_service {
    hyp_mutex_t mutex;
    hyp_daemon_ipc_listener_t *listener;
    hyp_daemon_coordinator_t *coordinator;
    hyp_daemon_runtime_worker_t *workers;
    ...
    hyp_daemon_runtime_application_callbacks_t application;
    /* Owned only by the convenience start() path. start_reserved() callers
     * retain their externally managed participant guard. */
    hyp_daemon_ipc_participant_guard_t *owned_participant_guard;
};

struct hyp_daemon_runtime_worker {
    hyp_daemon_runtime_service_t *service;
    hyp_mutex_t send_mutex;
    hyp_thread_t thread;
    bool thread_started;
    bool in_use;
    bool joining;
    atomic_bool done;
    hyp_daemon_ipc_connection_t *connection;
    ...
    hyp_daemon_runtime_application_session_t *application_session;
    bool application_session_opened;
    bool application_cancelled;
    atomic_bool disconnecting;

    hyp_thread_t application_thread;
    bool application_thread_started;
    atomic_bool application_thread_done;
    hyp_daemon_runtime_application_token_t application_request_token;
    hyp_daemon_runtime_application_token_t last_application_request_token;
    uint8_t *application_request;
    uint32_t application_request_length;
};

struct hyp_daemon_runtime_client {
    hyp_mutex_t exchange_mutex;
    hyp_mutex_t send_mutex;
    hyp_mutex_t state_mutex;
    hyp_daemon_ipc_connection_t *connection;
    hyp_daemon_client_id_t client_id;
    ...
    hyp_daemon_runtime_application_token_t next_application_token;
    hyp_daemon_runtime_application_token_t last_started_application_token;
    hyp_daemon_runtime_application_token_t active_application_token;
    hyp_daemon_runtime_application_token_t pending_cancel_token;
...
static hyp_daemon_hello_status_t runtime_rendezvous_compare(const char *active_version,
    ...
    hyp_daemon_build_identity_t requested = {
    ...
    hyp_daemon_hello_status_t status = hyp_daemon_hello_compare(&active, &requested, conflict_out);
    if (status != HYP_DAEMON_HELLO_INVALID && status != HYP_DAEMON_HELLO_COMPATIBLE &&
        status != HYP_DAEMON_HELLO_VERSION_CONFLICT && status != HYP_DAEMON_HELLO_BUILD_CONFLICT) {
        if (conflict_out) {
...
bool hyp_daemon_runtime_hello_request_encode(uint8_t out[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE],
                                             const hyp_daemon_build_identity_t *identity) {
    if (!out || !identity) {
        return false;
    }
    hyp_daemon_conflict_t validation;
    if (runtime_rendezvous_compare(identity->semantic_version, identity->build_fingerprint,
                                   ...
                                   &validation) != HYP_DAEMON_HELLO_COMPATIBLE) {
        return false;
    }
    memset(out, 0, HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE);
    runtime_put_u32(out + RENDEZVOUS_REQUEST_ABI_OFFSET, HYP_DAEMON_RENDEZVOUS_ABI);
    return runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                      HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP,
                                      identity->semantic_version, false) &&
           runtime_wire_string_encode(out + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                      HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP,
                                      identity->build_fingerprint, false);
}

static bool runtime_rendezvous_request_decode(
    const uint8_t payload[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE],
    char version[HYP_DAEMON_VERSION_TEXT_SIZE], char build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !version || !build ||
        runtime_get_u32(payload + RENDEZVOUS_REQUEST_ABI_OFFSET) != HYP_DAEMON_RENDEZVOUS_ABI ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_VERSION_OFFSET,
                                    HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP, version, false) ||
        !runtime_wire_string_decode(payload + RENDEZVOUS_REQUEST_BUILD_OFFSET,
                                    HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP, build, false)) {
        return false;
    }
    hyp_daemon_conflict_t validation;
...
static bool runtime_activation_action_valid(hyp_daemon_runtime_activation_action_t action) {
    return action == HYP_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
           action == HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE ||
           action == HYP_DAEMON_RUNTIME_ACTIVATION_UNINSTALL;
}

static const char *runtime_activation_action_text(hyp_daemon_runtime_activation_action_t action) {
    switch (action) {
...
static bool runtime_activation_request_encode(
    uint8_t out[HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    hyp_daemon_runtime_activation_action_t action, const hyp_daemon_build_identity_t *identity) {
    if (!out || !identity || !runtime_activation_action_valid(action)) {
        return false;
    }
    memset(out, 0, HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE);
    runtime_put_u32(out + ACTIVATION_REQUEST_ACTION_OFFSET, (uint32_t)action);
    return hyp_daemon_runtime_hello_request_encode(out + ACTIVATION_REQUEST_IDENTITY_OFFSET,
                                                   identity);
}

static bool runtime_activation_request_decode(
    const uint8_t payload[HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE],
    hyp_daemon_runtime_activation_action_t *action_out, char version[HYP_DAEMON_VERSION_TEXT_SIZE],
    char build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE]) {
    if (!payload || !action_out || !version || !build) {
        return false;
    }
    uint32_t raw_action = runtime_get_u32(payload + ACTIVATION_REQUEST_ACTION_OFFSET);
    if (raw_action < (uint32_t)HYP_DAEMON_RUNTIME_ACTIVATION_INSTALL ||
        raw_action > (uint32_t)HYP_DAEMON_RUNTIME_ACTIVATION_UNINSTALL ||
    ...
    hyp_daemon_runtime_activation_action_t action =
        (hyp_daemon_runtime_activation_action_t)raw_action;
    *action_out = action;
    return true;
}

static bool runtime_activation_response_encode(
    uint8_t out[HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    const hyp_daemon_runtime_activation_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE);
    runtime_put_u32(out + ACTIVATION_RESPONSE_ABI_OFFSET, HYP_DAEMON_RENDEZVOUS_ABI);
...
static bool runtime_activation_response_decode(
    const uint8_t payload[HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE],
    hyp_daemon_runtime_activation_result_t *result_out) {
    if (!payload || !result_out ||
        runtime_get_u32(payload + ACTIVATION_RESPONSE_ABI_OFFSET) != HYP_DAEMON_RENDEZVOUS_ABI) {
        return false;
    }
...
static bool runtime_process_image_reference_acquire(
    ...
    wchar_t *open_path = ok ? hyp_wide_path_to_extended(process_before.path) : NULL;
    ...
    ok = ok && runtime_windows_file_snapshot(file, &file_before, &size_before) &&
         (!fingerprint || hyp_daemon_build_fingerprint_native_file((uintptr_t)file, fingerprint)) &&
    ...
    ok = ok && fd >= 0 && fstat(fd, &file_before) == 0 && S_ISREG(file_before.st_mode) &&
         ...
         (!fingerprint || hyp_daemon_build_fingerprint_native_file((uintptr_t)fd, fingerprint)) &&
    ...
    bool ok = image_fd >= 0 && fstat(image_fd, &image_before) == 0 &&
              ...
              (!fingerprint ||
               hyp_daemon_build_fingerprint_native_file((uintptr_t)image_fd, fingerprint)) &&
...
bool hyp_daemon_runtime_process_build_fingerprint(uint64_t process_id,
...
static bool runtime_hello_response_encode(uint8_t out[HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
                                          const hyp_daemon_runtime_connect_result_t *result) {
    if (!out || !result) {
        return false;
    }
    memset(out, 0, HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE);
    ...
    if (!runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_ACTIVE_VERSION_OFFSET,
        ...
        !runtime_wire_string_encode(out + RENDEZVOUS_RESPONSE_MESSAGE_OFFSET,
                                    HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP, result->message, true)) {
        return false;
    }
    return true;
}
...
static bool runtime_hello_response_decode(
    const uint8_t payload[HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE],
    hyp_daemon_runtime_connect_result_t *result) {
    if (!payload || !result) {
        return false;
    }
    memset(result, 0, sizeof(*result));
    result->status = (hyp_daemon_runtime_connect_status_t)runtime_get_u32(
        payload + RENDEZVOUS_RESPONSE_CONNECT_STATUS_OFFSET);
    result->hello_status = (hyp_daemon_hello_status_t)runtime_get_u32(
    ...
    result->conflict.status = (hyp_daemon_hello_status_t)runtime_get_u32(
    ...
    if (result->status == HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        return result->hello_status == HYP_DAEMON_HELLO_COMPATIBLE &&
               result->conflict.status == HYP_DAEMON_HELLO_COMPATIBLE &&
               result->client_id != HYP_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id != 0;
    }
    if (result->status == HYP_DAEMON_RUNTIME_CONNECT_CONFLICT) {
        char expected[HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP];
        return result->hello_status >= HYP_DAEMON_HELLO_VERSION_CONFLICT &&
               result->hello_status <= HYP_DAEMON_HELLO_FEATURE_ABI_CONFLICT &&
               result->conflict.status == result->hello_status &&
               result->client_id == HYP_DAEMON_CLIENT_ID_INVALID &&
               result->authenticated_process_id == 0 &&
               hyp_daemon_conflict_format(&result->conflict, expected, sizeof(expected)) &&
               strcmp(result->message, expected) == 0;
    }
    return result->status == HYP_DAEMON_RUNTIME_CONNECT_REJECTED &&
           result->hello_status == HYP_DAEMON_HELLO_INVALID &&
           result->conflict.status == HYP_DAEMON_HELLO_INVALID &&
           result->client_id == HYP_DAEMON_CLIENT_ID_INVALID &&
           result->authenticated_process_id == 0;
}

static bool runtime_send_hello_response(hyp_daemon_ipc_connection_t *connection,
                                        const hyp_daemon_runtime_connect_result_t *result) {
    uint8_t payload[HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE];
    return runtime_hello_response_encode(payload, result) &&
           hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_RESPONSE,
                                     HYP_DAEMON_RUNTIME_OP_HELLO, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_worker_send_frame(hyp_daemon_runtime_worker_t *worker,
                                      hyp_daemon_frame_type_t type,
                                      hyp_daemon_runtime_operation_t operation, const void *payload,
                                      uint32_t length) {
    if (!worker) {
        return false;
    }
    hyp_mutex_lock(&worker->send_mutex);
    bool sent =
        worker->connection &&
        hyp_daemon_ipc_send_frame(worker->connection, type, (uint16_t)operation, payload, length);
    hyp_mutex_unlock(&worker->send_mutex);
    return sent;
}

static bool runtime_worker_send_status(hyp_daemon_runtime_worker_t *worker,
                                       hyp_daemon_runtime_operation_t operation, bool success) {
    uint8_t payload[STATUS_RESPONSE_SIZE];
    runtime_put_u32(payload, success ? 1U : 0U);
    return runtime_worker_send_frame(worker, HYP_DAEMON_FRAME_RESPONSE, operation, payload,
                                     (uint32_t)sizeof(payload));
}

static bool runtime_application_status_is_callback_result(
    hyp_daemon_runtime_application_status_t status) {
    return status == HYP_DAEMON_RUNTIME_APPLICATION_OK ||
           status == HYP_DAEMON_RUNTIME_APPLICATION_REJECTED ||
           status == HYP_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR ||
           status == HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED;
}

static bool runtime_worker_send_application_response(
    hyp_daemon_runtime_worker_t *worker, hyp_daemon_runtime_application_token_t request_token,
    hyp_daemon_runtime_application_status_t status, const uint8_t *response,
    uint32_t response_length, bool suppress_when_disconnecting) {
    if (!worker || request_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        status <= HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR ||
        status > HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED ||
        response_length > HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        (response_length > 0 && !response)) {
        return false;
    }
    uint64_t wire_length = APPLICATION_RESPONSE_PREFIX_SIZE + (uint64_t)response_length;
    if (wire_length > HYP_DAEMON_MAX_FRAME_SIZE || wire_length > UINT32_MAX) {
        return false;
    }
    ...
    bool sent = !disconnected && worker->connection &&
                hyp_daemon_ipc_send_frame(worker->connection, HYP_DAEMON_FRAME_RESPONSE,
                                          HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST, wire,
...
static void runtime_result_rejected(hyp_daemon_runtime_connect_result_t *result,
                                    const char *message) {
    memset(result, 0, sizeof(*result));
    result->status = HYP_DAEMON_RUNTIME_CONNECT_REJECTED;
    result->hello_status = HYP_DAEMON_HELLO_INVALID;
    result->conflict.status = HYP_DAEMON_HELLO_INVALID;
    (void)snprintf(result->message, sizeof(result->message), "%s", message);
}

static void runtime_service_begin_stopping_locked(hyp_daemon_runtime_service_t *service,
                                                  uint64_t deadline, bool emergency,
                                                  const char *reason) {
    if (service->state == HYP_DAEMON_RUNTIME_SERVICE_RUNNING) {
        service->state = HYP_DAEMON_RUNTIME_SERVICE_STOPPING;
        ...
        hyp_log_info("daemon.runtime_stopping", "reason", reason ? reason : "unspecified");
    } else if (service->state == HYP_DAEMON_RUNTIME_SERVICE_STOPPING &&
...
static void runtime_service_begin_stopping(hyp_daemon_runtime_service_t *service,
                                           uint32_t timeout_ms, bool emergency,
                                           const char *reason) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    hyp_mutex_lock(&service->mutex);
    runtime_service_begin_stopping_locked(service, deadline, emergency, reason);
    hyp_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections_except(hyp_daemon_runtime_service_t *service,
                                                         hyp_daemon_runtime_worker_t *except_worker,
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        hyp_daemon_runtime_worker_t *worker = &service->workers[i];
        if (worker != except_worker && worker->in_use && worker->connection &&
            (activation_owner || force || !worker->final_response_inflight)) {
            hyp_daemon_ipc_connection_interrupt(worker->connection);
        }
    }
    hyp_mutex_unlock(&service->mutex);
}

static void runtime_service_interrupt_connections(hyp_daemon_runtime_service_t *service) {
    runtime_service_interrupt_connections_except(service, NULL, false);
}

static void runtime_worker_disconnect(hyp_daemon_runtime_worker_t *worker) {
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_daemon_client_id_t client_id = HYP_DAEMON_CLIENT_ID_INVALID;
    ...
    (void)hyp_daemon_client_disconnected(service->coordinator, client_id, hyp_now_ms());
    ...
    if (hyp_daemon_coordinator_state(service->coordinator) == HYP_DAEMON_COORDINATOR_STOPPING) {
        runtime_service_begin_stopping(service, service->shutdown_timeout_ms, false,
                                       "coordinator_stopping");
    }
}

static bool runtime_worker_service_running(hyp_daemon_runtime_worker_t *worker) {
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_mutex_lock(&service->mutex);
    bool running = service->state == HYP_DAEMON_RUNTIME_SERVICE_RUNNING;
    hyp_mutex_unlock(&service->mutex);
    return running;
}

static bool runtime_worker_admit(hyp_daemon_runtime_worker_t *worker,
                                 hyp_daemon_client_id_t *client_id_out) {
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_daemon_client_id_t client_id = HYP_DAEMON_CLIENT_ID_INVALID;
    hyp_mutex_lock(&service->mutex);
    if (service->state == HYP_DAEMON_RUNTIME_SERVICE_RUNNING) {
        client_id = hyp_daemon_client_connected(service->coordinator, hyp_now_ms());
        if (client_id != HYP_DAEMON_CLIENT_ID_INVALID) {
    ...
    *client_id_out = client_id;
    return client_id != HYP_DAEMON_CLIENT_ID_INVALID;
}

static bool runtime_worker_commit_admission(hyp_daemon_runtime_worker_t *worker) {
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_mutex_lock(&service->mutex);
    bool committed = service->state == HYP_DAEMON_RUNTIME_SERVICE_RUNNING && worker->admitted &&
...
static bool runtime_worker_handle_subscribe(hyp_daemon_runtime_worker_t *worker,
    ...
    if (key_length == 0 || key_length > HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX ||
        length != SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length) {
        return false;
    }
    char project_key[HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1];
    ...
    hyp_daemon_subscription_id_t subscription_id = HYP_DAEMON_SUBSCRIPTION_ID_INVALID;
    hyp_daemon_subscription_result_t result = hyp_daemon_job_subscribe(
    ...
    return runtime_worker_send_frame(worker, HYP_DAEMON_FRAME_RESPONSE,
                                     HYP_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, response,
                                     (uint32_t)sizeof(response));
}

static bool runtime_worker_handle_unsubscribe(hyp_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!runtime_worker_service_running(worker) || !payload || length != UNSUBSCRIBE_REQUEST_SIZE) {
        return false;
    }
    hyp_daemon_subscription_id_t subscription_id = runtime_get_u64(payload);
    bool removed = hyp_daemon_job_unsubscribe(worker->service->coordinator, worker->client_id,
                                              subscription_id);
    return runtime_worker_send_status(worker, HYP_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, removed);
}

static void *runtime_application_worker(void *opaque) {
    hyp_daemon_runtime_worker_t *worker = opaque;
    hyp_daemon_runtime_service_t *service = worker->service;
    uint8_t *response = NULL;
    uint32_t response_length = 0;
    hyp_daemon_runtime_application_status_t status = service->application.request(
    ...
    bool valid_response =
        ...
        (status == HYP_DAEMON_RUNTIME_APPLICATION_OK || (response == NULL && response_length == 0));
...
static bool runtime_worker_reap_application(hyp_daemon_runtime_worker_t *worker, bool wait) {
    ...
    if (hyp_thread_join(&worker->application_thread) != 0) {
        return false;
    }
    ...
    worker->application_request_token = HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    return true;
}

static bool runtime_worker_handle_application(hyp_daemon_runtime_worker_t *worker,
                                              const uint8_t *payload, uint32_t length) {
    if (!payload || length < APPLICATION_REQUEST_PREFIX_SIZE) {
        return false;
    }
    hyp_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    uint32_t request_length = runtime_get_u32(payload + 8);
    if (request_length > HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
        length != APPLICATION_REQUEST_PREFIX_SIZE + request_length ||
        request_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
    ...
    if (!service->application.request) {
        return runtime_worker_send_application_response(
            worker, request_token, HYP_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE, NULL, 0, false);
    }
    (void)runtime_worker_reap_application(worker, false);
    if (worker->application_thread_started) {
        return runtime_worker_send_application_response(
            worker, request_token, HYP_DAEMON_RUNTIME_APPLICATION_BUSY, NULL, 0, false);
    }
    ...
    if (request_length > 0) {
        ...
        if (!request_copy) {
            return runtime_worker_send_application_response(
                worker, request_token, HYP_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0,
                false);
        }
    ...
    atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
    if (hyp_thread_create(&worker->application_thread, RUNTIME_WORKER_STACK_SIZE,
                          runtime_application_worker, worker) != 0) {
        free(worker->application_request);
        worker->application_request = NULL;
        worker->application_request_length = 0;
        return runtime_worker_send_application_response(
            worker, request_token, HYP_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR, NULL, 0, false);
    }
    worker->application_thread_started = true;
    return true;
}

static bool runtime_worker_handle_application_cancel(hyp_daemon_runtime_worker_t *worker,
                                                     const uint8_t *payload, uint32_t length) {
    if (!worker || !payload || length != APPLICATION_CANCEL_REQUEST_SIZE) {
        return false;
    }
    hyp_daemon_runtime_application_token_t request_token = runtime_get_u64(payload);
    if (request_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return false;
    }
...
static bool runtime_worker_handle_close_intent(hyp_daemon_runtime_worker_t *worker,
                                               uint32_t length) {
    if (length != 0) {
        return false;
    }
...
static bool runtime_worker_handle_disconnect(hyp_daemon_runtime_worker_t *worker, uint32_t length) {
    if (length != 0) {
        return false;
    }
    hyp_daemon_runtime_service_t *service = worker->service;
    ...
    hyp_mutex_unlock(&service->mutex);
    runtime_worker_disconnect(worker);
    bool sent = runtime_worker_send_status(worker, HYP_DAEMON_RUNTIME_OP_DISCONNECT, true);
    hyp_mutex_lock(&service->mutex);
...
static void runtime_worker_finish(hyp_daemon_runtime_worker_t *worker) {
    hyp_daemon_runtime_service_t *service = worker->service;
    runtime_worker_disconnect(worker);
    ...
    if (worker->connection) {
         ...
         * it before the handle closes; see hyp_daemon_ipc_connection_drain. */
        hyp_daemon_ipc_connection_drain(worker->connection, RUNTIME_WORKER_DRAIN_TIMEOUT_MS);
    }
    hyp_mutex_lock(&service->mutex);
    if (worker->connection) {
        hyp_daemon_ipc_connection_close(worker->connection);
...
static bool runtime_activation_peer_matches_claim(hyp_daemon_runtime_service_t *service,
    ...
    char peer_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE];
    return hyp_daemon_runtime_process_build_fingerprint(process_id, peer_fingerprint) &&
...
static bool runtime_control_request_decode(const uint8_t *payload, uint32_t length,
    ...
    if (!payload || length != HYP_DAEMON_CONTROL_REQUEST_SIZE ||
        payload[HYP_DAEMON_CONTROL_REQUEST_SIZE - 1U] != 0U) {
        return false;
    }
    memcpy(build_out, payload, HYP_DAEMON_BUILD_FINGERPRINT_SIZE);
    return strlen(build_out) == HYP_DAEMON_BUILD_FINGERPRINT_SIZE - 1U;
}

static void runtime_control_collect_clients_locked(hyp_daemon_runtime_service_t *service,
                                                   hyp_daemon_runtime_worker_t *except,
                                                   uint8_t *count_out,
                                                   uint32_t pids[HYP_DAEMON_CONTROL_CLIENT_CAP]) {
    uint8_t count = 0U;
    for (size_t index = 0; index < service->worker_capacity; index++) {
        hyp_daemon_runtime_worker_t *candidate = &service->workers[index];
        if (!candidate->in_use || !candidate->admission_committed || candidate == except) {
            continue;
        }
        if (count < HYP_DAEMON_CONTROL_CLIENT_CAP) {
            pids[count] = (uint32_t)candidate->peer_process_id;
        }
        count++;
    }
    *count_out =
        count > HYP_DAEMON_CONTROL_CLIENT_CAP ? (uint8_t)HYP_DAEMON_CONTROL_CLIENT_CAP : count;
}

static void runtime_worker_handle_status(hyp_daemon_runtime_worker_t *worker,
                                         const uint8_t *payload, uint32_t length) {
    hyp_daemon_runtime_service_t *service = worker->service;
    char requested_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
    ...
    uint8_t response[HYP_DAEMON_STATUS_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    if (peer_verified) {
        uint8_t count = 0U;
        uint32_t pids[HYP_DAEMON_CONTROL_CLIENT_CAP] = {0};
        hyp_mutex_lock(&service->mutex);
        response[0] = 1U;
        response[1] =
            (uint8_t)((service->permanent ? 0x01U : 0U) |
                      (service->state != HYP_DAEMON_RUNTIME_SERVICE_RUNNING ? 0x02U : 0U));
        ...
        response[8] = count;
        for (size_t index = 0; index < HYP_DAEMON_CONTROL_CLIENT_CAP; index++) {
        ...
        (void)snprintf((char *)response + 44U, HYP_DAEMON_BUILD_FINGERPRINT_SIZE, "%s",
    ...
    (void)runtime_worker_send_frame(worker, HYP_DAEMON_FRAME_RESPONSE, HYP_DAEMON_RUNTIME_OP_STATUS,
                                    response, (uint32_t)sizeof(response));
    runtime_worker_finish(worker);
}

static void runtime_worker_handle_stop(hyp_daemon_runtime_worker_t *worker, const uint8_t *payload,
                                       uint32_t length) {
    hyp_daemon_runtime_service_t *service = worker->service;
    char requested_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool peer_verified =
        runtime_control_request_decode(payload, length, requested_build) &&
        runtime_activation_peer_matches_claim(service, worker->peer_process_id, requested_build);
    uint8_t response[HYP_DAEMON_STOP_RESPONSE_SIZE];
    memset(response, 0, sizeof(response));
    bool accepted = false;
    if (peer_verified) {
        uint8_t count = 0U;
        ...
        if (service->committed_clients > 0) {
            ...
            response[1] = 0x02U;
        } else if (service->state == HYP_DAEMON_RUNTIME_SERVICE_RUNNING) {
    ...
    if (accepted) {
        ...
        hyp_log_info("daemon.stop_requested", "requester_pid", requester_pid, "requester_build",
                     requested_build);
        runtime_service_interrupt_connections_except(worker->service, worker, true);
    }
    (void)runtime_worker_send_frame(worker, HYP_DAEMON_FRAME_RESPONSE, HYP_DAEMON_RUNTIME_OP_STOP,
                                    response, (uint32_t)sizeof(response));
    if (accepted) {
        hyp_daemon_runtime_service_t *stop_service = worker->service;
        runtime_worker_finish(worker);
        hyp_mutex_lock(&stop_service->mutex);
        worker->final_response_inflight = false;
        stop_service->activation_response_inflight = false;
        hyp_mutex_unlock(&stop_service->mutex);
...
static void runtime_worker_handle_activation_shutdown(hyp_daemon_runtime_worker_t *worker,
                                                      const uint8_t *payload, uint32_t length) {
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_daemon_runtime_activation_result_t result = {0};
    hyp_daemon_runtime_activation_action_t action = 0;
    char requested_version[HYP_DAEMON_VERSION_TEXT_SIZE] = {0};
    char requested_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE] = {0};
    bool request_valid =
        length == HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE && payload &&
    ...
    uint8_t response[HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE];
    ...
    if (result.accepted) {
        ...
        hyp_log_info("daemon.activation_shutdown", "requester_pid", requester_pid,
    ...
    bool response_sent =
        encoded && runtime_worker_send_frame(worker, HYP_DAEMON_FRAME_RESPONSE,
                                             HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN, response,
                                             (uint32_t)sizeof(response));
    if (result.accepted && !response_sent) {
        hyp_log_error("daemon.activation_shutdown_ack_failed", "shutdown", "accepted",
                      "requester_build", requested_build, "action",
...
static void *runtime_connection_worker(void *opaque) {
    hyp_daemon_runtime_worker_t *worker = opaque;
    hyp_daemon_runtime_service_t *service = worker->service;
    hyp_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    char requested_version[HYP_DAEMON_VERSION_TEXT_SIZE];
    char requested_build[HYP_DAEMON_BUILD_FINGERPRINT_SIZE];

    int received = hyp_daemon_ipc_receive_frame_bounded(
        worker->connection, service->request_timeout_ms,
        HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE, &frame, &payload);
    if (received != 1 || frame.type != HYP_DAEMON_FRAME_REQUEST) {
        free(payload);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (frame.flags == HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN) {
        runtime_worker_handle_activation_shutdown(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == HYP_DAEMON_RUNTIME_OP_STATUS) {
        runtime_worker_handle_status(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags == HYP_DAEMON_RUNTIME_OP_STOP) {
        runtime_worker_handle_stop(worker, payload, frame.length);
        free(payload);
        return NULL;
    }
    if (frame.flags != HYP_DAEMON_RUNTIME_OP_HELLO ||
        frame.length != HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE ||
    ...
    hyp_daemon_runtime_connect_result_t hello_result;
    memset(&hello_result, 0, sizeof(hello_result));
    hyp_daemon_hello_status_t hello_status = runtime_rendezvous_compare(
    ...
    if (hello_status != HYP_DAEMON_HELLO_COMPATIBLE) {
        if (hello_status == HYP_DAEMON_HELLO_INVALID) {
            runtime_worker_finish(worker);
            return NULL;
        }
        hello_result.status = HYP_DAEMON_RUNTIME_CONNECT_CONFLICT;
        hello_result.hello_status = hello_status;
        (void)hyp_daemon_conflict_format(&hello_result.conflict, hello_result.message,
                                         sizeof(hello_result.message));
        if (!hyp_daemon_conflict_log_append(service->conflict_log_path, &hello_result.conflict,
            ...
            hyp_log_error("daemon.conflict_log_append_failed", "path", service->conflict_log_path);
        }
    ...
    if (!peer_image_verified) {
        ...
        peer_image_fingerprinted =
            hyp_daemon_runtime_process_build_fingerprint(worker->peer_process_id, peer_fingerprint);
    ...
    if (!peer_image_verified) {
        ...
        hyp_log_error("daemon.client_image_rejected", "reason", reason);
        ...
        runtime_result_rejected(&hello_result, "HYP daemon rejected this client's binary image");
        (void)snprintf(hello_result.message, sizeof(hello_result.message),
                       "HYP daemon rejected this client: %s. The client binary must match the "
                       "running daemon's build; close HYP sessions (or run 'daemon stop') and "
    ...
    if (!runtime_worker_admit(worker, &client_id)) {
        runtime_result_rejected(&hello_result, "HYP daemon is stopping");
        (void)runtime_send_hello_response(worker->connection, &hello_result);
        runtime_worker_finish(worker);
        return NULL;
    }
    if (service->application.request) {
        ...
        if (!worker->application_session) {
            runtime_result_rejected(&hello_result, "HYP daemon session initialization failed");
            hyp_mutex_lock(&service->mutex);
            worker->final_response_inflight = true;
            hyp_mutex_unlock(&service->mutex);
            runtime_worker_disconnect(worker);
    ...
    hello_result.status = HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED;
    hello_result.hello_status = HYP_DAEMON_HELLO_COMPATIBLE;
    hello_result.conflict.status = HYP_DAEMON_HELLO_COMPATIBLE;
    ...
    while (keep_running && runtime_worker_service_running(worker)) {
        ...
        payload = NULL;
        received = hyp_daemon_ipc_receive_frame(worker->connection, HYP_DAEMON_IPC_WAIT_FOREVER,
                                                &frame, &payload);
        if (received != 1 || frame.type != HYP_DAEMON_FRAME_REQUEST) {
            ...
            hyp_log_info("daemon.connection_end", "received", received_text, "frame_type",
                         received == 1 ? "non_request" : "none");
            break;
        }
        switch (frame.flags) {
        case HYP_DAEMON_RUNTIME_OP_HEARTBEAT: {
            bool valid = frame.length == 0 &&
                         hyp_daemon_client_heartbeat(service->coordinator, client_id, hyp_now_ms());
            keep_running =
                valid && runtime_worker_send_status(worker, HYP_DAEMON_RUNTIME_OP_HEARTBEAT, true);
            break;
        }
        case HYP_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE:
            keep_running = runtime_worker_handle_subscribe(worker, payload, frame.length);
            break;
        case HYP_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE:
            keep_running = runtime_worker_handle_unsubscribe(worker, payload, frame.length);
            break;
        case HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST:
            keep_running = runtime_worker_handle_application(worker, payload, frame.length);
            break;
        case HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL:
            keep_running = runtime_worker_handle_application_cancel(worker, payload, frame.length);
            break;
        case HYP_DAEMON_RUNTIME_OP_CLOSE_INTENT:
            keep_running = runtime_worker_handle_close_intent(worker, frame.length);
            break;
        case HYP_DAEMON_RUNTIME_OP_DISCONNECT:
            keep_running = false;
...
static void runtime_worker_reset_after_join(hyp_daemon_runtime_worker_t *worker) {
    free(worker->application_request);
    worker->thread_started = false;
    worker->in_use = false;
    worker->joining = false;
    worker->connection = NULL;
    worker->client_id = HYP_DAEMON_CLIENT_ID_INVALID;
    ...
    worker->application_thread_started = false;
    worker->application_request_token = HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->last_application_request_token = HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    worker->application_request = NULL;
    worker->application_request_length = 0;
...
static void runtime_reap_completed_workers(hyp_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
        hyp_daemon_runtime_worker_t *worker = &service->workers[i];
        hyp_mutex_lock(&service->mutex);
        ...
        int joined = hyp_thread_join(&worker->thread);
        hyp_mutex_lock(&service->mutex);
        if (joined == 0) {
...
static hyp_daemon_runtime_worker_t *runtime_find_free_worker_locked(
    hyp_daemon_runtime_service_t *service) {
    for (size_t i = 0; i < service->worker_capacity; i++) {
...
static void runtime_reject_inline(hyp_daemon_ipc_connection_t *connection, const char *message) {
    hyp_daemon_runtime_connect_result_t result;
    ...
    hyp_daemon_ipc_connection_drain(connection, RUNTIME_REJECT_DRAIN_TIMEOUT_MS);
    hyp_daemon_ipc_connection_close(connection);
}

static void runtime_accept_connection(hyp_daemon_runtime_service_t *service,
                                      hyp_daemon_ipc_connection_t *connection) {
    uint64_t peer_pid = hyp_daemon_ipc_connection_peer_pid(connection);
    if (peer_pid == 0) {
        hyp_daemon_ipc_connection_close(connection);
        return;
    }
    ...
    hyp_daemon_runtime_worker_t *worker =
    ...
    if (worker) {
        ...
        atomic_store_explicit(&worker->application_thread_done, false, memory_order_release);
        service->active_connections++;
        int created = hyp_thread_create(&worker->thread, RUNTIME_WORKER_STACK_SIZE,
    ...
    if (!running) {
        runtime_reject_inline(connection, "HYP daemon is stopping");
    } else {
        runtime_reject_inline(connection, "HYP daemon connection capacity reached");
    }
}

static bool runtime_service_can_exit(hyp_daemon_runtime_service_t *service, uint64_t now_ms) {
    bool can_exit = false;
    hyp_mutex_lock(&service->mutex);
    if (service->state == HYP_DAEMON_RUNTIME_SERVICE_STOPPING && service->active_connections == 0) {
        bool drained = hyp_daemon_should_exit(service->coordinator, now_ms);
        if (!drained && service->activation_shutdown_requested) {
            ...
            drained = hyp_daemon_active_clients(service->coordinator) == 0 &&
                      hyp_daemon_active_jobs(service->coordinator) == 0 &&
                      hyp_daemon_active_watches(service->coordinator) == 0;
        }
        bool deadline_reached =
            service->stop_deadline_ms != 0 && now_ms >= service->stop_deadline_ms;
        can_exit = drained || deadline_reached || service->emergency_stop;
        if (can_exit) {
            service->state = HYP_DAEMON_RUNTIME_SERVICE_EXITED;
        }
    }
    hyp_mutex_unlock(&service->mutex);
    return can_exit;
...
static void *runtime_accept_loop(void *opaque) {
    hyp_daemon_runtime_service_t *service = opaque;
    for (;;) {
        runtime_reap_completed_workers(service);
        hyp_mutex_lock(&service->mutex);
        hyp_daemon_runtime_service_state_t state = service->state;
        hyp_mutex_unlock(&service->mutex);
        if (state == HYP_DAEMON_RUNTIME_SERVICE_EXITED) {
            break;
        }
        if (state == HYP_DAEMON_RUNTIME_SERVICE_STOPPING) {
            runtime_service_interrupt_connections(service);
            if (runtime_service_can_exit(service, hyp_now_ms())) {
                break;
            }
        ...
        hyp_daemon_ipc_connection_t *connection = NULL;
        int accepted =
            hyp_daemon_ipc_accept(service->listener, RUNTIME_ACCEPT_POLL_MS, &connection);
...
static bool runtime_application_callbacks_valid(
    const hyp_daemon_runtime_application_callbacks_t *application) {
...
static bool runtime_participant_guard_release_complete(
    hyp_daemon_ipc_participant_guard_t **guard_io, uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    while (guard_io && *guard_io) {
        (void)hyp_daemon_ipc_participant_guard_release(guard_io);
        if (!*guard_io) {
            return true;
        }
...
static _Noreturn void runtime_cleanup_fail_stop(const char *component) {
    ...
    hyp_log_error("daemon.forced_shutdown", "component", component);
    (void)fflush(stdout);
    (void)fflush(stderr);
#ifdef _WIN32
...
static void runtime_startup_lock_release_complete(hyp_daemon_ipc_startup_lock_t **lock_io,
                                                  uint32_t timeout_ms) {
    uint64_t deadline = runtime_deadline_after(timeout_ms);
...
static void runtime_service_destroy_unstarted(hyp_daemon_runtime_service_t *service) {
    if (!service) {
        return;
    }
    hyp_daemon_ipc_listener_close(service->listener);
    hyp_daemon_coordinator_free(service->coordinator);
    ...
    for (size_t i = 0; i < service->worker_mutexes_initialized; i++) {
        hyp_mutex_destroy(&service->workers[i].send_mutex);
    }
    free(service->workers);
    hyp_mutex_destroy(&service->mutex);
    free(service);
}

hyp_daemon_runtime_service_t *hyp_daemon_runtime_service_start_reserved(
    const hyp_daemon_runtime_service_config_t *config,
    hyp_daemon_ipc_lifetime_reservation_t **reservation_io) {
    uint8_t validation[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    char active_process_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE];
    runtime_process_image_reference_t active_image;
    ...
    if (!reservation_io || !*reservation_io || !config || !config->endpoint ||
        ...
        config->request_timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER ||
        config->shutdown_timeout_ms == 0 ||
        config->shutdown_timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_application_callbacks_valid(&config->application) ||
        !hyp_daemon_runtime_hello_request_encode(validation, &config->identity)) {
        hyp_log_error("daemon.runtime.start_failed", "stage", "config_validation");
        return NULL;
    }
    ...
    if (!runtime_process_image_reference_acquire(runtime_current_process_id(), &active_image,
        ...
        hyp_log_error("daemon.runtime.start_failed", "stage", "active_image_identity");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }

    hyp_daemon_runtime_service_t *service = calloc(1, sizeof(*service));
    if (!service) {
        hyp_log_error("daemon.runtime.start_failed", "stage", "service_allocation");
        (void)runtime_process_image_reference_release(&active_image);
        return NULL;
    }
    service->active_image = active_image;
    runtime_process_image_reference_init(&active_image);
    hyp_mutex_init(&service->mutex);
    ...
    service->coordinator = hyp_daemon_coordinator_new(config->lease_timeout_ms);
    if (service->coordinator && config->permanent) {
        hyp_daemon_coordinator_set_permanent(service->coordinator, true);
    }
    ...
    service->state = HYP_DAEMON_RUNTIME_SERVICE_STARTING;

    if (!service->workers || !service->coordinator || !service->conflict_log_path ||
        !copied_identity) {
        hyp_log_error("daemon.runtime.start_failed", "stage", "service_initialization");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    for (size_t i = 0; i < service->worker_capacity; i++) {
        service->workers[i].service = service;
        hyp_mutex_init(&service->workers[i].send_mutex);
    ...
    service->listener = hyp_daemon_ipc_listen_reserved(config->endpoint, reservation_io);
    if (!service->listener) {
        hyp_log_error("daemon.runtime.start_failed", "stage", "listener_handoff");
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
    service->state = HYP_DAEMON_RUNTIME_SERVICE_RUNNING;
    if (hyp_thread_create(&service->accept_thread, RUNTIME_WORKER_STACK_SIZE, runtime_accept_loop,
                          service) != 0) {
        hyp_log_error("daemon.runtime.start_failed", "stage", "accept_thread");
        service->state = HYP_DAEMON_RUNTIME_SERVICE_EXITED;
        runtime_service_destroy_unstarted(service);
        return NULL;
    }
...
hyp_daemon_runtime_service_t *hyp_daemon_runtime_service_start(
    const hyp_daemon_runtime_service_config_t *config) {
    if (!config || !config->endpoint) {
        return NULL;
    }
    hyp_daemon_ipc_startup_lock_t *startup_lock = NULL;
    if (hyp_daemon_ipc_startup_lock_try_acquire(config->endpoint, &startup_lock) != 1) {
        return NULL;
    }
    int generation =
        hyp_daemon_ipc_generation_probe_under_startup_lock(config->endpoint, startup_lock);
    if (generation != 0 || !hyp_daemon_ipc_startup_lock_prepare_handoff(startup_lock)) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    hyp_daemon_ipc_participant_guard_t *participant_guard = NULL;
    if (hyp_daemon_ipc_participant_guard_try_join(config->endpoint, &participant_guard) != 1) {
        runtime_startup_lock_release_complete(&startup_lock, config->shutdown_timeout_ms);
        return NULL;
    }
    hyp_daemon_ipc_lifetime_reservation_t *reservation = NULL;
    if (hyp_daemon_ipc_lifetime_reservation_try_acquire(config->endpoint, &reservation) != 1) {
    ...
    hyp_daemon_runtime_service_t *service =
        hyp_daemon_runtime_service_start_reserved(config, &reservation);
...
hyp_daemon_runtime_service_state_t hyp_daemon_runtime_service_state(
    hyp_daemon_runtime_service_t *service) {
    if (!service) {
        return HYP_DAEMON_RUNTIME_SERVICE_EXITED;
    }
    hyp_mutex_lock(&service->mutex);
    hyp_daemon_runtime_service_state_t state = service->state;
    hyp_mutex_unlock(&service->mutex);
...
uint64_t hyp_daemon_runtime_service_clients_admitted_total(hyp_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    hyp_mutex_lock(&service->mutex);
...
size_t hyp_daemon_runtime_service_active_clients(hyp_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    hyp_mutex_lock(&service->mutex);
...
size_t hyp_daemon_runtime_service_active_connections(hyp_daemon_runtime_service_t *service) {
    if (!service) {
        return 0;
    }
    hyp_mutex_lock(&service->mutex);
...
size_t hyp_daemon_runtime_service_job_subscribers(hyp_daemon_runtime_service_t *service,
                                                  const char *project_key) {
    return service ? hyp_daemon_job_subscribers(service->coordinator, project_key) : 0;
}

uint64_t hyp_daemon_runtime_service_client_process_id(hyp_daemon_runtime_service_t *service,
                                                      hyp_daemon_client_id_t client_id) {
    if (!service || client_id == HYP_DAEMON_CLIENT_ID_INVALID) {
        return 0;
    }
...
static bool runtime_wait_for_count(hyp_daemon_runtime_service_t *service, size_t expected,
    ...
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        size_t actual = connections ? hyp_daemon_runtime_service_active_connections(service)
                                    : hyp_daemon_runtime_service_active_clients(service);
        if (actual == expected) {
            return true;
        }
        if (hyp_now_ms() >= deadline) {
...
bool hyp_daemon_runtime_service_wait_for_clients(hyp_daemon_runtime_service_t *service,
                                                 size_t expected, uint32_t timeout_ms) {
    return runtime_wait_for_count(service, expected, timeout_ms, false);
}

bool hyp_daemon_runtime_service_wait_for_connections(hyp_daemon_runtime_service_t *service,
                                                     size_t expected, uint32_t timeout_ms) {
    return runtime_wait_for_count(service, expected, timeout_ms, true);
}

bool hyp_daemon_runtime_service_wait_exited(hyp_daemon_runtime_service_t *service,
                                            uint32_t timeout_ms) {
    if (!service) {
        return false;
    }
    uint64_t deadline = runtime_deadline_after(timeout_ms);
    for (;;) {
        if (hyp_daemon_runtime_service_state(service) == HYP_DAEMON_RUNTIME_SERVICE_EXITED &&
...
bool hyp_daemon_runtime_service_job_reaped(hyp_daemon_runtime_service_t *service,
                                           const char *project_key) {
    return service && hyp_daemon_job_reaped(service->coordinator, project_key, hyp_now_ms());
}

bool hyp_daemon_runtime_service_stop(hyp_daemon_runtime_service_t *service, uint32_t timeout_ms) {
    if (!service) {
        return false;
    }
    if (hyp_daemon_runtime_service_state(service) == HYP_DAEMON_RUNTIME_SERVICE_EXITED) {
        return hyp_daemon_runtime_service_wait_exited(service, timeout_ms);
    }
...
bool hyp_daemon_runtime_service_free(hyp_daemon_runtime_service_t *service) {
    if (!service ||
        hyp_daemon_runtime_service_state(service) != HYP_DAEMON_RUNTIME_SERVICE_EXITED) {
        return false;
    }
    if (service->accept_thread_started && !service->accept_thread_joined) {
        if (hyp_thread_join(&service->accept_thread) != 0) {
            return false;
        }
    ...
    for (size_t i = 0; i < service->worker_capacity; i++) {
        ...
        if (worker->thread_started) {
            if (!atomic_load_explicit(&worker->done, memory_order_acquire) ||
                hyp_thread_join(&worker->thread) != 0) {
                return false;
            }
            runtime_worker_reset_after_join(worker);
        }
...
bool hyp_daemon_runtime_request_activation_shutdown(
    const hyp_daemon_ipc_endpoint_t *endpoint, const hyp_daemon_build_identity_t *identity,
    hyp_daemon_runtime_activation_action_t action, uint32_t timeout_ms,
    hyp_daemon_runtime_activation_result_t *result_out) {
    if (result_out) {
        memset(result_out, 0, sizeof(*result_out));
    }
    uint8_t request[HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER ||
        !runtime_activation_request_encode(request, action, identity)) {
        return false;
    }
    hyp_daemon_ipc_connection_t *connection = hyp_daemon_ipc_connect(endpoint, timeout_ms);
    bool sent = connection && hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                                        HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN,
                                                        request, (uint32_t)sizeof(request));
    hyp_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? hyp_daemon_ipc_receive_frame_bounded(
                              connection, timeout_ms, HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE,
                              &frame, &payload)
                        : 0;
    bool valid = received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                 frame.flags == HYP_DAEMON_RUNTIME_OP_ACTIVATION_SHUTDOWN &&
                 frame.length == HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE &&
...
static bool runtime_control_request_send(const hyp_daemon_ipc_endpoint_t *endpoint,
                                         const hyp_daemon_build_identity_t *identity,
                                         hyp_daemon_runtime_operation_t operation,
                                         uint32_t timeout_ms, uint32_t response_size,
                                         uint8_t **payload_out) {
    *payload_out = NULL;
    if (!endpoint || !identity || !identity->build_fingerprint ||
        timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    uint8_t request[HYP_DAEMON_CONTROL_REQUEST_SIZE];
    memset(request, 0, sizeof(request));
    int written = snprintf((char *)request, sizeof(request), "%s", identity->build_fingerprint);
    if (written <= 0 || (size_t)written != HYP_DAEMON_BUILD_FINGERPRINT_SIZE - 1U) {
        return false;
    }
    hyp_daemon_ipc_connection_t *connection = hyp_daemon_ipc_connect(endpoint, timeout_ms);
    bool sent =
        connection && hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST, operation,
                                                request, (uint32_t)sizeof(request));
    hyp_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received = sent ? hyp_daemon_ipc_receive_frame_bounded(connection, timeout_ms,
...
bool hyp_daemon_runtime_request_status(const hyp_daemon_ipc_endpoint_t *endpoint,
                                       const hyp_daemon_build_identity_t *identity,
                                       uint32_t timeout_ms,
                                       hyp_daemon_runtime_status_t *status_out) {
    if (!status_out) {
        return false;
    }
    memset(status_out, 0, sizeof(*status_out));
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, HYP_DAEMON_RUNTIME_OP_STATUS, timeout_ms,
                                      HYP_DAEMON_STATUS_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    status_out->client_count = payload[8] > HYP_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)HYP_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[8];
    for (size_t index = 0; index < HYP_DAEMON_CONTROL_CLIENT_CAP; index++) {
        status_out->client_pids[index] = runtime_control_read_u32(payload + 12U + index * 4U);
    }
    memcpy(status_out->build_fingerprint, payload + 44U, HYP_DAEMON_BUILD_FINGERPRINT_SIZE);
    status_out->build_fingerprint[HYP_DAEMON_BUILD_FINGERPRINT_SIZE - 1U] = '\0';
...
bool hyp_daemon_runtime_request_stop(const hyp_daemon_ipc_endpoint_t *endpoint,
                                     const hyp_daemon_build_identity_t *identity,
                                     uint32_t timeout_ms,
                                     hyp_daemon_runtime_stop_result_t *result_out) {
    if (!result_out) {
        return false;
    }
    memset(result_out, 0, sizeof(*result_out));
    uint8_t *payload = NULL;
    if (!runtime_control_request_send(endpoint, identity, HYP_DAEMON_RUNTIME_OP_STOP, timeout_ms,
                                      HYP_DAEMON_STOP_RESPONSE_SIZE, &payload)) {
        return false;
    }
    ...
    result_out->client_count = payload[4] > HYP_DAEMON_CONTROL_CLIENT_CAP
                                   ? (uint8_t)HYP_DAEMON_CONTROL_CLIENT_CAP
                                   : payload[4];
    for (size_t index = 0; index < HYP_DAEMON_CONTROL_CLIENT_CAP; index++) {
...
hyp_daemon_runtime_client_t *hyp_daemon_runtime_client_connect(
    const hyp_daemon_ipc_endpoint_t *endpoint, const hyp_daemon_build_identity_t *identity,
    uint32_t timeout_ms, hyp_daemon_runtime_connect_result_t *result_out) {
    if (result_out) {
        memset(result_out, 0, sizeof(*result_out));
    }
    uint8_t request[HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE];
    if (!endpoint || !identity || !result_out || timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER ||
        !hyp_daemon_runtime_hello_request_encode(request, identity)) {
        return NULL;
    }
    hyp_daemon_ipc_connection_t *connection = hyp_daemon_ipc_connect(endpoint, timeout_ms);
    if (!connection || !hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                                  HYP_DAEMON_RUNTIME_OP_HELLO, request,
    ...
    uint8_t *payload = NULL;
    int received = hyp_daemon_ipc_receive_frame_bounded(
        connection, timeout_ms, HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE, &frame, &payload);
    bool valid = received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                 frame.flags == HYP_DAEMON_RUNTIME_OP_HELLO &&
                 frame.length == HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE &&
                 runtime_hello_response_decode(payload, result_out);
    free(payload);
    if (!valid || result_out->status != HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED) {
        hyp_daemon_ipc_connection_close(connection);
        return NULL;
    }

    hyp_daemon_runtime_client_t *client = calloc(1, sizeof(*client));
    if (!client) {
        hyp_daemon_ipc_connection_close(connection);
        memset(result_out, 0, sizeof(*result_out));
        return NULL;
    }
    hyp_mutex_init(&client->exchange_mutex);
    hyp_mutex_init(&client->send_mutex);
    hyp_mutex_init(&client->state_mutex);
...
hyp_daemon_client_id_t hyp_daemon_runtime_client_id(const hyp_daemon_runtime_client_t *client) {
    return client ? client->client_id : HYP_DAEMON_CLIENT_ID_INVALID;
}

uint64_t hyp_daemon_runtime_client_process_id(const hyp_daemon_runtime_client_t *client) {
    return client ? client->authenticated_process_id : 0;
}

static bool runtime_client_exchange(hyp_daemon_runtime_client_t *client,
                                    hyp_daemon_runtime_operation_t operation, const void *request,
                                    uint32_t request_length, uint32_t timeout_ms,
                                    uint8_t **response_out, uint32_t expected_response_length) {
    *response_out = NULL;
    if (timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER) {
        return false;
    }
    hyp_mutex_lock(&client->exchange_mutex);
    hyp_mutex_lock(&client->state_mutex);
    if (!client->usable || client->closing || !client->connection) {
        hyp_mutex_unlock(&client->state_mutex);
        hyp_mutex_unlock(&client->exchange_mutex);
        return false;
    }
    hyp_daemon_ipc_connection_t *connection = client->connection;
    client->exchange_active = true;
    hyp_mutex_unlock(&client->state_mutex);

    hyp_mutex_lock(&client->send_mutex);
    bool sent = hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST, (uint16_t)operation,
                                          request, request_length);
    hyp_mutex_unlock(&client->send_mutex);
    hyp_daemon_frame_t frame = {0};
    uint8_t *payload = NULL;
    int received =
        sent ? hyp_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool valid = sent && received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
...
hyp_daemon_subscription_result_t hyp_daemon_runtime_client_job_subscribe(
    hyp_daemon_runtime_client_t *client, const char *project_key,
    hyp_daemon_subscription_id_t *subscription_id_out, uint32_t timeout_ms) {
    if (subscription_id_out) {
        *subscription_id_out = HYP_DAEMON_SUBSCRIPTION_ID_INVALID;
    }
    size_t key_length = 0;
    if (!client || !subscription_id_out ||
        !runtime_bounded_length(project_key, HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX + 1, &key_length) ||
        key_length == 0 || key_length > HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX) {
        return HYP_DAEMON_SUBSCRIPTION_REJECTED;
    }
    uint8_t request[SUBSCRIBE_REQUEST_PREFIX_SIZE + HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX];
    runtime_put_u32(request, (uint32_t)key_length);
    memcpy(request + SUBSCRIBE_REQUEST_PREFIX_SIZE, project_key, key_length);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, HYP_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE, request,
                                 (uint32_t)(SUBSCRIBE_REQUEST_PREFIX_SIZE + key_length), timeout_ms,
                                 &response, SUBSCRIBE_RESPONSE_SIZE)) {
        return HYP_DAEMON_SUBSCRIPTION_REJECTED;
    }
    hyp_daemon_subscription_result_t result =
        (hyp_daemon_subscription_result_t)runtime_get_u32(response);
    hyp_daemon_subscription_id_t subscription_id = runtime_get_u64(response + 4);
    free(response);
    if ((result != HYP_DAEMON_SUBSCRIPTION_STARTED && result != HYP_DAEMON_SUBSCRIPTION_JOINED) ||
...
bool hyp_daemon_runtime_client_job_unsubscribe(hyp_daemon_runtime_client_t *client,
                                               hyp_daemon_subscription_id_t subscription_id,
                                               uint32_t timeout_ms) {
    if (!client || subscription_id == HYP_DAEMON_SUBSCRIPTION_ID_INVALID) {
        return false;
    }
    uint8_t request[UNSUBSCRIBE_REQUEST_SIZE];
    runtime_put_u64(request, subscription_id);
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, HYP_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE, request,
...
bool hyp_daemon_runtime_client_heartbeat(hyp_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    if (!client) {
        return false;
    }
    uint8_t *response = NULL;
    if (!runtime_client_exchange(client, HYP_DAEMON_RUNTIME_OP_HEARTBEAT, NULL, 0, timeout_ms,
...
bool hyp_daemon_runtime_client_application_token_reserve(
    hyp_daemon_runtime_client_t *client, hyp_daemon_runtime_application_token_t *token_out) {
    if (token_out) {
        *token_out = HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    if (!client || !token_out) {
        return false;
    }
    ...
    bool valid = client->usable && !client->closing && client->connection &&
                 client->active_application_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
...
hyp_daemon_runtime_cancel_result_t hyp_daemon_runtime_client_application_cancel(
    hyp_daemon_runtime_client_t *client, hyp_daemon_runtime_application_token_t request_token) {
    if (!client || request_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID) {
        return HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    ...
    if (client->active_application_token == request_token) {
    ...
    } else if (client->active_application_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID &&
        ...
        hyp_mutex_unlock(&client->state_mutex);
        return HYP_DAEMON_RUNTIME_CANCEL_STALE;
    }
    hyp_mutex_unlock(&client->state_mutex);
    if (!dispatch) {
        return HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED;
    }
    ...
    hyp_mutex_unlock(&client->state_mutex);
    bool sent = !dispatch || hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                                       HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL,
    ...
    if (!dispatch) {
        ...
        return already_sent ? HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED
                            : (unavailable ? HYP_DAEMON_RUNTIME_CANCEL_ERROR
                                           : HYP_DAEMON_RUNTIME_CANCEL_STALE);
    }
    if (!sent) {
        hyp_mutex_lock(&client->state_mutex);
        client->usable = false;
        hyp_mutex_unlock(&client->state_mutex);
        hyp_daemon_ipc_connection_interrupt(connection);
        return HYP_DAEMON_RUNTIME_CANCEL_ERROR;
    }
    return HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED;
}

hyp_daemon_runtime_application_status_t hyp_daemon_runtime_client_application_request_tagged(
    hyp_daemon_runtime_client_t *client, hyp_daemon_runtime_application_token_t request_token,
    ...
    if (!client || !response_out || !response_length_out ||
        request_token == HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID ||
        timeout_ms == HYP_DAEMON_IPC_WAIT_FOREVER ||
        request_length > HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX ||
    ...
    if (client->pending_cancel_token != request_token) {
        client->pending_cancel_token = HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    }
    hyp_mutex_unlock(&client->state_mutex);
    ...
    hyp_mutex_unlock(&client->state_mutex);
    bool sent =
        request_can_send && hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                                      HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST,
    ...
    hyp_mutex_unlock(&client->state_mutex);
    if (send_cancel) {
        sent = hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                         HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, cancel_wire,
    ...
    int received =
        sent ? hyp_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &payload) : -1;
    bool protocol_valid = sent && received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                          frame.flags == HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST &&
                          frame.length >= APPLICATION_RESPONSE_PREFIX_SIZE && payload;
    hyp_daemon_runtime_application_status_t status = HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    uint32_t response_length = 0;
    if (protocol_valid) {
        hyp_daemon_runtime_application_token_t response_token = runtime_get_u64(payload);
        status = (hyp_daemon_runtime_application_status_t)runtime_get_u32(payload + 8);
        response_length = runtime_get_u32(payload + 12);
        protocol_valid = response_token == request_token &&
                         status >= HYP_DAEMON_RUNTIME_APPLICATION_OK &&
                         status <= HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED &&
                         response_length <= HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX &&
                         frame.length == APPLICATION_RESPONSE_PREFIX_SIZE + response_length &&
                         (status == HYP_DAEMON_RUNTIME_APPLICATION_OK || response_length == 0);
    }
...
hyp_daemon_runtime_application_status_t hyp_daemon_runtime_client_application_request(
    hyp_daemon_runtime_client_t *client, const void *request, uint32_t request_length,
    ...
    hyp_daemon_runtime_application_token_t request_token =
        HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (!hyp_daemon_runtime_client_application_token_reserve(client, &request_token)) {
        return HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR;
    }
    return hyp_daemon_runtime_client_application_request_tagged(client, request_token, request,
                                                                request_length, response_out,
                                                                response_length_out, timeout_ms);
}

bool hyp_daemon_runtime_client_close_begin(hyp_daemon_runtime_client_t *client) {
    if (!client) {
        return false;
    }
    ...
    hyp_daemon_ipc_connection_t *connection = client->connection;
    hyp_daemon_runtime_application_token_t cancel_token = client->active_application_token;
    bool send_cancel = client->close_interrupted_exchange && connection &&
                       client->application_request_sent && !client->application_cancel_sent &&
                       cancel_token != HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID;
    if (send_cancel) {
        client->application_cancel_sent = true;
    }
    hyp_mutex_unlock(&client->state_mutex);
    ...
    if (send_cancel) {
        ...
        (void)hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                        HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL, wire,
    ...
    if (client->close_interrupted_exchange && connection) {
        ...
        (void)hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                        HYP_DAEMON_RUNTIME_OP_CLOSE_INTENT, NULL, 0);
        hyp_daemon_ipc_connection_interrupt(connection);
    }
    hyp_mutex_unlock(&client->send_mutex);
    return true;
...
bool hyp_daemon_runtime_client_close_finish(hyp_daemon_runtime_client_t *client,
                                            uint32_t timeout_ms) {
    if (!client) {
        return false;
    }
    ...
    hyp_mutex_lock(&client->state_mutex);
    bool can_disconnect = timeout_ms != HYP_DAEMON_IPC_WAIT_FOREVER &&
    ...
    if (can_disconnect) {
        hyp_mutex_lock(&client->send_mutex);
        disconnect_sent = hyp_daemon_ipc_send_frame(connection, HYP_DAEMON_FRAME_REQUEST,
                                                    HYP_DAEMON_RUNTIME_OP_DISCONNECT, NULL, 0);
        hyp_mutex_unlock(&client->send_mutex);
    }
    if (disconnect_sent) {
        hyp_daemon_frame_t frame = {0};
        uint8_t *response = NULL;
        int received = hyp_daemon_ipc_receive_frame(connection, timeout_ms, &frame, &response);
        acknowledged = received == 1 && frame.type == HYP_DAEMON_FRAME_RESPONSE &&
                       frame.flags == HYP_DAEMON_RUNTIME_OP_DISCONNECT &&
    ...
    hyp_daemon_ipc_connection_close(connection);
    hyp_mutex_unlock(&client->exchange_mutex);
    hyp_mutex_destroy(&client->state_mutex);
    hyp_mutex_destroy(&client->send_mutex);
    hyp_mutex_destroy(&client->exchange_mutex);
    free(client);
    return acknowledged;
}

bool hyp_daemon_runtime_client_close(hyp_daemon_runtime_client_t *client, uint32_t timeout_ms) {
    if (!hyp_daemon_runtime_client_close_begin(client)) {
        return false;
    }
    return hyp_daemon_runtime_client_close_finish(client, timeout_ms);
}
