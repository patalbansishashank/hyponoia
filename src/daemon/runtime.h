/*
 * runtime.h — Mandatory per-account HYP daemon runtime.
 *
 ...
 */
#ifndef HYP_DAEMON_RUNTIME_H
#define HYP_DAEMON_RUNTIME_H
...
#define HYP_DAEMON_RUNTIME_WIRE_ABI 1U
...
#define HYP_DAEMON_RENDEZVOUS_ABI 1U
#define HYP_DAEMON_RENDEZVOUS_VERSION_TEXT_CAP 64U
#define HYP_DAEMON_RENDEZVOUS_BUILD_FINGERPRINT_CAP 65U
#define HYP_DAEMON_RENDEZVOUS_MESSAGE_CAP 512U
#define HYP_DAEMON_RENDEZVOUS_REQUEST_SIZE 133U
#define HYP_DAEMON_RENDEZVOUS_RESPONSE_SIZE 798U
...
#define HYP_DAEMON_ACTIVATION_SHUTDOWN_REQUEST_SIZE 137U
#define HYP_DAEMON_ACTIVATION_SHUTDOWN_RESPONSE_SIZE 24U
...
#define HYP_DAEMON_CONTROL_REQUEST_SIZE 65U
#define HYP_DAEMON_CONTROL_CLIENT_CAP 8U
#define HYP_DAEMON_STATUS_RESPONSE_SIZE 121U
#define HYP_DAEMON_STOP_RESPONSE_SIZE 40U
#define HYP_DAEMON_RUNTIME_PROJECT_KEY_MAX 4096U
#define HYP_DAEMON_RUNTIME_APPLICATION_PAYLOAD_MAX (HYP_DAEMON_MAX_FRAME_SIZE - 16U)
...
typedef enum {
    ...
    HYP_DAEMON_RUNTIME_OP_HEARTBEAT = 2,
    HYP_DAEMON_RUNTIME_OP_JOB_SUBSCRIBE = 3,
    HYP_DAEMON_RUNTIME_OP_JOB_UNSUBSCRIBE = 4,
    HYP_DAEMON_RUNTIME_OP_DISCONNECT = 5,
    HYP_DAEMON_RUNTIME_OP_APPLICATION_REQUEST = 6,
    HYP_DAEMON_RUNTIME_OP_APPLICATION_CANCEL = 7,
    ...
    HYP_DAEMON_RUNTIME_OP_CLOSE_INTENT = 9,
    ...
    HYP_DAEMON_RUNTIME_OP_STATUS = 10,
    ...
    HYP_DAEMON_RUNTIME_OP_STOP = 11,
} hyp_daemon_runtime_operation_t;

typedef enum {
    HYP_DAEMON_RUNTIME_ACTIVATION_INSTALL = 1,
    HYP_DAEMON_RUNTIME_ACTIVATION_UPDATE = 2,
    HYP_DAEMON_RUNTIME_ACTIVATION_UNINSTALL = 3,
} hyp_daemon_runtime_activation_action_t;

typedef struct {
    bool accepted;
    uint64_t active_clients;
    uint64_t active_connections;
} hyp_daemon_runtime_activation_result_t;

typedef struct hyp_daemon_runtime_service hyp_daemon_runtime_service_t;
typedef struct hyp_daemon_runtime_client hyp_daemon_runtime_client_t;

typedef enum {
    HYP_DAEMON_RUNTIME_SERVICE_STARTING = 1,
    HYP_DAEMON_RUNTIME_SERVICE_RUNNING = 2,
    HYP_DAEMON_RUNTIME_SERVICE_STOPPING = 3,
    HYP_DAEMON_RUNTIME_SERVICE_EXITED = 4,
} hyp_daemon_runtime_service_state_t;
...
typedef enum {
    HYP_DAEMON_RUNTIME_APPLICATION_TRANSPORT_ERROR = 0,
    HYP_DAEMON_RUNTIME_APPLICATION_OK = 1,
    HYP_DAEMON_RUNTIME_APPLICATION_BUSY = 2,
    HYP_DAEMON_RUNTIME_APPLICATION_UNAVAILABLE = 3,
    HYP_DAEMON_RUNTIME_APPLICATION_REJECTED = 4,
    HYP_DAEMON_RUNTIME_APPLICATION_HANDLER_ERROR = 5,
    HYP_DAEMON_RUNTIME_APPLICATION_CANCELLED = 6,
} hyp_daemon_runtime_application_status_t;

typedef uint64_t hyp_daemon_runtime_application_token_t;
#define HYP_DAEMON_RUNTIME_APPLICATION_TOKEN_INVALID UINT64_C(0)

typedef enum {
    HYP_DAEMON_RUNTIME_CANCEL_ERROR = -1,
    HYP_DAEMON_RUNTIME_CANCEL_STALE = 0,
    HYP_DAEMON_RUNTIME_CANCEL_ACCEPTED = 1,
} hyp_daemon_runtime_cancel_result_t;

typedef void hyp_daemon_runtime_application_session_t;

typedef hyp_daemon_runtime_application_session_t *(*hyp_daemon_runtime_application_session_open_fn)(
...
typedef hyp_daemon_runtime_application_status_t (*hyp_daemon_runtime_application_request_fn)(
    void *context, hyp_daemon_runtime_application_session_t *session,
...
typedef void (*hyp_daemon_runtime_application_request_cancel_fn)(
    void *context, hyp_daemon_runtime_application_session_t *session,
...
typedef void (*hyp_daemon_runtime_application_session_cancel_fn)(
    void *context, hyp_daemon_runtime_application_session_t *session);
typedef void (*hyp_daemon_runtime_application_session_close_fn)(
    void *context, hyp_daemon_runtime_application_session_t *session);

typedef struct {
    void *context;
    hyp_daemon_runtime_application_session_open_fn session_open;
    hyp_daemon_runtime_application_request_fn request;
    hyp_daemon_runtime_application_request_cancel_fn request_cancel;
    hyp_daemon_runtime_application_session_cancel_fn session_cancel;
    hyp_daemon_runtime_application_session_close_fn session_close;
} hyp_daemon_runtime_application_callbacks_t;
...
typedef struct {
    ...
    bool permanent;
} hyp_daemon_runtime_service_config_t;

typedef enum {
    HYP_DAEMON_RUNTIME_CONNECT_ERROR = 0,
    HYP_DAEMON_RUNTIME_CONNECT_ACCEPTED = 1,
    HYP_DAEMON_RUNTIME_CONNECT_CONFLICT = 2,
    HYP_DAEMON_RUNTIME_CONNECT_REJECTED = 3,
} hyp_daemon_runtime_connect_status_t;

typedef struct {
    hyp_daemon_runtime_connect_status_t status;
    hyp_daemon_hello_status_t hello_status;
...
} hyp_daemon_runtime_connect_result_t;
...
typedef struct {
    ...
    uint32_t client_pids[HYP_DAEMON_CONTROL_CLIENT_CAP];
} hyp_daemon_runtime_stop_result_t;
 ...
 * Client exchange timeouts must be finite, not HYP_DAEMON_IPC_WAIT_FOREVER. */
