...
int hyp_ask_vectors_fetch(hyp_ask_vectors_t *v, const char *const *qualified_names, int n,
    ...
    if (!v || !query || dim <= 0 || !out || !out_count) {
        av_err(v, "fetch: bad arguments");
        return HYP_ASK_VEC_ERR;
    }
    if (n <= 0 || !qualified_names) {
        return HYP_ASK_VEC_OK; /* nothing asked for is not an error */
    }
    hyp_ask_vec_meta_t m;
    int rc = hyp_ask_vectors_get_meta(v, &m);
    if (rc != HYP_ASK_VEC_OK) {
    ...
    /* Same gate as the scan: an unnormalised question turns "cosine" into a
     * number that still sorts, which is the silent-wrongness this lane exists
     * to avoid. */
    int bad = -1;
    if (!hyp_ask_vectors_are_unit(query, 1, dim, &bad)) {
        av_err(v, "fetch: the question vector is not unit-normalised, so its 'cosine' scores "
                  "would be silently wrong rather than loudly absent");
        return HYP_ASK_VEC_ERR;
    }
    ...
    const char *sql = "SELECT qualified_name, label, file_path, start_line, end_line, lang,"
                      ...
                      " WHERE qualified_name = ?1";
    if (sqlite3_prepare_v2(v->db, sql, -1, &st, NULL) != SQLITE_OK) {
        av_err_sqlite(v, "fetch prepare");
        return HYP_ASK_VEC_ERR;
    }
    hyp_ask_vec_hit_t *hits = calloc((size_t)n, sizeof(*hits));
    if (!hits) {
        sqlite3_finalize(st);
        av_err(v, "fetch: out of memory");
        return HYP_ASK_VEC_ERR;
    }
    int filled = 0;
    size_t want_bytes = (size_t)dim * sizeof(float);
    for (int i = 0; i < n; i++) {
        if (!qualified_names[i] || !qualified_names[i][0]) {
            continue;
        }
        sqlite3_reset(st);
        sqlite3_bind_text(st, 1, qualified_names[i], -1, SQLITE_STATIC);
        if (sqlite3_step(st) != SQLITE_ROW) {
            continue; /* not in the population — dropped, see the header */
        }
        const void *blob = sqlite3_column_blob(st, 8);
        int blob_len = sqlite3_column_bytes(st, 8);
        if (!blob || (size_t)blob_len != want_bytes) {
            hyp_log_warn("ask.vectors.bad_row", "qn", qualified_names[i], "bytes",
                         blob_len == 0 ? "0" : "mismatch");
            continue;
        }
        const float *row = (const float *)blob;
        ...
        hyp_ask_vec_hit_t *h = &hits[filled];
        h->qualified_name = av_strdup((const char *)sqlite3_column_text(st, 0));
        h->label = av_strdup((const char *)sqlite3_column_text(st, 1));
        h->file_path = av_strdup((const char *)sqlite3_column_text(st, 2));
        h->start_line = sqlite3_column_int(st, 3);
        h->end_line = sqlite3_column_int(st, 4);
        h->lang = av_strdup((const char *)sqlite3_column_text(st, 5));
        h->node_id = sqlite3_column_int64(st, 6);
        h->truncated = sqlite3_column_int(st, 7) != 0;
        h->score = (float)dot;
        filled++;
    }
    sqlite3_finalize(st);
